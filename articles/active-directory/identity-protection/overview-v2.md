...
幕後發生了哪些情況？ 嘗試從 Tor 瀏覽器登入的舉動因匿名 IP 位址，而觸發了 Azure AD 中的即時登入風險。 Azure AD 在處理要求時套用了 Identity Protection 所設定的登入風險原則，因為這名員工的登入風險層級達到閾值 (中級)。 由於這名員工先前已註冊了 MFA，因此能夠回應並通過 MFA 挑戰。 這名員工能夠成功通過 MFA 挑戰，即已向 Azure AD 表明他可能是合法的身分識別擁有者，因此其使用者風險層級並未提高。 

但如果這名員工不是嘗試登入的那個人呢？ 

1. 具有該員工之認證的惡意執行者嘗試從 Tor 瀏覽器登入該員工的 Exchange Online 帳戶，因為他們試圖隱藏其 IP 位址。 
