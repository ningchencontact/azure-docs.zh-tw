...
當使用者成功完成登入風險原則所觸發的 MFA 提示時, 它會將意見反應提供給來自合法使用者的身分識別保護。 因此, 觸發 MFA 提示的登入風險事件會自動關閉, 而 Identity Protection 會防止此事件造成使用者風險提升。 啟用登入風險原則可以在系統提示您進行 MFA 時, 允許使用者自行補救, 並在後續自動關閉相關聯的具風險登入, 藉此減少有風險的登入視圖中的 noisiness。
...
- 可觸發原則的登入風險層級：
   ...
   ![登入風險等級](./media/howto-configure-risk-policies/12.png)
