...
Azure AD 會偵測到可能遭盜用的身分識別之指示器的風險偵測。 透過設定風險原則，您可以定義對偵測結果的自動回應：

- 使用登入風險原則, 您可以設定在使用者登入期間偵測到的即時風險偵測回應。 
...
當使用者成功完成登入風險原則所觸發的 MFA 提示時, 它會將意見反應提供給來自合法使用者的身分識別保護。 因此, 觸發 MFA 提示的登入風險偵測會自動關閉, 而 Identity Protection 會防止此事件造成使用者風險提升。 啟用登入風險原則可以在系統提示您進行 MFA 時, 允許使用者自行補救, 並在後續自動關閉相關聯的具風險登入, 藉此減少有風險的登入視圖中的 noisiness。
...
- 您想要在符合登入風險層級時強制執行的存取類型：  
   ...
   ![存取權](./media/howto-configure-risk-policies/13.png)
...
- 您想要在符合登入風險層級時強制執行的存取類型：  
   ...
   ![存取權](./media/howto-configure-risk-policies/113.png)
...
![正在封鎖](./media/howto-configure-risk-policies/116.png)
