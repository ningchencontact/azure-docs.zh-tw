...
若要移除 Azure 資源受控識別 VM 擴充功能，請使用 [az vmss identity remove](/cli/azure/vmss/identity/#az-vmss-remove-identity) 命令從 VMSS 移除系統指派的受控識別：
...
2. 使用 [az identity create](/cli/azure/identity#az-identity-create)，建立使用者指派的受控識別。  `-g` 參數指定資源群組以在其中建立使用者指派的受控識別，而 `-n` 參數指定此資源群組的名稱。 請務必以您自己的值取代 `<RESOURCE GROUP>` 和 `<USER ASSIGNED IDENTITY NAME>` 參數的值：
...
1. 使用 [az identity create](/cli/azure/identity#az-identity-create)，建立使用者指派的受控識別。  `-g` 參數指定資源群組以在其中建立使用者指派的受控識別，而 `-n` 參數指定此資源群組的名稱。 請務必以您自己的值取代 `<RESOURCE GROUP>` 和 `<USER ASSIGNED IDENTITY NAME>` 參數的值：
