...
受控識別能夠讓 Azure 服務向支援 Azure AD 驗證的服務進行驗證，而不需要您程式碼中的認證資料。 
...
> [!NOTE]
> 將使用者指派的識別從受指派的任何資源中刪除，並不會移除參考。 若要使用 CURL 從虛擬機器移除使用者指派的受控識別，請參閱[從 Azure VM 移除使用者指派的識別](qs-configure-rest-vm.md#remove-a-user-assigned identity-from-an-azure-vm)。
...
如需如何使用 CURL 將使用者指派的識別指派給 Azure VM/VMSS 資訊的資訊，請參閱[使用 CURL 在 Azure VM 上設定受控識別](qs-configure-rest-vm.md#user-assigned-identity) (英文) 和[使用 CURL 在虛擬機器擴展集上設定受控識別](qs-configure-rest-vmss.md#user-assigned-identity) (英文)。
