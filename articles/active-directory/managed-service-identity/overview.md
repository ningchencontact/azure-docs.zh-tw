---
title: 受控服務識別 (適用於 Azure 資源)
description: 受控服務識別功能 (適用於 Azure 資源) 概觀。
...
Azure Active Directory (Azure AD) 中的受控服務識別功能可解決此問題。 這項功能會在 Azure AD 中將自動受控識別提供給 Azure 服務。 您可以使用此身分識別來完成任何支援 Azure AD 驗證的服務驗證 (包括 Key Vault)，不需要您程式碼中的任何認證。

受控服務識別功能是免費的，適用於 Azure 訂用帳戶的 Azure AD。 沒有任何額外成本。

## 此功能如何運作？<a name="how-does-it-work"></a>

受控身分識別有兩種：

- **系統指派的身分識別**會直接在 Azure 服務執行個體上啟用。 啟用此身分識別時，Azure 會在執行個體的訂用帳戶所信任的 Azure AD 租用戶中，建立執行個體的身分識別。 建立身分識別後，就會將認證佈建到執行個體。 系統指派的身分識別生命週期會直接繫結至已啟用該 MSI 的 Azure 服務執行個體。 如果執行個體已刪除，則 Azure 會自動清除 Azure AD 中的認證和身分識別。
...
您的程式碼可以使用受控服務識別來要求存取權杖，以便存取支援 Azure AD 驗證的服務。 Azure 會負責更新服務個體使用的認證。

下圖顯示受控服務識別與 Azure 虛擬機器一起運作的方式。

![受控服務識別與 Azure VM](media/overview/msi-vm-vmextension-imds-example.png)

### <a name="how-a-system-assigned-identity-works-with-an-azure-vm"></a>系統指派的身分識別如何與 Azure VM 一起運作
...
3. Azure Resource Manager 會在 VM 上設定身分識別：
    ...
    1. 佈建 VM 擴充功能，並新增服務主體用戶端識別碼和憑證。 (此步驟已規劃要淘汰。)
...
5. 您在 VM 上執行的程式碼可以向僅可從 VM 內存取的兩個端點要求權杖：
    ...
    - VM 擴充端點 (已規劃要淘汰)：`http://localhost:50342/oauth2/token` 
        - resource 參數指定將權杖傳送至哪個服務。 若要向 Azure Resource Manager 進行驗證，請使用 `resource=https://management.azure.com/`。
...
### <a name="how-a-user-assigned-identity-works-with-an-azure-vm"></a>使用者指派的身分識別如何與 Azure VM 一起運作
...
3. Azure Resource Manager 收到一個要求，要在 VM 上設定使用者指派的身分識別：
    1. 使用使用者指派的身分識別服務主體用戶端識別碼和憑證，來更新 Azure Instance Metadata Service 身分識別端點。
    1. 佈建 VM 擴充功能，並新增使用者指派的識別服務主體用戶端識別碼和憑證。 (此步驟已規劃要淘汰。)
4. 建立使用者指派的身分識別後，使用服務主體資訊對其授與 Azure 資源的存取權。 若要呼叫 Azure Resource Manager，請在 Azure AD 中 RBAC，將適當的角色指派給使用者所指派身分識別的服務主體。 若要呼叫 Key Vault，請將 Key Vault 中特定祕密或金鑰的存取權授與您的程式碼。
...
## <a name="how-can-i-use-managed-service-identities"></a>如何使用受控服務識別？

若要了解如何使用受控服務識別來存取不同的 Azure 資源，請嘗試下列教學課程。

了解如何使用受控服務識別搭配 Windows VM：
...
## 哪些 Azure 服務支援此功能？<a name="which-azure-services-support-managed-service-identity"></a>

受控服務識別可用來向支援 Azure AD 驗證的服務進行驗證。 如需支援受控服務識別功能的 Azure 服務清單，請參閱[支援受控服務識別的服務](services-support-msi.md)。

## <a name="next-steps"></a>後續步驟

透過下列快速入門，開始使用 Azure 受控服務識別功能：

* [使用 Windows VM 受控服務識別來存取 Azure Resource Manager](tutorial-windows-vm-access-arm.md)
* [使用 Linux VM 受控服務識別來存取 Azure Resource Manager](tutorial-linux-vm-access-arm.md)
