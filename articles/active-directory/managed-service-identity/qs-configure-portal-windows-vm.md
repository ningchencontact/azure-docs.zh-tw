...
在本文中，您會了解如何使用 Azure 入口網站，為 Azure 虛擬機器 (VM) 啟用和停用系統與使用者指派的身分識別。 
...
在本節中，您會了解如何使用 Azure 入口網站，為 VM 啟用和停用系統指派的身分識別。

### <a name="enable-system-assigned-identity-during-creation-of-a-vm"></a>在建立 VM 時啟用系統指派的身分識別

目前，Azure 入口網站不支援在建立 VM 時啟用系統指派的身分識別。 相反地，請參閱下列其中一篇 VM 建立快速入門文章來先建立 VM，然後繼續進行下一節，以了解如何在 VM 上啟用系統指派的身分識別：
...
### <a name="enable-system-assigned-identity-on-an-existing-vm"></a>在現有 VM 上啟用系統指派的身分識別
...
### <a name="remove-system-assigned-identity-from-a-vm"></a>從 VM 中移除系統指派的身分識別
...
## <a name="user-assigned-identity"></a>使用者指派的身分識別
 ...
 在本節中，您會了解如何使用 Azure 入口網站在 VM 中新增和移除使用者指派的身分識別。

### <a name="assign-a-user-assigned-identity-during-the-creation-of-a-vm"></a>在建立 VM 時指派使用者指派的身分識別

目前，Azure 入口網站不支援在建立 VM 時指派使用者指派的身分識別。 相反地，請參閱下列其中一篇 VM 建立快速入門文章來先建立 VM，然後繼續進行下一節，以了解如何對 VM 指派使用者指派的身分識別：

- [使用 Azure 入口網站建立 Windows 虛擬機器](../../virtual-machines/windows/quick-create-portal.md#create-virtual-machine)
- [使用 Azure 入口網站建立 Linux 虛擬機器](../../virtual-machines/linux/quick-create-portal.md#create-virtual-machine)

### <a name="assign-a-user-assigned-identity-to-an-existing-vm"></a>將使用者指派的身分識別指派至現有 VM

1. 使用與包含虛擬機器的 Azure 訂用帳戶相關聯的帳戶，登入 [Azure 入口網站](https://portal.azure.com)。
2. 瀏覽至所需 VM，然後依序按一下 [身分識別]、[使用者指派] 和 [\+新增]。

   ![對 VM 新增使用者指派的身分識別](./media/msi-qs-configure-portal-windows-vm/add-user-assigned-identity-vm-screenshot1.png)

3. 按一下想要新增到 VM 的使用者指派身分識別，然後按一下 [新增]。

    ![對 VM 新增使用者指派的身分識別](./media/msi-qs-configure-portal-windows-vm/add-user-assigned-identity-vm-screenshot2.png)

### <a name="remove-a-user-assigned-identity-from-a-vm"></a>從 VM 中移除使用者指派的身分識別

1. 使用與包含虛擬機器的 Azure 訂用帳戶相關聯的帳戶，登入 [Azure 入口網站](https://portal.azure.com)。
2. 瀏覽至所需 VM，按一下 [身分識別]、[使用者指派] 和想要刪除的使用者指派身分識別名稱，然後按一下 [移除] (按一下 [確認] 窗格中的 [是])。

   ![從 VM 中移除使用者指派的身分識別](./media/msi-qs-configure-portal-windows-vm/remove-user-assigned-identity-vm-screenshot.png)
