...
ms.locfileid: "56168660"
---
# <a name="tutorial-azure-active-directory-integration-with-alertops"></a>教學課程：Azure Active Directory 與 AlertOps 整合

在本教學課程中，您會了解如何整合 AlertOps 與 Azure Active Directory (Azure AD)。
AlertOps 與 Azure AD 整合提供下列優點：

* 您可以在 Azure AD 中控制可存取 AlertOps 的人員。
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 AlertOpsh (單一登入)。
...
若要設定 Azure AD 與 AlertOps 整合，您需要下列項目：

* Azure AD 訂用帳戶。 如果您沒有 Azure AD 環境，您可以在[這裡](https://azure.microsoft.com/pricing/free-trial/)取得一個月的試用帳戶
* 已啟用 AlertOps 單一登入的訂用帳戶

## <a name="scenario-description"></a>案例描述

在本教學課程中，您會在測試環境中設定和測試 Azure AD 單一登入。

* AlertOps 支援由 **SP 和 IDP** 起始的 SSO
...
4. 在搜尋方塊中，輸入 **AlertOps**，從結果面板中選取 [AlertOps]，然後按一下 [新增] 按鈕以新增應用程式。

    ![結果清單中的 AlertOps](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入

在本節中，您會以名為 **Britta Simon** 的測試使用者為基礎，設定及測試與 AlertOps 搭配運作的 Azure AD 單一登入。
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 AlertOps 中相關使用者之間的連結關聯性。

若要設定及測試與 AlertOps 搭配運作的 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 AlertOps 單一登入](#configure-alertops-single-sign-on)** - 在應用程式端設定單一登入設定。
...
若要使用 AlertOps 設定 Azure AD 單一登入，請執行下列步驟：

1. 在 [Azure 入口網站](https://portal.azure.com/)的 [AlertOps] 應用程式整合頁面上，選取 [單一登入]。
...
4. 在 [基本 SAML 組態] 區段上，若您想要以 **IDP** 起始模式設定應用程式，請執行下列步驟：
    ...
    ![AlertOps 網域及 URL 單一登入資訊](common/idp-intiated.png)

    a. 在 [識別碼] 文字方塊中，使用下列模式來輸入 URL：`https://<SUBDOMAIN>.alertops.com`

    b. 在 [回覆 URL] 文字方塊中，使用下列模式來輸入 URL：`https://<SUBDOMAIN>.alertops.com/login.aspx`

5. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]，然後執行下列步驟：

    ![AlertOps 網域及 URL 單一登入資訊](common/metadata-upload-additional-signon.png)
...
7. 在 [設定 AlertOps] 區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-alertops-single-sign-on"></a>設定 AlertOps 單一登入
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [AlertOps]。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [AlertOps]。

    ![應用程式清單中的 AlertOps 連結](common/all-applications.png)
...
當您在存取面板中按一下 [AlertOps] 圖格時，應該會自動登入您已設定 SSO 的 AlertOps。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
