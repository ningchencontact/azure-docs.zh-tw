...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Pluralsight (單一登入)。
...
4. 在搜尋方塊中，輸入 **Pluralsight**，從結果面板中選取 [Pluralsight]，然後按一下 [新增] 按鈕以新增應用程式。
     ...
     ![結果清單中的 Pluralsight](common/search-new-app.png)
...
5. **[建立 Pluralsight 測試使用者](#create-pluralsight-test-user)** - 在 Pluralsight 中建立一個與 Azure AD 中代表 Britta Simon 之使用者連結的 Britta Simon 對應項目。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Pluralsight] 應用程式整合頁面上，選取 [單一登入]。
...
5. Pluralsight 應用程式會預期要有特定格式的 SAML 判斷提示，因此您必須將自訂屬性對應新增至 SAML 權杖屬性設定。 以下螢幕擷取畫面顯示預設屬性清單。 按一下 [編輯] **** 圖示，以開啟 [使用者屬性] **** 對話方塊。
    ![映像](common/edit-attribute.png)

    >[!NOTE]
    >您也可以利用適當的值新增 **"Unique ID"** 屬性，例如 EmployeeID 和其他適合貴組織的值。 另請注意，這不是必要的屬性，但您可以新增它以識別唯一的使用者。

6. 除了以上屬性之外，Pluralsight 應用程式還預期 SAML 回應中會再多傳回幾個屬性。 在 [使用者屬性] 對話方塊的 [使用者宣告] 區段中，執行下列步驟以設定 SAML 權杖屬性，如下表所示：
...
### <a name="configure-pluralsight-single-sign-on"></a>設定 Pluralsight 單一登入

若要在 **Pluralsight** 端設定單一登入，您必須將從 Azure 入口網站下載的 [同盟中繼資料 XML] 傳送給 [Pluralsight 支援小組](mailto:support@pluralsight.com)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [Pluralsight]。
