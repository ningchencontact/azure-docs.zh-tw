...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Flock (單一登入)。
...
4. 在搜尋方塊中，輸入 **Flock**，從結果面板中選取 [Flock]，然後按一下 [新增] 按鈕以新增應用程式。
...
5. **[建立 Flock 測試使用者](#create-flock-test-user)** - 使 Flock 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
若要設定與 Flock 搭配運作的 Azure AD 單一登入，請執行下列步驟：

1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Flock] 應用程式整合頁面上，選取 [單一登入]。
...
4. 在 [基本 SAML 組態] 區段上，執行下列步驟：
    ...
    ![Flock 網域及 URL 單一登入資訊](common/sp-identifier.png)

    a. 在 [登入 URL] 文字方塊中，使用下列模式輸入 URL：`https://<subdomain>.flock.com/`

    b. 在 [識別碼 (實體識別碼)] 文字方塊中，使用下列模式輸入 URL：`https://<subdomain>.flock.com/`

    > [!NOTE]
    > 這些都不是真正的值。 請使用實際的「登入 URL」及「識別碼」來更新這些值。 請連絡 [Flock 用戶端支援小組](mailto:support@flock.com)以取得這些值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。

4. 在 [以 SAML 設定單一登入] 頁面的 [SAML 簽署憑證] 區段中，按一下 [下載]，以依據您的需求從指定選項下載 [憑證 (Base64)]，並儲存在您的電腦上。

    ![憑證下載連結](common/certificatebase64.png)

6. 在 [設定 Flock] 區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-flock-single-sign-on"></a>設定 Flock 單一登入
...
### <a name="create-flock-test-user"></a>建立 Flock 測試使用者
...
當您在存取面板中按一下 [Flock] 圖格時，應該會自動登入您已設定 SSO 的 Flock。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
