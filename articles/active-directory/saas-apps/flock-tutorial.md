...
3. **[建立 Flock 測試使用者](#create-a-flock-test-user)** - 讓 Flock 中的 Britta Simon 對應項目得以連結至 Azure AD 中代表該使用者的項目。
...
6. 在 [Flock 設定] 區段上，按一下 [設定 Flock] 以開啟 [設定登入] 視窗。 從 [快速參考] 區段中複製 [SAML 實體 ID 和 SAML 單一登入服務 URL]。

    ![Flock 設定](./media/flock-tutorial/tutorial_flock_configure.png) 

7. 在不同的網頁瀏覽器視窗中，以系統管理員身分登入您的 Flock 公司網站。

8. 從左方導覽面板選取 [Authentication] \(驗證\) 索引標籤，然後選取 [SAML Authentication] \(SAML 驗證\)。
