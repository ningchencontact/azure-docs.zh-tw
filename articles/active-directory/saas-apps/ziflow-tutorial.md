...
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 Ziflow 中相關使用者之間的連結關聯性。

若要設定及測試與 Ziflow 搭配運作的 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 Ziflow 單一登入](#configure-ziflow-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 Ziflow 測試使用者](#create-ziflow-test-user)** - 使 Ziflow 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
若要設定與 Ziflow 搭配運作的 Azure AD 單一登入，請執行下列步驟：

1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Ziflow]  應用程式整合頁面上，選取 [單一登入]  。
...
4. 在 [基本 SAML 組態]  區段上，執行下列步驟：
    ...
    ![Ziflow 網域與 URL 單一登入資訊](common/sp-identifier.png)

    a. 在 [登入 URL]  文字方塊中，使用下列模式輸入 URL：`https://ziflow-production.auth0.com/login/callback?connection=<UniqueID>`

    b. 在 [識別碼 (實體識別碼)]  文字方塊中，使用下列模式輸入 URL：`urn:auth0:ziflow-production:<UniqueID>`

    > [!NOTE]
    > 上述值並非真正的值。 您將使用實際值來更新「識別碼」和「登入 URL」中的唯一識別碼值，本教學課程稍後將加以說明。

5. 在 [以 SAML 設定單一登入]  頁面的 [SAML 簽署憑證]  區段中，按一下 [下載]  ，以依據您的需求從指定選項下載 [憑證 (Base64)]  ，並儲存在您的電腦上。

    ![憑證下載連結](common/certificatebase64.png)

6. 在 [設定 Ziflow]  區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-ziflow-single-sign-on"></a>設定 Ziflow 單一登入

1. 在不同的網頁瀏覽器視窗中，以安全性系統管理員身分登入 Ziflow。

2. 按一下右上角的「虛擬人偶」，然後按一下 [管理帳戶]  。
...
4. 在 [單一登入]  頁面上，執行下列步驟：
    ...
    e. 在 [識別碼提供者的組態設定]  區段中，複製反白顯示的唯一識別碼值，然後在上面附加 Azure 入口網站的 [基本 SAML 組態]  中的 [識別碼] 和 [登入 URL]。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]  、[所有應用程式]  及 [Ziflow]  。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [Ziflow]  。

    ![應用程式清單中的 Ziflow 連結](common/all-applications.png)
...
### <a name="create-ziflow-test-user"></a>建立 Ziflow 測試使用者

若要讓 Azure AD 使用者能夠登入 Ziflow，必須將他們佈建到 Ziflow 中。 在 Ziflow 中，需以手動方式佈建。
