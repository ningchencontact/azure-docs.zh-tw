...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 SciQuest Spend Director (單一登入)。
...
* SciQuest Spend Director 支援由 **SP** 起始的 SSO
* SciQuest Spend Director 支援 **Just In Time** 使用者佈建
...
4. 在搜尋方塊中，輸入 **SciQuest Spend Director**，從結果面板中選取 [SciQuest Spend Director]  ，然後按一下 [新增]  按鈕以新增應用程式。
     ...
     ![結果清單中的 SciQuest Spend Director](common/search-new-app.png)
...
5. **[建立 SciQuest Spend Director 測試使用者](#create-sciquest-spend-director-test-user)** - 在 SciQuest Spend Director 中建立一個與 Azure AD 中代表 Britta Simon 之使用者連結的 Britta Simon 對應項目。
...
若要設定與 SciQuest Spend Director 搭配運作的 Azure AD 單一登入，請執行下列步驟：

1. 在 [Azure 入口網站](https://portal.azure.com/)的 [SciQuest Spend Director]  應用程式整合頁面上，選取 [單一登入]  。
...
6. 在 [安裝 SciQuest Spend Director]  區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-sciquest-spend-director-single-sign-on"></a>設定 SciQuest Spend Director 單一登入

若要在 **SciQuest Spend Director** 端設定單一登入，您必須將從 Azure 入口網站下載的 [同盟中繼資料 XML]  和複製的適當 URL 傳送給 [SciQuest Spend Director 支援小組](https://www.jaggaer.com/contact-us/)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]  、[所有應用程式]  及 [SciQuest Spend Director]  。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [SciQuest Spend Director]  。

    ![應用程式清單中的 [SciQuest Spend Director] 連結](common/all-applications.png)
...
### <a name="create-sciquest-spend-director-test-user"></a>建立 SciQuest Spend Director 測試使用者
...
當您在存取面板中按一下 [SciQuest Spend Director] 圖格時，應該會自動登入您已設定 SSO 的 SciQuest Spend Director。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
