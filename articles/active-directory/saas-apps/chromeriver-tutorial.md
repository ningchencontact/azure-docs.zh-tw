...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Chromeriver (單一登入)。
...
* 已啟用 Chromeriver 單一登入的訂用帳戶

## <a name="scenario-description"></a>案例描述

在本教學課程中，您會在測試環境中設定和測試 Azure AD 單一登入。

* Chromeriver 支援由 **IDP** 起始的 SSO
...
4. 在搜尋方塊中，輸入 **Chromeriver**，從結果面板中選取 [Chromeriver]，然後按一下 [新增] 按鈕以新增應用程式。

    ![結果清單中的 Chromeriver](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入

在本節中，您會以名為 **Britta Simon** 的測試使用者為基礎，設定及測試與 Chromeriver 搭配運作的 Azure AD 單一登入。
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 Chromeriver 中相關使用者之間的連結關聯性。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Chromeriver] 應用程式整合頁面上，選取 [單一登入]。
...
4. 在 [以 SAML 設定單一登入] 頁面上，執行下列步驟：
    ...
    ![Chromeriver 網域及 URL 單一登入資訊](common/idp-intiated.png)

    a. 在 [識別碼] 文字方塊中，使用下列模式來輸入 URL：`https://<subdomain>.chromeriver.com`

    b. 在 [回覆 URL] 文字方塊中，使用下列模式來輸入 URL：`https://<subdomain>.chromeriver.com/login/sso/saml/consume?customerId=<uniqueid>`

    > [!NOTE]
    > 這些都不是真正的值。 請使用實際的識別碼和回覆 URL 更新這些值。 請連絡 [Chromeriver 用戶端支援小組](https://www.chromeriver.com/services/support)以取得這些值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。

5. 在 [以 SAML 設定單一登入] 頁面的 [SAML 簽署憑證] 區段中按一下 [下載]，以依據您的需求從指定選項下載**同盟中繼資料 XML**，並儲存在您的電腦上。

    ![憑證下載連結](common/metadataxml.png)

6. 在 [設定 Chromeriver] 區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-chromeriver-single-sign-on"></a>設定 Chromeriver 單一登入

若要在 **Chromeriver** 端設定單一登入，您必須將從 Azure 入口網站下載的 [同盟中繼資料 XML] 和複製的適當 URL 傳送給 [Chromeriver 支援小組](https://www.chromeriver.com/services/support)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [Chromeriver]。
...
### <a name="create-chromeriver-test-user"></a>建立 Chromeriver 測試使用者

若要讓 Azure AD 使用者能夠登入 Chromeriver，必須將他們佈建到 Chromeriver。 就 Chromeriver 而言，必須由您的 [Chromeriver 支援小組](https://www.chromeriver.com/services/support)建立使用者帳戶。
