...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Showpad (單一登入)。
...
* Showpad 支援由 **SP** 起始的 SSO
* Showpad 支援 **Just In Time** 使用者佈建
...
在本節中，您會以名為 **Britta Simon** 的測試使用者為基礎，設定及測試與 Showpad 搭配運作的 Azure AD 單一登入。
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 Showpad 中相關使用者之間的連結關聯性。

若要使用 Showpad 設定及測試 Azure AD 單一登入功能，您需要完成下列建置組塊：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 Showpad 單一登入](#configure-showpad-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 Showpad 測試使用者](#create-showpad-test-user)** - 使 Showpad 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
若要設定與 Showpad 搭配運作的 Azure AD 單一登入，請執行下列步驟：

1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Showpad]  應用程式整合頁面上，選取 [單一登入]  。
...
4. 在 [基本 SAML 組態]  區段上，執行下列步驟：
    ...
    ![Showpad 網域和 URL 單一登入資訊](common/sp-identifier.png)
...
6. 在 [設定 Showpad]  區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-showpad-single-sign-on"></a>設定 Showpad 單一登入
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]  、[所有應用程式]  及 [Showpad]  。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [Showpad]  。

    ![應用程式清單中的 Showpad 連結](common/all-applications.png)
...
### <a name="create-showpad-test-user"></a>建立 Showpad 測試使用者

本節會在 Showpad 中建立名為 Britta Simon 的使用者。 Showpad 支援依預設啟用的 Just-In-Time 使用者佈建。 在這一節沒有您需要進行的動作項目。 如果 Showpad 中還沒有任何使用者存在，在驗證之後就會建立新的使用者。

### <a name="test-single-sign-on"></a>測試單一登入 

在本節中，您會使用存取面板來測試您的 Azure AD 單一登入設定。

當您在存取面板中按一下 [Showpad] 圖格時，應該會自動登入您已設定 SSO 的 Showpad。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
