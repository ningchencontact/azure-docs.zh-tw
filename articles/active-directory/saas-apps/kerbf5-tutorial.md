...
1. 您必須將中繼資料憑證匯入稍後將在安裝程式中使用的 F5 (Kerberos)。 移至 [系統] > [憑證管理] > [流量憑證管理] > [SSL 憑證清單]  。 按一下右上角的 [匯入]  。

    ![F5 (Kerberos) 設定](./media/kerbf5-tutorial/configure01.png) 

1. 此外，您也需要主機名稱 (`Kerbapp.superdemo.live`) 的 **SSL 憑證**，在此範例中我們使用萬用字元憑證。

    ![F5 (Kerberos) 設定](./media/kerbf5-tutorial/configure02.png) 
 
1. 移至 – F5 BIG IP 並按一下[存取] > [引導式設定] > [同盟] > [SAML 服務提供者]  。

    ![F5 (Kerberos) 設定](./media/kerbf5-tutorial/configure03.png) 

    ![F5 (Kerberos) 設定](./media/kerbf5-tutorial/configure04.png)

1. 指定**實體識別碼** (與您在 Azure AD 應用程式設定中所設定的內容相同)

    ![F5 (Kerberos) 設定](./media/kerbf5-tutorial/configure05.png) 

1. 建立新的虛擬伺服器，指定**目的地位址**。 選擇我們先前上傳的**萬用字元憑證**(或您為應用程式上傳的**憑證**) 及**相關聯的私密金鑰**。

    ![F5 (Kerberos) 設定](./media/kerbf5-tutorial/configure06.png)

1. 上傳設定**中繼資料**，並**為 SAML IDP 連接器指定新的名稱**，而您也必須指定先前上傳的同盟憑證。

    ![F5 (Kerberos) 設定](./media/kerbf5-tutorial/configure07.png)  

1. **建立新的**後端應用程式集區，指定後端應用程式伺服器的 **IP 位址**。
 
    ![F5 (Kerberos) 設定](./media/kerbf5-tutorial/configure08.png)

1. 在 [單一登入設定]  底下選擇 [Kerberos]  ，然後選取 [進階設定]  。 要求必須在 `user@domain.suffix` 中建立。

1. 在 [使用者名稱來源]  底下指定 `session.saml.last.attr.name. http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname`。 如需變數和值的完整清單，請參閱附錄。
帳戶名稱是已建立的 F5 委派帳戶 (請查看 F5 文件)。

    ![F5 (Kerberos) 設定](./media/kerbf5-tutorial/configure09.png)   

1. 請連絡 [F5 (Kerberos) 用戶端支援小組](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45)以取得**端點檢查屬性**文件的詳細資料。

    ![F5 (Kerberos) 設定](./media/kerbf5-tutorial/configure10.png) 

1. 請連絡 [F5 (Kerberos) 用戶端支援小組](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45)以取得**工作階段管理屬性**文件的詳細資料。
