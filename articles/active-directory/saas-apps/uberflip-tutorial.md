...
* Uberflip 支援由 **SP** 和 **IDP** 起始的 SSO

* Uberflip 支援 **Just In Time** 使用者佈建
...
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 Uberfli 中相關使用者之間的連結關聯性。

若要設定及測試與 Uberflip 搭配運作的 Azure AD 單一登入，您需要完成下列建置組塊：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 Uberflip 單一登入](#configure-uberflip-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 Uberflip 測試使用者](#create-uberflip-test-user)** - 在 Uberflip 中建立一個與 Azure AD 中代表 Britta Simon 之項目連結的 Britta Simon 對應項目。
...
4. 在 [基本 SAML 設定] 區段上，如果您想要以 **IDP** 起始模式設定應用程式，請執行下列步驟：
    ...
    在 [回覆 URL] 文字方塊中，使用下列模式來輸入 URL：`https://app.uberflip.com/sso/saml2/<IDPID>/<ACCOUNTID>`
    
    > [!NOTE]
    > 這不是真實的值。 請使用實際的「回覆 URL」來更新此值。 請連絡 [Uberflip 用戶端支援小組](mailto:support@uberflip.com)以取得此值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。
...
7. 在 [設定 Uberflip] 區段上，依據您的需求複製適當的 URL。
...
若要在 **Uberflip** 端設定單一登入，您必須將從 Azure 入口網站下載的 [同盟中繼資料 XML] 和複製的適當 URL 傳送給 [Uberflip 支援小組](mailto:support@uberflip.com)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [Uberflip]。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [Uberflip]。

    ![應用程式清單中的 Uberflip 連結](common/all-applications.png)
...
### <a name="create-uberflip-test-user"></a>建立 Uberflip 測試使用者

本節會在 Uberflip 中建立名為 Britta Simon 的使用者。 Uberflip 支援依預設啟用的 Just-In-Time 使用者佈建。 在這一節沒有您需要進行的動作項目。 如果 Uberflip 中還沒有任何使用者存在，在驗證之後就會建立新的使用者。
...
當您在存取面板中按一下 [Uberflip] 圖格時，應該會自動登入您已設定 SSO 的 Uberflip。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
