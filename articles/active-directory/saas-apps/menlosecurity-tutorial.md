...
3. **[建立 Menlo Security 測試使用者](#creating-a-menlo-security-test-user)** - 使 Menlo Security 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
3. 在 [Menlo Security 網域與 URL] 區段上，執行下列步驟：
...
6. 在 [Menlo Security 組態] 區段上，按一下 [設定 Menlo Security] 以開啟 [設定登入] 視窗。 從 [快速參考] 區段中複製 [SAML 實體 ID] 和 [SAML 單一登入服務 URL]。

    ![設定單一登入](./media/menlosecurity-tutorial/tutorial_menlosecurity_configure.png) 

7. 若要在 **Menlo Security** 端設定單一登入，請以系統管理員身分登入 **Menlo Security** 網站。

8. 前往 [設定] 下的 [驗證] 並執行下列動作︰
