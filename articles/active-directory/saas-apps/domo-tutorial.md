...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Domo (單一登入)。
...
4. 在搜尋方塊中，輸入 **Domo**，從結果面板中選取 [Domo]，然後按一下 [新增] 按鈕以新增應用程式。

     ![結果清單中的 Domo](common/search-new-app.png)
...
若要設定搭配 Domo 的 Azure AD 單一登入，請執行下列步驟：

1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Domo] 應用程式整合頁面上，選取 [單一登入]。
...
8. 在 [設定 Domo] 區段上，依據您的需求複製適當的 URL。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [Domo]。
