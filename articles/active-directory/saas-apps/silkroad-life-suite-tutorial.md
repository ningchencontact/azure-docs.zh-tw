...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 SilkRoad Life Suite (單一登入)。
...
* SilkRoad Life Suite 支援 **SP** 起始的 SSO
...
為了讓單一登入運作，必須在 Azure AD 使用者與 SilkRoad Life Suite 中的相關使用者之間建立連結關聯性。

若要設定及測試對 SilkRoad Life Suite 的 Azure AD 單一登入，您需要完成下列建置組塊：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 SilkRoad Life Suite 單一登入](#configure-silkroad-life-suite-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 SilkRoad Life Suite 測試使用者](#create-silkroad-life-suite-test-user)** - 在 SilkRoad Life Suite 中建立一個與 Azure AD 中代表 Britta Simon 之項目連結的對應項目。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [SilkRoad Life Suite] 應用程式整合頁面上，選取 [單一登入]。
...
5. 如果您沒有**服務提供者中繼資料檔案**，請在 [基本 SAML 設定] 區段上執行下列步驟：
    ...
    a. 在 [登入 URL] 文字方塊中，以下列模式輸入 URL︰`https://<subdomain>.silkroad-eng.com/Authentication/`
...
7. 在 [設定 SilkRoad Life Suite] 區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-silkroad-life-suite-single-sign-on"></a>設定 SilkRoad Life Suite 單一登入
...
1. 在 [編輯驗證來源]  對話方塊中，執行下列步驟：
    ...
    g. 在 [登入服務 URL] 文字方塊中，貼上您從 Azure 入口網站複製的 [登入 URL] 值。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [SilkRoad Life Suite]。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [SilkRoad Life Suite] 。

    ![應用程式清單中的 [SilkRoad Life Suite] 連結](common/all-applications.png)
...
### <a name="create-silkroad-life-suite-test-user"></a>建立 SilkRoad Life Suite 測試使用者
...
當您在存取面板中按一下 [SilkRoad Life Suite] 圖格時，應該會自動登入您已設定 SSO 的 SilkRoad Life Suite。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
