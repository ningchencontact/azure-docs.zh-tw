...
3. 在 [組態] 頁面上，執行下列步驟：
    ...
    c. Tableau Server 傳回 URL — Tableau Server 使用者將存取的 URL，例如 <http://tableau_server>。 不建議使用 `http://localhost`。 使用包含結尾斜線的 URL (例如`http://tableau_server/`) 不支援。 複製 [Tableau Server 傳回 URL]，並將其貼到 [Tableau Server 網域及 URL] 區段中的 Azure AD [單一登入 URL] 文字方塊。
