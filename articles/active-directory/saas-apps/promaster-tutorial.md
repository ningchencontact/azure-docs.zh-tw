...
4. 在搜尋方塊中，輸入 **ProMaster (by Inlogik)**，從結果面板中選取 [ProMaster (by Inlogik)]，然後按一下 [新增] 按鈕以新增應用程式。
     ...
     ![結果清單中的 ProMaster (by Inlogik)](common/search-new-app.png)
...
2. **[設定 ProMaster (by Inlogik) 單一登入](#configure-promaster-by-inlogik-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 ProMaster (by Inlogik) 測試使用者](#create-promaster-by-inlogik-test-user)** - 使 ProMaster (by Inlogik) 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
4. 在 [基本 SAML 設定] 區段上，如果您想要以 **IDP** 起始模式設定應用程式，請執行下列步驟：
    ...
    ![ProMaster (by Inlogik) 網域與 URL 單一登入資訊](common/idp-intiated.png)
...
5. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]，然後執行下列步驟：
    ...
    ![ProMaster (by Inlogik) 網域與 URL 單一登入資訊](common/metadata-upload-additional-signon.png)
    ...
    > [!NOTE]
    > 這些都不是真正的值。 請使用實際的「識別碼」、「回覆 URL」及「登入 URL」來更新這些值。 請連絡 [ProMaster (by Inlogik) 用戶端支援小組](mailto:michael.boldiston@inlogik.com)以取得這些值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。

6. 在 [以 SAML 設定單一登入] 頁面的 [SAML 簽署憑證] 區段中，按一下 [複製] 按鈕以複製 [應用程式同盟中繼資料 URL]，並將其儲存在您的電腦上。

    ![憑證下載連結](common/copy-metadataurl.png)

### <a name="configure-promaster-by-inlogik-single-sign-on"></a>設定 ProMaster (by Inlogik) 單一登入

若要在 **ProMaster (by Inlogik)** 端設定單一登入，您必須將**應用程式同盟中繼資料 URL** 傳送給 [ProMaster (by Inlogik) 支援小組](mailto:michael.boldiston@inlogik.com)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [ProMaster (by Inlogik)]。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [ProMaster (by Inlogik)]。

    ![應用程式清單中的 ProMaster (by Inlogik) 連結](common/all-applications.png)
...
在本節中，您會在 ProMaster (by Inlogik) 中建立名為 Britta Simon 的使用者。 請與  [ProMaster (by Inlogik) 支援小組](mailto:michael.boldiston@inlogik.com)合作，在 ProMaster (by Inlogik) 平台中新增使用者。 您必須先建立和啟動使用者，然後才能使用單一登入。
