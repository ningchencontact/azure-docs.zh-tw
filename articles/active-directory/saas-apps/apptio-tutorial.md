...
4. 在搜尋方塊中，鍵入 **Apptio**，從結果面板中選取 [Apptio]，然後按一下 [新增] 按鈕以新增應用程式。
     ...
     ![結果清單中的 Apptio](common/search-new-app.png)
...
5. **[建立 Apptio 測試使用者](#create-apptio-test-user)** - 在 Apptio 中建立一個與 Azure AD 中代表 Britta Simon 之使用者連結的 Britta Simon 對應項目。
...
若要設定與 Apptio 搭配運作的 Azure AD 單一登入，請執行下列步驟：

1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Apptio] 應用程式整合頁面上，選取 [單一登入]。
...
8. 在 [設定 Apptio] 區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-apptio-single-sign-on"></a>設定 Apptio 單一登入
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [Apptio]。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [Apptio]。

    ![應用程式清單中的 Apptio 連結](common/all-applications.png)
