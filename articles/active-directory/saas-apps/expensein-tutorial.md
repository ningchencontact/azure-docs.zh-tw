---
title: 教學課程：Azure Active Directory 與 ExpenseIn 整合 | Microsoft Docs
description: 了解如何設定 Azure Active Directory 與 ExpenseIn 之間的單一登入。
...
在本教學課程中，您將了解如何整合 ExpenseIn 與 Azure Active Directory (Azure AD)。 在整合 ExpenseIn 與 Azure AD 時，您可以︰

* 在 Azure AD 中控制可存取 ExpenseIn 的人員。
...
* 已啟用 ExpenseIn 單一登入 (SSO) 的訂用帳戶。

## <a name="scenario-description"></a>案例描述

在本教學課程中，您會在測試環境中設定和測試 Azure AD SSO。 ExpenseIn 支援由 **SP 和 IDP** 起始的 SSO。

## <a name="adding-expensein-from-the-gallery"></a>從資源庫新增 ExpenseIn

若要設定將 ExpenseIn 整合到 Azure AD 中，您需要從資源庫將 ExpenseIn新增到受控 SaaS 應用程式清單。
...
以名為 **B. Simon** 的測試使用者，設定及測試與 ExpenseIn 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 ExpenseIn 中相關使用者之間的連結關聯性。

若要設定及測試與 ExpenseIn 搭配運作的 Azure AD SSO，請完成下列建置組塊：
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [ExpenseIn]  應用程式整合頁面上，尋找 [管理]  區段並選取 [單一登入]  。
...
5. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]  ，然後執行下列步驟：
    ...
    在 [登入 URL]  文字方塊中，輸入 URL：`https://app.expensein.com/saml`

1. 在 [以 SAML 設定單一登入]  頁面的 [SAML 簽署憑證]  區段中，按一下複製按鈕以複製 [應用程式同盟中繼資料 URL]  ，然後按一下 [下載]  以下載 [憑證 (Base64)]  並儲存在您的電腦上。

   ![憑證下載連結](./media/expensein-tutorial/copy-metdataurl-certificate.png)

1. 在 [設定 ExpenseIn]  區段上，根據您的需求複製適當的 URL。
...
### <a name="create-expensein-test-user"></a>建立 ExpenseIn 測試使用者

若要讓 Azure AD 使用者可以登入 ExpenseIn，必須將他們佈建到 ExpenseIn。 在 ExpenseIn 中，需手動進行佈建。

**若要佈建使用者帳戶，請執行下列步驟：**

1. 以系統管理員身分登入 ExpenseIn。
