...
4. 在搜尋方塊中，輸入 **AnswerHub**，從結果面板中選取 [AnswerHub]，然後按一下 [新增] 按鈕以新增應用程式。
...
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 AnswerHub 中相關使用者之間的連結關聯性。
...
5. **[建立 AnswerHub 測試使用者](#create-answerhub-test-user)** - 在 AnswerHub 中建立一個與 Azure AD 中代表 Britta Simon 之項目連結的 Britta Simon 對應項目。
...
4. 在 [基本 SAML 組態] 區段上，執行下列步驟：
    ...
    ![AnswerHub 網域與 URL 單一登入資訊](common/sp-identifier.png)

    a. 在 [登入 URL] 文字方塊中，使用下列模式輸入 URL：`https://<company>.answerhub.com`

    b. 在 [識別碼 (實體識別碼)] 文字方塊中，使用下列模式輸入 URL：`https://<company>.answerhub.com`

    > [!NOTE]
    > 這些都不是真正的值。 請使用實際的「登入 URL」及「識別碼」來更新這些值。 請連絡 [AnswerHub 用戶端支援小組](mailto:success@answerhub.com)以取得這些值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。

5. 在 [以 SAML 設定單一登入] 頁面的 [SAML 簽署憑證] 區段中，按一下 [下載]，以依據您的需求從指定選項下載 [憑證 (Base64)]，並儲存在您的電腦上。

    ![憑證下載連結](common/certificatebase64.png)

6. 在 [設定 AnswerHub] 區段上，依據您的需求複製適當的 URL。
...
4. 在左側瀏覽窗格的 [社交設定] 區段中，按一下 [SAML 設定]。

5. 按一下 [IDP 設定]  索引標籤。

6. 在 [IDP 設定]  索引標籤上，執行下列步驟：

    ![SAML 設定](./media/answerhub-tutorial/ic785172.png "SAML 設定")  
  
    a. 在 [IDP 登入 URL] 文字方塊中，貼上您從 Azure 入口網站複製的**登入 URL**。
  
    b. 在 [IDP 登出 URL] 文字方塊中，貼上您從 Azure 入口網站複製的**登出 URL** 值。

    c. 在 [IDP 名稱識別碼格式] 文字方塊中，輸入與在 Azure 入口網站的 [使用者屬性] 區段中所選相同的使用者識別碼值。
  
    d. 按一下 [金鑰和憑證] 。

7. 在 [金鑰和憑證] 索引標籤上，執行下列步驟：
...
2. 在應用程式清單中，選取 [AnswerHub]。
    ...
    ![應用程式清單中的 AnswerHub 連結](common/all-applications.png)
...
### <a name="create-answerhub-test-user"></a>建立 AnswerHub 測試使用者

若要讓 Azure AD 使用者能夠登入 AnswerHub，必須將他們佈建到 AnswerHub。 就 AnswerHub 而言，需以手動方式佈建。
...
4. 在左側瀏覽窗格的 [管理使用者] 區段中，按一下 [建立或匯入使用者]，然後按一下 [使用者和群組]。
...
當您在存取面板中按一下 [AnswerHub] 圖格時，應該會自動登入您設定 SSO 的 AnswerHub。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
