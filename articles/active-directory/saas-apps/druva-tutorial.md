---
title: 教學課程：Azure Active Directory 單一登入 (SSO) 與 Druva 整合 | Microsoft Docs
...
## <a name="configure-and-test-azure-ad-single-sign-on-for-druva"></a>設定及測試 Druva 的 Azure AD 單一登入

以名為 **B.Simon** 的測試使用者，設定及測試與 Druva 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 Druva 中相關使用者之間的連結關聯性。
...
1. **[設定 Druva SSO](#configure-druva-sso)** - 在應用程式端設定單一登入設定。
    * **[建立 Druva 測試使用者](#create-druva-test-user)** - 使 Druva 中對應的 B.Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. Druva 應用程式需要特定格式的 SAML 判斷提示，您需要將自訂屬性對應加入您的 SAML 權杖屬性組態。 以下螢幕擷取畫面顯示預設屬性清單。

    ![image](common/default-attributes.png)

1. 除了上述屬性外，Druva 應用程式還需要在 SAML 回應中多傳回幾個屬性，如下所示。 這些屬性也會預先填入，但您可以根據您的需求來檢閱這些屬性。
...
在本節中，您會將 Druva 的存取權授與 B.Simon，讓其能夠使用 Azure 單一登入。
...
1. 按一下左上角的 [Druva] 標誌，然後按一下 [Druva 雲端設定]  。

    ![設定](./media/druva-tutorial/ic795091.png "設定")

1. 在 [單一登入]  索引標籤上，按一下 [編輯]  。

    ![單一登入設定](./media/druva-tutorial/ic795092.png "單一登入設定")

1. 在 [編輯單一登入設定]  頁面執行下列步驟：

    ![單一登入設定](./media/druva-tutorial/ic795095.png "單一登入設定")

    1. 在 [識別碼提供者登入 URL]  文字方塊中，貼上您從 Azure 入口網站複製的 [登入 URL]  值。

    1. 在記事本中開啟您的 base-64 編碼的憑證，將其內容複製到您的剪貼簿，然後貼到 [識別提供者憑證]  文字方塊中。

       > [!NOTE]
       > 若要啟用系統管理員的單一登入，請選取 [系統管理員透過 SSO 提供者登入 Druva Cloud]  和 [允許 Druva 雲端系統管理員的故障保全 (failsafe) 存取 (建議)]  核取方塊。 Druva 建議您啟用 [系統管理員故障保全]  ，讓他們在 IDP 中發生任何失敗時，必須存取 DCP 主控台。 此外，也可讓系統管理員同時使用 SSO 和 DCP 密碼來存取 DCP 主控台。

    1. 按一下 [檔案]  。 這可讓您使用 SSO 來存取 Druva 雲端平台。

### <a name="create-druva-test-user"></a>建立 Druva 測試使用者

本節會在 Druva 中建立名為 B.Simon 的使用者。 Druva 支援依預設啟用的 Just-In-Time 使用者佈建。 在這一節沒有您需要進行的動作項目。 如果 Druva 中還沒有任何使用者存在，在驗證之後就會建立新的使用者。
