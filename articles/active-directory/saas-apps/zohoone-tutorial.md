...
3. **[建立 Zoho One 測試使用者](#create-a-zoho-one-test-user)** - 讓 Zoho One 中的 Britta Simon 對應項目得以連結至 Azure AD 中代表該使用者的項目。
...
3. 如果您想要以 **IDP** 起始模式設定應用程式，請在 [Zoho One 網域及 URL] 區段上執行下列步驟：
...
7. 在 [Zoho One 組態] 區段上，按一下 [設定 Zoho One] 以開啟 [設定登入] 視窗。 從 [快速參考] 區段中複製 [登出 URL 和 SAML 單一登入服務 URL]。

    ![Zoho One 設定](./media/zohoone-tutorial/tutorial_zohoone_configure.png) 

8. 在不同的網頁瀏覽器視窗中，以系統管理員身分登入您的 Zoho One 公司網站。

9. 在 [Organization]\(組織\) 索引標籤上，按一下 [SAML Authentication]\(SAML 驗證\) 下的 [Setup]\(設定\)。
