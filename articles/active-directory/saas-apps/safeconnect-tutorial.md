...
ms.locfileid: "40024575"
---
# <a name="tutorial-azure-active-directory-integration-with-safeconnect"></a>教學課程：Azure Active Directory 與 SafeConnect 整合

在本教學課程中，您會了解如何整合 SafeConnect 與 Azure Active Directory (Azure AD)。

SafeConnect 與 Azure AD 整合提供下列優點：

- 您可以在 Azure AD 中控制可存取 SafeConnect 的人員。
- 您可以讓使用者使用他們的 Azure AD 帳戶自動登入 SafeConnect (單一登入)。
...
若要設定與 SafeConnect 的 Azure AD 整合，您需要下列項目：

- Azure AD 訂用帳戶
- 已啟用 SafeConnect 單一登入的訂用帳戶
...
在本節中，您會以名為 "Britta Simon" 的測試使用者為基礎，設定及測試與 SafeConnect 搭配運作的 Azure AD 單一登入。

若要讓單一登入能夠運作，Azure AD 必須知道 SafeConnect 與 Azure AD 中互相對應的使用者。 換句話說，必須在 Azure AD 使用者和 SafeConnect 中的相關使用者之間建立連結關聯性。

若要使用 SafeConnect 來設定並測試 Azure AD 單一登入，您需要完成下列建置組塊：
...
在本節中，您會在 Azure 入口網站中啟用 Azure AD 單一登入，然後在您的 SafeConnect 應用程式中設定單一登入。
...
5. 按一下 [儲存]  按鈕。
    ...
    ![設定單一登入儲存按鈕](./media/safeconnect-tutorial/tutorial_general_400.png)

6. 若要在 **SafeConnect** 端設定單一登入，您必須將已下載的**中繼資料 XML** 傳送給 [SafeConnect 支援小組](mailto:support@impulse.com)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
在本節中，您會將 SafeConnect 的存取權授與 Britta Simon，讓她能夠使用 Azure 單一登入。
...
當您在存取面板中按一下 [SafeConnect] 圖格時，應該會自動登入您的 SafeConnect 應用程式。
