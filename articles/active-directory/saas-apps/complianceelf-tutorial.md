...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Compliance ELF (單一登入)。
...
* Compliance ELF 支援由 **SP 和 IDP** 起始的 SSO
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Compliance ELF] 應用程式整合頁面上，選取 [單一登入]。
...
4. 在 [基本 SAML 設定] 區段上，如果您想要以 **IDP** 起始模式設定應用程式，請執行下列步驟：
    ...
    ![Compliance ELF 網域及 URL 單一登入資訊](common/idp-identifier.png)

    在 [識別碼] 文字方塊中，鍵入 URL：`https://sso.cordium.com`
...
5. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]，然後執行下列步驟：
    ...
    > [!NOTE]
    > [登入 URL] 的值不是真正的值。 使用實際的登入 URL 來更新此值。 請連絡 [Compliance ELF 用戶端支援小組](mailto:support@complianceelf.com)以取得此值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。

6. 在 [以 SAML 設定單一登入] 頁面的 [SAML 簽署憑證] 區段中，按一下 [複製] 按鈕以複製 [應用程式同盟中繼資料 URL]，並將其儲存在您的電腦上。

    ![憑證下載連結](common/copy-metadataurl.png)

### <a name="configure-compliance-elf-single-sign-on"></a>設定 Compliance ELF 單一登入

若要在 **Compliance ELF** 端設定單一登入，您必須將「應用程式同盟中繼資料 URL」傳送給 [Compliance ELF 支援小組](mailto:support@complianceelf.com)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [Compliance ELF]。
...
### <a name="create-compliance-elf-test-user"></a>建立 Compliance ELF 測試使用者
