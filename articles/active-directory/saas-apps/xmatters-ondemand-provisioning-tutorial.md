...
4. 在搜尋方塊中，輸入 **xMatters OnDemand**。
    ...
    ![xMatters OnDemand 佈建](./media/xmatters-ondemand-provisioning-tutorial/AppSearch.png)

5. 在結果窗格中，選取 [xMatters OnDemand]，然後按一下 [新增] 按鈕，將 xMatters OnDemand 新增到您的 SaaS 應用程式清單。

    ![xMatters OnDemand 佈建](./media/xmatters-ondemand-provisioning-tutorial/AppSearchResults.png)

    ![xMatters OnDemand 佈建](./media/xmatters-ondemand-provisioning-tutorial/AppCreation.png)
...
> [!TIP]
> 您也可以選擇為 xMatters OnDemand 啟用 SAML 型單一登入，請遵循 [xMatters OnDemand 單一登入教學課程](xmatters-ondemand-tutorial.md)中提供的指示。 雖然自動使用者佈建和單一登入這兩個功能互相補充，您還是可以將它們分開設定。
...
2. 從您的 SaaS 應用程式清單選取 xMatters OnDemand。
    ...
    ![xMatters OnDemand 佈建](./media/xmatters-ondemand-provisioning-tutorial/ApplicationInstanceSearch.png)

3. 選取 [佈建] 索引標籤。
    
    ![xMatters OnDemand 佈建](./media/xmatters-ondemand-provisioning-tutorial/ProvisioningTab.png)

4. 將 [佈建模式] 設定為 [自動]。

    ![xMatters OnDemand 佈建](./media/xmatters-ondemand-provisioning-tutorial/ProvisioningCredentials.png)
...
6. 如步驟 5 所示填寫欄位後，按一下 [測試連線]，以確保 Azure AD 可以連線到 xMatters OnDemand。 如果連線失敗，請確定您的 xMatters OnDemand 帳戶具有系統管理員權限，然後再試一次。
    ...
    ![xMatters OnDemand 佈建](./media/xmatters-ondemand-provisioning-tutorial/TestConnection.png)
    
7. 在 [通知電子郵件] 欄位中，輸入應該收到佈建錯誤通知的個人或群組電子郵件地址，然後選取 [發生失敗時傳送電子郵件通知] 核取方塊。

    ![xMatters OnDemand 佈建](./media/xmatters-ondemand-provisioning-tutorial/EmailNotification.png)

8. 按一下 [檔案] 。

9. 在 [對應] 區段下方，選取 [將 Azure Active Directory 使用者同步至 xMatters OnDemand]。

    ![xMatters OnDemand 佈建](./media/xmatters-ondemand-provisioning-tutorial/SyncUsers.png)
...
10. 在 [屬性對應] 區段中，檢閱從 Azure AD 同步至 xMatters OnDemand 的使用者屬性。 選取為 [比對] 屬性 (Property) 的屬性 (Attribute) 會用來比對 xMatters OnDemand 中的使用者帳戶以進行更新作業。 選取 [儲存] 按鈕以認可所有變更。
    ...
    ![xMatters OnDemand 佈建](./media/xmatters-ondemand-provisioning-tutorial/UserAttributes.png)

11. 在 [對應] 區段下方，選取 [將 Azure Active Directory 群組同步至 xMatters OnDemand]。

    ![xMatters OnDemand 佈建](./media/xmatters-ondemand-provisioning-tutorial/SyncGroups.png)
...
12. 在 [屬性對應] 區段中，檢閱從 Azure AD 同步至 xMatters OnDemand 的群組屬性。 選取為 [比對] 屬性 (Property) 的屬性 (Attribute) 會用來比對 xMatters OnDemand 中的群組以進行更新作業。 選取 [儲存] 按鈕以認可所有變更。
    ...
    ![xMatters OnDemand 佈建](./media/xmatters-ondemand-provisioning-tutorial/GroupAttributes.png)
...
14. 若要為xMatters OnDemand 啟用 Azure AD 佈建服務，請將 [設定] 區段中的 [佈建狀態] 變更為 [開啟]。
    ...
    ![xMatters OnDemand 佈建](./media/xmatters-ondemand-provisioning-tutorial/ProvisioningStatus.png)

15. 透過在 [設定] 區段的 [範圍] 中選擇需要的值，定義要佈建到 xMatters OnDemand 的使用者和/或群組。

    ![xMatters OnDemand 佈建](./media/xmatters-ondemand-provisioning-tutorial/ScopeSelection.png)

16. 當您準備好要佈建時，按一下 [儲存]。

    ![xMatters OnDemand 佈建](./media/xmatters-ondemand-provisioning-tutorial/SaveProvisioning.png)
