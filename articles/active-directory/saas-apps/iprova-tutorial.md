...
* Azure AD 訂用帳戶。 如果您沒有 Azure AD 環境，您可以在 [Microsoft Azure](https://azure.microsoft.com/pricing/free-trial/) 網站上取得一個月的試用帳戶。
* 已啟用 iProva SSO 的訂用帳戶。
...
若要進行將 iProva 整合到 Azure AD 中的設定，請從資源庫將 iProva 新增到受控 SaaS 應用程式清單。

若要從資源庫新增 iProva，請遵循下列步驟：
...
4. 在搜尋方塊中，輸入 **iProva**。 從結果面板中選取 [iProva]，然後選取 [新增] 以新增應用程式。
...
- [從 iProva 擷取組態資訊](#retrieve-configuration-information-from-iprova)，以準備執行後續步驟。
- [設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)，讓您的使用者能夠使用此功能。
- [設定 iProva 單一登入](#configure-iprova-single-sign-on)，以在應用程式端設定單一登入設定。
...
- [建立 iProva 測試使用者](#create-an-iprova-test-user)，以使 iProva 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
若要設定與 iProva 搭配運作的 Azure AD 單一登入，請遵循下列步驟。
...
3. 在 [以 SAML 設定單一登入] 頁面上選取 [編輯] 圖示，以開啟 [基本 SAML 組態] 對話方塊。
    ...
    ![基本 SAML 組態中的 [編輯] 圖示](common/edit-urls.png)
...
6. 在 [使用者屬性] 對話方塊的 [使用者宣告] 區段中，設定 SAML 權杖屬性，如上圖所示。 請遵循下列步驟。
    ...
    ![使用者宣告](common/new-save-attribute.png)
...
7. 在 [以 SAML 設定單一登入] 頁面的 [SAML 簽署憑證] 區段中，選取 [複製] 圖示以複製**應用程式同盟中繼資料 URL**，並儲存在您的電腦上。
...
3. 在左側功能表中，選取 [使用者和群組]。
    ...
    ![使用者和群組連結](common/users-groups-blade.png)
...
### <a name="create-an-iprova-test-user"></a>建立 iProva 測試使用者
...
當您在存取面板中選取 [iProva] 圖格時，應該會自動登入您已設定 SSO 的 iProva。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
