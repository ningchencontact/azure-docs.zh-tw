---
title: 教學課程：Azure Active Directory 與 Acadia 整合 | Microsoft Docs
description: 了解如何設定 Azure Active Directory 與 Acadia 之間的單一登入。
...
ms.locfileid: "65859495"
---
# <a name="tutorial-azure-active-directory-integration-with-acadia"></a>教學課程：Azure Active Directory 與 Acadia 整合

在本教學課程中，您將了解如何整合 Acadia 與 Azure Active Directory (Azure AD)。
Acadia 與 Azure AD 整合可提供下列優點：

* 您可以在 Azure AD 中控制可存取 Acadia 的人員。
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Acadia (單一登入)。
...
若要設定 Azure AD 與 Acadia 的整合，您需要下列項目：

* Azure AD 訂用帳戶。 如果您沒有 Azure AD 環境，您可以在[這裡](https://azure.microsoft.com/pricing/free-trial/)取得一個月的試用帳戶
* 已啟用 Acadia 單一登入的訂用帳戶

## <a name="scenario-description"></a>案例描述

在本教學課程中，您會在測試環境中設定和測試 Azure AD 單一登入。

* Acadia 支援由 **SP 和 IDP** 起始的 SSO
* Acadia 支援 **Just In Time** 使用者佈建

## <a name="adding-acadia-from-the-gallery"></a>從資源庫新增 Acadia

若要進行將 Acadia 整合到 Azure AD 中的設定，您必須從資源庫將 Acadia 新增至受控 SaaS 應用程式清單。

**若要從資源庫新增 Acadia，請執行下列步驟：**
...
5. **[建立 Acadia 測試使用者](#create-acadia-test-user)** - 使 Acadia 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
4. 在 [基本 SAML 組態] 區段上，若您想要以 **IDP** 起始模式設定應用程式，請執行下列步驟：
    ...
    b. 在 [回覆 URL] 文字方塊中，使用下列模式來輸入 URL：`https://<CUSTOMER>.acadia.sysalli.com/Shibboleth.sso/SAML2/POST`

5. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]，然後執行下列步驟：

    ![Acadia 網域和 URL 單一登入資訊](common/metadata-upload-additional-signon.png)
...
7. 在 [設定 Acadia] 區段上，依據您的需求複製適當的 URL。
...
在本節中，您會把 Acadia 的存取權授與 Britta Simon，讓她能夠使用 Azure 單一登入。

1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [Acadia]。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取[Acadia]。

    ![應用程式清單中的 Acadia 連結](common/all-applications.png)
...
### <a name="create-acadia-test-user"></a>建立 Acadia 測試使用者

本節會在 Acadia 中建立名為 Britta Simon 的使用者。 Acadia 支援依預設啟用的 Just-In-Time 使用者佈建。 在這一節沒有您需要進行的動作項目。 如果 Acadia 中還沒有任何使用者存在，在驗證之後就會建立新的使用者。

### <a name="test-single-sign-on"></a>測試單一登入 

在本節中，您會使用存取面板來測試您的 Azure AD 單一登入設定。

當您在存取面板中按一下 [Acadia] 圖格時，應該會自動登入您已設定 SSO 的 Acadia。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
