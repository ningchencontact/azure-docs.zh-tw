...
4. 在搜尋方塊中，輸入 **FreshDesk**。 從結果面板中選取 [FreshDesk]，然後選取 [新增] 按鈕以新增應用程式。

    ![建立 Azure AD 測試使用者](./media/freshdesk-tutorial/tutorial_freshdesk_addfromgallery.png)
...
建立此連結關聯性的方法是將 Azure AD 中 [使用者名稱] 的值指定為 FreshDesk 中**電子郵件地址**的值。
...
2. 在 [單一登入] 對話方塊上，選取 [SAML 型登入] 做為 [模式]，以啟用單一登入。
    ...
    ![設定單一登入](./media/freshdesk-tutorial/tutorial_freshdesk_samlbase.png)

3. 在 [FreshDesk 網域與 URL] 區段上，執行下列步驟：

    ![設定單一登入](./media/freshdesk-tutorial/tutorial_freshdesk_url.png)
...
4. 應用程式需要特定格式的 SAML 判斷提示，需要您將自訂屬性對應新增到您的 SAML 權杖屬性設定。 以下螢幕擷取畫面顯示上述的範例。 [使用者識別碼] 的預設值是 **user.userprincipalname**，但是 **FreshDesk** 會預期它是與使用者電子郵件地址對應的值。 對此您可以使用清單中的 **user.mail** 屬性，或者根據組織組態使用適當的屬性值。

    ![設定單一登入](./media/freshdesk-tutorial/tutorial_attribute.png)

5. 在 [SAML 簽署憑證] 區段上，按一下 [憑證 (Base64)]，然後將憑證檔案儲存在您的電腦上。

    ![設定單一登入](./media/freshdesk-tutorial/tutorial_freshdesk_certificate.png)

    > [!NOTE]
    > 如果您有任何問題，請參考此[連結](https://support.freshdesk.com/support/discussions/topics/317543)。

6. 按一下 [儲存]  按鈕。

    ![設定單一登入](./media/freshdesk-tutorial/tutorial_general_400.png)

7. 如果您的系統中尚未安裝 **OpenSSL**，請在系統中加以安裝。

8. 開啟 [命令提示字元] 並執行下列命令：

    a. 在命令提示字元中輸入 `openssl x509 -inform DER -in FreshDesk.cer -out certificate.crt` 值。

    > [!NOTE]
    > 這裡的 **FreshDesk.cer** 是您從 Azure 入口網站下載的憑證。

    b. 在命令提示字元中輸入 `openssl x509 -noout -fingerprint -sha256 -inform pem -in certificate.crt` 值。 
    
    > [!NOTE]
    > 這裡的 **certificate.crt** 是在上一個步驟中產生的輸出憑證。

    c. 複製 [指紋] 值並貼到 [記事本] 中。 移除指紋中的冒號並取得最終指紋值。

9. 在 [FreshDesk 組態] 區段上，按一下 [設定 FreshDesk] 以開啟 [設定登入] 視窗。 從 [快速參考]區段中複製 [SAML 單一登入服務 URL 和登出 URL]。

    ![設定單一登入](./media/freshdesk-tutorial/tutorial_freshdesk_configure.png)
...
11. 選取 [設定] 圖示，並且在 [安全性] 區段中執行下列步驟：
    ...
    e. 在 [安全性憑證指紋] 文字方塊中，貼上您先前在移除冒號後取得的 [指紋] 值。
...
1. 在 **Azure 入口網站**的左方瀏覽窗格中，按一下 [Azure Active Directory] 圖示。
    ...
    ![建立 Azure AD 測試使用者](./media/freshdesk-tutorial/create_aaduser_01.png) 

2. 移至 [使用者和群組]，然後按一下 [所有使用者] 以顯示使用者清單。

    ![建立 Azure AD 測試使用者](./media/freshdesk-tutorial/create_aaduser_02.png) 

3. 在對話方塊的頂端，按一下 [新增] 以開啟 [使用者] 對話方塊。

    ![建立 Azure AD 測試使用者](./media/freshdesk-tutorial/create_aaduser_03.png) 

4. 在 [使用者]  對話頁面上，執行下列步驟：

    ![建立 Azure AD 測試使用者](./media/freshdesk-tutorial/create_aaduser_04.png) 
