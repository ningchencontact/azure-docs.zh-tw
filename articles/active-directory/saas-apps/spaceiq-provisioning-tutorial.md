...
description: 瞭解如何設定 Azure Active Directory 以自動布建和取消布建使用者帳戶至 SpaceIQ。
...
ms.locfileid: "74134862"
---
# <a name="tutorial-configure-spaceiq-for-automatic-user-provisioning"></a>教學課程：設定 SpaceIQ 來自動布建使用者

本教學課程的目的是要示範要在 SpaceIQ 和 Azure Active Directory （Azure AD）中執行的步驟，以設定 Azure AD 自動布建和取消布建使用者和/或群組至 SpaceIQ。
...
## <a name="set-up-spaceiq-for-provisioning"></a>設定 SpaceIQ 以提供布建
...
3.  複製**SCIM 持有人權杖**。 此值將會在 Azure 入口網站中 SpaceIQ 應用程式的 [布建] 索引標籤的 [秘密權杖] 欄位中輸入。 按一下 [**啟動**]

    ![SpaceIQ 建立權杖](media/spaceiq-provisioning-tutorial/token.png)

## <a name="add-spaceiq-from-the-gallery"></a>從資源庫新增 SpaceIQ

將 SpaceIQ 設定為使用 Azure AD 自動布建使用者之前，您需要從 Azure AD 應用程式庫將 SpaceIQ 新增至受控 SaaS 應用程式清單。
...
4. 在搜尋方塊中，輸入**SpaceIQ**，在結果面板中選取 [ **SpaceIQ** ]，然後按一下 [**新增**] 按鈕以新增應用程式。

    ![結果清單中的 SpaceIQ](common/search-new-app.png)

## <a name="configuring-automatic-user-provisioning-to-spaceiq"></a>設定自動使用者布建至 SpaceIQ 

本節將引導您逐步設定 Azure AD 布建服務，以根據 Azure AD 中的使用者和/或群組指派，在 SpaceIQ 中建立、更新和停用使用者和/或群組。

> [!TIP]
> 您也可以選擇啟用 SpaceIQ 的 SAML 型單一登入，請遵循[SpaceIQ 單一登入教學](https://docs.microsoft.com/azure/active-directory/saas-apps/spaceiq-tutorial)課程中提供的指示。 單一登入可以與自動使用者布建分開設定，雖然這兩個功能彼此的補充

### <a name="to-configure-automatic-user-provisioning-for-spaceiq-in-azure-ad"></a>若要在 Azure AD 中設定 SpaceIQ 的自動使用者布建：
...
5. 在 [**管理員認證**] 區段下，于 [**租使用者 URL**] 中輸入 `https://api.spaceiq.com/scim`。 輸入稍早在**秘密權杖**中所取得的**SCIM Authentication Token**值。 按一下 [**測試連接**] 以確保 Azure AD 可以連接到 SpaceIQ。 如果連線失敗，請確定您的 SpaceIQ 帳戶具有系統管理員許可權，然後再試一次。
...
9. 在 [**屬性對應**] 區段中，檢查從 Azure AD 同步處理到 SpaceIQ 的使用者屬性。 選取為 [比對] 屬性**的屬性會**用來比對 SpaceIQ 中的使用者帳戶，以進行更新作業。 選取 [儲存] 按鈕以認可所有變更。
...
12. 若要啟用 SpaceIQ 的 Azure AD 布建服務，請在 [**設定**] 區段中將 [布建**狀態**] 變更為 [**開啟**]。
...
此作業會對在 [設定] 區段的 [範圍] 中定義的所有使用者和/或群組，啟動首次同步處理。 初始同步處理會比後續同步處理花費更多時間執行，只要 Azure AD 佈建服務正在執行，這大約每 40 分鐘便會發生一次。 您可以使用 [**同步處理詳細資料**] 區段來監視進度，並遵循連結來布建活動報告，其中描述 SpaceIQ 上的 Azure AD 布建服務所執行的所有動作。
