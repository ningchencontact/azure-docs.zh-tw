---
title: 教學課程：Azure Active Directory 單一登入 (SSO) 與 Watch by Colors 整合 | Microsoft Docs
description: 了解如何設定 Azure Active Directory 與 Watch by Colors 之間的單一登入。
...
* 已啟用 Watch by Colors 單一登入 (SSO) 的訂用帳戶。
...
## <a name="adding-watch-by-colors-from-the-gallery"></a>從資源庫新增 Watch by Colors

若要進行將 Watch by Colors 整合到 Azure AD 中的設定，您必須從資源庫將 Watch by Colors 新增至受控 SaaS 應用程式清單。
...
以名為 **B.Simon** 的測試使用者，設定及測試與 Watch by Colors 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 Watch by Colors 中相關使用者之間的連結關聯性。

若要設定及測試與 Watch by Colors 搭配運作的 Azure AD SSO，請完成下列建置組塊：

1. **[設定 Azure AD SSO](#configure-azure-ad-sso)** - 讓您的使用者能夠使用此功能。
    1. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 B.Simon 測試 Azure AD 單一登入。
    1. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 B.Simon 能夠使用 Azure AD 單一登入。
1. **[設定 Watch by Colors SSO](#configure-watch-by-colors-sso)** - 在應用程式端設定單一登入設定。
    1. **[建立 Watch by Colors 測試使用者](#create-watch-by-colors-test-user)** - 使 Watch by Colors 中對應的 B.Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Watch by Colors]  應用程式整合頁面上，尋找 [管理]  區段並選取 [單一登入]  。
...
1. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]  ，然後執行下列步驟：
    ...
    在 [登入 URL]  文字方塊中，輸入 URL：`https://app.colorscorporation.com/login`
...
在本節中，您會將 Watch by Colors 的存取權授與 B.Simon，讓她能夠使用 Azure 單一登入。

1. 在 Azure 入口網站中，選取 [企業應用程式]  ，然後選取 [所有應用程式]  。
1. 在應用程式清單中，選取 [Watch by Colors]  。
...
5. 在 [SSO (單一登入)]  頁面上，執行下列步驟：
    ...
    b. 在 [URL]  文字方塊中，貼上您從 Azure 入口網站複製的 [同盟中繼資料 URL]  。

    c. 按一下 [匯入]  ，就會自動在頁面上自動填入下列欄位。

    d. 按一下 [檔案]  。

### <a name="create-watch-by-colors-test-user"></a>建立 Watch by Colors 測試使用者

若要讓 Azure AD 使用者能夠登入 Watch by Colors，必須將他們佈建到 Watch by Colors 中。 在 Watch by Colors 中，佈建是手動工作。

**若要佈建使用者帳戶，請執行下列步驟：**

1. 以安全性系統管理員身分登入 Watch by Colors。

1. 在頁面右上角按一下 **設定檔** > [使用者]   > [新增使用者]  。

    ![Watch by Colors 組態](./media/watch-by-colors-tutorial/config03.png)

1. 在 [使用者詳細資料]  頁面上，執行下列步驟：

    ![Watch by Colors 組態](./media/watch-by-colors-tutorial/config04.png)
...
當您在存取面板中按一下 [Watch by Colors] 圖格時，應該會自動登入您已設定 SSO 的 Watch by Colors。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
