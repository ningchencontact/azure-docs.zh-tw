...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Palo Alto 網路 - GlobalProtect (單一登入)。
...
4. 在搜尋方塊中，輸入 **Palo Alto 網路 - GlobalProtect**，從結果面板選取 [Palo Alto 網路 - GlobalProtect]，然後按一下 [新增] 按鈕以新增應用程式。
     ...
     ![結果清單中的 Palo Alto 網路 - GlobalProtect](common/search-new-app.png)
...
5. **[建立 Palo Alto 網路 - GlobalProtect 測試使用者](#create-palo-alto-networks---globalprotect-test-user)** - 使 [Palo Alto 網路 - GlobalProtect ] 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
5. Palo Alto 網路 - GlobalProtect 應用程式需要特定格式的 SAML 判斷提示。 設定此應用程式的下列宣告。 您可以在應用程式整合頁面的 [使用者屬性] 區段中，管理這些屬性的值。 在 [以 SAML 設定單一登入] 頁面上，按一下 [編輯] 按鈕以開啟 [使用者屬性] 對話方塊。
...
8. 在 [安裝 Palo Alto 網路 - GlobalProtect] 區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-palo-alto-networks---globalprotect-single-sign-on"></a>設定 Palo Alto 網路 - GlobalProtect 單一登入
