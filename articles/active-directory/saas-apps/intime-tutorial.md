...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 InTime (單一登入)。
...
4. 在搜尋方塊中，輸入 **InTime**，從結果面板中選取 [InTime]  ，然後按一下 [新增]  按鈕以新增應用程式。
     ...
     ![結果清單中的 InTime](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入

在本節中，您會以名為 **Britta Simon** 的測試使用者身分，使用 InTime 設定及測試 Azure AD 單一登入。
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 InTime 中相關使用者之間的連結關聯性。

若要使用 InTime 設定及測試與 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 InTime 單一登入](#configure-intime-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 InTime 測試使用者](#create-intime-test-user)** - 使 InTime 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [InTime]  應用程式整合頁面上，選取 [單一登入]  。
...
7. 在 [設定 InTime]  區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-intime-single-sign-on"></a>設定 InTime 單一登入

若要在 **InTime** 端設定單一登入，您必須將從 Azure 入口網站下載的 [同盟中繼資料 XML]  和所複製的適當 URL 傳送給 [ 支援小組](mailto:hdollard@intimesoft.com)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]  、[所有應用程式]  及 [InTime]  。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [InTime]  。

    ![應用程式清單中的 InTime 連結](common/all-applications.png)
...
### <a name="create-intime-test-user"></a>建立 InTime 測試使用者
...
當您在存取面板中按一下 InTime 圖格時，應該會自動登入您已設定 SSO 的 InTime。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
