...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 The Funding Portal (單一登入)。
...
4. 在搜尋方塊中，輸入 **The Funding Portal**，從結果面板中選取 **The Funding Portal**，然後按一下 [新增] 按鈕以新增應用程式。

     ![結果清單中的 The Funding Portal](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入

在本節中，您會以名為 **Britta Simon** 的測試使用者為基礎，設定及測試與 The Funding Portal 搭配運作的 Azure AD 單一登入。
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 The Funding Portal 中相關使用者之間的連結關聯性。

若要搭配 The Funding Portal 來設定及測試 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 The Funding Portal 單一登入](#configure-the-funding-portal-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 The Funding Portal 測試使用者](#create-the-funding-portal-test-user)** - 使 The Funding Portal 中 Britta Simon 的對應使用者連結到該使用者在 Azure AD 中的代表身分。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [The Funding Portal] 應用程式整合頁面上，選取 [單一登入]。
...
5. The Funding Portal 應用程式預期 SAML 判斷提示會包含名為 "externalId1" 的屬性。 "externalId1" 的值應該是可辨識的 studentID。 請設定此應用程式的 "externalId1" 宣告。 您可以從應用程式的 [使用者屬性] 管理這些屬性的值。 以下螢幕擷取畫面顯示上述的範例。
...
8. 在 [設定 The Funding Portal] 區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-the-funding-portal-single-sign-on"></a>設定 The Funding Portal 單一登入
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [The Funding Portal]。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [The Funding Portal] 。

    ![應用程式清單中的 The Funding Portal 連結](common/all-applications.png)
