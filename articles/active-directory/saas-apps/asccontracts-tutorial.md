...
5. **[建立 ASC Contracts 測試使用者](#create-asc-contracts-test-user)** - 在 ASC Contracts 中建立一個與 Azure AD 中代表 Britta Simon 之使用者連結的 Britta Simon 對應項目。
...
若要設定與 ASC Contracts 搭配運作的 Azure AD 單一登入，請執行下列步驟：

1. 在 [Azure 入口網站](https://portal.azure.com/)的 [ASC Contracts]  應用程式整合頁面上，選取 [單一登入]  。
...
4. 在 [以 SAML 設定單一登入]  頁面上，執行下列步驟：
    ...
    ![ASC Contracts 網域及 URL 單一登入資訊](common/idp-intiated.png)

    a. 在 [識別碼]  文字方塊中，使用下列模式來輸入 URL：`https://<subdomain>.asccontracts.com/shibboleth`
...
6. 在 [設定 ASC Contracts]  區段上，依據您的需求複製適當的 URL。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]  、[所有應用程式]  及 [ASC Contracts]  。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [ASC Contracts]  。

    ![應用程式清單中的 ASC Contracts 連結](common/all-applications.png)
