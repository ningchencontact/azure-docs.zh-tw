...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Cimpl (單一登入)。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Cimpl]  應用程式整合頁面上，選取 [單一登入]  。
...
4. 在 [基本 SAML 組態]  區段上，執行下列步驟：
    ...
    b. 在 [識別碼 (實體識別碼)]  文字方塊中，使用下列模式輸入 URL：`https://sso.etelesolv.com/<TENANTNAME>`
...
6. 在 [設定 Cimpl]  區段上，依據您的需求複製適當的 URL。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]  、[所有應用程式]  及 [Cimpl]  。
...
當您在存取面板中按一下 [Cimpl] 圖格時，應該會自動登入您已設定 SSO 的 Cimpl。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
