---
title: 教學課程：Azure Active Directory 與 Wandera 整合 | Microsoft Docs
description: 了解如何設定 Azure Active Directory 與 Wandera 之間的單一登入。
...
在本教學課程中，您會了解如何將 Wandera 與 Azure Active Directory (Azure AD) 進行整合。 在整合 Wandera 與 Azure AD 時，您可以︰

* 在 Azure AD 中控制可存取 Wandera 的人員。
* 讓使用者使用其 Azure AD 帳戶自動登入 Wandera。
...
* Wandera 支援由 **IDP** 起始的 SSO

## <a name="adding-wandera-from-the-gallery"></a>從資源庫新增 Wandera

若要設定將 Wandera 整合到 Azure AD 中，您需要從資源庫將 Wandera 新增到受控 SaaS 應用程式清單。

1. 使用公司或學校帳戶或個人的 Microsoft 帳戶登入 [Azure 入口網站](https://portal.azure.com)。
1. 在左方瀏覽窗格上，選取 [Azure Active Directory]  服務。
1. 巡覽至 [企業應用程式]  ，然後選取 [所有應用程式]  。
1. 若要新增應用程式，請選取 [新增應用程式]  。
1. 在 [從資源庫新增]  區段的搜尋方塊中輸入 **Wandera**。
1. 從結果面板選取 [Wandera]  ，然後新增應用程式。 當應用程式新增至您的租用戶時，請等候幾秒鐘。


## <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入

以名為 **B.Simon** 的測試使用者，設定及測試與 Wandera 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 Wandera 中相關使用者之間的連結關聯性。

若要設定及測試與 Wandera 搭配運作的 Azure AD SSO，請完成下列建置組塊：
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Wandera]  應用程式整合頁面上，尋找 [管理]  區段並選取 [單一登入]  。
...
1. 在 [基本 SAML 組態]  區段上，輸入下列欄位的值：
    ...
    在 [回覆 URL]  文字方塊中，使用下列模式來輸入 URL：`https://radar.wandera.com/saml/acs/<tenant id>`
...
1. 在 [以 SAML 設定單一登入]  頁面上，按一下 [SAML 簽署憑證]  的編輯/畫筆圖示，以編輯設定。

    ![簽署選項](common/signing-option.png)

    1. 針對 [簽署選項]  選取 [簽署 SAML 回應及判斷提示]  。

    1. 將 [簽署演算法]  選取為 [SHA-256]  。

1. 在 [設定 Wandera]  區段上，根據您的需求複製適當的 URL。
...
在本節中，您會將 Wandera 的存取權授與 B.Simon，讓其能夠使用 Azure 單一登入。

1. 在 Azure 入口網站中，選取 [企業應用程式]  ，然後選取 [所有應用程式]  。
1. 在應用程式清單中，選取 [Wandera]  。
...
當您在存取面板中按一下 [Wandera] 圖格時，應該會自動登入您已設定 SSO 的 Wandera。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
