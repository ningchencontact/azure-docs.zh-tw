...
1. 在搜尋方塊中，輸入 **etouches**，從結果面板中選取 [etouches]，然後按一下 [新增] 按鈕以新增應用程式。

    ![結果清單中的 etouches](./media/etouches-tutorial/tutorial_etouches_addfromgallery.png)
...
在 etouches 中，將 Azure AD 中**使用者名稱**的值指派為 **Username** 的值，以建立連結關聯性。

若要設定及測試與 etouches 搭配運作的 Azure AD 單一登入，您需要完成下列建構元素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
1. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
1. **[建立 etouches 測試使用者](#create-an-etouches-test-user)** - 在 etouches 中建立一個與 Azure AD 中代表使用者之項目連結的 Britta Simon 對應項目。
1. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
1. **[測試單一登入](#test-single-sign-on)** - 驗證組態是否能運作。

### <a name="configure-azure-ad-single-sign-on"></a>設定 Azure AD 單一登入

在本節中，您會於 Azure 入口網站啟用 Azure AD 單一登入，然後在您的 etouches 應用程式中設定單一登入。

**若要使用 etouches 設定 Azure AD 單一登入，請執行下列步驟：**

1. 在 Azure 入口網站的 [etouches] 應用程式整合頁面上，按一下 [單一登入]。

    ![設定單一登入][4]

1. 在 [單一登入] 對話方塊上，於 [模式] 選取 [SAML 登入]，以啟用單一登入。
 
    ![單一登入對話方塊](./media/etouches-tutorial/tutorial_etouches_samlbase.png)

1. 在 [etouches 網域與 URL] 區段上，執行下列步驟：

    ![etouches 網域及 URL 單一登入資訊](./media/etouches-tutorial/tutorial_etouches_url.png)

    a. 在 [登入 URL] 文字方塊中，使用下列模式輸入 URL︰ `https://www.eiseverywhere.com/saml/accounts/?sso&accountid=<ACCOUNTID>`

    b. 在 [識別碼] 文字方塊中，使用下列模式輸入 URL： `https://www.eiseverywhere.com/<instance name>`

    > [!NOTE] 
    > 這些都不是真正的值。 您將使用實際的「登入 URL」與「識別碼」來更新值，稍後會在本教學課程中說明。
    > 

1. etouches 應用程式需要特定格式的 SAML 判斷提示。 設定此應用程式的下列宣告。 您可以從應用程式的 [使用者屬性] 管理這些屬性的值。 以下螢幕擷取畫面顯示上述的範例。 

    ![使用者屬性](./media/etouches-tutorial/tutorial_etouches_attribute.png) 
...
1. 在 [單一登入] 對話方塊的 [使用者屬性] 區段中，如圖所示設定 SAML 權杖屬性，然後執行下列步驟：
    ...
    ![新增屬性](./media/etouches-tutorial/tutorial_attribute_04.png)

    ![[新增屬性] 對話方塊](./media/etouches-tutorial/tutorial_attribute_05.png)
...
1. 在 [SAML 簽署憑證] 區段上，按一下 [中繼資料 XML]，然後將中繼資料檔案儲存在您的電腦上。
    ...
    ![憑證下載連結](./media/etouches-tutorial/tutorial_etouches_certificate.png) 

1. 按一下 [儲存]  按鈕。

    ![設定單一登入儲存按鈕](./media/etouches-tutorial/tutorial_general_400.png)
...
1. 為了設定應用程式的 SSO，請在 etouches 應用程式中執行下列步驟： 
    ...
    f. 這會開啟網頁，並執行 SSO。 一旦 SSO 開始運作，您就可以設定使用者名稱。

    g. 在 [使用者名稱] 欄位中選取**電子郵件地址**，如下圖所示。 

    h. 複製**預存程序實體識別碼**值，並將其貼至 [識別碼] 文字方塊中 (位於 Azure 入口網站的 [etouches 網域與 URL] 區段)。

    i. 複製 **SSO URL / ACS** 值，並將其貼至 [單一登入 URL] 文字方塊中 (位於 Azure 入口網站的 [etouches 網域與 URL] 區段)。
...
1. 在 **Azure 入口網站**的左方瀏覽窗格中，按一下 [Azure Active Directory] 圖示。
    ...
    ![Azure Active Directory 按鈕](./media/etouches-tutorial/create_aaduser_01.png) 

1. 若要顯示使用者清單，請移至 [使用者和群組]，然後按一下 [所有使用者]。
    
    ![[使用者和群組] 與 [所有使用者] 連結](./media/etouches-tutorial/create_aaduser_02.png) 

1. 若要開啟 [使用者] 對話方塊，按一下對話方塊頂端的 [新增]。
 
    ![[新增] 按鈕](./media/etouches-tutorial/create_aaduser_03.png) 

1. 在 [使用者]  對話頁面上，執行下列步驟：
 
    ![[使用者] 對話方塊](./media/etouches-tutorial/create_aaduser_04.png) 
...
1. 在應用程式清單中，選取 [etouches] 。

    ![應用程式清單中的 etouches 連結](./media/etouches-tutorial/tutorial_etouches_app.png) 
