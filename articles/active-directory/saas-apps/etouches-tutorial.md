...
3. **[建立 etouches 測試使用者](#create-an-etouches-test-user)** - 在 etouches 中建立一個與 Azure AD 中代表使用者之項目連結的 Britta Simon 對應項目。
...
3. 在 [etouches 網域與 URL] 區段上，執行下列步驟：
...
4. etouches 應用程式需要特定格式的 SAML 判斷提示。 設定此應用程式的下列宣告。 您可以從應用程式的 [使用者屬性] 管理這些屬性的值。 以下螢幕擷取畫面顯示上述的範例。 
...
8. 為了設定應用程式的 SSO，請在 etouches 應用程式中執行下列步驟： 
