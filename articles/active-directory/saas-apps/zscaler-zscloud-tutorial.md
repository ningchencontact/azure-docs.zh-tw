...
1. **[建立 Zscaler ZSCloud 測試使用者](#creating-a-zscaler-zscloud-test-user)** - 使 Zscaler ZSCloud 中 Britta Simon 的對應使用者連結到該使用者在 Azure AD 中的代表身分。
...
1. 在 [Zscaler ZSCloud 網域與 URL] 區段上，執行下列步驟：
...
1. 在 [Zscaler ZSCloud 設定] 區段中，按一下 [設定 Zscaler ZSCloud] 開啟 [設定登入] 視窗。 從 [快速參考] 區段中複製 [SAML 單一登入服務 URL]。

    ![設定單一登入](./media/zscaler-zscloud-tutorial/tutorial_zscalerzscloud_configure.png) 

1. 在不同的 Web 瀏覽器視窗中，以管理員身分登入您的 ZScaler ZSCloud 公司網站。
