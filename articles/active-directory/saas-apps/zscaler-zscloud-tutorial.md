...
4. 在搜尋方塊中，輸入 **Zscaler ZSCloud**，從結果面板中選取 [Zscaler ZSCloud]，然後按一下 [新增] 按鈕以新增應用程式。
    ...
    ![結果清單中的 Zscaler ZSCloud](./media/zscaler-zscloud-tutorial/tutorial_zscalerzscloud_addfromgallery.png)
...
4. 在 [基本 SAML 組態] 區段上，執行下列步驟：
    ...
    ![Zscaler ZSCloud 網域和 URL 單一登入資訊](./media/zscaler-zscloud-tutorial/tutorial_zscalerzscloud_url.png)

    在 [登入 URL] 文字方塊中，鍵入使用者用來登入您 ZScaler ZSCloud 的 URL。

    > [!NOTE] 
    > 您必須使用實際的「登入 URL」來更新此值。 請連絡 [Zscaler Two 用戶端支援小組](https://help.zscaler.com/)以取得這些值。

5. Zscaler ZSCloud 應用程式需要特定格式的 SAML 判斷提示。 設定此應用程式的下列宣告。 您可以在應用程式整合頁面的 [使用者屬性與宣告] 區段中管理這些屬性的值。 在 [以 SAML 設定單一登入] 頁面上，按一下 [編輯] 按鈕以開啟 [使用者屬性與宣告] 對話方塊。

    ![屬性連結](./media/zscaler-zscloud-tutorial/tutorial_zscalerzscloud_attribute.png)
...
7. 在 [SAML 簽署憑證] 頁面上的 [SAML 簽署憑證] 區段中，按一下 [下載] 以下載**憑證 (Base64)**，然後將憑證檔案儲存在電腦上。
    ...
    ![憑證下載連結](./media/zscaler-zscloud-tutorial/tutorial_zscalerzscloud_certificate.png) 

8. 在 [設定 Zscaler ZSCloud] 區段上，依據需求複製適當的 URL。
    ...
    ![Zscaler ZSCloud 設定](common/configuresection.png)

9. 在不同的 Web 瀏覽器視窗中，以管理員身分登入您的 Zscaler ZSCloud 公司網站。
...
12. 在 [設定使用者驗證]  對話方塊頁面上執行下列步驟：
    ...
    ![系統管理](./media/zscaler-zscloud-tutorial/ic800207.png)
...
本節目標是在 Zscaler ZSCloud 中建立名為 Britta Simon 的使用者。 Zscaler ZSCloud 支援預設啟用的 Just-In-Time 佈建。 在這一節沒有您需要進行的動作項目。 嘗試存取 Zscaler ZSCloud 時，如果使用者還不存在，就會建立新使用者。
>[!Note]
>如果您需要手動建立使用者，請連絡  [Zscaler ZSCloud 支援小組](https://help.zscaler.com/) \(英文\)。
...
5. 在 [使用者和群組] 對話方塊中，從清單中選取使用者 (例如 **Britta Simon**)，然後按一下畫面底部的 [選取] 按鈕。
    ...
    ![映像](./media/zscaler-zscloud-tutorial/tutorial_zscalerzscloud_users.png)

6. 從 [選取角色] 對話方塊的清單中選擇適當的使用者角色，然後按一下畫面底部的 [選取] 按鈕。

    ![映像](./media/zscaler-zscloud-tutorial/tutorial_zscalerzscloud_roles.png)

7. 在 [新增指派] 對話方塊中，選取 [指派] 按鈕。

    ![映像](./media/zscaler-zscloud-tutorial/tutorial_zscalerzscloud_assign.png)
