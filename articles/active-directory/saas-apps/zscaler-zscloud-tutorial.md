...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Zscaler ZSCloud (單一登入)。
...
在本節中，您將以名為 **Britta Simon** 的測試使用者為基礎，設定及測試與 Zscaler ZSCloud 搭配運作的 Azure AD 單一登入。
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 Zscaler ZSCloud 中相關使用者之間的連結關聯性。

若要使用 Zscaler ZSCloud 來設定並測試 Azure AD 單一登入，您需要完成下列建置組塊：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 Zscaler ZSCloud 單一登入](#configure-zscaler-zscloud-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 Zscaler ZSCloud 測試使用者](#create-zscaler-zscloud-test-user)** - 使 Zscaler ZSCloud 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Zscaler ZSCloud]  應用程式整合頁面上，選取 [單一登入]  。
...
4. 在 [基本 SAML 組態]  區段上，執行下列步驟：
    ...
    ![Zscaler ZSCloud 網域和 URL 單一登入資訊](common/sp-signonurl.png)
...
5. Zscaler ZSCloud 應用程式會預期特定格式的 SAML 判斷提示，因此您必須將自訂屬性對應新增至您的 SAML 權杖屬性設定。 以下螢幕擷取畫面顯示預設屬性清單。 按一下 [編輯] ****  圖示，以開啟 [使用者屬性] ****  對話方塊。
...
### <a name="configure-zscaler-zscloud-single-sign-on"></a>設定 Zscaler ZSCloud 單一登入
...
當您在存取面板中按一下 [Zscaler ZSCloud] 圖格時，應該會自動登入您已設定 SSO 的 Zscaler ZSCloud。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
