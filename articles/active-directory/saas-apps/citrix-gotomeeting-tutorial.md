...
4. 在搜尋方塊中，輸入 **GoToMeeting**，從結果面板中選取 [GoToMeeting]，然後按一下 [新增] 按鈕以新增應用程式。
     ...
     ![結果清單中的 GoToMeeting](common/search-new-app.png)
...
5. **[建立 GoToMeeting 測試使用者](#create-gotomeeting-test-user)** - 使 GoToMeeting 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
4. 在 [以 SAML 設定單一登入] 頁面上，按一下 [編輯] 按鈕以開啟 [基本 SAML 組態] 對話方塊。
    ...
    b. 在 [回覆 URL] 文字方塊中，使用下列模式來輸入 URL：`https://authentication.logmeininc.com/saml/acs`

    c. 按一下 [設定其他 URL] 並設定以下 URL
...
6. 在 [設定 GoToMeeting] 區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-gotomeeting-single-sign-on"></a>設定 GoToMeeting 單一登入

1. 在不同的瀏覽器視窗中，登入您的 [GoToMeeting 組織中心](https://organization.logmeininc.com/)。 系統會提示您確認 IdP 已更新。

2. 啟用 [我的識別提供者已使用新網域更新] 核取方塊。 完成後，請按一下 [完成]。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [GoToMeeting]。
...
### <a name="create-gotomeeting-test-user"></a>建立 GoToMeeting 測試使用者
