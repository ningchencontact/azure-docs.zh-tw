...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Intralinks (單一登入)。
...
4. 在搜尋方塊中，輸入 **Intralinks**，從結果面板中選取 [Intralinks]  ，然後按一下 [新增]  按鈕以新增應用程式。

     ![結果清單中的 Intralinks](common/search-new-app.png)
...
5. **[建立 Intralinks 測試使用者](#create-intralinks-test-user)** - 在 Intralinks 中建立一個與 Azure AD 中代表 Britta Simon 之使用者連結的 Britta Simon 對應項目。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Intralinks]  應用程式整合頁面上，選取 [單一登入]  。
...
4. 在 [基本 SAML 組態]  區段上，執行下列步驟：
    ...
    ![Intralinks 網域及 URL 單一登入資訊](common/sp-signonurl.png)

    在 [登入 URL]  文字方塊中，以下列模式輸入 URL︰`https://<company name>.Intralinks.com/?PartnerIdpId=https://sts.windows.net/<AzureADTenantID>`

    > [!NOTE]
    > 這不是真正的值。 請使用實際的「登入 URL」來更新此值。 請連絡 [Intralinks 用戶端支援小組](https://www.intralinks.com/contact)以取得此值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。

5. 在 [以 SAML 設定單一登入]  頁面的 [SAML 簽署憑證]  區段中按一下 [下載]  ，以依據您的需求從指定選項下載**同盟中繼資料 XML**，並儲存在您的電腦上。

    ![憑證下載連結](common/metadataxml.png)

6. 在 [安裝 Intralinks]  區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-intralinks-single-sign-on"></a>設定 Intralinks 單一登入

若要在 **Intralinks** 端設定單一登入，您必須將從 Azure 入口網站下載的 [同盟中繼資料 XML]  和複製的適當 URL 傳送給 [Intralinks 支援小組](https://www.intralinks.com/contact)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]  、[所有應用程式]  及 [Intralinks]  。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [Intralinks]  。

    ![應用程式清單中的 [Intralinks] 連結](common/all-applications.png)
...
當您在存取面板中按一下 [Intralinks] 圖格時，應該會自動登入您已設定 SSO 的 Intralinks。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
