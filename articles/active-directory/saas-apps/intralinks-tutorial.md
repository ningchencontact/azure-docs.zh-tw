...
3. **[建立 Intralinks 測試使用者](#creating-an-intralinks-test-user)** - 使 Intralinks 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
3. 在 [Intralinks 網域及 URL] 區段中，執行下列步驟：
...
6. 若要在 **Intralinks** 端設定單一登入，您必須將已下載的**中繼資料 XML** 傳送給 [Intralinks 支援小組](https://www.intralinks.com/contact-1)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
3. 若要開啟 [使用者] 對話方塊，按一下對話方塊頂端的 [新增]。
    ...
    ![建立 Azure AD 測試使用者](./media/intralinks-tutorial/create_aaduser_03.png) 
...
5. 在 [Intralinks 新增應用程式] 上，執行下列步驟：
...
6.  在 Azure 入口網站的 [Intralinks] 應用程式整合頁面上，按一下 [單一登入]。

    ![設定單一登入][4]

7. 在 [單一登入] 對話方塊中，選取 [連結的登入] 作為 [模式]。
 
    ![設定單一登入](./media/intralinks-tutorial/tutorial_intralinks_linkedsignon.png)

8. 從 [Intralinks 小組](https://www.intralinks.com/contact-1)取得其他 Intralinks 應用程式之 SP 起始的 SSO URL，然後在 [設定登入 URL] 中輸入此 URL，如下所示。 
...
10. 將應用程式指派給使用者或群組，如**[指派 Azure AD 測試使用者](#assigning-the-azure-ad-test-user)** 一節所示。
