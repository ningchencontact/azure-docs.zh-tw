...
在本教學課程中，您將了解如何整合 PureCloud by Genesys 與 Azure Active Directory (Azure AD)。 在整合 PureCloud by Genesys 與 Azure AD 時，您可以︰

* 在 Azure AD 中控制可存取 PureCloud by Genesys 的人員。
...
1. **[設定 PureCloud by Genesys SSO](#configure-purecloud-by-genesys-sso)** - 在應用程式端設定單一登入設定。
    1. **[建立 PureCloud by Genesys 測試使用者](#create-purecloud-by-genesys-test-user)** - 在 PureCloud by Genesys 中建立 B.Simon 的對應項目，且該項目須與 Azure AD 中代表使用者的項目連結。
...
1. 在 [設定 PureCloud by Genesys]  區段上，依據您的需求複製適當的 URL。
...
在本節中，您會將 PureCloud by Genesys 的存取權授與 B.Simon，讓其能夠使用 Azure 單一登入。
...
1. 按一下頂端的 [系統管理員]  ，然後瀏覽至 [整合]  底下的 [單一登入]  。

    ![設定單一登入](./media/purecloud-by-genesys-tutorial/configure01.png)

1. 切換至 [ADFS/Azure AD(Premium)]  索引標籤，然後執行下列步驟：
...
1. 按一下頂端的 [系統管理員]  ，然後瀏覽至 [人員和權限]  底下的 [人員]  。
