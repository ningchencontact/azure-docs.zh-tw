...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Fuze (單一登入)。
...
* Fuze 支援 **Just In Time** 使用者佈建

## <a name="adding-fuze-from-the-gallery"></a>從資源庫新增 Fuze

若要設定將 Fuze 整合到 Azure AD 中，您需要從資源庫將 Fuze 新增到受控 SaaS 應用程式清單中。
...
4. 在搜尋方塊中，輸入 **Fuze**，從結果面板中選取 [Fuze]  ，然後按一下 [新增]  按鈕以新增應用程式。
...
5. **[建立 Fuze 測試使用者](#create-fuze-test-user)** - 使 Fuze 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Fuze]  應用程式整合頁面上，選取 [單一登入]  。
...
6. 在 [設定 Fuze]  區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-fuze-single-sign-on"></a>設定 Fuze 單一登入

若要在 **Fuze** 端設定單一登入，您必須將從 Azure 入口網站下載的 [同盟中繼資料 XML]  和所複製的適當 URL 傳送給 [Fuze 支援小組](https://www.fuze.com/support)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]  、[所有應用程式]  及 [Fuze]  。
...
### <a name="create-fuze-test-user"></a>建立 Fuze 測試使用者

Fuze 應用程式支援 Just In Time 使用者佈建，所以使用者會在登入時自動建立。 如需任何其他詳細資料，請連絡 Fuze [支援](https://www.fuze.com/support)。

### <a name="test-single-sign-on"></a>測試單一登入 

在本節中，您會使用存取面板來測試您的 Azure AD 單一登入設定。

當您在存取面板中按一下 Fuze 圖格時，應該會自動登入您已設定 SSO 的 Fuze。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
