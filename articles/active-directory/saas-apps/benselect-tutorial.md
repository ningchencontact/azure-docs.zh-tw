...
* 在 Azure AD 中管控可存取 BenSelect 的人員。
* 讓使用者使用其 Azure AD 帳戶自動登入 BenSelect。
...
1. 在 [從資源庫新增]  區段的搜尋方塊中輸入 **BenSelect**。
...
1. 在 [基本 SAML 組態]  區段上，輸入下列欄位的值：
    ...
    在 [回覆 URL]  文字方塊中，使用下列模式來輸入 URL：`https://www.benselect.com/enroll/login.aspx?Path=<tenant name>`
...
1. 在 [管理使用者宣告]  區段上，執行下列步驟：
    ...
    c. 在 [參數 1]  下拉式清單中，選取 [user.userprincipalname]  。
...
在本節中，您會將 BenSelect 的存取權授與 B.Simon，讓其能夠使用 Azure 單一登入。
...
當您在存取面板中按一下 BenSelect 圖格時，應該會自動登入您已設定 SSO 的 BenSelect。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
