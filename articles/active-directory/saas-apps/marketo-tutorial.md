...
4. 在 [以 SAML 設定單一登入] 頁面上，執行下列步驟：
    ...
    > [!NOTE]
    > 這些都不是真正的值。 請使用實際的識別碼和回覆 URL 更新這些值。 請連絡[Marketo 用戶端支援小組](https://investors.marketo.com/contactus.cfm)以取得這些值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。
