...
若要設定 Azure AD 與 BorrowBox 的整合，您需要下列項目：
...
若要設定及測試與 BorrowBox 搭配運作的 Azure AD 單一登入，您需要完成下列構成要素：
...
本節會在 BorrowBox 中建立名為 Britta Simon 的使用者。 BorrowBox 支援預設會啟用的 Just-In-Time 使用者佈建。 在這一節沒有您需要進行的動作項目。 如果 BorrowBox 中還沒有使用者存在，在驗證之後就會建立新的使用者。
