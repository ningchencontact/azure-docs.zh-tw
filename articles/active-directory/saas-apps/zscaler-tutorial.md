...
5. 在結果窗格中，選取 [Zscaler]，然後按一下 [新增] 按鈕以新增應用程式。
...
4. **[建立 Zscaler 測試使用者](#creating-a-zscaler-test-user)** - 使 Zscaler 中 Britta Simon 的對應使用者連結到該使用者在 Azure AD 中的代表身分。
...
3. 在 [Zscaler 網域與 URL] 區段上，執行下列步驟：
...
6. 在 [Zscaler 設定] 區段中，按一下 [設定 Zscaler] 開啟 [設定登入] 視窗。 從 [快速參考] 區段中複製 [SAML 單一登入服務 URL]。

    ![設定單一登入](./media/zscaler-tutorial/tutorial_zscaler_configure.png) 

7. 在不同的 Web 瀏覽器視窗中，以系統管理員身分登入您的 Zscaler 公司網站。
