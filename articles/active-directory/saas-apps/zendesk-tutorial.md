...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Zendesk (單一登入)。
...
2. **[設定 Zendesk 單一登入](#configure-zendesk-single-sign-on)** - 在應用程式端設定單一登入設定。
...
4. 在 [基本 SAML 組態] 區段上，執行下列步驟：
    ...
    b. 在 [識別碼 (實體識別碼)] 文字方塊中，使用下列模式輸入 URL：`<subdomain>.zendesk.com`
