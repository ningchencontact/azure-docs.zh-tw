---
title: 教學課程：Azure Active Directory 與 Canvas 整合 | Microsoft Docs
description: 了解如何設定 Azure Active Directory 與 Canvas 之間的單一登入。
...
ms.locfileid: "54824332"
---
# <a name="tutorial-azure-active-directory-integration-with-canvas"></a>教學課程：Azure Active Directory 與 Canvas 整合

在本教學課程中，您會了解如何將 Canvas 與 Azure Active Directory (Azure AD) 整合。
將 Canvas 與 Azure AD 整合可提供下列優點：

* 您可以在 Azure AD 中控制可存取 Canvas 的人員。
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Canvas (單一登入)。
...
* 已啟用 Canvas 單一登入的訂用帳戶

## <a name="scenario-description"></a>案例描述

在本教學課程中，您會在測試環境中設定和測試 Azure AD 單一登入。

* Canvas 支援 **SP** 起始的 SSO
...
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 Canvas 中相關使用者之間的連結關聯性。

若要設定及測試與 Canvas 搭配運作的 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 Canvas 單一登入](#configure-canvas-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 Canvas 測試使用者](#create-canvas-test-user)** - 在 Canvas 中建立一個與 Azure AD 中代表 Britta Simon 之項目連結的 Britta Simon 對應項目。
...
若要設定與 Canvas 搭配運作的 Azure AD 單一登入，請執行下列步驟：

1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Canvas] 應用程式整合頁面上，選取 [單一登入]。
...
4. 在 [基本 SAML 組態] 區段上，執行下列步驟：
    ...
    ![Canvas 網域及 URL 單一登入資訊](common/sp-identifier.png)

    a. 在 [登入 URL] 文字方塊中，使用下列模式輸入 URL：`https://<tenant-name>.instructure.com`

    b. 在 [識別碼 (實體識別碼)] 文字方塊中，使用下列模式輸入 URL：`https://<tenant-name>.instructure.com/saml2`

    > [!NOTE]
    > 這些都不是真正的值。 請使用實際的「登入 URL」及「識別碼」來更新這些值。 請連絡 [Canvas 用戶端支援小組](https://community.canvaslms.com/community/help)以取得這些值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。
...
7. 在 [安裝 Canvas] 區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-canvas-single-sign-on"></a>設定 Canvas 單一登入
...
4. 在 [目前的整合] 頁面上，執行下列步驟：
    ...
    d. 在 [變更密碼連結] 文字方塊中，貼上您從 Azure 入口網站複製的 [變更密碼 URL] 值。

    e. 在 [憑證指紋] 文字方塊中，貼上您從 Azure 入口網站複製的憑證 [指紋] 值。

    f. 從 [登入屬性] 清單中選取 [NameID]。

    g. 從 [識別碼格式] 清單中選取 [emailAddress]。
...
在本節中，您會將 Canvas 的存取權授與 Britta Simon，讓她能夠使用 Azure 單一登入。

1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [Canvas]。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [Canvas]。

    ![應用程式清單中的 Canvas 連結](common/all-applications.png)
...
### <a name="create-canvas-test-user"></a>建立 Canvas 測試使用者

若要讓 Azure AD 使用者能夠登入 Canvas，必須將他們佈建到 Canvas。 就 Canvas 而言，需以手動方式佈建使用者。
...
當您在存取面板中按一下 [Canvas] 圖格時，應該會自動登入您設定 SSO 的 Canvas。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
