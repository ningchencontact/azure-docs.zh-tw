...
1. 在 [基本 SAML 組態]  區段上，輸入下列欄位的值：
    ...
    > [!NOTE]
    > 這不是真正的值。 請使用實際的「登入 URL」來更新此值。 請連絡 [BlueJeans for Azure AD 用戶端支援小組](https://support.bluejeans.com/contact)以取得此值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。
...
當您在存取面板中按一下 [BlueJeans for Azure AD] 圖格時，應該會自動登入您已設定 SSO 的 BlueJeans for Azure AD。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
