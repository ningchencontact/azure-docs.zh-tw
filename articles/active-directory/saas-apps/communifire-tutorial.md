...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Communifire (單一登入)。
...
* 已啟用 Communifire 單一登入的訂用帳戶

## <a name="scenario-description"></a>案例描述

在本教學課程中，您會在測試環境中設定和測試 Azure AD 單一登入。

* Communifire 支援由 **SP 和 IDP** 起始的 SSO

* Communifire 支援 **Just In Time** 使用者佈建
...
5. **[建立 Communifire 測試使用者](#create-communifire-test-user)** - 使 Communifire 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
若要設定與 Communifire 搭配運作的 Azure AD 單一登入，請執行下列步驟：

1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Communifire] 應用程式整合頁面上，選取 [單一登入]。
...
### <a name="configure-communifire-single-sign-on"></a>設定 Communifire 單一登入

若要在 **Communifire** 端設定單一登入，您必須將從 Azure 入口網站下載的 [同盟中繼資料 XML] 和複製的適當 URL 傳送給 [Communifire 支援小組](mailto:support@axerosolutions.com)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [Communifire]。
...
當您在存取面板中按一下 [Communifire] 圖格時，應該會自動登入您已設定 SSO 的 Communifire。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
