...
3. **[建立 PolicyStat 測試使用者](#creating-a-policystat-test-user)** - 使 PolicyStat 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
3. 在 [PolicyStat 網域及 URL] 區段中，執行下列步驟：
...
5. 本節的目的是要說明如何依據 SAML 通訊協定來使用同盟，讓使用者能夠用自己在 Azure AD 中的帳戶在 PolicyStat 中進行驗證。
...
12. 按一下 [您的 IDP 中繼資料]，然後在 [您的 IDP 中繼資料] 區段中，執行下列步驟：
