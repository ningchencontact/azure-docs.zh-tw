...
ms.locfileid: "65989397"
---
# <a name="tutorial-integrate-freshservice-with-azure-active-directory"></a>教學課程：整合 Freshservice 與 Azure Active Directory
...
在本教學課程中，您會在測試環境中設定和測試 Azure AD SSO。 Freshservice 支援 **SP** 起始的 SSO
...
以名為 **Britta Simon** 的測試使用者，設定及測試與 Freshservice 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 Freshservice 中相關使用者之間的連結關聯性。

若要設定及測試與 Freshservice 搭配運作的 Azure AD SSO，請完成下列建置組塊：

1. **[設定 Azure AD SSO](#configure-azure-ad-sso)** - 讓您的使用者能夠使用此功能。
2. **[設定 Freshservice SSO](#configure-freshservice-sso)** - 在應用程式端設定單一登入設定。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Freshservice]  應用程式整合頁面上，尋找 [管理]  區段並選取 [單一登入]  。
...
1. 在 [設定 Freshservice]  區段上，依據您的需求複製適當的 URL。
...
2. 將擴充功能新增至瀏覽器之後，按一下 [設定 Freshservice ]  便會將您導向到 Freshservice 應用程式。 請從該處提供用以登入 Freshservice 的管理員認證。 瀏覽器擴充功能會自動為您設定應用程式，並自動執行步驟 3 到 6。

    ![設定組態](common/setup-sso.png)

3. 如果您想要手動設定 Freshservice，請開啟新的網頁瀏覽器視窗，並以系統管理員身分登入 Freshservice 公司網站，然後執行下列步驟：
...
當您在存取面板中選取 [Freshservice] 圖格時，應該會自動登入您已設定 SSO 的 Freshservice。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
