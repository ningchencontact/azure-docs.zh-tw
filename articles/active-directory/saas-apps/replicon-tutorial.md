...
在本教學課程中，您將了解如何整合 Replicon 與 Azure Active Directory (Azure AD)。 在整合 Replicon 與 Azure AD 時，您可以︰

* 在 Azure AD 中控制可存取 Replicon 的人員。
* 讓使用者使用其 Azure AD 帳戶自動登入 Replicon。
...
以名為 **B.Simon** 的測試使用者，設定及測試與 Replicon 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 Replicon 中相關使用者之間的連結關聯性。

若要設定及測試與 Replicon 搭配運作的 Azure AD SSO，請完成下列建置組塊：

1. **[設定 Azure AD SSO](#configure-azure-ad-sso)** - 讓您的使用者能夠使用此功能。
2. **[設定 Replicon SSO](#configure-replicon-sso)** - 在應用程式端設定單一登入設定。
...
5. **[建立 Replicon 測試使用者](#create-replicon-test-user)** - 在 Replicon 中建立 B.Simon 的對應項目，且該項目與 Azure AD 中代表使用者的項目連結。
...
1. 在 [基本 SAML 設定]  頁面上，輸入下列欄位的值：
    ...
    1. 在 [回覆 URL]  文字方塊中，使用下列模式來輸入 URL：`https://global.replicon.com/!/saml2/<client name>/sso/post`
...
### <a name="configure-replicon-sso"></a>設定 Replicon SSO
...
本節的目標是要在 Replicon 中建立一個名為 B.Simon 的使用者。
...
當您在存取面板中選取 Replicon 圖格時，應該會自動登入您已設定 SSO 的 Replicon。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
