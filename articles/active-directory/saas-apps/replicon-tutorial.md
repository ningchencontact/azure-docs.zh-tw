...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Replicon (單一登入)。
...
4. 在搜尋方塊中，輸入 **Replicon**，從結果面板中選取 [Replicon]  ，然後按一下 [新增]  按鈕以新增應用程式。
    ...
    ![結果清單中的 Replicon](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入

在本節中，您會以名為 **Britta Simon** 的測試使用者為基礎，使用 Replicon 來設定和測試 Azure AD 單一登入。
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 Replicon 中相關使用者之間的連結關聯性。

若要設定及測試與 Replicon 搭配運作的 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 Replicon 單一登入](#configure-replicon-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 Replicon 測試使用者](#create-replicon-test-user)** - 在 Replicon 中建立 Britta Simon 的對應項目，且該項目與 Azure AD 中代表使用者的項目連結。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Replicon]  應用程式整合頁面上，選取 [單一登入]  。
...
4. 在 [基本 SAML 組態]  區段上，執行下列步驟：
    ...
    ![Replicon 網域及 URL 單一登入資訊](common/sp-identifier-reply.png)

    a. 在 [登入 URL]  文字方塊中，以下列模式輸入 URL︰`https://na2.replicon.com/<companyname>/saml2/sp-sso/post`

    b. 在 [識別碼]  方塊中，使用下列模式輸入 URL：`https://global.replicon.com/<companyname>`

    c. 在 [回覆 URL]  文字方塊中，使用下列模式來輸入 URL：`https://global.replicon.com/!/saml2/<companyname>/sso/post`
...
6. 在 [設定 Replicon]  區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-replicon-single-sign-on"></a>設定 Replicon 單一登入

1. 在不同的網頁瀏覽器視窗中，以系統管理員身分登入您的 Replicon 公司網站。

2. 若要設定 SAML 2.0，請執行下列步驟：
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]  、[所有應用程式]  及 [Replicon]  。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [Replicon]  。

    ![應用程式清單中的 [Replicon] 連結](common/all-applications.png)
...
當您在存取面板中按一下 [Replicon] 圖格時，應該會自動登入您已設定 SSO 的 Replicon。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
