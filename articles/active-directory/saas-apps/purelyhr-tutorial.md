...
若要讓單一登入運作，Azure AD 必須知道 PurelyHR 與 Azure AD 中互相對應的使用者。 換句話說，必須建立 Azure AD 使用者和 PurelyHR 中相關使用者之間的連結關聯性。

在 PurelyHR 中，將 Azure AD 中**使用者名稱**的值，指派為 **Username** 的值，以建立連結關聯性。
...
在本節中，您會在 Azure 入口網站中啟用 Azure AD 單一登入，然後在您的 PurelyHR 應用程式中設定單一登入。

**若要使用 PurelyHR 設定 Azure AD 單一登入，請執行下列步驟：**

1. 在 Azure 入口網站的 [PurelyHR] 應用程式整合頁面上，按一下 [單一登入]。
...
1. 如下所述貼上方塊中的值-
    ...
    b. 在 [Idp 簽發者 URL] 方塊中，貼上從 Azure 入口網站複製的 **SAML 實體識別碼**。

    c. 在 [Idp 端點 URL] 方塊中，貼上從 Azure 入口網站複製的 **SAML 單一登入服務 URL**。 
...
**若要將 Britta Simon 指派至 PurelyHR，請執行以下步驟：**
