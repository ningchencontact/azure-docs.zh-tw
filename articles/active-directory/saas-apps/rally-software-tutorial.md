...
- 您可以讓使用者使用他們的 Azure AD 帳戶自動登入 Rally Software (單一登入)。
...
單一登入若要運作，Azure AD 必須知道 Rally Software 與 Azure AD 中互相對應的使用者。 換句話說，Azure AD 使用者和 Rally Software 中的相關使用者之間必須建立連結關聯性。

在 Rally Software 中，將 Azure AD 中**使用者名稱**的值，指派為 **Username** 的值，以建立連結關聯性。
...
在本節中，您會在 Azure 入口網站中啟用 Azure AD 單一登入，然後在您的 Rally Software 應用程式中設定單一登入。

**若要使用 Rally Software 設定 Azure AD 單一登入，請執行下列步驟：**

1. 在 Azure 入口網站的 [Rally Software] 應用程式整合頁面上，按一下 [單一登入]。
...
1. 在 [建立使用者]  區段中，執行下列步驟：
    ...
    b. 在 **電子郵件地址**文字方塊中，輸入使用者電子郵件，例如**brittasimon\@contoso.com**。
...
**若要指派 Britta Simon 到 Rally Software，請執行以下步驟：**
