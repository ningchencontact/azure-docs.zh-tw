...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Halogen Software (單一登入)。
...
* Halogen Software 支援由 **SP** 起始的 SSO
...
4. 在搜尋方塊中輸入 **Halogen Software**，並從結果面板中選取 [Halogen Software]，然後按一下 [新增] 按鈕以新增應用程式。

     ![結果清單中的 Halogen Software](common/search-new-app.png)
...
5. **[建立 Halogen Software 測試使用者](#create-halogen-software-test-user)** - 使 Halogen Software 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
若要設定與 Halogen Software 搭配運作的 Azure AD 單一登入，請執行下列步驟：

1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Halogen Software] 應用程式整合頁面上，選取 [單一登入]。
...
4. 在 [基本 SAML 組態] 區段上，執行下列步驟：
    ...
    > [!NOTE]
    > 這些都不是真正的值。 請使用實際的「登入 URL」及「識別碼」來更新這些值。 請連絡 [Halogen Software 用戶端支援小組](https://support.halogensoftware.com/)以取得這些值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。

4. 在 [以 SAML 設定單一登入] 頁面的 [SAML 簽署憑證] 區段中按一下 [下載]，以依據您的需求從指定選項下載**同盟中繼資料 XML**，並儲存在您的電腦上。

    ![憑證下載連結](common/metadataxml.png)

6. 在 [設定 Halogen Software] 區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-halogen-software-single-sign-on"></a>設定 Halogen Software 單一登入

1. 在不同的瀏覽器視窗中，以系統管理員身分登入您的 **Halogen Software** 。

2. 按一下 [選項]  索引標籤。
  
    ![何謂 Azure AD Connect](./media/halogen-software-tutorial/tutorial_halogen_12.png)

3. 在左瀏覽窗格中，按一下 [SAML 組態] 。
  
    ![何謂 Azure AD Connect](./media/halogen-software-tutorial/tutorial_halogen_13.png)

4. 在 [SAML 組態]  頁面上，執行下列步驟：

    ![何謂 Azure AD Connect](./media/halogen-software-tutorial/tutorial_halogen_14.png)
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [Halogen Software]。
...
### <a name="create-halogen-software-test-user"></a>建立 Halogen Software 測試使用者
...
2. 按一下 [使用者中心] 索引標籤，然後按一下 [建立使用者]。
    ...
    ![何謂 Azure AD Connect](./media/halogen-software-tutorial/tutorial_halogen_300.png)  

3. 在 [新增使用者]  對話頁面上，執行下列步驟：

    ![何謂 Azure AD Connect](./media/halogen-software-tutorial/tutorial_halogen_301.png)
...
當您在存取面板中按一下 [Halogen Software] 圖格時，應該會自動登入您已設定 SSO 的 Halogen Software。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
