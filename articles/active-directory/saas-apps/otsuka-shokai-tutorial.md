---
title: 教學課程：Azure Active Directory 與 Otsuka Shokai 整合 | Microsoft Docs
...
在本教學課程中，您會在測試環境中設定和測試 Azure AD SSO。 Otsuka Shokai 支援 **IDP** 起始的 SSO。
...
1. 按一下 [屬性]  索引標籤，複製 [應用程式識別碼]  並將其儲存在電腦上以供稍後使用。
...
2. **[設定 Otsuka Shokai](#configure-otsuka-shokai)** 以在應用程式端設定 SSO 設定。
...
5. **[建立 Otsuka Shokai 測試使用者](#create-otsuka-shokai-test-user)** ，使 Otsuka Shokai 中對應的 B. Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. 除了以上屬性之外，Otsuka Shokai 應用程式還預期 SAML 回應中會再多傳回幾個屬性。 在 [使用者屬性]  對話方塊的 [使用者宣告]  區段中，執行下列步驟以設定 SAML 權杖屬性，如下表所示：
...
當您在存取面板中選取 Otsuka Shokai 圖格時，應該會自動登入您已設定 SSO 的 Otsuka Shokai。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
