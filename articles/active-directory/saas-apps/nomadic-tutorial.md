...
- 您可以讓使用者使用他們的 Azure AD 帳戶自動登入 Nomadic (單一登入)。
...
若要讓單一登入能夠運作，Azure AD 必須知道 Nomadic 與 Azure AD 中互相對應的使用者。 換句話說，必須在 Azure AD 使用者與 Nomadic 中的相關使用者之間建立連結關聯性。

在 Nomadic 中，將 Azure AD 中**使用者名稱**的值指派為 **Username** 的值，以建立連結關聯性。
...
在本節中，您會於 Azure 入口網站啟用 Azure AD 單一登入，然後在您的 Nomadic 應用程式中設定單一登入。

**若要設定與 Nomadic 搭配運作的 Azure AD 單一登入，請執行下列步驟：**

1. 在 Azure 入口網站的 [Nomadic] 應用程式整合頁面上，按一下 [單一登入]。
...
1. 在 [Nomadic 網域與 URL] 區段上，執行下列步驟：
    ...
    b. 在 [識別碼] 文字方塊中，使用下列模式輸入 URL：`https://<company name>.nomadic.fm/auth/saml2/sp`、`https://<company name>.staging.nomadic.fm/auth/saml2/sp`

    > [!NOTE] 
    > 這些都不是真正的值。 使用實際的「登入 URL」及「識別碼」來更新這些值。 請連絡 [Nomadic 用戶端支援小組](mailto:help@nomadic.fm)以取得這些值。 
