---
title: 教學課程：Azure Active Directory 單一登入 (SSO) 與 PandaDoc 整合 | Microsoft Docs
...
ms.locfileid: "72532966"
---
# <a name="tutorial-azure-active-directory-single-sign-on-sso-integration-with-pandadoc"></a>教學課程：Azure Active Directory 單一登入 (SSO) 與 PandaDoc 整合
...
## <a name="configure-and-test-azure-ad-single-sign-on-for-pandadoc"></a>設定及測試 PandaDoc 的 Azure AD 單一登入
...
1. **[設定 PandaDoc SSO](#configure-pandadoc-sso)** - 在應用程式端設定單一登入設定。
    1. **[建立 PandaDoc 測試使用者](#create-pandadoc-test-user)** 使 PandaDoc 中 B.Simon 的對應使用者連結到該使用者在 Azure AD 中的代表身分。
...
1. PandaDoc 應用程式需要特定格式的 SAML 判斷提示，而您需要將自訂屬性對應新增到您的 SAML 權杖屬性設定。 以下螢幕擷取畫面顯示預設屬性清單

    ![image](common/edit-attribute.png)

1. 除了上述屬性外，PandaDoc 應用程式還需要在 SAML 回應中多傳回幾個屬性，如下所示。 這些屬性也會預先填入，但您可以根據您的需求來檢閱這些屬性。
...
當您在存取面板中按一下 PandaDoc 圖格時，應該會自動登入您已設定 SSO 的 PandaDoc。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
...
- [嘗試搭配 Azure AD 使用 PandaDoc](https://aad.portal.azure.com/)
