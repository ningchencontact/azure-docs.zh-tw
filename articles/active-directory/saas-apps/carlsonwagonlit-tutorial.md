...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Carlson Wagonlit Travel (單一登入)。
...
* Carlson Wagonlit Travel 支援由 **IDP** 起始的 SSO
...
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 Carlson Wagonlit Travel 中相關使用者之間的連結關聯性。

若要設定及測試與 Carlson Wagonlit Travel 搭配運作的 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 Carlson Wagonlit Travel 單一登入](#configure-carlson-wagonlit-travel-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 Carlson Wagonlit Travel 測試使用者](#create-carlson-wagonlit-travel-test-user)** - 在 Carlson Wagonlit Travel 中建立一個與 Azure AD 中代表使用者之項目連結的 Britta Simon 對應項目。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Carlson Wagonlit Travel]  應用程式整合頁面上，選取 [單一登入]  。
...
4. 在 [基本 SAML 組態]  區段上，執行下列步驟：
    ...
    ![Carlson Wagonlit Travel 網域及 URL 單一登入資訊](common/idp-identifier.png)
...
6. 在 [設定 Carlson Wagonlit Travel]  區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-carlson-wagonlit-travel-single-sign-on"></a>設定 Carlson Wagonlit Travel 單一登入

若要在 **Carlson Wagonlit Travel** 端設定單一登入，您必須將從 Azure 入口網站下載的 [同盟中繼資料 XML]  和複製的適當 URL 傳送給 [Carlson Wagonlit Travel 支援小組](http://www.carlsonwagonlit.in/content/cwt/in/en/technical-assistance.html)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]  、[所有應用程式]  及 [Carlson Wagonlit Travel]  。
...
### <a name="create-carlson-wagonlit-travel-test-user"></a>建立 Carlson Wagonlit Travel 測試使用者
...
當您在存取面板中按一下 [Carlson Wagonlit Travel] 圖格時，應該會自動登入您已設定 SSO 的 Carlson Wagonlit Travel。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
