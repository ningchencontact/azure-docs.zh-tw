...
以名為 **B.Simon** 的測試使用者，設定及測試與 Veracode 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 Veracode 中相關使用者之間的連結關聯性。
...
1. **[設定 Veracode SSO](#configure-veracode-sso)** - 在應用程式端設定單一登入設定。
    * **[建立 Veracode 測試使用者](#create-veracode-test-user)** - 讓 Veracode 中對應的 B.Simon 連結到使用者在 Azure AD 中的代表項目。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Veracode]  應用程式整合頁面上，尋找 [管理]  區段並選取 [單一登入]  。
...
1. Veracode 應用程式需要特定格式的 SAML 判斷提示，您需要將自訂屬性對應新增到您的 SAML 權杖屬性組態。 以下螢幕擷取畫面顯示預設屬性清單。

    ![image](common/default-attributes.png)

1. 除了上述屬性外，Veracode 應用程式還需要在 SAML 回應中多傳回幾個屬性，如下所示。 這些屬性也會預先填入，但您可以根據您的需求來檢閱這些屬性。
...
在本節中，您會將 Veracode 的存取權授與 B.Simon，讓其能夠使用 Azure 單一登入。
