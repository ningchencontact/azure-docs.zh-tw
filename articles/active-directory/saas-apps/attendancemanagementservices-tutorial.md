...
1. 在 [Attendance Management Services 網域與 URL] 區段上，執行下列步驟：
    ...
    > [!NOTE]
    > 這些都不是真正的值。 使用實際的「登入 URL」及「識別碼」來更新這些值。 請連絡 [Attendance Management Services 用戶端支援小組](https://www.obcnet.jp/)以取得這些值。
...
1. 在 [OBCiD 資訊] 區段中，執行下列步驟：
    ...
    a. 在  **OBCiD**文字方塊中，輸入電子郵件的使用者，例如**BrittaSimon\@contoso.com**。
