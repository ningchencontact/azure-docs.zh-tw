...
1. 在搜尋方塊中，輸入 **eDigitalResearch**，從結果面板中選取 [eDigitalResearch]，然後按一下 [新增] 按鈕以新增應用程式。
...
1. 在 [eDigitalResearch 網域與 URL] 區段中，執行下列步驟：
...
1. 在 [eDigitalResearch 設定] 區段上，按一下 [設定 eDigitalResearch] 以開啟 [設定登入] 視窗。 從 [快速參考] 區段中複製 [登出 URL] 和 [SAML 實體識別碼]。

    ![eDigitalResearch 組態](./media/edigitalresearch-tutorial/tutorial_edigitalresearch_configure.png) 

1. 若要在 **eDigitalResearch** 端設定單一登入，您必須將已下載的「憑證 (Base64) 檔案」、「SAML 實體識別碼」及「登出 URL」傳送給 [eDigitalResearch 支援小組](http://www.maruedr.com/contact)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在應用程式清單中，選取 [eDigitalResearch] 。
