...
* 已啟用 O.C. Tanner - AppreciateHub 單一登入的訂用帳戶
...
在本節中，您會設定及測試與 O.C. 搭配運作的 Azure AD 單一登入 以名為 **Britta Simon** 的測試使用者身分，使用 O.C. Tanner - AppreciateHub 設定及測試 Azure AD 單一登入。
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 O.C. Tanner - AppreciateHub 中的相關使用者之間建立連結關聯性。
...
4. 如果您有**服務提供者中繼資料檔案**，請在 [基本 SAML 設定] 區段上執行下列步驟：
    ...
    >[!NOTE]
    >您可以從[這裡](https://fed.appreciatehub.com/fed/sp/metadata)下載**服務提供者中繼資料檔案**
...
3. 在 [使用者] 屬性中，執行下列步驟。
    ...
    b. 在 [使用者名稱] 欄位中，輸入 BrittaSimon@contoso.com 之類的使用者。
