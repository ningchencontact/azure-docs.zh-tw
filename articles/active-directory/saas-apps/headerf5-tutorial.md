...
1. 您必須將中繼資料憑證匯入稍後將在安裝程式中使用的 F5 (標頭式)。 移至 [系統] > [憑證管理] > [流量憑證管理] > [SSL 憑證清單]  。 按一下右上角的 [匯入] 

    ![F5 (標頭式) 設定](./media/headerf5-tutorial/configure12.png)
 
1. 此外，您也需要主機名稱 (`headerapp.superdemo.live`) 的 **SSL 憑證**，在此範例中我們使用萬用字元憑證。

    ![F5 (標頭式) 設定](./media/headerf5-tutorial/configure13.png)

1. 移至 – F5 (標頭式) BIG IP 並按一下[存取] > [引導式設定] > [同盟] > [SAML 服務提供者]  。
...
1. 建立新的虛擬伺服器，指定**目的地位址**，[重新導向連接埠]  是選用項目。 選擇我們先前上傳的**萬用字元憑證**(或您為應用程式上傳的**憑證**) 及**相關聯的私密金鑰**。
...
1. 在 [單一登入] 下，選擇 [HTTP 標頭式]  。 您可以根據您的應用程式新增其他標頭。 如需 SAML 工作階段變數清單，請參閱附錄

    ![F5 (標頭式) 設定](./media/headerf5-tutorial/configure07.png) 

1. 請連絡 [F5 (標頭式) 用戶端支援小組](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45)以取得**端點檢查屬性**文件的詳細資料。

    ![F5 (標頭式) 設定](./media/headerf5-tutorial/configure08.png)

1. 請連絡 [F5 (標頭式) 用戶端支援小組](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45)以取得**工作階段管理屬性**文件的詳細資料。
