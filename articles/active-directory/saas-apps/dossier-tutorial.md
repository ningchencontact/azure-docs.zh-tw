...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Dossier (單一登入)。
...
4. 在搜尋方塊中，輸入**Dossier**，從結果面板中選取 [ZIVVER]，然後按一下 [新增] 按鈕以新增應用程式。
     ...
     ![結果清單中的 Dossier](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入

在本節中，您會以名為 **Britta Simon** 的測試使用者為基礎，設定及測試與 Dossier 搭配運作的 Azure AD 單一登入。
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 Dossier 中相關使用者之間的連結關聯性。

若要設定及測試與 Dossier 搭配運作的 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 Dossier 單一登入](#configure-dossier-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 Dossier 測試使用者](#create-dossier-test-user)** - 使 ZIVVER 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. 在 [Azure 入口網站](https://portal.azure.com/) 的 [Dossier] 應用程式整合頁面上，選取 [單一登入]。
...
4. 在 [基本 SAML 組態] 區段上，執行下列步驟：
    ...
    ![Dossier 網域及 URL 單一登入資訊](common/sp-identifier-reply.png)
    ...
    b. 在 [識別碼 (實體識別碼)] 文字方塊中，使用下列模式輸入 URL：`Dossier/<CLIENTNAME>`
    ...
    > [!NOTE]
    > 這些都不是真正的值。 請使用實際的「登入 URL」、「識別碼」和「回覆 URL」來更新這些值。 請連絡 [Dossier 用戶端支援小組](mailto:support@intellimedia.ca)以取得這些值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。

4. 在 [以 SAML 設定單一登入] 頁面的 [SAML 簽署憑證] 區段中，按一下 [複製] 按鈕，以依據您的需求從指定選項複製 [應用程式同盟中繼資料 Url]，並將其儲存到您的電腦上。

    ![憑證下載連結](common/copy-metadataurl.png)

6. 在 [設定 Dossier] 區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-dossier-single-sign-on"></a>設定 Dossier 單一登入

若要在 **Dossier** 端設定單一登入，您必須將**應用程式同盟中繼資料 URL** 傳送給 [Dossier 支援小組](mailto:support@intellimedia.ca)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [Dossier]。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [Dossier]。

    ![應用程式清單中的 Dossier 連結](common/all-applications.png)
...
### <a name="create-dossier-test-user"></a>建立 Dossier 測試使用者
...
當您在存取面板中按一下 [Dossier] 圖格時，應該會自動登入您設定 SSO 的 Dossier。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
