...
3. **[建立 TonicDM 測試使用者](#create-a-tonicdm-test-user)** - 讓 TonicDM 中的 Britta Simon 對應項目得以連結至 Azure AD 中代表該使用者的項目。
...
3. 在 [TonicDM 網域及 URL] 區段中，執行下列步驟：
...
6. 在 [TonicDM 設定] 區段上，按一下 [設定 TonicDM] 以開啟 [設定登入] 視窗。 從 [快速參考] 區段中複製 [登出 URL、SAML 實體識別碼和 SAML 單一登入服務 URL]。

    ![TonicDM 設定](./media/tonicdm-tutorial/tutorial_tonicdm_configure.png) 

7. 若要在 **TonicDM** 端設定單一登入，您必須將已下載的**憑證 (Base64)、登出 URL、SAML 實體識別碼及 SAML 單一登入服務 URL** 傳送給 [TonicDM 支援小組](mailto:support@tonicdm.com)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
