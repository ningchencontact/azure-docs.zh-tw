...
3. **[建立 ForeSee CX Suite 測試使用者](#create-a-foresee-cx-suite-test-user)** - 讓 ForeSee CX Suite 中的 Britta Simon 對應項目得以連結至 Azure AD 中代表該使用者的項目。
...
3. 如果您有**服務提供者中繼資料檔案**，請在 [ForeSee CX Suite 網域及 URL] 區段上執行下列步驟：
...
7. 若要在 **ForeSee CX Suite** 端設定單一登入，您必須將已下載的**中繼資料 XML** 傳送給 [ForeSee CX Suite 支援小組](mailto:support@foresee.com)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
