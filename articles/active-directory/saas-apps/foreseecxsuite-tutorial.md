...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 ForeSee CX Suite (單一登入)。
...
* ForeSee CX Suite 支援 **Just In Time** 使用者佈建
...
4. 在搜尋方塊中，輸入 **ForeSee CX Suite**，從結果面板中選取 [ForeSee CX Suite]  ，然後按一下 [新增]  按鈕以新增應用程式。

     ![結果清單中的 [ForeSee CX Suite]](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入

在此節中，您會以名為 **Britta Simon** 的測試使用者為基礎，設定及測試與 ForeSee CX Suite 搭配運作的 Azure AD 單一登入。
為了讓單一登入運作，必須在 Azure AD 使用者與 ForeSee CX Suite 中的相關使用者之間建立連結關聯性。

若要搭配 ForeSee CX Suite 設定和測試 Azure AD 單一登入，您需要完成下列基本工作：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 ForeSee CX Suite 單一登入](#configure-foresee-cx-suite-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 ForeSee CX Suite 測試使用者](#create-foresee-cx-suite-test-user)** - 在 ForeSee CX Suite 中建立一個與 Azure AD 中代表 Britta Simon 之項目連結的 Britta Simon 對應項目。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [ForeSee CX Suite]  應用程式整合頁面上，選取 [單一登入]  。
...
4. 如果您有**服務提供者中繼資料檔案**，請在 [基本 SAML 設定]  區段上執行下列步驟：
    ...
    ![[ForeSee CX Suite 網域及 URL] 單一登入資訊](common/sp-identifier.png)
...
6. 在 [設定 ForeSee CX Suite]  區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-foresee-cx-suite-single-sign-on"></a>設定 ForeSee CX Suite 單一登入

若要在 **ForeSee CX Suite** 端設定單一登入，您必須將從 Azure 入口網站下載的 [同盟中繼資料 XML]  和所複製的適當 URL 傳送給 [ForeSee CX Suite 支援小組](mailto:support@foresee.com)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
2. 在應用程式清單中，選取 [ForeSee CX Suite]  。

    ![應用程式清單中的 [ForeSee CX Suite] 連結](common/all-applications.png)
...
在本節中，您會在 ForeSee CX Suite 中建立名為 Britta Simon 的使用者。 請與 [ForeSee CX Suite 支援小組](mailto:support@foresee.com)合作，以新增必須新增至 ForeSee CX Suite 平台允許清單的使用者或網域。 如果是由小組新增網域，將會自動將使用者佈建到 ForeSee CX Suite 平台。 您必須先建立和啟動使用者，然後才能使用單一登入。
