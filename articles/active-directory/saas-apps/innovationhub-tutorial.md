...
4. 在搜尋方塊中，輸入 **Innovation Hub**，從結果面板中選取 [Innovation Hub]，然後按一下 [新增] 按鈕以新增應用程式。
...
3. **[建立 Innovation Hub 測試使用者](#create-an-innovation-hub-test-user)** - 在 Innovation Hub 中建立一個與 Azure AD 中代表 Britta Simon 使用者連結的 Britta Simon 對應項目。
...
3. 在 [Innovation Hub 網域與 URL] 區段中，執行下列步驟：
...
4. Innovation Hub 應用程式需要特定格式的 SAML 判斷提示。 請設定此應用程式的下列宣告。 您可以在應用程式整合頁面的 [使用者屬性] 區段中管理這些屬性的值。 以下螢幕擷取畫面顯示上述的範例。
...
6. 若要在 **Innovation Hub** 端設定單一登入，您必須將複製起來的**同盟中繼資料 URL** 傳送給 [Innovation Hub 支援小組](mailto:support@readify.net)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
