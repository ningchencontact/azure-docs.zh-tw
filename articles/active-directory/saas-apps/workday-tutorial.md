...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Workday (單一登入)。
...
4. 在搜尋方塊中，輸入 **Workday**，從結果面板中選取 [Workday]，然後按一下 [新增] 按鈕以新增應用程式。
     ...
     ![結果清單中的 Workday](common/search-new-app.png)
...
5. **[建立 Workday 測試使用者](#create-workday-test-user)** - 在 Workday 中建立一個與 Azure AD 中代表 Britta Simon 之使用者連結的 Britta Simon 對應項目。
...
若要設定與 Workday 搭配運作的 Azure AD 單一登入，請執行下列步驟：

1. 在 [Azure 入口網站](https://portal.azure.com/) 的 [Workday] 應用程式整合頁面上，選取 [單一登入]。
...
5. 按一下 [設定額外的 URL]，然後執行下列步驟：
    ...
    ![Workday 網域及 URL 單一登入資訊](./media/workday-tutorial/reply.png)

    在 [回覆 URL] 文字方塊中，使用下列模式來輸入 URL：`https:\//impl.workday.com/<tenant>/login-saml.htmld`
...
8. 在 [安裝 Workday] 區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-workday-single-sign-on"></a>設定 Workday 單一登入
...
3. 在 [重新導向 URL]  區段中，執行下列步驟：
    ...
    b. 在 [Login Redirect URL] \(登入重新導向 URL\) 文字方塊和 [Mobile Redirect URL] \(行動裝置重新導向 URL\) 文字方塊中，輸入您在 Azure 入口網站的 [基本 SAML 設定] 區段上輸入的 [登入 URL]。

    c. 在 Azure 入口網站的 [安裝 Workday] 區段上，複製 [登出 URL]，然後將它貼到 [Logout Redirect URL] \(登出重新導向 URL\) 文字方塊中。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [Workday]。
...
當您在存取面板中按一下 [Workday] 圖格時，應該會自動登入您已設定 SSO 的 Workday。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
