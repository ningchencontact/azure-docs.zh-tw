...
3. 在 [EverBridge 網域與 URL] 區段中，執行下列步驟：
...
6. 在 [EverBridge 組態] 區段上，按一下 [設定 EverBridge] 以開啟 [設定登入] 視窗。 從 [快速參考] 區段中複製 [SAML 單一登入服務 URL]。

    ![設定單一登入](./media/everbridge-tutorial/tutorial_everbridge_configure.png) 

6. 若要取得為應用程式設定的 SSO，您必須以系統管理員身分登入 Everbridge 租用戶。

7. 在頂端的功能表中，按一下 [設定] 索引標籤，然後選取 [安全性] 之下的 [單一登入]。
