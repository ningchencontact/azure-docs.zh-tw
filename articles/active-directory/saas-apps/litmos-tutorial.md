...
1. 在搜尋方塊中，輸入 **Litmos**，從結果面板中選取 [Litmos]，然後按一下 [新增] 按鈕以新增應用程式。
...
1. **[建立 Litmos 測試使用者](#create-a-litmos-test-user)** - 使 Litmos 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. 進行設定時，您需要為 Litmos 應用程式自訂 [SAML Token 屬性]  。
...
1. 複製 [litmos 的 SAML 端點是:] 下的值，然後貼到 Azure 入口網站中 [Litmos 網域及 URL] 區段的 [回覆 URL] 文字方塊中。 
