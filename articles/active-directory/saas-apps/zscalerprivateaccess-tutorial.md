...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Zscaler Private Access (ZPA) (單一登入)。
...
* 已啟用 Zscaler Private Access (ZPA) 單一登入的訂用帳戶

## <a name="scenario-description"></a>案例描述

在本教學課程中，您會在測試環境中設定和測試 Azure AD 單一登入。

* Zscaler Private Access (ZPA) 支援 **SP** 起始的 SSO
...
4. 在搜尋方塊中，輸入 **Zscaler Private Access (ZPA)** ，從結果面板中選取 [Zscaler Private Access (ZPA)]  ，然後按一下 [新增]  按鈕以新增應用程式。
...
為了讓單一登入運作，必須在 Azure AD 使用者與 Zscaler Private Access (ZPA) 中的相關使用者之間建立連結關聯性。

若要設定及測試與 Zscaler Private Access (ZPA) 搭配運作的 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 Zscaler Private Access (ZPA) 單一登入](#configure-zscaler-private-access-zpa-single-sign-on)** - 在應用程式端設定單一登入設定。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Zscaler Private Access (ZPA)]  應用程式整合頁面上，選取 [單一登入]  。
...
4. 在 [基本 SAML 組態]  區段上，執行下列步驟：
    ...
    ![Zscaler Private Access (ZPA) 網域與 URL 單一登入資訊](common/sp-identifier.png)

    a. 在 [登入 URL]  文字方塊中，使用下列模式輸入 URL：`https://samlsp.private.zscaler.com/auth/login?domain=<your-domain-name>`

    b. 在 [識別碼 (實體識別碼)]  文字方塊中，輸入 URL：`https://samlsp.private.zscaler.com/auth/metadata`
...
6. 在 [設定 Zscaler Private Access (ZPA)]  區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-zscaler-private-access-zpa-single-sign-on"></a>設定 Zscaler Private Access (ZPA) 單一登入

1. 在不同的 Web 瀏覽器視窗中，以系統管理員身分登入您的 Zscaler Private Access (ZPA) 公司網站。

2. 瀏覽至 [系統管理員]  ，然後按一下 [Idp 組態]  。

    ![在應用程式端設定單一登入](./media/zscalerprivateaccess-tutorial/tutorial_zscalerprivateaccess_04.png)

3. 在 [Idp 組態]  區段中，按一下 [新增 IDP 組態]  。

    ![在應用程式端設定單一登入](./media/zscalerprivateaccess-tutorial/tutorial_zscalerprivateaccess_05.png)

4. 在 [新增 IDP 組態]  區段中，執行下列步驟：
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]  、[所有應用程式]  及 [Zscaler Private Access (ZPA)]  。
...
當您在存取面板中按一下 [Zscaler Private Access (ZPA)] 圖格時，應該會自動登入您已設定 SSO 的 Zscaler Private Access (ZPA)。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
