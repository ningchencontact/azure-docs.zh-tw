...
3. **[建立 Vidyard 測試使用者](#create-a-vidyard-test-user)** - 讓 Vidyard 中的 Britta Simon 對應項目得以連結至 Azure AD 中代表該使用者的項目。
...
3. 如果您想要以 **IDP** 起始模式設定應用程式，請在 [Vidyard 網域及 URL] 區段上執行下列步驟：
...
7. 在 [Vidyard 設定] 區段上，按一下 [設定 Vidyard] 以開啟 [設定登入] 視窗。 從 [快速參考] 區段中複製 [SAML 單一登入服務 URL]。

    ![Vidyard 設定](./media/vidyard-tutorial/tutorial_vidyard_configure.png)

8. 在不同的網頁瀏覽器視窗中，以系統管理員身分登入您的 Vidyard Software 公司網站。

9. 從 Vidyard 儀表板中，選取 [Group] \(群組\) > [Security] \(安全性\)
...
11. 在 [SAML Configuration] \(SAML 設定\) 區段中，執行下列步驟：
...
12. 從 [Single Sign On] \(單一登入\) 索引標籤中，選取現有設定檔旁邊的 [Assign] \(指派\)
...
13. 選取您的組織群組，此群組會顯示在 [Groups Available to Assign] \(可指派的群組\) 中。

    ![Vidyard 設定](./media/vidyard-tutorial/configure5.png)

14. 您可以在 [Groups Currently Assigned] \(目前指派的群組\) 底下看到所指派的群組。 根據您的組織選取群組的角色，然後按一下 [Confirm] \(確認\)。
