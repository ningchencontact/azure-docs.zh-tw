---
title: 教學課程：Azure Active Directory 單一登入 (SSO) 與 Figma 整合 | Microsoft Docs
...
在本教學課程中，您會了解如何整合 Figma 與 Azure Active Directory (Azure AD)。 在整合 Figma 與 Azure AD 時，您可以︰

* 在 Azure AD 中控制可存取 Figma 的人員。
* 讓使用者使用其 Azure AD 帳戶自動登入 Figma。
...
1. 在 [從資源庫新增]  區段的搜尋方塊中輸入 **Figma**。
1. 從結果面板選取 [Figma]  ，然後新增應用程式。 當應用程式新增至您的租用戶時，請等候幾秒鐘。

## <a name="configure-and-test-azure-ad-single-sign-on-for-figma"></a>設定及測試 Figma 的 Azure AD 單一登入

以名為 **B.Simon** 的測試使用者，設定及測試與 Figma 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 Figma 中相關使用者之間的連結關聯性。

若要設定及測試與 Figma 搭配運作的 Azure AD SSO，請完成下列建置組塊：

1. **[設定 Azure AD SSO](#configure-azure-ad-sso)** - 讓您的使用者能夠使用此功能。
    * **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 B.Simon 測試 Azure AD 單一登入。
    * **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 B.Simon 能夠使用 Azure AD 單一登入。
1. **[設定 Figma SSO](#configure-figma-sso)** - 在應用程式端設定單一登入設定。
    * **[建立 Figma 測試使用者](#create-figma-test-user)** - 在 Figma 中建立 B.Simon 的對應項目，且該項目與 Azure AD 中代表使用者的項目連結。
...
1. 在 [基本 SAML 設定]  區段上，如果您想要以 **IDP** 起始模式設定應用程式，請輸入下列欄位的值：
    ...
    b. 在 [回覆 URL]  文字方塊中，使用下列模式來輸入 URL：`https://www.figma.com/saml/<TENANT ID>/consume`
...
1. 除了上述屬性外，Figma 應用程式還需要在 SAML 回應中多傳回幾個屬性，如下所示。 這些屬性也會預先填入，但您可以根據您的需求來檢閱這些屬性。
...
在本節中，您會將 Figma 的存取權授與 B.Simon，讓其能夠使用 Azure 單一登入。
...
- [嘗試搭配 Azure AD 使用 Figma](https://aad.portal.azure.com/)