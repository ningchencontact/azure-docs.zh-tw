...
5. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]，然後執行下列步驟：
    ...
    在 [登入 URL] 文字方塊中，以下列模式輸入 URL︰`https://connect.<example>.com/`

    > [!NOTE]
    > 這些都不是真正的值。 請使用實際的「識別碼」、「回覆 URL」及「登入 URL」來更新這些值。 請連絡 [RStudio Connect 用戶端支援小組](mailto:support@rstudio.com)以取得這些值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。
...
若要在 **RStudio Connect** 端設定單一登入，您必須將**應用程式同盟中繼資料 Url** 傳送至 [RStudio Connect 支援小組](mailto:support@rstudio.com)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
