...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 IMPAC Risk Manager (單一登入)。
...
* 已啟用 IMPAC Risk Manager 單一登入的訂用帳戶

## <a name="scenario-description"></a>案例描述

在本教學課程中，您會在測試環境中設定和測試 Azure AD 單一登入。

* IMPAC Risk Manager 支援由 **SP 和 IDP** 起始的 SSO
...
4. 在搜尋方塊中，輸入 **IMPAC Risk Manager**，從結果面板中選取 [IMPAC Risk Manager]  ，然後按一下 [新增]  按鈕以新增應用程式。
    ...
    ![結果清單中的 IMPAC Risk Manager](common/search-new-app.png)
...
5. **[建立 IMPAC Risk Manager 測試使用者](#create-impac-risk-manager-test-user)** - 在 IMPAC Risk Manager 中建立一個與 Azure AD 中代表 Britta Simon 之使用者連結的 Britta Simon 對應項目。
...
若要設定與 IMPAC Risk Manager 搭配運作的 Azure AD 單一登入，請執行下列步驟：

1. 在 [Azure 入口網站](https://portal.azure.com/)的 [IMPAC Risk Manager]  應用程式整合頁面上，選取 [單一登入]  。
...
4. 在 [基本 SAML 設定]  區段上，如果您想要以 **IDP** 起始模式設定應用程式，請執行下列步驟：
    ...
    ![IMPAC Risk Manager 網域及 URL 單一登入資訊](common/idp-intiated.png)
...
5. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]  ，然後執行下列步驟：
    ...
    ![IMPAC Risk Manager 網域及 URL 單一登入資訊](common/metadata-upload-additional-signon.png)
    ...
    > [!NOTE]
    > 這些都不是真正的值。 請使用實際的「識別碼」、「回覆 URL」及「登入 URL」來更新這些值。 請連絡 [IMPAC Risk Manager 客戶支援小組](mailto:rmsupport@Impac.co.nz)以取得這些值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。

6. 在 [以 SAML 設定單一登入]  頁面的 [SAML 簽署憑證]  區段中，按一下 [下載]  ，以依據您的需求從指定選項下載 [憑證 (Base64)]  ，並儲存在您的電腦上。

    ![憑證下載連結](common/certificatebase64.png)

7. 在 [安裝 IMPAC Risk Manager]  區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-impac-risk-manager-single-sign-on"></a>設定 IMPAC Risk Manager 單一登入

若要在 **IMPAC Risk Manager** 端設定單一登入，您必須將從 Azure 入口網站下載的 [憑證 (Base64)]  和複製的適當 URL 傳送給 [IMPAC Risk Manager 支援小組](mailto:rmsupport@Impac.co.nz)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
當您在存取面板中按一下 [IMPAC Risk Manager] 圖格時，應該會自動登入您已設定 SSO 的 IMPAC Risk Manager。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
