...
4. 在 [以 SAML 設定單一登入] 頁面上，執行下列步驟：
    ...
    > [!NOTE]
    > 這些都不是真正的值。 請使用實際的識別碼和回覆 URL 更新這些值。 請連絡 [ 用戶端支援小組](https://www.beeline.com/contact-us/)以取得這些值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。
...
7. 在 [設定 BeeLine] 區段上，依據您的需求複製適當的 URL。
...
若要在 **BeeLine** 端設定單一登入，您必須將從 Azure 入口網站下載的 [同盟中繼資料 XML] 和所複製的適當 URL 傳送給 [ 支援小組](https://www.beeline.com/contact-us/)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [BeeLine]。
...
當您在存取面板中按一下 BeeLine 圖格時，應該會自動登入您已設定 SSO 的 BeeLine。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
