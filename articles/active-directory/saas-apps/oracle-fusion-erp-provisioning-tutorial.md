...
>
> 此連接器目前為預覽版。 如需預覽功能之一般 Microsoft Azure 使用規定的詳細資訊，請參閱[Microsoft Azure 預覽的補充使用](https://azure.microsoft.com/support/legal/preview-supplemental-terms/)規定
...
## <a name="set-up-oracle-fusion-erp-for-provisioning"></a>設定 Oracle 融合 ERP 以提供布建
...
> [!TIP]
> 您也可以遵循[Oracle 融合 Erp 單一登入教學](oracle-fusion-erp-tutorial.md)課程中提供的指示，選擇啟用 ORACLE 融合 ERP 的 SAML 型單一登入。 單一登入可以與自動使用者布建分開設定，雖然這兩個功能彼此互補。
...
5. 在 [**管理員認證**] 區段下，于 [**租使用者 URL**] 中輸入 `https://ejlv.fa.em2.oraclecloud.com/hcmRestApi/scim/`。 在 [**管理員**使用者名稱] 和 [**密碼**] 欄位中，輸入稍早抓取的系統管理員使用者名稱和密碼。 按一下 Azure AD 和 Oracle 融合 ERP 之間的 [**測試連接**]。 
...
9. 在 [**屬性對應**] 區段中，檢查從 Azure AD 同步至 ORACLE 融合 ERP 的使用者屬性。 選取為 [比對] 屬性**的屬性會**用來比對 ORACLE 融合 ERP 中的使用者帳戶，以進行更新作業。 選取 [儲存] 按鈕以認可所有變更。
...
11. 在 [**屬性對應**] 區段中，檢查從 Azure AD 同步至 ORACLE 融合 ERP 的群組屬性。 選取為 [比對] 屬性**的屬性會**用來比對 ORACLE 融合 ERP 中的群組以進行更新作業。 選取 [儲存] 按鈕以認可所有變更。
...
14. 在 [**設定**] 區段的 [**範圍**] 中選擇所需的值，以定義您想要布建到 Oracle 融合 ERP 的使用者和/或群組。
...
* Oracle 融合 ERP 中的角色會對應到 Azure AD 中的群組。 若要從 Azure AD 將角色指派給 Oracle 融合 ERP 中的使用者，您必須將使用者指派給在 Oracle 融合 ERP 中以角色命名的所需 Azure AD 群組。
