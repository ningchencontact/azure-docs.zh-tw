...
* Getabstract 支援 **Just In Time** 使用者佈建
...
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 Getabstract 中相關使用者之間的連結關聯性。

若要設定及測試與 Getabstract 搭配運作的 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 Getabstract 單一登入](#configure-getabstract-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 Getabstract 測試使用者](#create-getabstract-test-user)** - 使 Getabstract 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
4. 在 [基本 SAML 組態] 區段上，若您想要以 **IDP** 起始模式設定應用程式，請執行下列步驟：
    ...
    ![Getabstract 網域與 URL 單一登入資訊](common/idp-intiated.png)
...
### <a name="create-getabstract-test-user"></a>建立 Getabstract 測試使用者

本節會在 Getabstract 中建立名為 Britta Simon 的使用者。 Getabstract 支援依預設啟用的 Just-In-Time 使用者佈建。 在這一節沒有您需要進行的動作項目。 如果 Getabstract 中還沒有任何使用者存在，在驗證之後就會建立新的使用者。
