...
* Reward Gateway 支援由 **IDP** 起始的 SSO
...
4. 在搜尋方塊中，輸入 **Reward Gateway**，從結果面板中選取 [Reward Gateway]，然後按一下 [新增] 按鈕以新增應用程式。
...
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 Reward Gateway 中相關使用者之間的連結關聯性。

若要設定及測試與 Reward Gateway 搭配運作的 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 Reward Gateway 單一登入](#configure-reward-gateway-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 Reward Gateway 測試使用者](#create-reward-gateway-test-user)** - 使 Reward Gateway 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Reward Gateway] 應用程式整合頁面上，選取 [單一登入]。
...
4. 在 [以 SAML 設定單一登入] 頁面上，執行下列步驟：
    ...
    ![Reward Gateway 網域和 URL 單一登入資訊](common/idp-intiated.png)
...
6. 在 [設定 Reward Gateway] 區段上，依據您的需求複製適當的 URL。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [Reward Gateway]。
...
### <a name="create-reward-gateway-test-user"></a>建立 Reward Gateway 測試使用者

在本節中，您會在 Reward Gateway 中建立名為 Britta Simon 的使用者。 請與  [Reward Gateway 支援小組](mailto:clientsupport@rewardgateway.com)合作，以在 Reward Gateway 平台中新增使用者。 您必須先建立和啟動使用者，然後才能使用單一登入。

### <a name="test-single-sign-on"></a>測試單一登入

在本節中，您會使用存取面板來測試您的 Azure AD 單一登入設定。

當您在存取面板中按一下 [Reward Gateway] 圖格時，應該會自動登入您已設定 SSO 的 Reward Gateway。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
