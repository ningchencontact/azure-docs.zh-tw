---
title: 教學課程：Azure Active Directory 與 productboard 整合 | Microsoft Docs
description: 了解如何設定 Azure Active Directory 與 productboard 之間的單一登入。
...
在本教學課程中，您會了解如何整合 productboard 與 Azure Active Directory (Azure AD)。 在整合 productboard 與 Azure AD 時，您可以︰

* 在 Azure AD 中控制可存取 productboard 的人員。
* 讓使用者使用其 Azure AD 帳戶自動登入 productboard。
...
* 已啟用 productboard 單一登入 (SSO) 的訂用帳戶。

## <a name="scenario-description"></a>案例描述

在本教學課程中，您會在測試環境中設定和測試 Azure AD SSO。

* productboard 支援 **SP 和 IDP** 初始化的 SSO
* productboard 支援 **Just In Time** 使用者佈建

## <a name="adding-productboard-from-the-gallery"></a>從資源庫新增 productboard

若要設定將 productboard 整合到 Azure AD 中，您需要從資源庫將 productboard 新增到受控 SaaS 應用程式清單。

1. 使用公司或學校帳戶或個人的 Microsoft 帳戶登入 [Azure 入口網站](https://portal.azure.com)。
1. 在左方瀏覽窗格上，選取 [Azure Active Directory]  服務。
1. 巡覽至 [企業應用程式]  ，然後選取 [所有應用程式]  。
1. 若要新增應用程式，請選取 [新增應用程式]  。
1. 在 [從資源庫新增]  區段的搜尋方塊中輸入 **productboard**。
1. 從結果面板選取 [productboard]  ，然後新增應用程式。 當應用程式新增至您的租用戶時，請等候幾秒鐘。

## <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入

以名為 **B.Simon** 的測試使用者，設定及測試與 productboard 搭配運作的 Azure AD SSO。 若要讓 SSO 能夠運作，您必須建立 Azure AD 使用者與 productboard 中相關使用者之間的連結關聯性。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [productboard]  應用程式整合頁面上，尋找 [管理]  區段並選取 [單一登入]  。
...
1. 如果您想要以 **SP** 起始的模式設定應用程式，請按一下 [設定其他 URL]  ，然後執行下列步驟：
    ...
    > [!NOTE]
    > 這些都不是真正的值。 使用實際的回覆 URL 與登入 URL 更新這些值。 請連絡 [productboard 用戶端支援小組](mailto:support@productboard.com)以取得這些值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。
...
在本節中，您會將 productboard 的存取權授與 B.Simon，讓其能夠使用 Azure 單一登入。

1. 在 Azure 入口網站中，選取 [企業應用程式]  ，然後選取 [所有應用程式]  。
1. 在應用程式清單中，選取 **productboard**。
...
當您在存取面板中按一下 productboard 圖格時，應該會自動登入您設定 SSO 的 productboard。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
