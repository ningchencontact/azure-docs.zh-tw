...
6. 在 [設定 LoginRadius]  區段上，依據您的需求複製適當的 URL。
...
若要在 **LoginRadius** 端設定單一登入，您必須將從 Azure 入口網站下載的 [中繼資料 XML]  和複製的適當 URL 傳送給 [LoginRadius 支援小組](mailto:support@loginradius.com)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
當您在存取面板中按一下 LoginRadius 圖格時，應該會自動登入您已設定 SSO 的 LoginRadius。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
