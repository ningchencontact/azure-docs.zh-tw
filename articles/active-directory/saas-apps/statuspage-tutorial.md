...
2. **[設定 StatusPage 單一登入](#configure-statuspage-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 StatusPage 測試使用者](#create-statuspage-test-user)** - 在 StatusPage 中建立 Britta Simon 的對應項目，且該項目與 Azure AD 中代表使用者的項目連結。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [StatusPage] 應用程式整合頁面上，選取 [單一登入]。
...
4. 在 [以 SAML 設定單一登入] 頁面上，執行下列步驟：
    ...
    ![StatusPage 網域與 URL 單一登入資訊](common/idp-intiated.png)
...
### <a name="configure-statuspage-single-sign-on"></a>設定 StatusPage 單一登入
...
2. 在應用程式清單中，選取 [StatusPage] 。
    ...
    ![應用程式清單中的 StatusPage 連結](common/all-applications.png)
...
StatusPage 支援 Just-in-Time 佈建。 您已在[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)中啟用該功能。
...
當您在存取面板中按一下 [StatusPage] 圖格時，應該會自動登入您設定 SSO 的 StatusPage。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
