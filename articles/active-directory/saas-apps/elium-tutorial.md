...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Elium (單一登入)。
...
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 Elium 中相關使用者之間的連結關聯性。

若要設定及測試與 Elium 搭配運作的 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 Elium 單一登入](#configure-elium-single-sign-on)** - 在應用程式端設定單一登入設定。
...
### <a name="configure-elium-single-sign-on"></a>設定 Elium 單一登入
...
本節會在 Elium 中建立名為 Britta Simon 的使用者。 Elium 支援依預設啟用的 **Just-In-Time 佈建**。 在這一節沒有您需要進行的動作項目。 如果 Elium 中還沒有使用者存在，在您嘗試存取 Elium 時就會建立新的使用者。
