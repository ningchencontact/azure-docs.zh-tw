...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Way We Do (單一登入)。
...
4. 在搜尋方塊中，輸入 **Way We Do**，從結果面板中選取 [Way We Do]，然後按一下 [新增] 按鈕以新增應用程式。
    ...
    ![結果清單中的 [Way We Do]](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入

在本節中，您會以名為 **Britta Simon** 的測試使用者身分，使用 Way We Do 設定和測試 Azure AD 單一登入。
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 Way We Do 中相關使用者之間的連結關聯性。

若要使用 Way We Do 來設定和測試 Azure AD 單一登入，您需要完成下列基本工作：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 Way We Do 單一登入](#configure-way-we-do-single-sign-on)** - 在應用程式端設定單一登入設定。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Way We Do] 應用程式整合頁面上，選取 [單一登入]。
...
4. 在 [基本 SAML 組態] 區段上，執行下列步驟：
    ...
    ![[Way We Do 網域及 URL] 單一登入資訊](common/sp-identifier.png)
...
6. 在 [設定 Way We Do] 區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-way-we-do-single-sign-on"></a>設定 Way We Do 單一登入

1. 在不同的網頁瀏覽器視窗中，以安全性系統管理員身分登入 Way We Do。

2. 在 Way We Do 中的任何頁面上，按一下右上角的**人員圖示**，然後在下拉式清單中按一下 [Account] \(帳戶\)。

    ![Way We Do [Account] \(帳戶\)](./media/waywedo-tutorial/tutorial_waywedo_account.png) 

3. 按一下**功能表圖示**以開啟推送導覽功能表，然後按一下 [Single Sign On] \(單一登入\)。

    ![Way We Do 單一登入](./media/waywedo-tutorial/tutorial_waywedo_single.png)

4. 在 [Single sign-on setup] \(單一登入設定\) 頁面上，執行下列步驟：
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [Way We Do]。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [Way We Do]。

    ![應用程式清單中的 [Way We Do] 連結](common/all-applications.png)
...
當您在存取面板中按一下 [Way We Do] 圖格時，應該會自動登入您已設定 SSO 的 Way We Do。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
