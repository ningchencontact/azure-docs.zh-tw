...
description: 在本教學課程中，您會了解如何設定 Azure Active Directory 與 Projectplace 之間的單一登入。
...
* 您可以使用 Azure AD 來控制可存取 Projectplace 的人員。
...
## <a name="add-projectplace-from-the-gallery"></a>從資源庫新增 Projectplace
...
4. 在搜尋方塊中，輸入 **Projectplace**。 在搜尋結果中，選取 [Projectplace]  ，然後選取 [新增]  。

     ![搜尋結果](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入

在本節中，您會使用名為 Britta Simon 的測試使用者，設定及測試與 Projectplace 搭配運作的 Azure AD 單一登入。
若要啟用單一登入，您必須建立 Azure AD 使用者與 Projectplace 中相關使用者之間的關聯性。

若要設定及測試與 Projectplace 搭配運作的 Azure AD 單一登入，您需要完成下列步驟：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** ，讓您的使用者能夠使用此功能。
2. 在應用程式端 **[設定 Projectplace 單一登入](#configure-projectplace-single-sign-on)** 。
...
5. **[建立 Projectplace 測試使用者](#create-a-projectplace-test-user)** ，其會連結至 Azure AD 中代表該使用者的項目。
...
若要設定與 Projectplace 搭配運作的 Azure AD 單一登入，請使用下列步驟：
...
4. 在 [基本 SAML 組態]  對話方塊的 [登入 URL]  方塊中，以此模式輸入 URL：

    `https://<company>.projectplace.com`

   ![[基本 SAML 組態] 對話方塊](common/sp-signonurl.png)
    > [!NOTE]
    > 此值是預留位置。 您需要使用實際的登入 URL。 請連絡 [Projectplace 支援小組](https://success.planview.com/Projectplace/Support)以取得此值。 您也可以參考 Azure 入口網站中 [基本 SAML 組態]  對話方塊所示的模式。

5. 在 [以 SAML 設定單一登入]  頁面的 [SAML 簽署憑證]  區段中，依據您的需求選取 [同盟中繼資料 XML]  旁邊的 [下載]  連結，並將憑證儲存在您的電腦上：
...
若要在 **Projectplace** 端設定單一登入，您必須將從 Azure 入口網站下載的 [同盟中繼資料 XML]  憑證和複製的 URL 傳送給 [Projectplace 支援小組](https://success.planview.com/Projectplace/Support)。 此小組會確保兩端的 SAML SSO 連線都已正確設定。
...
在本節中，您會將 Projectplace 的存取權授與 Britta Simon，讓她能夠使用 Azure AD 單一登入。
...
### <a name="create-a-projectplace-test-user"></a>建立 Projectplace 測試使用者
...
當您在存取面板中選取 Projectplace 圖格時，應該會自動登入您設定 SSO 的 Projectplace 執行個體。 如需詳細資訊，請參閱[在「我的應用程式」入口網站上存取和使用應用程式](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
