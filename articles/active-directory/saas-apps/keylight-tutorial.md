...
1. 在搜尋方塊中，輸入 **LockPath Keylight**。

    ![建立 Azure AD 測試使用者](./media/keylight-tutorial/tutorial_keylight_search.png)

1. 在結果面板中，選取 [LockPath Keylight]，然後按一下 [新增] 按鈕以新增應用程式。
...
1. **[建立 LockPath Keylight 測試使用者](#creating-a-lockpath-keylight-test-user)** - 使 LockPath Keylight 中對應的 Britta Simon 連結到該該使用者在 Azure AD 中的代表項目。
...
1. 在 [LockPath Keylight 網域及 URL] 區段中，執行下列步驟：
...
1. 在 [LockPath Keylight 組態] 區段上，按一下 [設定 LockPath Keylight] 以開啟 [設定登入] 視窗。 從 [快速參考] 區段中複製 [登出 URL 和 SAML 單一登入服務 URL]。
...
1. 在應用程式清單中，選取 [LockPath Keylight]。
