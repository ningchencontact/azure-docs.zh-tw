...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 AppDynamics (單一登入)。
...
* 已啟用 AppDynamics 單一登入的訂用帳戶

## <a name="scenario-description"></a>案例描述

在本教學課程中，您會在測試環境中設定和測試 Azure AD 單一登入。

* AppDynamics 支援 **SP** 起始的 SSO

* AppDynamics 支援 **Just In Time** 使用者佈建
...
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 AppDynamics 中相關使用者之間的連結關聯性。

若要設定及測試與 AppDynamics 搭配運作的 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 AppDynamics 單一登入](#configure-appdynamics-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 AppDynamics 測試使用者](#create-appdynamics-test-user)** - 使 AppDynamics 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. 在 [Azure 入口網站](https://portal.azure.com/) 的 [AppDynamics]  應用程式整合頁面上，選取 [單一登入]  。
...
4. 在 [基本 SAML 組態]  區段上，執行下列步驟：
    ...
    ![AppDynamics 網域和 URL 單一登入資訊](common/sp-identifier.png)

    a. 在 [登入 URL]  文字方塊中，使用下列模式輸入 URL：`https://<companyname>.saas.appdynamics.com?accountName=<companyname>`

    b. 在 [識別碼 (實體識別碼)]  文字方塊中，使用下列模式輸入 URL：`https://<companyname>.saas.appdynamics.com/controller`

    > [!NOTE]
    > 這些都不是真正的值。 請使用實際的「登入 URL」及「識別碼」來更新這些值。 請連絡 [AppDynamics 用戶端支援小組](https://www.appdynamics.com/support/)以取得這些值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。

4. 在 [以 SAML 設定單一登入]  頁面的 [SAML 簽署憑證]  區段中，按一下 [下載]  ，以依據您的需求從指定選項下載 [憑證 (Base64)]  ，並儲存在您的電腦上。

    ![憑證下載連結](common/certificatebase64.png)

6. 在 [設定 AppDynamics]  區段上，依據您的需求複製適當的 URL。
...
1. 在頂端工具列中，按一下 [設定]  ，然後按一下 [管理]  。
    ...
    ![系統管理](./media/appdynamics-tutorial/ic790216.png "系統管理")
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]  、[所有應用程式]  及 [AppDynamics]  。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，輸入 **AppDynamics** 並加以選取。

    ![應用程式清單中的 AppDynamics 連結](common/all-applications.png)
...
### <a name="create-appdynamics-test-user"></a>建立 AppDynamics 測試使用者
...
當您在存取面板中按一下 [AppDynamics] 圖格時，應該會自動登入您已設定 SSO 的 AppDynamics。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
