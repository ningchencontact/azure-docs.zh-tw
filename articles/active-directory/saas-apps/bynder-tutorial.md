...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Bynder (單一登入)。
...
4. 在搜尋方塊中，輸入 **Bynder**，從結果面板中選取 [Bynder]，然後按一下 [新增] 按鈕以新增應用程式。
     ...
     ![結果清單中的 Bynder](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入

在本節中，您會以名為 **Britta Simon** 的測試使用者為基礎，設定及測試與 Bynder 搭配運作的 Azure AD 單一登入。
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 Bynder 中相關使用者之間的連結關聯性。

如要設定及測試搭配 Bynder 的 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 Bynder 單一登入](#configure-bynder-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 Bynder 測試使用者](#create-bynder-test-user)** - 在 Bynder 中建立一個與 Azure AD 中代表 Britta Simon 之使用者連結的 Britta Simon 對應項目。
...
若要設定與 Bynder 搭配運作的 Azure AD 單一登入，請執行下列步驟：

1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Bynder] 應用程式整合頁面上，選取 [單一登入]。
...
4. 在 [基本 SAML 組態] 區段上，若您想要以 **IDP** 起始模式設定應用程式，請執行下列步驟：
    ...
    ![Bynder 網域及 URL 單一登入資訊](common/idp-intiated.png)
...
5. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]，然後執行下列步驟：
    ...
    ![Bynder 網域及 URL 單一登入資訊](common/metadata-upload-additional-signon.png)
...
7. 在 [設定 Bynder] 區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-bynder-single-sign-on"></a>設定 Bynder 單一登入

若要在 **Bynder** 端設定單一登入，您必須將從 Azure 入口網站下載的 [同盟中繼資料 XML] 和所複製的適當 URL 傳送給 [Bynder 支援小組](https://www.bynder.com/en/support/)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [Bynder]。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [Bynder] 。

    ![應用程式清單中的 [Bynder] 連結](common/all-applications.png)
...
本節會在 Bynder 中建立名為 Britta Simon 的使用者。 Bynder 支援預設會啟用的 Just-In-Time 使用者佈建。 在這一節沒有您需要進行的動作項目。 如果 Bynder 中還沒有任何使用者存在，在驗證之後就會建立新的使用者。
...
當您在存取面板中按一下 [Bynder] 圖格時，應該會自動登入您已設定 SSO 的 Bynder。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
