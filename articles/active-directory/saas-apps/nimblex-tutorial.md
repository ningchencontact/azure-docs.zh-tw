...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Nimblex (單一登入)。
...
* Nimblex 支援由 **SP** 起始的 SSO

* Nimblex 支援 **Just In Time** 使用者佈建
...
4. 在搜尋方塊中，鍵入 **Nimblex**，從結果面板中選取 [Nimblex]，然後按一下 [新增] 按鈕以新增應用程式。
     ...
     ![結果清單中的 Nimblex](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入

在本節中，您會以名為 **Britta Simon** 的測試使用者為基礎，設定及測試與 Nimblex 搭配運作的 Azure AD 單一登入。
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 Nimblex 中相關使用者之間的連結關聯性。

若要設定及測試與 Nimblex 搭配使用的 Azure AD 單一登入，您需要完成下列構成元素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 Nimblex 單一登入](#configure-nimblex-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 Nimblex 測試使用者](#create-nimblex-test-user)** - 在 Nimblex 中建立一個與 Azure AD 中代表 Britta Simon 之使用者連結的 Britta Simon 對應項目。
...
若要設定與 Nimblex 搭配運作的 Azure AD 單一登入，請執行下列步驟：

1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Nimblex] 應用程式整合頁面上，選取 [單一登入]。
...
6. 在 [安裝 Nimblex] 區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-nimblex-single-sign-on"></a>設定 Nimblex 單一登入

1. 在不同的網頁瀏覽器視窗中，以安全性系統管理員身分登入 Nimblex。
...
5. 在 [Edit SSO Provider] \(編輯 SSO 提供者\) 頁面上，執行下列步驟：
    ...
    c. 在 [Identity Provider Sso Target Url] \(識別提供者單一登入 URL\) 文字方塊中，貼上您從 Azure 入口網站複製的 [登入 URL] 值。
...
### <a name="create-nimblex-test-user"></a>建立 Nimblex 測試使用者

本節會在 Nimblex 中建立名為 Britta Simon 的使用者。 Nimblex 支援預設會啟用的 Just-In-Time 使用者佈建。 在這一節沒有您需要進行的動作項目。 如果 Nimblex 中還沒有任何使用者存在，在驗證之後就會建立新的使用者。
...
當您在存取面板中按一下 [Nimblex] 圖格時，應該會自動登入您已設定 SSO 的 Nimblex。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
