...
description: 了解如何設定 Azure Active Directory 與 Percolate 之間的單一登入。
...
若要設定 Azure AD 與 Percolate 的整合，您需要下列項目：

* Azure AD 訂用帳戶。 如果您沒有 Azure AD 環境，您可以申請[免費帳戶](https://azure.microsoft.com/free/)。
* 已啟用 Percolate 單一登入的訂用帳戶
...
## <a name="adding-percolate-from-the-gallery"></a>從資源庫新增 Percolate

若要進行將 Percolate 整合到 Azure AD 中的設定，您必須從資源庫將 Percolate 新增至受控 SaaS 應用程式清單。

**若要從資源庫新增 Percolate，請執行下列步驟：**
...
4. 在搜尋方塊中輸入 **Percolate**，並從結果面板中選取 [Percolate]，然後按一下 [新增] 按鈕以新增應用程式。

     ![結果清單中的 Percolate](common/search-new-app.png)
...
2. **[設定 Percolate 單一登入](#configure-percolate-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 Percolate 測試使用者](#create-percolate-test-user)** - 使 Percolate 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
5. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]，然後執行下列步驟：
    ...
    在 [登入 URL] 文字方塊中，輸入 URL：`https://percolate.com/app/login`
...
1. 在不同的網頁瀏覽器視窗中，以系統管理員身分登入 Percolate。

2. 在首頁左側按一下 [設定]。
    
    ![設定單一登入](./media/percolate-tutorial/configure01.png)

3. 在功能表列左側，按一下 [組織] 下方的 [SSO]。

    ![設定單一登入](./media/percolate-tutorial/configure02.png)
    ...
    e. [識別提供者中繼資料 URL] 是選擇性欄位，如果您已從 Azure 入口網站複製 [應用程式同盟中繼資料 URL]，請將其貼到 [識別提供者中繼資料 URL] 文字方塊中。

    f. 針對 [是否應簽署 AuthNRequests？]，選取 [否]。

    g. 針對 [啟用 SSO 自動佈建]，選取 [否]。
...
2. 在應用程式清單中，選取[Percolate]。
    ...
    ![應用程式清單中的 Percolate 連結](common/all-applications.png)
...
### <a name="create-percolate-test-user"></a>建立 Percolate 測試使用者

若要讓 Azure AD 使用者可以登入 Percolate，則必須將他們佈建到 Percolate。 在 Percolate 中，需以手動方式佈建。

**若要佈建使用者帳戶，請執行下列步驟：**

1. 以系統管理員身分登入 Percolate。

2. 在功能表列左側，按一下 [組織] 下方的 [使用者]，然後瀏覽至 [新增使用者]。

    ![設定單一登入](./media/percolate-tutorial/configure03.png)
