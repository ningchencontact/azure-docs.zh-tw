...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 UltiPro (單一登入)。
...
4. 在搜尋方塊中輸入 **UltiPro**，並從結果面板中選取 [UltiPro]，然後按一下 [新增] 按鈕以新增應用程式。

     ![結果清單中的 UltiPro](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入

在本節中，您會以名為 **Britta Simon** 的測試使用者為基礎，設定及測試與 UltiPro 搭配運作的 Azure AD 單一登入。
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 UltiPro 中相關使用者之間的連結關聯性。

若要使用 UltiPro 來設定並測試 Azure AD 單一登入，您需要完成下列建置組塊：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 UltiPro 單一登入](#configure-ultipro-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 UltiPro 測試使用者](#create-ultipro-test-user)** - 使 UltiPro 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
若要設定與 UltiPro 搭配運作的 Azure AD 單一登入，請執行下列步驟：

1. 在 [Azure 入口網站](https://portal.azure.com/) 的 [UltiPro] 應用程式整合頁面上，選取 [單一登入]。
...
4. 在 [基本 SAML 組態] 區段上，執行下列步驟：
    ...
    ![UltiPro 網域和 URL 單一登入資訊](common/sp-identifier-reply.png)
...
### <a name="configure-ultipro-single-sign-on"></a>設定 UltiPro 單一登入
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [UltiPro]。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，輸入 **UltiPro** 並加以選取。

    ![應用程式清單中的 UltiPro 連結](common/all-applications.png)
...
當您在存取面板中按一下 [UltiPro] 圖格時，應該會自動登入您已設定 SSO 的 UltiPro。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
