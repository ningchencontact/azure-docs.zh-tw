...
description: 在本教學課程中，您將了解如何設定 Azure Active Directory 與 Predictix Ordering 之間的單一登入。
...
在本教學課程中，您將了解如何整合 Predictix Ordering 與 Azure Active Directory (Azure AD)。
此整合有下列優點：

* 您可以使用 Azure AD 控制可存取 Predictix Ordering 的人員。
...
* 已啟用單一登入的 Predictix Ordering 訂用帳戶。
...
## <a name="add-predictix-ordering-from-the-gallery"></a>從資源庫新增 Predictix Ordering
...
在本節中，您會使用名為 Britta Simon 的測試使用者，設定及測試與 Predictix Ordering 搭配運作的 Azure AD 單一登入。
若要啟用單一登入，您必須建立 Azure AD 使用者與 Predictix Ordering 中相關使用者之間的關聯性。

若要設定及測試與 Predictix Ordering 搭配運作的 Azure AD 單一登入，您必須完成下列步驟：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** ，讓您的使用者能夠使用此功能。
2. 在應用程式端 **[設定 Predictix Ordering 單一登入](#configure-predictix-ordering-single-sign-on)** 。
...
5. **[建立 Predictix Ordering 測試使用者](#create-a-predictix-ordering-test-user)** ，這會連結至 Azure AD 中代表該使用者的項目。
...
若要在 Predictix Ordering 端設定單一登入，您必須將從 Azure 入口網站下載的憑證和複製的 URL 傳送給 [Predictix Ordering 支援小組](https://www.predix.io/support/)。 此小組會確定兩端的 SAML SSO 連線都已正確設定。
...
接下來，您需要在 Predictix Ordering 中建立名為 Britta Simon 的使用者。 請與 [Predictix Ordering 支援小組](https://www.predix.io/support/)合作，以新增使用者。 您必須先建立和啟動使用者，然後才能使用單一登入。

### <a name="test-single-sign-on"></a>測試單一登入

您現在必須使用存取面板來測試您的 Azure AD 單一登入組態。

當您在存取面板中選取 [Predictix Ordering] 圖格時，應該會自動登入您已設定 SSO 的 Predictix Ordering 執行個體。 如需詳細資訊，請參閱[在我的應用程式入口網站上存取和使用應用程式](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
