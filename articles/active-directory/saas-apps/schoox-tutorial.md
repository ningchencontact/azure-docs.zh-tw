...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Schoox (單一登入)。
...
4. 在搜尋方塊中，輸入 **Schoox**，從結果面板中選取 [Schoox]  ，然後按一下 [新增]  按鈕以新增應用程式。
     ...
     ![結果清單中的 Schoox](common/search-new-app.png)
...
2. **[設定 Schoox 單一登入](#configure-schoox-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 Schoox 測試使用者](#create-schoox-test-user)** - 在 Schoox 中建立 Britta Simon 的對應項目，且該項目與 Azure AD 中代表該使用者的項目連結。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Schoox]  應用程式整合頁面上，選取 [單一登入]  。
...
4. 在 [基本 SAML 設定]  區段上，如果您想要以 **IDP** 起始模式設定應用程式，請執行下列步驟：
    ...
    ![Schoox 網域及 URL 單一登入資訊](common/idp-identifier.png)

    在 [識別碼]  文字方塊中，鍵入 URL：`https://saml.schoox.com/saml/adfsmetadata`

5. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]  ，然後執行下列步驟：

    ![image](common/both-preintegrated-signon.png)

    在 [登入 URL]  文字方塊中，以下列模式輸入 URL︰`https://saml.schoox.com/saml/login?idpUrl=<entityID>`
...
7. 在 [設定 Schoox]  區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-schoox-single-sign-on"></a>設定 Schoox 單一登入

若要在 **Schoox** 端設定單一登入，您必須將從 Azure 入口網站下載的 [同盟中繼資料 XML]  和所複製的適當 URL 傳送給 [Schoox 支援小組](https://www.schoox.com/help/)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]  、[所有應用程式]  及 [Schoox]  。
...
### <a name="create-schoox-test-user"></a>建立 Schoox 測試使用者
...
當您在存取面板中按一下 [Schoox] 圖格時，應該會自動登入您已設定 SSO 的 Schoox。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
