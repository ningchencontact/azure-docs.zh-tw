...
* Cisco Webex 支援 **Just In Time** 使用者佈建
...
8. 在 Azure 入口網站的 [以 SAML 設定單一登入] 頁面上，按一下 [編輯] 圖示以開啟 [基本 SAML 設定] 對話方塊。

    ![編輯基本 SAML 組態](common/edit-urls.png)

9. 在 [基本 SAML 組態] 區段上，執行下列步驟，以上傳已下載的**服務提供者中繼資料**檔案並設定應用程式：
    ...
    ![Cisco Webex 網域及 URL 單一登入資訊](common/sp-identifier-reply.png)

    在 [登入 URL] 文字方塊中，以下列模式輸入 URL︰`https://<SUBDOMAIN>.webex.com/`

    > [!Note]
    > [登入 URL] 的值不是真正的值。 使用實際的登入 URL 來更新此值。 請連絡 [Cisco Webex 用戶端支援小組](https://www.webex.co.in/support/support-overview.html)以取得此值。

10. Cisco Webex 應用程式需要特定格式的 SAML 判斷提示。 設定此應用程式的下列宣告。 您可以在應用程式整合頁面的 [使用者屬性] 區段中，管理這些屬性的值。 在 [以 SAML 設定單一登入] 頁面上，按一下 [編輯] 按鈕以開啟 [使用者屬性] 對話方塊。

    ![映像](common/edit-attribute.png)

11. 在 [使用者屬性] 對話方塊的 [使用者宣告] 區段中，如上圖所示設定 SAML 權杖屬性，然後執行下列步驟：
...
12. 在 [以 SAML 設定單一登入] 頁面的 [SAML 簽署憑證] 區段中，按一下 [下載] 以依據您的需求從指定選項下載**同盟中繼資料 XML**，並儲存在您的電腦上。

    ![憑證下載連結](common/metadataxml.png)

13. 在 [設定 Cisco Webex] 區段上，依據您的需求複製適當的 URL。
...
1. 在 Cisco Webex 公司網站系統管理員頁面上，使用檔案瀏覽器選項來找出並上傳 Azure AD 中繼資料檔案。 然後，選取 [需要中繼資料中憑證授權單位所簽署的憑證 (較安全)]，然後前往下一個畫面。 

    ![設定單一登入](./media/cisco-webex-tutorial/tutorial_cisco_webex_11.png)

2. 選取 [測試 SSO 連接]，並在新的瀏覽器索引標籤開啟時，透過登入向 Azure AD 進行驗證。

3. 返回 [Cisco Cloud Collaboration Management] 瀏覽器索引標籤。如果測試成功，請選取 [這項測試成功。啟用單一登入] 選項，然後按 [儲存]。
...
本節的目標是要在 Cisco Webex 中建立名為 Britta Simon 的使用者。 Cisco Webex 支援預設啟用的 Just-In-Time 佈建和自動使用者佈建。 在這一節沒有您需要進行的動作項目。 如果 Cisco Webex 中還沒有該使用者，當您嘗試存取 Cisco Webex 時，就會建立新的使用者。
