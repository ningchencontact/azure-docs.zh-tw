...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Montage Online (單一登入)。
...
4. 在搜尋方塊中，輸入 **Montage Online**，從結果面板中選取 [Montage Online]，然後按一下 [新增] 按鈕以新增應用程式。
     ...
     ![結果清單中的 Montage Online](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入

在本節中，您會以名為 **Britta Simon** 的測試使用者身分，搭配 Montage Online 設定及測試 Azure AD 單一登入。
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 Montage Online 中相關使用者之間的連結關聯性。

若要搭配 Montage Online 來設定及測試 Azure AD 單一登入，您需要完成下列建置組塊：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 Montage Online 單一登入](#configure-montage-online-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 Montage Online 測試使用者](#create-montage-online-test-user)** - 使 Montage Online 中 Britta Simon 的對應使用者連結到該使用者在 Azure AD 中的代表身分。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Montage Online] 應用程式整合頁面上，選取 [單一登入]。
...
4. 在 [基本 SAML 組態] 區段上，執行下列步驟：
    ...
    ![Montage Online 網域及 URL 單一登入資訊](common/sp-identifier.png)
...
6. 在 [設定 Montage Online] 區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-montage-online-single-sign-on"></a>設定 Montage Online 單一登入

若要設定 **Montage Online** 端的單一登入，您必須將從 Azure 入口網站下載的 [憑證 (Base64)] 和複製的適當 URL 傳送給 [Montage Online 支援小組](https://www.montage.co.nz/contact-us/)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [Montage Online]。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [Montage Online]。

    ![應用程式清單中的 Montage Online 連結](common/all-applications.png)
...
### <a name="create-montage-online-test-user"></a>建立 Montage Online 測試使用者
...
當您在存取面板中按一下 [Montage Online] 圖格時，應該會自動登入您已設定 SSO 的 Montage Online。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
