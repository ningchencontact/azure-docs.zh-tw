...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 HubSpot (單一登入)。
...
4. 在搜尋方塊中，輸入 **HubSpot**，從結果面板中選取 [HubSpot]，然後按一下 [新增] 按鈕以新增應用程式。
...
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 HubSpot 中相關使用者之間的連結關聯性。

若要設定及測試與 HubSpot 搭配運作的 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 HubSpot 單一登入](#configure-hubspot-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 HubSpot 測試使用者](#create-hubspot-test-user)** - 在 HubSpot 中建立 Britta Simon 的對應項目，且該項目與 Azure AD 中代表使用者的項目連結。
...
1. 在 [Azure 入口網站](https://portal.azure.com/) 的 [HubSpot] 應用程式整合頁面上，選取 [單一登入]。
...
4. 在 [基本 SAML 設定] 區段上，如果您想要以 **IDP** 起始模式設定應用程式，請執行下列步驟：
    ...
    a. 在 [識別碼] 文字方塊中，使用下列模式來輸入 URL：`https://api.hubspot.com/login-api/v1/saml/login?portalId=<CUSTOMER ID>`

    b. 在 [回覆 URL] 文字方塊中，使用下列模式來輸入 URL：`https://api.hubspot.com/login-api/v1/saml/acs?portalId=<CUSTOMER ID>`

    > [!NOTE]
    > 這些都不是真正的值。 請使用實際的識別碼及回覆 URL 來更新這些值 (本教學課程稍後會說明)。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。

5. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]，然後執行下列步驟：

    ![HubSpot 網域與 URL 單一登入資訊](common/metadata-upload-additional-signon.png)
...
7. 在 [設定 HubSpot] 區段上，依據您的需求複製適當的 URL。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [HubSpot]。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [HubSpot]。

    ![應用程式清單中的 HubSpot 連結](common/all-applications.png)
...
### <a name="create-hubspot-test-user"></a>建立 HubSpot 測試使用者

若要讓 Azure AD 使用者可以登入 HubSpot，則必須將他們佈建至 HubSpot。 HubSpot 需以手動方式佈建。
...
當您在存取面板中按一下 [HubSpot] 圖格時，應該會自動登入您已設定 SSO 的 HubSpot。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
