...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 Sansan (單一登入)。
...
4. 在搜尋方塊中，輸入 **Sansan**，從結果面板中選取 [Sansan]，然後按一下 [新增] 按鈕以新增應用程式。

     ![結果清單中的 Sansan](common/search-new-app.png)
...
5. **[建立 Sansan 測試使用者](#create-sansan-test-user)** - 在 Sansan 中建立一個與 Azure AD 中代表 Britta Simon 之使用者連結的 Britta Simon 對應項目。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [Sansan] 應用程式整合頁面上，選取 [單一登入]。
...
4. 在 [基本 SAML 組態] 區段上，執行下列步驟：
    ...
    ![Sansan 網域及 URL 單一登入資訊](common/sp-signonurl.png)
...
9. 在 [安裝 Sansan] 區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-sansan-single-sign-on"></a>設定 Sansan 單一登入
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]、[所有應用程式] 及 [Sansan]。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [Sansan]。

    ![應用程式清單中的 [Sansan] 連結](common/all-applications.png)
...
當您在存取面板中按一下 [Sansan] 圖格時，應該會自動登入您已設定 SSO 的 Sansan。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
