...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 OfficeSpace Software (單一登入)。
...
* 已啟用 OfficeSpace Software 單一登入的訂用帳戶
...
5. **[建立 OfficeSpace Software 測試使用者](#create-officespace-software-test-user)** - 在 OfficeSpace Software 中建立一個與 Azure AD 中代表 Britta Simon 之使用者連結的 Britta Simon 對應項目。
...
若要設定與 OfficeSpace Software 搭配運作的 Azure AD 單一登入，請執行下列步驟：

1. 在 [Azure 入口網站](https://portal.azure.com/)的 [OfficeSpace Software]  應用程式整合頁面上，選取 [單一登入]  。
...
9. 在 [安裝 OfficeSpace Software]  區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-officespace-software-single-sign-on"></a>設定 OfficeSpace Software 單一登入
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]  、[所有應用程式]  及 [OfficeSpace Software]  。
...
本節會在 OfficeSpace Software 中建立名為 Britta Simon 的使用者。 OfficeSpace Software 支援預設會啟用的 Just-In-Time 使用者佈建。 在這一節沒有您需要進行的動作項目。 如果 OfficeSpace Software 中還沒有任何使用者存在，在驗證之後就會建立新的使用者。
...
當您在存取面板中按一下 [OfficeSpace Software] 圖格時，應該會自動登入您已設定 SSO 的 OfficeSpace Software。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
