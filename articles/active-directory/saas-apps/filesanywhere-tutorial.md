...
* 您可以讓使用者使用其 Azure AD 帳戶自動登入 FilesAnywhere (單一登入)。
...
* 已啟用 FilesAnywhere 單一登入的訂用帳戶

## <a name="scenario-description"></a>案例描述

在本教學課程中，您會在測試環境中設定和測試 Azure AD 單一登入。

* FilesAnywhere 支援由 **SP** 和 **IDP** 起始的 SSO

* FilesAnywhere 支援 **Just In Time** 使用者佈建
...
4. 在搜尋方塊中，輸入 **FilesAnywhere**，從結果面板中選取 [FilesAnywhere]  ，然後按一下 [新增]  按鈕以新增應用程式。

     ![結果清單中的 FilesAnywhere](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>設定和測試 Azure AD 單一登入

在本節中，您會以名為 **Britta Simon** 的測試使用者身分，使用 FilesAnywhere 設定及測試 Azure AD 單一登入。
若要讓單一登入能夠運作，必須建立 Azure AD 使用者與 FilesAnywhere 中相關使用者之間的連結關聯性。

若要設定並測試與 FilesAnywhere 搭配運作的 Azure AD 單一登入，您需要完成下列建置組塊：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)** - 讓您的使用者能夠使用此功能。
2. **[設定 FilesAnywhere 單一登入](#configure-filesanywhere-single-sign-on)** - 在應用程式端設定單一登入設定。
3. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)** - 使用 Britta Simon 測試 Azure AD 單一登入。
4. **[指派 Azure AD 測試使用者](#assign-the-azure-ad-test-user)** - 讓 Britta Simon 能夠使用 Azure AD 單一登入。
5. **[建立 FilesAnywhere 測試使用者](#create-filesanywhere-test-user)** - 使 FilesAnywhere 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
1. 在 [Azure 入口網站](https://portal.azure.com/)的 [FilesAnywhere]  應用程式整合頁面上，選取 [單一登入]  。
...
5. 如果您想要以 **SP** 起始模式設定應用程式，請按一下 [設定其他 URL]  ，然後執行下列步驟：
    ...
    在 [登入 URL]  文字方塊中，以下列模式輸入 URL︰`https://<sub domain>.filesanywhere.com/`

    > [!NOTE]
    > 這些都不是真正的值。 使用實際的回覆 URL 與登入 URL 更新這些值。 請連絡 [FilesAnywhere 用戶端支援小組](mailto:support@FilesAnywhere.com)以取得這些值。 您也可以參考 Azure 入口網站中**基本 SAML 組態**區段所示的模式。
...
9. 在 [設定 FilesAnywhere]  區段上，依據您的需求複製適當的 URL。
...
### <a name="configure-filesanywhere-single-sign-on"></a>設定 FilesAnywhere 單一登入

若要設定 **FilesAnywhere** 端的單一登入，您必須將從 Azure 入口網站下載的 [憑證 (Base64)]  和複製的適當 URL 傳送給 [FilesAnywhere 支援小組](mailto:support@FilesAnywhere.com)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
1. 在 Azure 入口網站中，依序選取 [企業應用程式]  、[所有應用程式]  及 [FilesAnywhere]  。

    ![企業應用程式刀鋒視窗](common/enterprise-applications.png)

2. 在應用程式清單中，選取 [FilesAnywhere]  。

    ![應用程式清單中的 FilesAnywhere 連結](common/all-applications.png)
...
### <a name="create-filesanywhere-test-user"></a>建立 FilesAnywhere 測試使用者

本節會在 FilesAnywhere 中建立名為 Britta Simon 的使用者。 FilesAnywhere 支援依預設啟用的 Just-In-Time 使用者佈建。 在這一節沒有您需要進行的動作項目。 如果 FilesAnywhere 中還沒有使用者，在驗證之後就會建立新的使用者。

### <a name="test-single-sign-on"></a>測試單一登入 

在本節中，您會使用存取面板來測試您的 Azure AD 單一登入設定。

當您在存取面板中按一下 [FilesAnywhere] 圖格時，應該會自動登入您設定 SSO 的 FilesAnywhere。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)。
