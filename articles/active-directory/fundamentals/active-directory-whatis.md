...
- **針對 IT 管理員。** Azure AD 為組織提供更安全的解決方案，使用更強的身分識別管理和單一登入 (SSO) 存取來存取數千個[雲端式 SaaS 應用程式](../saas-apps/tutorial-list.md)和內部部署應用程式。 透過這些應用程式，您的員工也員工能享受雲端式應用程式的安全性、順暢的存取、增強的共同作業、自動化的身分識別生命週期，有助於提高安全性與合規性。
...
* 請查看 Azure Active Directory 的 [開發人員指南](../develop/azure-ad-developers-guide.md)
