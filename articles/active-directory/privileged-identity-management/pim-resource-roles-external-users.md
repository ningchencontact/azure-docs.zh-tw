---
title: 邀請來賓並在 PIM 中指派 Azure 資源角色-Azure Active Directory |Microsoft Docs
...
ms.locfileid: "70804222"
---
# <a name="invite-guest-users-and-assign-azure-resource-roles-in-pim"></a>邀請來賓使用者並在 PIM 中指派 Azure 資源角色

Azure Active Directory （Azure AD）企業對企業（B2B）是 Azure AD 內的一組功能，可讓組織使用任何帳戶與外部來賓使用者（來賓）和廠商共同作業。 當您結合 B2B 與 Azure AD Privileged Identity Management （PIM）時，您可以繼續將合規性和治理需求套用至來賓。 例如，您可以將這些 PIM 功能用於具有來賓的 Azure 身分識別工作：
...
當您使用 B2B 共同作業時，您可以邀請外部使用者以來賓身分提供給您的組織。 來賓似乎在您的組織中，但來賓沒有任何相關聯的認證。 只要來賓必須經過驗證，他們就必須在其主要組織中進行驗證，而不是在您的組織中。 這表示如果來賓不再擁有其主要組織的存取權，他們也會失去貴組織的存取權。 例如，如果來賓離開其組織，他們會自動失去您在 Azure AD 中與他們共用的任何資源的存取權，而不需要執行任何動作。 如需 B2B 的詳細資訊，請參閱[什麼是 Azure Active Directory B2B 中的來賓使用者存取權？](../b2b/what-is-b2b.md)。

![此圖顯示來賓使用者在您的目錄中的顯示方式，但已在其主目錄中進行驗證](./media/pim-resource-roles-external-users/b2b-external-user.png)
...
使用 PIM，您可以邀請來賓，使其符合 Azure 資源角色的資格，如同成員使用者。
...
身為外部使用者，您必須先接受 Azure AD 組織的邀請，並可能啟用您的角色。
...
1. 系統可能會要求您接受使用規定，並指定是否要保持登入。

    Azure 入口網站隨即開啟。 如果您剛具備某個角色的資格，則沒有資源的存取權。

1. 若要啟用您的角色，請開啟含有啟用角色連結的電子郵件。 電子郵件會如下所示。

    ![來自 PIM 的電子郵件訊息，指出您符合使用「啟用角色」連結的角色](./media/pim-resource-roles-external-users/email-role-assignment.png)

1. 按一下 [啟用角色] 以在 PIM 中開啟您有資格的角色。

    ![PIM 中的 [我的角色] 頁面列出符合資格的角色](./media/pim-resource-roles-external-users/my-roles-eligible.png)
...
就像成員使用者一樣，您可以查看 audit 記錄以追蹤來賓正在進行的動作。
...
1. 若要查看來賓的活動，請按一下 [ **Azure Active Directory**  > **使用者**> 來賓名稱]。

1. 按一下 [**審核記錄**] 以查看組織的審查記錄。 如有需要，您可以搜尋篩選條件。

    ![目錄 audit 記錄清單日期、目標、起始者和活動](./media/pim-resource-roles-external-users/audit-directory.png)

## <a name="next-steps"></a>後續步驟

- [在 PIM 中指派 Azure AD 系統管理員角色](pim-how-to-add-role-to-user.md)
