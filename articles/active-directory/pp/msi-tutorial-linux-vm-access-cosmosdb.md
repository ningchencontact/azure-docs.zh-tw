...
本教學課程會示範如何以 Linux 虛擬機器建立及使用使用者指派的受控服務識別 (MSI)，然後使用該識別來存取 Azure Cosmos DB。 您會了解如何：

> [!div class="checklist"]
> * 建立使用者指派的受控服務識別 (MSI)
> * 將使用者指派的 MSI 指派至 Linux 虛擬機器
> * 將 MSI 存取權授與 Azure Cosmos DB 執行個體
> * 使用使用者指派的 MSI 識別取得存取權杖，並使用它存取 Azure Cosmos DB
...
2. 選取 [計算]，然後選取 [Ubuntu Server 16.04 LTS VM]。
...
5. 在 [資源群組] 下選擇 [建立新項目]，然後輸入此 VM 的資源群組名稱。 完成時，按一下 [確定]。
6. 選取 VM 的大小。 若要查看更多大小，請選取 [檢視全部] 或變更支援的磁碟類型篩選條件。 在 [設定] 刀鋒視窗中保留預設值，然後按一下 [確定]。
...
2. 使用 [az identity create](/cli/azure/identity#az_identity_create)，建立使用者指派的 MSI。 `-g` 參數會指定要建立 MSI 的資源群組，而 `-n` 參數則指定其名稱。 請務必以您自己的值取代 `<RESOURCE GROUP>` 和 `<MSI NAME>` 參數的值：
    ...
    回應會包含所建立之使用者指派 MSI 的詳細資料，與下列範例類似 (請記下 MSI 的 `clientId` 和 `id` 值，因為在後續步驟中將會用到)：
...
如果您還沒有 Cosmos DB 帳戶，請於此時建立。 也可以跳過此步驟，直接使用現有的 Cosmos DB 帳戶。 
...
## <a name="grant-your-user-assigned-msi-access-to-the-cosmos-db-account-access-keys"></a>將使用者指派的 MSI 存取權授與 Cosmos DB 帳戶存取金鑰

Cosmos DB 原生並不支援 Azure AD 驗證。  不過，您可以使用 MSI 從資源管理員中擷取 Cosmos DB 存取金鑰，然後使用該金鑰來存取 Cosmos DB。  在此步驟中，您會將使用者指派的 MSI 存取權授與 Cosmos DB 帳戶的金鑰。

首先，在 Azure Resource Manager 中使用 Azure CLI 將 MSI 識別存取權授與Cosmos DB 帳戶。 將 `<SUBSCRIPTION ID>`、`<RESOURCE GROUP>` 和 `<COSMOS DB ACCOUNT NAME>` 的值更新為適用於環境的值。 將 `<MSI PRINCIPALID>` 取代為[建立使用者指派的 MSI](#create-a-user-assigned-msi) 時，`az identity create` 命令所傳回的 `principalId` 屬性。  使用存取金鑰時，Cosmos DB 支援兩種層級的資料細微性：對帳戶的讀取/寫入存取，以及對帳戶的唯讀存取。  如果您想要取得帳戶的讀取/寫入金鑰，請指派 `DocumentDB Account Contributor` 角色；如果要取得帳戶的唯讀金鑰，請指派 `Cosmos DB Account Reader Role` 角色：
...
## <a name="get-an-access-token-using-the-user-assigned-msi-and-use-it-to-call-azure-resource-manager"></a>使用使用者指派的 MSI 來取得存取權杖，並使用它來呼叫 Azure Resource Manager
...
- 如需 MSI 的概觀，請參閱[適用於 Azure 資源的受控服務識別 (MSI)](msi-overview.md)。
