...
如需詳細資訊，請查看[授權需求](access-reviews-overview.md#license-requirements)。

## <a name="create-and-perform-an-access-review-for-guests"></a>建立和執行來賓的存取權檢閱

首先，身為全域管理員或使用者系統管理員，請移至 [身分[識別管理] 頁面](https://portal.azure.com/#blade/Microsoft_AAD_ERM/DashboardBlade/)，以確定您的組織已準備好存取審查。
...
1. 若要建立群組的存取權審查，請選取 [審查] 僅包含來賓使用者成員，並讓成員自行審查。 如需詳細資訊，請參閱[建立群組或應用程式的存取權審查](create-access-review.md)。

2. 要求每個來賓檢閱他們自己的成員資格。 根據預設，已接受邀請的每個來賓都會收到 Azure AD 的電子郵件，其中包含存取權檢閱的連結。 Azure AD 有關于如何[審查群組或應用程式存取權](perform-access-review.md)的來賓指示。

3. 在檢閱者提供輸入後，停止存取權檢閱並套用變更。 如需詳細資訊，請參閱[完成群組或應用程式的存取權審查](complete-access-review.md)。
...
1. 若要建立群組的存取權審查，請選取 [審查] 僅包含來賓使用者成員。 然後指定一或多個檢閱者。 如需詳細資訊，請參閱[建立群組或應用程式的存取權審查](create-access-review.md)。

2. 要求檢閱者提供意見。 根據預設，它們各自會收到 Azure AD 的電子郵件，其中包含存取面板的連結，他們會在其中[審查群組或應用程式的存取權](perform-access-review.md)。
...
1. 若要建立應用程式的存取權審查，請選取 [審查] 僅包含來賓，而該使用者會檢查自己的存取權。 如需詳細資訊，請參閱[建立群組或應用程式的存取權審查](create-access-review.md)。

2. 要求每個來賓檢閱其自己的應用程式存取權。 根據預設，已接受邀請的每個來賓都會收到 Azure AD 的電子郵件。 該電子郵件有一個連結可連至貴組織存取面板中的存取權檢閱。 Azure AD 有關于如何[審查群組或應用程式存取權](perform-access-review.md)的來賓指示。
...
1. 若要建立應用程式的存取權審查，請選取 [審查] 僅包含來賓。 然後指定一或多個使用者作為檢閱者。 如需詳細資訊，請參閱[建立群組或應用程式的存取權審查](create-access-review.md)。
...
2. 若要建立該群組的存取權審查，請選取要做為成員本身的審核者。 如需詳細資訊，請參閱[建立群組或應用程式的存取權審查](create-access-review.md)。

3. 要求每個來賓檢閱他們自己的成員資格。 根據預設，已接受邀請的每個來賓會收到 Azure AD 的電子郵件，其中的連結可連至您組織的存取面板以執行存取權檢閱。 Azure AD 有關于如何[審查群組或應用程式存取權](perform-access-review.md)的來賓指示。  未接受邀請的來賓會在檢閱結果中顯示為「未通知到」。

4. 在檢閱者提供輸入後，停止存取權檢閱。 如需詳細資訊，請參閱[完成群組或應用程式的存取權審查](complete-access-review.md)。
