...
7. 選取 [ **儲存**]。
    ...
    ![設定單一登入 [儲存] 按鈕](./media/active-directory-saas-adobeexperiencemanager-tutorial/tutorial_general_400.png)

8. 在另一個瀏覽器視窗中，開啟 **Adobe Experience Manager** 管理入口網站。

9. 選取 [Settings] \(設定\) > [Security] \(安全性\) > [Users] \(使用者\)。

    ![設定單一登入 [儲存] 按鈕](./media/active-directory-saas-adobeexperiencemanager-tutorial/tutorial_adobeexperiencemanager_user.png)
...
12. 在 [Add Certificate from CER file] \(從 CER 檔案新增憑證\) 底下，按一下 [Select Certificate File] \(選取憑證檔案\)。 瀏覽並選取您已經從 Azure 入口網站下載的憑證檔案。
    ...
    ![設定單一登入 [儲存] 按鈕](./media/active-directory-saas-adobeexperiencemanager-tutorial/tutorial_adobeexperiencemanager_user2.png)
...
16. 返回管理畫面。 然後選取 [Settings] \(設定\) > [Operations] \(作業\) > [Web Console] \(Web 主控台\)。
    ...
    ![設定單一登入 [儲存] 按鈕](./media/active-directory-saas-adobeexperiencemanager-tutorial/tutorial_adobeexperiencemanager_admin2.png)
