...
-  使用 Azure AD 應用程式 Proxy 來發行內部部署應用程式，並將 **Azure Active Directory** 設定為驗證來源。 如需指示，請參閱[使用 Azure AD 應用程式 Proxy 發行應用程式](application-proxy-publish-azure-portal.md)。 
   ...
   ![顯示內部部署應用程式設定內部 URL 和驗證](media/active-directory-b2b-hybrid-cloud-to-on-premises/OnPremAppSettings.PNG)
...
![MIM 和 B2B 指令碼解決方案的圖表](media/active-directory-b2b-hybrid-cloud-to-on-premises/MIMScriptSolution.PNG)
...
- 您可以為來賓使用者設定多重要素驗證 (MFA) 原則，以在應用程式 Proxy 驗證期間使用 MFA。 如需詳細資訊，請參閱 [B2B 共同作業使用者的條件式存取](active-directory-b2b-mfa-instructions.md)。
