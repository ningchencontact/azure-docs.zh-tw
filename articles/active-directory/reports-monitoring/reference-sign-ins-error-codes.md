...
除了使用 Azure 入口網站來存取登入資料以外，您也可以使用[報告 API](concept-reporting-api.md)。
...
|50079|使用者需要註冊第二個因素驗證 (非互動式登入)|
|50085|重新整理權杖需要社交 IDP 登入。 讓使用者嘗試以使用者名稱/密碼重新登入|
...
|50173|需要全新的驗證權杖。 讓使用者使用全新的認證來重新簽署|
...
如需詳細資訊，請參閱 [Azure Active Directory 入口網站中的登入活動報告](concept-sign-ins.md)。
