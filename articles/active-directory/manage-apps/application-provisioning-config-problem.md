...
-   **目前狀態–** 在 [服務設定] 底下畫面底部的 [ **Azure Active Directory &gt; 企業應用程式 &gt; \[應用程式名稱]\] [&gt;** 布建] 區段中，可以看到給定應用程式的最後一次布建執行的摘要。 [目前狀態] 區段會顯示布建週期是否已開始布建使用者帳戶。 您可以觀賞迴圈的進度、查看已布建多少個使用者和群組，以及查看有多少角色已建立。 如有任何錯誤，可在布建[記錄（預覽）](../reports-monitoring/concept-provisioning-logs.md?context=azure/active-directory/manage-apps/context/manage-apps-context)中找到詳細資料。
