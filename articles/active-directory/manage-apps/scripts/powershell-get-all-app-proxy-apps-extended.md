...
description: PowerShell 範例，其中會列出所有 Azure Active Directory （Azure AD）應用程式 Proxy 應用程式以及應用程式識別碼（AppId）、名稱（DisplayName）、外部 URL （ExternalUrl）、內部 URL （InternalUrl）和驗證類型（ExternalAuthenticationType).
...
此 PowerShell 腳本範例會列出所有 Azure Active Directory （Azure AD）應用程式 Proxy 應用程式的相關資訊，包括應用程式識別碼（AppId）、名稱（DisplayName）、外部 URL （ExternalUrl）、內部 URL （InternalUrl），以及驗證類型（ExternalAuthenticationType）。
