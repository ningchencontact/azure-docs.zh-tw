...
description: 此 PowerShell 範例會列出所有使用預設網域（. msappproxy.net）的 Azure Active Directory （Azure AD）應用程式 Proxy 應用程式。
...
---
# <a name="get-all-application-proxy-apps-using-default-domains-msappproxynet"></a>使用預設網域取得所有應用程式 Proxy 應用程式（. msappproxy.net）

此 PowerShell 腳本範例會列出所有使用預設網域（. msappproxy.net）的 Azure Active Directory （Azure AD）應用程式 Proxy 應用程式。
