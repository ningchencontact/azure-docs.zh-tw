...
![應用程式的 [屬性] 頁面上找到的應用程式識別碼](./media/application-proxy-wildcard/01.png)
...
- 內部 URL：
    ...
    ![範例：在 內部 URL 的萬用字元](./media/application-proxy-wildcard/42.png)

- 外部 URL：

    ![範例：在 外部 URL 的萬用字元](./media/application-proxy-wildcard/43.png)

- 內部應用程式 SPN：

    ![範例：在 SPN 組態的萬用字元](./media/application-proxy-wildcard/44.png)

藉由發行萬用字元應用程式，您現在可以透過瀏覽至您使用的 URL 來存取三個應用程式 (例如，`travel.adventure-works.com`)。

設定會實作下列結構：

![顯示的範例組態所實作的結構](./media/application-proxy-wildcard/05.png)

| 色彩 | 描述 |
| ---   | ---         |
| 藍色  | 明確發行並顯示在 Azure 入口網站中的應用程式。 |
...
- 在 [內部 URL]  中，您設定 **finance** 而不是萬用字元。
    ...
    ![範例：在內部 URL 中設定而不是萬用字元的財務](./media/application-proxy-wildcard/52.png)

- 在 [外部 URL]  中，您設定 **finance** 而不是萬用字元。

    ![範例：在 外部 URL 中設定而不是萬用字元的財務](./media/application-proxy-wildcard/53.png)

- 在內部應用程式 SPN中，您設定 **finance** 而不是萬用字元。

    ![範例：在 SPN 組態中設定而不是萬用字元的財務](./media/application-proxy-wildcard/54.png)

設定會實作下列案例：

![顯示的範例案例所實作的組態](./media/application-proxy-wildcard/09.png)
...
- 若要深入了解**自訂網域**，請參閱[使用的 Azure AD 應用程式 Proxy 中的自訂網域](application-proxy-configure-custom-domain.md)。
- 若要深入了解**發佈應用程式**，請參閱[使用 Azure AD 應用程式 Proxy 發佈應用程式](application-proxy-add-on-premises-application.md)
