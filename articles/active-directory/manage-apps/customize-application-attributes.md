...
1. 依序展開**對應**來檢視和編輯 Azure AD 之間流動的使用者屬性和目標應用程式。 如果目標應用程式支援的話，這個區段可讓您選擇性地設定 佈建群組和使用者帳戶。
   ...
   ![使用對應來檢視和編輯使用者屬性](./media/customize-application-attributes/21.png)

1. 選取 **對應**以開啟相關的組態**屬性對應**螢幕。 若要正常運作的 SaaS 應用程式需要某些屬性對應。 若為必要的屬性，[刪除]  功能就無法使用。

   ![若要設定應用程式的屬性對應中使用 屬性對應](./media/customize-application-attributes/22.png)
...
1. 選取現有**屬性對應**來開啟**編輯屬性**螢幕。 您可以在這裡編輯 Azure AD 之間流動的使用者屬性和目標應用程式。
   ...
   ![若要編輯使用者屬性中使用 編輯屬性](./media/customize-application-attributes/23.png)
...
![範例會示範 ServiceNow 與佈建群組和使用者物件](./media/customize-application-attributes/24.png)
...
- 屬性 IsSoftDeleted 通常是應用程式的預設對應的一部分。 IsSoftdeleted 可以是任一四個案例中，則為 true （使用者不在範圍內，因未獲指派從應用程式，使用者不在範圍內，因為不符合 [範圍篩選器、 已虛刪除使用者在 Azure AD 中，或 AccountEnabled] 屬性設定為 false 使用者）。 
