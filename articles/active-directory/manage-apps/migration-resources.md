...
| [部署方案：從 AD FS 移轉至密碼雜湊同步](https://aka.ms/ADFSTOPHSDPDownload) \(英文\) | 使用密碼雜湊同步處理時，可以將使用者密碼的雜湊從內部部署 Active Directory 同步至 Azure AD。 這可讓 Azure AD 驗證使用者，而不需要與內部部署 Active Directory 互動。| 
