...
- 首先檢查的一般問題
  ...
  * 確定**條件式存取原則**或**身分識別保護**原則未封鎖使用者存取。 [檢查特定的條件式存取原則](#problems-with-conditional-access-policies)或是[檢查特定應用程式的條件式存取原則](#check-a-specific-applications-conditional-access-policy)或[停用特定的條件式存取原則](#disable-a-specific-conditional-access-policy)
