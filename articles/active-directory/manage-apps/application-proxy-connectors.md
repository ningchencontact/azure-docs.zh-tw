...
若要成功部署應用程式 Proxy，您至少需要一個連接器，但我們建議兩個以上可獲得較佳的復原功能。 執行 Windows Server 2012 R2 的電腦上安裝連接器或更新版本。 連接器需與應用程式 Proxy 服務以及您發佈的內部部署應用程式進行通訊。
...
\* 此機器會用來引發一些預設連線限制，超出建議的設定的.NET 自訂設定。 我們建議先使用預設設定執行測試，然後再連絡支援人員為您的租用戶變更此限制。
...
在允許連接器將要求傳送至應用程式 Proxy 服務的網路上，任何地方都可以安裝連接器。 重點是執行連接器的電腦也可存取您的應用程式。 您可以在貴公司網路內或在雲端中執行的虛擬機器上安裝連接器。 連接器可以執行在周邊網路中，也稱為非軍事區 (DMZ)，但它不需要因為所有流量都是輸出，因此您的網路保持安全。
...
- **網域控制站**：如果連接器使用 Kerberos 限制委派來執行單一登入 (SSO)，其會先連絡網域控制器後，才將要求傳送至後端。 連接器有 Kerberos 票證的快取 (但是在忙碌環境中)，網域控制器的回應速度可能會影響效能。 在 Azure 中執行、但與內部部署之網域控制器通訊的連接器會更常發生這個問題。
...
您可以檢查 [服務] 視窗中的服務狀態。 連接器包含兩個 Windows 服務︰實際連接器和更新程式。 這兩者必須一直執行。
 ...
 ![範例：服務視窗會顯示 Azure AD 服務本機](./media/application-proxy-connectors/aad-connector-services.png)
