...
若要設定權杖加密，您需要將包含公開金鑰的 X509 憑證檔案，上傳到代表該應用程式的 Azure AD 應用程式物件。 若要取得 X509 憑證，您可以從應用程式本身下載它，或者在應用程式廠商提供加密金鑰的情況下，從應用程式廠商處取得，或者在應用程式希望您提供私用金鑰的情況下，使用加密工具、上傳至應用程式金鑰存放區的私用金鑰部分，以及上傳至 Azure AD 的對應公開金鑰憑證，來加以建立。

Azure AD 會使用 AES-256 加密 SAML 判斷提示的資料。

## <a name="configure-saml-token-encryption"></a>設定 SAML 權杖加密

若要設定 SAML 權杖加密，請遵循下列步驟。

1. 取得與應用程式中設定的私用金鑰對應的公開金鑰憑證。

    建立要用於加密的非對稱金鑰組。 或者，如果應用程式會提供用來加密的公開金鑰，請遵循應用程式的指示下載 X509 憑證。

    公用金鑰應該儲存在 .cer 格式的 X509 憑證檔案中。
...
1. 在 [權杖加密] 頁面上，選取 [匯入憑證] 以匯入包含公用 X509 憑證的 .cer 檔案。

    ![匯入包含 X509 憑證的 .cer 檔案](./media/howto-saml-token-encryption/import-certificate-small.png)
...
1. 使用 X509 憑證更新應用程式的 `keyCredentials` 以進行加密。 下列範例示範如何執行。
