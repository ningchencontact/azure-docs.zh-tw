...
* **目標系統** - Azure AD 佈建服務用來進行同步處理的目標端使用者存放庫。 這通常是 SaaS 應用程式（例如 Salesforce、ServiceNow、G Suite、Dropbox for Business），但在某些情況下，可以是內部部署系統，例如 Active Directory （例如： Workday 輸入同步處理至 Active Directory）。
