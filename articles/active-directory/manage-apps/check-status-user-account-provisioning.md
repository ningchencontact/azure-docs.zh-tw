...
ms.locfileid: "67807701"
---
# <a name="tutorial-reporting-on-automatic-user-account-provisioning"></a>教學課程：關於使用者帳戶自動佈建的報告

Azure Active Directory (Azure AD) 包含[使用者帳戶佈建服務](user-provisioning.md)可協助自動化佈建解除佈建使用者帳戶在 SaaS 應用程式和其他系統，以端對端身分識別生命週期管理。 對於 [Azure AD 應用程式庫](https://azuremarketplace.microsoft.com/marketplace/apps/category/azure-active-directory-apps?page=1&subcategories=featured)之 [精選] 區段中的所有應用程式和系統，Azure AD 支援已預先整合的使用者佈建連接器。
...
* **Azure 入口網站**-這篇文章主要在說明擷取報告資訊，從[Azure 入口網站](https://portal.azure.com)，可同時佈建摘要報告，以及詳細的佈建稽核記錄檔指定應用程式。
...
## <a name="getting-provisioning-reports-from-the-azure-portal"></a>取得佈建在 Azure 入口網站的報告

若要取得佈建指定的應用程式的報表資訊，請先啟動[Azure 入口網站](https://portal.azure.com)並瀏覽至 企業應用程式，設定佈建。 例如，如果您要在 LinkedIn Elevate 中佈建使用者，則應用程式詳細資料的導覽路徑為︰
...
1. 匯出事件︰如果同步處理規則事件指出應執行某動作 (新增、更新、刪除)，則匯出事件中會記錄該動作的執行結果。
   ...
   ![範例：顯示活動和狀態的 [稽核記錄] 頁面](./media/check-status-user-account-provisioning/audit_logs.PNG)
