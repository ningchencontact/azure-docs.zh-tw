...
1. 登入裝載您的 Active Directory 網域服務 (AD DS) 安裝的伺服器。
1. 在 [伺服器管理員] 中，移至 [工具]   >  [群組原則管理]  。
...
## <a name="step-4-auto-enable-the-extension-for-internet-explorer"></a>步驟 4：自動啟用擴充功能的 Internet Explorer
...
## <a name="step-5-optional-disable-remember-password-prompt"></a>步驟 5 (選用)：停用 [記住密碼] 提示字元

當使用者登入使用存取面板擴充功能的網站時，Internet Explorer 可能會顯示下列提示詢問「是否要儲存密碼？」

![顯示 「 您要儲存您的密碼...」提示字元](./media/deploy-access-panel-browser-extension/remember-password-prompt.png)
...
1. 尋找名為 [開啟表單上使用者名稱和密碼的自動完成功能]  的設定。
   ...
   > 舊版的 Active Directory 可能會列出這項設定，但名為 [不允許以自動完成儲存密碼]  。 該設定的組態不同於本教學課程中所描述的設定。
    ...
    ![請記得將 [使用者設定] 下尋找此](./media/deploy-access-panel-browser-extension/disable-auto-complete.png)

1. 以滑鼠右鍵按一下上述設定，然後選取 [編輯]  。
1. 在標題為 [開啟表單上使用者名稱和密碼的自動完成功能]  的視窗中選取 [停用]  。

    ![選取開啟自動完成功能上的 [停用] 選項](./media/deploy-access-panel-browser-extension/disable-passwords.png)
...
## <a name="step-6-testing-the-deployment"></a>步驟 6：測試和部署
...
1. 可能需要幾個登入若要完全更新與此機器的群組原則變更。 若要強制更新，開啟 [命令提示字元]  視窗，然後執行下列命令：`gpupdate /force`
