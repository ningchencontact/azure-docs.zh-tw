...
*錯誤 AADSTS70001：在目錄\/* 中找不到識別碼為 ' HTTPs:/contoso.com ' 的應用程式。

**可能的原因**

從應用程式傳送至 SAML 要求中 Azure AD 的屬性,與在AzureAD中為應用程式設定的識別碼值不相符。`Issuer`
...
請確定 SAML `AssertionConsumerServiceURL`要求中的值符合 Azure AD 中設定的 [回復 URL] 值。 如果您在 Azure 入口網站中使用具有我的應用程式安全瀏覽器擴充功能的[測試經驗](../develop/howto-v1-debug-saml-sso-issues.md), 則不需要手動執行這些步驟。
...
*錯誤 AADSTS50105：已登入的使用者 '\@brian contoso.com ' 未指派給應用程式*的角色。
...
Azure AD 不支援應用程式針對單一登入所傳送的 SAML 要求。 以下為一些常見問題：
...
1. 捕捉 SAML 要求。 請遵循[如何在 Azure AD 中將 saml 型單一登入進行偵錯工具](../develop/howto-v1-debug-saml-sso-issues.md)教學課程, 以瞭解如何捕捉 saml 要求。
