...
1. 開啟檔案，並將內容解壓縮至您的電腦。
1. 若要執行此工具，以滑鼠右鍵按一下名為的檔案*AccessPanelExtensionDiagnosticTool.js* ，然後選取**以開啟** > **Microsoft Windows Based Script Host**.
...
1. 檢閱診斷結果會出現，並選取**是**以修正問題。 **檢查結果** 對話方塊隨即出現該怎麼辦的擴充功能無法運作的相關資訊。  
1. 讀取訊息，然後選取**確定**。
...
1. 移至**程式**索引標籤，然後選取**管理附加元件**。
1. 選取 **存取面板延伸模組**中**Microsoft Corporation**區段，然後選取**啟用**。
1. 若要儲存變更，關閉所有 Internet Explorer 瀏覽器視窗中，您必須開啟。 變更在的下次開啟 Internet Explorer 會生效。
...
1. 在 Internet Explorer 中，選取**齒輪圖示**右上角的視窗，然後選取**網際網路選項**。
1. 移至**隱私權**索引標籤，然後確認**InPrivate 瀏覽啟動時停用工具列和延伸模組**已清除核取方塊。
1. 若要儲存變更，關閉所有 Internet Explorer 瀏覽器視窗中，您必須開啟。 變更在的下次開啟 Internet Explorer 會生效。
...
1. 在搜尋結果中，選取**解除安裝程式**。
    ...
    ![從控制項面板中選取 解除安裝程式選項](./media/manage-access-panel-browser-extension/uninstall-program-control-panel.png)

1. 從清單中，選取**存取面板延伸模組**，然後選取**解除安裝**。
