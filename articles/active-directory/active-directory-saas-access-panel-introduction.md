...
需要有此延伸模組，使用者才能登入密碼型單一登入。 安裝之後，使用者也可以按一下 [登入以開始使用]，透過登入延伸模組來啟用其他功能。 

- 使用者瀏覽應用程式的 [登入 URL]，即可直接登入應用程式。 使用者巡覽至應用程式的登入 URL 時，延伸模組將會偵測到此作業並提供選項，讓使用者從延伸模組登入它。
- 使用者也可以從存取面板使用延伸模組的**快速搜尋**功能，來啟動其任何應用程式。 
- 延伸模組也會在 [最近使用的] 區段下方，向使用者顯示他們啟動的最後三個應用程式。
...
如果您使用的「我的應用程式 URL」與 https://myapps.microsoft.com 不同，則必須透過下列步驟設定您的預設 URL：
1. 在尚未登入延伸模組時，**以滑鼠右鍵按一下**延伸模組圖示。
2. 按一下功能表中的 [Select My Apps URL] (選取我的應用程式 URL)。
3. **選取**預設 URL。
4. 按一下延伸模組圖示。
