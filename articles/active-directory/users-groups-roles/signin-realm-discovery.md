...
ms.locfileid: "59007517"
---
# <a name="home-realm-discovery-during-sign-in-for-microsoft-365-services"></a>Microsoft 365 服務的登入期間的主領域探索
...
在過去，提供在登入網域，或某些舊版的應用程式的主領域探索原則所控管主領域探索。 比方說，在舊版的探索行為的 Azure Active Directory 使用者可以輸入其使用者名稱，但仍會在其組織的認證集合畫面會送達。 發生這種情況的正確使用者提供的組織的網域名稱為"contoso.com"時。 此行為不允許的資料粒度，以自訂個別的使用者體驗。
...
除了改善的登入的使用者經驗，這項變更會包含可以協助降低大規模的使用者名稱的列舉型別的不當使用的機制。

這項變更最初針對受控網域並開始推出在 2019 年，但無法啟動 2019年結尾所導入的同盟網域。 同盟網域的確切的推出日期取決於客戶的意見反應。

> [!IMPORTANT]
> 這項功能會依賴舊的網域層級主領域探索來強制同盟的同盟網域有顯著的影響。 某些組織有訓練員工能夠使用 Azure Active Directory 中不存在，但是包含適當的網域名稱，使用者名稱登入，因為網域名稱將使用者路由目前到其組織的網域的端點。 新的登入行為不允許這樣做。 若要修正使用者名稱，會通知使用者，它們並不允許使用不存在於 Azure Active Directory 的使用者名稱登入。
>
> 如果您或貴組織有相依於舊版的行為的做法，請務必為組織系統管理員以更新員工的登入和驗證的文件，並訓練員工使用其 Azure Active Directory 使用者名稱登入。
  
如果您有新的行為方面的疑慮，請留下您的註解中**意見反應**一節。  
