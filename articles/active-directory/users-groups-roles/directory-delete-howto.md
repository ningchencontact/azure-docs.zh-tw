...
在您當初設定 Azure Active Directory 租用戶時，可能也為組織啟用了以授權為基礎的訂用帳戶，像是 Azure Active Directory Premium P2、Office 365 商務進階版或企業行動力 + 安全性 E5。 為了避免資料意外遺失，在完全刪除這些訂用帳戶之前，將無法無法刪除目錄。 訂用帳戶的狀態必須為**取消佈建**，租用戶才能刪除。 **過期**或**取消**的訂用帳戶會進入**停用**狀態，而最終階段為**取消佈建**狀態。 
