...
組織有 1,000 位使用者。 所有使用者都需要 Enterprise Mobility + Security (EMS) 授權。 200 位使用者是在財務部門，而且需要 Office 365 企業版 E3 授權。 組織目前有內部部署執行的 PowerShell 指令碼，並在使用者就職和離職時新增及移除授權。 不過，組織想要將指令碼取代為群組授權，這樣授權就可以自動由 Azure AD 管理。
