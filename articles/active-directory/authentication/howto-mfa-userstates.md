---
title: Azure 多重要素驗證使用者狀態-Azure Active Directory
...
| Enabled |已在 Azure MFA 中註冊使用者，但使用者尚未註冊。 系統將在他們下一次登入時提示他們註冊。 |資料分割  它們會繼續運作，直到註冊程序完成為止。 | 是的。 工作階段到期之後，必須進行 Azure MFA 註冊。| 是的。 存取權杖到期之後，必須進行 Azure MFA 註冊。 |
| 已強制 |已註冊使用者，而且使用者已完成 Azure MFA 的註冊程序。 |是的。 應用程式需要應用程式密碼。 |是的。 在登入時需要使用 Azure MFA。 | 是的。 在登入時需要使用 Azure MFA。 |
