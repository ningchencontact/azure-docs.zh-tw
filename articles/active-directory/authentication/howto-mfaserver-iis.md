---
title: IIS 驗證和 Azure MFA Server-Azure Active Directory
...
2. 按一下 [HTTP] 索引標籤。
3. 按一下 [新增]。
4. 在 [新增基底 URL] 對話方塊中，輸入執行 HTTP 驗證之網站的 URL （例如 <http://localhost/owa>），並提供應用程式名稱（選擇性）。 應用程式名稱會出現在 Azure Multi-Factor Authentication 報表中，而且可能顯示在簡訊或行動應用程式驗證訊息內。
...
1. 如果在 IIS 6 上執行，請按一下 [ **ISAPI** ] 索引標籤。選取 web 應用程式執行所在的網站（例如 [預設的網站]），以啟用該網站的 Azure 多因素驗證 ISAPI 篩選外掛程式。
