---
title: Azure MFA 版本和取用方案-Azure Active Directory
...
> [!NOTE]
> 自 2018 年 9 月 1 日起，新客戶無法再將 Azure Multi-Factor Authentication 當做獨立供應項目購買。 多重要素驗證將繼續以 Azure AD Premium 或 Microsoft 365 商務版授權的功能形式提供。
...
購買包含 Azure 多重要素驗證的授權，例如 Azure Active Directory Premium，或包含 Azure AD Premium 或條件式存取的授權配套，並將它們指派給 Azure Active Directory 中的使用者。

### <a name="consumption-based-licensing"></a>以耗用量為基礎的授權
...
請務必注意，您可以有授權，但仍需支付以耗用量為基礎的組態。 如果您設定了每個驗證的 Azure MFA 提供者，則會向您收取每個雙步驟驗證要求的費用，甚至是具有授權的使用者所執行的要求。 如果在不連結至 Azure AD 租用戶的網域上設定每個使用者 Azure MFA 提供者，則會以每個啟用的使用者計費，即使您的使用者在 Azure AD 具有授權。
