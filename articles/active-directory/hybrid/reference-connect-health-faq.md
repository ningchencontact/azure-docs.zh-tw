...
**問：在安裝 Azure AD Connect Health 代理程式期間，是否需要重新啟動我的伺服器？**

資料分割 安裝代理程式不需要您重新啟動伺服器。 不過，安裝某些先決條件的步驟可能需要重新啟動伺服器。

例如，在 Windows Server 2008 R2 上安裝 .NET 4.5 Framework 需要重新啟動伺服器。

**問：Azure AD Connect Health 是否透過傳遞 HTTP Proxy 運作？**

是的。 若是進行中的作業，您可以將 Health 代理程式設定為使用 HTTP Proxy 來轉送輸出 HTTP 要求。
...
資料分割 目前不支援為基本驗證指定任意使用者名稱和密碼的機制。
...
**問：我收到有關「健全狀況服務的資料並非最新」的警示。我該如何進行解決這個問題？**
...
**問：如何解決 Azure AD Connect Health 警示？**
...
**問：我收到「測試驗證要求 (綜合交易) 無法取得權杖」警示。我該如何進行解決這個問題？**
