...
**问：Azure AD Connect Health 是否支援 Azure 德國雲端？**
...
**问：在個別的伺服器上安裝 Azure AD Connect Health 代理程式有什麼影響？**
...
**问：在安裝 Azure AD Connect Health 代理程式期間，是否需要重新啟動我的伺服器？**

沒有。 安裝代理程式不需要您重新啟動伺服器。 不過，安裝某些先決條件的步驟可能需要重新啟動伺服器。

例如，在 Windows Server 2008 R2 上安裝 .NET 4.5 Framework 需要重新啟動伺服器。

**问：Azure AD Connect Health 是否透過傳遞 HTTP Proxy 運作？**

是。 若是進行中的作業，您可以將 Health 代理程式設定為使用 HTTP Proxy 來轉送輸出 HTTP 要求。
深入了解[設定 Health 代理程式的 HTTP Proxy](how-to-connect-health-agent-install.md#configure-azure-ad-connect-health-agents-to-use-http-proxy)。
...
沒有。 目前不支援為基本驗證指定任意使用者名稱和密碼的機制。

**问：我需要開放哪些防火牆連接埠，Azure AD Connect Health 代理程式才能運作？**

請參閱[需求](how-to-connect-health-agent-install.md#requirements)一節，以取得防火牆連接埠清單與其他的連線需求。

**问：為什麼我會在 Azure AD Connect Health 入口網站中看到兩部名稱相同的伺服器？**
...
**问：Health 代理程式註冊失敗的常見原因為何？該如何解決問題？**

Health 代理程式會因為下列可能原因而無法註冊：

* 因為防火牆封鎖流量，代理程式無法與所需端點通訊。 在 Web 應用程式 Proxy 伺服器上尤其常見。 請確定您已允許針對所需端點和連接埠的輸出通訊。 如需詳細資訊，請參閱[需求](how-to-connect-health-agent-install.md#requirements)一節。
...
**问：我收到有關「健全狀況服務的資料並非最新」的警示。我該如何進行解決這個問題？**
...
**问：是否需要在 Web 應用程式 Proxy 伺服器上啟用稽核？**

否，不需要在 Web 應用程式 Proxy 伺服器上啟用稽核。

**问：如何解決 Azure AD Connect Health 警示？**
...
**问：我收到「測試驗證要求 (綜合交易) 無法取得權杖」警示。我該如何進行解決這個問題？**
...
**问：我收到電子郵件，指出無法針對最近的勒索軟體攻擊修補我的機器。我為什麼收到這封電子郵件？**
...
**问：為什麼 PowerShell Cmdlet <i>Get-MsolDirSyncProvisioningError</i> 在結果中顯示的同步錯誤比較少？**
