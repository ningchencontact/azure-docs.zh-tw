---
title: 混合式身分識別設計 - Azure 生命週期採用策略 | Microsoft Docs
...
* 系統管理員可以提供條件式存取根據應用程式資源、 裝置和使用者身分識別、 網路位置和 multi-factor authentication。
...
| 同步處理管理選項 | 優點 | 缺點 |
| --- | --- | --- |
| 同步型 (透過 DirSync 或 AADConnect) |從內部部署和雲端同步的使用者和群組 <br>  **原則控制**：系統管理員可以透過 Active Directory 設定帳戶原則，無須在雲端中執行額外的工作，即可管理密碼原則、工作站、限制、鎖定控制等。  <br>  **存取控制**：可以限制對雲端服務的存取，以便透過公司環境、線上伺服器或兩者來存取服務。 <br>  減少撥打支援電話的次數：使用者要記住的密碼越少，就越不可能忘記密碼。 <br>  安全性：使用者身分識別和資訊會受到保護，因為單一登入中使用的所有伺服器和服務都由內部部署環境掌控。 <br>  支援增強式驗證：您可以搭配雲端服務使用增強式驗證 (也稱為雙重要素驗證)。 不過，如果使用增強式驗證，則必須使用單一登入。 | |
| 同盟型 (透過 AD FS) |由 Security Token Service (STS) 啟用。 當您設定 STS 以提供 Microsoft 雲端服務的單一登入存取時，您會在內部部署 STS 與您在 Azure AD 租用戶中指定的同盟網域之間建立同盟信任。 <br> 可讓使用者使用同一組認證來取得多個資源的存取權 <br>使用者不需要維護多組認證。 但是，使用者必須向每個參與的資源提供認證。支援 B2B 和 B2C 案例。 |需要專業的人員的部署和維護專用的內部部署 AD FS 伺服器上。 如果您打算對 STS 使用 AD FS，使用增強式驗證會受到限制。 如需詳細資訊，請參閱 [設定 AD FS 2.0 的進階選項](https://go.microsoft.com/fwlink/?linkid=235649)。 |
