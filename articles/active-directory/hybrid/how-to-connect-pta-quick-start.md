...
4. 如果您的伺服器和 Azure AD 之間有防火牆，請設定下列項目：
   ...
   - 驗證憑證，請解除封鎖下列 Url: **mscrl.microsoft.com:80**， **crl.microsoft.com:80**， **ocsp.msocsp.com:80**，和**www\.microsoft.com:80**。 由於這些 URL 會用於其他 Microsoft 產品的憑證驗證，因此您可能已將這些 URL 解除封鎖。
...
安裝多個傳遞驗證代理程式，可確保高可用性，但不是確定性的負載平衡驗證代理程式之間。 若要判斷您的租用戶所需的幾個驗證代理程式，請考慮尖峰和平均登入要求，您會看到您的租用戶上的負載。 在標準的 4 核心 CPU、16-GB RAM 伺服器上，單一驗證代理程式每秒可處理 300 到 400 次驗證，乃是效能評定的基準。
...
對大多數客戶而言是能夠以高可用性和容量共有三個驗證代理程式。 應在靠近網域控制站的地方安裝驗證代理程式，以改善登入的延遲情形。
...
>[!IMPORTANT]
>如果虛擬機器上安裝驗證代理程式，您無法複製虛擬機器，設定另一個驗證代理程式。 這個方法很**不支援**。

## <a name="step-5-configure-smart-lockout-capability"></a>步驟 5：設定智慧鎖定功能

智慧鎖定可鎖定嘗試猜測使用者密碼的不良執行者或使用暴力的方法來取得協助。 藉由在內部部署 Active Directory 中設定 Azure AD 中的智慧鎖定設定和/或適當的鎖定設定，攻擊可以篩選出其到達 Active Directory。 讀取[這篇文章](../authentication/howto-password-smart-lockout.md)若要深入了解如何在您要保護您的使用者帳戶的租用戶設定智慧鎖定。
