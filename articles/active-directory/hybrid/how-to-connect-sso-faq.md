...
**問：哪些應用程式會利用無縫 SSO 的 `domain_hint` 或 @no__t 1 參數功能？**
...
| 網路版 Outlook | HTTPs： \//office365 .com/contoso .com |
| Office 365 入口網站 | HTTPs： \//portal. domain_hint = contoso .com，HTTPs： \//www .com？ domain_hint = contoso .com |
...
**問：無縫 SSO 支援 `Alternate ID` 做為使用者名稱，而不是 `userPrincipalName`？**
...
請在執行 Azure AD Connect 的內部部署伺服器上依照下列步驟操作：
   ...
   **Step 1。取得已啟用無縫 SSO 的 AD 樹系列表（@ no__t-0）
   ...
   **Step 2。更新在 @ no__t-0 上設定的每個 AD 樹系上的 Kerberos 解密金鑰
...
**問：如何停用無縫 SSO？**
   ...
   **Step 1。停用您租使用者的功能 @ no__t-0

   **選項 A：使用 Azure AD Connect @ no__t-0 停用
   ...
   **Step 2。取得已啟用無縫 SSO 的 AD 樹系列表（@ no__t-0）
   ...
   **Step 3。從您看到的每個 AD 樹系中，手動刪除 `AZUREADSSOACCT` 電腦帳戶。**
