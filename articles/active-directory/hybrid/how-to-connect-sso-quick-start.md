...
>[!IMPORTANT]
> 無縫 SSO 會建立名為的電腦帳戶`AZUREADSSOACC`您內部部署中每個 AD 樹系的 Active Directory (AD) 中。 `AZUREADSSOACC`電腦帳戶需要強保護，基於安全性考量。 只有網域系統管理員應該能夠管理的電腦帳戶。 確定的已停用電腦帳戶的 Kerberos 委派，，而且沒有 Active Directory 中的其他帳戶有委派權限`AZUREADSSOACC`電腦帳戶。 它們是安全遭到意外刪除，而且只有網域系統管理員可以存取儲存的電腦帳戶在組織單位 (OU)。
