...
- 您可以讓使用者使用他們的 Azure AD 帳戶自動登入 Canvas (單一登入)
...
若要讓單一登入能夠運作，Azure AD 必須知道 Canvas 與 Azure AD 中互相對應的使用者。 換句話說，必須在 Azure AD 使用者與 Canvas 中的相關使用者之間建立連結關聯性。

在 Canvas 中，將 [Username] 的值指派為 Azure AD 中 [使用者名稱] 的值，以建立連結關聯性。
...
在本節中，您會在 Azure 入口網站中啟用 Azure AD 單一登入，然後在您的 Canvas 應用程式中設定單一登入。

**若要設定與 Canvas 搭配運作的 Azure AD 單一登入，請執行下列步驟：**

1. 在 Azure 入口網站的 [Canvas] 應用程式整合頁面上，按一下 [單一登入]。
...
若要讓 Azure AD 使用者能夠登入 Canvas，必須將他們佈建到 Canvas。
...
**若要將 Britta Simon 指派給 Canvas，請執行下列步驟：**
