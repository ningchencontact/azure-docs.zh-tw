...
3. **[建立 TINFOIL SECURITY 測試使用者](#create-a-tinfoil-security-test-user)** - 使 TINFOIL SECURITY 中對應的 Britta Simon 連結到該使用者在 Azure AD 中的代表項目。
...
7. 在 [TINFOIL SECURITY 組態] 區段上，按一下 [設定 TINFOIL SECURITY] 以開啟 [設定登入] 視窗。 從 [快速參考] 區段中複製 [SAML 單一登入服務 URL]。
...
當您在存取面板中按一下 [TINFOIL SECURITY] 圖格時，應該會自動登入 TINFOIL SECURITY 應用程式。 如需「存取面板」的詳細資訊，請參閱[存取面板簡介](active-directory-saas-access-panel-introduction.md)。
