...
4. 在搜尋方塊中，輸入 **FactSet**，從結果面板中選取 [FactSet]，然後按一下 [新增] 按鈕以新增應用程式。
    ...
    ![結果清單中的 FactSet](./media/active-directory-saas-factset-tutorial/tutorial_factset_addfromgallery.png)
...
3. **[建立 FactSet 測試使用者](#create-a-factset-test-user)** - 在 FactSet 中建立 Britta Simon 的對應項目，且該項目與 Azure AD 中代表使用者的項目連結。
...
1. 在 Azure 入口網站的 [FactSet] 應用程式整合頁面上，按一下 [單一登入]。
...
3. 在 [FactSet 網域與 URL] 區段上，執行下列步驟：

    ![FactSet 網域與 URL 單一登入資訊](./media/active-directory-saas-factset-tutorial/tutorial_factset_url.png)
...
6. 若要在 **FactSet** 端設定單一登入，您必須將已下載的**中繼資料 XML** 傳送給 [FactSet 支援小組](mailto:support@factset.com)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
