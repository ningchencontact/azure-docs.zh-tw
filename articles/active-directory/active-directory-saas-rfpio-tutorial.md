...
5. 在結果窗格中，選取 [RFPIO]，然後選取 [新增] 按鈕以新增應用程式。
...
3. **[建立 RFPIO 測試使用者](#creating-a-rfpio-test-user)** -- 在 RFPIO 中建立一個與 Azure AD 中代表使用者的項目連結的 Britta Simon 對應項目。
...
3. 如果您想要以 **IDP** 起始模式設定應用程式，請在 [RFPIO 網域和 URL] 區段上執行下列步驟：
...
7. 在不同的網頁瀏覽器視窗中，以系統管理員身分登入您的 **RFPIO** 網站。
...
* [有關如何與 Azure Active Directory 整合 SaaS 應用程式的教學課程清單](active-directory-saas-tutorial-list.md)
