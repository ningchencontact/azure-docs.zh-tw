...
在 Azure 資源 ([Azure 虛擬機器](msi-qs-configure-cli-windows-vm.md)或 [Azure 虛擬機器擴展集](msi-qs-configure-cli-windows-vmss.md)) 上啟用 MSI 之後： 

1. 如果您要在本機主控台中使用 Azure CLI，請先使用 [az login](/cli/azure/#az_login) 登入 Azure。 使用您想部署 VM 或虛擬機器擴展集且已與 Azure 訂用帳戶相關聯的帳戶：
...
- 若要在 Azure 虛擬機器上啟用 MSI，請參閱[使用 Azure CLI 設定 Azure VM 受控服務身分識別 (MSI)](msi-qs-configure-cli-windows-vm.md)。
- 若要在 Azure 虛擬機器擴展集上啟用 MSI，請參閱[使用 Azure 入口網站設定 Azure 虛擬機器擴展集的受控服務身分識別 (MSI)](msi-qs-configure-portal-windows-vmss.md)
