...
4. 在搜尋方塊中，輸入 **G Suite**，從結果面板中選取 **G Suite**，然後按一下 [新增] 按鈕以新增應用程式。
    ...
    ![結果清單中的 G Suite](./media/active-directory-saas-googleapps-tutorial/tutorial_googleapps_addfromgallery.png)
...
3. 在 [G Suite 網域與 URL] 區段上，如果要設定 **Gmail**，執行下列步驟：

    ![G Suite 網域與 URL 單一登入資訊](./media/active-directory-saas-googleapps-tutorial/tutorial_googleapps_urlgmail.png)
...
4. 在 [G Suite 網域與 URL] 區段上，如果要設定 **Google Cloud Platform**，執行下列步驟：

    ![G Suite 網域與 URL 單一登入資訊](./media/active-directory-saas-googleapps-tutorial/tutorial_googleapps_url1.png)
...
7. 在 [G Suite 設定] 區段上，按一下 [設定 G Suite] 以開啟 [設定登入] 視窗。 從 [快速參考] 區段中，複製 [登出 URL、SAML 實體識別碼和 SAML 單一登入服務 URL]。

    ![G Suite 設定](./media/active-directory-saas-googleapps-tutorial/tutorial_googleapps_configure.png) 

8. 在瀏覽器中開啟新索引標籤，然後使用系統管理員帳戶登入 [G Suite 管理控制台](http://admin.google.com/)。

9. 按一下 [安全性] 。 如果您沒有看到連結，它可能隱藏在畫面底部的 [其他控制項]  功能表之下。
   
    ![按一下 [安全性]。][10]

10. 在 [安全性] 頁面上，按一下 [設定單一登入 (SSO)]。
...
2. 在應用程式清單中，選取 [G Suite]。
    ...
    ![應用程式清單中的 G Suite 連結](./media/active-directory-saas-googleapps-tutorial/tutorial_googleapps_app.png)  
