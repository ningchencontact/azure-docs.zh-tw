...
> 
> 如果您的系統管理員已開啟安全性資訊體驗，您將需要使用下列逐步文章來設定驗證方法：<ul><li>[若要使用的驗證應用程式設定的安全性資訊](security-info-setup-auth-app.md)</li><li>[若要使用文字訊息設定安全性資訊](security-info-setup-text-msg.md)</li><li>[若要使用的電話設定安全性資訊](security-info-setup-phone-number.md)</li></ul>
...
1. 一般來說，登入您的帳戶，然後選擇**登入另一種方式**連結**雙步驟驗證**頁面。

    ![變更登入驗證方法](media/security-info/two-factor-auth-signin-another-way.png)

    >[!Note]
    >如果您沒有看到**登入另一種方式**連結，這表示您尚未設定任何其他驗證方法，並且您必須連絡您的系統管理員以取得協助，登入您的帳戶。 您的系統管理員可協助您登入之後，請確定您新增額外的驗證方法。 如需新增驗證方法的詳細資訊，請參閱[管理雙步驟驗證設定](multi-factor-authentication-end-user-manage-settings.md)一文。
    > 
    >如果您看到**登入另一種方式**連結，但仍未看到任何其他驗證方法，您必須連絡您的系統管理員以取得協助，登入您的帳戶。
