...
- **搭配您的公司或學校帳戶和 MyApps 入口網站使用雙步驟驗證。** 建立和刪除您的應用程式密碼使用的指示，在 建立和使用這篇文章的 MyApps 入口網站區段的應用程式密碼。 如需 MyApps 入口網站及其使用方式的詳細資訊，請參閱[什麼是 Azure Active Directory 中的 MyApps 入口網站？](active-directory-saas-access-panel-introduction.md)。
