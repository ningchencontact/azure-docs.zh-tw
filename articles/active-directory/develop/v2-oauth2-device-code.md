...
Microsoft 身分識別平臺支援[裝置程式碼授](https://tools.ietf.org/html/draft-ietf-oauth-device-flow-12)與, 可讓使用者登入受輸入限制的裝置, 例如智慧型電視、IoT 裝置或印表機。  若要啟用此流程，裝置會讓使用者在其他裝置的瀏覽器中瀏覽網頁，以執行登入程序。  使用者登入後，裝置可取得所需的存取權杖和重新整理權杖。  

> [!IMPORTANT]
> 目前, Microsoft 身分識別平臺端點僅支援 Azure AD 租使用者的裝置流程, 而非個人帳戶。  這表示您必須使用設定為租使用者或端點的`organizations`端點。  這種支援將于近期內啟用。 
>
...
收到`user_code` 和`verification_uri`之後, 用戶端會向使用者顯示這些專案, 指示他們使用其行動電話或電腦瀏覽器登入。  此外，用戶端可使用 QR 代碼或類似機制顯示 `verfication_uri_complete`，由這類機制代替使用者輸入 `user_code`。
...
| `bad_verification_code`| 無法辨識傳送`device_code`至端點的。`/token` | 確認用戶端是否在要求中傳送正確的 `device_code`。 |
