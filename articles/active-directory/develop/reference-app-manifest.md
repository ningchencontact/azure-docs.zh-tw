...
|availableToOtherTenants|布林值|`true`|如果此值設為 true，則此應用程式可供其他租用戶使用。 如果設為 false，則只有註冊應用程式的租用戶才能使用。 如需詳細資訊，請參閱：[如何使用多租用戶應用程式模式以任何 Azure Active Directory (AD) 使用者來登入](howto-convert-app-to-be-multi-tenant.md)。 |
