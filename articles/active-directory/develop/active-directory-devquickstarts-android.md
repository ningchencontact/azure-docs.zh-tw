---
title: Azure AD Android 使用者入門 | Microsoft Docs
...
Azure AD Authentication Library (ADAL) Android 程式庫讓您的應用程式能夠使用業界標準 OAuth2 和 OpenID Connect 來支援 [Microsoft Azure Active Directory 帳戶](https://azure.microsoft.com/services/active-directory/)，藉以開始使用 [Microsoft Azure 雲端](https://cloud.microsoft.com) & [Microsoft Graph API](https://graph.microsoft.io)。 本文範例示範您應用程式應體驗的所有正常生命週期，包括：
...
![拓撲](./media/active-directory-android-topology.png)

此應用程式可以用於所有的 Azure AD 帳戶。 其可支援單一和多重組織案例 (會在步驟中討論)， 也展現出開發人員能夠以何種方式建置應用程式以便與企業使用者連線，並透過 Microsoft Graph 存取其 Azure + O365 資料。 驗證流程期間，使用者必須登入並同意應用程式的權限，在某些情況下可能需要系統管理員同意應用程式。 此範例中的大部分邏輯會示範如何驗證使用者，並對 Microsoft Graph 進行基本呼叫。
...
## <a name="steps-to-run"></a>執行步驟

### <a name="register-and-configure-your-app"></a>註冊並設定應用程式 
...
### <a name="configure-your-code"></a>設定程式碼
...
1. 如需程式庫機制及新情節與功能設定方式的詳細資訊，請參閱 [ADAL Android Wiki](https://github.com/AzureAD/azure-activedirectory-library-for-android/wiki) (英文)。 
...
ADAL 程式庫會公開驗證遙測資料，以協助應用程式開發人員了解其應用程式的行為、建立更好的體驗。 如此可擷取登入成功的案例、活動中的使用者，以及其他多種有趣的見解分析資訊。 若要使用驗證遙測資料，應用程式開發人員需先建立遙測服務，用以彙總和儲存事件。
