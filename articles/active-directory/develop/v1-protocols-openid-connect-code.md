...
| 參數 |  | 描述 |
| --- | --- | --- |
| post_logout_redirect_uri |建議使用 |使用者在成功登出後應重新導向至的 URL。此 URL 必須與您在應用程式註冊入口網站中為應用程式註冊的其中一個重新導向 URI 相符。  如果未包含*post_logout_redirect_uri* ，則會顯示一般訊息給使用者。 |
