...
```
元数据是简单的 JavaScript 对象表示法 (JSON) 文档。 如需範例，請參閱下列程式碼片段。 [OpenID Connect 規格](https://openid.net)中有程式碼片段內容的完整說明。 請注意，提供租用戶而非 `common` 來取代上面的 {tenant}，會導致傳回的 JSON 物件中有租用戶特有的 URI。
