...
```
中繼資料是簡單的「JavaScript 物件標記法」(JSON) 文件。 如需範例，請參閱下列程式碼片段。 [OpenID Connect 規格](https://openid.net)中有程式碼片段內容的完整說明。 請注意，提供租使用者識別碼，而非 `common` 取代上述的 {tenant}，會導致傳回的 JSON 物件中有租使用者專屬的 Uri。
...
在使用者經過驗證之後，傳送至登入要求中指定之 `redirect_uri` 的回應範例如下所示：
...
驗證 `id_token` 之後，即可利用該使用者開始工作階段，並使用 `id_token` 中的宣告來取得應用程式中的使用者相關資訊。 這種資訊可以用於顯示、記錄、個人化等。如需 `id_tokens` 和宣告的詳細資訊，請參閱[AAD id_tokens](id-tokens.md)。

## <a name="send-a-sign-out-request"></a>傳送登出要求

當您想要將使用者登出應用程式時，只是清除應用程式的 Cookie 或結束使用者的工作階段還是不夠。 您也必須將使用者重新導向至 `end_session_endpoint` 進行登出。如果您無法這麼做，使用者將能夠重新驗證您的應用程式，而不需要再次輸入其認證，因為他們將會與 Azure AD 端點進行有效的單一登入會話。
...
| 參數 |  | 描述 |
| --- | --- | --- |
| post_logout_redirect_uri |建議使用 |使用者在成功登出後應重新導向至的 URL。 此 URL 必須符合在應用程式註冊入口網站中為您的應用程式註冊的其中一個重新導向 Uri。  如果未包含*post_logout_redirect_uri* ，使用者就會顯示為一般訊息。 |
...
成功的回應會使用 `response_mode=form_post`傳送至 `redirect_uri`，如下所示：
