...
Broker support is enabled on a per-PublicClientApplication basis. 此功能預設為停用。 Use the `WithBroker()` parameter (set to true by default) when you create the PublicClientApplication through the PublicClientApplicationBuilder.
