...
適用于開發人員的 Azure Active Directory （v1.0）（Azure AD）藉由提供身分識別即服務，簡化應用程式開發人員的驗證，並支援業界標準的通訊協定（例如 OAuth 2.0 和 OpenID Connect），以及開放原始碼程式庫針對不同的平臺，協助您快速開始撰寫程式碼。
...
* 開發人員可以使用開放原始碼 Azure AD 驗證程式庫，為您處理通訊協定的細節，以輕鬆完成驗證。 如需詳細資訊，請參閱 Microsoft 身分識別平台的 [v2.0 驗證程式庫](reference-v2-libraries.md)和 [v1.0 驗證程式庫](active-directory-authentication-libraries.md)。
...
下表提供由 Azure AD 發出的每一種宣告的簡短描述。 如需詳細資訊，請參閱 Azure AD 所簽發的[存取權杖](access-tokens.md)和[識別碼權杖](id-tokens.md)。
