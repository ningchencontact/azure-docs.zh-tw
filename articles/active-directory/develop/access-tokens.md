...
| `preferred_username` | 字串 | 代表使用者的主要使用者名稱。 它可以是電子郵件地址、電話號碼或未指定格式的一般使用者名稱。 其值是可變動的，並且可能隨著時間改變。 因為它是可變動的這個值必須不使用來製作授權決策。  但可以將它用於使用者名稱提示。 `profile`範圍才能接收此宣告。 |
| `name` | 字串 | 提供人類看得懂的值，用以識別權杖的主體。 值不保證是唯一的所以會變動，以及它設計成只用於顯示用途。 `profile`範圍才能接收此宣告。 |
...
| `roles` | 字串陣列，權限清單 | 由您應用程式公開的權限集合，而要求應用程式已獲得用於呼叫的權限。 針對[應用程式權杖](#user-and-application-tokens)，這會使用於[用戶端認證](v1-oauth2-client-creds-grant-flow.md)取代使用者範圍的流量。  針對[的使用者語彙基元](#user-and-application-tokens)下拉式功能表中的使用者已指派給目標的應用程式的角色。 |
...
| `unique_name` | 字串 | 只存在於 v1.0 權杖中。 提供人類看得懂的值，用以識別權杖的主體。 此值不一定是唯一的租用戶內，而且只用於顯示用途。 |
...
| `in_corp`| 布林值 | 指出用戶端是否是從公司網路登入的。 若非如此，宣告不包含在內。 |
