...
As defined by the [OAuth2 Authorization Framework][OAuth2-Role-Def], an entity capable of granting access to a protected resource. 個人資源擁有者就是指使用者。 例如，當[用戶端應用程式](#client-application)想要透過[Microsoft Graph API][Microsoft-Graph]存取使用者的信箱時，它需要信箱的資源擁有者所擁有的許可權。
