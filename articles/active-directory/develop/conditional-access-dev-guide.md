...
在最常見的情況下, 條件式存取不會變更應用程式的行為, 或需要開發人員所做的任何變更。 只有在某些情況下, 當應用程式間接或以無訊息方式要求服務的權杖時, 應用程式才需要變更程式碼來處理條件式存取「挑戰」。 這有如執行互動式登入要求一樣簡單。
...
條件式存取原則可以套用至應用程式, 但也可以套用至應用程式存取 Web API。 若要深入瞭解如何設定條件式存取原則, 請參閱[快速入門:具有 Azure Active Directory 條件式存取](../conditional-access/app-based-mfa.md)的特定應用程式需要 MFA。
...
當您的應用程式嘗試存取具有條件式存取原則的服務時, 可能會遇到條件式存取挑戰。 這項挑戰會在來自`claims` Azure AD 回應的參數中編碼。 以下是這項挑戰參數的範例： 
...
> [!NOTE]
> 在此例中, 這是多重要素驗證錯誤的描述, 但有各種`interaction_required`可能與條件式存取相關的範圍。
...
如果應用程式是使用 ADAL 程式庫，就一律會以互動方式重試取得權杖失敗。 當此互動式要求發生時, 使用者就有機會遵守條件式存取。 除非要求是`AcquireTokenSilentAsync`或`PromptBehavior.Never` , 在這種情況下, 應用程式需要執行互動式```AcquireToken```要求, 讓終端使用者有機會遵守原則。
...
* `login()`透過互動式登入要求取得識別碼權杖, 但不取得任何服務的存取權杖 (包括受條件式存取保護的 Web API)。
...
當應用程式需要存取權杖來呼叫 Web API 時，它會嘗試 `acquireToken(…)`。 如果權杖會話已過期, 或我們需要遵守條件式存取原則, 則*acquireToken*函式會失敗, 且應用程式會`acquireTokenPopup()`使用`acquireTokenRedirect()`或。

![使用 ADAL 的單頁應用程式流程圖](./media/conditional-access-dev-guide/spa-using-adal-scenario.png)

讓我們逐步解說使用條件式存取案例的範例。 使用者只需登陸網站，且沒有工作階段。 我們執行 `login()` 呼叫時，無需進行多重要素驗證即可取得 ID 權杖。 然後使用者會叫用按鈕，要求應用程式向 web API 要求資料。 應用程式嘗試進行`acquireToken()`呼叫但失敗, 因為使用者尚未執行多重要素驗證, 而且必須符合條件式存取原則。
