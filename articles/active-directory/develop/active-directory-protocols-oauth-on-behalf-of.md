...
> [!IMPORTANT]
> 使用 [OAuth 2.0 隱含授與](active-directory-dev-understanding-oauth2-implicit-grant.md)的公用用戶端不能使用 OBO 流程。 這些用戶端必須將其存取權杖傳遞至中介層機密用戶端，才能執行 OBO 流程。 若要進一步了解哪些用戶端可執行 OBO 呼叫，請參閱[用戶端限制](#client-limitations)。
