...
> [!IMPORTANT]
> 在預覽期間收到客戶的意見之後, 我們已在 Azure AD 條件式存取中實作為[驗證會話管理功能](https://go.microsoft.com/fwlink/?linkid=2083106)。 您可以使用這項新功能, 設定 [登入頻率] 來設定重新整理權杖存留期。 2019年11月1日之後, 您將無法使用可設定的權杖存留期原則來設定重新整理權杖, 但您仍然可以使用它來設定存取權杖。
