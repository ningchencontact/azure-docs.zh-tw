...
> [!NOTE]
> *msal.js* 的目標是「Microsoft 身分識別平台」  端點，其可讓個人、學校及公司帳戶登入和取得權杖。 「Microsoft 身分識別平台」  有[一些限制](azure-ad-endpoint-comparison.md#limitations)。
...
在本指南建立的範例應用程式中，`callMSGraph()` 方法是用來針對受保護資源提出 HTTP `GET` 要求，那些受保護資源需要權杖才能存取，然後再將內容傳回給使用者。 此方法會在「HTTP 授權標頭」  中新增取得的權杖。 對於本指南建立的範例應用程式，資源為 Microsoft 圖形 API *me* 端點，它會顯示使用者的設定檔資訊。
