...
* **相容**：Microsoft 已在基本案例中測試這些程式庫並確認其使用 v2.0 端點。 Microsoft 不提供这些库的修复程序，且尚未审查这些库。 問題和功能要求應重新導向至程式庫的開放原始碼專案。
