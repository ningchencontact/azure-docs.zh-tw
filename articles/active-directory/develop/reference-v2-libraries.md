...
description: 相容的用戶端程式庫和伺服器中介軟體程式庫，以及相關的程式庫、 來源和範例的連結，Microsoft 身分識別平台的端點。
...
我們建議您使用的通訊協定網域專家遵循安全性開發生命週期 (SDL) 方法所撰寫的程式庫。 這類程式庫包含[Microsoft 如下所示][Microsoft-SDL]。 如果您交付裝置通訊協定的程式碼，您應該遵循的方法，例如 Microsoft SDL。 請密切注意的安全性考量每個通訊協定的標準規格中。

> [!NOTE]
> 需要針對 Azure Active Directory 驗證程式庫 (ADAL) 嗎？ 請參閱[ADAL 程式庫指南](active-directory-authentication-libraries.md)。

## <a name="types-of-libraries"></a>程式庫類型

Microsoft 身分識別平台端點適用於兩種類型的程式庫︰

* **用戶端程式庫**：原生用戶端和伺服器使用用戶端程式庫來取得存取權杖來呼叫資源，例如 Microsoft Graph。
...
* **相容**：Microsoft 已在基本案例中測試這些程式庫，並確認其使用 Microsoft 身分識別平台的端點。 Microsoft 不提供這些程式庫的修正程式，也尚未完成審查這些程式庫。 問題和功能要求應重新導向至程式庫的開放原始碼專案。

如需與 Microsoft 身分識別平台端點搭配運作的程式庫的清單，請參閱下列各節。

## <a name="microsoft-supported-client-libraries"></a>Microsoft 支援的用戶端程式庫

您可以使用 用戶端驗證程式庫來取得權杖，以呼叫受保護的 web API。
...
![Angular](media/sample-v2-code/logo_angular.png) | Angular MSAL （預覽） | [NPM](https://www.npmjs.com/package/@azure/msal-angular) |[GitHub](https://github.com/AzureAD/microsoft-authentication-library-for-js/blob/dev/lib/msal-angular/README.md) | | | |
...
您可以使用 中介軟體程式庫來協助保護 web 應用程式和 web Api。 Web 應用程式或 web Api 撰寫 ASP.NET 或 ASP.NET Core 使用的中介軟體程式庫。
...
任何符合標準的程式庫，您可以使用 Microsoft 身分識別平台的端點。 請務必知道哪裡可以尋求支援：

* 如需程式庫程式碼中的問題和新功能要求，請連絡程式庫擁有者。
* 如需服務端通訊協定實作中的問題和新功能要求，請連絡 Microsoft。
* [提出功能要求](https://feedback.azure.com/forums/169401-azure-active-directory)您想要查看通訊協定中的其他功能。
* [建立支援要求](https://docs.microsoft.com/azure/azure-supportability/how-to-create-azure-support-request)如果您發現問題，Microsoft 身分識別平台端點不是符合 OAuth 2.0 或 OpenID Connect 1.0。
