...
1. 選取 [驗證] 功能表，然後新增下列資訊：
   1. 在 [**回復 URL**] `https://localhost:44321/signin-oidc`中，新增「Web」類型。
...
4. 當 [註冊應用程式] 頁面出現時，輸入您應用程式的註冊資訊：
   ...
   - 在 [重新導向 URI （選擇性）] 區段中，選取下拉式方塊中的 [ **Web** ]，然後輸入`https://localhost:44326/`下列重新導向 uri：。
1. 選取 [註冊] 以建立應用程式。
1. 選取 [驗證] 功能表，然後新增下列資訊：
...
1. 按一下 [**註冊**] 以註冊應用程式。
1. 在左側功能表上，按一下 [**驗證**]，然後在 [重新*導向 uri*] 底下，選取 [Web]。 您必須輸入兩個不同的重新導向 Uri：一個用於登入頁面，另一個用於 [圖形使用者] 頁面。 對於這兩種情況，您應該使用相同的主機和埠號碼，然後在 [使用者] 頁面的 [登入] 頁面和 [msal4jsample/圖形/使用者] 中，輸入 "/msal4jsample/secure/aad"。
 根據預設，此範例會使用： 
...
4. 當 [註冊應用程式] 頁面出現時，輸入您應用程式的註冊資訊：
   ...
   - 在 [重新導向 URI （選擇性）] 區段中，選取下拉式方塊中的 [ **Web** ]，然後輸入`http://localhost:5000/getAToken`下列重新導向 uri：。
1. 選取 [註冊] 以建立應用程式。
