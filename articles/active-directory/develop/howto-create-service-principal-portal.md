...
如果應用程式註冊設定為 [否]，只有[全域系統管理員](../users-groups-roles/directory-assign-admin-roles.md)才能註冊應用程式。 如果您的帳戶指派給「使用者」角色，但應用程式註冊設定僅限於系統管理員使用者，請洽詢系統管理員將您指派為全域系統管理員角色，或讓使用者可以註冊應用程式。
