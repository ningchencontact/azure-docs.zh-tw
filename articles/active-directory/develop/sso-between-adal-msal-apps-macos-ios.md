...
ms.locfileid: "71268917"
---
# <a name="how-to-sso-between-adal-and-msal-apps-on-macos-and-ios"></a>HOW TO：MacOS 和 iOS 上的 ADAL 與 MSAL 應用程式之間的 SSO
...
當您在 MSAL `MSALAccount`結果中收到物件時，它會`identifier`在屬性中包含帳戶識別碼。 應用程式應該針對後續的無訊息要求使用此識別碼。

除了之外`identifier`， `MSALAccount`物件還包含名`username`為的可顯示識別碼。 這會在`userId` ADAL 中轉譯為。 `username`不會視為唯一的識別碼，而且可以隨時變更，因此應該只用于與 ADAL 的回溯相容性案例。 MSAL 支援使用`username`或`identifier`的快取查詢，這是`identifier`建議查詢的位置。
...
如果您有 MSAL 應用程式和 ADAL 應用程式，且使用者第一次登入以`username` `userId`MSAL 為基礎的應用程式，您可以`MSALAccount`從物件儲存，並將它傳遞至以 adal 為基礎的應用程式，以取得 adal 應用程式中的 SSO。 然後 ADAL 可以使用`acquireTokenSilentWithResource:clientId:redirectUri:userId:completionBlock:` API 以無訊息模式尋找帳戶資訊。
...
ADAL 2.7. x `homeAccountId` `ADUserInformation`會透過這個屬性，在結果中的物件中傳回：
...
`homeAccountId`在 ADAL 的中，相當`identifier`于 MSAL 中的。 您可以儲存此識別碼，以用於使用`accountForIdentifier:error:` API 進行帳戶查閱的 MSAL。

#### <a name="adals-userid"></a>ADAL`userId`

如果`homeAccountId`無法使用，或您只有可顯示的識別碼，您可以使用 ADAL 的`userId`來查閱 MSAL 中的帳戶。

在 MSAL 中，首先依據`username`或`identifier`查閱帳戶。 如果您`identifier`有，請一律使用來查詢，而且`username`只會使用做為回復。 如果找到帳戶，請使用 acquireTokenSilent 呼叫中的帳戶。
...
由於`homeAccountId`舊版 ADAL 無法使用，因此您必須`username`使用來查詢帳戶：
