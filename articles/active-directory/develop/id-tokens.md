...
description: 了解如何使用 Azure AD 端點 v1.0 與 v2.0 所發出的 id_tokens。
...
|`at_hash`| 字串 |只有當識別碼權杖是隨著 OAuth 2.0 存取權杖一起簽發時，存取權杖雜湊才會包含在識別碼權杖中。 它可用于验证访问令牌的真实性。 如需有關執行此驗證的詳細資料，請參閱 [OpenID Connect 規格](https://openid.net/specs/openid-connect-core-1_0.html)。 |
...
驗證 `id_token` 與[驗證存取權杖](access-tokens.md#validating-tokens)的第一個步驟非常類似 - 您的用戶端應確認正確的簽發者是否已傳回權杖，而且該權杖並未遭到竄改。 因為 `id_tokens` 一律是 JWT，所以有許多程式庫可用來驗證這些權杖 - 我們建議您使用其中一個程式庫，而不需自己執行此作業。
