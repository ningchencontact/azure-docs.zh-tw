...
您通常只需要一個參數（`scopes`）。 不過，視您的 Windows 系統管理員設定原則的方式而定，您的 windows 電腦上的應用程式可能不允許查詢已登入的使用者。 在此情況下，請使用第二個方法 `.WithUsername()`，並以 UPN 格式（`joe@contoso.com`）傳入登入使用者的使用者名稱。
