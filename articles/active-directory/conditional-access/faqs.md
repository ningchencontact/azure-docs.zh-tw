...
## <a name="which-applications-work-with-conditional-access-policies"></a>哪些應用程式搭配使用條件式存取原則？

使用條件式存取原則的應用程式的相關資訊，請參閱[應用程式與 Azure Active Directory 中使用條件式存取規則之瀏覽器](technical-reference.md)。

## <a name="are-conditional-access-policies-enforced-for-b2b-collaboration-and-guest-users"></a>B2B 共同作業與來賓使用者強制執行條件式存取原則？
...
條件式存取原則設定來存取服務的需求。 在向該服務進行驗證時會強制執行。 原則並不直接在用戶端應用程式上設定， 而是在用戶端呼叫服務時套用。 例如，在 SharePoint 上設定的原則會套用至呼叫 SharePoint 的用戶端。 在 Exchange 上設定的原則會套用至 Outlook。

## <a name="does-a-conditional-access-policy-apply-to-service-accounts"></a>要將條件式存取原則套用到服務帳戶？

條件式存取原則會套用到所有的使用者帳戶。 這包括作為服務帳戶的使用者帳戶。 通常，自動執行的服務帳戶無法滿足條件式存取原則的需求。 例如，可能需要多重要素驗證。 服務帳戶可以排除原則，使用條件式存取原則管理設定。 

## <a name="are-graph-apis-available-for-configuring-conditional-access-policies"></a>是 Graph Api 可用來設定條件式存取原則？
