...
> [!IMPORTANT]
> - 本文中的所有範例程式碼/指令碼都假設用戶端在已啟用 Azure 資源受控識別的虛擬機器上執行。 在 Azure 入口網站中使用 VM「連線」功能，從遠端連線到您的 VM。 如需有關在虛擬機器上啟用 Azure 資源受控識別的詳細資訊，請參閱[使用 Azure 入口網站在虛擬機器上設定 Azure 資源受控識別](qs-configure-portal-windows-vm.md)，或其中一篇變化文章 (使用 PowerShell、CLI、範本或 Azure SDK)。 
...
| 拼音            | [從 Azure 資源的受控識別啟用的虛擬機器管理資源](https://github.com/Azure-Samples/resources-ruby-manage-resources-with-msi/) |
