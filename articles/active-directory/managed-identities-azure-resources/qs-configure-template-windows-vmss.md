...
> [!NOTE]
> 您可以選擇性地在範本的 `extensionProfile` 元素中指定，以布建 Azure 資源的受控識別虛擬機器擴展集延伸模組。 此步驟是選用的，因為您可以使用 Azure Instance Metadata Service (IMDS) 識別端點以擷取權杖。  如需詳細資訊，請參閱[從 VM 擴充功能遷移至 AZURE IMDS 以進行驗證](howto-migrate-vm-extension.md)。
