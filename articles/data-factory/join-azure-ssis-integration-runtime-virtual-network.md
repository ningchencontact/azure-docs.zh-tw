...
例如，如果您的 Azure SSIS IR 位於 `UK South`，您會從服務標籤的 ip[範圍下載連結](https://www.microsoft.com/en-us/download/details.aspx?id=56519)或透過服務標籤[探索 API](https://aka.ms/discoveryapi)，取得服務標籤的 IP 範圍清單 `BatchNodeManagement.UKSouth`。 然後將下列相關 IP 範圍路由的 Udr 套用至下一個躍點類型為**網際網路**。
