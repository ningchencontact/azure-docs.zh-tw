...
如果 SSIS 目錄已裝載於具有虛擬網路服務端點/受控執行個體 (預覽) 的 Azure SQL Database 中，您可以將 Azure-SSIS IR 加入至：

- 相同虛擬網路
- 具有網路對網路連線的虛擬網路，不同於處理具有虛擬網路服務端點/受控執行個體 (預覽) 的 Azure SQL Database 時所用的虛擬網路

虛擬網路可透過傳統部署模型或 Azure Resource Manager 部署模型來部署。 如果您打算將 Azure-SSIS IR 加入受控執行個體 (預覽) 已經加入的「相同虛擬網路」，請確定 Azure-SSIS IR 位於用於受控執行個體 (預覽) 的「不同子網路」中。   
...
Azure-SSIS IR 需要在與虛擬網路相同的資源群組之下建立特定的網路資源，包括 Azure 負載平衡器、Azure 公用 IP 位址，以及網路安全性群組。
...
8. 現在，您可以針對您的 Azure-SSIS IR 使用 [動作] 資料行中的 [啟動] 按鈕，以啟動 IR。 啟動 Azure-SSIS IR 需要約 20 至 30 分鐘。 
