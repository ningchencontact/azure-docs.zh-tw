...
- **建議**：更新 Azure 函數以傳回有效的 JSON 承載。 例如， C#函式可以傳回 `(ActionResult)new<OkObjectResult("{`\"識別碼\"：\"123\"`}");`。
...
- **Message**： AzureMLExecutePipeline 活動遺漏 JSON 中的 LinkedService 定義。
...
- **原因**：在 AzureML 服務連結服務中提供的認證無效，或沒有該作業的許可權。
...
- **建議**：請查看 AzureMLService 以取得更多錯誤記錄，並修正 ML 管線
...
- **原因**：同時將太多作業提交到 HDInsight。

- **建議**：請考慮限制提交至 HDInsight 的並行作業數目。 如果作業是由相同的活動所提交，請參閱 Data Factory 活動並行。 變更觸發程式，讓並行管線執行會隨著時間分散。 如錯誤所建議，請參閱 HDInsight 檔以調整 `templeton.parallellism.job.submit`。
...
- **建議**：這是來自 HDInsight 的例外狀況。 它會隱藏實際的問題。 請洽詢 HDInsight 小組以取得支援。 提供叢集名稱和活動運行時間範圍。
...
- **建議**：權杖可能已過期。 請提供有效的驗證方法。 使用 Fiddler 或 Postman 來檢查 URL 上的功能。
...
- **原因**：此錯誤可能有多個原因，例如網路連線、DNS 失敗、伺服器憑證驗證或超時。
