...
-   目前，您無法直接使用會呼叫 xcopy 或 robocopy (或類似工具) 的指令碼來複製檔案。 因應措施是建立 `cmd` 檔案，且其中須包含會呼叫 xcopy 或 robocopy 工具的指令碼 (例如 `install.cmd`)，並改為呼叫此 `cmd` 檔案。 例如︰

    ```
    start /wait cmd /c "call install.cmd > %CUSTOM\_SETUP\_SCRIPT\_LOG\_DIR%\\install.cmd.log"
    ```

-   目前，您無法直接呼叫 `gacutil.exe` 以安裝全域組件快取 (GAC) 中的組件。 因應措施是使用 `gacinstall.cmd` (提供於公開預覽容器中)。
...
2.  若要查看其他自訂安裝範例，請透過 Azure 儲存體總管連線至公用預覽容器。
    ...
    c. 選取已連線的公用預覽容器，然後按兩下 `CustomSetupScript` 資料夾。 此資料夾中包含下列項目：
       ...
       1. 一個 `Sample` 資料夾，其中包含自訂安裝程式，用以在 Azure-SSIS IR 的每個節點上安裝基本工作。 此工作不會執行任何動作，而會休眠數秒鐘。 此資料夾還包含 `gacinstall.cmd`，用以取代 `gacutil.exe`。

       2. 一個 `UserScenarios` 資料夾，其中包含實際使用者案例的八個自訂安裝。
