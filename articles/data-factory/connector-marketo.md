...
>[!NOTE]
>此 Marketo 連接器會建置在 Marketo REST API 上方。 請注意，Marketo 在服務端上有[並行要求限制](http://developers.marketo.com/rest-api/)。 如果您收到錯誤，當中指出「嘗試使用 REST API 時發生錯誤：在 '20' 秒內超出速率上限 '100' (606)」，或「嘗試使用 REST API 時發生錯誤：已達到並行存取限制 '10' (615)」，請考慮降低並行複製活動回合數，以縮減對服務發出的要求數。
...
| 屬性 | 說明 | 必要 |
|:--- |:--- |:--- |
| type | 類型屬性必須設定為：**Marketo** | yes |
| endpoint | Marketo 伺服器的端點。 (亦即 123-ABC-321.mktorest.com)  | yes |
| clientId | Marketo 服務的用戶端識別碼。  | yes |
| clientSecret | Marketo 服務的用戶端密碼。 將此欄位標記為 SecureString，將它安全地儲存在 Data Factory 中，或[參考 Azure Key Vault 中儲存的祕密](store-credentials-in-key-vault.md)。 | yes |
...
| 屬性 | 說明 | 必要 |
|:--- |:--- |:--- |
| type | 資料集的類型屬性必須設定為：**MarketoObject** | yes |
...
| 屬性 | 說明 | 必要 |
|:--- |:--- |:--- |
| type | 複製活動來源的類型屬性必須設定為：**MarketoSource** | yes |
