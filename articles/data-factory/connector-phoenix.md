...
>[!NOTE]
>如果叢集不支援 HDInsight 等黏性工作階段，請明確地在 http 路徑設定結尾新增節點索引 (例如，指定 `/hbasephoenix0` 而不是 `/hbasephoenix`)。
...
| 屬性 | 說明 | 必要 |
|:--- |:--- |:--- |
| 類型 | 資料集的類型屬性必須設定為： **PhoenixObject** | 是 |
...
| 屬性 | 說明 | 必要 |
|:--- |:--- |:--- |
| 類型 | 複製活動來源的 type 屬性必須設定為：**PhoenixSource** | 是 |
