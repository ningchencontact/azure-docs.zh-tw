...
2. **將服務識別存取權授與您的 Azure Key Vault。** 在 Key Vault -> 存取原則 -> 新增 -> 搜尋此服務識別應用程式識別碼，以授與 [祕密] 權限下拉式清單中的 [取得] 權限。 這樣可以讓這個指定的處理站存取 Key Vault 中的密碼。
3. **建立指向您的 Azure Key Vault 的已連結服務。** 請參閱 [Azure Key Vault 已連結服務](#azure-key-vault-linked-service)。
4. **建立資料存放區已連結服務，其中參考儲存在 Key Vault 的對應密碼。** 請參閱[參考儲存在 Key Vault 中的祕密](#reference-secret-stored-in-key-vault)。
...
## <a name="reference-secret-stored-in-key-vault"></a>參考儲存在 Key Vault 中的祕密

當您在參考 Key Vault 密碼的已連結服務中設定欄位時，支援下列屬性：
