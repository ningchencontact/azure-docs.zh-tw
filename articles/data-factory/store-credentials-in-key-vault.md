...
1. **擷取 data factory 受控身分識別**藉由複製的 「 受管理身分識別應用程式識別碼 」 站一起產生的值。 如果您使用 ADF 撰寫 UI，受管理的身分識別應用程式識別碼將會顯示在 Azure 金鑰保存庫的連結的服務建立視窗;您也可以擷取它從 Azure 入口網站，請參閱[擷取 data factory 受控身分識別](data-factory-service-identity.md#retrieve-managed-identity)。
