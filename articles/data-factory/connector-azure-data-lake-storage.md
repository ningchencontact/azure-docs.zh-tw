...
>[!NOTE]
>如果您使用 Data Factory UI 來撰寫，而且未在 IAM 中使用「儲存體 Blob 資料讀取器/參與者」角色設定服務主體，則在執行測試連接或流覽/流覽資料夾時，請選擇 [測試與檔案路徑的連接] 或 [從指定的路徑流覽]，並指定具有 [執行] 許可權的檔案系統或路徑以繼續。
...
>[!NOTE]
>如果您使用 Data Factory UI 來撰寫，且未在 IAM 中使用「儲存體 Blob 資料讀取器/參與者」角色設定受控識別，則在執行測試連接或流覽/流覽資料夾時，請選擇 [測試與檔案路徑的連接] 或 [從指定的路徑流覽]，並指定具有 [執行] 許可權的檔案系統或路徑以繼續。
...
| 屬性   | 描述                                                  | 必要項 |
| ---------- | ------------------------------------------------------------ | -------- |
| 類型       | Dataset 中 `location` 下的 type 屬性必須設定為**AzureBlobFSLocation**。 | 是      |
...
| 屬性 | 描述 | 必要項 |
|:--- |:--- |:--- |
| 類型 | 資料集的類型屬性必須設為 **AzureBlobFSFile**。 |是 |
...
| 屬性                 | 描述                                                  | 必要項                                      |
| ------------------------ | ------------------------------------------------------------ | --------------------------------------------- |
| 類型                     | `storeSettings` 下的 type 屬性必須設定為**AzureBlobFSReadSetting**。 | 是                                           |
...
| 屬性 | 描述 | 必要項 |
|:--- |:--- |:--- |
| 類型 | 複製活動來源的類型屬性必須設為 **AzureBlobFSSource**。 |是 |
...
| 屬性                 | 描述                                                  | 必要項 |
| ------------------------ | ------------------------------------------------------------ | -------- |
| 類型                     | `storeSettings` 下的 type 屬性必須設定為**AzureBlobFSWriteSetting**。 | 是      |
...
| 屬性 | 描述 | 必要項 |
|:--- |:--- |:--- |
| 類型 | 複製活動接收的類型屬性必須設為 **AzureBlobFSSink**。 |是 |
| copyBehavior | 當來源是來自檔案型資料存放區的檔案時，會定義複製行為。<br/><br/>允許的值包括：<br/><b>- PreserveHierarchy (預設值)</b>：保留目標資料夾中的檔案階層。 來源檔案到來源資料夾的相對路徑，與目標檔案到目標資料夾的相對路徑相同。<br/><b>- FlattenHierarchy</b>：來自來源資料夾的所有檔案都在目標資料夾的第一層中。 目標檔案會有自動產生的名稱。 <br/><b>- MergeFiles</b>：將來源資料夾的所有檔案合併成一個檔案。 若已指定檔案名稱，合併檔案的名稱會是指定的名稱。 否則，就會是自動產生的檔案名稱。 | 否 |
| maxConcurrentConnections | 同時連接到資料存放區的連接數目。 只有當您想要限制與資料存放區的並行連接時，才指定。 | 否 |
...
當您使用二進位複製選項或二進位格式來將來源設定為 Data Lake Storage Gen1，並使用二進位複製選項或二進位格式做為 Data Lake Storage Gen2 時，您可以在 [**資料複製工具設定**] 頁面或 [**複製活動** > **設定**] 索引標籤上，找到活動撰寫的 [**保留**] 選項。
