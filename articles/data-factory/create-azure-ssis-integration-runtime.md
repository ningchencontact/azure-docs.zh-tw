...
執行下列命令來建立 Azure SSIS 整合執行階段在 Azure 中執行 SSIS 封裝： 使用的資料庫 （Azure SQL Database 與類型為基礎的一節的指令碼您使用 azure SQL Managed 執行個體 （私人預覽中））。 

### <a name="azure-sql-database-to-host-the-ssisdb-database-ssis-catalog"></a>Azure SQL Database 主控 SSISDB 資料庫 （SSIS 目錄） 
...
### <a name="azure-sql-managed-instance-private-preview-to-host-the-ssisdb-database"></a>Azure SQL Managed 執行個體 （私人預覽中） 來主控 SSISDB 資料庫
...
以下是完整的指令碼，以建立 Azure SSIS IR，並將它加入至 VNet。 此指令碼假設您使用 Azure SQL Managed 執行個體 (MI) 來裝載 SSIS 目錄。 
