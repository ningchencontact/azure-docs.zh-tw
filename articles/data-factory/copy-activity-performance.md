...
2. **如何將單一複製活動的效能最大化**：
   ...
   **如果要在 Azure Integration Runtime 上執行複製活動：**
   ...
   - 您所執行的特定複製模式不會因新增更多 Diu 而受益。  即使您已指定較大的 DIU 值，實際使用的 DIU 還是會維持不變，因此您會取得與之前相同的輸送量。  如果是這種情況，您可以同時執行多個複本，同時參考步驟3，以最大化匯總輸送量。
...
您將收取 **# 個已使用的 diu，\* 複製持續時間 \* 單位價格/DIU 小時**。 請在[這裡](https://azure.microsoft.com/pricing/details/data-factory/data-pipeline/)查看目前的價格。 每個訂用帳戶類型可能會套用當地貨幣和個別折扣。
