...
如需可用來定義資料集的區段和屬性完整清單，請參閱[資料集](concepts-datasets-linked-services.md)一文。 本節提供 Oracle Respopnsys 資料集所支援的屬性清單。

若要從 Oracle Respopnsys 複製資料，請將資料集的類型屬性設定為 **ResponsysObject**。 以下是支援的屬性：

| 屬性 | 描述 | 必要項 |
|:--- |:--- |:--- |
| 型別 | 資料集的類型屬性必須設定為：**ResponsysObject** | 是 |
...
如需可用來定義活動的區段和屬性完整清單，請參閱[Pipelines](concepts-pipelines-activities.md)一文。 本節提供 Oracle Respopnsys 來源所支援的屬性清單。

### <a name="oracle-responsys-as-source"></a>以 Oracle Responsys 作為來源

若要從 Oracle Respopnsys 複製資料，請將複製活動中的來源類型設定為 **RespopnsysSource**。 複製活動的 **source** 區段支援下列屬性：

| 屬性 | 描述 | 必要項 |
|:--- |:--- |:--- |
| 型別 | 複製活動來源的類型屬性必須設定為：**ResponsysSource** | 是 |
