---
title: "使用 Azure Data Factory 的 Spark 中的資料複製 |Microsoft 文件"
description: "了解如何將資料從 Spark 複製到支援的接收資料存放區，在 Azure Data Factory 管線中使用複製活動。"
...
---
# <a name="copy-data-from-spark-using-azure-data-factory"></a>從 Spark 使用 Azure Data Factory 複製資料 

本文將概述如何使用 Azure Data Factory 中的複製活動，從 Spark 複製資料。 本文是根據[複製活動概觀](copy-activity-overview.md)一文，該文提供複製活動的一般概觀。
...
您可以從 Spark 資料複製到任何支援的接收資料存放區。 如需複製活動所支援作為來源/接收器的資料存放區清單，請參閱[支援的資料存放區](copy-activity-overview.md#supported-data-stores-and-formats)表格。
...
下列各節提供有關用來定義特定的 Data Factory 實體至 Spark 連接器屬性詳細資料。
...
| 連接埠 | Spark 伺服器用來接聽用戶端連線之 TCP 通訊埠。  | 是 |
| 伺服器類型 | Spark 伺服器類型。 <br/>允許的值為： **SharkServer**， **SharkServer2**， **SparkThriftServer** | 否 |
| thriftTransportProtocol | Thrift 層中使用的傳輸通訊協定。 <br/>允許的值為：**二進位**， **SASL**，* * HTTP * * | 否 |
| authenticationType | 用來存取 Spark 伺服器驗證方法。 <br/>允許的值為：**匿名**， **Username**， **UsernameAndPassword**， **WindowsAzureHDInsightService** | 是 |
| username | 您用來存取 Spark 伺服器使用者名稱。  | 否 |
...
| httpPath | 對應至 Spark 伺服器部分的 URL。  | 否 |
...
如需可用來定義資料集的區段和屬性完整清單，請參閱[資料集](concepts-datasets-linked-services.md)一文。 本節提供 Spark 資料集所支援的屬性的清單。

若要從 Spark 中複製資料，請將設定的資料集的類型屬性**SparkObject**。 沒有任何額外的特定類型的屬性，在這種類型的資料集。
...
如需可用來定義活動的區段和屬性完整清單，請參閱[管線](concepts-pipelines-activities.md)一文。 本節提供一份 Spark 來源所支援的屬性。

### <a name="sparksource-as-source"></a>做為來源 SparkSource

若要從 Spark 中複製資料，請將來源類型複製活動中**SparkSource**。 複製活動的 **source** 區段支援下列屬性：
