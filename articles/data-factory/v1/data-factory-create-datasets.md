...
| 結構 |資料集的結構描述。<br/><br/>如需詳細資料，請參閱[資料集結構](#Structure)。 |否 |NA |
...
| policy |定義資料集配量必須符合的準則或條件。 <br/><br/>如需詳細資料，請參閱[資料集原則](#Policy)一節。 |否 |NA |
...
| 間隔 |指定頻率的倍數。<br/><br/>「頻率 x 間隔」會決定產生配量的頻率。 例如，如果您需要將資料集以每小時為單位來切割，請將 <b>frequency</b> 設定為 <b>Hour</b>，將 <b>interval</b> 設定為 <b>1</b>。<br/><br/>請注意，如果您將 **frequency** 指定為 **Minute**，則應該將 interval 設定為不小於 15。 |是 |NA |
...
| anchorDateTime |定義排程器用來計算資料集配量界限的時間絕對位置。 <br/><br/>請注意, 如果此屬性具有比指定頻率更細微的日期部分, 則會忽略更細微的部分。 例如，如果 **interval** 為 **hourly** (frequency：hour 且 interval：1)，而且 **anchorDateTime** 包含**分鐘和秒鐘**，則會忽略 **anchorDateTime** 的分鐘和秒鐘部分。 |否 |01/01/0001 |
...
| minimumRows |驗證 **Azure SQL 資料庫**或 **Azure 資料表**中的資料是否包含最小的資料列數。 |<ul><li>Azure SQL 資料庫</li><li>Azure 資料表</li></ul> |否 |NA |
