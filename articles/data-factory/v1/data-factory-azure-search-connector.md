...
如果您要將資料從雲端資料存放區複製到「Azure 認知搜尋」，`executionLocation` 屬性是必要的。 下列 JSON 程式碼片段顯示必須在「複製活動」的 `typeProperties` 下進行的變更。 參閱[在雲端資料存放區之間複製資料](data-factory-data-movement-activities.md#global)一節以取得支援的值和更多詳細資料。
