---
title: 使用 Azure Data Factory 以累加方式複製新的和變更的檔案只會根據 LastModifiedDate |Microsoft Docs
description: 建立 Azure data factory，然後使用複製資料工具以累加方式將只根據 LastModifiedDate 的新檔案。
...
在這個教學課程中，您會使用 Azure 入口網站來建立資料處理站。 然後，您可以使用複製資料工具建立管線，以累加方式複製新的和變更的檔案，只能根據其 「 LastModifiedDate 」 從 Azure Blob 儲存體到 Azure Blob 儲存體。 
...
1. 建立名為容器**來源**。 您可以使用各種工具來執行這些工作，例如 [Azure 儲存體總管](https://storageexplorer.com/)。
...
4. 在 [選擇輸入檔案或資料夾] 頁面上，執行以下步驟︰
    ...
    c. 請檢查**二進位複製**然後按一下**下一步**。
    
     ![選擇輸入檔案或資料夾](./media/tutorial-incremental-copy-lastmodified-copy-data-tool/check-binary-copy.png)
     
5. 在 [**目的地資料存放區**頁面上，選取**Azureblobstorage-client-master.zip**也就是相同的儲存體帳戶做為資料來源存放區，然後**下一步]**。
...
10. 請注意，系統會自動選取左側的 [監視] 索引標籤。 [動作] 資料行中會有連結可供檢視活動執行詳細資料，以及重新執行管線。 選取 **重新整理**重新整理清單，然後選取**檢視活動執行**連結**動作**資料行。 
    ...
    ![監視管線回合](./media/tutorial-incremental-copy-lastmodified-copy-data-tool/monitor-pipeline-runs1.png)

11. 管線中只有一個活動 (複製活動)，所以您只會看到一個項目。 如需關於複製作業的詳細資料，請選取 [動作] 資料行中的 [詳細資料] 連結 (眼鏡圖示)。 

    ![監視管線回合](./media/tutorial-incremental-copy-lastmodified-copy-data-tool/monitor-pipeline-runs2.png)
    
    假設沒有在檔案**來源**容器中 blob 儲存體帳戶，讓您將不會看到已複製到任何檔案**目的地**blob 儲存體帳戶中的容器。
    
    ![監視管線回合](./media/tutorial-incremental-copy-lastmodified-copy-data-tool/monitor-pipeline-runs3.png)
    
12. 建立空的文字檔，並將它命名為 file1.txt。 若要將 file1.txt 檔案上傳**來源**在儲存體帳戶的容器。 您可以使用各種工具來執行這些工作，例如 [Azure 儲存體總管](https://storageexplorer.com/)。   

    ![監視管線回合](./media/tutorial-incremental-copy-lastmodified-copy-data-tool/monitor-pipeline-runs3-1.png)
    
13. 回到**管線回合**檢視中，選取**所有的管線執行**，並等候相同自動重新觸發的管線。  

    ![監視管線回合](./media/tutorial-incremental-copy-lastmodified-copy-data-tool/monitor-pipeline-runs4.png)

14. 選取 **檢視活動執行**第二個管線執行 if 您看到它出現，並執行相同，以檢閱詳細資料。  

    ![監視管線回合](./media/tutorial-incremental-copy-lastmodified-copy-data-tool/monitor-pipeline-runs5.png)

    您會看到一個檔案 (file1.txt) 複製**來源**容器**目的地**儲存體帳戶的容器。
    
    ![監視管線回合](./media/tutorial-incremental-copy-lastmodified-copy-data-tool/monitor-pipeline-runs6.png)
    
15. 建立另一個空的文字檔案，並將它命名為 file2.txt。 若要將 file2.txt 檔案上傳**來源**在儲存體帳戶的容器。 您可以使用各種工具來執行這些工作，例如 [Azure 儲存體總管](https://storageexplorer.com/)。  
    
16. 執行步驟 13 與 14，相同，而且您會看到只需將新檔案 (file2.txt) 複製**來源**容器**目的地**儲存體帳戶中的下一個管線執行的容器。  
    
    ![監視管線回合](./media/tutorial-incremental-copy-lastmodified-copy-data-tool/monitor-pipeline-runs7.png)

    您也可以確認相同使用 Azure 儲存體總管 (https://storageexplorer.com/)掃描的檔案。
    
    ![監視管線回合](./media/tutorial-incremental-copy-lastmodified-copy-data-tool/monitor-pipeline-runs8.png)
