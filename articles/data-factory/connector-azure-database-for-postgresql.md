...
| connectionString | ODBC 連接字串，用於連線到適用於 PostgreSQL 的 Azure 資料庫。<br/>您也可以將密碼放在 Azure Key Vault 中，並從連接字串中提取 `password` 設定。 如需詳細資訊，請參閱下列範例，並[在 Azure Key Vault 中儲存認證](store-credentials-in-key-vault.md)。 | 是 |
...
| 查詢 | 使用自訂 SQL 查詢來讀取資料。 例如： `"SELECT * FROM MyTable"` | 否（如果已指定資料集中的 tableName 屬性） |
...
| 屬性 | 說明 | 必要項 |
|:--- |:--- |:--- |
| type | 複製活動接收器的 type 屬性必須設定為**AzurePostgreSQLSink**。 | 是 |
