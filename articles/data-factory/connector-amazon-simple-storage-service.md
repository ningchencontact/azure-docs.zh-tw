...
- 適用於 Amazon S3 貯體作業的 `s3:ListBucket` 或 `s3:GetBucketLocation`。 

>[!NOTE]
>在使用 Data Factory GUI 來撰寫時，測試連線和瀏覽/導覽檔案路徑等作業也需要 `s3:ListAllMyBuckets` 權限。 如果您不想授與此權限，請跳過連結服務建立頁面中的測試連線，並直接在資料集設定中指定路徑。
