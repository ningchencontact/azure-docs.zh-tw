---
title: 使用 Data Factory 將資料複製到 Azure Cosmos DB 或複製其中的資料 | Microsoft Docs
description: 了解如何使用 Data Factory 將資料從支援的來源資料存放區複製到 Azure Cosmos DB，或從 Azure Cosmos DB 複製到支援的接收存放區。
...
您可以將資料從 Azure Cosmos DB 複製到任何支援的接收資料存放區，或將資料從任何支援的來源資料存放區複製到 Azure Cosmos DB。 如需複製活動作為來源和接收端支援的資料存放區清單，請參閱[支援的資料存放區和格式](copy-activity-overview.md#supported-data-stores-and-formats)。

您可以使用 Azure Cosmos DB 連接器來：
...
下列各節提供屬性的相關詳細資料，您可使用這些屬性來定義 Azure Cosmos DB 專屬的 Data Factory 實體。

## <a name="linked-service-properties"></a>連結服務屬性

以下是針對 Azure Cosmos DB 連結服務支援的屬性：

| 屬性 | 說明 | 必要 |
|:--- |:--- |:--- |
| type | **type** 屬性必須設定為 **CosmosDb**.。 | 是 |
| connectionString |指定連線到 Azure Cosmos DB 資料庫所需的資訊。<br /><br />**注意**：您必須在連接字串中指定資料庫資訊，如後續範例所示。 將此欄位標記為 **SecureString** 類型，將它安全地儲存在 Data Factory 中。 您也可以[參考 Azure Key Vault 中儲存的認證](store-credentials-in-key-vault.md)。 |是 |
...
本節提供 Azure Cosmos DB 資料集所支援的屬性清單。 

如需定義資料集的區段和屬性完整清單，請參閱[資料集和連結服務](concepts-datasets-linked-services.md)。 

若要從 Azure Cosmos DB 複製資料或將資料複製到其中，請將資料集的 **type** 屬性設定為 **DocumentDbCollection**。 以下是支援的屬性：
...
若要將資料複製到 Azure Cosmos DB，請將複製活動中的**接收**類型設定為 **DocumentDbCollectionSink**。 
