...
2. 允許 Azure 服務存取 SQL Server。 確認已為執行 SQL Database 的伺服器啟用 [允許存取 Azure 服務]  設定。 此設定可讓 Data Factory 將資料寫入您的資料庫執行個體。 若要確認並開啟此設定，請前往 Azure SQL Server > [概觀] > [設定伺服器防火牆] > 將 [允許存取 Azure 服務]  選項設定為 [開啟]  。
