...
>[!NOTE]
>Azure SQL Database 受控執行個體[Always Encrypted](https://docs.microsoft.com/sql/relational-databases/security/encryption/always-encrypted-database-engine?view=azuresqldb-mi-current)現在不支援此連接器。 若要解決，您可以使用[一般的 ODBC 連接器](connector-odbc.md)以及 SQL Server ODBC 驅動程式透過自我裝載的整合執行階段。 請遵循[本指南](https://docs.microsoft.com/sql/connect/odbc/using-always-encrypted-with-the-odbc-driver?view=azuresqldb-mi-current)具有 ODBC 驅動程式下載及連接字串組態。

>[!NOTE]
>服務主體和受管理的身分識別驗證目前不支援此連接器。 若要解決，請選擇 Azure SQL Database 連接器並手動指定您的受控執行個體的伺服器。

## <a name="prerequisites"></a>必要條件

若要存取 Azure SQL Database 受控執行個體[公用端點](../sql-database/sql-database-managed-instance-public-endpoint-securely.md)，您可以使用 Azure Data Factory 受控 Azure 整合執行階段。 請確定您啟用公用端點，並也允許公用端點上的流量的網路安全性群組，讓 Azure Data Factory 可以連線到您的資料庫。 如需詳細資訊，請參閱 <<c0> [ 本指南](../sql-database/sql-database-managed-instance-public-endpoint-configure.md)。

若要存取 Azure SQL Database 受控執行個體的私用端點，請設定[自我裝載整合執行階段](create-self-hosted-integration-runtime.md)可存取資料庫。 如果您佈建為受管理的執行個體相同的虛擬網路中的自我裝載的整合執行階段，請確定您的整合執行階段電腦會在您的受控執行個體的不同子網路。 如果您佈建您在不同的虛擬網路中的自我裝載的整合執行階段於您受管理的執行個體時，您可以使用虛擬網路對等互連或虛擬網路連線的虛擬網路。 如需詳細資訊，請參閱[將您的應用程式連線到 Azure SQL Database 受控執行個體](../sql-database/sql-database-managed-instance-connect-app.md)。

## <a name="get-started"></a>開始使用

[!INCLUDE [data-factory-v2-connector-get-started](../../includes/data-factory-v2-connector-get-started.md)]

下列各節提供有關用來定義 Azure SQL Database 受控執行個體連接器專屬的 Azure Data Factory 實體的屬性的詳細資料。
...
| connectionString |這個屬性會指定**connectionString**使用 SQL 驗證連線到受管理的執行個體所需的資訊。 如需詳細資訊，請參閱下列範例。 <br/>將做為這個欄位標記**SecureString**若要將它安全地儲存在 Azure Data Factory。 您也可以放在 Azure Key Vault 的密碼。 如果是 SQL 驗證，提取`password`設定連接字串。 如需詳細資訊，請參閱表格後的 JSON 範例和[在 Azure Key Vault 中儲存認證](store-credentials-in-key-vault.md)。 |是 |
| connectVia | 用來連線到資料存放區的[整合執行階段](concepts-integration-runtime.md)。 如果您的受控執行個體有公用端點，而且允許存取 Azure Data Factory，您可以使用自我裝載的整合執行階段或在 Azure 整合執行階段。 如果未指定，則會使用預設的 Azure 整合執行階段。 |是 |

**範例 1：使用 SQL 驗證**

預設連接埠為 1433年。 如果您使用 Azure SQL Database 受控執行個體具有公用端點，明確指定連接埠 3342。
...
> [!TIP]
> 深入了解支援的寫入行為、 組態和最佳作法[將資料載入 Azure SQL Database 受控執行個體的最佳做法](#best-practice-for-loading-data-into-azure-sql-database-managed-instance)。
...
| writeBatchTimeout |此屬性會指定在逾時前等待批次插入作業完成的時間。<br/>時間範圍是允許的值。 範例是 “00:30:00”，也就是 30 分鐘。 |否 |
| preCopyScript |此屬性會指定複製活動將資料寫入到受管理的執行個體前執行的 SQL 查詢。 每一複製回合只會叫用此查詢一次。 您可以使用此屬性來清除預先載入的資料。 |否 |
...
將資料附加為此 Azure SQL Database 受控執行個體的接收連接器的預設行為。 Azure Data Factory 會執行大量插入來有效率地寫入至您的資料表。 您可以設定來源，並據以接收將複製活動中。
...
例如，，您也可以在 Azure Data Factory 中建立管線，其中含有**複製活動**與鏈結**預存程序活動**。 先前的資料複製您的來源存放區至暫存資料表，比方說， **##UpsertTempTable**，資料集內資料表的名稱。 然後，後者會叫用來從暫存資料表的來源資料合併到目標資料表，並清除 暫存資料表的預存程序。
...
**選項 2：** 您也可以選擇[叫用的複製活動中的預存程序](#invoke-a-stored-procedure-from-a-sql-sink)。 這種方法會執行每個資料列，而不是在複製活動中，並不適合用於大規模的 upsert 的預設方法為使用大量插入的來源資料表中。
...
> [!TIP]
> 叫用預存程序以處理資料列而不是使用大量作業，我們不建議大規模的複本。 進一步了解[將資料載入 Azure SQL Database 受控執行個體的最佳做法](#best-practice-for-loading-data-into-azure-sql-database-managed-instance)。
