...
若要從**Parquet、分隔文字、JSON、Avro 和二進位格式**複製資料，請參閱格式為基礎的資料集上的[Parquet 格式](format-parquet.md)、[分隔的文字格式](format-delimited-text.md)、 [avro 格式](format-avro.md)和[二進位格式](format-binary.md)一文，以及支援的設定。 下列屬性在以格式為基礎之`location`資料集的 [設定] 下支援 HDFS：
...
> [!NOTE]
> 下一節中所述的檔案**共用**類型資料集的 Parquet/Text 格式仍然受支援，以適用于回溯相容性的複製/查閱活動。 建議您繼續使用此新模型，而 ADF 撰寫 UI 已切換為產生這些新的類型。
...
如需可用來定義活動的區段和屬性完整清單，請參閱[Pipelines](concepts-pipelines-activities.md)一文。 本節提供 HDFS 來源所支援的屬性清單。
...
若要從**Parquet、分隔文字、JSON、Avro 和二進位格式**複製資料，請參閱格式為基礎之複製活動來源的[Parquet 格式](format-parquet.md)、[分隔文字格式](format-delimited-text.md)、 [Avro 格式](format-avro.md)和[二進位格式](format-binary.md)文章，並加以支援設置。 下列屬性在以格式為基礎的`storeSettings`複製來源的設定下支援 HDFS：

| 屬性                 | 描述                                                  | 必要項                                      |
| ------------------------ | ------------------------------------------------------------ | --------------------------------------------- |
| type                     | 底下的 type 屬性`storeSettings`必須設定為**HdfsReadSetting**。 | 是                                           |
...
若要以**ORC 格式**從 HDFS 複製資料，複製活動的 [**來源**] 區段中支援下列屬性：

| 屬性 | 描述 | 必要項 |
|:--- |:--- |:--- |
| 型別 | 複製活動來源的類型屬性必須設定為：**HdfsSource** |是 |
