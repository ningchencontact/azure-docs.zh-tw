---
title: "將資料從使用 Azure Data Factory (Beta) 的 Google BigQuery 複製 |Microsoft 文件"
description: "了解如何將資料從 Google BigQuery 複製到支援的接收資料存放區，在 Azure Data Factory 管線中使用複製活動。"
...
本文將概述如何使用 Azure Data Factory 中的複製活動，從 Google BigQuery 複製資料。 本文是根據[複製活動概觀](copy-activity-overview.md)一文，該文提供複製活動的一般概觀。
...
您可以從 Google BigQuery 資料複製到任何支援的接收資料存放區。 如需複製活動所支援作為來源/接收器的資料存放區清單，請參閱[支援的資料存放區](copy-activity-overview.md#supported-data-stores-and-formats)表格。
...
下列各節提供有關用來定義特定的 Data Factory 實體至 Google BigQuery 連接器屬性詳細資料。
...
| 電子郵件 | 服務帳戶電子郵件識別碼用於 ServiceAuthentication，只用於在自我裝載的紅外線  | 否 |
| keyFilePath | 用來驗證服務帳戶電子郵件地址，並只可用在自我裝載的紅外線.p12 金鑰檔完整路徑  | 否 |
...
如需可用來定義資料集的區段和屬性完整清單，請參閱[資料集](concepts-datasets-linked-services.md)一文。 本節提供一份 Google BigQuery 資料集所支援的屬性。

若要從 Google BigQuery 複製資料，設定要的資料集的類型屬性**GoogleBigQueryObject**。 沒有任何額外的特定類型的屬性，在這種類型的資料集。
...
如需可用來定義活動的區段和屬性完整清單，請參閱[管線](concepts-pipelines-activities.md)一文。 本節提供一份 Google BigQuery 來源所支援的屬性。

### <a name="googlebigquerysource-as-source"></a>做為來源 GoogleBigQuerySource

若要從 Google BigQuery 複製資料，請將來源類型複製活動中**GoogleBigQuerySource**。 複製活動的 **source** 區段支援下列屬性：

| 屬性 | 說明 | 必要 |
|:--- |:--- |:--- |
| type | 複製活動來源的類型屬性必須設定為： **GoogleBigQuerySource** | 是 |
