...
| accessKeyId | 密碼存取金鑰的識別碼。 若要尋找存取金鑰和祕密，請前往 [Google Cloud Storage] > [設定] > [互通性]。 |有 |
...
| serviceUrl | 指定自訂 S3 端點做為 **`https://storage.googleapis.com`**。 | 有 |
...
| fileName   | 在指定的貯體 + folderPath 檔案名稱。 如果您想要使用萬用字元來篩選檔案，略過這項設定，並在 活動來源設定中指定。 | 無       |
...
| version | 如果已啟用 S3 版本設定功能，則為 S3 物件的版本。 |無 |
