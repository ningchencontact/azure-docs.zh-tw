...
偵錯設定可從您的資料流的每個來源會出現在側邊面板，然後選取 資料流程 設計工具工具列上的 來源設定 」，也可以編輯。 您可以選取限制及/或檔案來源，以便在此用於您的每個來源轉換。 此設定中的資料列限制為僅針對目前的偵錯工作階段。 您也可以使用在來源中的取樣設定成來源 transforamtion 限制資料列。
