...
在資料流中依名稱選取資料行值。 如果有多個相符項目，則會傳回第一個相符項目。 如果沒有相符項目則會傳回 NULL 值。 傳回的值必須是型別轉換的其中一個類型轉換函式 （TO_DATE、 TO_STRING...）。在設計階段已知的資料行名稱應該解決只是根據其名稱。 不支援計算的輸入，但您可以使用參數的替代項目
...
選取的資料行值的相對位置 (從 1 開始) 資料流中。 如果位置超出範圍則會傳回 NULL 值。 傳回的值必須為型別轉換的其中一個類型轉換函式 （TO_DATE、 TO_STRING...）不支援計算的輸入，但您可以使用參數的替代項目
...
將從 UTC 轉換為時間戳記。 您可以選擇性地傳遞 'GMT'、 'PST'、 'UTC'、 'America/開曼' 形式的時區。 它會預設為目前的時區
...
將任何數值或字串轉換為整數值。 是選用的 Java 十進位格式可用來進行轉換。會截斷任何 long、 float、 double
* ``toInteger(123) -> 123``
* ``toInteger('123') -> 123``
* ``toInteger('$123', '$###') -> 123``
*********************************
