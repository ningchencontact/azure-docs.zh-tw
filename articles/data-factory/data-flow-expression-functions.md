...
description: Azure Data Factory 對應資料流程運算式函式語言
author: kromerm
ms.author: makromer
...
將從 UTC 轉換為時間戳記。 您可以選擇性地傳遞 'GMT'、 'PST'、 'UTC'、 'America/開曼' 形式的時區。 其預設值為目前的時區 * ``fromUTC(currentTimeStamp()) -> 12-12-2030T19:18:12``
...
根據條件套用一個值或另一個值。 如果另一個則是未指定，則會視為 NULL。 這兩個值必須相容 (數字、字串...) * ``iif(custType == 'Premium', 10, 4.5)``
* ``iif(amount > 100, 'High')``
...
取得第一個參數對目前資料列之前的 n 個資料列評估的值。 第二個參數是要回頭查看以前的資料列數目和預設值為 1。 如果沒有那麼多資料列，則會傳回 null 值，除非指定了預設值 * ``lag(amount, 2) -> 60``
...
取得第一個參數對目前資料列之後的 n 個資料列評估的值。 第二個參數是期待的資料列數目和預設值為 1。 如果沒有那麼多資料列，則會傳回 null 值，除非指定了預設值 * ``lead(amount, 2) -> 60``
...
傳回 NULL 值。 如果沒有名為 'null' 的資料行名稱，請使用函式 syntax(null())。 任何使用此函式的作業都將導致 NULL * ``custId = NULL (for derived field)``
* ``custId == NULL -> NULL``
* ``'nothing' + NULL -> NULL``
* ``10 * NULL -> NULL'``
* ``NULL == '' -> NULL'``
...
將指定選擇性的小數位數和選擇性的捨入模式的數字四捨五入。 如果省略小數位數，則它會預設為 0。  如果省略捨入模式，則預設為 ROUND_HALF_UP(5)。 捨入計算的值包括 1 - ROUND_UP 2 - ROUND_DOWN 3 - ROUND_CEILING 4 - ROUND_FLOOR 5 - ROUND_HALF_UP 6 - ROUND_HALF_DOWN 7 - ROUND_HALF_EVEN 8 - ROUND_UNNECESSARY * ``round(100.123) -> 100.0``
* ``round(2.5, 0) -> 3.0``
...
計算的改變基本的資料型別指定值 0(256) 只能是位元長度的資料行集的 sha-2 摘要 224、 256、 384、 512。 這可以用來計算資料列的指紋 * ``sha2(256, 'gunchus', 8.2, 'bojjus', true, toDate('2010-4-4')) -> 'd3b2bff62c3a00e9370b1ac85e428e661a7df73959fa1a96ae136599e9ee20fd'``
*********************************
<code>sin</code>
==============================
...
以指定的選擇性時間戳記格式將字串轉換為日期。 如需所有可能的格式，請參閱 Java SimpleDateFormat。 如果省略時間戳記，則使用預設模式 yyyy-[M]M-[d]d hh:mm:ss[.f...] * ``toTimestamp('2016-12-31 00:12:00') -> 2012-8-8T00:12:00``
...
Java HotSpot(TM) 64 位元伺服器 VM 警告： 選項 MaxPermSize 忽略 = 2048m;8.0 [成功] 中已移除支援的總時間：5 秒，已完成 2019 年 4 月 3 日下午 4:17:11 > D:\kadfdataflow\DataflowExecutor\target\scala-2.11\classes runMain com.microsoft.dataflow.FunctionPrinter [info] 編譯 1 Scala 來源...[error]D:\kadfdataflow\DataflowExecutor\src\main\scala\com\microsoft\dataflow\FunctionPrinter.scala:92： 後置運算子 [error] typeOf[SLICE.type 不允許應用程式類型]，[error] ^ [error] 其中一個錯誤中找到任何 [錯誤] (資料流程 / 編譯： 編譯) 編譯失敗 [錯誤] 總時間：24 秒，已完成 2019 年 4 月 3 日下午 4:43:43 >> D:\kadfdataflow\DataflowExecutor\target\scala-2.11\classes runMain com.microsoft.dataflow.FunctionPrinter [info] 編譯 1 Scala 來源...[資訊]執行 com.microsoft.dataflow.FunctionPrinter 接聽位址的傳輸 dt_socket:8000 *********************************
<code>abs</code>
==============================
<code><b>abs(<i>&lt;value1&gt;</i> : number) => number</b></code><br/><br/>
數字配對的正模數。
...
==============================
<code><b>log(<i>&lt;value1&gt;</i> : number, [<i>&lt;value2&gt;</i> : number]) => double</b></code><br/><br/>
計算對數值。 選擇性基底可以是提供其他歐數使用 * ``log(100, 10) -> 2``
*********************************
<code>log10</code>
==============================
<code><b>log10(<i>&lt;value1&gt;</i> : number) => double</b></code><br/><br/>
...
==============================
<code><b>toInteger(<i>&lt;value&gt;</i> : any, [<i>&lt;format&gt;</i> : string]) => integer</b></code><br/><br/>
將任何數值或字串轉換成整數值。 可使用選擇性的 Java 十進位格式進行轉換。截斷任何長值、浮點數、雙精準數 * ``toInteger(123) -> 123``
* ``toInteger('123') -> 123``
* ``toInteger('$123', '$###') -> 123``
*********************************
<code>toLong</code>
