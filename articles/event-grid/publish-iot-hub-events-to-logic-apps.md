...
1. 在 [Azure 入口網站](https://portal.azure.com) 中，選取 **新增** > **企業整合** > **邏輯應用程式**。
...
1. 選取 [新增步驟]，然後選取 [新增動作]。 
   ...
   ![新增步驟、新增動作](./media/publish-iot-hub-events-to-logic-apps/new-step.png)
...
## <a name="publish-an-event-from-iot-hub"></a>從 IoT 中樞發佈事件
...
4. 使用下列值來建立事件訂用帳戶： 
   * **名稱**：提供描述性名稱。
   * **訂閱所有事件類型**：取消選取此核取方塊。
   * **事件類型**：選取 [DeviceCreated]。
   * **訂閱者類型**：選取 [Web Hook]。
   * **訂閱者端點**：貼上您從的邏輯應用程式複製的 URL。 

   您可以在這裡儲存事件訂用帳戶，並接收在 IoT 中樞內建立之每個裝置的通知。 但在此教學課程中，我們可使用選用欄位來篩選特定裝置： 

   * **前置詞篩選**：輸入 `devices/Building1_` 以篩選建築物 1 中的裝置事件。
   * **後置詞篩選**：輸入 `_Temperature` 以篩選與溫度相關的裝置事件。
