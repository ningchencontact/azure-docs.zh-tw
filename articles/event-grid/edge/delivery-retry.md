...
ms.locfileid: "72992388"
---
# <a name="delivery-and-retry"></a>傳遞和重試

Event Grid 提供持久的傳遞。 它會嘗試立即為每個相符的訂用帳戶傳遞每個訊息一次。 如果訂閱者的端點未確認收到事件或發生失敗，事件方格會根據固定的**重試排程**和**重試原則**來重試傳遞。  目前，事件方格模組會將事件一次傳遞給訂閱者。 但裝載是具有單一事件的陣列。
