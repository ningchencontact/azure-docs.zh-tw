...
您可以使用 Azure PowerShell，在建立[應用程式閘道](application-gateway-multi-site-overview.md)時設定 [Web 流量重新導向](application-gateway-introduction.md)。 在本教學課程中，您會設定接聽程式和規則，以將抵達應用程式閘道的 Web 流量重新導向至外部網站。
...
需要接聽程式才能讓應用程式閘道適當地路由傳送流量。 使用 New-azapplicationgatewayHTTPlistener 搭配您先前建立的前端設定和前端埠，來建立接聽[程式](/powershell/module/az.network/new-azapplicationgatewayhttplistener)。 您可以藉由建立可新增至您所建立之路由規則的重新導向設定，來將流量重新導向。

接聽程式需要路由規則才能知道要將傳入流量傳送到何處。 使用 New-azapplicationgatewayrequestroutingrule 搭配重新[導向](/powershell/module/az.network/new-azapplicationgatewayrequestroutingrule)設定，建立名為*redirectRule*的基本規則。
