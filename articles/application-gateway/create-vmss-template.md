...
description: 本文提供指示來建立使用 Azure Resource Manager 範本的 Azure 應用程式閘道
...
ms.locfileid: "67449542"
---
# <a name="create-an-application-gateway-using-the-azure-resource-manager-template"></a>建立使用 Azure Resource Manager 範本的應用程式閘道
...
這篇文章會逐步引導您下載並修改現有[Azure Resource Manager 範本](../azure-resource-manager/resource-group-authoring-templates.md)從 GitHub 和從 GitHub、 Azure PowerShell 和 Azure CLI 部署範本。

如果您是只要直接從 GitHub 部署範本而不需要任何變更，請跳至從 GitHub 部署範本。
...
1. 將檔案儲存至您電腦上的本機資料夾。
1. 如果您已熟悉使用 Azure Resource Manager 範本，請跳至步驟 7。
2. 開啟您儲存的檔案，查看 **parameters** 這行下方的內容。
3. Azure 資源管理員範本的參數提供值的預留位置，可以在部署期間填寫。
...
1. 儲存檔案。 您可以測試 JSON 範本和參數的範本，使用線上 JSON 驗證工具，像是[JSlint.com](https://www.jslint.com/)。

## <a name="deploy-the-azure-resource-manager-template-using-azure-powershell"></a>部署使用 Azure PowerShell 的 Azure Resource Manager 範本

[!INCLUDE [updated-for-az](../../includes/updated-for-az.md)]

如果您從未使用過 Azure PowerShell，請參閱：[如何安裝和設定 Azure PowerShell](/powershell/azure/overview)，並遵循指示登入 Azure，然後選取您的訂用帳戶。
...
1. 如有需要建立資源群組，使用**New-azureresourcegroup** cmdlet。 在下列範例中，您會在美國東部位置建立名為 AppgatewayRG 的資源群組。

    ```azurepowershell
    New-AzResourceGroup -Name AppgatewayRG -Location "West US"
    ```

1. 執行**新增 AzResourceGroupDeployment** cmdlet，來部署新的虛擬網路，使用上述的範本和參數檔案，下載並修改。
...
## <a name="deploy-the-azure-resource-manager-template-using-the-azure-cli"></a>部署 Azure Resource Manager 範本使用 Azure CLI

若要部署您下載使用 Azure CLI 的 Azure Resource Manager 範本，請遵循下列步驟：
...
1. 執行`az group deployment create`cmdlet，來部署新的虛擬網路使用的範本和參數檔案，下載並修改在上一個步驟。 輸出後顯示的清單可說明所使用的參數。

    ```azurecli
    az group deployment create --resource-group appgatewayRG --name TestAppgatewayDeployment --template-file azuredeploy.json --parameters @azuredeploy-parameters.json
    ```

## <a name="deploy-the-azure-resource-manager-template-using-click-to-deploy"></a>部署 Azure Resource Manager 範本使用按一下部署
...
如果您想要設定 SSL 卸載，請參閱：[設定適用於 SSL 卸載的應用程式閘道](application-gateway-ssl.md)。

如果您想要將應用程式閘道設定為使用內部負載平衡器，請參閱：[利用內部負載平衡器 (ILB) 建立應用程式閘道](application-gateway-ilb.md)。
