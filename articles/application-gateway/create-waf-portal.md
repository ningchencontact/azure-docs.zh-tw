...
您可以使用 Azure 入口網站建立包含 [Web 應用程式防火牆](waf-overview.md) (WAF) 的[應用程式閘道](overview.md)。 WAF 會使用 [OWASP](https://www.owasp.org/index.php/Category:OWASP_ModSecurity_Core_Rule_Set_Project) 規則來保護您的應用程式。 這些規則包括防禦諸如 SQL 插入攻擊、跨網站指令碼攻擊，以及工作階段劫持等攻擊。
...
![Web 應用程式防火牆範例](./media/create-waf-portal/scenario-waf.png)
...
3. 針對應用程式閘道輸入這些值：
    ...
    ![建立新的應用程式閘道](./media/create-waf-portal/application-gateway-create.png)
...
5. 按一下 [選擇虛擬網路]，按一下 [新建]，然後針對虛擬網路輸入這些值：
    ...
    ![建立虛擬網路](./media/create-waf-portal/application-gateway-vnet.png)
...
2. 按一下 [子網路]，然後按一下 [子網路]。
    ...
    ![建立子網路](./media/create-waf-portal/application-gateway-subnet.png)
...
1. 開啟互動式殼層，並確定它是設定為 **PowerShell**。
    ...
    ![安裝自訂延伸模組](./media/create-waf-portal/application-gateway-extension.png)
...
3. 按一下 [新增目標]，將您所建立的每個虛擬機器新增至後端集區。
    ...
    ![新增後端伺服器](./media/create-waf-portal/application-gateway-backend.png)
...
7. 按一下 [檔案] 。
    ...
    ![設定診斷](./media/create-waf-portal/application-gateway-diagnostics.png)

## <a name="test-the-application-gateway"></a>測試應用程式閘道

1. 在 [概觀] 畫面上尋找應用程式閘道的公用 IP 位址。 按一下 [所有資源]，然後按一下 [myAGPublicIPAddress]。

    ![記錄應用程式閘道公用 IP 位址](./media/create-waf-portal/application-gateway-record-ag-address.png)

2. 將公用 IP 位址複製並貼到您瀏覽器的網址列。

    ![測試應用程式閘道](./media/create-waf-portal/application-gateway-iistest.png)
