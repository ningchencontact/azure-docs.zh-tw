...
2. 接受其他設定的預設值，然後按一下 [確定]。

### <a name="settings"></a>設定

1. 按一下 [選擇虛擬網路]，按一下 [新建]，然後針對虛擬網路輸入這些值：

    - myVNet - 作為虛擬網路的名稱。
    - 10.0.0.0/16 - 作為虛擬網路位址空間。
    - myAGSubnet - 作為子網路名稱。
    - 10.0.0.0/24 - 作為子網路位址範圍。

    ![建立虛擬網路](./media/application-gateway-create-gateway-portal/application-gateway-vnet.png)

6. 按一下 [確定] 回到 [設定] 頁面。
7. 在 [前端 IP 組態] 下方確認 [IP 位址類型] 設為 [公用]，然後在 [公用 IP 位址] 下方確認已選取 [新建]。 輸入 *myAGPublicIPAddress* 作為公用 IP 位址名稱。 接受其他設定的預設值，然後按一下 [確定]。

### <a name="summary"></a>總結

檢閱 [摘要] 分頁上的設定，然後按一下 [確定] 以建立網路資源、公用 IP 位址和應用程式閘道。 可能需要幾分鐘的時間來建立應用程式閘道。 請等候部署成功完成後，再繼續進行至下一節。
...
5. 在 [目標] 下方按一下 **IP 位址或 FQDN**，然後選取 [虛擬機器]。
6. 在 [虛擬機器] 下方新增 myVM 和 myVM2 虛擬機器及其相關聯的網路介面。
