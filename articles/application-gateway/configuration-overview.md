...
您可以設定應用程式閘道具有公用 IP 位址或私人的 IP 位址，或兩者。 當您裝載的後端，才能透過網際網路對向的 VIP 網際網路用戶端存取時需要公用 IP。 公用 IP 並不需要不公開到網際網路，也稱為內部負載平衡器 (ILB) 端點的內部端點。 使用 ILB 設定閘道適合不會對網際網路公開的內部企業營運應用程式。 對於位在不會對網際網路公開的安全性界限中的多層式應用程式內的服務/階層也十分實用，但仍需要循環配置資源負載散發、工作階段綁定或安全通訊端層 (SSL) 終止。

支援只有一個公用 IP 位址或一個私人 IP 位址。 您選擇同時建立應用程式閘道的前端 IP。 
...
- 如果您要在相同的應用程式閘道執行個體上設定一個以上的 web 應用程式或多個相同的父系網域的子網域，然後選擇 多站台接聽程式。 多站台接聽程式，您將此外要輸入主機名稱。 這是因為應用程式閘道會依賴 HTTP 1.1 主機標頭來裝載多個網站上的相同公用 IP 位址和連接埠。![1551057450710](C:/Users/absha/AppData/Roaming/Typora/typora-user-images/1551057450710.png)
