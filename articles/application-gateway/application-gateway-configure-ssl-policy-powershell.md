...
應用程式閘道隨附三個可用的預先定義原則。 `Get-AzApplicationGatewaySslPredefinedPolicy` Cmdlet 會擷取這些原則。 每個原則都有不同的通訊協定版本與已啟用的加密套件。 這些預先定義的原則可以用來快速地在應用程式閘道上設定 SSL 原則。 根據預設，如果未定義特定的 SSL 原則，則會選取 **AppGwSslPolicy20150501**。
