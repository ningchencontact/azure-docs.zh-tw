...
在此範例中，您要建立三個虛擬機器，作為應用程式閘道的後端伺服器。 您也會在虛擬機器上安裝 IIS, 以確認應用程式閘道如預期般運作。

1. 在 Azure 入口網站上，選取 [建立資源]。
2. 在熱門清單中選取 [ **Windows Server 2016 Datacenter** ]。
3. 針對虛擬機器，請輸入這些值：

    - [**資源群組**] 中, 選取 [**新建**], 然後輸入*myResourceGroupAG*。
    - **虛擬機器名稱**: *myVM1*
    - **區域**：*回饋美國東部*
    - 使用者**名稱**: *azureuser*
    - **密碼**：*Azure123456!*
...
4. 選取 [下一步：磁碟]。
5. 選取**下一步: 網路**功能
6. 針對 [**虛擬網路**], 選取 [**建立新**的], 然後輸入虛擬網路的下列值:

   - myVNet - 作為虛擬網路的名稱。
   - 10.0.0.0/16 - 作為虛擬網路位址空間。
   - 第一個子網名稱的*myBackendSubnet*
   - *10.0.1.0/24* - 作為子網路位址空間。
   - *myAGSubnet* -作為第二個子網名稱。
   - 10.0.0.0/24 - 作為子網路位址空間。
7. 選取 [確定]。

8. 確認在 [**網路介面**] 底下, 為子網選取了 [ **myBackendSubnet** ] **, 然後選取 [下一步]:管理**。
9. 選取 [關閉] 來停用開機診斷。
10. 按一下 [審核] [ **+ 建立**], 檢查 [摘要] 頁面上的設定, 然後選取 [**建立**]。
11. 建立兩部以上的虛擬機器: *myVM2*和*myVM3* , 並將它們放在*MyVNet*虛擬網路和*myBackendSubnet*子網中。
...
1. 在 [基本] 索引標籤上，為下列應用程式閘道設定輸入這些值：
   ...
   - **资源组**：選取 **myResourceGroupAG** 作為資源群組。
   - **應用程式閘道名稱**：輸入 myAppGateway 作為應用程式閘道的名稱。
   - **區域**-選取 **(美國) [美國東部**]。

        ![建立新的應用程式閘道：基本知識](./media/application-gateway-create-gateway-portal/application-gateway-create-basics.png)

2.  在 [**設定虛擬網路**] 底下, 選取 [ **myVNet** ] 作為虛擬網路的名稱。
3. 選取子網的 [ **myAGSubnet** ]。
...
3. 在 [**後端目標**]、[**目標型別**] 底下, 從下拉式清單中選取 [**虛擬機器**]。

5. 在 [**目標**] 底下, 選取用於**myVM1**的網路介面。
6. 選取 [新增]。
7. 重複新增具有*myVM2*作為目標的*映射*後端集區, 以及使用*myVM3*作為目標的*影片*後端集區。
8. 選取 [**新增**] 以儲存後端集區設定, 並返回 [**後端**] 索引標籤。
...
2. 將公用 IP 位址複製並貼到您瀏覽器的網址列。 例如, HTTP:\//52.188.72.175: 8080。
   ...
   埠8080上的接聽程式會將此要求路由傳送至預設後端集區。

3. 將 URL 變更為*HTTP://&lt;ip 位址&gt;: 8080/images/test.htm*, 將 ip &lt;位址&gt;取代為您的 ip 位址, 您應該會看到如下列範例所示的內容:

    ![在應用程式閘道中測試影像 URL](./media/application-gateway-create-url-route-portal/application-gateway-iistest-images.png)

   埠8080上的接聽程式會將此要求路由傳送至*映射*後端集區。

4. 將 URL 變更為*HTTP://&lt;ip 位址&gt;: 8080/video/test.htm*, 將 ip &lt;位址&gt;取代為您的 ip 位址, 您應該會看到如下列範例所示的內容:

    ![在應用程式閘道中測試影片 URL](./media/application-gateway-create-url-route-portal/application-gateway-iistest-video.png)

   埠8080上的接聽程式會將此要求路由傳送至*影片*後端集區。
