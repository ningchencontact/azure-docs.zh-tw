...
本教學課程需要 Azure PowerShell 模組版本 1.0.0 或更新版本，才能建立憑證，並安裝 IIS。 執行 `Get-Module -ListAvailable Az` 以尋找版本。 如果您需要升級，請參閱[安裝 Azure PowerShell 模組](/powershell/azure/install-az-ps)。 若要在本教學課程中執行命令，則也需要執行 `Login-AzAccount` 以建立與 Azure 的連線。
