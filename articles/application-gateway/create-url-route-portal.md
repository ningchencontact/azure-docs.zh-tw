...
2. 在隨即開啟的 [新增後端集區]  視窗中，輸入下列值以建立空的後端集區：
    ...
    - **名稱**：輸入 *appGatewayBackendPool* 作為後端集區的名稱。
    - **新增不含目標的後端集區**：選取 [是]  以建立不含目標的後端集區。 您將會在建立應用程式閘道之後再新增後端目標。

3. 在 [新增後端集區]  視窗中，選取 [新增]  以儲存後端集區設定，並返回 [後端]  索引標籤。
4. 現在，再新增兩個後端集區，名稱分別為 imagesBackendPool  和 videoBackendPool  。

     ![建立新的應用程式閘道：後端](./media/create-url-route-portal/backends.png)
...
2. 在隨即開啟的 [新增路由規則]  視窗中，針對 [規則名稱]  輸入 Rule2  。

3. 路由規則需要接聽程式。 在 [新增路由規則]  視窗內的 [接聽程式]  索引標籤上，針對接聽程式輸入下列值：

    - **接聽程式名稱**：輸入 myBackendListener  作為接聽程式的名稱。
    - **前端 IP**：選取 [公用]  以選擇您針對前端所建立的公用 IP。
    - **連接埠**：8080
...
5. 針對 [HTTP 設定]  ，選取 [myHTTPSetting]  。 接受 [新增 HTTP 設定]  視窗中其餘設定的預設值，然後選取 [新增]  以返回 [新增路由規則]  視窗。 

1. 在 [路徑型路由]  底下，選取 [新增多個目標以建立路徑型規則]  。
2. 在 [新增路徑規則]  視窗上，為路徑規則輸入下列值：

   - **路徑**：/images/ *\**
   - **路徑規則名稱**：*映像*
   - **HTTP 設定**：選取 [myHTTPSetting] 
   - **後端目標**：imagesBackendPool 
9. 選取 [新增]  。
10. 再新增一個名為 Video  的路徑規則，讓其具有 /video/ *\** 路徑和 videoBackendPool  。
11. 選取 [儲存變更並返回路由規則]  。

    ![新增路由規則](media/create-url-route-portal/add-routing-rule.png)
...
2. 選取 [計算]  ，然後選取 [熱門]  清單中的 [Windows Server 2016 Datacenter]  。 [建立虛擬機器]  頁面隨即出現。
...
3. 再多建立兩個虛擬機器，並使用您剛完成的步驟來安裝 IIS。 使用 myVM2  和 myVM3  作為虛擬機器名稱以及 Set-AzVMExtension 中的 **VMName** 值。
...
5. 在 [虛擬機器]  和 [網路介面]  下方，從下拉式清單中選取 [myVM1]  及其相關聯的網路介面。

    ![新增後端伺服器](./media/create-url-route-portal/backend-pool.png)

6. 選取 [ **儲存**]。
7. 重複同樣步驟來新增 myVM2  和 imagesBackendPool  的介面，然後新增 myVM3  和 videoBackendPool  的介面。
