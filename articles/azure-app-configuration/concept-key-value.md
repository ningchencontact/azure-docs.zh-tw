...
若設定存放區在應用程式架構內，則其使用方式可能會要求特定的索引鍵值命名配置。 例如，Java 的 Spring Cloud 架構會定義 `Environment` 資源，以提供 Spring 應用程式的設定，讓其可透過變數 (包括應用程式名稱和設定檔) 來設置參數。 Spring Cloud 相關設定資料的索引鍵通常會以這兩個元素開頭，並以分隔符號隔開。
