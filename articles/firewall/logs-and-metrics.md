...
- **防火牆健全狀況狀態**-根據 SNAT 埠可用性指出防火牆的健全狀況。
  ...
  - 原因：指出防火牆的對應狀態的原因。 
     ...
     如果使用 SNAT 埠 > 95%，則會將其視為已耗盡，而健全狀況為50%，狀態 = 已**降級**，原因 =**SNAT 埠**。 防火牆會持續處理流量，而且現有的連線不會受到影響。 不過，可能不會間歇地建立新的連接。

     如果使用 SNAT 埠 < 95%，則會將防火牆視為狀況良好，且健康情況會顯示為100%。

     如果未回報 SNAT 埠的使用方式，健康狀態會顯示為0%。 
