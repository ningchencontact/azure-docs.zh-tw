...
Azure 防火牆沒有輸入規則和輸出規則的概念。 但是有應用程式規則和網路規則，這些規則會套用到進入防火牆的任何流量。 首先會套用網路規則，然後套用應用程式規則，之後規則就會終止。

例如，如果符合網路規則時，封包不會由應用程式規則進行評估。 如果沒有符合的網路規則，而且封包通訊協定是 HTTP/HTTPS，則封包會由應用程式規則評估。 如果仍然找不到符合的規則，那麼封包會根據基礎結構規則集合進行評估。 如果仍然沒有相符項目，則封包預設會遭到拒絕。
...
> [!Note]
> FQDN 標記目前只能使用 Azure PowerShell 和 REST 來設定。 請按一下[這裡](https://aka.ms/firewallapplicationrule)深入了解。 
