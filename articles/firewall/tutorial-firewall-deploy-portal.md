...
網路規則集合一律比應用程式規則集合優先處理。 所有的規則都會終止，因此如果在網路規則集合中找到相符項目，則不會處理下列工作階段的應用程式規則集合。
...
1. 在 [Test-FW01] 頁面的 [設定] 底下，按一下 [規則]。
2. 按一下 [新增應用程式規則集合]。
3. 在 [名稱] 中，鍵入 **App-Coll01**。
1. 在 [優先順序] 中，鍵入 **200**。
2. 在 [動作] 中，選取 [允許]。

6. 在 [規則] 底下的 [名稱] 中，鍵入 **AllowGH**。
7. 在 [來源位址] 中，鍵入 **10.0.2.0/24**。
8. 在 [通訊協定：連接埠] 中，鍵入 **http、https**。 
9. 在 [目標 FQDN] 中，鍵入 **github.com**
10. 按一下 [新增] 。

> [!NOTE]
> Azure 防火牆包含內建的規則集合，適用於依預設允許的基礎結構 FQDN。 這些 FQDN 特定於平台，且無法用於其他用途。 允許的基礎結構 FQDN 包括：
>- 儲存體平台映像存放庫 (PIR) 的計算存取權。
>- 受控磁碟狀態儲存體存取權。
>- Windows 診斷
>
> 您可以建立最後處理的「全部拒絕」應用程式規則集合，來覆寫此內建的基礎結構規則集合。 它一律會在基礎結構規則集合之前進行處理。 依預設會拒絕基礎結構規則集合中找不到的任何項目。
...
10. 在目的地位址中，鍵入 **209.244.0.3、209.244.0.4**
11. 在 [目的地連接埠] 中，鍵入 **53**。
12. 按一下 [新增] 。
...
若不再需要，可刪除 **Test-FW-RG** 資源群組，來將所有防火牆相關的資源刪除。
