...
本文中回答的問題可協助您判斷 Avere vFXT 解決方案是否適合您的需求。 文中提供 Avere vFXT 功能的基本資訊，並說明如何搭配其他 Azure 元件和外部廠商的產品使用。 
...
### <a name="is-the-avere-vfxt-a-storage-solution"></a>Avere vFXT 是儲存體解決方案嗎？

否。 Avere vFXT 是附加至儲存體環境的檔案系統**快取**，例如 EMC 或 NetApp NAS 或 Blob 容器。 VFXT 可簡化用戶端的資料要求，並快取提供的資料，長時間地大規模改善效能。 VFXT 本身不會儲存資料。 它沒有背後所儲存資料量的相關資訊。

### <a name="is-the-avere-vfxt-a-tiering-solution"></a>Avere vFXT 是階層處理解決方案嗎？

Avere vFXT 不會自動處理經常性儲存層與非經常性儲存層之間的階層資料。  

### <a name="how-do-i-know-if-an-environment-is-right-for-the-avere-vfxt"></a>如何得知環境是否適合使用 Avere vFXT？
...
* 因為檔案存取時間很長 (達數十毫秒或秒，視需求而定)，使得整體效能很慢或不一致。 終端客戶無法接受此延遲。

* 需要處理的資料位於 WAN 環境的遠端，而永久移動該資料並不切實際。 該資料可能位於不同的 Azure 區域或客戶資料中心。

* 大量用戶端都在要求資料，例如在高效能運算 (HPC) 叢集中。 龐大的並行要求數會增加延遲。

* 客戶想要在 Azure 虛擬機器中依「現況」執行目前的管線，而且需要 POSIX 型共用儲存體 (或快取) 解決方案以獲得延展性。 使用 Avere vFXT for Azure，您就不必重新架構工作管線，對 Azure Blob 儲存體進行原生呼叫。

* HPC 應用程式以 NFSv3 用戶端為基礎。 (在某些情況下，可以使用 SMB 2.1 用戶端，但效能會受到限制。)

   下圖嘗試精簡說明此問題的解答。 您的工作流程越接近右上方，Avere 快取解決方案就越可能適用於您的環境。
...
VFXT 快取解決方案的建置可處理數百、數千或成千上萬個計算核心。 如果您的幾個機器都在執行輕鬆的工作，就不適合 Avere vFXT 解決方案。

一般 Avere vFXT 客戶執行的工作負載需求約從 1,000 個 CPU 核心起。 這些環境的核心數目可達 50,000 個或以上。 隨著這些工作負載成長，會要求更多的輸送量或更多的 IOPS，而 vFXT 可以調整，因此您可以新增節點以提供支援。

### <a name="how-much-data-can-an-avere-vfxt-environment-store"></a>Avere vFXT 環境可以儲存多少資料？

Avere vFXT 是快取，並不會特別儲存資料。 而是結合使用 RAM 與 SSD 來儲存快取的資料。 資料會永久儲存在後端儲存體系統 (例如，NetApp NAS 系統或 Blob 容器)。 Avere vFXT 系統並沒有背後所儲存資料量的相關資訊；vFXT 只會快取用戶端所要求資料的子集。  

### <a name="what-regions-are-supported"></a>支援哪些區域？

自 2018 年 11 月 1 日起，除了主權區域 (中國、德國) 地區和政府區域，所有區域都支援 Avere vFXT for Azure。 請確定您想要使用的區域可支援數量龐大的計算核心，以及建立 Avere vFXT 叢集所需的 VM 執行個體。 尚不支援主權區域和政府雲端。

### <a name="how-do-i-get-help-with-the-avere-vfxt"></a>如何取得 Avere vFXT 相關的協助？

專門支援小組會提供 Avere vFXT for Azure 相關的協助。 請依照[取得有關系統的協助](avere-vfxt-open-ticket.md#open-a-support-ticket-for-your-avere-vfxt)中的指示，從 Azure 入口網站開啟支援票證。 

### <a name="is-the-avere-vfxt-highly-available"></a>Avere vFXT 可用性高嗎？
...
是，客戶可使用一個以上的雲端提供者搭配 Avere vFXT 叢集。 它支援與 AWS S3 標準貯體、雲端服務標準貯體，以及 Azure Blob 容器。 
...
### <a name="what-kind-of-azure-virtual-machines-does-the-avere-vfxt-run-on"></a>Avere vFXT 可在哪種 Azure 虛擬機器上執行？  
...
### <a name="can-i-run-the-vfxt-cluster-as-a-virtual-machine-scale-set"></a>vFXT 叢集可以執行為虛擬機器擴展集嗎？

Avere vFXT 不支援虛擬機器擴展集 (VMSS) 部署。 數個內建的可用性支援機制是專為參與叢集的不可部分完成 VM 而設計。  

### <a name="can-i-run-the-vfxt-cluster-on-low-priority-vms"></a>可以在低優先順序的 VM 上執行 vFXT 叢集嗎？

否，系統要求基礎的虛擬機器擴展集。

### <a name="can-i-run-the-vfxt-cluster-in-containers"></a>可以在容器中執行 vFXT 叢集嗎？
...
否。 Avere vFXT 中所使用的高可用性模型目前不支援位於不同可用性區域中的個別 vFXT 叢集成員。
...
否，提供的系統就是叢集設備，並已在特定的虛擬機器類型上經過測試。 這項限制可協助客戶，避免其建立的系統不支援一般 Avere vFXT 工作流程的高效能需求。 

## <a name="technical---disks"></a>技術 - 磁碟

### <a name="what-type-of-disks-are-supported-for-the-azure-vms"></a>Azure VM 支援哪種磁碟？
...
Avere vFXT for Azure 目前不支援本機 SSD。 用於 Avere vFXT 的磁碟必須要能關機和重新啟動，但此組態中的本機連接 SSD 只能終止。
...
不支援卸離後重新連接 SSD。 在切換使用之間，來源上的中繼資料或檔案內容可能已經變更，這樣做會造成資料完整性問題。

### <a name="does-the-system-encrypt-the-cache"></a>系統會將快取加密嗎？

資料會等量分散在磁碟，但不會加密。 不過，您可以將磁碟本身加密。 如需詳細資訊，請參閱 [這裡](https://docs.microsoft.com/azure/virtual-machines/linux/security-policy#encryption)。

## <a name="technical---networking"></a>技術 - 網路

### <a name="what-network-is-recommended"></a>建議使用哪種網路？

如果使用內部部署儲存體搭配 Avere vFXT，您使用的網路連線應為 1 Gbps 或更快。 如果您有少量的資料，並願意在執行工作之前，將資料複製到雲端，VPN 連線必須要夠快。 

> [!TIP] 
> 網路連結越慢，初始的冷讀取速度會愈慢。 讀取速度緩慢會增加工作管線的延遲情況。 

### <a name="can-i-run-the-avere-vfxt-in-a-different-virtual-network-than-my-compute-cluster"></a>可以在我的計算叢集以外的虛擬網路中執行 Avere vFXT 嗎？

是，您可以在不同的虛擬網路中建立 Avere vFXT 系統。 如需詳細資訊，請閱讀[規劃您的 Avere vFXT 系統](avere-vfxt-deploy-plan.md)。

### <a name="does-the-avere-vfxt-require-its-own-subnet"></a>Avere vFXT 需要自己的子網路嗎？

是。 Avere vFXT 必須執行為 HA 叢集，並且需要多個 IP 位址才能運作。 如果叢集位於自己的子網路中，您要避免 IP 位址發生衝突，這可能會造成安裝和一般操作發生問題。 只要 IP 位址沒有重疊，叢集的子網路可以位在現有的 VNet 內。

### <a name="can-i-run-the-avere-vfxt-on-infiniband"></a>可以在 Infiniband 上執行 Avere vFXT 嗎？

否，Avere vFXT 只能使用 Ethernet/IP。

### <a name="how-do-i-access-my-on-premises-nas-environment-from-the-avere-vfxt"></a>如何從 Avere vFXT 存取我的內部部署 NAS 環境呢？

Avere vFXT 環境和任何其他的 Azure VM 並無不同，它需要透過網路閘道或 VPN 路由存取客戶資料中心 (反之亦然)。 如果 Azure ExpressRoute 連線可用於您的環境，請考慮使用。

### <a name="what-are-the-bandwidth-requirements-for-the-avere-vfxt"></a>Avere vFXT 有哪些頻寬需求？
...
### <a name="can-i-run-the-vfxt-with-public-ip-addresses"></a>可以使用公用 IP 位址執行 vFXT 嗎？

否，vFXT 是為了在採用最佳做法保護安全的網路環境內運作。  

## <a name="technical---backend-storage-core-filers"></a>技術 - 後端儲存體 (核心檔案管理工具)
...
### <a name="how-does-the-avere-environment-store-data"></a>Avere 環境如何儲存資料？

Avere vFXT 不是儲存體。 它是快取，會從稱為核心檔案管理工具的多個儲存體目標讀取和寫入資料。 Avere vFXT 的進階 SSD 磁碟上儲存的資料是暫時性的，最後都會排清到後端核心檔案管理工具儲存體。
...
* Azure Blob 容器 (僅限 LRS) 
* AWS S3 貯體 
* Google 雲端貯體

### <a name="why-doesnt-the-avere-vfxt-support-all-nfs-filers"></a>Avere vFXT 為什麼不支援所有 NFS 檔案管理工具？
...
您可以在 Azure (慣用) 或 vFXT 核心檔案管理工具層級中設定 Blob 加密。  

### <a name="can-i-use-my-own-encryption-key-for-a-blob-core-filer"></a>可以對 Blob 核心檔案管理工具使用我自己的加密金鑰嗎？

根據預設，資料是使用適用於 Azure Blob、資料表、檔案及佇列的 Microsoft 受控金鑰來加密。 您可以使用自己的金鑰來加密 Azure Blob 和檔案。 如果您選擇使用 vFXT 加密，就必須使用 Avere 產生的金鑰並將它儲存在本機。 

## <a name="purchasing"></a>購買

### <a name="how-do-i-get-avere-vfxt-for-azure-licensing"></a>如何取得 Avere vFXT for Azure 授權？

透過 Azure Marketplace 即可輕鬆取得 Avere vFXT for Azure 授權。 註冊 Azure 帳戶，然後依照[部署 vFXT 叢集](avere-vfxt-deploy.md)中的指示來建立 Avere vFXT 叢集。 

### <a name="how-much-does-the-avere-vfxt-cost"></a>Avere vFXT 的費用是多少？
...
若要開始使用 Avere vFXT for Azure，請閱讀以下連結，了解如何規劃和部署自己的系統：
