...
VFXT 叢集啟動並執行之後，您會想要知道如何將用戶端連接到該叢集，並（選擇性）如何將您的資料移至新的 Blob 儲存體容器。 如果您使用 NAS 儲存系統，您需要在建立叢集之後新增它。
...
1. 建立 Avere vFXT 叢集
   ...
   * 建立叢集*控制器*  
     ...
     如果您在部署期間建立新的虛擬網路或子網，您的控制器將會有公用 IP 位址。 這表示控制器可以做為從子網外部連接到叢集的跳躍主機。
