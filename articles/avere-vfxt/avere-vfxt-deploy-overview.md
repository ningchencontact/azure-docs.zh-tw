...
1. 建立叢集控制器
   ...
   「叢集控制器」是一部簡單 VM，它位於與 Avere vFXT 叢集相同的虛擬網路。 控制器會建立 vFXT 節點並構成叢集，而且它也提供命令列介面讓您在叢集生命週期內管理叢集。

   若您使用公用 IP 位址來設定您的控制器，它也能當成跳板主機以便從虛擬網路外部連線到叢集。

   建立 vFXT 叢集及管理其節點所需的所有軟體都已預先安裝在叢集控制器上。

   閱讀[建立叢集控制器 VM](avere-vfxt-deploy.md#create-the-cluster-controller-vm)以取得詳細資料。

1. 為叢集節點建立執行階段角色 

   Azure 使用[角色型存取控制](https://docs.microsoft.com/azure/role-based-access-control/) (RBAC) 來授權叢集節點 VM 執行特定工作。 例如，叢集節點必須能指派或重新指派 IP 位址給其他叢集節點。 在建立叢集之前，您必須定義能為它們授與適當權限的角色。

   叢集控制器預先安裝的軟體包括可供您自訂的原型角色。 閱讀[建立叢集節點存取角色](avere-vfxt-deploy.md#create-the-cluster-node-access-role)以取得指示。

1. 建立 Avere vFXT 叢集 

   在控制器上，編輯適當的叢集建立指令碼並執行它，以建立叢集。 [編輯部署指令碼](avere-vfxt-deploy.md#edit-the-deployment-script)提供詳細指示。 
