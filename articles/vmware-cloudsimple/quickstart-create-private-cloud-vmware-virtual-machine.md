...
1. 取得您想要上傳至 vCenter 的 ISO 或 vSphere 範本, 以建立虛擬機器, 並在您的本機系統上使用它。
...
4. 流覽至所建立的新資料夾, 然後按一下 **[上傳**檔案]。 依照畫面上的指示上傳 ISO。
...
3. 選取 [**建立新的虛擬機器**], 然後按 **[下一步]** 。
    ...
    ![新增虛擬機器嚮導](media/create-vcenter-virtual-machine-02.png)

4. 將電腦命名為, 選取**工作負載 VM 的**資料夾, 然後按 **[下一步]** 。

    ![選取名稱和資料夾](media/create-vcenter-virtual-machine-03.png)

5. 選取**工作負載**計算資源, 然後按 **[下一步]** 。

    ![選取計算資源](media/create-vcenter-virtual-machine-04.png)
...
7. 保留預設的 [ESXi 6.5 相容性] 選項, 然後按 **[下一步]** 。
    ...
    ![選取相容性](media/create-vcenter-virtual-machine-06.png)

8. 為虛擬機器選取 ISO 的 [客體作業系統], 然後按 **[下一步]** 。

    ![自訂客體作業系統](media/create-vcenter-virtual-machine-07.png)

9. 選取 [硬碟和網路選項]。 針對新的 CD/DVD 光碟機, 請選取 [資料存放區**ISO**檔案]。  如果您想要允許從公用 IP 位址到此虛擬機器的流量, 請選取 [網路] 作為 [ **vm-1**]。
...
11. 檢查設定, 然後按一下 **[確定]** 以建立虛擬機器。

    ![審查選項](media/create-vcenter-virtual-machine-11.png)

虛擬機器現在已新增至工作負載計算資源, 並可供使用。 

![VCenter 中的新虛擬機器](media/create-vcenter-virtual-machine-12.png)

基本設定現已完成。 您可以開始使用您的私用雲端, 類似于您使用內部部署虛擬機器基礎結構的方式。

接下來的章節包含有關設定私人雲端工作負載的 DNS 和 DHCP 伺服器, 以及修改預設網路設定的選擇性資訊。

## <a name="add-users-and-identity-sources-to-vcenter-optional"></a>將使用者和身分識別來源新增至 vCenter (選擇性)

CloudSimple 會指派具有使用者名稱 **cloudowner@cloudsimple.local** 的預設 vCenter 使用者帳戶。 您不需要進行其他帳戶設定即可開始使用。  CloudSimple 通常會為系統管理員指派執行正常作業所需的許可權。  將您的內部部署 active directory 或 Azure AD 設定為私人雲端上的[其他身分識別來源](https://docs.azure.cloudsimple.com/set-vcenter-identity/)。
...
這些 Microsoft 文章說明如何將 Windows server 設定為 DNS 伺服器和 DHCP 伺服器。
<br>
...
CloudSimple 入口網站中的 [網路] 頁面可讓您指定防火牆資料表的設定, 以及虛擬機器的公用 IP 位址。
...
必須將此 IP 位址對應到的虛擬機器, 才能使用上述指定的本機位址進行設定。 設定 IP 位址的程式是虛擬機器作業系統特有的。 如需正確的程式, 請參閱虛擬機器作業系統的檔。

#### <a name="example"></a>範例

例如, 以下是 Ubuntu 16.04 的詳細資料。

將靜態方法新增至檔案中```/etc/network/interfaces```的 inet 位址系列設定。 變更位址、網路遮罩和閘道的值。 在此範例中, 我們使用 eth0 介面、內部 IP 位址192.168.24.10、閘道位址192.168.24.1 和網路遮罩255.255.255.0。 
...
根據預設, 所有來自網際網路的連入流量都會**遭到拒絕**。 如果您想要開啟任何其他埠, 請建立[防火牆資料表](https://docs.azure.cloudsimple.com/firewall/)。

將內部 IP 位址設定為靜態 IP 位址之後, 請確認您可以從虛擬機器內部連線到網際網路。

```
ping 8.8.8.8
```

確認您可以使用公用 IP 位址從網際網路連線到虛擬機器。

請確認虛擬機器上的任何防火牆 (iptable) 規則都未封鎖埠80輸入。
...
在您的桌面上啟動瀏覽器, 並將它指向公用 IP 位址的埠 80, 以流覽虛擬機器上的檔案。 
...
您也可以使用防火牆規則功能來修改流量的保護方式。 如需詳細資訊, 請參閱[設定防火牆資料表和規則](https://docs.azure.cloudsimple.com/firewall/)。
...
若要安裝方案, 您必須在有限期間內要求額外的許可權。 請參閱[提升許可權](https://docs.azure.cloudsimple.com/escalate-private-cloud-privileges)。
...
* [在 CloudSimple 網路上設定 VPN 閘道](https://docs.azure.cloudsimple.com/vpn-gateway)
