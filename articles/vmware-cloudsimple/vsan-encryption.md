...
ms.locfileid: "69544470"
---
# <a name="configure-vsan-encryption-for-your-cloudsimple-private-cloud"></a>為您的 CloudSimple 私人雲端設定 vSAN 加密

您可以設定 vSAN 軟體加密功能, 讓您的 CloudSimple 私人雲端可以與在 Azure 虛擬網路中執行的金鑰管理伺服器搭配運作。

使用 vSAN 加密時, VMware 需要使用外部 KMIP 1.1 相容的協力廠商金鑰管理伺服器 (KMS) 工具。 您可以使用任何受 VMware 認證且適用于 Azure 的支援 KMS。

以下說明如何使用在 Azure 虛擬網路中執行的 HyTrust KeyControl KMS。 適用于 vSAN 的任何其他認證的協力廠商 KMS 解決方案都可以使用類似的方法。
...
![Azure 虛擬網路中的 KMS 叢集](media/vsan-kms-cluster.png)

## <a name="how-to-deploy-the-solution"></a>如何部署解決方案

部署程式具有下列步驟:

1. [確認符合必要條件](#verify-that-prerequisites-are-met)
...
### <a name="verify-that-prerequisites-are-met"></a>確認符合必要條件
...
若要繼續進行設定, 您需要 ExpressRoute 的授權金鑰和對等線路 URI, 再加上 Azure 訂用帳戶的存取權。 這項資訊可在 CloudSimple 入口網站的 [虛擬網路連線] 頁面上取得。 如需指示, 請參閱[設定與私人雲端的虛擬網路](virtual-network-connection.md)連線。 如果您在取得資訊時遇到任何問題, 請開啟<a href="https://portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/newsupportrequest" target="_blank">支援要求</a>。
...
若要在虛擬網路的 Azure Resource Manager 中部署 HyTrust KeyControl 叢集, 請執行下列工作。 如需詳細資訊, 請參閱<a href="https://docs.hytrust.com/DataControl/Admin_Guide-4.0/Default.htm#OLH-Files/Azure.htm%3FTocPath%3DHyTrust%2520DataControl%2520and%2520Microsoft%2520Azure%7C_____0" target="_blank">HyTrust 檔</a>。
...
移至 [HTTPs://] [*公用-ip*], 其中 [*公用 Ip* ] 是 KeyControl 節點 VM 的公用 ip 位址。 請遵循<a href="https://docs.hytrust.com/DataControl/Admin_Guide-4.0/Default.htm#OLH-Files/Azure.htm%3FTocPath%3DHyTrust%2520DataControl%2520and%2520Microsoft%2520Azure%7C_____0" target="_blank">HyTrust 檔</a>中的下列步驟。

1. <a href="https://docs.hytrust.com/DataControl/4.2/Admin_Guide-4.2/index.htm#Books/VMware-vSphere-VSAN-Encryption/configuring-kmip-server.htm%3FTocPath%3DHyTrust%2520KeyControl%2520with%2520VSAN%25C2%25A0and%2520VMware%2520vSphere%2520VM%2520Encryption%7C_____2" target="_blank">設定 KMIP 伺服器</a>
2. <a href="https://docs.hytrust.com/DataControl/4.2/Admin_Guide-4.2/index.htm#Books/VMware-vSphere-VSAN-Encryption/creating-user-for-vmcrypt.htm%3FTocPath%3DHyTrust%2520KeyControl%2520with%2520VSAN%25C2%25A0and%2520VMware%2520vSphere%2520VM%2520Encryption%7C_____3" target="_blank">建立 VMware 加密的憑證配套</a>
...
遵循 HyTrust 指示,<a href="https://docs.hytrust.com/DataControl/4.2/Admin_Guide-4.2/index.htm#Books/VMware-vSphere-VSAN-Encryption/creating-KMS-Cluster.htm%3FTocPath%3DHyTrust%2520KeyControl%2520with%2520VSAN%25C2%25A0and%2520VMware%2520vSphere%2520VM%2520Encryption%7C_____4" target="_blank">在 vCenter 中建立 KMS</a>叢集。
