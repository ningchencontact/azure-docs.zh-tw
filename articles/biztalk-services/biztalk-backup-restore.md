...
* 如果批次中有作用中的訊息，請在執行備份前 **先** 處理批次。 無論是建立隨選備份或排定備份，都不會儲存批次中的訊息。 
    ...
    **如果在使用備份與作用中批次的訊息，則不會備份這些訊息，而且會遺失。**
* 選用：在 BizTalk 服務入口網站中，會停止任何管理作業。

## <a name="create-a-backup"></a>建立備份
您隨時都可以建立備份，完全決由掌控。 若要建立備份，請使用[用於管理 Azure 上之 BizTalk 服務的 REST API](/previous-versions/azure/reference/dn232347(v=azure.100))。

## <a name="restore"></a>Restore
若要還原備份，請使用[用於管理 Azure 上之 BizTalk 服務的 REST API](/previous-versions/azure/reference/dn232347(v=azure.100))。
