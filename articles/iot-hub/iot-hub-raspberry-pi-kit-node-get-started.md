...
1. 下載 Raspbian。
   ...
   a. [下載 Raspbian Stretch](http://downloads.raspberrypi.org/raspbian/images/raspbian-2017-07-05/) (.zip 檔案)。
...
1. 使用下列其中一個 SSH 用戶端，從您的主機電腦連接到 Raspberry Pi：
   ...
   **Windows 使用者**
   ...
   a. 下載並安裝適用於 Windows 的 [PuTTY](http://www.putty.org/)。 
...
您已執行範例應用程式收集感應器資料並傳送至 IoT 中樞。 若要查看 Raspberry Pi 傳送至 IoT 中樞的訊息，或要傳送訊息至 Raspberry Pi，請參閱[使用適用於 Visual Studio Code 的 Azure IoT 中樞工具組擴充功能來傳送和接收裝置與 IoT 中樞之間的訊息](iot-hub-vscode-iot-toolkit-cloud-device-messaging.md)。
