...
* **使用階段式推出來執行組態變更：** 階段式推出是一個整體程序，操作員可藉以將變更部署至更大範圍的 IoT 裝置集合。 目標是逐漸進行變更，以降低進行大規模重大變更的風險。  操作員應使用解決方案的介面來建立[自動裝置](iot-hub-auto-device-config.md)設定，且目標條件應以初始的裝置集合（例如「未配置的」群組）為目標。 接著，操作員應驗證初始裝置集合中的組態變更。
