...
3. 執行**npm install-S azure-iothub\@模組-preview** ，將服務 SDK 安裝在**node_modules**子目錄中。
...
2. 與您在上述步驟中所做的一樣，請為您的裝置程式碼建立一個目錄，然後使用 NPM 將它初始化並安裝裝置 SDK （**NPM install-S amqp\@模組-preview**）。
