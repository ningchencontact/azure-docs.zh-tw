...
| 裝置到雲端傳送 | 較高的100傳送作業/秒或12個傳送作業/秒/單位 <br/> 例如，兩個 S1 單位是 2 @ no__t-012 = 24/秒，但是您在單位上至少有100個傳送作業/秒。 有九個 S1 單位，您可以在整個單位中有108個傳送作業/秒（9 @ no__t-012）。 | 120傳送作業/秒/單位 | 6000傳送作業/秒/單位 |
| 雲端到裝置的傳送<sup>1</sup> | 1.67 傳送作業/秒/單位（100個訊息/分鐘/單位） | 1.67 傳送作業/秒/單位（100傳送作業/分鐘/單位） | 83.33 傳送作業/秒/單位（5000傳送作業/分鐘/單位） |
| 雲端到裝置的接收<sup>1</sup> <br/> (僅限裝置使用 HTTPS 時)| 16.67 接收作業/秒/單位（1000接收作業/分鐘/單位） | 16.67 接收作業/秒/單位（1000接收作業/分鐘/單位） | 833.33 接收作業/秒/單位（50000接收作業/分鐘/單位） |
...
例如，您可以使用模擬裝置，每秒將200的裝置到雲端訊息傳送到您的 S1 IoT 中樞（其限制為每秒 D2C 個）。 在第一分鐘或兩個中，訊息會立即處理。 不過，由於裝置會繼續傳送比節流限制更多的訊息，IoT 中樞一開始只會處理每秒100個訊息，並將其餘部分放在佇列中。 您開始注意到增加的延遲。 最後，您會在佇列填滿時開始取得 `429 ThrottlingException`，而[IoT 中樞計量](iot-hub-metrics.md)中的「節流錯誤數目」開始增加。
