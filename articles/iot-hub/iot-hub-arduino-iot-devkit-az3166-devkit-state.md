...
- 依照[快速入門指南](https://docs.microsoft.com/en-us/azure/iot-hub/iot-hub-arduino-iot-devkit-az3166-get-started)來設定開發環境。
...
1. 輸入您稍早記下的函式應用程式名稱。
2. 按一下 [連線] 按鈕。
3. 在幾秒內，頁面就會重新整理並顯示 DevKit 的 WiFi 連線狀態和每個內建感應器的狀態。
...
您已了解如何︰
- 將 MXChip IoT DevKit 裝置連接到您的 Azure IoT 套件。
