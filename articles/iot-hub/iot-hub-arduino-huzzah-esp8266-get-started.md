...
1. 輸入 `F1`，然後輸入 **Arduino** 並選取 [Arduino: 面板設定]。
1. 按一下 [選取的面板:] 的方塊並輸入 **esp8266**，然後選取 [Adafruit HUZZAH ESP8266 (esp8266)]。
...
1. 輸入 `F1`，然後輸入 **Arduino** 並選取 [Arduino: 上傳]，以建置範例應用程式並將其部署至 Feather HUZZAH ESP8266。
