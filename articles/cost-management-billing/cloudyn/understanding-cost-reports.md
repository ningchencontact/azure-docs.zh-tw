...
若要將報告儲存至雲端服務提供者，您必須已經設定儲存體帳戶。 檢視任何報告時，請按一下 [動作]，然後選取 [排程報告]。 為報告命名，然後新增自己的 URL 或使用自動建立的 URL。 選取 [儲存至儲存體]，然後選取儲存體帳戶或新增儲存體帳戶。 輸入要附加至報告檔案名稱的前置詞。 選取 CSV 或 JSON 檔案格式，然後儲存報表。
...
- 在 [使用 Cloudyn 報告](../../cost-management/use-reports.md) 中了解 Cloudyn 所含的報告。
- 了解如何使用報告建立 [儀表板](../../cost-management/dashboards.md)。
