...
完成時，您的預算會顯示在 **[成本] &gt; [** 成本**分析**] 和 [隨時間變化的成本與預算] 報告中的最成本報表。 您也可以使用 [動作]，根據預算閾值來排程報表。
...
![成本與預算一段時間報告中所顯示的累計成本和總預算範例](./media/manage-budgets/accumulated-budget.png)
...
在 [隨時間變化的成本與預算] 報告中，按一下 [**動作**]，然後選取 [**排程報告**]。 在 [閾值] 索引標籤中，選取閾值計量。 例如，[成本百分比與預算]。 選取警示類型，並輸入預算的百分比值。 如果您只想要收到一次通知，選取 [連續警示的數目]，然後輸入 _1_。 按一下 [檔案]。
