...
**開始日期在2018年5月1日前的直接註冊**：直接企業 AZURE （EA）客戶已設定為 Azure 服務的年度計費週期（不包括 Marketplace 服務）。 您的計費週期會以週年日為基礎。 週年日是指您的合約開始生效的日期。 您必須超過承諾用量金額 ('MC') 閾值的 150%，才會收到第一張 Azure 服務超額發票。  當服務使用量總計超過 MC 閾值的 150% 時，即會自動根據週年日將您轉換為每季計費週期。  若未超過 MC 閾值的 150%，註冊將會繼續採用年度計費週期，且您將在承諾用量年度結束時收到超額發票。

在**2018 年5月1日之後直接註冊開始日期**：客戶的 Azure 耗用量和按月計費的費用，將會依月份計費週期計算。  Azure 承諾用量金額未涵蓋的任何費用，都將計為超額款項。  

**具有在 2018 5 月1日之前啟動之註冊的間接**註冊：間接企業 AZURE （EA）客戶已設定為每季計費週期。  通路合作夥伴 (CP) 將會直接向客戶開立發票。  

**開始日期為2018年5月1日之後的間接註冊**：任何開始日期大於或等於5月 1 2018 日的間接合約都會按月計費。  
...
自 2019 年 4 月的計費週期起，客戶將開始收到單一 Azure 發票，因為我們已將所有 Azure 和 Azure Marketplace 費用整合為單一發票，而不是兩張個別的發票。 （這項變更不會影響澳大利亞、日本或新加坡的客戶）。在轉換成合併發票期間，您會收到部分 Marketplace 發票。 這張最後的個別發票會顯示您的計費更新日期之前的 Marketplace 費用。 該日期之後的 Marketplace 費用將出現在您的 Azure 發票上。 在轉換期間之後，您將在合併發票上看到所有的 Azure 和 Marketplace 費用。  
...
1. 在這一行中，您會看到套用至註冊的信用額度，以及簡短說明，例如：服務等級協定點數。
...
Excel 會將所有欄位匯入為「一般」文字，並假設某個數位以數學標準表示： "1000.00"。  因此，以句點 (.) 作為千位分隔符號，並逗點作為小數點分隔符號 (,) 的歐洲貨幣 – "1.000,00"，將會不正確地顯示。 此問題可能會隨著您的區域語言設定而不同。
...
- **基於使用方式：** 以交易單位測量的產品。  例如，虛擬機器會按小時計費，Bing API 搜尋會依搜尋次數計費。
- **每月費用：** 根據使用量/存取，按月計費。
...
新的計量方式已在 2016 年 1 月 27 日到 1 月 28 日之間生效。 在此轉換期間，您部署的 .csv 下載中會注意到兩個明細專案：一個用於 A1 計量上所發出的使用量，另一個用於在新的專用計量上發出，並對應至您部署的大小。
...
- **基於使用方式：** 產品是以交易單位來測量。  例如，虛擬機器會按小時計費，而 Bing API 搜尋會依搜尋次數計費。
- 以**非使用方式為基礎：** 一次性費用或週期性的每月費用，與使用量無關。
...
- **一般用途**：在供應專案期間，現有合格使用者可以使用「Microsoft Azure 企業版」內容套件試用版供應專案（以下稱為「供應專案」）的延伸 Power BI Pro，讓他們能夠透過使用特定 Power BI 內容套件，存取與其 Microsoft Azure 耗用量相關的特定見解。
- **資格**：如果使用者有與組織 Microsoft Azure 帳單、服務、服務及/或成本管理相關的功能，則 ENTERPRISE 合約（EA）下的使用者可能會參與該供應專案。
- **排除項目**：
  ...
  - 詞彙：供應專案將于2017年6月1日開始，並于2018年5月31日結束。  使用者可在這 12 個月內隨時接受供應項目，但所有使用者的供應項目皆於 2018 年 5 月 31 日終止，無論其接受供應項目的時間點為何。
...
資源標籤可從 Azure 入口網站管理。 您可以經由 [https://portal.azure.com](https://portal.azure.com) 聯絡 Azure 訂用帳戶小組。 請依照[此連結](https://docs.microsoft.com/azure/azure-portal/supportability/how-to-create-azure-support-request)上的步驟提出支援要求。
...
**原始單位總計：** 詳細使用量報告中的已使用數量。
**每個單位的 MOCP 資源：** 上游使用量系統會以不同的單位來發出每個服務的使用方式。 預設**每單位耗用量：** EA 測量單位。 預設**價格：** Azure EA 入口網站的單價。
**總成本：** 從詳細的使用量報告或承諾用量使用量，到 Azure EA 入口網站的超額部分，延長成本。
