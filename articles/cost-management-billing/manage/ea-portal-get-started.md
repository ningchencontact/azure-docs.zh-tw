...
若要管理註冊中的 Azure 服務，有五個不同的企業系統管理使用者角色：
...
每個帳戶都需要唯一的公司、學校或 Microsoft 帳戶。 如需有關 Azure EA 入口網站系統管理角色的詳細資訊，請參閱[了解 Azure 中的 Azure Enterprise 合約系統管理角色](understand-ea-roles.md)。
...
如果您已設定為 EA 系統管理員，則不需要收到啟用電子郵件，即可登入 Azure EA 入口網站。 您可以繼續[https://ea.azure.com](https://ea.azure.com) ，並使用您的電子郵件地址（公司、學校或 live ID）和密碼登入。
...
關聯 Azure in Open 訂用帳戶與 EA 將會喪失所有尚未使用的 Azure in Open 信用額度。 為避免喪失信用額度，建議客戶先將 Azure in Open 訂用帳戶中的信用額度用盡，然後再將帳戶加入其 EA 之中。  
...
下載使用量詳細資料報表。 按一下 [**報告**]，然後按一下 [**下載使用量**] 索引標籤。在報表清單中，針對想要取得的每月報表按一下 [**下載**]。
...
1. 在 Azure EA 入口網站的左側導覽區域中按一下 [報表]，然後檢視 [使用量摘要] 索引標籤。  
  ![建立和檢視使用量摘要及下載報告](./media/ea-portal-get-started/create-ea-view-usage-summary-and-download-reports.png)
2. 選取承諾期間。
...
- **帳戶**： Azure EA 入口網站上的組織單位，用來管理訂用帳戶並使用於報表。
- **帳戶擁有**者：在 Microsoft Azure 上識別用來管理訂閱和服務系統管理員的人員。 他們可以檢視此帳戶及其相關聯訂用帳戶的使用量資料。
- **修訂訂**用帳戶：註冊修訂下的單一一年或同限期訂閱。
- **承諾**用量：根據此預付金額的使用量折扣承諾費率，Microsoft Azure 服務的年度預付金。
- **部門系統管理員**：識別用來管理部門、建立新帳戶和帳戶擁有者、查看其管理之部門的使用量詳細資料，並在授與許可權時查看成本的人員。
- **註冊編號**： Microsoft 提供的唯一識別碼，用來識別與 enterprise 合約相關聯的特定註冊。
- **企業系統管理員**：識別用來管理 Microsoft Azure 上部門和部門擁有者和帳戶和帳戶擁有者的人員。 他們能夠管理企業系統管理員，以及針對與 Enterprise 註冊相關聯的所有帳戶和訂用帳戶，檢視使用量資料、已計費數量和未計費費用。
- **Enterprise 合約**： microsoft 授權合約，適用于有集中購買的客戶，想要標準化其整個組織的 microsoft 技術，並在標準的 microsoft 軟體上維護資訊技術基礎結構。
- **Enterprise 合約註冊**： enterprise 合約方案中的註冊，以折扣費率提供大量的 Microsoft 產品。
- **Microsoft 帳戶**：一種 Web 服務，可讓參與的網站以一組認證來驗證使用者。
- **Microsoft Azure enterprise 註冊修訂（註冊修訂）** ：由企業簽署的修訂，可讓他們在企業註冊時存取 Microsoft Azure。
- **Azure EA 入口網站**：企業客戶用來管理其 Microsoft Azure 帳戶及其相關訂閱的入口網站。
- **耗用的資源數量**：一個月內使用的個別 Microsoft Azure 服務數量。
- **服務管理員**：識別用來存取和管理 Azure EA 入口網站上訂閱和開發專案的人員。
- **訂**用帳戶：代表 Azure EA 入口網站訂用帳戶，而且是由相同服務管理員所管理 Microsoft Azure 服務的容器。
- **工作或學校帳戶**：針對已設定 active directory 與雲端同盟，且所有帳戶都在單一租使用者上的組織。

### <a name="enrollment-statuses"></a>註冊狀態：

- **待決**：註冊系統管理員必須登入 Azure EA 入口網站。 登入之後，註冊將會切換為 [作用中] 狀態。
- 作用**中：註冊**為使用中，而且可以在 Azure EA 入口網站中建立帳戶和訂用帳戶。 註冊的作用中狀態會維持到 Enterprise 合約結束日期為止。
- 無限制的**延伸詞彙**：經過 enterprise 期間結束日期之後，不會有無限期的擴充詞彙。 其可讓選擇延長期限的 EA 客戶在 Enterprise 合約結束時繼續無限期地使用 Azure。 在 EA 註冊達到 Enterprise 合約結束日期之前，註冊系統管理員應決定是否要更新註冊，方法是新增額外的預付金、轉移到新的註冊、遷移至 Microsoft 線上訂閱方案 (MOSP)，或確認停用所有與註冊相關聯的服務。
- 已**過期**： ea 客戶已選擇不延長的期限，且 ea 註冊已達到 enterprise 期間結束日期、註冊將到期，且所有相關聯的服務都將停用。
- 已**轉移**：所有相關聯帳戶和服務都已轉移到新註冊的註冊，將會顯示為已傳輸狀態。 請注意，如果在續約時產生新的註冊號碼，則註冊不會自動轉移。 先前的註冊號碼必須包含在客戶的續約文件中，以利自動轉移。
...
可以。 請務必注意，您身為帳戶擁有者的所有 Azure 訂用帳戶都會轉換成您的 Enterprise 合約。 這包括使用每月點數 (例如 Visual Studio、AzurePass、MPN、BizSpark 等) 的訂用帳戶，這表示您這麼做就會失去每月點數。
...
可以。 使用 Power BI 的 Microsoft Azure 企業版內容套件，您可以快速匯入和分析 Enterprise 註冊的 Azure 使用量，了解哪個部門、帳戶或訂閱使用量最多、您的組織使用最多的服務，或是追蹤消費和使用趨勢。
