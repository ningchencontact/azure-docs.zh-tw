...
1. 將兩個額外的資源新增至範本。
    ...
    * 若要讓 SQL Database 擴充功能匯入 BACPAC 檔案，您必須允許來自 Azure 服務的流量。 在 SQL Server 定義底下新增下列防火牆規則定義：
...
在 Azure 入口網站中，請從新部署的資源群組中選取 SQL 資料庫。 選取 [查詢編輯器 (預覽)]  ，然後輸入系統管理員認證。 您應該會看到兩個已匯入資料庫中的資料表。
