...
`resourceGroup()` 函式不能用於[部署在訂用帳戶層級](deploy-to-subscription.md)中的範本， 只能用於部署到資源群組中的範本。 即使父範本已部署至訂用帳戶，您也可以在以資源群組為目標的[連結或嵌套範本（具有內部範圍）](linked-templates.md)中使用 `resourceGroup()` 函數。 在該案例中，連結或嵌套的範本會部署在資源群組層級。 如需有關在訂用帳戶層級部署中將資源群組設為目標的詳細資訊，請參閱[將 Azure 資源部署至多個訂用帳戶或資源群組](cross-resource-group-deployment.md)。
