...
* 您已設定 Azure 訂用帳戶的[服務](/azure/devops/pipelines/library/connect-to-azure?view=azure-devops)連線。 管線中的工作會在服務主體的身分識別下執行。 如需建立連線的步驟，請參閱[建立 DevOps 專案](template-tutorial-use-azure-pipelines.md#create-a-devops-project)。

* 您有一個從**Azure 資源群組**入門範本建立的 Visual Studio 專案。 如需建立該類型專案的詳細資訊，請參閱[透過 Visual Studio 建立和部署 Azure 資源群組](create-visual-studio-deployment-project.md)。
...
1. 如果您先前尚未新增管線，您必須建立新的管線。 從您的 Azure DevOps 組織，選取 [**管線**] 和 [**新增管線**]。
   ...
   ![加入新的管線](./media/add-template-to-azure-pipelines/new-pipeline.png)
...
當您將工作設定為 [`AzurePowerShell@3`] 時，管線會使用 AzureRM 模組中的命令來驗證連接。 根據預設，Visual Studio 專案中的 PowerShell 腳本會使用 AzureRM 模組。 如果您已將腳本更新為使用[Az 模組](/powershell/azure/new-azureps-module-az)，請將工作設定為 [`AzurePowerShell@4`]。
...
下列 YAML 會顯示[Azure Resource Manager 範本部署](https://github.com/microsoft/azure-pipelines-tasks/blob/master/Tasks/AzureResourceManagerTemplateDeploymentV3/README.md)工作：
...
- `deploymentScope`：從選項中選取部署範圍： [`Management Group`]、[`Subscription`] 和 [`Resource Group`]。 在本逐步解說中使用**資源群組**。 若要深入了解範圍，請參閱[部署範圍](deploy-rest.md#deployment-scope)。
...
部署工作會連結至名為 `WebSite.json` 的範本，以及名為網站. parameters 的參數檔案。 使用範本和參數檔案的名稱。
...
如需有關使用 Azure Pipelines 搭配 Resource Manager 範本的逐步程式，請參閱[教學課程：使用 Azure Pipelines 持續整合 Azure Resource Manager 範本](template-tutorial-use-azure-pipelines.md)。
