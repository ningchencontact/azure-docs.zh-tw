...
此函式可以接受任意數目的引數，並且可針對參數接受字串或陣列。 不過，您無法同時為參數提供陣列和字串。 字串只會與其他字串串連。
...
| utcShortOutput | string | 2019 年 3 月 5 日 |
