...
在您的範本內，dependsOn 元素可讓您定義一個資源作為一或多個資源的相依項目。 其值是以逗號分隔的資源名稱清單。 此清單可以包含有條件地[部署](conditional-resource-deployment.md)的資源。 當條件式資源未部署時，Azure Resource Manager 會自動將其從所需的相依性移除。
