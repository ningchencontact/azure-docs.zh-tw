...
1. 在 **parameters** 區段的結尾新增兩個參數，以設定儲存體帳戶金鑰和 BACPAC URL：
    ...
    在 **adminPassword** 之後新增逗號。 若要從 VS Code 將 JSON 檔案格式化，請按 **[SHIFT]+[ALT]+F**。

    請參閱[準備 BACPAC 檔案](#prepare-a-bacpac-file)以了解如何取得這兩個值。
...
1. 將兩個額外的資源新增至範本。
    ...
    * 使用下列 JSON，將 SQL Database 擴充功能資源新增至資料庫定義：
        ...
        * **storageKeyType**：指定要使用的儲存體金鑰類型。 其值可以是 `StorageAccessKey` 或 `SharedAccessKey`。 在本教學課程中使用 `StorageAccessKey`。
...
請考慮使用您在準備 bacpac 檔案時所用的相同專案名稱，讓所有資源都儲存在相同的資源群組中。  這麼做比較容易管理資源，例如清除資源。 如果您使用相同的專案名稱，則可從指令碼中移除 **New-AzResourceGroup** 命令，或在系統詢問您是否要更新現有資源群組時回答 y 或 n。

使用所產生的密碼。 請參閱[必要條件](#prerequisites)。

## <a name="verify-the-deployment"></a>驗證部署

若要從您的用戶端電腦存取 SQL Server，您需要新增額外的防火牆規則。 如需詳細資訊，請參閱[建立和管理 IP 防火牆規則](../sql-database/sql-database-firewall-configure.md#create-and-manage-ip-firewall-rules)。
