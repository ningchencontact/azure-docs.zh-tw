...
2. 您具有管理群組上的寫入權限，以及管理群組上的「擁有者」或「參與者」角色。 若要查看您有哪些權限，請選取管理群組，然後選取 **IAM**。 若要深入了解 RBAC 角色，請參閱[使用 RBAC 管理存取權與權限](../role-based-access-control/overview.md)。  
...
5. 在清單中選取具有正確識別碼的訂用帳戶。
    ...
    ![子系](media/management-groups/add_context_sub.png)
1. 選取 [儲存]
...
3. 選取您計畫要作為父代的管理群組。
5. 在頁面頂端，選取 [新增管理群組]。
6. 在開啟的功能表中，選取是要新的管理群組還是使用現有管理群組。
    - 選取新項目即會建立新的管理群組。
