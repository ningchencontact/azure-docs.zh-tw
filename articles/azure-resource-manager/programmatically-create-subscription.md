...
* 您的帳戶必須是 Azure EA 註冊中的帳戶擁有者。 若非如此，請要求「註冊管理員」[使用 EA 入口網站將您新增為帳戶擁有者](https://ea.azure.com/helpdocs/addNewAccount) (需要登入)。 請依照您收到的邀請電子郵件中的指示來手動建立初始訂用帳戶。 請先確認帳戶擁有權，並手動建立初始 EA 訂用帳戶，才能繼續進行下一步。 不能光只是將該帳戶新增至註冊。

* 如果您想使用服務主體來建立 EA 訂用帳戶，您必須[授與該服務主體建立訂用帳戶的能力](grant-access-to-create-subscription.md)。
...
| `OfferType`   | yes      | 字串 | 訂用帳戶的供應項目方案。 EA 的兩個選項為 [MS-AZR-0017P](https://azure.microsoft.com/pricing/enterprise-agreement/) (生產環境使用) 和 [MS-AZR-0148P](https://azure.microsoft.com/offers/ms-azr-0148p/) (開發/測試環境使用，必須[使用 EA 入口網站來開啟](https://ea.azure.com/helpdocs/DevOrTestOffer))。                |
| `EnrollmentAccountObjectId`      | yes       | 字串 | 用來建立訂用帳戶並加以收費的註冊帳戶物件識別碼。 此值是您從 `Get-AzureRmEnrollmentAccount` 取得的 GUID。 |
