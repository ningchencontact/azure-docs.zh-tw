...
| 控制和管理平面記錄和審核| yes | 活動記錄會公開您資源上執行的所有寫入作業（PUT、POST、DELETE）;請參閱[查看活動記錄以在資源上審核動作](resource-group-audit.md)。 |
| 資料平面記錄和審核| N/A | |
...
| 安全性控制 | 是/否 | 注意事項|
|---|---|--|
| 驗證| yes | 以[Azure Active Directory](/azure/active-directory)為基礎。|
| 授權| yes | |
