...
開啟設定檔，並檢查其內容。 請注意其中包含存取權杖。 不要以手動方式再次登入，請載入設定檔。
...
或者，您可以從 PowerShell 叫用 REST 作業來登入。 從驗證回應，您可以擷取存取權杖以用於其他作業。 如需叫用 REST 作業擷取存取權杖的範例，請參閱[建立要求](/rest/api/#create-the-request)。
