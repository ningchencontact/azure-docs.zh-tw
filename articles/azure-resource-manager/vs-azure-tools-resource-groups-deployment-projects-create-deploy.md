...
本文說明如何使用[已安裝 Azure 部署和 ASP.NET 工作負載的 Visual Studio 2017 和更新版本](/dotnet/azure/dotnet-tools)。 如果您使用 Visual Studio 2015 Update 2 和 Microsoft Azure SDK for .NET 2.9，或 Visual Studio 2013 與 Azure SDK 2.9，您的經驗會大致相同。
...
5. 選擇 [部署]  按鈕，將專案部署至 Azure。 PowerShell 主控台會在 Visual Studio 執行個體的外部開啟。 出現提示時，請在 PowerShell 主控台中輸入 SQL Server 系統管理員密碼。 **PowerShell 主控台可能會隱藏在其他項目之後或在工作列中最小化。** 尋找此主控台並加以選取，以便提供密碼。
   ...
   >
   > 如需詳細資訊，請參閱[安裝和設定 Azure PowerShell 模組](/powershell/azure/install-Az-ps)。
   > 
   > 
