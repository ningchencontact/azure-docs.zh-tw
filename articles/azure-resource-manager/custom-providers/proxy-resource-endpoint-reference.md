...
執行「Proxy」資源**端點**的**端點**必須處理 Azure 中新 API 的要求和回應。 在此情況下， **resourceType**會為 `PUT`、`GET`和 `DELETE` 產生新的 AZURE 資源 API，以在單一資源上執行 CRUD，以及 `GET` 來抓取所有現有的資源。
