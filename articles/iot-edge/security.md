...
Azure IoT Edge 可以在不同硬體上執行，同時支援 Linux 和 Windows，並且適用於各種部署案例。  評估的風險取決於許多考量，包括解決方案擁有權、部署地理位置、資料敏感度、隱私權、應用程式類別以及法規需求。  它並非提供對於特定案例的死板解決方案，而是根據針對一定規模而設計的基礎紮實原則，合理地設計出可延伸的安全性架構。 
 
本文提供安全性架構概觀。 如需詳細資訊，請參閱[保護智慧邊緣](https://azure.microsoft.com/blog/securing-the-intelligent-edge/) \(英文\)。
