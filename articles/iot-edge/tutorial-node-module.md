...
您可以利用和快速入門相同的方式，使用 Azure 入口網站將 Node.ms 模組部署到 IoT Edge 裝置，但您也可以從 Visual Studio Code 部署和監視模組。 下列幾節會使用先決條件中所列的「適用於 VS Code 的 Azure IoT Edge 擴充功能」。 如果尚未安裝該擴充功能，請立即安裝。 
