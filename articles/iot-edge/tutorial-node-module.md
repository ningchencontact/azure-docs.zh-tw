...
* [Node.js 和 npm](https://nodejs.org)。 npm 會隨 Node.js 一起散發，這表示當您下載 Node.js 時，就會自動將 npm 安裝到電腦上。
...
3. 在命令選擇區中，輸入並執行命令 **Azure: Sign in**，然後遵循指示來登入 Azure 帳戶。 如果您已經登入，則可以略過此步驟。

4. 在命令選擇區中，輸入並執行命令 **Azure IoT Edge: New IoT Edge solution**。 在命令選擇區中，提供下列資訊來建立解決方案： 
...
VS Code 視窗會載入您的 IoT Edge 方案工作區。 裡面會有 **.vscode** 資料夾、**modules** 資料夾、**.env** 檔案，以及部署資訊清單範本檔案
...
4. 在部署資訊清單中新增 NodeModule 模組對應項。 在 `moduleContent` 區段底部，於 `$edgeHub` 模組對應項之後插入下列 JSON 內容： 
    ```json
        "NodeModule": {
            "properties.desired":{
...
您可以利用和快速入門相同的方式，使用 Azure 入口網站將 Node.ms 模組部署到 IoT Edge 裝置，但您也可以從 Visual Studio Code 部署和監視模組。 下列幾節會使用先決條件中所列的「適用於 VS Code 的 Azure IoT Edge 擴充功能」。 如果還未安裝該擴充功能，請立即安裝。 
...
7. 瀏覽至 NodeModule 所在的解決方案資料夾。 開啟 **config** 資料夾，然後選取 **deployment.json** 檔案。 按一下 [選取 Edge 部署資訊清單]。

8. 重新整理 [Azure IoT 中樞裝置] 區段。 您應該會看到新的 **NodeModule** 正在與 **TempSensor** 模組以及 **$edgeAgent** 和 **$edgeHub** 一起執行。 
