...
本文說明 Azure 入口網站如何引導您建立部署資訊清單，並且將部署推送到 IoT Edge 裝置。 如需根據其共用標記，建立以多部裝置為目標為的部署詳細資訊，請參閱[大規模部署和監視 IoT Edge 模組](how-to-deploy-monitor.md)

## <a name="prerequisites"></a>先決條件

* 在您 Azure 訂用帳戶中的 [IoT 中樞](../iot-hub/iot-hub-create-through-portal.md)。 
...
部署資訊清單是 JSON 文件，說明要部署哪個模組、資料如何在模組之間流動，以及所需的模組對應項屬性。 如需部署資訊清單的運作方式以及如何建立的詳細資訊，請參閱[了解如何使用、設定以及重複使用 IoT Edge 模組](module-composition.md)。
...
4. 請提供模組的名稱，然後指定容器映像。 例如︰ 
   * **名稱** - tempSensor
   * **映像 URI** - mcr.microsoft.com/azureiotedge-simulated-temperature-sensor:1.0
5. 視需要填寫選擇性欄位。 如需容器建立選項、重新啟動原則和所需狀態的詳細資訊，請參閱 [EdgeAgent 所需屬性](module-edgeagent-edgehub.md#edgeagent-desired-properties)。 如需模組對應項的詳細資訊，請參閱[定義或更新所需屬性](module-composition.md#define-or-update-desired-properties)。
6. 選取 [ **儲存**]。
7. 重複步驟 2-6 以將其他模組新增至您的部署。 
8. 選取 [下一步] 繼續前往路由區段。
...
## <a name="view-modules-on-your-device"></a>在您的裝置上檢視模組

一旦您將模組部署到您的裝置，就可以在入口網站的 [裝置詳細資料] 頁面中檢視所有項目。 此頁面會顯示每個已部署模組的名稱，以及像是部署狀態和結束代碼的實用資訊。 

## <a name="next-steps"></a>後續步驟

了解如何[大規模部署和監視 IoT Edge 模組](how-to-deploy-monitor.md)
