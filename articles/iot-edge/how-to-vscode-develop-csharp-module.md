...
### <a name="create-a-docker-image-and-publish-it-to-your-registry"></a>建立 Docker 映像並將它發行到您的登錄中
...
### <a name="deploy-your-iot-edge-modules"></a>部署您的 IoT Edge 模組
...
2. 以下列程式碼取代 **routes** 區段：
    ...
    ```
   > [!NOTE]
   > 執行階段中的宣告式規則會定義這些訊息的流向。 在本教學課程中，您需要兩個路由。 第一個路由會透過 "input1" 端點，將訊息從溫度感應器傳輸到篩選模組。 這是您搭配 FilterMessages 處理常式設定的端點。 第二個路由會將訊息從篩選模組傳輸到 IoT 中樞。 在此路由中，upstream 是一個特殊目的地，用來告知「IoT Edge 中樞」將訊息傳送給「IoT 中樞」。
...
在本教學課程中，您已建立 IoT Edge 模組並在 VS Code 中將它部署到 IoT Edge 裝置。 在 VS Code 中開發 Azure IoT Edge 時，若要了解其他案例，請參閱下列教學課程：
