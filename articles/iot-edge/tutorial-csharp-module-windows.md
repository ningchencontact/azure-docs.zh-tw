...
本教學課程示範如何使用 **Visual Studio 2017** 以 **C#** 開發模組，以及如何將其部署到 **Windows 裝置**。 如果您要開發適用於 Linux 裝置的模組，請改為移至[開發適用於 Linux 裝置的 C# IoT Edge 模組](tutorial-csharp-module.md)。 
...
Azure IoT Tools 擴充功能會針對 Visual Studio 2017 中所有支援的 IoT Edge 模組語言，提供專案範本。 這些範本具有您部署運作中模組以測試 IoT Edge 時需要的所有檔案和程式碼，或可成為您利用自己的商務邏輯自訂此範本的起點。 

1. 以系統管理員身分執行 Visual Studio。

2. 選取 [檔案]   > [新增]   > [專案]  。 

3. 在新的專案視窗中，選取 [Azure IoT]  專案類型，然後選擇 [Azure IoT Edge]  專案。 將專案和解決方案重新命名為 **CSharpTutorialApp** 之類的描述性項目。 選取 [確定]  以建立專案。 
