...
本文列出了在 Linux ARM32v7/armhf IoT Edge 设备上安装 Azure IoT Edge 运行时的步骤。 例如，這些步驟適用於 Raspberry Pi 裝置。 如需支援的 ARM32 作業系統的清單，請參閱 < [Azure IoT Edge 支援系統](support.md#operating-systems)。 
...
以下命令用于安装基于 Moby 的引擎和命令行接口 (CLI)。 CLI 對於開發相當有用，但是對於生產部署則是選擇性的。
...
您可以使用 `/etc/iotedge/config.yaml` 上的組態檔來設定精靈。 默认情况下，该文件有写保护，因此你可能需要提升权限才能对其进行编辑。
...
如果您在上一節中使用**手動設定**步驟，IoT Edge 執行階段應會在您的裝置上成功佈建並執行。 或者，如果使用了**自动配置**步骤，则需要完成一些额外的步骤，以便运行时可以代表你向 IoT 中心注册你的设备。 有关后续步骤，请参阅[在 Linux 虚拟机上创建和预配模拟 TPM IoT Edge 设备](how-to-auto-provision-simulated-device-linux.md#give-iot-edge-access-to-the-tpm)。
