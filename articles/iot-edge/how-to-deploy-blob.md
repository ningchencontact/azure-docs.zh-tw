...
4. 使用下列資訊，將您複製的 JSON 更新為**容器建立選項**：
   ...
   - 根據您的容器作業系統來取代 `<storage mount>`。 提供[磁片](https://docs.docker.com/storage/volumes/)區的名稱，或 IoT Edge 裝置上，blob 模組將用來儲存其資料之現有目錄的絕對路徑。 儲存體掛接會將您提供的裝置上的位置對應至模組中的集合位置。
