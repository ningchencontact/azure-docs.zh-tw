...
ms.locfileid: "75774428"
---
下列範例顯示如何在西雅圖的 Equinix 網際網路 Exchange 上建立 Exchange 連線。 如果您使用不同的提供者和不同的設定，請在您提出要求時替換成該資訊。

使用 PowerShell Cmdlet **AzPeeringExchangeConnectionObject**來建立 Powershell 連線物件，以用來產生新的對等互連要求：

以下是建立 Exchange 連線的範例：
...
> [!IMPORTANT]
> Microsoft 將開始布建要求的對等互連，而 `ConnectionState` 將會反映進度。
> 請參閱[Exchange 對等的逐步](../walkthrough-exchange-all.md)解說檔，以取得有關布建的相關步驟。

您可以檢查 ConnectionState，如下所示：
