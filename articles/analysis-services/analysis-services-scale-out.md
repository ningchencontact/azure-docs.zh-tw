...
2. 在 [Separate the processing server from the querying pool]\(區隔處理伺服器與查詢集區\) 中，選取 [是] 以將處理伺服器從查詢伺服器排除。 使用預設連接字串 (不含[) 的用戶端](#connections)連接`:rw`會重新導向至查詢集區中的複本。 
