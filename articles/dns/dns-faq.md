...
DNS 區域會用於裝載特定網域的 DNS 記錄。 例如，contoso.com 網域可能包含數筆 DNS 記錄。 The records might include mail.contoso.com for a mail server and www\.contoso.com for a website. 這些記錄會裝載在 contoso.com DNS 區域中。
...
To set up co-hosting, modify the NS records for the domain to point to the name servers of both providers. 名稱伺服器 (NS) 記錄可控制哪些提供者會收到網域的 DNS 查詢。 您可以在 Azure DNS 中、在另一個提供者中，以及在父區域中修改這些 NS 記錄。 父區域通常會透過網域名稱註冊機構來設定。 如需有關 DNS 委派的詳細資訊，請參閱 [DNS 網域委派](dns-domain-delegation.md)。
