...
## <a name="scenario-name-resolution-scoped-to-a-single-virtual-network"></a>案例：範圍設定為單一虛擬網路的名稱解析
...
## <a name="scenario-name-resolution-across-virtual-networks"></a>案例：虛擬網路之間的名稱解析
...
當有網路網路用戶端發出 DNS 查詢以查閱 VNETA-VM1.contoso.com 時，Azure 將會傳回來自公用區域的公用 IP 記錄。 如果從另一個 VM，就會發出相同的 DNS 查詢 (例如：VNETA-VM2) 在相同的虛擬網路 A，Azure 會從私用區域傳回的私人 IP 記錄。 
...
深入了解 DNS 區域和記錄，請瀏覽：[DNS 區域和記錄概觀](dns-zones-records.md)。
