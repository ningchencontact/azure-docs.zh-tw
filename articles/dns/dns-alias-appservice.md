...
現在，您需要記下應用程式的 IP 位址和主機名稱。

1. 開啟您的資源群組，然後按一下第一個應用程式 (在此範例中為 **App-01**)。
2. 在左側資料行中，按一下 [屬性]。
3. 記下 [URL] 下方的位址，並在 [輸出 IP 位址] 下方，記下清單中的第一個 IP 位址。 稍後當您設定流量管理員端點時，將會使用此資訊。
4. 針對 **App-02** 重複執行。
...
當您備妥 DNS 區域時，可以為區域頂點新增別名記錄。

1. 開啟您的資源群組，然後按一下 DNS 區域。
2. 按一下 [記錄集]。
3. 使用下表來新增記錄集：

   |名稱  |類型  |別名記錄集  |別名類型  |Azure 資源|
   |---------|---------|---------|---------|-----|
   |@     |具有使用 |是|Azure 資源|流量管理員 - 您的設定檔|

## <a name="add-custom-hostnames"></a>新增自訂主機名稱

將自訂主機名稱新增至這兩個 Web 應用程式。

1. 開啟您的資源群組，然後按一下第一個 Web 應用程式。
2. 在左側資料行中，按一下 [自訂網域]。
3. 按一下 [新增主機名稱]。
4. 在 [主機名稱] 下方，輸入您的網域名稱。 例如 contoso.com。

   您的網域應該會通過驗證，並且在 [主機名稱可用性] 和 [網域擁有權] 旁顯示綠色核取記號。
5. 按一下 [新增主機名稱]。
6. 若要在 [指派至網站的主機名稱] 下方查看新的主機名稱，請重新整理瀏覽器。 重新整理頁面不一定會立即顯示變更。
7. 針對第二個 Web 應用程式重複執行此程序。

## <a name="test-your-web-apps"></a>測試您的 Web 應用程式

現在您可以進行測試，以確定可連線到您的 Web 應用程式，並確保它會負載平衡。
