---
title: 自動調整 Windows 虛擬桌面預覽版工作階段主機-Azure
description: 說明如何為 Windows 虛擬桌面 Preview 工作階段主機設定自動調整腳本。
...
針對 Azure 中的許多 Windows 虛擬桌面預覽部署, 虛擬機器成本代表總 Windows 虛擬桌面部署成本的重要部分。 若要降低成本, 最好在離峰使用時間關閉和解除配置工作階段主機虛擬機器 (Vm), 然後在尖峰使用時間時重新開機它們。
...
2. 在 scaler VM 上建立資料夾, 以保存調整腳本及其設定 (例如, **C:\\調整-HostPool1**)。
...
| LimitSecondsToForceLogOffUser | 強制使用者登出前等待的秒數。如果設定為 0, 則不會強制使用者登出。  |
