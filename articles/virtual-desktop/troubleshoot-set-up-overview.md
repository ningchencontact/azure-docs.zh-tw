...
>[!NOTE]
>我們目前不採用 Windows 虛擬桌面處於預覽狀態時的支援案例。 當我們參考 Windows 虛擬桌面支援服務時, 請立即前往我們的 Tech 社區論壇。 請造訪[Windows 虛擬桌面技術社區](https://techcommunity.microsoft.com/t5/Windows-Virtual-Desktop/bd-p/WindowsVirtualDesktop), 討論產品小組和主動式社區成員的問題。 如果您需要解決支援問題, 請在發生問題時包含活動識別碼和大約時間範圍。

| **問題**                                                            | **建議的解決方案**  |
|----------------------------------------------------------------------|-------------------------------------------------|
| 建立租使用者                                                    | 如果發生 Azure 中斷, 請聯絡[Azure 支援](https://azure.microsoft.com/support/options/);否則, 請聯絡**遠端桌面服務/Windows 虛擬桌面支援**。|
...
| 從 Azure 入口網站管理 Windows 虛擬桌面工作階段主機環境    | 請聯絡**Azure 支援**。 <br> <br> 如需使用遠端桌面服務/Windows 虛擬桌面 PowerShell 時的管理問題, 請參閱[Windows 虛擬桌面 PowerShell](troubleshoot-powershell.md)或聯絡**遠端桌面服務/Windows 虛擬桌面支援小組**。 |
| 管理系結至主機集區和應用程式群組的 Windows 虛擬桌面設定 (應用程式群組)      | 請參閱[Windows 虛擬桌面 PowerShell](troubleshoot-powershell.md), 或洽詢**遠端桌面服務/Windows 虛擬桌面支援小組**。 <br> <br> 如果問題系結至範例圖形化使用者介面 (GUI), 請與 Yammer 社區聯繫。|
| 遠端桌面用戶端在啟動時無法正常運作                                                 | 請參閱[遠端桌面用戶端](troubleshoot-client-connection.md)連線, 如果無法解決問題, 請聯絡**遠端桌面服務/Windows 虛擬桌面支援小組**。  <br> <br> 如果是網路問題, 您的使用者必須聯絡其網路系統管理員。 |
| 已連線但沒有摘要                                                                 | 使用[遠端桌面用戶端](troubleshoot-client-connection.md)連線的「使用者連接但不顯示任何內容 (沒有摘要)」區段進行疑難排解。 <br> <br> 如果您的使用者已指派給應用程式群組, 請向**遠端桌面服務/Windows 虛擬桌面支援小組**呈報。 |
