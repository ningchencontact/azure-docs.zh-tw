...
* 使用 [**立即執行**], 並`https://jwt.ms`測試您的 web 或行動應用程式以外的原則。 此網站的作用就像信賴憑證者應用程式。 它會顯示 Azure AD B2C 原則所產生的 JSON Web 權杖 (JWT) 內容。 若要建立測試應用程式, 請流覽至 Azure 入口網站中**Azure AD B2C** \> **應用**程式, 並新增具有下列值的應用程式:

  * **名稱**：TestApp
  * **Web 應用程式/WEB API**:否
  * **原生用戶端**：否

  然後, 新增`https://jwt.ms`作為 [**回復 URL**]。
