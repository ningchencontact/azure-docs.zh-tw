---
title: Azure Active Directory B2C 中開發人員的自訂原則使用注意事項 | Microsoft Docs
...
* 使用自訂原則來撰寫和上傳自訂驗證使用者旅程。 
   ...
   * 定義使用者旅程中的條件式分支。 * 將啟用 REST API 功能的服務整合到您的自訂驗證使用者旅程。  
...
* 正式運作的日期還未確定。<br>
* 若案例和使用者旅程超過 Azure AD B2C 產品可作為客戶身分識別與存取管理 (CIAM) 平台的許可範圍，Microsoft 可自行判斷 (並可基於任何原因) 是否應將其加上旗標並加以拒絕或施加限制。
...
將問題張貼在 Stack Overflow，網址為 [https://aka.ms/aadb2cso](https://aka.ms/aadb2cso)
