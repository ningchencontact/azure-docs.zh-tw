...
1. 在 [名稱] 方塊中，輸入應用程式的名稱 (例如 *Contoso.AADB2C.API*)，然後選取 [確定]。

    ![在 Visual Studio 中建立新的 Visual Studio 專案](media/aadb2c-ief-rest-api-netfw/aadb2c-ief-rest-api-netfw-create-project.png)

1. 在 [新的 ASP.NET Web 應用程式] 視窗中，選取 [Web API] 或 [Azure API 應用程式] 範本。
...
1. 建立新的模型 (`OutputClaimsModel`)，然後將下列屬性新增至 `OutputClaimsModel` 類別：
...
1. 如果 *IdentityController.cs* 檔案尚未開啟，請按兩下該檔案，然後將檔案中的程式碼取代為下列程式碼：
...
1. 在 [發佈] 視窗中，選取 [Microsoft Azure App Service]，然後選取 [發佈]。
    ...
    > [!TIP]
    > 如需如何發佈的詳細資訊，請參閱[在 Azure 中建立 ASP.NET Web 應用程式](../app-service/app-service-web-get-started-dotnet-framework.md)。

1. 在 [Web 應用程式名稱] 方塊中，輸入唯一的應用程式名稱 (有效字元為 a-z、0-9 及連字號 (-))。 Web 應用程式的 URL 是 http://<app_name>.azurewebsites.NET，其中 *app_name* 是 Web 應用程式的名稱。 您可以接受自動產生的名稱 (這是唯一的)。

    ![設定 App Service 屬性](media/aadb2c-ief-rest-api-netfw/aadb2c-ief-rest-api-netfw-publish-to-azure-3.png)

1. 若要開始建立 Azure 資源，請選取 [建立]。
...
1. 使用 SignUpOrSignIn.xml 檔重複上述步驟。
