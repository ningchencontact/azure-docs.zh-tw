...
2. 按一下 [ **+ 容器**]。
3. 針對 [**名稱**], 輸入*root*。 這可以是您選擇的名稱 (例如*wingtiptoys*), 但我們會在此範例中使用*root*來求簡單。
4. 針對 [**公用存取層級**], 選取 [ **Blob**], 然後選取 **[確定]** 。
5. 按一下 [**根**] 以開啟新的容器。
6. 按一下 [上傳]。
7. 按一下 [選取檔案] 旁的資料夾圖示。
8. 流覽至, 然後選取您稍早在 [頁面 UI 自訂] 區段中建立的**customize-ui.html** 。
9. 如果您想要上傳至子資料夾, 請展開 [ **Advanced** ], 然後在 **[上傳到資料夾**] 中輸入資料夾名稱。
10. 選取 [上傳]。
11. 選取您上傳的**customize-ui.html** blob。
12. 在 [ **URL** ] 文字方塊的右邊, 選取 [**複製到剪貼**簿] 圖示, 將 URL 複製到剪貼簿。
13. 在網頁瀏覽器中, 流覽至您複製的 URL, 以確認您上傳的 blob 可供存取。 如果無法存取, 例如, 如果您遇到`ResourceNotFound`錯誤, 請確定容器的存取類型設定為 [ **blob**]。
...
2. 將`css`、 `fonts`和`images`資料夾底下的所有檔案上傳至 Blob 儲存體, 如上一節所述。 
3. 接下來, 在\* `wingtip`或`contoso`的根目錄中開啟每個 .html 檔案 (不論您在第一個步驟中選取的方式), 並將 http://localhost"" 的所有實例取代為您在步驟2中上傳的 css、影像和字型檔案的 url。
4. \*儲存 .html 檔案, 並將其上傳至 Blob 儲存體。
5. 現在修改延伸模組檔案, 如先前在[修改擴充](#modify-the-extensions-file)檔案中所述。
6. 如果您看到缺少的字型、影像或 css, 請檢查延伸模組原則和\*.html 檔案中的參考。
