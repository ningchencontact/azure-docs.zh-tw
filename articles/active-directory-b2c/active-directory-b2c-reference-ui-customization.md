...
您可以自訂客戶註冊、登入 (請參閱以上附註，了解關於商標的例外狀況)、密碼重設及設定檔編輯頁面的外觀和風格 (藉由設定[原則](active-directory-b2c-reference-policies.md))。 客戶在您的應用程式與 Azure AD B2C 所提供的頁面之間瀏覽時，將會有順暢的體驗。
