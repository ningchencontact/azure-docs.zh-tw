...
> [!IMPORTANT] 
> 啟用「**遠端桌面**連線」只會開啟 Linux 電腦上的**RDP**埠。 身為老師, 第一次使用 SSH 連線到 Linux 機器, 並安裝 RDP 和 GUI 套件, 讓您稍後可以使用 RDP 連線到 Linux 電腦。 然後, 您可以**發行**映射, 讓學生可以透過 RDP 連線到學生的 Linux vm。 
...
老師/教授可以藉由切換至 [**虛擬機器**] 視圖, 然後選取 [連線] 圖示,來連線到學生 VM。 在這之前, 老師必須**發佈**已安裝 RDP 和 GUI 套件的範本映射。 
