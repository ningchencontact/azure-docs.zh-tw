...
2. 在清單中選取特定或所有使用者。 若要選取特定使用者，選取清單第一個資料行中的核取方塊。 若要選取所有使用者，則選取第一個資料行標題 (**名稱**) 前面的核取方塊，或全部選取清單中所有使用者的核取方塊。 您可以看到的狀態**邀請狀態**這份清單中。  在下圖中，所有學生的邀請狀態設為**不會傳送邀請**。 

    ![選取學員](../media/tutorial-setup-classroom-lab/select-students.png)
1. 選取 **電子郵件圖示 （信封）** 中的資料列 （或） 選取其中一個**傳送邀請**工具列上。 您也可以將滑鼠停留在清單中，若要查看的電子郵件圖示的學生名稱中。 
...
4. 在 [透過電子郵件傳送註冊連結] 頁面上，遵循下列步驟： 
    ...
    2. 在 [透過電子郵件傳送註冊連結] 頁面上，選取 [傳送]。 您會看到的狀態變更的邀請**正在傳送邀請**然後**邀請已送出**。 
        
        ![傳送邀請](../media/tutorial-setup-classroom-lab/invitations-sent.png)
