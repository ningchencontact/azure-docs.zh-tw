...
1. 選取左側功能表上的 [使用者]  。
2. 選取 **每位使用者的配額：** 工具列上。 
3. 在 **每位使用者的配額**頁面上，指定您想要提供給每位使用者 （學生） 的時數： 
    1. **值為 0 小時 （只能排程）** 。 使用者可以使用其 Vm，只會在排定的時間，或您為實驗室擁有者會在 Vm 上，對它們。

        ![零小時-排定的時間](../media/how-to-configure-student-usage/zero-hours.png)
    1. **每位使用者的實驗室小時總數**。 使用者可以使用其 Vm 組數 （針對此欄位指定）**除了排定的時間**。 如果選取此選項，請在文字方塊中輸入**時數**。 
...
> [!IMPORTANT]
> 在傳送之前註冊連結的學生，老師必須是設定類別的排程，如果它們挑選 0 的配額時數，或指定實驗室的配額時數。
>
