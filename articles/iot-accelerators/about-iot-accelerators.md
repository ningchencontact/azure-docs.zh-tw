...
ms.locfileid: "37097983"
---
# <a name="what-are-azure-iot-solution-accelerators"></a>Azure IoT 解決方案加速器是什麼？

雲端式 IoT 解決方案通常會使用自訂程式碼和多個雲端服務來管理裝置的連線、資料處理和分析，以及呈現。

IoT 解決方案加速器集合了完整、隨時可部署的 IoT 解決方案，以實作常見的 IoT 案例，例如遠端監視、連線的處理站，以及預測性維護。 當您部署解決方案加速器時，該部署中會包含所有必要的雲端式服務以及任何必要的應用程式程式碼。

解決方案加速器是建立自有 IoT 解決方案的起點。 所有解決方案加速器的原始程式碼都是開放原始碼，並可在 GitHub 中取得。 建議您下載並[自訂](iot-accelerators-remote-monitoring-customize.md)解決方案加速器以因應您的需求。

您也可以先使用解決方案加速器作為學習工具，再從零開始建置自訂的 IoT 解決方案。 解決方案加速器會實作經過證實適用於雲端式 IoT 解決方案的做法，以供您遵循。

每個解決方案加速器中的應用程式程式碼均包含儀表板，以供您管理解決方案加速器。 例如，您可以使用儀表板來檢視已連線裝置傳來的遙測資料、佈建新裝置，或升級已連線裝置的韌體：

[![解決方案儀表板](./media/about-iot-accelerators/dashboard-inline.png)](./media/about-iot-accelerators/dashboard-expanded.png#lightbox)
...
原始的解決方案加速器是使用模型檢視控制器 (MVC) 架構，透過 .NET 所撰寫的。 Microsoft 正在將解決方案加速器更新為新的微服務架構。 GitHub 中有提供每個微服務的 [Java](https://github.com/Azure/azure-iot-pcs-remote-monitoring-java) 和 [.NET](https://github.com/Azure/azure-iot-pcs-remote-monitoring-dotnet) 版本。 下表顯示解決方案加速器的目前狀態：
...
若要試用 IoT 解決方案加速器，請參閱[部署雲端式遠端監視解決方案](quickstart-remote-monitoring-deploy.md)快速入門。
