...
1. 將**services\scripts\local\launch\idesettings\vscode 中的**檔案 vscode**資料夾，複製**到您剛才建立的資料夾。
1. 在 VS Code 中開啟 [**調試] 面板**，並執行 [**執行所有微服務**] 設定。 此組態會在 Docker 中執行裝置模擬微服務，並且在偵錯工具中執行其他微服務。

在偵錯主控台中**執行 [執行所有 microsoervices** ] 的輸出看起來如下所示：

[![部署-本機微服務](./media/deploy-locally-vscode/auth-debug-results-inline.png)](./media/deploy-locally-vscode/auth-debug-results-expanded.png#lightbox)
