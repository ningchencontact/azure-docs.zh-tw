...
裝置模型結構描述的 [模擬](iot-accelerators-remote-monitoring-device-schema.md#simulation) 區段會定義模擬裝置的內部狀態：
...
裝置模型結構描述的 [CloudToDeviceMethods](iot-accelerators-remote-monitoring-device-schema.md#cloudtodevicemethods) 區段會定義模擬裝置回應的方法。
...
[建立模擬的裝置](iot-accelerators-remote-monitoring-test.md)一文會向您示範如何在本機執行裝置模擬服務。 在本機執行服務可以更輕鬆地為您模擬的裝置偵錯，再將其部署至雲端。
