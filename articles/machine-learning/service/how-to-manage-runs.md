...
description: 了解如何啟動、 狀態、 標記，並組織您的機器學習服務實驗。
...
## <a name="start-and-status-a-run"></a>啟動和執行狀態
...
## <a name="cancel-or-fail-runs"></a>取消或失敗執行
 ...
 如果您注意到發生錯誤，或您執行的時間似乎長，無法完成，請使用[ `cancel()` ](https://docs.microsoft.com/python/api/azureml-core/azureml.core.run(class)?view=azure-ml-py#cancel--)方法完成之前停止執行，並將其標示為已取消。
