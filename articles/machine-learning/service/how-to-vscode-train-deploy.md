...
1. 在 Visual Studio Code 活動列中，按一下 Azure 圖示。 [Azure:Machine Learning] \(Azure: 機器學習\) 提要欄位隨即出現。
...
5. 在 JSON 組態檔中指定任何進階屬性，此檔案會在新的索引標籤中開啟。您可以在此檔案中為計算目標指定一個唯一名稱。

6. 當您設定完計算目標時，按一下右下方的 [Submit] \(提交\)。

以下是一個 Azure Machine Learning Compute (AMLCompute) 範例：[![在 VS Code 中建立 AML Compute](./media/vscode-tools-for-ai/CreateAMLCompute.gif)](./media/vscode-tools-for-ai/CreateAMLCompute.gif#lightbox)
...
1. 當您設定完模型屬性時，按一下右下方的 [Submit] \(提交\)。 
...
1. 當您設定完服務屬性時，按一下右下方的 [Submit] \(提交\)。 在這個服務屬性檔案中，您可以指定可能想要使用的本機 Docker 檔案或 schema.json 檔案。
