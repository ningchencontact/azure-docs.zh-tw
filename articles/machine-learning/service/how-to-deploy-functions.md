...
* __輸入腳本__。 此腳本會接受要求、使用模型對要求評分，並傳回結果。
    ...
    >
    > 根據預設，封裝函式時，會將輸入視為文字。 如果您有興趣取用輸入的原始位元組（例如，針對 Blob 觸發程式），您應該使用[AMLRequest 來接受原始資料](https://docs.microsoft.com/azure/machine-learning/service/how-to-deploy-and-where#binary-data)。
...
> [!IMPORTANT]
> 建立要與 Azure Functions 搭配使用的推斷設定時，您必須使用[環境](https://docs.microsoft.com/python/api/azureml-core/azureml.core.environment%28class%29?view=azure-ml-py)物件。 下列範例示範如何建立環境物件，並將它與推斷設定搭配使用：
>
> ```python
...
* 瞭解[如何在函式檔中](https://docs.microsoft.com/azure/azure-functions/functions-create-function-linux-custom-imag)設定函數應用程式。
