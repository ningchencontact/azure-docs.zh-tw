...
1. 建立虛擬網路後方的計算資源（例如，Machine Learning 叢集），或將計算資源附加至工作區（例如，HDInsight 叢集、虛擬機器或 Azure Kubernetes Service 叢集）。 計算資源可以用於實驗或模型部署。
...
> * 為計算叢集指定的子網必須有足夠的未指派 IP 位址，以容納目標 Vm 的數目。 如果子網沒有足夠的未指派 IP 位址，將會部分配置計算叢集。
...
> * 如果工作區的 Azure 儲存體帳戶也在虛擬網路中受到保護，它們必須位於與 Azure Machine Learning 計算叢集相同的虛擬網路中。

Machine Learning 計算叢集會自動在包含虛擬網路的資源群組中配置額外的網路資源。 針對每個計算叢集，服務會配置下列資源：
