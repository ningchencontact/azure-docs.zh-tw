...
Azure Machine Learning 服務依賴其他 Azure 服務來處理計算資源。 計算資源 (計算目標) 用於定型和部署模型。 這些計算目標可建立在虛擬網路內部。 例如，您可以使用資料科學 VM 來定型模型，然後將模型部署到 Azure Kubernetes Service。 如需虛擬網路的詳細資訊，請參閱[虛擬網路概觀](https://docs.microsoft.com/azure/virtual-network/virtual-networks-overview)文件。
...
如果您不確定是否已修改這些設定，請參閱[設定 Azure 儲存體防火牆和虛擬網路](https://docs.microsoft.com/azure/storage/common/storage-network-security)文件的__變更預設網路存取規則__一節，然後使用可_允許_從__所有網路__存取的步驟。

## <a name="use-machine-learning-compute"></a>使用 Machine Learning Compute

若要在虛擬網路中使用 Machine Learning Compute，請使用下列資訊來了解網路需求：
...
- 如果您打算透過限制流量來保護虛擬網路，您必須針對 Machine Learning Compute 服務開放一些連接埠。 如需詳細資訊，請參閱[必要連接埠](#mlcports)一節。

- 確認您對虛擬網路其訂用帳戶或資源群組的安全性原則或鎖定是否限制了管理虛擬網路的權限。

- 如果您要將多個的 Machine Learning Compute 叢集放在一個虛擬網路，您可能必須要求一個或多項資源的配額增加。
...
若要透過 **Azure 入口網站**建立 Machine Learning Compute 叢集，請使用下列步驟：

1. 使用 [Azure 入口網站](https://portal.azure.com)來選取 Azure Machine Learning 服務工作區。

1. 從 [應用程式] 區段中，選取 [計算]。 然後，選取 [新增計算]。 
...
您也可以使用 **Azure Machine Learning SDK** 建立 Machine Learning Compute 叢集。 下列程式碼會在名為 `mynetwork` 的虛擬網路其 `default` 子網路中建立新的 Machine Learning Compute 叢集：
...
在建立程序完成後，您可以使用叢集將模型定型。 如需詳細資訊，請參閱[選取與使用定型的計算目標](how-to-set-up-training-targets.md)文件。

## <a name="use-a-virtual-machine-or-hdinsight"></a>使用虛擬機器或 HDInsight

若要虛擬網路中的虛擬機器或 HDInsight 叢集搭配您的工作區使用，請使用下列步驟：

> [!IMPORTANT]
> Azure Machine Learning 服務僅支援執行 Ubuntu 的虛擬機器。

1. 使用 Azure 入口網站、Azure CLI 等建立 VM 或 HDInsight 叢集，並將它放在 Azure 虛擬網路中。 如需詳細資訊，請參閱下列文件：
...
1. 若要允許 Azure Machine Learning 服務與 VM 或叢集上的 SSH 連接埠通訊，您必須設定 NSG 的來源項目。 SSH 連接埠通常是連接埠 22。 若要允許來自此來源的流量，請使用以下資訊：
   ...
   ![用於對虛擬網路內的 VM 或 HDInsight 進行實驗的輸入規則螢幕擷取畫面](./media/how-to-enable-virtual-network/experimentation-virtual-network-inbound.png)

    為 NSG 保留預設輸出規則。 如需詳細資訊，請參閱[安全性群組](https://docs.microsoft.com/azure/virtual-network/security-overview#default-security-rules)文件的預設安全性規則一節。
    
1. 將 VM 或 HDInsight 叢集附加至 Azure Machine Learning 服務工作區。 如需詳細資訊，請參閱[為模型定型設定計算目標](how-to-set-up-training-targets.md)文件。

## <a name="use-azure-kubernetes-service-aks"></a>使用 Azure Kubernetes Service (AKS)

> [!IMPORTANT]
> 請先檢查必要條件，並為您的叢集規劃 IP 位址，再繼續進行以下所述的步驟。 您可以參閱[在 Azure Kubernetes Service (AKS) 中設定進階網路](https://docs.microsoft.com/azure/aks/configure-advanced-networking)
> 
> 為 NSG 保留預設輸出規則。 如需詳細資訊，請參閱[安全性群組](https://docs.microsoft.com/azure/virtual-network/security-overview#default-security-rules)文件的預設安全性規則一節。
>
> Azure Kubernetes Service 與 Azure 虛擬網路應在相同的區域中。

若要將虛擬網路中的 Azure Kubernetes Service 新增至您的工作區，請從 __Azure 入口網站__使用下列步驟：
...
您在虛擬網路中也可以使用 **Azure Machine Learning SDK** 新增 Azure Kubernetes 服務。 下列程式碼會在名為 `mynetwork` 的虛擬網路其 `default` 子網路中建立新的 Azure Kubernetes Service：
...
* [使用 SSL 保護部署的模型](how-to-secure-web-service.md)
