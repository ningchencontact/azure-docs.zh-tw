...
本檔假設您已熟悉 Azure 虛擬網路以及一般的 IP 網路功能。 本檔也假設您已建立要與計算資源搭配使用的虛擬網路和子網。 如果您不熟悉 Azure 虛擬網路, 請閱讀下列文章以瞭解服務:
...
若要使用, 請將預設 Azure 儲存體帳戶用於虛擬網路中的工作區, 請使用下列步驟:
...
若要搭配虛擬網路背後的 Key Vault 使用 Azure Machine Learning 測試功能, 請遵循下列步驟:
...
如果您不想要使用預設輸出規則, 而且想要限制虛擬網路的輸出存取, 請遵循下列步驟:
...
* 在資源所在區域中, Azure Batch 服務所使用之每個 IP 位址的使用者定義路由。 這些 Udr 可讓 batch 服務與計算節點進行通訊, 以進行工作排程。 若要取得 Batch 服務的 IP 位址清單, 請聯絡 Azure 支援。
...
1. 請確定控制虛擬網路的 NSG 群組已針對使用__AzureMachineLearning__ __服務__標籤的 Azure Machine Learning 服務啟用輸入規則
...
1. 請確定控制虛擬網路的 NSG 群組已針對評分端點啟用輸入規則, 以便從虛擬網路外部呼叫
