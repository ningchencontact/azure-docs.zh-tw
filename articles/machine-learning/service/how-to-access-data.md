...
當您建立工作區時，會將 Azure Blob 容器和 Azure 檔案共用登錄至名為 `workspaceblobstore` 的工作區，並分別 `workspacefilestore`。 它們會儲存 Blob 容器的連線資訊，以及在附加至工作區的儲存體帳戶中布建的檔案共用。 `workspaceblobstore` 會設定為預設資料存放區。
