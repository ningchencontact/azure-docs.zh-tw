---
title: '視覺化介面範例 #5：分類以預測變換 + appetency + 向上銷售'
...
因為您正試著回答「哪一個？」這個問題， 這稱為分類問題，但您可以在此專案中套用相同的邏輯，以處理任何類型的機器學習問題，不論其為回歸、分類、叢集等。

以下是此實驗的完成圖形：

![實驗圖表](./media/how-to-ui-sample-classification-predict-churn/experiment-graph.png)
...
4. 選取範例5實驗的 [**開啟**] 按鈕。
    ...
    ![開啟實驗](media/how-to-ui-sample-classification-predict-churn/open-sample5.png)

## <a name="data"></a>Data

此實驗的資料來自 KDD 杯2009。 它有50000個數據列和230個功能資料行。 這項工作是針對使用這些功能的客戶，預測變換、appetency 和向上銷售。 如需有關資料和工作的詳細資訊，請參閱[KDD 網站](https://www.kdd.org/kdd-cup/view/kdd-cup-2009)。
...
- 原始資料集包含許多遺漏值。 使用 [**清除遺漏的資料**] 模組，將遺漏的值取代為0。
