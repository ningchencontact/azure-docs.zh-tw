...
在這篇文章，了解如何使用 Azure Machine Learning 服務來管理您的模型的生命週期。 Azure Machine Learning 使用機器學習服務作業 (MLOps) 的方法，進而改善品質和您的機器學習解決方案的一致性。 

Azure Machine Learning 服務提供下列 MLOps 功能：

- **從任何地方的 ML 專案部署**
- **監視 ML 應用程式的操作和 ML 相關問題**-比較模型之間的訓練和推斷的輸入、 瀏覽模型特定的計量和 ML 基礎結構上提供監視和警示。
- **擷取建立 ML 生命週期的端對端稽核記錄所需的資料**，包括誰發行模型、 為什麼會有變更，以及當模型已部署或生產環境中使用。
- **自動化使用 Azure Machine Learning 和 Azure DevOps 的端對端 ML 生命週期**經常更新模型，測試新的模型，並持續推出新的 ML 模型，與其他應用程式和服務。

若要聽到更 MLOps 和它們如何套用在 Azure Machine Learning 服務背後的概念上，請觀賞下列影片。

> [!VIDEO https://www.microsoft.com/videoplayer/embed/RE2X1GX]

## <a name="deploy-ml-projects-from-anywhere"></a>從任何地方的 ML 專案部署

### <a name="turn-your-training-process-into-a-reproducible-pipeline"></a>將您的訓練程序轉換成可重現的管線
使用 Azure Machine learning 的 ML 管線將拼接在一起的所有參與您模型定型程序，從功能擷取至超參數調整模型評估的資料準備步驟。

如需詳細資訊，請參閱 < [ML 管線](concept-ml-pipelines.md)。

### <a name="register-and-track-ml-models"></a>註冊並追蹤 ML 模型
...
## <a name="capture-an-end-to-end-audit-trail-of-the-ml-lifecycle"></a>擷取 ML 生命週期的端對端稽核記錄

Azure ML 可讓您追蹤所有 ML 資產的端對端稽核線索。 具體而言：

- 與追蹤資訊的存放庫 / 分支 / 認可您程式碼出處的 Git 整合，azure ML。
- Azure ML 資料集協助您追蹤和版本資料。
- Azure ML 執行歷程記錄管理的程式碼、 資料和計算用來定型模型。
- Azure ML 模型登錄會擷取所有與您 （實驗的定型的模型它，其所部署的位置，及其部署是否狀況良好） 相關聯的中繼資料。

## <a name="automate-the-end-to-end-ml-lifecycle"></a>自動化的端對端 ML 生命週期 

若要建立持續整合程序，可訓練的模型，您可以使用 GitHub 與 Azure 的管線。 在典型的案例中，當資料科學家會檢查到 Git 存放庫，針對專案中，變更 Azure 管線會開始定型執行。 然後可以檢查執行結果，以查看 定型模型的效能特性。 您也可以建立管線，以部署為 web 服務模型。
