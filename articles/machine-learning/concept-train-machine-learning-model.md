...
+ **設計**工具： Azure Machine Learning 設計工具可讓您輕鬆進入機器學習服務，以建立概念證明，或針對幾乎沒有程式碼撰寫經驗的使用者。 它可讓您使用拖放 web 型 UI 來定型模型。 您可以使用 Python 程式碼作為設計的一部分，或在不撰寫任何程式碼的情況下訓練模型。
