...
## <a name="error-0013"></a>錯誤0013  
 ...
 當定型的模型與連接的計分模組不相容時, 就會發生此錯誤。 <!--For example, connecting the output of [Train Matchbox Recommender](train-matchbox-recommender.md) to [Score Model](score-model.md) (instead of [Score Matchbox Recommender](score-matchbox-recommender.md)) will generate this error when the experiment is run.  -->
...
**解決方案：** 請考慮將輸出中的維度數目減少為小於輸入中的維度數目。 這在 PCA 的應用程式中是常見的。   <!--For more information, see [Principal Component Analysis](principal-component-analysis.md).  -->
...
## <a name="error-0100"></a>錯誤0100  
 ...
 當您建立自訂模組, 且自訂模組 xml 定義檔中**Language**元素的 name 屬性具有不正確值時, Azure Machine Learning 會發生此錯誤。 目前, 此屬性唯一的有效值為`R`。 例如:  
  
 `<Language name="R" sourceFile="CustomAddRows.R" entryPoint="CustomAddRows" />`  
  
**解決方案：** 確認自訂模組 xml 定義檔中**Language**元素的 name 屬性設定為`R`。 儲存檔案、更新自訂模組 zip 封裝, 然後再次嘗試新增自訂模組。  
...
**解決方案：** 檢查所有埠和參數的識別碼值是否為唯一。 儲存 xml 檔案、更新自訂模組 zip 封裝, 然後再次嘗試新增自訂模組。  
...
**解決方案：** 請確定選取的檔案是有效的 .zip 檔案, 而且是使用其中一種支援的壓縮演算法進行壓縮。  
...
**解決方案：** 將一個或多個 xml 模組定義檔案解壓縮至磁片磁碟機上的暫存資料夾, 以確認該檔案位於 zip 套件的根資料夾中。 任何 xml 檔案都應該直接在您解壓縮 zip 套件的資料夾中。 當您建立 zip 封裝時, 請務必不要選取包含 xml 檔案的資料夾來壓縮, 因為這會在 zip 封裝中建立子資料夾, 其名稱與您選取要壓縮的資料夾相同。  
...
**解決方案：** 請確定自訂模組 xml 定義的**Language**元素中的**sourceFile**屬性值是正確的, 而且來源檔案存在於 zip 封裝的正確相對路徑中。  
  
|例外狀況訊息|  
|------------------------|  
|參考的 R 腳本檔案不存在。|  
|找不到參考的{0}R 腳本檔案 ' '。 請確定檔案的相對路徑是正確的定義位置。|  
...
**解決方案：** 請確定自訂模組 xml 定義檔中任何**Arg**元素的 type 屬性是支援的類型。  
...
**解決方案：** 請確定自訂模組 XML 定義檔中 Input 元素的 type 屬性是支援的類型。  
...
**解決方案：** 請確定自訂模組 xml 定義檔中 Output 元素的 type 屬性是支援的類型。  
...
**解決方案：** 請確定自訂模組 xml 定義中定義的輸入和輸出埠數目上限未超過支援的埠數目上限。  
  
|例外狀況訊息|  
|------------------------|  
|已超過支援的輸入或輸出埠數目。|  
|已超過支援的 '{0}' 埠數目。 允許的 '{0}' 埠數目上限為 '{1}'。| 
...
**解決方案：** 當資料行選擇器引數的語法在自訂模組 xml 定義中包含錯誤時, 就會產生此錯誤。  
...
**解決方案：** 請確定 portId 屬性符合自訂模組 xml 定義中所定義輸入埠的識別碼值。  
  
|例外狀況訊息|  
|------------------------|  
|資料行選擇器參考了不存在的輸入埠識別碼。|  
|資料行選擇器參考了不存在的輸入埠{0}識別碼 ' '。|  
...
**解決方案：** 請確定每個專案都已正確開啟和關閉。 請確定 XML 格式沒有任何錯誤。  
...
**解決方案：** 請確定自訂模組定義檔會定義自訂模組支援的元素和屬性。  
...
**解決方案：** 請確認自訂模組中包含的任何腳本檔案的檔案格式和副檔名。  
...
**解決方案：** 檢查輸入資料集及其中繼資料。 請確定只有一個資料行包含權數。  
...
**解決方案：** 如有必要, 請變更資料行的資料格式, 或選擇不同的資料行。  
...
|Dataset 中的資料行數目超過允許的限制。|  
|'{0}' 中資料集的資料行數超過允許的數目。 '|  
|'{0}' 中資料集的資料行數超過允許的限制 '{1}'。|  
|'{0}' 中資料集的資料行數超過允許的{1} {2}' ' 限制 ' '。|  
## <a name="error-0130"></a>錯誤0130  
 訓練資料集中的所有資料列都包含遺漏值時, 就會發生例外狀況。  
  
 訓練資料集內的某些資料行是空的時, 就會發生這種情況。  
  
**解決方案：** 使用 [[清除遺漏的資料](clean-missing-data.md)] 模組來移除具有所有遺漏值的資料行。  
...
**解決方案：** 使用錯誤訊息中提供的詳細資料, 以決定如何繼續進行。  
  
|例外狀況訊息|  
|------------------------|  
|上傳壓縮的資料集失敗|  
|壓縮的{0}資料集失敗, 並出現下列訊息:{1}|  
|Zip 資料{0}集失敗, {1}發生例外狀況, 訊息為:{2}|  
...
**解決方案：** 錯誤訊息會提供 .zip 檔案中找到的檔案清單。 複製所需檔案的名稱, 並將它貼到 [**要解壓縮的資料集**] 文字方塊中。  
...
**解決方案：** 重新流覽模組。 如果您想要解壓縮的檔案名出現在找到的檔案清單中, 請將檔案名複製並貼到 [**要解壓縮的資料集**] 屬性方塊中。 如果您在清單中看不到想要的檔案名, 請確認您有正確的 .zip 檔案和所需檔案的正確名稱。  
...
|------------------------|
|當標籤資料行遺失或已標記的資料列數目不足時, 就會發生例外狀況。|  
|當標籤資料行遺失或有少於{0}標記的資料列時, 就會發生例外狀況|  
  

## <a name="error-0135"></a>錯誤0135  
 僅支援以距心為基礎的叢集。  
  
**解決方案：** 如果您嘗試評估以自訂叢集演算法為基礎的群集模型, 但該叢集不使用距心來初始化叢集, 您可能會遇到這個錯誤訊息。  
...
**解決方案：** 如果您嘗試讀取大型資料集, 但無法完成作業, 則資料集的縮減取樣可能會有説明。  
...
|無法轉換: {0}。|  
|無法轉換: {0}, 在資料列{1}上。|  
|無法將類型{0}的資料行轉換成資料列{1}上類型{2}的資料行。|  
|無法將類型{0}的資料{2}行 "" 轉換成資料列{1}上類型{3}的資料行。|  
|無法將類型{0}的資料{2}行 "" 轉換成資料{3}列類型{1}的資料行{4}""。| 
...
**解決方案：** 某些作業會對功能和類別資料行執行統計作業, 而且如果沒有足夠的值, 則運算可能會失敗, 或傳回不正確結果。 檢查您的資料集, 以查看功能和標籤資料行中有多少值, 並判斷您嘗試執行的作業是否為統計有效。  
...
**解決方案：** 確認 URL 參照的是有效的 GitHub 存放庫。 不支援其他網站類型。  
...
**解決方案：** 請檢查 GitHub 存放庫的 URL 是否有效, 並以 \blob\ 或 \tree\\結尾。  
  
|例外狀況訊息|  
|------------------------|  
|無法剖析 GitHub URL。|  
|無法剖析 GitHub URL (在存放庫\\名稱後面必須是\\' \blob ' 或 ' \tree '):{0}|  
...
**解決方案：** 編輯檔案名, 使結合的路徑和檔案名不超過248個字元。  
...
**解決方案：** 問題可能是暫時性的;您可能會嘗試在另一次存取檔案。 或者, 請確認您擁有必要的許可權, 而且來源有效。  
  
|例外狀況訊息|  
|------------------------|  
|GitHub 存取錯誤。|  
|GitHub 存取錯誤。 [https://doi.org/10.13012/J8PN93H8]({0})|  
...
|------------------------|  
|無法解析指定的伺服器名稱|  
|無法解析指定{0}的 documents.azure.com。|
