...
1. 在介面中, 將**多元類神經網路**模組新增至您的實驗。 您可以在 [**分類**] 分類的 [ **Machine Learning**]、[**初始化**] 下找到此模組。
...
3. **隱藏層規格**:選取要建立的網路架構類型。
    ...
    - **完全連線的案例**:選取此選項可使用預設的類神經網路架構來建立模型。 針對多元類神經網路模型, 預設值如下所示:
...
5. **隱藏節點的數目**:此選項可讓您自訂預設架構中隱藏節點的數目。 輸入隱藏節點的數目。 預設值是一個具有100節點的隱藏層。

6. **學習速度**:定義在每個反復專案上執行的步驟大小 (在更正之前)。較大的學習速率值可能會使模型的聚合速度更快, 但它可以下限本機 minima。

7. **學習反覆運算次數**:指定演算法處理定型案例的最大次數。

8. **初始學習權數直徑**:指定學習程式開始時的節點權數。

9. **動力**:指定從先前的反復專案中學習到節點時要套用的權數。
  
11. **隨機範例**:選取此選項可在反復專案之間隨機播放案例。

    如果您取消選取此選項, 則每次執行實驗時, 都會以完全相同的順序來處理案例。

12. **亂數字種子**:如果您想要確保跨相同實驗的執行可重複性, 請輸入要做為種子的值。
