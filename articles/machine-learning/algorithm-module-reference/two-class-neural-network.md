...
1.  將**兩個類別的類神經網路**模組新增至您的實驗。 您可以在 [**分類**] 分類的 [ **Machine Learning**]、[**初始化**] 下找到此模組。  
...
3.  針對 [**隱藏層規格**], 選取要建立的網路架構類型。  
    ...
    -   **完全連線的案例**:會使用預設的類神經網路架構, 定義為兩個類別的神經網路, 如下所示:
...
10. 選取 [**隨機播放範例**] 選項, 在反復專案之間隨機播放案例。 如果您取消選取此選項, 則每次執行實驗時, 都會以完全相同的順序來處理案例。
  
11. 針對 [**亂數字種子**], 輸入要做為種子的值。
  
     當您想要確保跨相同實驗的執行可重複性時, 指定種子值會很有用。  否則會使用系統時鐘值做為種子, 這會在每次執行實驗時, 產生稍微不同的結果。
  
13. 將加上標籤的資料集新增至實驗, 並連接其中一個[定型模組](module-reference.md)。  
