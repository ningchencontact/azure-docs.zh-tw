...
資料庫伺服器已經預先設定，而且與 SQL Server 相關的 Windows 服務（如 `SQL Server (MSSQLSERVER)`）已設為自動執行。 唯一的手動步驟包括使用 Microsoft Machine Learning Server 啟用資料庫內分析。 若要這麼做，您可以在 SQL Server Management Studio （SSMS）中執行下列命令作為一次性動作。 當您以電腦系統管理員身分登入、在 SSMS 中開啟新查詢，並確定選取的資料庫為 `master` 時，請執行此命令：
...
當您不再需要 Hadoop 相關服務時，可以藉由執行 ```systemctl stop hadoop-namenode hadoop-datanode hadoop-yarn``` 來停止。

此範例示範如何在遠端 Spark 內容中開發和測試 MRS （這是 DSVM 上的獨立 Spark 實例），並可在 `/dsvm/samples/MRS` 目錄中取得。
...
若要讓 DSVM 上的 Spark 實例存取 Blob 儲存體或 Azure Data Lake Storage 中儲存的資料，您必須根據 $SPARK _HOME/會議/core-site.xml 中找到的範本，建立並設定 `core-site.xml` 檔案。 您也必須具有適當的認證，才能存取 Blob 儲存體和 Azure Data Lake Storage。 （請注意，範本檔案會使用 Blob 儲存體和 Azure Data Lake Storage 設定的預留位置）。
