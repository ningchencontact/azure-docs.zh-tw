---
title: 快速入門：建立 CentOS Linux
titleSuffix: Azure Data Science Virtual Machine
description: 在 Azure 中建立和設定 Linux 資料科學虛擬機器以進行分析和機器學習。
...
ms.locfileid: "70195329"
---
# <a name="quickstart-set-up-a-linux-centos-data-science-virtual-machine-in-azure"></a>快速入門：在 Azure 中設定 Linux CentOS 資料科學虛擬機器
...
您必須具備下列必要項目，才能建立 Linux 資料科學虛擬機器：

* **Azure 訂用帳戶**：若要取得 Azure 訂用帳戶，請參閱[建立免費 Azure 帳戶](https://azure.microsoft.com/free/)。
* **Azure 儲存體帳戶**：若要取得 Azure 儲存體帳戶，請參閱[建立儲存體帳戶](../../storage/common/storage-quickstart-create-account.md)。 如果您不想使用現有的 Azure 儲存體帳戶，您可以在建立 DSVM 時建立儲存體帳戶。

## <a name="other-versions-of-the-data-science-virtual-machine"></a>其他版本的資料科學虛擬機器

下列版本也提供此資料科學虛擬機器：

* [Ubuntu](dsvm-ubuntu-intro.md)：Ubuntu 映像具有許多與 CentOS 映像相同的工具，包括深度學習架構。 
* [Windows](provision-vm.md)

## <a name="create-a-linux-data-science-virtual-machine"></a>建立 Linux 資料科學虛擬機器

若要建立 Linux DSVM 的執行個體：

1. 移至 [Azure 入口網站](https://portal.azure.com/#create/microsoft-ads.linux-data-science-vmlinuxdsvm)中的虛擬機器清單。
1. 選取 [建立]  以開啟精靈。

   ![設定資料科學虛擬機器的精靈](./media/linux-dsvm-intro/configure-linux-data-science-virtual-machine.png)
1. 在精靈的每個步驟中，輸入或選取下列資訊：
   ...
   **1** **基本**：
      ...
      * 訂用帳戶  ：如果您有多個訂用帳戶，請選取要在其中建立機器及計費的訂用帳戶。 您必須具有為訂用帳戶建立資源的權限。
      * **資源群組**：您可以建立新的資源群組，或使用現有的群組。
      * **位置**：選取要用於 DSVM 的資料中心。 一般情況下，請選取擁有您大部分資料的資訊中心，或是最接近您實際位置而可最快速地存取網路的資料中心。
   
   **2** **大小**：選取符合您的功能需求和成本限制的伺服器類型。 選取 [檢視全部]  以查看更多 VM 大小的選項。
   
   
   **3** **設定**：
      * **磁碟類型**：如果您偏好固態硬碟 (SSD)，請選取 [進階]  。 否則，請選取 [標準]  。
      * **儲存體帳戶**：您可以在訂用帳戶中建立新的 Azure 儲存體帳戶，或使用在精靈的**基本**步驟中選取的相同位置中現有的 Azure 帳戶。
      * **其他參數**：在大部分的情況下，請使用預設值來設定其他參數。 若要查看非預設值，請將滑鼠停留在資訊連結上方以取得參數。
   
   **4** **摘要**：確認您輸入的資訊正確無誤。
   
   **5** **購買**：若要開始佈建，請選取 [購買]  。 系統會提供交易條款的連結。 DSVM 只會對您在 [大小]  中選取的伺服器大小產生計算資源的費用，除此之外不會產生額外費用。

佈建需要 10 到 20 分鐘的時間。 在 Azure 入口網站中會顯示佈建的狀態。

## <a name="how-to-access-the-linux-data-science-virtual-machine"></a>如何存取 Linux 資料科學虛擬機器

建立 DSVM 之後，您就可以使用 SSH 登入。 針對文字殼層介面，請使用您在精靈的 [基本]  區段中建立的帳戶認證。 您可以在 Windows 上下載 SSH 用戶端工具，例如 [PuTTy](https://www.putty.org)。 如果您偏好圖形化桌面 (X Windows 系統)，可以在 PuTTy 上使用 X11 轉送或安裝 X2Go 用戶端。

> [!NOTE]
> 在測試中，X2Go 用戶端的效能優於 X11 轉寄。 我們建議您使用 X2Go 用戶端作為圖形化桌面介面。

## <a name="install-and-configure-the-x2go-client"></a>安裝和設定 X2Go 用戶端

Linux DSVM 已經佈建了 X2Go 伺服器，且已可接受用戶端連線。 若要連線至 Linux DSVM 圖形化桌面，請在用戶端上完成下列程序：

1. 從 [X2Go](https://wiki.x2go.org/doku.php/doc:installation:x2goclient)下載並安裝您用戶端平台適用的 X2Go 用戶端。
1. 執行 X2Go 用戶端。 選取 [新增工作階段]  。 此時會開啟具有多個索引標籤的組態視窗。 輸入下列組態參數︰
   * **[工作階段] 索引標籤**：
     * **主機**：輸入 Linux DSVM 的主機名稱或 IP 位址。
     * **登入**：在 Linux DSVM 上輸入使用者名稱。
     * **SSH 連接埠**：保留預設值 **22**。
     * **工作階段類型**：將值變更為 **XFCE**。 Linux DSVM 目前僅支援 XFCE 桌面。
   * [媒體]  索引標籤：如果您不需要使用聲音支援和用戶端列印，可關閉這些功能。
   * **共用資料夾**︰如果您想要將來自用戶端機器的目錄掛接在 Linux DSVM 上，請新增要與 DSVM 共用的用戶端機器目錄。

當您透過 X2Go 用戶端使用 SSH 用戶端或 XFCE 圖形化桌面登入 DSVM 之後，便可開始使用已安裝並設定於 DSVM 上的工具。 在 XFCE 上，您可以看到許多工具的應用程式功能表捷徑和桌面圖示。
