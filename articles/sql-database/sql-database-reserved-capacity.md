...
在使用 Azure SQL Database 時，預付 Azure SQL Database 計算資源的費用，會比預付型方案的價格還要划算。 若使用 Azure SQL Database 保留容量，即表示您預先承諾會使用 SQL Database 一或三年的時間，來換取大幅的計算費用折扣。 若要購買 SQL Database 保留容量，您需要指定 Azure 區域、部署類型、服務和期限。 
...
如需企業客戶和預付型方案客戶的保留容量購買收費方式詳細資料，請參閱[常見問題集](#frequently-asked-questions)。
...
4. 填寫必要欄位。 符合所選屬性的現有或全新單一資料庫或彈性集區，就有資格獲得保留容量折扣。 取得折扣的 SQL Database 執行個體實際數目取決於選取的範圍和數量。
   ...
   ![提交 SQL Database 保留容量購買前的螢幕擷取畫面](./media/sql-database-reserved-vcores/sql-reserved-vcores-purchase.png)
    ...
    |部署類型|您要為其購買保留容量的 SQL 部署類型。|
    |服務層|SQL Database 執行個體的服務層。
    |詞彙        |一年或三年。|
