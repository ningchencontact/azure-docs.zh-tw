...
如需有關分區資料庫概念的詳細資訊，請參閱[對 SQL Server 資料庫進行分區化](https://blog.pythian.com/sharding-sql-server-database/) \(英文\)。
...
5. 請遵循下列步驟，使用 Azure 入口網站建立作業：[建立和管理彈性資料庫作業](sql-database-elastic-jobs-create-and-manage.md)。
...
## <a name="failures-and-logs"></a>失敗和記錄

如果指令碼在多次嘗試之後失敗，工作就會記錄錯誤並繼續。 在工作結束後 (亦即針對群組中的所有資料庫執行)，您可以檢查其失敗的嘗試清單。 記錄會提供詳細資料以偵錯錯誤的指令碼。
...
- Azure 服務匯流排
  ...
  Azure 服務匯流排可用來協調 Azure 雲端服務內的工作。 請參閱 [服務匯流排價格](https://azure.microsoft.com/pricing/details/service-bus/)。

- Azure 儲存體

  Azure 儲存體帳戶可用來將診斷輸出記錄儲存於問題需要進一步偵錯的事件中 (請參閱[在 Azure 雲端服務和虛擬機器中啟用診斷](../cloud-services/cloud-services-dotnet-diagnostics.md))。 如需價格，請參閱 [Azure 儲存體價格](https://azure.microsoft.com/pricing/details/storage/)。
...
- ShardMapRefresh
  ...
  查詢分區對應，以判斷用來作為分區的所有資料庫
- ScriptSplit

  跨 ‘GO’ 陳述式來將指令碼分割成批次
- ExpandJob

  從以資料庫群組為目標的工作，針對每個資料庫建立子工作
- ScriptExecution

  使用定義的認證，針對特定資料庫執行指令碼
- Dacpac

  使用特定認證，將 DACPAC 套用至特定資料庫
