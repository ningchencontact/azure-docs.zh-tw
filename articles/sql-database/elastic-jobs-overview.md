---
title: Azure SQL 彈性資料庫作業 (預覽) |Microsoft Docs
...
藉由在 t-sql 或`sp_add_jobstep` `Add-AzSqlElasticJobStep -MaxParallelism` PowerShell 中設定預存程式的`@max_parallelism`參數, 來設定作業執行所在的並行資料庫數目。
