...
1. 在 [新增] 頁面上，選取 [Azure Marketplace] 區段中的 [資料庫]，然後按一下 [精選] 區段中的 [SQL Database]。

   ![建立空白資料庫](./media/sql-database-design-first-database/create-empty-database.png)

   1. 使用下列資訊填寫 **SQL Database** 表單，如上圖所示︰
   ...
   1. 按一下 [伺服器] 來使用現有伺服器，或為您的資料庫建立及設定新的伺服器。 選取伺服器，或按一下 [建立新伺服器] 並且在 [新伺服器] 表單中填寫下列資訊︰
   ...
   1. 按一下 [定價層] 可指定服務層、DTU 或 vCore 的數目，以及儲存體數量。 您可以瀏覽 DTU/vCore 數目的選項，以及可供您每個服務層使用的儲存體。 預設會選取**標準**[以 DTU 為基礎的購買模型](sql-database-service-tiers-dtu.md)，但您也可以選擇[以虛擬核心為基礎的購買模型](sql-database-service-tiers-vcore.md)。
      ...
      在選取服務層、DTU 數目和儲存體數量之後，按一下 [套用]。

   1. 為空白資料庫輸入 [定序] (在此教學課程中使用預設值)。 如需定序的詳細資訊，請參閱[定序](/sql/t-sql/statements/collations)。

1. 您現在已完成 **SQL Database** 表單，請按一下 [建立] 來佈建資料庫。 這個步驟可能需要幾分鐘的時間。
...
使用 [SQL Server Management Studio](/sql/ssms/sql-server-management-studio-ssms) 建立對 Azure SQL Database 伺服器的連線。

1. 開啟 SQL Server Management Studio。

1. 在 [連接到伺服器] 對話方塊中，輸入下列資訊：
   ...
   1. 按一下 [連接到伺服器] 對話方塊中的 [選項]。 在 [連線到資料庫] 區段中，輸入 *yourDatabase* 以連線到這個資料庫。

      ![連線到伺服器上的 DB](./media/sql-database-design-first-database/options-connect-to-db.png)  

   1. 按一下 [ **連接**]。 [物件總管] 視窗隨即在 SSMS 中開啟。

1. 在 [物件總管] 中，展開 [資料庫]，然後展開 *yourDatabase* 以檢視範例資料庫中的物件。
...
1. 在 [物件總管] 中展開 *yourDatabase* 之下的 **Tables** 節點，以查看您建立的資料表。
...
1. 以滑鼠右鍵按一下下列連結，將其儲存至 *sampleData* 資料夾。
...
1. 開啟命令提示字元視窗，並瀏覽至 *sampleData* 資料夾。

1. 執行下列命令，將範例資料插入資料表中，並以適用於您環境的值取代 *server*、*database*、*user*, 和 *password* 的值。
