...
計算成本會反映已為應用程式佈建的總計算容量。 在業務關鍵服務層中，我們會自動配置至少 3 個複本。 為了反映這項額外的計算資源配置，「業務關鍵」中的 vCore 價格大約會高出 2.7 倍。 基於相同原因，業務關鍵層中較高的每一 GB 儲存體價格，則是反映 SSD 儲存體的高 IO 和低延遲性。 同時，備份儲存體的成本並無不同，因為在這兩種情況下，我們都使用相同類別的標準儲存體。
...
以 vCore 為基礎的新服務層對於所有使用 100 個或更多 DTU 的彈性集區或資料庫，都會提供相當的效能選項。  我們將持續新增更多計算大小，以支援低於 100 DTU 的工作負載。
