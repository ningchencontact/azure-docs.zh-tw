...
本文描述設定及控制[作用中異地複寫](sql-database-active-geo-replication.md)和[自動容錯移轉群組](sql-database-auto-failover-group.md)的驗證需求。 它也提供設定使用者以存取次要資料庫的必要步驟。 最後，它也會描述如何在使用[異地還原](sql-database-recovery-using-backups.md#geo-restore)之後啟用復原資料庫的存取權。 如需復原選項的詳細資訊，請參閱 [商務持續性概觀](sql-database-business-continuity.md)。
