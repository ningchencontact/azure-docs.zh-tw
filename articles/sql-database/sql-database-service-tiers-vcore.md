...
|可用性|1 replica, no read-scale replicas|3 個複本、1 個[讀取規模複本](sql-database-read-scale-out.md)、<br/>zone-redundant high availability (HA)|1 read-write replica, plus 0-4 [read-scale replicas](sql-database-read-scale-out.md)|
|備份|[Read-access geo-redundant storage (RA-GRS)](../storage/common/storage-designing-ha-apps-with-ragrs.md), 7-35 days (7 days by default)|[RA-GRS](../storage/common/storage-designing-ha-apps-with-ragrs.md)、7-35 天 (預設為 7 天)|Snapshot-based backups in Azure remote storage. 還原時可使用這些快照集進行快速復原。 Backups are instantaneous and don't impact compute I/O performance. Restores are fast and aren't a size-of-data operation (taking minutes rather than hours or days).|
|記憶體內|不支援|支援的|不支援|
|||
...
|Gen5     |**佈建計算**<br>- Intel E5-2673 v4 (Broadwell) 2.3 GHz processors<br>- Provision up to 80 vCores (1 vCore = 1 hyper-thread)<br><br>**無伺服器計算**<br>- Intel E5-2673 v4 (Broadwell) 2.3 GHz processors<br>- Auto-scale up to 16 vCores (1 vCore = 1 hyper-thread)|**佈建計算**<br>- 5.1 GB per vCore<br>- Provision up to 408 GB<br><br>**無伺服器計算**<br>- Auto-scale up to 24 GB per vCore<br>- Auto-scale up to 48 GB max|
