...
採用 SQL Database 時，每個資料庫會彼此隔離並且可攜，且各自會在[以 DTU 為基礎的購買模型](sql-database-service-tiers-dtu.md)或[以虛擬核心為基礎的購買模型](sql-database-service-tiers-vcore.md) 內有自己的服務層和保證的效能層級。 SQL Database 會針對不同的需求提供不同的效能層級，並可讓資料庫集區化，以充分利用資和節省成本。
