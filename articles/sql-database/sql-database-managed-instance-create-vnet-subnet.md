---
title: Azure SQL Database 受控執行個體建立 VNet | Microsoft Docs
description: 本主題會講解如何建立可在其中部署 Azure SQL Database 受控執行個體的虛擬網路 (VNet)。
...
- 將受控執行個體連線到 Azure 資源  
  ...
  > [!Note]
  > 請先[決定受控執行個體的子網路大小](sql-database-managed-instance-determine-size-vnet-subnet.md)，再部署第一個執行個體，因為在其中放入資源後，就無法再調整子網路的大小。
...
您可以變更 VNet 與子網路的名稱，以及調整與您的網路資源關聯的 IP 位址。 一旦按下 [購買] 按鈕，此表單就會建立並設定您的環境。 若您不需要兩個子網路，可將預設子網路刪除。 
...
- 了解如何[為受控執行個體修改現有的虛擬網路](sql-database-managed-instance-configure-vnet-subnet.md) (英文)
