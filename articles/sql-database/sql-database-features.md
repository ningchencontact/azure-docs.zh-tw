...
| [檔案和檔案群組](https://docs.microsoft.com/sql/relational-databases/databases/database-files-and-filegroups) | 僅限主要檔案群組 | 是的。 系統會自動指派檔案路徑，且不能在 `ALTER DATABASE ADD FILE` 的[語句](sql-database-managed-instance-transact-sql-information.md#alter-database-statement)中指定檔案位置。  |
...
| 備份保留期 | 是的。 預設值為7天，最大值為35天。 | 是的。 預設值為7天，最大值為35天。 |
