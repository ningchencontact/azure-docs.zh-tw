...
- 支援最多 100 TB 的資料庫大小
- 近乎即時不論大小計算資源上沒有任何 IO 影響資料庫備份 （以 Azure Blob 儲存體中的檔案快照集為基礎）  
...
- **儲存體**：
  ...
  您設定超大規模資料庫時，不需要指定資料大小上限。 在超大規模層中，會依據資料庫實際使用的儲存體向您收費。 10 GB 和 100 TB，之間會以動態方式調整 10 GB 到 40 GB 之間的增量自動配置儲存體。  
...
| 問題 | 描述 |
| :---- | :--------- |
| 邏輯伺服器的 [管理備份] 窗格不會顯示超大規模資料庫將會篩選從 SQL server  | 超大規模有不同的方法，來管理備份，而且這類的長期保留和備份保留設定的時間點不會套用 / 都會失效。 據此，超大規模資料庫不會出現在 [管理備份] 窗格中。 |
