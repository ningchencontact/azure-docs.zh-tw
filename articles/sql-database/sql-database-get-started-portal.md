...
1. 在您資料庫的 SQL 資料庫分頁上，尋找並按一下左側功能表中的 [資料總管 (預覽)]

   ![尋找查詢編輯器](./media/sql-database-get-started-portal/find-query-editor.PNG)

2. 按一下 [登入]，檢閱登入資訊，然後按一下 [確定] 以使用 SQL 伺服器驗證搭配您稍早建立的伺服器管理員登入和密碼進行登入。

   ![登入](./media/sql-database-get-started-portal/login-menu.png)

3. 按一下 [確定] 以登入。

4. 以 **ServerAdmin** 身分進行驗證後，在查詢編輯器窗格中輸入下列查詢。
