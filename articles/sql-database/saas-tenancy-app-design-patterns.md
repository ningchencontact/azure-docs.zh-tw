...
![只有一個單一租用戶資料庫的獨立應用程式的程式設計。][image-standalone-app-st-db-111a]
...
![與每個租用戶資料庫的多租用戶應用程式的程式設計。][image-mt-app-db-per-tenant-132d]
...
![每位租用戶一個資料庫，使用彈性集區的多租用戶應用程式設計。][image-mt-app-db-per-tenant-pool-153p]
...
![具有分區化多租用戶資料庫的多租用戶應用程式的程式設計。][image-mt-app-sharded-mt-db-174s]
...
可辨識之租用戶群組的資源需求之間有很大差異時，混合式模型就會脫穎而出。  例如，假設不保證參與免費試用的租用戶能擁有與訂閱租用戶一樣高的效能等級。  此原則可能適用於免費試用階段的租用戶，而且這些租用戶即將儲存在所有免費試用租用戶共用的多租用戶資料庫中。  當免費試用租用戶訂閱基本服務層時，可以將租用戶移到另一個可能有較少租用戶的多租用戶資料庫。  支付進階服務層費用的訂閱者可以移到其擁有的全新單一租用戶資料庫。
