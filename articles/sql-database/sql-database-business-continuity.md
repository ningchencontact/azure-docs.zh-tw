...
ms.locfileid: "67442040"
---
# <a name="overview-of-business-continuity-with-azure-sql-database"></a>使用 Azure SQL Database 的商務持續性概觀

Azure SQL Database 中的**商務持續性**是指在面臨中斷時可讓您的企業持續運作的機制、原則和程序 (特別是其運算基礎結構)。 在大部分情況下，Azure SQL Database 會處理雲端環境中可能發生的干擾性事件，並且讓您的應用程式和商務程序持續執行。 不過，有一些無法處理 SQL Database 自動例如的干擾性事件：
...
- 通常由應用程式 Bug 或人為錯誤所造成的資料損毀或刪除。 這類失敗特定應用程式，通常無法偵測到的資料庫服務。
- 可能由天然災害所造成的資料中心中斷。 此案例需要某種程度的異地備援，讓應用程式能容錯移轉到替代資料中心。
- 升級或維護錯誤期間發生的非預期的問題的已規劃基礎結構維護或升級可能需要快速還原到先前的資料庫狀態。

若要減輕的本機硬體和軟體失敗，包括 SQL Database[高可用性架構](sql-database-high-availability.md)，可保證自動復原，從這些失敗達 99.995%可用性 SLA。  

若要防止資料遺失您的業務，SQL Database 會自動建立完整資料庫備份每週、 差異資料庫備份每隔 12 小時，和交易記錄備份每隔 5-10 分鐘的時間。 備份會儲存在 RA-GRS 儲存體中，至少 7 天的所有服務層。 Basic 以外的所有服務層都支援時間點還原的可設定的備份保留期限最多 35 天前。 

SQL Database 也提供數個商務持續性功能，您可以使用以減少各種非計劃的案例。 

- [時態表](sql-database-temporal-tables.md)可讓您從任何時間點還原資料列版本。
- [內建自動備份](sql-database-automated-backups.md)並[還原時間點](sql-database-recovery-using-backups.md#point-in-time-restore)可讓您還原至某個時間點的完整資料庫設定的保留期限內的時間為 35 天。
...
## <a name="recover-a-database-within-the-same-azure-region"></a>在相同的 Azure 區域內的資料庫復原

您可以使用自動資料庫備份至過去的時間將資料庫還原至某個點。 如此一來您可以從人為錯誤所造成的資料損毀復原。 時間點還原可讓您建立新的資料庫中相同的伺服器，表示資料損毀的事件之前的狀態。 對於大多數的資料庫還原作業會小於 12 小時。 可能需要較長的時間非常大或非常活躍的資料庫復原。 如需復原時間的詳細資訊，請參閱[資料庫復原時間](sql-database-recovery-using-backups.md#recovery-time)。 

如果最大支援的備份保留期限的時間點還原 (PITR) 不足，無法為您的應用程式，您可以擴充它藉由設定資料庫的長期保留 (LTR) 原則。 如需詳細資訊，請參閱[長期備份保留](sql-database-long-term-retention.md)。
...
- 最後，您可以快速地從中斷復原如果您已設定異地次要資料庫使用[作用中異地複寫](sql-database-active-geo-replication.md)該[自動容錯移轉群組](sql-database-auto-failover-group.md)針對您的資料庫或資料庫。 根據您選擇的這些技術，您可以使用手動或自動容錯移轉。 雖然容錯移轉本身只需要幾秒鐘的時間就能完成，服務將需要至少 1 小時才能啟動。 這是依據中斷的規模來確保容錯移轉之正當性的必要作法。 此外，基於非同步複寫的本質，容錯移轉可能會造成小規模的資料遺失。 

當您開發商務持續性計劃時，您必須了解應用程式在干擾性事件之後完全復原所需的最大可接受時間。 若要完全復原的應用程式所需的時間即為復原時間目標 (RTO)。 您也必須了解應用程式可以容忍最近資料更新 （時間間隔） 最大期限從非計劃性的干擾性事件復原時遺失。 潛在資料遺失就是所謂的復原點目標 (RPO)。

不同的復原方法提供不同層級的 RPO 和 RTO。 您可以選擇特定的復原方法，或使用 achicethe 復原的完整應用程式的方法的組合。 下表比較 RPO 和 RTO 的每個復原選項。
...
您可以選擇使用資料庫備份和作用中異地複寫取決於您的應用程式需求而定的組合。 如需獨立資料庫和彈性集區，使用這些商務持續性功能的設計考量的討論，請參閱 <<c0> [ 設計雲端災害復原應用程式](sql-database-designing-cloud-solutions-for-disaster-recovery.md)和[彈性集區災害復原策略](sql-database-disaster-recovery-strategies-for-applications-with-elastic-pool.md)。
