...
| 資料行層級加密 (Azure Data Services)| 是 | 透過[Always Encrypted](sql-database-always-encrypted.md)。 |
...
| 網路隔離，而且防火牆支援| 是 | 防火牆在這兩個資料庫和伺服器層級;網路隔離[受管理的執行個體](sql-database-managed-instance.md)只 |
| 強制通道的支援| 是 | [受控執行個體](sql-database-managed-instance.md)經由[Azure ExpressRoute](../expressroute/index.yml) VPN |
