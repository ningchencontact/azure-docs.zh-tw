...
| 名稱（Blob） | 名稱（事件中樞/Log Analytics） | 說明 | Blob 類型 | 事件中樞/Log Analytics 類型 |
|-------------|---------------------------------|-------------|-----------|-------------------------------|
...
| data_sensitivity_information | data_sensitivity_information_s | 由經過審核的查詢所傳回的資訊類型和敏感度標籤，是根據資料庫中的分類資料行。 深入瞭解[Azure SQL Database 資料探索與分類](sql-database-data-discovery-and-classification.md) | Nvarchar （4000） | 字串 |
...
| is_column_permission | is_column_permission_s | 表示這是否為數據行層級許可權的旗標。 1 = true，0 = false | bit | 字串 |
| N/A | is_server_level_audit_s | 表示此 audit 是否在伺服器層級的旗標 | N/A | 字串 |
| object_ 識別碼 | object_id_d | 發生 audit 之實體的識別碼。 這包括：伺服器物件、資料庫、資料庫物件和架構物件。 如果實體是伺服器本身，或如果不是在物件層級執行，則為0 | int | int |
| object_name | object_name_s | 發生 audit 之實體的名稱。 這包括：伺服器物件、資料庫、資料庫物件和架構物件。 如果實體是伺服器本身，或如果不是在物件層級執行，則為0 | sysname | 字串 |
| permission_bitmask | permission_bitmask_s | 適用時，會顯示已授與、拒絕或撤銷的許可權 | Varbinary （16） | 字串 |
| response_rows | response_rows_d | 結果集中傳回的資料列數目 | bigint | int |
| schema_name | schema_name_s | 發生動作的架構內容。 若為在架構外發生的審核，則為 Null | sysname | 字串 |
...
| 句 | statement_s | 已執行的 t-sql 語句（如果有的話） | Nvarchar （4000） | 字串 |
| 均 | succeeded_s | 指出觸發事件的動作是否成功。 對於登入和批次以外的事件，這只會報告許可權檢查成功或失敗，而不是作業。 1 = 成功，0 = 失敗 | bit | 字串 |
| target_database_principal_id | target_database_principal_id_d | 授與/拒絕/撤銷作業執行所在的資料庫主體。 0（如果不適用） | int | int |
| target_database_principal_name | target_database_principal_name_s | 動作的目標使用者。 Null （如果不適用） | 字串 | 字串 |
| target_server_principal_id | target_server_principal_id_d | 授與/拒絕/撤銷作業執行所在的伺服器主體。 如果不適用，則傳回0 | int | int |
