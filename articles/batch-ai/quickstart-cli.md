...
本快速入門說明如何使用 Microsoft Cognitive Toolkit，建立 GPU 叢集及執行訓練作業。

訓練指令碼 [ConvNet_MNIST.py](https://github.com/Azure/BatchAI/blob/master/recipes/CNTK/CNTK-GPU-Python/CNTK-GPU-Python.ipynb) 可在 Batch AI GitHub 頁面取得。 此指令碼可在手寫數字的 MNIST 資料庫上訓練卷積神經網路。
...
* 存取 Azure CLI 2.0 (2.0.31 版或更高版本)。 您可以使用 [Cloud Shell](https://docs.microsoft.com/en-us/azure/cloud-shell/overview) 中提供的 Azure CLI 2.0，或遵循[這些指示](https://docs.microsoft.com/en-us/cli/azure/install-azure-cli?view=azure-cli-latest)在本機進行安裝。

# <a name="cloud-shell-only"></a>僅限 Cloud Shell
...
下列命令會使用 Ubuntu DSVM 作為作業系統映像，以建立單一節點 GPU 叢集 (VM 大小為 Standard_NC6)：

```azurecli
az batchai cluster create -n nc6 -g batchai.quickstart -s Standard_NC6 -i UbuntuDSVM -t 1 --generate-ssh-keys
```

Ubuntu DSVM 可讓您在 Docker 容器中執行任何訓練作業，以及直接在 VM 上執行最受歡迎的深入學習架構。

`--generate-ssh-keys` 選項會告訴 Azure CLI 產生私人和公用 ssh 金鑰 (如果您沒有這些金鑰)。 您可以使用目前的使用者名稱和所產生的 ssh 金鑰來存取叢集節點。

請注意，如果您使用 Cloud Shell，請將 ~/.ssh 資料夾備份到某些永久儲存體。
...
下列命令會在與 batchai.repices 資源群組相同的區域中建立新的儲存體帳戶。 使用唯一的儲存體帳戶名稱更新此命令。

```azurecli
az storage account create -n <storage account name> --sku Standard_LRS -g batchai.quickstart
```

如果所選的儲存體帳戶名稱無法使用，則以上命令會報告對應的錯誤。 在此情況下，請選擇其他名稱，然後重試。

# <a name="data-deployment"></a>資料部署
...
下列命令會建立 Azure Blob 容器 `data` 並將訓練資料複製到 `mnist_cntk` 資料夾中：
```azurecli
...
* `stdOutErrPathPrefix` - Batch AI 將在該路徑中建立包含作業輸出和記錄的目錄；
...
* `<AZURE_BATCHAI_STORAGE_ACCOUNT>` 告知在作業提交期間，將透過 --storage-account-name 參數或您電腦上的 `AZURE_BATCHAI_STORAGE_ACCOUNT` 環境變數來指定儲存體帳戶名稱。
...
訓練指令碼會在標準輸出目錄的 `stderr.txt` 檔案中報告訓練進度。 您可以使用下列命令來監視進度：
...
作業會將所產生的模型檔案儲存在 `id` 屬性等於 `MODEL` 的輸出目錄中，您可以使用下列命令來列出模型檔案並取得下載 URL：
...
或者，您可以使用入口網站或 Azure 儲存體總管來檢查所產生的檔案。 為了區分來自不同作業的輸出，Batch AI 會為每個作業建立唯一的資料夾結構。 您可以使用所提交作業的 `jobOutputDirectoryPathSegment` 屬性，尋找包含輸出之資料夾的路徑：
