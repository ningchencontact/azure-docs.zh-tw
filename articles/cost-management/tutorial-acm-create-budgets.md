...
監看式[如何建立要監視您的消費使用 Azure 成本管理預算](https://www.youtube.com/watch?v=ExIVG_Gr45A)影片以了解如何建立預算，以及在 Azure 中監視消費。
...
如果您有隨用隨付、MSDN 或 Visual Studio 訂用帳戶，則您發票的計費期間可能與行事曆月份不同。 對於這些類型的訂用帳戶和資源群組中中,，您可以建立對齊預算發票期間或行事曆月。 若要建立符合您的發票期間的預算，選取 重設時間計費月份、 計費季或年計費。 若要建立日曆月份調整預算，選取的重設期間的每月、 每季或每年。
