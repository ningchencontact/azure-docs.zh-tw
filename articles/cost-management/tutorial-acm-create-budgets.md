...
當您建立或編輯訂用帳戶或資源群組範圍的預算時，可以將它設定為呼叫動作群組。 當達到預算限額時，動作群組可以執行各種不同的動作。 目前只有訂用帳戶和資源群組範圍支援動作群組。 如需有關動作群組的詳細資訊，請參閱在[Azure 入口網站中建立和管理動作群組](../azure-monitor/platform/action-groups.md)。 如需使用以預算為基礎的自動化和動作群組的詳細資訊，請參閱[使用 Azure 預算管理成本](../billing/billing-cost-management-budget-scenario.md)。
...
## <a name="edit-an-existing-budget"></a>編輯現有的預算
如果您想要變更現有的預算，請流覽至 [主要預算] 視窗，然後選取您想要編輯的預算。 在 [預算詳細資料] 視窗中，選取 [**編輯預算**] 並進行所需的變更，然後選取 [**儲存**]。

![顯示編輯預算的範例](./media/tutorial-acm-create-budgets/edit-budget.png)
