...
| 維度 | Amazon CUR header | 範圍 | 註解 |
| --- | --- | --- | --- |
| 可用性區域 | lineitem/AvailabilityZone | 所有 |   |
| Location | product/Region | 所有 |   |
| 計量器 |   | 所有 |   |
| Meter category | lineItem/ProductCode | 所有 |   |
| Meter subcategory | lineitem/UsageType | 所有 |   |
| 作業 | lineItem/Operation | 所有 |   |
| 資源 | lineItem/ResourceId | 所有 |   |
| 資源類型 | product/instanceType | 所有 | If product/instanceType is null, lineItem/UsageType is used. |
| ResourceGuid | N/A | 所有 | Azure meter GUID. |
| 服務名稱 | product/ProductName | 所有 | If product/ProductName is null, lineItem/ProductCode is used. |
| 服務層級 |   |   |   |
| 訂用帳戶 ID | lineItem/UsageAccountId | Consolidated account and management group |   |
| 訂用帳戶名稱 | N/A | Consolidated account and management group | Account names are collected using the AWS Organization API. |
| Tag | resourceTags/\* | 所有 | The _user:_ prefix is removed from user-defined tags to allow cross-cloud tags. The _aws:_ prefix is left intact. |
| Billing account ID | bill/PayerAccountId | 管理群組 |   |
| Billing account name | N/A | 管理群組 | Account names are collected using the AWS Organization API. |
| 提供者 | N/A | 管理群組 | Either AWS or Azure. |
