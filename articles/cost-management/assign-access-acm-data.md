...
目前，Azure 成本管理具有有限的支援進行跨租用戶驗證。 在某些情況下您嘗試驗證跨租用戶，您可能會收到**拒絕存取**成本分析中的錯誤。 如果您設定另一個租用戶的訂用帳戶的角色型存取控制 (RBAC)，然後嘗試檢視成本資料，可能會發生此問題。

*若要解決此問題*:設定跨租用戶 RBAC 之後，請等候一小時。 然後，嘗試在 成本分析檢視成本或成本管理存取權授與這兩個租用戶中的使用者。  
