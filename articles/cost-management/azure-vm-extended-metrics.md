...
例如，您可以監視 Azure VM 的 CPU % 和記憶體 %。 Azure VM 計量會對應到使用中的_CPU_和 _\Memory 百分比\% 認可的位元組數_。
...
標準計量是主機電腦計量。 _CPU 計量百分比_是其中一個範例。 來賓 VM 也有一些基本計量，它們也稱為擴充計量。 擴充計量的範例包括_使用中的 \Memory\% 認可的位元組_和 _\Memory\Available 的位元組_。
