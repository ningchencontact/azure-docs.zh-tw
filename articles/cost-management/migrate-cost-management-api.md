...
| 目的 | Microsoft Customer Agreement (MCA) API |
| --- | --- |
...
| 計量識別碼 | meterId | Exact string values differ. |
...
| 優惠名稱 | None | Use publisherName and productOrderName. |
| OfferId | None | &nbsp;  |
| 訂單編號 | None | &nbsp;  |
...
Make another GET call to the location. The response to the GET call is the same until the operation reaches a completion or failure state. When completed, the response to the GET call location returns the download URL. Just as if the operation was executed at the same time. 以下是範例：
...
| Old property | New property | 注意 |
| --- | --- | --- |
| billingPeriodId  | _Not applicable_ | 不適用。 For Microsoft Customer Agreements, the invoice and associated price sheet replaced the concept of billingPeriodId. |
| meterId  | meterId | &nbsp;  |
...
| --- | --- | --- |
| 計量識別碼 | _meterId_ | 計量的唯一識別碼。 Same as meterId. |
| Meter name | meterName | 計量的名稱。 Meter represents the Azure service deployable resource. |
| Meter category  | service | 計量的分類類別名稱。 Same as the service in the Microsoft Customer Agreement Price Sheet. Exact string values differ. |
| Meter subcategory | meterSubCategory | 計量的子分類類別名稱。 Based on the classification of high-level feature set differentiation in the service. For example, Basic SQL DB vs Standard SQL DB. |
| Meter region | meterRegion | &nbsp;  |
| 單位 | _Not applicable_ | Can be parsed from unitOfMeasure. |
| 測量單位 | unitOfMeasure | &nbsp;  |
...
| 單位價格 | unitPrice | Microsoft Customer Agreement unit price. |
...
| 內含數量 | includedQuantity | Not applicable to services in Microsoft Customer Agreements. Show with values of zero. |
...
| offerId | 不適用。 Corresponds to productOrderName in MCA. |
| meterCategory  | 不適用。 Corresponds to Service in MCA. |
| unit | 不適用。 Can be parsed from unitOfMeasure. |
...
Reserved Instance Purchase Recommendations APIs provide virtual machine usage over the last 7, 30, or 60 days. APIs also provide reservation purchase recommendations. 這些包括：
