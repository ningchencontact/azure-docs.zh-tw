...
1. 填寫含有下列資訊的基本叢集詳細資料。
    ...
    **設定** | **建議的值** | **欄位描述**
    ...
    | 可用性區域 | *1*、*2* 和/或 *3* | 針對支援區域的區域備援，請選取您的執行個體將位於哪些區域 (選擇性)。 依預設會在相同的資料中心建立叢集節點。 請使用 [Azure可用性區域](/azure/availability-zones/az-overview)，將叢集執行個體放在相同區域的各個可用性區域中。 選取數個可用性區域，可以消除單一失敗點並確保高可用性。 
