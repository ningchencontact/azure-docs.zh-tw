---
title: 使用 Azure DevOps Project 將 ASP.NET 應用程式部署至 Azure SQL Database | VSTS 教學課程
...
Azure DevOps 專案提供簡化的體驗，讓您可以自備現有程式碼和 Git 存放庫，或者從其中一個範例應用程式中進行選擇，以在 Azure 中建立持續整合 (CI) 和持續傳遞 (CD) 管線。  DevOps 專案會自動建立 Azure 資源 (例如 Azure SQL Database)、在 VSTS 中建立及設定發行管線 (VSTS 中包含 CI 的組建定義)，也會設定 CD 的發行定義，然後建立用於監視的 Azure Application Insights 資源。

您將：

> [!div class="checklist"]
> * 為 ASP.NET 應用程式和 Azure SQL Database 建立 Azure DevOps 專案
...
## <a name="create-an-azure-devops-project-for-an-aspnet-app-and-azure-sql-database"></a>為 ASP.NET 應用程式和 Azure SQL Database 建立 Azure DevOps 專案

Azure DevOps Project 會在 VSTS 中建立 CI/CD 管線。  您可以建立**新的 VSTS** 帳戶，或使用**現有的帳戶**。  Azure DevOps Project 也會在您選擇的 **Azure 訂用帳戶**中建立 **Azure 資源**，例如 Azure SQL Database。
...
1. 此程序需要幾分鐘的時間才能完成。  完成後，Azure 入口網站中會載入 Azure DevOps **專案儀表板**。  您可以直接從 **Azure 入口網站**中的 [所有資源] 瀏覽至 [Azure DevOps 專案儀表板]。  若要檢視執行中的應用程式，請在儀表板右側選取 [瀏覽]。
...
Azure DevOps Project 會自動建立並設定必要的步驟，以從您的 VSTS 帳戶部署至 Azure 訂用帳戶。  這些步驟包括設定 Azure 服務連線，以對您的 Azure 訂用帳戶驗證 VSTS。  自動化也會建立 VSTS 發行定義，且發行會將 CD 提供給 Azure。  請遵循下列步驟，對 VSTS 發行定義進行更深入的檢查。
...
1. Azure DevOps 專案設定了隨機的 SQL 密碼，並將此密碼用於發行定義。  在瀏覽器左側，選取 [變數]。 

1. **只有已變更 SQL Server 密碼時，才需要執行此步驟。**  目前有單一**密碼**變數。  在 [值] 文字方塊右側，選取 [掛鎖] 圖示。  **輸入**新密碼，然後選取 [儲存]。

1. 在瀏覽器左側選取 [工作]，然後選擇您的**環境**。  

1. 工作是您的部署程序所執行的活動，會在 [階段] 中分組。  此發行定義有單一階段。  此階段包含 [Azure App Service 部署] 和 [Azure SQL Database 部署] 工作。
...
您現在已準備好要與小組共同對應用程式執行 CI/CD 程序，以將您最新的工作自動部署至網站。  對 VSTS git 存放庫進行的每項變更都會在 VSTS 中起始一個組建，且 VSTS Release Management 定義會對 Azure 執行部署。  請遵循下列步驟或使用其他技術，來認可您對存放庫的變更。  程式碼變更會啟動 CI/CD 程序，並自動將您的新變更部署至 Azure。
...
1. 現在，您可以選擇性地使用 **SQL Server Management Studio** 或 **Visual Studio** 之類的用戶端工具連線至 Azure SQL Server 和資料庫。  請使用 [伺服器名稱] 屬性進行連線。
 ...
 > [!NOTE]
 > 如果您變更 SQL 登入的密碼，則必須依照**檢查 VSTS CD Release Management 定義**一節中的說明，變更 VSTS 發行定義變數中的密碼
...
如果您要測試，您可以清除資源，以避免產生費用。  當您不再需要時，可以使用 Azure DevOps 專案儀表板上的 [刪除] 功能，將 Azure SQL Database 和在本教學課程中建立的相關資源刪除。  **請注意**，刪除功能會終結 Azure DevOps Project 在 Azure 和 VSTS 中建立的資料，且一旦終結後即無法再擷取。
