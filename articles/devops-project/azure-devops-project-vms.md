---
title: 使用 Azure DevOps Project 將 ASP.NET 應用程式部署至 Azure 虛擬機器 | VSTS 教學課程
...
Azure DevOps 專案提供簡化的體驗，讓您可以自備現有程式碼和 Git 存放庫，或者從其中一個範例應用程式中進行選擇，以在 Azure 中建立持續整合 (CI) 和持續傳遞 (CD) 管線。  DevOps 專案會自動建立 Azure 資源 (例如新的 Azure 虛擬機器)、在 VSTS 中建立及設定發行管線 (VSTS 中包含 CI 的組建定義)，也會設定 CD 的發行定義，然後建立用於監視的 Azure Application Insights 資源。

您將：

> [!div class="checklist"]
> * 為 ASP.NET 應用程式建立 Azure DevOps 專案
...
## <a name="create-an-azure-devops-project-for-an-aspnet-app"></a>為 ASP.NET 應用程式建立 Azure DevOps 專案

Azure DevOps Project 會在 VSTS 中建立 CI/CD 管線。  您可以建立**新的 VSTS** 帳戶，或使用**現有的帳戶**。  Azure DevOps Project 也會在您選擇的 **Azure 訂用帳戶**中建立 **Azure 資源**，例如虛擬機器。
...
1. 選取您的 **Azure 訂用帳戶**。  您可以選擇性地選取 [變更] 連結，然後輸入更多組態詳細資料，例如變更 Azure 資源的位置。
 
1. 為新的 Azure 虛擬機器資源輸入 [虛擬機器名稱]、[使用者名稱] 和 [密碼]，然後選擇 [完成]。

1. Azure 虛擬機器需要幾分鐘的時間才會準備就緒。  系統會在您 VSTS 帳戶下的存放庫中設定範例 ASP.NET 應用程式、執行組建和發行，然後將您的應用程式部署至新建立的 Azure VM。  

    完成後，Azure 入口網站中會載入 Azure DevOps **專案儀表板**。  您可以直接從 **Azure 入口網站**中的 [所有資源] 瀏覽至 [Azure DevOps 專案儀表板]。  

    此儀表板會顯示您的 VSTS **程式碼存放庫**、**VSTS CI/CD 管線**，和您**在 Azure 中執行的應用程式**。    

    ![儀表板檢視](_img/azure-devops-project-vms/dashboardnopreview.png)

1. Azure DevOps 專案會自動設定 CI 組建和發行觸發程序，而將任何程式碼變更自動部署至您的存放庫。  您可以在 VSTS 中進一步設定其他選項。  若要檢視執行中的應用程式，請在儀表板右側選取 [瀏覽]。
...
1. 將滑鼠游標移至 [狀態] 欄位旁的組建定義右側。 選取隨即出現的 [省略符號]。  這個動作會開啟功能表，您可以用它來執行數個活動，例如**將新的組建排入佇列**、**暫停組建**，以及**編輯組建定義**。
...
1. 選取 [觸發程序]。  Azure DevOps 專案已自動建立 CI 觸發程序，且每次對存放庫的認可都會啟動新的組建。  選擇性地選擇要在 CI 程序中包含還是排除分支。
...
Azure DevOps Project 會自動建立並設定必要的步驟，以從您的 VSTS 帳戶部署至 Azure 訂用帳戶。  這些步驟包括設定 Azure 服務連線，以對您的 Azure 訂用帳戶驗證 VSTS。  自動化也會建立 VSTS 發行定義，且此發行定義會將 CD 提供給 Azure 虛擬機器。  請遵循下列步驟，對 VSTS 發行定義進行更深入的檢查。
...
1. 工作是您的部署程序所執行的活動，會在 [階段] 中分組。  此發行定義有兩個**階段**。  第一個階段包含 [Azure 資源群組部署] 工作，會設定要部署的 VM，並將新的 VM 新增至 [VSTS 部署群組]。  VSTS 中的 VM 部署群組會管理**部署目標**機器的邏輯群組。
...
您現在已準備好要與小組共同對應用程式執行 CI/CD 程序，以將您最新的工作自動部署至網站。  對 VSTS git 存放庫進行的每項變更都會在 VSTS 中起始一個組建，且 VSTS Release Management 定義會對您的 Azure VM 執行部署。  請遵循下列步驟或使用其他技術，將變更認可至您的存放庫。  程式碼變更會起始 CI/CD 程序，並自動將您的新變更部署至 Azure VM 上的 IIS 網站。

1. 從 VSTS 功能表中選取 [程式碼]，並瀏覽至您的存放庫。

1. 瀏覽至 **Views\Home** 目錄，選取 **Index.cshtml** 檔案旁的**省略符號**，然後選擇 [編輯]。

1. 對檔案進行變更，例如，變更其中一個 **div 標記**內的某些文字。  在右上方，選取 [認可]。  再次選取 [認可] 以推送您的變更。 

1. 不久之後，**VSTS 中會啟動一個組建**，然後會執行發行以部署變更。  請使用 DevOps 專案儀表板監視**組建狀態**，或使用您的 VSTS 帳戶在瀏覽器中檢視。
...
如果您要測試，您可以清除資源，以避免產生費用。  當您不再需要時，可以使用 Azure DevOps 專案儀表板上的 [刪除] 功能，將 Azure 虛擬機器和在本教學課程中建立的相關資源刪除。  **請注意**，刪除功能會終結 Azure DevOps Project 在 Azure 和 VSTS 中建立的資料，且一旦終結後即無法再擷取。
