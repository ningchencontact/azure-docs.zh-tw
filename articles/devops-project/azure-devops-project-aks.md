---
title: 使用 Azure DevOps Project 將 ASP.NET Core 應用程式部署至 Azure Kubernetes Service (AKS) | VSTS 教學課程
description: DevOps Project 可讓您輕鬆地開始使用 Azure。 Azure DevOps Project 可讓您以幾個快速步驟輕鬆地將 ASP.NET Core 應用程式部署至 Azure Kubernetes Service (AKS)。
ms.author: mlearned
ms.manager: douge
ms.prod: devops
...
Azure DevOps 專案提供簡化的體驗，讓您可以自備現有程式碼和 Git 存放庫，或者從其中一個範例應用程式中進行選擇，以在 Azure 中建立持續整合 (CI) 和持續傳遞 (CD) 管線。  DevOps 專案會自動建立 Azure 資源 (例如 AKS)、在 VSTS 中建立及設定發行管線 (VSTS 中包含 CI 的組建定義)，也會設定 CD 的發行定義，然後建立用於監視的 Azure Application Insights 資源。

您將：

> [!div class="checklist"]
> * 為 ASP.NET Core 應用程式和 AKS 建立 Azure DevOps 專案
...
## <a name="create-an-azure-devops-project-for-an-aspnet-core-app-and-aks"></a>為 ASP.NET Core 應用程式和 AKS 建立 Azure DevOps 專案

Azure DevOps Project 會在 VSTS 中建立 CI/CD 管線。  您可以建立**新的 VSTS** 帳戶，或使用**現有的帳戶**。  Azure DevOps Project 也會在您選擇的 **Azure 訂用帳戶**中建立 **Azure 資源**，例如 AKS 叢集。
...
1. 此程序需要幾分鐘的時間才能完成。  系統會在您 VSTS 帳戶下的存放庫中設定範例 ASP.NET Core 應用程式、建立 AKS 叢集、執行組建和發行，然後將您的應用程式部署至 Azure。  

    完成後，Azure 入口網站中會載入 Azure DevOps **專案儀表板**。  您可以直接從 **Azure 入口網站**中的 [所有資源] 瀏覽至 [Azure DevOps 專案儀表板]。  

    此儀表板會顯示您的 VSTS **程式碼存放庫**、**VSTS CI/CD 管線**和 **AKS 叢集**。  您可以在 VSTS 中進一步設定其他選項。  若要檢視執行中的應用程式，請在儀表板右側選取 [瀏覽]。
...
1. 在 DevOps 專案儀表板的右側，選取 [Kubernetes 服務]。
...
Azure DevOps Project 會自動建立並設定必要的步驟，以從您的 VSTS 帳戶部署至 Azure 訂用帳戶。  這些步驟包括設定 Azure 服務連線，以對您的 Azure 訂用帳戶驗證 VSTS。  自動化也會建立 VSTS 發行定義，且發行定義會將 CD 提供給 Azure。  請遵循下列步驟，對 VSTS 發行定義進行更深入的檢查。
...
1. 在 [置放] 圖示右側，選取 [持續部署觸發程序] **圖示** (呈閃電狀)。這個發行定義具有已啟用的 CD 觸發程序。  每當有新的組建成品可用時，觸發程序就會建立部署。  您可以選擇性地停用觸發程序，讓您的部署需要手動執行。 
...
如果您要測試，您可以清除資源，以避免產生費用。  當您不再需要時，可以使用 Azure DevOps 專案儀表板上的 [刪除] 功能，將 Azure Kubernetes 叢集和在本教學課程中建立的相關資源刪除。  **請注意**，刪除功能會終結 Azure DevOps Project 在 Azure 和 VSTS 中建立的資料，且一旦終結後即無法再擷取。
...
> [使用 Kubernetes 儀表板](https://docs.microsoft.com/vsts/pipelines/release/define-multistage-release-process?view=vsts)
