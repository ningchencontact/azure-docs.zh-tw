...
ms.locfileid: "37966899"
---
# <a name="create-a-cicd-pipeline-for-php-with-the-azure-devops-project"></a>使用 Azure DevOps Project 建立適用於 PHP 的 CI/CD 管線

Azure DevOps Project 提供簡化的體驗，在 Visual Studio Team Services (VSTS，這是 Microsoft 針對 Azure 提供的 DevOps 解決方案) 中建立 Azure 資源、並設定適用於 PHP 應用程式的持續整合 (CI) 和持續傳遞 (CD) 管線。  
...
1. 建立**新的** VSTS 帳戶，或選擇**現有的**帳戶。  為 VSTS 專案選擇 [名稱]。  選取您應用程式的 [Azure 訂用帳戶]、[位置]，然後選擇 [名稱]。  完成時，選擇 [完成]。
    ...
    ![輸入 VSTS 資訊](_img/azure-devops-project-php/vstsazureinfo.png)
...
Azure DevOps 專案會自動設定 CI 組建和發行觸發程序。  您現在已準備好利用 CI/CD 程序與小組共同進行 PHP 應用程式的作業，這個程序會自動將您的最新工作部署到網站上。
...
1. 在瀏覽器的左側，巡覽至 **resources/views/welcome.blade.php** 檔案。
