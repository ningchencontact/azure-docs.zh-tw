...
1. 選取 .NET 應用程式範例。 .NET 範例包含開放原始碼 ASP.NET Framework 或跨平台.NET Core Framework 的選項。

    ![.NET Framework](_img/azure-devops-project-aspnet-core/chooselanguagedotnet.png)

1. 選取 .NET Core 應用程式架構。  
    此範例為 ASP.NET Core MVC 應用程式。
    
2. 選取 [下一步] 。  
    Windows 上的 Web 應用程式是預設的部署目標。  您可以選擇性地選擇 Linux 上的 Web 應用程式或用於容器的 Web 應用程式。  您先前選擇的應用程式架構，會指出這裡可用的 Azure 服務部署目標類型。  
...
1. 在瀏覽器的左側，移至 **Views/Home/index.cshtml** 檔案。

1. 選取 [編輯]，並進行 h2 標題的變更。 例如，輸入**立即開始使用 Azure DevOps Projects** 或進行其他變更。
