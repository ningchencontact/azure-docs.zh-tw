...
ms.locfileid: "47407598"
---
# <a name="create-a-cicd-pipeline-for-net-with-the-azure-devops-project"></a>使用 Azure DevOps Project 建立適用於 .NET 的 CI/CD 管線

使用 **Azure DevOps Project** 為您的 .NET Core 或 ASP.NET 應用程式設定持續整合 (CI) 與持續傳遞 (CD)。  Azure DevOps Project 可簡化 Azure DevOps Services 建置及發行管線的初始設定。
...
1. 在幾分鐘後，Azure 入口網站中便會載入 **DevOps Project 儀表板**。  系統會在您 Azure DevOps Services 組織中的存放庫中設定範例應用程式、執行建置，然後將您的應用程式部署到 Azure。  此儀表板可顯示您的 **程式碼存放庫**、**Azure DevOps Services CI/CD 管線**，與您**在 Azure 中的應用程式**。  若要檢視執行中的應用程式，請在儀表板右側選取 [瀏覽]。
...
1. 在瀏覽器中，瀏覽到 [Azure DevOps Project 儀表板]。  您現在應該會看到正在進行中的建置。  您所做的變更會透過 Azure DevOps Services CI/CD 管線自動建置及部署。

## <a name="examine-the-azure-devops-services-cicd-pipeline"></a>檢查 Azure DevOps Services CI/CD 管線

Azure DevOps Project 會在 Azure DevOps Services 組織中自動設定完整的 Azure DevOps Services CI/CD 管線。  瀏覽管線，並視需要進行自訂。  依照下列步驟執行以熟悉 Azure DevOps Services 建置及發行管線。
...
1. 從這個檢視**檢查建置管線的各種工作**。  建置會執行各種工作，例如從 Azure Repos Git 存放庫擷取來源、還原相依性，以及發佈用來進行部署的輸出。
...
1. 選取 [觸發程序]。  Azure DevOps 專案已自動建立 CI 觸發程序，且每次對存放庫的認可都會建立新的建置。  您可以選擇性地選擇要在 CI 程序中包含還是排除分支。
