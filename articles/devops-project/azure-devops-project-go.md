...
1. 免費建立**新的** VSTS 帳戶，或選擇**現有的**帳戶。  為 VSTS 專案選擇 [名稱]。  選取您應用程式的 [Azure 訂用帳戶]、[位置]，然後選擇 [名稱]。  完成時，選擇 [完成]。
    ...
    ![輸入 VSTS 資訊](_img/azure-devops-project-go/vstsazureinfo.png)
...
1. 在瀏覽器的左側，巡覽至 **views/index.html** 檔案。

1. 選取 [編輯]，來進行部分文字的變更。  例如，修改其中一個 div 標籤內的文字。

1. 選擇 [認可]，然後儲存您的變更。

1. 在瀏覽器中，巡覽至 [Azure DevOps 專案儀表板]。  您現在應該會看到正在進行中的組建。  您所做的變更會透過 VSTS CI/CD 管線自動建置及部署。
