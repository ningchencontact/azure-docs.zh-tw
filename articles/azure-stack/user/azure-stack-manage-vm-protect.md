...
若要防止資料遺失或意外的停機狀況，您需要為使用者應用程式或其資料實作備份復原或災害復原計劃。 此計劃或許專屬於個別應用程式，但都遵循根據組織完整業務續航力和災害復原 (BC/DR) 策略所建立的架構。 [Azure Stack：商務持續性和災害復原的考量](https://aka.ms/azurestackbcdrconsiderationswp)是個不錯的起點。
...
## <a name="considerations-for-iaas-vms"></a>IaaS VM 的考量
安裝在 IaaS VM 中的作業系統將限制您可以使用哪些產品來保護其包含的資料。 針對以 Windows 為主的 IaaS VM，您可以使用 Microsoft 和合作夥伴的產品來保護資料。 針對以 Linux 為主的 IaaS VM，唯一的選項是使用協力廠商產品。 請參閱[此資料表適用於所有 BC/DR 合作夥伴，其中包含 Azure Stack 已驗證的產品](https://aka.ms/azurestackbcdrpartners) \(英文\)。
