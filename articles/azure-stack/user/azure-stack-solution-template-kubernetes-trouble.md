---
title: 針對 Azure Stack Kubernetes (K8) 部署進行疑難排解 | Microsoft Docs
description: 了解如何針對 Azure Stack Kubernetes (K8) 部署進行疑難排解。
...
在進入叢集的疑難排解步驟之前，請先檢閱 Azure Stack Kubernetes 叢集部署程序。 此部署會使用 Azure Resource Manager 解決方案範本來建立 VM，並為叢集安裝 ACS 引擎。
...
1. 從 Marketplace 項目收集輸入參數。
    ...
    -  **使用者名稱：** 這是屬於 Kubernetes 叢集和 DVM 的 Linux 虛擬機器的使用者名稱。
...
您可以在支援 Kubernetes 叢集的 VM 上收集記錄。 您也可以檢閱部署記錄。 此外，您可能也需要詢問 Azure Stack 系統管理員來確認您使用的 Azure Stack 版本，並從 Azure Stack 取得與部署相關的記錄。

1. 檢閱[部署狀態](#review-deployment-status)，並從 Kubernetes 叢集中的主要節點[擷取記錄](#get-logs-from-a-vm)。
2. 您需要使用最新版的 Azure Stack。 如果您不確定 Azure Stack 版本，請連絡 Azure Stack 系統管理員。 Kubernetes Cluster Marketplace 項目 0.3.0 需要 Azure Stack 1808 版或更新版本。
3.  檢閱 VM 建立檔案。 您可能遇到了下列問題：  
    - 公開金鑰可能無效。 檢閱您已建立的金鑰。  
    - 建立 VM 時可能觸發了內部錯誤，或觸發了建立錯誤。 錯誤可能由數個因素造成，包括 Azure Stack 訂用帳戶的容量限制。
    - VM 的完整網域名稱 (FQDN) 是否以重複的前置詞開頭？
...
5. 如果您有任何關於部署的問題，可以張貼您的問題，或參閱是否有人已在 [Azure Stack 論壇](https://social.msdn.microsoft.com/Forums/azure/home?forum=azurestack)中回答過的問題。 

## <a name="review-deployment-status"></a>檢閱部署狀態

您可以在部署 Kubernetes 叢集時檢閱部署狀態，以檢閱任何問題。

1. 開啟 [Azure Stack 入口網站](https://portal.local.azurestack.external)。
2. 選取 [資源群組]，然後選取部署 Kubernetes 叢集時所用資源群組的名稱。
3. 依序選取 [部署] 和 [部署名稱]。
...
您將需要連線至叢集的主要 VM、開啟 bash 提示字元，並執行指令碼來產生記錄。 您可以在叢集資源群組中找到主要叢集，其名稱為 `k8s-master-<sequence-of-numbers>`。 

### <a name="prerequisites"></a>必要條件

您需要在用來管理 Azure Stack 的電腦上具有 bash 提示字元。 使用 bash 來執行指令碼以存取記錄。 在 Windows 電腦上，您可以使用隨 Git 一起安裝的 bash 提示字元。 若要取得最新版本的 git，請參閱 [git 下載](https://git-scm.com/downloads)。
...
2. 執行下列 bash 命令：
    ...
    > [!Note]  
    > 在 Windows 上，您不需要執行 `sudo`，只要使用 `chmod 744 getkuberneteslogs.sh` 即可。
...
4. 在命令所建立的資料夾中擷取記錄。 此命令會建立新的資料夾，並為其加上時間戳記。
