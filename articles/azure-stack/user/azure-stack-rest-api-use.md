...
此流程會要求您的用戶端向 Microsoft Azure 登入端點進行驗證。 此端點會傳回權杖。 在每個傳送至 Azure Stack API 之要求的標頭中提供該權杖。 Azure 會使用 Oauth2.0。

本文使用 curl 公用程式提供 Azure Stack 特定的簡單範例。 本主題將逐步引導您進行擷取權杖以存取 Azure Stack API 的流程。 大部分的語言都會提供 Oauth 2.0 程式庫，其具有健全的權杖管理，並會處理重新整理權杖之類的工作。

查看搭配一般 REST 用戶端 (例如 curl) 使用 Azure Stack REST API 的完整流程，可協助您了解基本要求，以及您可預期會在回應承載中接收到的內容。

本文也不會探索適用於擷取權杖的所有選項，例如互動式登入或建立專用的應用程式識別碼。 如需詳細資訊，請參閱 [Azure REST API 參考](https://docs.microsoft.com/rest/api/) \(英文\)。
...
針對每個值：
  ...
  **資源**
  ...
  ```
 > [!NOTE]  
 > 如果您是嘗試存取租用戶 API 的系統管理員，則您必須確定使用租用戶端點，例如 `https://adminmanagement.{region}.{Azure Stack domain}/metadata/endpoints?api-version=2015-01-011
  
  例如，使用 Azure Stack 開發套件：
  ```
