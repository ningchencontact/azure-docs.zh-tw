...
2. 在基礎板管理控制器 (BMC) 設定正確的 IP 位址，並根據 OEM 提供的文件套用所有 BIOS 設定。
3. 使用由硬體製造商提供、執行於 HLH 上的工具，將目前的韌體基準套用至所有元件。
4. 在 Azure Stack 管理入口網站中執行「新增節點」作業。
5. 驗證新增節點作業執行成功。 若要這麼做，請查看[縮放單位的**狀態**](#monitor-add-node-operations)。 

## <a name="add-the-node"></a>新增節點 
您可以使用管理主控台或 PowerShell 來新增節點。 新增節點作業會先新增縮放單位節點作為可用的計算容量，然後自動擴充儲存體容量。 容量會自動擴充，因為 Azure Stack 是*計算*和*儲存體*一併調整的超融合系統。

### <a name="use-the-admin-console"></a>使用管理主控台
1. 以 Azure Stack 操作員身分登入 Azure Stack 管理入口網站。
2. 瀏覽至 [區域管理] > [縮放單位]，然後選取要擴充容量的縮放單位。  
   ![選取縮放單位](media/azure-stack-add-scale-node/select-node1.png)

   ![檢視縮放單位詳細資料](media/azure-stack-add-scale-node/select-node2.png)
 
3. 必要參數如下：  
   - **BMC IP 位址**，用於您所新增的每個縮放單位節點。 (每行一個 IP 位址。)![新增節點](media/azure-stack-add-scale-node/add-node.png)

### <a name="use-powershell"></a>使用 PowerShell

使用 **New-AzsScaleUnitNodeObject** Cmdlet 新增一或多個節點。  
