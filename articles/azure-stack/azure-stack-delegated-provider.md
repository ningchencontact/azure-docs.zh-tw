...
1. **建立委派的提供者訂用帳戶**：讓使用者訂閱僅包含訂用帳戶服務的供應項目。 接著，訂閱此供應項目的使用者便可藉由為其他使用者註冊委派的供應項目，將這些供應項目延伸至其他使用者。
...
#### <a name="delegated-provider-requirements"></a>委派的提供者需求
...
若要使用本逐步介紹，除了 Azure Stack 操作員帳戶，您還需要兩個 Azure AD 帳戶。 如果您沒有這兩個帳戶，則必須加以建立。 這些帳戶可以屬於任何 Azure AD 使用者，而且可稱為委派的提供者和使用者。
...
| 使用者 |使用者 |
 ...
 > [!NOTE]
 > 在 CSP 轉售商的案例中，若要建立此委派提供者，則租用戶目錄 (使用者 AAD) 中需要有這些使用者。 Azure Stack 操作員必須先讓該租用戶 AAD [上線](azure-stack-enable-multitenancy.md)，然後遵循[這些步驟](azure-stack-csp-howto-register-tenants.md)來設定使用量和計費方式。
...
2. 委派供應項目。 移至 **DelegatedOffer**。 在 [設定] 之下，選取 [委派的提供者]，然後選取 [新增]。

3. 從下拉式清單為委派的提供者選取訂用帳戶，然後選取 [委派]。
...
1. 選取 [+ 建立資源]、[租用戶供應項目 + 方案]，然後選取 [供應項目]。

    ![建立新的供應項目](media/azure-stack-delegated-provider/image5.png)

2. 將名稱指派給供應項目。 本範例使用 **ResellerOffer**。 選取要做為基礎的委派供應項目，然後選取 [建立]。
...
如有需要，可在屬於相同目錄租用戶之新的或現有的委派的提供者訂用帳戶之間，移動訂用帳戶。 使用 PowerShell Cmdlet [Move-AzsSubscription](/powershell/module/azs.subscriptions.admin) 即可進行此操作。
