...
1. 瀏覽**儲存體帳戶**，然後尋找 **frphealthaccount**。 **frphealthaccount** 帳戶是用來儲存所有作業系統事件的通用儲存體帳戶。

   ![儲存體帳戶](media/azure-stack-integrate-security/storage-accounts.png)

1. 選取 [frphealthaccount]，然後按一下 [存取金鑰]。

   ![存取金鑰](media/azure-stack-integrate-security/access-keys.png)

1. 將存取金鑰複製到剪貼簿。
1. 開啟 [Azure 儲存體總管]。
1. 在 [編輯] 功能表上，選取 [目標 Azure Stack]。
1. 選取 [新增帳戶]，然後選取 [使用儲存體帳戶名稱和金鑰]。
...
1. 按 [下一步]、檢閱摘要，然後 [完成] 精靈。
1. 您現在可以瀏覽個別的 Blob 容器，並下載事件。
