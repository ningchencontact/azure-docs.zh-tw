...
若要深入了解 GitHub 存放庫以及如何貢獻，請參閱[存放庫的讀我檔案](https://github.com/Azure/AzureStack-QuickStart-Templates/blob/master/README.md)。 


## <a name="create-a-vm-using-a-custom-github-template"></a>建立會使用自訂 GitHub 範本的虛擬機器
在此範例教學課程中，[101-vm-linux-minikube](https://github.com/Azure/AzureStack-QuickStart-Templates/tree/master/101-vm-linux-minikube) Azure Stack 快速入門範本用於在執行 Minikube 的 AzureStack 上部署 Ubuntu 16.04 虛擬機器，以便管理 kubenetes 叢集。

Minikube 是種可讓您在本機輕鬆執行 Kubernetes 的工具。 Minikube 會在虛擬機器中執行單一節點 Kubernetes 叢集，適用於想試用 Kubernetes 的使用者，或想使用 Kubernetes 進行日常開發作業的使用者。 Minikube 可支援在 Linux 虛擬機器上執行簡單、單一節點的 Kubernetes 叢集。 要執行具備完整功能的 Kubernetes 叢集，這是最快速、最直接的方式。 Minikube 可讓開發人員在本機電腦上，開發並測試以 Kubernetes 為基礎的應用程式部署。 在架構上，Minikube 虛擬機器會在本機上執行主要節點和代理程式節點元件：
- 主要節點元件如 API 伺服器、排程器和 etcd 伺服器，會以稱為 LocalKube 的單一 Linux 程序加以執行。
...
> [!IMPORTANT]
> 開始這些步驟之前，必須已將 Ubuntu 虛擬機器映像 (在此範例中為 Ubuntu Server 16.04 LTS) [新增到 Azure Stack 市集中](asdk-marketplace-item.md)。
...
5. 若您想要修改範本 JSON，可以執行這項操作；若否，完成時請按一下 [儲存] 以關閉 [編輯範本] 對話方塊。

   ![](media/asdk-create-vm-template/5.PNG) 

6.  按一下 [參數]、視需要填入或修改可用欄位，然後再按 [確定]。 選擇要使用的訂用帳戶、建立或選擇現有的資源群組名稱，然後按一下 [建立] 來起始範本部署。
...
4. 開啟網頁瀏覽器並造訪 Kubernetes 儀表板位址。 恭喜，您現在已擁有使用 Minikube 且具備完整功能的 Kubernetes 安裝了！
...
> [深入了解進階評估工作](asdk-advanced-eval.md)