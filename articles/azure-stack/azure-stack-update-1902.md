...
> [!IMPORTANT]
> - 在更新成 1902 之前，請先安裝 1901 的[最新 Azure Stack Hotfix](#azure-stack-hotfixes) (如果有的話)。
...
-->
- 為了改進套件的完整性和安全性並簡化離線擷取的管理工作，Microsoft 已將更新套件的格式從 .exe 和 .bin 檔案變更為 .zip 檔案。 新的格式讓有時會導致更新的準備工作延滯的解壓縮程序變得更可靠。 相同的套件格式也適用於來自 OEM 的更新套件。

- 操作員現在可以直接使用 `Test-AzureStack -Group UpdateReadiness` (而非在 `include` 陳述式後面另外傳遞 10 個參數)，來改善其在執行 **Test-AzureStack** 時的 Azure Stack 操作員體驗。 例如︰
...
- 改善 **Test-AzureStack** 在進行秘密輪替整備測試時的可靠性。
...
- 如果您沒有硬體生命週期主機 (HLH)：在組建 1902 之前，您必須將群組原則 **Computer Configuration\Windows Settings\Security Settings\Local Policies\Security Options** 設定為 [Send LM 和 NTLM – 使用 NTLMv2 工作階段安全性 (若已交涉)]。 自組建 1902 開始，則必須讓此原則保持 [未定義] 或將其設定為 [只傳送 NTLMv2 回應] (此為預設值)。 否則，您就無法建立 PowerShell 遠端工作階段，且會收到**拒絕存取**錯誤：
