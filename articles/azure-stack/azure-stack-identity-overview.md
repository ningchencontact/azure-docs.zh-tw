...
- 是以 username@domain 格式建立。 雖然 AD FS 可將使用者帳戶對應到 Active Directory 執行個體，但 AD FS 不支援使用 *\<網域>\<別名>* 格式。 
- 可以設定為使用多重要素驗證。 
...
當您登入貴組織的租用戶入口網站時，您會使用 https://portal.local.azurestack.external URL。 
