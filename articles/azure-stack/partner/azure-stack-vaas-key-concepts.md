...
1. 在解決方案儀表板上，建立新的解決方案或選取現有的解決方案。 這會重新整理並啟動工作流程圖格。
2. 若要建立新的工作流程，請在任何圖格上選取 [開始]。 如需有關每個工作流程的特定資訊，請參閱下列文章：
    ...
    - 套件驗證 (每月更新)：[驗證來自 Microsoft 的軟體更新](azure-stack-vaas-validate-microsoft-updates.md)
    - 套件驗證 (套件簽署)：[驗證 OEM 套件](azure-stack-vaas-validate-oem-package.md)
...
VaaS 中的測試是由針對 Azure Stack 解決方案執行的一組作業所組成的。 測試會有依分類 (例如功能性或可靠性) 識別的各種不同預定目的，並且會以一或多項 Azure Stack 服務為目標。 每個測試會定義一組自己的參數，其中有些會由上層工作流程的通用參數所指定。
