...
4. 選取 [建立] 以建立訂用帳戶。 新的訂用帳戶會出現在 [使用者訂用帳戶] 下方。 當使用者登入使用者入口網站時，他們就會看到訂用帳戶的詳細資料。

### <a name="to-make-an-add-on-plan-available"></a>提供附加方案

雲端操作員可以隨時將方案新增至先前建立的訂用帳戶：
...
>[!NOTE]
>如果 Azure Stack 環境支援[多租用戶](azure-stack-enable-multitenancy.md)，您也可以從遠端目錄租用戶訂閱供應項目。
...
1. 在使用者入口網站中，選取 [所有服務]。 接著，選取 [一般] 類別下方的 [訂用帳戶]，然後選取您想要變更的訂用帳戶。 如果有任何可用的附加方案，[+新增方案] 就會可供使用，並且會有 [附加方案] 的圖格。 

   如果 [+新增方案] 非作用中，表示與此訂用帳戶相關聯的供應項目沒有附加方案。
