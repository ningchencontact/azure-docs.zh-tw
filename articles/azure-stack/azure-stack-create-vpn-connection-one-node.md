...
本文說明如何在兩個個別「Azure Stack 開發套件」(ASDK) 環境中的兩個虛擬網路之間建立站對站 VPN 連線。 在您設定連線時，會了解 VPN 閘道在 Azure Stack 中的運作方式。

### <a name="connection"></a>連線

下圖顯示完成時連線設定看起來的樣子。

![站對站 VPN 連線設定](media/azure-stack-create-vpn-connection-one-node-tp2/OneNodeS2SVPN.png)

### <a name="before-you-begin"></a>開始之前

若要完成連線設定，請務必在開始前備妥下列項目：

* 兩個符合 ASDK 硬體需求的伺服器和其他必要條件，如[快速入門：評估 Azure Stack 開發套件](azure-stack-deploy-overview.md)所說明。
...
若要完成連線設定，您必須部署兩個 ASDK 環境。

> [!NOTE]
> 針對您部署的每個 ASDK，依照[部署指示](azure-stack-run-powershell-script.md)操作。 在此文章中，ASDK 環境稱為 **POC1** 和 **POC2**。
...
## <a name="review-and-complete-the-network-configuration-table"></a>檢閱並完成網路設定表

下表摘要說明這兩個 ASDK 環境的網路設定。 請使用此表格後面所列的程序來新增您網路特定的外部 BGPNAT 位址。

### <a name="network-configuration-table"></a>網路設定表
...
|閘道子網路     |10.0.11.0/24|10.0.21.0/24|
|外部 BGPNAT 位址     |         |         |

> [!NOTE]
> 範例環境中的外部 BGPNAT IP 位址是 10.16.167.195 (用於 POC1) 和 10.16.169.131 (用於 POC2)。 使用下列程序來決定您 ASDK 主機的外部 BGPNAT IP 位址，然後將這些位址新增到先前的網路設定表。
...
3. 將 IP 位址新增到上一節中出現的網路設定表。

4. 在 POC2 上重複執行此程序。

## <a name="create-the-network-resources-in-poc1"></a>在 POC 1 中建立網路資源

現在，您可以建立設定閘道所需的 POC1 網路資源。 下列指示說明如何使用 Azure Stack 使用者入口網站來建立資源。 您也可以使用 PowerShell 程式碼來建立資源。
...
4. 選取 [虛擬網路]。
5. 針對 [名稱]、[位址空間]、[子網路名稱] 及 [子網路位址範圍]，使用先前網路設定表中顯示的值。
...
4. 在 [名稱] 中，輸入 **GW1**。
5. 選取 [虛擬網路] 項目以選擇虛擬網路。 從清單中選取 [VNET-01]。
6. 選取 [公用 IP 位址] 功能表項目。 當 [選擇公用 IP 位址] 視窗開啟時，請選取 [建立新項目]。
...
更廣泛來說，區域網路閘道資源一律是指連線另一端的遠端閘道。 由於「Azure Stack 開發套件」的設計方式之故，您必須提供另一個 ASDK 之網路位址轉譯 (NAT) VM 上的外部網路介面卡 IP 位址作為區域網路閘道的公用 IP 位址。 您接著需在 NAT VM 上建立 NAT 對應，以確保正確連接兩端。
...
4. 從資源清單中，選取 [區域網路閘道]。
5. 在 [名稱] 中，輸入 **POC2-GW**。
6. 在 [IP 位址] 中，輸入 POC2 的外部 BGPNAT 位址。 此位址先前已顯示在網路設定表中。
...
### <a name="sign-in-as-a-tenant-again"></a>再次以租用戶身分登入

服務系統管理員可以用租用戶身分登入，以測試其租用戶可能使用的方案、供應項目及訂用帳戶。 如果您還沒有租用戶帳戶，請先[建立租用戶帳戶](azure-stack-add-new-user-aad.md)再登入。

### <a name="create-virtual-network-and-vm-subnet"></a>建立虛擬網路與 VM 子網路

1. 使用租用戶帳戶來登入。
2. 在使用者入口網站中，選取 [+ 建立資源]。
3. 移至 [Marketplace]，然後選取 [網路]。
4. 選取 [虛擬網路]。
5. 使用先前網路設定表中顯示的資訊，來指定 POC2 [名稱]、[位址空間]、[子網路名稱] 及 [子網路位址範圍] 的值。
6. 在 [訂用帳戶] 中，會顯示您先前建立的訂用帳戶。
...
### <a name="create-gateway-subnet"></a>建立閘道子網路

1. 從儀表板開啟您建立的虛擬網路資源 (**VNET-02**)。
2. 在 [設定] 刀鋒視窗上選取 [子網路]。
3. 選取 [閘道子網路] 以將閘道子網路新增到虛擬網路。
4. 子網路名稱預設為 **GatewaySubnet**。 閘道子網路很特別﹐必須具有此特定名稱，才能正常運作。
5. 在 [位址範圍] 欄位中，確認位址是 **10.0.21.0/24**。
6. 選取 [確定] 以建立閘道子網路。

### <a name="create-virtual-network-gateway"></a>建立虛擬網路閘道
...
### <a name="create-local-network-gateway-resource"></a>建立區域網路閘道資源

1. 在 POC2 使用者入口網站中，選取 [+ 建立資源]。
2. 移至 [Marketplace]，然後選取 [網路]。
3. 從資源清單中，選取 [區域網路閘道]。
4. 在 [名稱] 中，輸入 **POC1-GW**。
5. 在 [IP 位址] 中，輸入先前網路設定表中所列的 POC1 外部 BGPNAT 位址。
6. 在 [位址空間] 欄位中，從 POC1，輸入 **VNET-01** 的位址空間 **10.0.10.0/23**。
7. 確認您的 [訂用帳戶]、[資源群組] 和 [位置] 正確無誤，然後選取 [建立]。

## <a name="create-connection"></a>建立連線
...
9. 在 [共用金鑰 (PSK)] 中，輸入 **12345**。 如果您選擇不同的值，請記住該值必須與您在 POC1 上所建立共用金鑰的值相符。 選取 [確定] 。
...
由於 ASDK 是獨立的並與實體主機部署所在的網路隔離，因此閘道所連接的「外部」VIP 網路實際上並不在外部。 取而代之的是，VIP 網路隱藏在執行網路位址轉譯的路由器之後。

路由器是一部名為 **AzS-bgpnat01** 的 Windows Server 虛擬機器，它在 ASDK 基礎結構中執行「路由及遠端存取服務」(RRAS) 角色。 您必須在 AzS-bgpnat01 虛擬機器上設定 NAT，才能允許站對站 VPN 連線連接兩端。

若要設定 VPN 連線，您必須建立靜態 NAT 對應路由，以將 BGPNAT 虛擬機器上的外部介面對應至邊緣閘道集區的 VIP。 VPN 連線中的每個連接埠都必須要有一個靜態 NAT 對應路由。
...
3. 複製並編輯下列 PowerShell 指令碼。 若要在每個「Azure Stack 開發套件」上設定 NAT，請在已提高權限的 Windows PowerShell ISE 中執行該指令碼。 在指令碼中，加入 `External BGPNAT address` 和 `Internal IP address` 預留位置的值：
...
既然已建立站對站連線，您現在應驗證是否可以透過它傳送流量。 若要驗證，請登入您在任一 ASDK 環境中建立的其中一個虛擬機器。 接著，Ping 您在另一個環境中建立的虛擬機器。
...
6. 開啟已提高權限的 [Windows PowerShell] 視窗。
7. 輸入 **ipconfig /all**。
8. 會顯示落在 **10.0.20.0/24** 內的 IPv4 位址。 在範例環境中，該位址是 **10.0.20.4**，但您的位址可能會不同。
9. 若要建立允許虛擬機器回應 Ping 的防火牆規則，請執行下列 PowerShell 命令：
...
10. 從 POC2 上的虛擬機器，透過通道 Ping POC1 上的虛擬機器。 若要這麼做，您需 Ping 從 VM01 記錄的 DIP。 在範例環境中，這是 **10.0.10.4**，但請務必 Ping 您在實驗室中記下的位址。 您應該會看到如下列範例所示的結果：
...
3. 在 [連線] 視窗中，會顯示 [資料輸入] 和 [資料輸出] 的統計資料。 在以下的螢幕擷取畫面中，那些大的數字歸因於額外的檔案傳輸。 您應該會在該處看到一些非零的值。
