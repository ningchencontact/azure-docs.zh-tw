...
ms.locfileid: "55239837"
---
# <a name="add-tenant-for-usage-and-billing-to-azure-stack"></a>將用於使用量與帳單的租用戶新增至 Azure Stack

*適用於：Azure Stack 整合式系統*

若要將終端使用者新增至雲端服務提供者 (CSP) 所管理的 Azure Stack 部署，請遵循本文說明的必要步驟。 當新的租用戶使用資源時，Azure Stack 會向其 CSP 訂用帳戶報告使用量。
...
下圖顯示要讓新客戶能夠使用 Azure Stack 以及設定客戶使用量追蹤，CSP 所必須遵循的步驟。 透過新增終端客戶，您也能夠管理 Azure Stack 中的資源。 您有兩個選項可管理其資源：

1. 您可以保有終端客戶，並將本機 Azure Stack 訂用帳戶的認證提供給終端客戶。  
2. 終端客戶可在本機使用其訂用帳戶，並將 CSP 新增為具有擁有者權限的來賓。  

## <a name="steps-to-add-an-end-customer"></a>新增終端客戶的步驟
...
使用新客戶的訂用帳戶更新您的註冊。 Azure 會使用來自「合作夥伴中心」的客戶身分識別來報告客戶的使用量。 這個步驟可確保每個客戶的使用量都會報告在該客戶的個別 CSP 訂用帳戶底下。 這可讓您更輕鬆地追蹤使用者使用量與帳單。
...
下節描述 **New-AzureRmResource** Cmdlet 的參數：
...
> [!NOTE]  
> 租用戶必須向他們所使用的每個 Azure Stack 註冊。 如果您有兩個 Azure Stack 部署，且租用戶會使用這兩個部署，則您必須使用租用戶訂用帳戶更新每個部署的初始註冊。
