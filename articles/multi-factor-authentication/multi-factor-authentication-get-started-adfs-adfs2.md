...
確定已將使用者從 Active Directory 匯入伺服器中。 如果您想要將內部 IP 位址列入允許清單，以便從這些位置登入網站時不需進行雙步驟驗證，請參閱＜信任的 IP＞一節。
...
4. 輸入應列入允許清單中的 IP 位址、IP 位址範圍或子網路。 如果輸入子網路，請選取適當的網路遮罩，然後按一下 [確定] 按鈕。 現在已加入信任的 IP。
