...
在傳統部署模型中使用 VPN 閘道將一個虛擬網路連線到另一個虛擬網路 (VNet 對 VNet)，類似於將虛擬網路連線到內部部署網站位置。 这两种连接类型都使用 VPN 网关来提供使用 IPsec/IKE 的安全隧道。
...
Azure 會使用每個區域網路站台中指定的設定，來決定如何路由傳送 VNet 之間的流量。 每个 VNet 都必须指向要将流量路由到的相应本地网络。 您需決定要用來參考每個區域網路站台的名稱。 最好使用描述性的項目。
...
2. 按一下 [子網路、大小和路由類型]。 在 [閘道組態] 頁面上，按一下 [子網路]。
...
## <a name="updatelocal"></a>步驟 6 - 更新本機網站

為兩個 VNet 建立虛擬網路閘道之後，您必須調整本機網站 **VPN 閘道 IP 位址**值。

|VNet 名称|已連線網站|閘道 IP 位址|
|:--- |:--- |:--- |
...
2. 按一下以開啟 VNet 的 [概觀] 頁面。 在頁面的 [VPN 連線] 中，您可以檢視虛擬網路閘道的 IP 位址。
...
3. 匯出並檢視網路組態檔。 在您的電腦上建立目錄，然後將網路組態檔匯出到該目錄。 在此範例中，會將網路組態檔匯出到 **C:\AzureNet**。
   ...
   ```
4. 使用文字編輯器開啟檔案，並且檢視 VNet 和網站的名稱。 這些名稱是您建立連線時使用的名稱。<br>VNet 名称以 **VirtualNetworkSite name =** 形式列出<br>網站名稱會列為 **LocalNetworkSiteRef name =**
...
在示例中，可以看到共享密钥完全相同。 共用金鑰必須一律相符。 請務必使用 VNet 和區域網路網站的確切名稱，取代這些範例中的值。
...
* 虚拟网络和本地网络站点的地址空间不得重叠。 位址空間重疊會導致建立虛擬網路或上傳 netcfg 組態檔失敗。
* 不支持一对虚拟网络之间存在冗余隧道。
