...
* 以**路由為基礎的閘道**：路由式 vpn 的原則或流量選取器會設定為任何對任何（或萬用字元）。

* 以原則為基礎的**閘道**：原則式 vpn 會根據內部部署網路與 Azure VNet 之間的位址首碼組合，透過 IPsec 通道來加密和導向封包。 原則 (或流量選取器) 通常定義為 VPN 組態中的存取清單。
