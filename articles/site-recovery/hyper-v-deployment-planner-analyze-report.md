...
**相容的虛擬機器數目**：系統會計算其所需的網路頻寬、所需的儲存體帳戶數目，以及 Azure 核心的相容 VM 總數。

**所有相容虛擬機器的磁碟總數**：所有相容 VM 的磁碟總數。
...
**相容的虛擬機器**：可以使用 Azure Site Recovery 受 Azure 保護的 VM 數目。 這是系統會計算其所需的網路頻寬、儲存體帳戶數目、Azure 核心數目的相容 VM 總數。 在 [相容 VM] 區段中可取得每部相容 VM 的詳細資料。
...
**所需的 RPO**：以分鐘為單位的所需復原點目標。 此報告會針對三個 RPO 值而產生：15 (預設值)、30 和 60 分鐘。 報告中的頻寬建議會隨著您在工作表右上方的 [所需的 RPO] 下拉式清單中選取的項目變更。 如果您使用 -DesiredRPO 參數以自訂值產生報告，這個自訂值會顯示為 [所需的 RPO] 下拉式清單中的預設值。
...
**每月/年的 Azure 儲存成本**：橫條圖顯示進階和標準儲存體針對複寫及 DR 演練產生的總儲存成本。 您可以在[成本估計](hyper-v-deployment-planner-cost-estimation.md)工作表中檢視每個 VM 的詳細成本分析。
...
**尖峰 R/W IOPS (含成長因子)**：磁碟上的尖峰工作負載讀/寫 IOPS (預設值為第 95 個百分位數)，以及未來的成長因子 (預設值為 30%)。 VM 的總讀取/寫入 IOPS 不一定是 VM 個別磁碟的讀取/寫入 IOPS 的總和。 VM 的尖峰讀取/寫入 IOPS 是其個別磁碟在剖析期間內每分鐘的讀取/寫入 IOPS 總和的尖峰。
...
**磁碟數量**：VM 上的虛擬機器磁碟 (VHD) 總數。

**磁碟大小 (GB)**：VM 的所有磁碟總大小。 此工具也會顯示 VM 中個別磁碟的磁碟大小。
...
**磁碟區 (VHD 路徑)**：存在 VHD/VHDX 之 Hyper-V 主機的每個磁碟區。 

**可用空間 (GB)**：磁碟區上的可用空間。

**磁碟區上所需的總儲存空間 (GB)**：磁碟區上要成功初始複寫和差異複寫所需的可用儲存空間總計。 

**為了成功複寫而要在磁碟區上佈建的額外儲存體總量 (GB)**：建議為了成功初始複寫和差異複寫，而必須在磁碟區上佈建的額外空間總量。
...
**初始複寫所需的最小頻寬 (Mbps)**：VM 初始複寫所需的最小頻寬 (Mbps)。

**差異複寫所需的最小頻寬 (Mbps)**：VM 差異複寫所需的最小頻寬 (Mbps)。
