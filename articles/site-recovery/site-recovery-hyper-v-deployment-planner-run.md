...
description: "本文說明當您從 Hyper-V 移至 Azure 時，執行 Azure Site Recovery 部署規劃工具的模式。"
...
## <a name="modes-of-running-the-deployment-planner"></a>執行部署規劃工具的模式
...
Azure Site Recovery 不支援具有 iSCSI 和傳遞磁碟的 VM。 不過，此工具也無法偵測及分析虛擬機器所連結的 iSCSI 和傳遞磁碟。
...
|-TargetRegion|(選用) 複寫的目標 Azure 區域。 因為 Azure 在每個區域有不同的成本，若要產生特定目標 Azure 區域的報告，請使用此參數。 預設值是 WestUS2 或上次使用的目標區域。 請參閱[支援的目標區域](site-recovery-hyper-v-deployment-planner-cost-estimation.md#supported-target-regions)清單。|
|-OfferId|(選用) 與訂用帳戶相關聯的優惠。 預設值為 MS-AZR-0003P (預付型方案)。|
|-Currency|(選用) 在所產生的報告中用於顯示成本的貨幣。 預設值是美元 ($) 或上次使用的貨幣。 請參閱[支援的貨幣](site-recovery-hyper-v-deployment-planner-cost-estimation.md#supported-currencies)清單。|
...
* [虛擬機器儲存體放置](site-recovery-hyper-v-deployment-planner-analyze-report.md#vm-storage-placement-recommendation)
