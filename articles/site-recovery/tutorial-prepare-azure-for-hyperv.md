---
title: 準備 Azure 以使用 Azure Site Recovery 進行內部部署 Hyper-V VM 的災害復原 | Microsoft Docs
...
本文是一系列中的第一個教學課程，說明如何為內部部署 VM 設定災害復原。 如果您要保護 Hyper-V VM，本文息息相關。

> [!NOTE]
> 這些教學課程皆設計成顯示案例的最簡單部署路徑。 可能的話，會使用預設選項，而不會顯示所有可能的設定與路徑。 如需詳細指示，請參閱對應案例的**操作說明**區段。

本文說明如何在您想要將內部部署 VM (Hyper-V) 複寫至 Azure 時，準備好 Azure 元件。 在本教學課程中，您了解如何：
