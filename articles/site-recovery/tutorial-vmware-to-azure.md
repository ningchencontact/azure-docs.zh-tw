...
* [準備內部部署 VMware](tutorial-prepare-on-premises-vmware.md)
...
3. Site Recovery 會檢查您是否有一或多個相容的 Azure 儲存體帳戶和網路。
   ...
   ![目標索引標籤](./media/tutorial-vmware-to-azure/storage-network.png)
...
7. 在 [應用程式一致快照頻率] 中，使用 60 分鐘預設值作為應用程式一致快照集的建立頻率。 選取 [確定] 以建立原則。
   ...
   ![建立複寫原則](./media/tutorial-vmware-to-azure/replication-policy.png)
