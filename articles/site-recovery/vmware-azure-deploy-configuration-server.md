...
1. 是否可以將已安裝設定伺服器的 VM 用於不同用途？ **否**，設定伺服器必須是單一用途伺服器，不支援使用它作為共用伺服器。
2. 是否可以將已在設定伺服器中註冊的保存庫與新建立的保存庫交換？ **否**，在保存庫向設定伺服器註冊之後，即無法變更。
3. 是否可以使用同一部設定伺服器來保護實體和虛擬機器？ **是**，可以使用同一部設定伺服器來複寫實體和虛擬機器。 不過，不支援容錯回復至實體機器。
4. 設定伺服器會用在哪裡？ 請參考[這裡](vmware-azure-architecture.md)的 Azure Site Recovery 架構，以深入了解設定伺服器及其功能。
5. 哪裡可以找到最新版的設定伺服器？ 您可以從 [Microsoft 下載中心](https://aka.ms/asrconfigurationserver)直接下載。 請參考[這裡](vmware-azure-manage-configuration-server.md#upgrade-the-configuration-server)文章中有關升級設定伺服器的步驟。
