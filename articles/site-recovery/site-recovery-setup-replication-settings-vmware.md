...
3. 從伺服器清單中選取組態伺服器。
4. 按一下 [SERVICEPRINCIPAL] 。 組態伺服器應該會在 1 至 2 分鐘內產生關聯。
...
3. 根據需要來變更設定。
4. 按一下 [檔案] 。 根據多少 VM 使用該複寫原則而定，應該會在 2 至 5 分鐘內儲存原則。
...
2. 按一下 [中斷關聯]。
3. 從伺服器清單中選取組態伺服器。
4. 按一下 [SERVICEPRINCIPAL] 。 組態伺服器應該會在 1 至 2 分鐘內中斷關聯。
