...
- 應用程式工作負載分成三個子網路：10.1.0.0/24、10.1.1.0/24、10.1.2.0/24，分別名為 **Subnet 1**、**Subnet 2**、**Subnet 3**。
- Azure 東南亞為目標區域，並具有可模擬來源上位址空間和子網路設定的復原虛擬網路。 此虛擬網路稱為 **Recovery VNet**。
- 複本節點 (例如針對 Always On、網域控制站等所需的節點) 會置於具有位址空間 20.1.0.0/16 的虛擬網路中，位址為 20.1.0.0/24 的 Subnet 4 內。 此虛擬網路稱為 **Azure VNet**，且位於 Azure 東南亞。
...
![容錯移轉前的 Azure 對 Azure 連線能力](./media/site-recovery-retain-ip-azure-vm-failover/azure-to-azure-connectivity-before-failover.png)
...
![Azure 對 Azure 連線能力完整區域容錯移轉](./media/site-recovery-retain-ip-azure-vm-failover/azure-to-azure-connectivity-full-region-failover.png)
...
容錯移轉前設定的外觀將如下所示：
- 應用程式 VM 是裝載於 Azure 東亞並運用 Azure 虛擬網路，其中第一個應用程式是搭配位址空間 10.1.0.0/16，第二個應用程式則是搭配 15.1.0.0/16。 虛擬網路針對第一個和第二個應用程式分別名為 **Source VNet1** 和 **Source VNet2**。
- 每個 VNet 都會再進一步分成兩個子網路。
- Azure 東南亞為目標區域，且具有復原虛擬網路 Recovery VNet1 和 Recovery VNet2。
- 複本節點 (例如針對 Always On、網域控制站等所需的節點) 會放置於具有位址空間 20.1.0.0/16 的虛擬網路中，位址為 20.1.0.0/24 的 **Subnet 4** 內。 此虛擬網路稱為 Azure VNet，且位於 Azure 東南亞。
...
![容錯移轉前的 Azure 對 Azure 連線能力隔離應用程式](./media/site-recovery-retain-ip-azure-vm-failover/azure-to-azure-connectivity-isolated-application-before-failover.png)
...
![容錯移轉後的 Azure 對 Azure 連線能力隔離應用程式](./media/site-recovery-retain-ip-azure-vm-failover/azure-to-azure-connectivity-isolated-application-after-failover.png)
...
如需詳細資訊，請參閱[對等互連文件](../virtual-network/virtual-network-create-peering.md#register)和[價格](https://azure.microsoft.com/en-us/pricing/details/virtual-network/)。
...
![容錯移轉前的內部部署對 Azure 連線能力](./media/site-recovery-retain-ip-azure-vm-failover/on-premises-to-azure-connectivity-before-failover.png)
...
![容錯移轉後的內部部署對 Azure 連線能力](./media/site-recovery-retain-ip-azure-vm-failover/on-premises-to-azure-connectivity-after-failover.png)
