...
如果在為 Hyper-V VM 啟用保護功能時發生問題，請進行下列檢查：
...
4. 檢查在 VM 上的 Hyper-V-VMMS\Admin 記錄中出現的問題。 此記錄檔位於 [應用程式及服務記錄檔] > [Microsoft] > [Windows] 中。
...
5. 在客體 VM 上，確定所執行的是最新版的 Integration Services。
...
4. 檢查 Hyper-V 伺服器與 Azure 之間的連線。 若要這麼做，請在 Hyper-V 主機上開啟工作管理員。 在 [效能] 索引標籤上，按一下 [開啟資源監視器]。 在 [網路] 索引標籤 > [網路活動的程序] 上，查看 cbengine.exe 是否正在傳送大量資料 (以 MB 為單位)。
5. 檢查 Hyper-V 主機是否可連線至 Azure 儲存體 Blob URL。 若要這麼做，請勾選 **cbengine.exe**。 檢視 [TCP 連線]，以驗證從主機到 Azure 儲存體 Blob 的連線。
6. 檢查效能問題，如下所述。
...
2. 確認 VSS 服務正在執行，且狀態良好：
    - 若要這麼做，請登入客體 VM。 然後，開啟管理員命令提示字元中，並執行下列命令，以檢查是否所有 VSS 寫入器都處於良好的狀態。
...
5. 確認備份服務已啟用。 請在 [Hyper-V 設定] > [Integration Services] 中進行此確認。
...
7. 檢查 VM 是否經歷偏高的變換率：
    - 您可以在 Hyper-V 主機上使用效能計數器，測量客體 VM 的每日資料變更率。 若要這麼做，請啟用下列計數器。 請在各 VM 磁碟間彙總此值 5-15 分鐘的樣本，以取得 VM 變換量。
        - 類別：「Hyper-V 虛擬儲存裝置」
        - 計數器：「每秒的寫入位元組數」</br>
    ...
    - 此外，您可以[確認儲存體的延展性目標](https://docs.microsoft.com/azure/storage/common/storage-scalability-targets.md#scalability-targets-for-a-storage-account)。
...
**0x80070032** | 「Hyper-V 磁碟區陰影複製要求者無法連線至虛擬機器 <./VMname>，因為版本不符合 Hyper-V 所預期的版本」 | 檢查是否已安裝最新的 Windows 更新。<br/><br/> [升級](https://docs.microsoft.com/windows-server/virtualization/hyper-v/manage/manage-hyper-v-integration-services.md#keep-integration-services-up-to-date)至最新版的 Integration Services。
...
1. 讓 [分析] 與 [偵錯] 記錄可在 [事件檢視器] 中檢視。 若要這麼做，請在 [事件檢視器] 中按一下 [檢視] > [顯示分析與偵錯記錄檔]。 [分析] 記錄會出現在 **Hyper-V-VMMS** 下。
