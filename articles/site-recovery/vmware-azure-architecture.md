...
2. 在復原服務保存庫中，指定來源複寫設定。 在此程序進行期間，設定內部部署組態伺服器。 若要將此伺服器部署為 VMware VM，您可以下載已備妥的 OVF 範本，並將其匯入 VMware 以建立 VM。
...
1. 您可以容錯移轉單一機器，或建立復原計劃以進行多個 VM 的容錯移轉。
...
2. 備妥元件後，容錯回復分三個階段進行：
    ...
    c. 第 3 階段：工作負載進行容錯回復之後，您必須重新啟用內部部署 VM 的複寫。
