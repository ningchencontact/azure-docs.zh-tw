...
> [!IMPORTANT]
> 從主要目標伺服器的 9.10.0 版開始，最新的主要目標伺服器只能安裝在 Ubuntu 16.04 伺服器上。 CentOS6.6 伺服器上不允許安裝新的主要目標。 不過，您可以使用 9.10.0 版來繼續升級舊的主要目標伺服器。
...
* 若要選擇要在其中部署主要目標的主機，請決定容錯回復的目的地是現有的內部部署虛擬機器，還是新的虛擬機器。 
    * 針對現有的虛擬機器，主要目標的主機應該能存取虛擬機器的資料存放區。
    * 如果內部部署虛擬機器不存在 (針對替代位置復原的情況)，容錯回復虛擬機器就會建立在與主要目標相同的主機上。 您可以選擇任何 ESXi 主機來安裝主要目標。
* 主要目標應位於可與處理序伺服器及設定伺服器通訊的網路上。
* 主要目標的版本必須等於或早於處理序伺服器和設定伺服器的版本。 例如，若設定伺服器的版本是 9.4，則主要目標的版本可以是 9.4 或 9.3，但不能是 9.5。
...
1.  在管理系統上升級的選取項目中，選取 [No automatic updates] \(沒有自動更新\) 選項，然後選取 **Enter**。
    ...
    > [!WARNING]
    > 由於 Azure Site Recovery 主要目標伺服器需要非常特定版本的 Ubuntu，因此您需要確認已停用虛擬機器的核心升級。 如果啟用，則任何一般升級都將導致主要目標伺服器故障。 請確定您選取 [No automatic updates]\(不自動更新\) 選項。
...
### <a name="configure-the-machine-as-a-master-target-server"></a>設定電腦以作為主要目標伺服器
...
如果您的主要目標有網際網路連線，您可以使用下列步驟來下載安裝程式。 否則，您可以從處理序伺服器複製安裝程式並加以安裝。
...
1. 將新的 1-TB 磁碟連結至 Linux 主要目標虛擬機器，然後啟動該機器。
...
> [!IMPORTANT]
> 主要目標伺服器的版本必須等於或早於處理序伺服器和設定伺服器的版本。 如果不符合此條件，重新保護會成功，但複寫會失敗。


> [!NOTE]
> 安裝主要目標伺服器之前，請檢查虛擬機器上的 **/etc/hosts** 檔案包含會將本機主機名稱對應到與所有網路介面卡相關之 IP 位址的項目。
...
2. 記下設定伺服器的 IP 位址。 執行下列命令來安裝主要目標伺服器，並向設定伺服器註冊該伺服器。
...
2. 執行下列命令來安裝主要目標伺服器，並向設定伺服器註冊該伺服器。
...
執行安裝程式。 它會自動偵測主要目標上是否已安裝代理程式。 選取 [Y]\(是\) 進行升級。安裝完成之後，請使用下列命令來檢查所安裝的主要目標版本：
...
* 請確定您未在任何管理元件 (例如主要目標) 上啟動 Storage vMotion。 如果在成功重新保護之後移動主要目標，將會無法中斷虛擬機器磁碟 (VMDK) 的連結。 在此情況下，容錯回復會失敗。

* 主要目標在虛擬機器上不應有任何快照集。 如果有快照集，容錯回復會失敗。

* 由於一些自訂 NIC 設定，因而會在啟動期間停用網路介面，並導致主要目標代理程式無法初始化。 請確定已正確設定下列屬性。 在乙太網路卡檔案的 /etc/sysconfig/network-scripts/ifcfg-eth* 中檢查這些屬性。
    * BOOTPROTO=dhcp
    * ONBOOT=yes
