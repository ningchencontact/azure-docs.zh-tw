...
description: "摘要說明使用 Azure Site Recovery 服務將 VMware VM 複寫到 Azure 所支援的作業系統和元件。"
...
本文摘要說明將 VMware VM 災害復原至 Azure (使用 [Azure Site Recovery](site-recovery-overview.md) 服務) 所支援的元件和設定。
...
**伺服器** | **需求** | **詳細資料**
--- | --- | ---
**VMware** | vCenter Server 6.5、6.0 或 5.5 或 vSphere 6.5、6.0、5.5 | 我們建議使用 vCenter 伺服器
**實體伺服器** | NA
...
XFSv5 | 「行動服務」自 9.10 版開始支援 XFS 檔案系統上的 XFSv5 功能，像是中繼資料總和檢查碼。 使用 xfs_info 公用程式來檢查磁碟分割的 XFS 超級區塊。 若 ftype 設為 1，則代表使用了 XFSv5 功能。
...
IPv4 | yes 
保留來源 IP 位址 | yes 
虛擬網路服務端點<br/><br/> (Azure 儲存體防火牆和 VNET) | 否 
...
主機 SAN (ISCSI) | yes
主機多重路徑 (MPIO) | 是 - 測試標的︰Microsoft DSM、EMC PowerPath 5.7 SP4、EMC PowerPath DSM for CLARiiON
客體/伺服器 VMDK | yes 
客體/伺服器 EFI/UEFI| 部分 (僅適用於 Windows Server 2012 和更新版本移轉到 Azure)。 </br></br> ** 請參閱表格結尾處的附註。
客體/伺服器共用叢集磁碟 | 否 
客體/伺服器加密磁碟 | 否 
客體/伺服器 NFS | 否 
客體/伺服器 SMB 3.0 | 否
客體/伺服器 RDM | yes<br/><br/> 實體伺服器為 N/A 
客體/伺服器磁碟 > 1 TB | yes<br/><br/>最多 4095 GB 
客體/伺服器磁碟使用 4K 邏輯與 4k 實體磁區大小 | yes
客體/伺服器磁碟使用 4K 邏輯與 512 位元組實體磁區大小 | yes 
客體/伺服器磁碟區使用等量磁碟 > 1 TB<br/><br/> LVM-邏輯磁碟區管理：客體/伺服器 - 儲存體空間 | 否，客體/伺服器熱機新增/移除磁碟 | 否，客體/伺服器 - 排除磁碟 | 是，客體/伺服器多重路徑 (MPIO) | N/A
...
進階儲存體 | yes 
匯入/匯出服務 | 否 
虛擬網路服務端點<br/><br/> 設定在目標儲存體/快取儲存體帳戶 (用於儲存複寫資料) 上的 Azure 儲存體防火牆和 VNET | 否 
...
**作業系統磁碟大小** | 最多 2048 GB | 若不支援，則必要條件檢查會失敗
...
**資料磁碟 VHD 大小** | 最多 4095 GB | 若不支援，則必要條件檢查會失敗
**網路介面卡** | 支援多個介面卡 | 
...
**VM 名稱** | 1 - 63 個字元。<br/><br/> 只能使用字母、數字和連字號。<br/><br/> 電腦名稱必須以字母或數字為開頭或結尾。 | 更新 Site Recovery 中電腦屬性的值。
