...
您需要記錄查詢，以從 Log Analytics 擷取任何資料。  無論您要[分析入口網站中的資料](../../azure-monitor/log-query/portals.md)、[設定在特定條件下進行通知的警示規則](../../azure-monitor/platform/alerts-metric.md)，或是使用 [Log Analytics API](https://dev.loganalytics.io/) 擷取資料，會需要使用查詢來指定您想要的資料。  本文說明記錄查詢在 Log Analytics 中的使用方式，並且提供在建立之前應該了解的概念。
...
- **入口網站。** 您可以在 [Azure 入口網站](../../azure-monitor/log-query/portals.md)中執行記錄資料的互動式分析。  這可讓您在各種不同的格式和視覺效果中編輯查詢及分析結果。  
- **警示規則。** [警示規則](../../azure-monitor/platform/alerts-overview.md)會主動識別您的工作區中資料的問題。  每個警示規則是根據以固定間隔自動執行的記錄搜尋。  會檢查結果來判斷是否應該建立警示。
...
- **匯出。**  當您將資料從 Log Analytics 工作區匯入至 Excel 或 [Power BI](../../azure-monitor/platform/powerbi.md) 時，您會建立記錄查詢以定義要匯出的資料。
...
- **Log Analytics API。**  [Log Analytics 記錄搜尋 API](../../azure-monitor/platform/alerts-overview.md) 可讓任何 REST API 用戶端從工作區擷取記錄資料。  API 要求包含針對 Log Analytics 執行的查詢，來判斷要擷取的資料。

![記錄檔搜尋](media/log-query-overview/queries-overview.png)

## <a name="write-a-query"></a>撰寫查詢
Log Analytics 使用[資料總管查詢語言的版本](../../azure-monitor/log-query/get-started-queries.md)，可讓您以各種方式擷取和分析記錄資料。  您通常會從基本查詢開始，然後在您的需求變得更複雜時，使用更進階的函式。
...
- 深入了解[您用來建立和編輯記錄搜尋的入口網站](../../azure-monitor/log-query/portals.md)。
- 請參閱使用新的查詢語言[撰寫查詢的教學課程](../../azure-monitor/log-query/get-started-queries.md)。
