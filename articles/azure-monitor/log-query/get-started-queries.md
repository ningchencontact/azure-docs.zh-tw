...
> [!NOTE]
> 完成本教學課程之前，您應該先完成[開始使用 Azure 監視器 Log Analytics](get-started-portal.md) 。

> [!NOTE]
> 如果您要從至少一部虛擬機器收集資料，您可以在自己的環境中進行此練習。 如果沒有，則使用我們的[示範環境](https://portal.loganalytics.io/demo)，其中包含許多範例資料。


在本教學課程中，您將瞭解如何在 Azure 監視器中撰寫記錄查詢。 它會告訴您如何：
...
如需 Azure 監視器中記錄查詢的詳細資訊，請參閱[Azure 監視器中記錄查詢的總覽](log-query-overview.md)。
...
> [!WARNING]
> 搜尋查詢的速度通常會比以資料表為基礎的查詢慢，因為它們必須處理更多資料。 
...
**extend** 會在結果集內保留所有原始資料行，並定義其他資料行。 下列查詢會使用**extend**來加入*EventCode*資料行。 請注意，此資料行可能不會顯示在資料表的結尾，因此您必須擴充記錄的詳細資料來加以查看。
...
若要根據連續值建立群組，最好是使用 **bin** 將範圍分成可管理的單位。 下列查詢會分析 Perf 記錄，這些記錄會測量特定電腦上的可用記憶體 (可用的 MB 數)。 它會計算過去7天內每1小時期間的平均值：
