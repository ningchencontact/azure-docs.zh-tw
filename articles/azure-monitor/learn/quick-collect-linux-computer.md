...
[Azure Log Analytics](../platform/agent-windows.md) 可直接從資料中心或其他雲端環境內的實體或虛擬 Linux 電腦，將資料收集到單一存放庫，來進行詳細的分析和相互關聯。 本快速入門向您示範如何以幾個簡單步驟來設定和收集 Linux 電腦的資料。  針對 Azure Linux VM，請參閱下列主題[收集關於 Azure 虛擬機器的資料](quick-collect-azurevm.md)。  

在繼續之前，您應該先檢閱 Log Analytics 代理程式[概觀](../platform/log-analytics-agent.md)文章，以了解哪些組態受到支援、[支援的 Linux 作業系統](../platform/log-analytics-agent.md#supported-linux-operating-systems)，以及[網路防火牆組態](../platform/log-analytics-agent.md#network-firewall-requirements)。 
...
1. 在 Azure 入口網站中，選取 [所有服務]  。 在資源清單中，輸入 **Log Analytics**。 當您開始輸入時，清單會根據您輸入的文字進行篩選。 選取 [Log Analytics]  。

    ![Azure 入口網站](media/quick-collect-linux-computer/azure-portal-01.png) 
...
1. 在 Azure 入口網站中，選取左上角的 [所有服務]  。 在資源清單中，輸入 **Log Analytics**。 當您開始輸入時，清單會根據您輸入的文字進行篩選。 選取 [Log Analytics]  。
2. 在 Log Analytics 工作區清單中，選取稍早建立的 *DefaultLAWorkspace*。
3. 選取 [進階設定]  。

    ![Log Analytics 進階設定](media/quick-collect-linux-computer/log-analytics-advanced-settings-01.png) 
...
6. 當您第一次為新的 Log Analytics 工作區設定 Linux 效能計數器時，系統會提供選項，讓您快速建立數個常用的計數器。 這些計數器旁邊皆會列出核取方塊。 
    ...
    ![選取的預設 Windows 效能計數器](media/quick-collect-linux-computer/linux-perfcounters-default.png)
...
1. 在 Azure 入口網站中，移至 Log Analytics 並選取稍早建立的工作區。
2. 選取 [記錄搜尋]  圖格，然後在 [記錄搜尋] 窗格的查詢欄位中輸入 `Perf`，再按 Enter 鍵，或選取查詢欄位右邊的搜尋按鈕。

    ![Log Analytics 記錄搜尋查詢範例](media/quick-collect-linux-computer/log-analytics-portal-queryexample.png)
...
![刪除 Log Analytics 資源](media/quick-collect-linux-computer/log-analytics-portal-delete-resource.png)
