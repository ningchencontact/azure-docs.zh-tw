...
本文說明如何設定適用於 VM 的 Azure 監視器來監視 Azure 虛擬機器和虛擬機器擴展集的作業系統健康情況，以及您環境中的虛擬機器，包括探索和對應可能裝載於其中的應用程式相依性。  
...
* 安裝 **ServiceMap** 和 **InfrastructureInsights** 解決方案。 這只可以使用本文中提供的 Azure Resource Manager 範本來完成。   
...
適用於 VM 的 Azure 監視器對應功能會從 Microsoft Dependency Agent 取得它的資料。 Dependency Agent 連線至 Log Analytics 需仰賴 Log Analytics 代理程式，因此，系統必須搭配 Dependency Agent 來安裝及設定 Log Analytics 代理程式。 當您為單一 Azure VM 啟用適用於 VM 的 Azure 監視器時，或使用大規模部署適用的方法時，Azure VM 的 Dependency Agent 擴充功能可用來將代理程式安裝為該上架體驗的一部分。 在混合式環境中，可透過手動方式或使用自動部署方法，將 Dependency Agent 下載並安裝至那些裝載在 Azure 之外的虛擬機器。  
...
您的使用者需獲派下列存取權，才能啟用並存取適用於 VM 的 Azure 監視器功能。  
  
- 若要啟用解決方案，您必須成為 Log Analytics 參與者角色的成員。  

- 若要檢視效能、健康情況和對應資料，您必須成為 Azure VM 和 Log Analytics 工作區 (其中已設定適用於 VM 的 Azure 監視器) 的「監視讀者」角色成員。   

如需有關如何控制 Log Analytics 工作區存取的詳細資訊，請參閱[管理工作區](../../log-analytics/log-analytics-manage-access.md)。
...
本節將指示如何使用 Azure 原則或 Azure PowerShell 大規模部署適用於 VM 的 Azure 監視器。  

以下摘要說明預先設定 Log Analytics 工作區時需執行的步驟，在此之後您才能繼續上架虛擬機器。

1. 建立新工作區 (如果不存在的話)，以用來支援適用於 VM 的 Azure 監視器。 建立新工作區之前先檢閱[管理工作區](../../log-analytics/log-analytics-manage-access.md?toc=/azure/azure-monitor/toc.json)，以了解成本、管理和合規性考量，然後再繼續進行。       
2. 在工作區中啟用效能計數器，以收集 Linux 和 Windows VM 上的資料。
3. 在工作區中安裝並啟用 **ServiceMap** 和 **InfrastructureInsights** 解決方案。  
...
如果解決方案所參考的 Log Analytics 工作區未設定為已收集此解決方案所需的效能計數器，則必須啟用它們。 您可以如[此處](../../azure-monitor/platform/data-sources-performance-counters.md)所述的方式手動完成此動作，或藉由下載並執行可從 [Azure PowerShell 資源庫](https://www.powershellgallery.com/packages/Enable-VMInsightsPerfCounters/1.1)取得的 PowerShell 指令碼來完成此動作。
...
檢閱適用於 [Log Analytics Linux 和 Windows 代理程式](../../azure-monitor/platform/log-analytics-agent.md)的需求和部署方法。  
...
藉由為您的虛擬機器啟用監視，適用於 VM 的 Azure 監視器就能使用此資訊來進行分析。  若要了解如何使用健康情況功能，請參閱[檢視適用於 VM 的 Azure 監視器健康情況](vminsights-health.md)，或者，若要檢視探索到的應用程式相依性，請參閱[檢視適用於 VM 的 Azure 監視器對應](vminsights-maps.md)。  