...
此管理元件名為*microsoft.intelligencepacks.applicationdependencymonitor. microsoft.intelligencepacks.applicationdependencymonitor*。 它會寫入%Programfiles%\Microsoft Monitoring Agent\Agent\Health Service State\management packs Pack \ 資料夾。 管理組件所使用的資料來源是 %Program files%\Microsoft Monitoring Agent\Agent\Health Service State\Resources\<AutoGeneratedID>\Microsoft.EnterpriseManagement.Advisor.ApplicationDependencyMonitorDataSource.dll。

## <a name="diagnostic-and-usage-data"></a>診斷及使用方式資料
