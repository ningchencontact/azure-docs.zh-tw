...
Log Analytics 中的所有資源都包含於[工作區](../../log-analytics/log-analytics-manage-access.md)中。  如 [Log Analytics 工作區和自動化帳戶](solutions.md#log-analytics-workspace-and-automation-account)所述，工作區不會包含於管理解決方案中，但在安裝解決方案前就必須存在。  如果無法使用，則解決方案安裝會失敗。
