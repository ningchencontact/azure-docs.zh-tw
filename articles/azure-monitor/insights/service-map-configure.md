...
本文說明設定服務對應與啟用代理程式的詳細資料。 如需使用服務對應的相關資訊，請參閱[在 Azure 中使用服務對應解決方案]( service-map.md)。
...
![服務對應代理程式](media/service-map-configure/agents.png)
...
1.  依照 [Log Analytics 代理程式概觀](../../azure-monitor/platform/log-analytics-agent.md)中所述的其中一種方法來安裝適用於 Windows 的 Log Analytics 代理程式。
...
1.  依照 [Log Analytics 代理程式概觀](../../azure-monitor/platform/log-analytics-agent.md)中所述的其中一種方法來安裝 Log Analytics 代理程式。
...
- 部署和設定後，了解如何[使用服務對應]( service-map.md)。