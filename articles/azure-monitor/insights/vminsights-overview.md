---
title: 適用於 VM 的 Azure 監視器 (預覽) 是什麼？ | Microsoft Docs
...
ms.locfileid: "51714089"
---
# <a name="what-is-azure-monitor-for-vms-preview"></a>適用於 VM 的 Azure 監視器 (預覽) 是什麼？

適用於 VM 的 Azure 監視器可藉由分析 Windows 和 Linux VM 的效能和健康情況 (包括其不同的程序以及其他資源和外部程序的互連相依性)，大規模監視 Azure 虛擬機器 (VM) 和 Azure 虛擬機器擴展集。 此解決方案包含針對在內部部署環境或其他雲端提供者上裝載的 VM，監視效能和應用程式相依性的支援。 它包含三個主要功能，以提供此深入解析：
...
>[!NOTE]
>目前，僅針對 Azure 虛擬機器和虛擬機器擴展集提供健康情況功能。 效能和對應可支援您的環境或其他雲端提供者中裝載的 Azure VM 和虛擬機器。
>
...
![從入口網站查看虛擬機器深入見解檢視方塊](./media/vminsights-overview/vminsights-azmon-directvm-01.png)
