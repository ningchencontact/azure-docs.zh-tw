...
- **磁片容量**:顯示每個磁片的互動式磁片使用方式圖表, 其由下列觀點呈現給容器內的節點:
    ...
    - 所有磁片的磁片% 使用量
    - 所有磁片的可用磁碟空間
    - 顯示每個節點磁片的方格、其% 已使用的空間、% 已使用空間的趨勢、可用磁碟空間 (GiB), 以及可用磁碟空間的趨勢 (GiB)。 在資料表中選取資料列時,% 已使用的空間和可用磁碟空間 (GiB) 如下所示 

- **磁片 IO**:針對根據下列各方面呈現給容器內節點的每個磁片, 提供互動式磁片使用圖表:

    - 所有磁片上的磁片 i/o 摘要 (依讀取位元組/秒、寫入位元組/秒, 以及讀取和寫入位元組/秒趨勢) 
    - 八個效能圖表顯示關鍵效能指標, 以協助測量和識別磁片 i/o 瓶頸。
...
- **網路**：提供每個節點網路介面卡的互動式網路使用圖表, 以及呈現關鍵效能指標的方格, 以協助測量網路介面卡的效能。  
