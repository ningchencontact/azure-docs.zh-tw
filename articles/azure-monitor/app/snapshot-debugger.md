...
3. 檢閱套件已新增至 [ApplicationInsights.config](../../azure-monitor/app/configuration-with-applicationinsights-config.md) 的預設選項：
...
如果快照集可用於指定的例外狀況或問題識別碼，則 Application Insights 入口網站中的[例外狀況](../../azure-monitor/app/asp-net-exceptions.md)會出現 [開啟偵錯快照集] 按鈕。

![例外狀況的 [開啟偵錯快照集] 按鈕](./media/snapshot-debugger/snapshot-on-exception.png)
