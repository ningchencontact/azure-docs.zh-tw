...
> [!NOTE]
> 此頁面上的程序會在執行階段將 SDK 加入您的 Web 應用程式。 如果您不想要更新或重建您的原始程式碼，此執行階段檢測非常好用。 不過，如果可以的話，我們建議您改為 [將 SDK 加入原始程式碼](java-get-started.md) 。 這樣可以提供您更多選項，包括撰寫程式碼來追蹤使用者活動。
> 
> 
...
2. 建立新的 Application Insights 資源，並將應用程式類型設定為 Java web 應用程式的。
    ...
    ![填寫名稱，選擇 [Java Web 應用程式]，然後按一下 [建立]](./media/java-live/02-create.png)

    會在幾秒鐘內建立資源。

4. 開啟新的資源，並取得其檢測金鑰。 您很快需要將此金鑰貼到程式碼專案中。
   
    ![在新資源概觀中，按一下 [屬性] 並複製檢測金鑰](./media/java-live/03-key.png)
...
* 事件相互關聯是 HTTP 要求元件的補充。 它會將識別碼指派給伺服器收到的每個要求，並將此識別碼加入為遙測的每個項目的屬性，作為 'Operation.Id' 屬性。 它可讓您相互關聯與每個要求關聯的遙測，方法是在 [診斷搜尋](../../azure-monitor/app/diagnostic-search.md)中設定篩選器。
...
您可能需要 [設定防火牆例外狀況以傳送輸出資料](../../azure-monitor/app/ip-addresses.md)。
...
![範例資料](./media/java-live/5-results.png)
...
* [將遙測加入至您的網頁](javascript.md) ，即可監視頁面檢視和使用者度量。
* [設定 Web 測試](../../azure-monitor/app/monitor-web-app-availability.md) ，以確認應用程式處於線上狀態且能夠回應。
* [擷取記錄追蹤](java-trace-logs.md)
* [搜尋事件和記錄](../../azure-monitor/app/diagnostic-search.md) 以協助診斷問題。
* [設定 Spring Boot 初始設定式應用程式](https://docs.microsoft.com/java/azure/spring-framework/configure-spring-boot-java-applicationinsights) \(英文\)
