...
若要停用每日數量上限電子郵件，請在 Application Insights 資源的 [設定]  區段之下，從 [使用量和估計成本]  窗格選取 [每日上限]  。 此處有設定可在達到上限時，以及已達到可調整的警告層級時傳送電子郵件。 如果您想要停用磁碟區相關的所有每日上限電子郵件取消選取這兩個方塊。

## <a name="legacy-enterprise-per-node-pricing-tier"></a>定價層的舊版企業 （每個節點）

Azure Application insights 的早期採用者，還有兩個可能的定價層：基本和企業。 基本定價層是與上述相同，預設層。 它包含所有的企業層功能，不需要額外收費。 基本層的物料單，主要的內嵌資料的磁碟區上。 

> [!NOTE]
> 已重新命名這些舊版的定價層。 現在稱為 「 企業 」 定價層**每個節點**，現在稱為基本定價層**每 GB**。 下列及 Azure 入口網站中，會使用這些新的名稱。  

每個節點 (前身為 Enterprise) 層每節點計費，而且每個節點會收到每日資料額度。 在每個節點定價層中，您會支付超過所含額度的內嵌資料。 如果您使用 Operations Management Suite，您應該選擇每個節點層。 
...
購買 Operations Management Suite E1 和 E2 的客戶可取得額外的元件，無需額外的費用為每個節點的 Application Insights[先前所宣布](https://blogs.technet.microsoft.com/msoms/2017/05/19/azure-application-insights-enterprise-as-part-of-operations-management-suite-subscription/)。 具體來說，Operations Management Suite E1 和 E2 的每個單位包含 Application Insights 每個節點層的一個節點的權利。 每個 Application Insights 節點包含每天最多 200 MB 擷取的資料 (與 Log Analytics 資料擷取分開計算)，資料可保留 90 天而無須額外付費。 層是更在本文稍後詳細說明。 

由於此階層是僅適用於 Operations Management Suite 訂用帳戶的客戶，沒有 Operations Management Suite 訂用帳戶的客戶沒有看到可選取這一層的選項。

> [!NOTE]
> 若要確保您取得此權利，您的 Application Insights 資源必須在每個節點定價層中。 此權利只能以節點為單位套用。 在 每 GB 層中的 application Insights 資源無法實現任何效益。 在 [使用量和估計成本]  窗格中所顯示的估計成本中看不到此權利。 此外，如果您將訂用帳戶移至新的 Azure 監視 2018 年 4 月定價模型時，每 GB 」 層是可用的唯一層級。 如果您擁有 Operations Management Suite 訂用帳戶，建議您不要將訂用帳戶移轉至新的 Azure 監視定價模型。

### <a name="how-the-per-node-tier-works"></a>每個節點層的運作方式

* 您需支付傳送任何應用程式遙測的每個節點的層中的每個節點。
...
* 每個節點 (資料粒度為小時) 偵測每日 200 MB 的資料量配置。 未使用的資料配置不會累計到隔天。
  * 如果您選擇的每個節點定價層，每個訂用帳戶取得將遙測傳送至該訂用帳戶中的 Application Insights 資源的節點數目為基礎的資料每日資料額度。 因此，如果您有 5 個節點全天候傳送資料，您會有合起來 1GB 的資料額度，套用至該訂用帳戶中的所有 Application Insights 資源。 由於內含資料會在所有節點之間共用，因此特定節點所傳送的資料是否超過其他節點並無妨。 如果某一天，Application Insights 資源收到超過此訂用帳戶的每日資料配置中包含的資料，適用於每 GB 超額資料傳輸費用。 
  ...
  * 每個節點層每日額度不會被共用，您已選擇 每 GB 層的應用程式。 未使用額度不會逐日累計。 
...
| 在 2 部 Vm; 上執行的 3 個應用程式這些應用程式的 Application Insights 資源已在相同的訂用帳戶和每個節點層 | 2 | 
...
您可以撰寫指令碼以使用 Azure Resource Management 設定的定價層。 [了解作法](powershell.md#price)。
