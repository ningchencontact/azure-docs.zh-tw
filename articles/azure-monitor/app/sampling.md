...
2. **停用調適性取樣**：在 [ApplicationInsights.config](../../azure-monitor/app/configuration-with-applicationinsights-config.md) 中，將 `AdaptiveSamplingTelemetryProcessor` 節點移除或設成註解。
...
3. **啟用固定取樣率模組。** 將此程式碼片段新增至 [ApplicationInsights.config](../../azure-monitor/app/configuration-with-applicationinsights-config.md)：
...
* 如果您使用 Java SDK 版本 2.0.1 或更新版本，就可以設定 ApplicationInsights.xml 來開啟固定速率取樣。 根據預設取樣功能為關閉。 使用固定取樣率，瀏覽器 SDK 會自動同步至取樣相關的事件。
