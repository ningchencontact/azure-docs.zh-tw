...
Application Insights Profiler 會每隔一小時收集兩分鐘的 Profiler 資料，而在 [設定 Application Insights Profiler] 頁面按下 [[立即分析](profiler-settings.md ?toc=/azure/azure-monitor/toc.json)] 按鈕時也會進行相同操作。 但只有在其可以附加至 Profiler 執行時所提出的要求時，才會上傳分析資料。 
