...
![彈性的標誌](./media/partners/elastic.png)
...
![Logic Monitor 標誌](./media/partners/logicmonitor.png)
...
Microsoft Azure DSM 與 Microsoft Azure 事件中樞通訊協定均可從 [IBM 支援網站](https://www.ibm.com/support)下載。 您可以[從這裡深入了解與 Azure 的整合][qradar-doc]。
...
Serverless360 是一個平台工具，以便操作、 管理及監視 Azure 無伺服器元件。 管理能力是其中一個主要的挑戰與無伺服器的實作。 數百個小型且獨立的無伺服器服務會散佈於不同位置，管理和操作這類解決方案很複雜。 Serverless360 可解決這些挑戰與豐富的精細的工具。 它可以監視無伺服器的服務，例如 Azure Functions、 Logic Apps，Event Grids，服務匯流排佇列、 主題、 轉送、 事件中樞、 儲存體佇列、 檔案、 blob 和資料表。 Serverless360 適用於 Azure Marketplace。 這些功能是適用於 SaaS 和私用裝載 （裝載於您自己的環境）。  
...
SignalFx 是即時營運智慧中的翹楚，適用於資料導向的 DevOps。 此服務會探索並收集雲端中每個元件的計量。 它取代了傳統的點工具，並可供即時掌握現今的動態環境。 利用可大幅調整的 SignalFx 平台，SaaS 平台已針對容器和微服務型架構進行最佳化，並在所有大小的組織之間提供功能強大視覺效果、主動警示以及共同作業分級功能。 SignalFx 整合，直接與 Azure 監視器，以及透過開放原始碼連接器這類*Telegraf*， *statsD*，並*collectd*提供最佳的類別儀表板、 分析和適用於 Azure 的警示。
...
![Sumo Logic 標誌](./media/partners/SumoLogic.png)
...
[influxdata-doc]: ./../../azure-monitor/platform/collect-custom-metrics-linux-telegraf.md "Azure 監視流入資料 Telegraf 整合。"
...
[serverless360-doc]: https://docs.serverless360.com/docs/associating-service-principals "Serverless360 文件。"
