...
從 [Log Analytics [設定] 中的 [資料] 功能表](agent-data-sources.md#configuring-data-sources)來設定 Log Analytics 中的 IIS 記錄檔。  您只需選取 [Collect W3C format IIS log files]\(收集 W3C 格式的 IIS 記錄檔) 即可完成設定。
...
* 了解 [記錄檔搜尋](../../log-analytics/log-analytics-queries.md) ，其可分析從資料來源和方案所收集的資料。
