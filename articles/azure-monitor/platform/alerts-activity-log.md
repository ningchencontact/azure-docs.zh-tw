---
title: 建立、 檢視和管理活動中 Azure 監視器的記錄警示
description: 如何使用 Azure 入口網站、 Azure Resource Manager 範本和 Azure PowerShell 建立活動記錄警示。
author: msvijayn
...
3. 在 [定義警示條件]  下提供下列資訊，然後按一下 [完成]  。
   ...
   - 底下**目標準則**，按一下**新增條件**並包括各種類別目錄的顯示目標的所有可用訊號**活動記錄檔**;中的類別目錄名稱附加**監視服務**名稱。

   - 從顯示的各種作業可能類型的清單中選取訊號**活動記錄**。
     ...
     **警示邏輯**：
     ...
     - **事件等級**- 事件的嚴重性等級。 _詳細資訊_ _資訊_ _警告_ _錯誤_或_嚴重_。
...
或者，是瀏覽或篩選事件，透過簡單的比喻可以了解條件可以在 活動記錄檔，建立警示規則所在[Azure 入口網站中的活動記錄](activity-log-view.md#azure-portal)。 在 Azure 監視-活動記錄檔，可以篩選或尋找必要的事件並使用，以建立警示**新增活動記錄警示**按鈕，然後遵循步驟 4 和更新版本如上述的教學課程中所述。
...
[Azure 監視-活動記錄警示 API](https://docs.microsoft.com/rest/api/monitor/activitylogalerts)為 REST API 並使用 Azure Resource Manager REST API 完全相容。 因此它可以透過 Powershell 和 Azure CLI 搭配資源管理員 Cmdlet 來使用。
