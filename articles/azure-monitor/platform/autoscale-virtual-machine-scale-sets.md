...
7. 最後，我們來看一下自動調整的**通知**區段。 自動調整通知能讓您在相應放大或縮小動作成功觸發時執行三件事。
   ...
   - 傳送電子郵件給一組使用者
   - 觸發 webhook 呼叫。 引發時，此 webhook 會傳送有關自動調整狀態和擴展集資源的中繼資料。 若要深入了解自動調整 webhook 的承載，請參閱[針對自動調整設定 Webhook 與電子郵件通知](autoscale-webhook-email.md)。
...
[使用 PowerShell 管理自動調整](../../monitoring-and-diagnostics/insights-powershell-samples.md#create-and-manage-autoscale-settings)
