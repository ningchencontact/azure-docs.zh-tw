...
|private_bytes_metric|私用位元組 |位元組|平均值|The total amount of memory the Analysis Services engine process and Mashup container processes have allocated, not including memory shared with other processes.|ServerResourceType|
|virtual_bytes_metric|虛擬位元組 |位元組|平均值|The current size of the virtual address space that Analysis Services engine process and Mashup container processes are using.|ServerResourceType|
...
|CurrentConnections|連接：目前的連接|計數|平均值|目前已建立的用戶端連接數目。|ServerResourceType|
...
|mashup_engine_private_bytes_metric|M Engine Private Bytes |位元組|平均值|The total amount of memory Mashup container processes have allocated, not including memory shared with other processes.|ServerResourceType|
|mashup_engine_virtual_bytes_metric|M Engine Virtual Bytes |位元組|平均值|The current size of the virtual address space Mashup container processes are using.|ServerResourceType|

## <a name="microsoftapimanagementservice"></a>Microsoft.ApiManagement/service

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|Requests|Requests|計數|總計|The total number of gateway requests in a given period. It can be sliced by various dimensions to help you diagnose issues. |Location, BackendResponseCode, LastErrorReason, GatewayResponseCode|
|TotalRequests|閘道要求總數|計數|總計|The total number of gateway requests in a given period. This metric has been deprecated, we recommend using the new `Requests` metric. |位置，主機名稱|
|SuccessfulRequests|成功的閘道要求|計數|總計|The total number of successful gateway requests in a given period. This metric has been deprecated, we recommend using the new `Requests` metric.|位置，主機名稱|
|UnauthorizedRequests|未經授權閘道器要求|計數|總計| The total number of unauthorized gateway requests in a given period. This metric has been deprecated, we recommend using the new `Requests` metric.|位置，主機名稱|
|FailedRequests|失敗的閘道要求|計數|總計|The total number of failed gateway requests in a given period. This metric has been deprecated, we recommend using the new `Requests` metric.|位置，主機名稱|
|OtherRequests|其他閘道器要求|計數|總計|The total number of gateway requests in a given period that do not fall into the successful, unauthorized, or failed categories. This metric has been deprecated, we recommend using the new `Requests` metric. |位置，主機名稱|
|課程時間|閘道要求的整體持續期間|毫秒|平均值|The time between when API Management receives a request from a client and when it returns a response to the client.|位置，主機名稱|
...
|EventHubTotalEvents|Total EventHub Events|計數|總計|The total number of events sent to EventHub from API Management in a given period.|Location|
|EventHubSuccessfulEvents|Successful EventHub Events|計數|總計|The total number of successful EventHub events in a given period.|Location|
|EventHubTotalFailedEvents|Failed EventHub Events|計數|總計|The total number of failed EventHub events in a given period.|Location|
|EventHubRejectedEvents|Rejected EventHub Events|計數|總計|The total number of rejected EventHub events (wrong configuration or unauthorized) in a given period.|Location|
|EventHubThrottledEvents|Throttled EventHub Events|計數|總計|The total number of throttled EventHub events in a given period.|Location|
|EventHubTimedoutEvents|Timed Out EventHub Events|計數|總計|The total number of timed out EventHub events in a given period.|Location|
|EventHubDroppedEvents|Dropped EventHub Events|計數|總計|The total number of events skipped because of queue size limit reached in a given period.|Location|
|EventHubTotalBytesSent|Size of EventHub Events|位元組|總計|The total size of EventHub events in bytes in a given period.|Location|
...
|TotalUpdateDeploymentRuns|Total Update Deployment Runs|計數|總計|Total software update deployment runs|SoftwareUpdateConfigurationName, Status|
|TotalUpdateDeploymentMachineRuns|Total Update Deployment Machine Runs|計數|總計|Total software update deployment machine runs in a software update deployment run|SoftwareUpdateConfigurationName, Status, TargetComputer, SoftwareUpdateConfigurationRunId|
...
|CreatingNodeCount|建立節點計數|計數|總計|正在建立的節點數目|無維度|
|StartingNodeCount|啟動節點計數|計數|總計|啟動的節點數目|無維度|
|WaitingForStartTaskNodeCount|等候啟動工作節點計數|計數|總計|等待啟動工作完成的節點數目|無維度|
|StartTaskFailedNodeCount|啟動工作失敗的節點計數|計數|總計|啟動工作失敗的節點數目|無維度|
|IdleNodeCount|閒置的節點計數|計數|總計|閒置節點的數目|無維度|
...
|RunningNodeCount|執行節點計數|計數|總計|執行節點的數目|無維度|
...
|setcommands9|設定 (分區 9)|計數|總計||無維度|
...
|延遲|延遲|毫秒|平均值|延遲，以毫秒為單位。|ApiName, OperationName, Region|
|CharactersTranslated|轉譯字元數|計數|總計|傳入文字要求中的字元總數。|ApiName, OperationName, Region|
|CharactersTrained|Characters Trained|計數|總計|Total number of characters trained.|ApiName, OperationName, Region|
|SpeechSessionDuration|語音工作階段持續時間|秒|總計|語音工作階段的總持續時間 (秒)。|ApiName, OperationName, Region|
|TotalTransactions|交易數總計|計數|總計|交易總數。|無維度|
...
|Network In|Network In Billable|位元組|總計|The number of billable bytes received on all network interfaces by the Virtual Machine(s) (Incoming Traffic)|無維度|
|Network Out|Network Out Billable|位元組|總計|The number of billable bytes out on all network interfaces by the Virtual Machine(s) (Outgoing Traffic)|無維度|
|Disk Read Bytes|Disk Read Bytes|位元組|總計|Bytes read from disk during monitoring period|無維度|
|Disk Write Bytes|Disk Write Bytes|位元組|總計|Bytes written to disk during monitoring period|無維度|
|Disk Read Operations/Sec|Disk Read Operations/Sec|每秒計數|平均值|磁碟讀取 IOPS|無維度|
|Disk Write Operations/Sec|Disk Write Operations/Sec|每秒計數|平均值|磁碟寫入 IOPS|無維度|
|剩餘 CPU 信用額度|剩餘 CPU 信用額度|計數|平均值|可用於高載的信用額度總數|無維度|
|CPU Credits Consumed|CPU Credits Consumed|計數|平均值|由虛擬機器取用的信用額度總數|無維度|
|每一磁碟讀取位元組/秒|Data Disk Read Bytes/Sec (Deprecated)|每秒計數|平均值|Bytes/Sec read from a single disk during monitoring period|SlotId|
|每一磁碟寫入位元組/秒|Data Disk Write Bytes/Sec (Deprecated)|每秒計數|平均值|Bytes/Sec written to a single disk during monitoring period|SlotId|
|每一磁碟讀取作業/秒|Data Disk Read Operations/Sec (Deprecated)|每秒計數|平均值|Read IOPS from a single disk during monitoring period|SlotId|
|每一磁碟寫入作業/秒|Data Disk Write Operations/Sec (Deprecated)|每秒計數|平均值|Write IOPS from a single disk during monitoring period|SlotId|
|每一磁碟 QD|Data Disk QD (Deprecated)|計數|平均值|資料磁碟佇列深度 (或佇列長度)|SlotId|
|OS 每一磁碟讀取位元組/秒|OS Disk Read Bytes/Sec (Deprecated)|每秒計數|平均值|Bytes/Sec read from a single disk during monitoring period for OS disk|無維度|
|OS 每一磁碟寫入位元組/秒|OS Disk Write Bytes/Sec (Deprecated)|每秒計數|平均值|Bytes/Sec written to a single disk during monitoring period for OS disk|無維度|
|OS 每一磁碟讀取作業/秒|OS Disk Read Operations/Sec (Deprecated)|每秒計數|平均值|Read IOPS from a single disk during monitoring period for OS disk|無維度|
|OS 每一磁碟寫入作業/秒|OS Disk Write Operations/Sec (Deprecated)|每秒計數|平均值|Write IOPS from a single disk during monitoring period for OS disk|無維度|
|OS 每一磁碟 QD|OS Disk QD (Deprecated)|計數|平均值|OS 磁碟佇列深度 (或佇列長度)|無維度|
|Data Disk Read Bytes/sec|資料磁碟讀取位元組/秒 (預覽)|每秒計數|平均值|Bytes/Sec read from a single disk during monitoring period|LUN|
|Data Disk Write Bytes/sec|資料磁碟寫入位元組/秒 (預覽)|每秒計數|平均值|Bytes/Sec written to a single disk during monitoring period|LUN|
|Data Disk Read Operations/Sec|資料磁碟讀取作業/秒 (預覽)|每秒計數|平均值|Read IOPS from a single disk during monitoring period|LUN|
|Data Disk Write Operations/Sec|資料磁碟寫入作業/秒 (預覽)|每秒計數|平均值|Write IOPS from a single disk during monitoring period|LUN|
|Data Disk Queue Depth|Data Disk Queue Depth (Preview)|計數|平均值|資料磁碟佇列深度 (或佇列長度)|LUN|
|OS Disk Read Bytes/sec|OS 磁碟讀取位元組/秒 (預覽)|每秒計數|平均值|Bytes/Sec read from a single disk during monitoring period for OS disk|無維度|
|OS Disk Write Bytes/sec|OS 磁碟寫入位元組/秒 (預覽)|每秒計數|平均值|Bytes/Sec written to a single disk during monitoring period for OS disk|無維度|
|OS Disk Read Operations/Sec|OS 磁碟讀取作業/秒 (預覽)|每秒計數|平均值|Read IOPS from a single disk during monitoring period for OS disk|無維度|
|OS Disk Write Operations/Sec|OS 磁碟寫入作業/秒 (預覽)|每秒計數|平均值|Write IOPS from a single disk during monitoring period for OS disk|無維度|
|OS Disk Queue Depth|OS Disk Queue Depth (Preview)|計數|平均值|OS 磁碟佇列深度 (或佇列長度)|無維度|
|Inbound Flows|Inbound Flows (Preview)|計數|平均值|Inbound Flows are number of current flows in the inbound direction (traffic going into the VM)|無維度|
|Outbound Flows|Outbound Flows (Preview)|計數|平均值|Outbound Flows are number of current flows in the outbound direction (traffic going out of the VM)|無維度|
|Inbound Flows Maximum Creation Rate|Inbound Flows Maximum Creation Rate (Preview)|每秒計數|平均值|The maximum creation rate of inbound flows (traffic going into the VM)|無維度|
|Outbound Flows Maximum Creation Rate|Outbound Flows Maximum Creation Rate (Preview)|每秒計數|平均值|The maximum creation rate of outbound flows (traffic going out of the VM)|無維度|
...
|Premium OS Disk Cache Read Hit|Premium OS Disk Cache Read Hit (Preview)|百分比|平均值|Premium OS Disk Cache Read Hit|無維度|
|Premium OS Disk Cache Read Miss|Premium OS Disk Cache Read Miss (Preview)|百分比|平均值|Premium OS Disk Cache Read Miss|無維度|
|Network In Total|Network In Total|位元組|總計|虛擬機器 (連入流量) 在所有網路介面上接收到的位元組數目|無維度|
|Network Out Total|Network Out Total|位元組|總計|虛擬機器 (連出流量) 在所有網路介面上送出的位元組數目|無維度|

## <a name="microsoftcomputevirtualmachinescalesets"></a>Microsoft.Compute/virtualMachineScaleSets

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|Percentage CPU|Percentage CPU|百分比|平均值|目前虛擬機器正在使用中的已配置計算單位百分比|VMName|
|Network In|Network In Billable|位元組|總計|The number of billable bytes received on all network interfaces by the Virtual Machine(s) (Incoming Traffic)|VMName|
|Network Out|Network Out Billable|位元組|總計|The number of billable bytes out on all network interfaces by the Virtual Machine(s) (Outgoing Traffic)|VMName|
|Disk Read Bytes|Disk Read Bytes|位元組|總計|Bytes read from disk during monitoring period|VMName|
|Disk Write Bytes|Disk Write Bytes|位元組|總計|Bytes written to disk during monitoring period|VMName|
|Disk Read Operations/Sec|Disk Read Operations/Sec|每秒計數|平均值|磁碟讀取 IOPS|VMName|
|Disk Write Operations/Sec|Disk Write Operations/Sec|每秒計數|平均值|磁碟寫入 IOPS|VMName|
|剩餘 CPU 信用額度|剩餘 CPU 信用額度|計數|平均值|可用於高載的信用額度總數|無維度|
|CPU Credits Consumed|CPU Credits Consumed|計數|平均值|由虛擬機器取用的信用額度總數|無維度|
|每一磁碟讀取位元組/秒|Data Disk Read Bytes/Sec (Deprecated)|每秒計數|平均值|Bytes/Sec read from a single disk during monitoring period|SlotId|
|每一磁碟寫入位元組/秒|Data Disk Write Bytes/Sec (Deprecated)|每秒計數|平均值|Bytes/Sec written to a single disk during monitoring period|SlotId|
...
|OS 每一磁碟寫入作業/秒|OS Disk Write Operations/Sec (Deprecated)|每秒計數|平均值|Write IOPS from a single disk during monitoring period for OS disk|無維度|
|OS 每一磁碟 QD|OS Disk QD (Deprecated)|計數|平均值|OS 磁碟佇列深度 (或佇列長度)|無維度|
|Data Disk Read Bytes/sec|資料磁碟讀取位元組/秒 (預覽)|每秒計數|平均值|Bytes/Sec read from a single disk during monitoring period|LUN, VMName|
|Data Disk Write Bytes/sec|資料磁碟寫入位元組/秒 (預覽)|每秒計數|平均值|Bytes/Sec written to a single disk during monitoring period|LUN, VMName|
|Data Disk Read Operations/Sec|資料磁碟讀取作業/秒 (預覽)|每秒計數|平均值|Read IOPS from a single disk during monitoring period|LUN, VMName|
|Data Disk Write Operations/Sec|資料磁碟寫入作業/秒 (預覽)|每秒計數|平均值|Write IOPS from a single disk during monitoring period|LUN, VMName|
|Data Disk Queue Depth|Data Disk Queue Depth (Preview)|計數|平均值|資料磁碟佇列深度 (或佇列長度)|LUN, VMName|
|OS Disk Read Bytes/sec|OS 磁碟讀取位元組/秒 (預覽)|每秒計數|平均值|Bytes/Sec read from a single disk during monitoring period for OS disk|VMName|
|OS Disk Write Bytes/sec|OS 磁碟寫入位元組/秒 (預覽)|每秒計數|平均值|Bytes/Sec written to a single disk during monitoring period for OS disk|VMName|
|OS Disk Read Operations/Sec|OS 磁碟讀取作業/秒 (預覽)|每秒計數|平均值|Read IOPS from a single disk during monitoring period for OS disk|VMName|
|OS Disk Write Operations/Sec|OS 磁碟寫入作業/秒 (預覽)|每秒計數|平均值|Write IOPS from a single disk during monitoring period for OS disk|VMName|
|OS Disk Queue Depth|OS Disk Queue Depth (Preview)|計數|平均值|OS 磁碟佇列深度 (或佇列長度)|VMName|
|Inbound Flows|Inbound Flows (Preview)|計數|平均值|Inbound Flows are number of current flows in the inbound direction (traffic going into the VM)|VMName|
|Outbound Flows|Outbound Flows (Preview)|計數|平均值|Outbound Flows are number of current flows in the outbound direction (traffic going out of the VM)|VMName|
|Inbound Flows Maximum Creation Rate|Inbound Flows Maximum Creation Rate (Preview)|每秒計數|平均值|The maximum creation rate of inbound flows (traffic going into the VM)|VMName|
|Outbound Flows Maximum Creation Rate|Outbound Flows Maximum Creation Rate (Preview)|每秒計數|平均值|The maximum creation rate of outbound flows (traffic going out of the VM)|VMName|
...
|Network In Total|Network In Total|位元組|總計|虛擬機器 (連入流量) 在所有網路介面上接收到的位元組數目|VMName|
|Network Out Total|Network Out Total|位元組|總計|虛擬機器 (連出流量) 在所有網路介面上送出的位元組數目|VMName|

## <a name="microsoftcontainerinstancecontainergroups"></a>Microsoft.ContainerInstance/containerGroups

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|CpuUsage|CPU 使用率|計數|平均值|所有核心的 CPU 使用率 (以 millicores 為單位)。|containerName|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|TotalPullCount|Total Pull Count|計數|平均值|Number of image pulls in total|無維度|
|SuccessfulPullCount|Successful Pull Count|計數|平均值|Number of successful image pulls|無維度|
|TotalPushCount|Total Push Count|計數|平均值|Number of image pushes in total|無維度|
|SuccessfulPushCount|Successful Push Count|計數|平均值|Number of successful image pushes|無維度|
|RunDuration|Run Duration|毫秒|總計|Run Duration in milliseconds|無維度|
...
|kube_node_status_condition|各種節點條件的狀態|計數|總計|各種節點條件的狀態|condition, status, status2, node|
...
|DCIProfilesCount|設定檔執行個體計數|計數|最後一頁||無維度|
|DCIInteractionsPerMonthCount|每月互動數計數|計數|最後一頁||無維度|
|DCIKpisCount|KPI 計數|計數|最後一頁||無維度|
|DCISegmentsCount|區段計數|計數|最後一頁||無維度|
|DCIPredictiveMatchPoliciesCount|預測比對計數|計數|最後一頁||無維度|
|DCIPredictionsCount|預測計數|計數|最後一頁||無維度|

## <a name="microsoftdataboxedgedataboxedgedevices"></a>Microsoft.DataBoxEdge/dataBoxEdgeDevices

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|NICReadThroughput|Read Throughput (Network)|每秒位元組|平均值|The read throughput of the network interface on the device in the reporting period for all volumes in the gateway.|InstanceName|
|NICWriteThroughput|Write Throughput (Network)|每秒位元組|平均值|The write throughput of the network interface on the device in the reporting period for all volumes in the gateway.|InstanceName|
|CloudReadThroughputPerShare|Cloud Download Throughput (Share)|每秒位元組|平均值|The download throughput to Azure from a share during the reporting period.|共用|
|CloudUploadThroughputPerShare|Cloud Upload Throughput (Share)|每秒位元組|平均值|The upload throughput to Azure from a share during the reporting period.|共用|
|BytesUploadedToCloudPerShare|Cloud Bytes Uploaded (Share)|位元組|平均值|The total number of bytes that is uploaded to Azure from a share during the reporting period.|共用|
|TotalCapacity|Total Capacity|位元組|平均值|Total Capacity|無維度|
|AvailableCapacity|Available Capacity|位元組|平均值|The available capacity in bytes during the reporting period.|無維度|
|CloudUploadThroughput|Cloud Upload Throughput|每秒位元組|平均值|The cloud upload throughput  to Azure during the reporting period.|無維度|
|CloudReadThroughput|Cloud Download Throughput|每秒位元組|平均值|The cloud download throughput to Azure during the reporting period.|無維度|
|BytesUploadedToCloud|Cloud Bytes Uploaded (Device)|位元組|平均值|The total number of bytes that is uploaded to Azure from a device during the reporting period.|無維度|
...
|MaxAllowedResourceCount|Maximum allowed entities count|計數|最大值||無維度|
|MaxAllowedFactorySizeInGbUnits|Maximum allowed factory size (GB unit)|計數|最大值||無維度|
|ResourceCount|Total entities count|計數|最大值||無維度|
|FactorySizeInGbUnits|Total factory size (GB unit)|計數|最大值||無維度|
...
|JobEndedCancelled|Canceled Jobs|計數|總計|Count of canceled jobs.|無維度|
|JobAUEndedSuccess|成功 AU 時間|秒|總計|成功作業的 AU 時間總計。|無維度|
|JobAUEndedFailure|失敗 AU 時間|秒|總計|失敗作業的 AU 時間總計。|無維度|
|JobAUEndedCancelled|Canceled AU Time|秒|總計|Total AU time for canceled jobs.|無維度|
...
|storage_used|使用的儲存空間|位元組|平均值|使用的儲存空間|無維度|
...
|backup_storage_used|Backup Storage used|位元組|平均值|Backup Storage used|無維度|
...
|network_bytes_egress|Network Out|位元組|總計|跨作用中連線的網路輸出|無維度|
|network_bytes_ingress|Network In|位元組|總計|跨作用中連線的網路輸入|無維度|
|pg_replica_log_delay_in_seconds|複本延隔時間|秒|最大值|Replica lag in seconds|無維度|
|pg_replica_log_delay_in_bytes|複本之間的最大延隔時間|位元組|最大值|Lag in bytes of the most lagging replica|無維度|
...
|cpu_percent|CPU 百分比|百分比|平均值|CPU 百分比|無維度|
|memory_percent|記憶體百分比|百分比|平均值|記憶體百分比|無維度|
|iops|IOPS|計數|平均值|IO Operations per second|無維度|
|storage_percent|儲存體百分比|百分比|平均值|儲存體百分比|無維度|
|storage_used|使用的儲存空間|位元組|平均值|使用的儲存空間|無維度|
|active_connections|作用中的連線|計數|平均值|作用中的連線|無維度|
|network_bytes_egress|Network Out|位元組|總計|跨作用中連線的網路輸出|無維度|
...
|d2c.endpoints.egress.builtIn.events|路由：傳遞至訊息/事件的訊息|計數|總計|IoT 中樞路由成功將訊息傳遞至內建端點 (訊息/事件) 的次數。 This metric only starts working when routing is enabled (https://aka.ms/iotrouting) for the IoT hub.|無維度|
|d2c.endpoints.latency.builtIn.events|路由：訊息/事件的訊息延遲|毫秒|平均值|訊息輸入到 IoT 中樞與遙測訊息輸入到內建端點 (訊息/事件) 之間的平均延遲 (毫秒)。 This metric only starts working when routing is enabled (https://aka.ms/iotrouting) for the IoT hub.|無維度|
...
|EventGridDeliveries|Event Grid deliveries (preview)|計數|總計|The number of IoT Hub events published to Event Grid. Use the Result dimension for the number of successful and failed requests. EventType dimension shows the type of event (https://aka.ms/ioteventgrid).|Result, EventType|
...
|deviceDataUsage|Total device data usage|位元組|總計|傳輸至任何已連線至 IotHub 之裝置或從該裝置傳出的位元組數|無維度|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|AvailableStorage|可用的儲存體|位元組|總計|Total available storage reported at 5 minutes granularity|CollectionName, DatabaseName, Region|
|CassandraConnectionClosures|Cassandra 連線終止|計數|總計|Number of Cassandra connections that were closed, reported at a 1 minute granularity|Region, ClosureReason|
|CassandraRequestCharges|Cassandra 要求費用|計數|總計|RUs consumed for Cassandra requests made|DatabaseName、CollectionName、Region、OperationType、ResourceType|
|CassandraRequests|Cassandra 要求|計數|計數|Number of Cassandra requests made|DatabaseName, CollectionName, Region, OperationType, ResourceType, ErrorCode|
|DataUsage|資料使用量|位元組|總計|Total data usage reported at 5 minutes granularity|CollectionName, DatabaseName, Region|
|DocumentCount|文件計數|計數|總計|Total document count reported at 5 minutes granularity|CollectionName, DatabaseName, Region|
|DocumentQuota|文件配額|位元組|總計|Total storage quota reported at 5 minutes granularity|CollectionName, DatabaseName, Region|
|IndexUsage|索引使用量|位元組|總計|Total index usage reported at 5 minutes granularity|CollectionName, DatabaseName, Region|
|MetadataRequests|中繼資料要求|計數|計數|中繼資料要求計數。 Cosmos DB maintains system metadata collection for each account, that allows you to enumerate collections, databases, etc., and their configurations, free of charge.|DatabaseName, CollectionName, Region, StatusCode, |
...
|ReplicationLatency|P99 Replication Latency|毫秒|平均值|異地複寫啟用的帳戶其跨來源和目標區域的 P99 複寫延遲|SourceRegion、TargetRegion|
|ServiceAvailability|服務可用性|百分比|平均值|Account requests availability at one hour, day or month granularity|無維度|
...
|PublishFailCount|Publish Failed Events|計數|總計|發佈至本主題失敗的事件總數|ErrorType、Error|
|UnmatchedEventCount|不相符的事件|計數|總計|不符合本主題中任何事件訂閱的事件總數|無維度|
|PublishSuccessLatencyInMs|Publish Success Latency|計數|總計|Publish success latency in milliseconds|無維度|
...
|大小|大小|位元組|平均值|EventHub 的大小 (以位元組為單位)。|EntityName|
|INREQS|Incoming Requests (Deprecated)|計數|總計|Total incoming send requests for a namespace (Deprecated)|無維度|
|SUCCREQ|Successful Requests (Deprecated)|計數|總計|Total successful requests for a namespace (Deprecated)|無維度|
|FAILREQ|Failed Requests (Deprecated)|計數|總計|Total failed requests for a namespace (Deprecated)|無維度|
|SVRBSY|Server Busy Errors (Deprecated)|計數|總計|Total server busy errors for a namespace (Deprecated)|無維度|
|INTERR|Internal Server Errors (Deprecated)|計數|總計|Total internal server errors for a namespace (Deprecated)|無維度|
|MISCERR|Other Errors (Deprecated)|計數|總計|Total failed requests for a namespace (Deprecated)|無維度|
|INMSGS|Incoming Messages (Deprecated) (Deprecated)|計數|總計|命名空間的傳入訊息總數。 此計量已過時。 Please use Incoming Messages metric instead (Deprecated)|無維度|
|EHINMSGS|連入訊息數 (已過時)|計數|總計|Total incoming messages for a namespace (Deprecated)|無維度|
|OUTMSGS|Outgoing Messages (Deprecated) (Deprecated)|計數|總計|命名空間的傳出訊息總數。 此計量已過時。 Please use Outgoing Messages metric instead (Deprecated)|無維度|
|EHOUTMSGS|傳出訊息數 (已過時)|計數|總計|Total outgoing messages for a namespace (Deprecated)|無維度|
|EHINMBS|Incoming bytes (Deprecated) (Deprecated)|位元組|總計|命名空間的事件中樞傳入訊息輸送量。 此計量已過時。 Please use Incoming bytes metric instead (Deprecated)|無維度|
|EHINBYTES|傳入位元組數 (已過時)|位元組|總計|Event Hub incoming message throughput for a namespace (Deprecated)|無維度|
|EHOUTMBS|Outgoing bytes (Deprecated) (Deprecated)|位元組|總計|命名空間的事件中樞傳出訊息輸送量。 此計量已過時。 Please use Outgoing bytes metric instead (Deprecated)|無維度|
|EHOUTBYTES|傳出位元組數 (已過時)|位元組|總計|Event Hub outgoing message throughput for a namespace (Deprecated)|無維度|
|EHABL|Archive backlog messages (Deprecated)|計數|總計|Event Hub archive messages in backlog for a namespace (Deprecated)|無維度|
|EHAMSGS|Archive messages (Deprecated)|計數|總計|Event Hub archived messages in a namespace (Deprecated)|無維度|
|EHAMBS|Archive message throughput (Deprecated)|位元組|總計|Event Hub archived message throughput in a namespace (Deprecated)|無維度|
...
|IncomingMessages|傳入訊息 (預覽)|計數|總計|Microsoft.EventHub 的傳入訊息。 (預覽)|無維度|
|OutgoingMessages|傳出訊息 (預覽)|計數|總計|Microsoft.EventHub 的傳出訊息。 (預覽)|無維度|
...
|NumActiveWorkers|Number of Active Workers|計數|最大值|Number of Active Workers|ClusterDnsName, MetricName|

## <a name="microsoftinsightsautoscalesettings"></a>Microsoft.Insights/AutoscaleSettings

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|ObservedMetricValue|觀察的計量值|計數|平均值|執行時由自動調整規模計算的值|MetricTriggerSource|
|MetricThreshold|計量閾值|計數|平均值|當自動調整規模執行時設定的自動調整規模閾值。|MetricTriggerRule|
|ObservedCapacity|觀察的容量|計數|平均值|執行時向自動調整規模回報的容量。|無維度|
...
|availabilityResults/count|可用性集合|計數|計數|Count of availability tests|availabilityResult/name、availabilityResult/location、availabilityResult/success|
...
|dependencies/failed|Dependency call failures|計數|計數|應用程式對外部資源進行相依性呼叫的失敗計數。|dependency/type、dependency/performanceBucket、operation/synthetic、cloud/roleInstance、cloud/roleName|
...
|requests/rate|Server request rate|每秒計數|平均值|Rate of server requests per second|request/performanceBucket、request/resultCode、operation/synthetic、cloud/roleInstance、request/success、cloud/roleName|
|exceptions/count|例外狀況|計數|計數|所有未攔截的例外狀況合併計數。|cloud/roleName、cloud/roleInstance、client/type|
|exceptions/browser|瀏覽器例外狀況|計數|計數|在瀏覽器中擲回的未攔截例外狀況計數。|無維度|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|CacheUtilization|快取使用率|百分比|平均值|在叢集範圍的使用率等級|None|
|QueryDuration|查詢持續時間|毫秒|平均值|查詢持續時間 (以秒為單位)|查詢狀態|
|IngestionUtilization|擷取使用率|百分比|平均值|在叢集中使用的擷取位置的比率|None|
|KeepAlive|保持運作|計數|平均值|例行性檢查指出對查詢的叢集回應|None|
|IngestionVolumeInMB|擷取量 (以 MB 為單位)|計數|總計|擷取到叢集的資料總量 (以 MB 為單位)|資料庫|
|IngestionLatencyInSeconds|擷取延遲 (以秒為單位)|秒|平均值|從來源 (例如，訊息位於 EventHub 中) 擷取到叢集的擷取時間，以秒為單位|None|
|EventProcessedForEventHubs|已處理的事件 (針對事件中樞)|計數|總計|從事件中樞擷取時，由叢集處理的事件數目|None|
|IngestionResult|擷取結果|計數|計數|擷取作業數|狀態|
|CPU|CPU|百分比|平均值|CPU 使用率等級|None|
| ContinuousExportNumOfRecordsExported | Number of records exported in continuous export | 計數 | 總計 | Number of records exported for every storage artifact written during the export operation  | None |
| ExportUtilization | Export Utilization | 百分比 | 最大值 | Export utilization | None |
| ContinuousExportPendingCount | Continuous Export Pending Count | 計數 | 最大值 | The number of pending continuous export jobs ready for execution | None |
| ContinuousExportMaxLatenessMinutes | Continuous Export Max Lateness Minutes | 計數 | 最大值 | The max time in minutes of all continuous exports which are pending and ready for execution | None |

## <a name="microsoftlocationbasedservicesaccounts"></a>Microsoft.LocationBasedServices/accounts

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|用量|用量|計數|計數|API 呼叫的計數|ApiCategory、ApiName|
...
|RunsCancelled|Runs Canceled|計數|總計|Number of workflow runs canceled.|無維度|
...
|ActionsSucceeded|成功的動作 |計數|總計|已成功的工作流程動作數目。|無維度|
|ActionsFailed|動作的失敗|計數|總計|已失敗的工作流程動作數目。|無維度|
...
|ActionThrottledEvents|動作節流的事件|計數|總計|工作流程動作已節流的事件數目。|無維度|
|TriggersStarted|啟動的觸發程序 |計數|總計|已啟動的工作流程觸發程序數目。|無維度|
|TriggersCompleted|完成的觸發程序 |計數|總計|已完成的工作流程觸發程序數目。|無維度|
|TriggersSucceeded|成功的觸發程序 |計數|總計|已成功的工作流程觸發程序數目。|無維度|
|TriggersFailed|失敗的觸發程序 |計數|總計|已失敗的工作流程觸發程序數目。|無維度|
...
|TriggerThrottledEvents|觸發程序節流的事件|計數|總計|工作流程觸發程序節流的事件數目。|無維度|
|BillableActionExecutions|可計費的動作執行|計數|總計|計費的工作流程動作執行數目。|無維度|
|BillableTriggerExecutions|可計費的觸發程序執行|計數|總計|計費的工作流程觸發程序執行數目。|無維度|
|TotalBillableExecutions|可計費的總執行次數|計數|總計|計費的工作流程執行數目。|無維度|
|BillingUsageNativeOperation|為使用原生作業執行計費|計數|總計|要計費的原生作業執行數。|無維度|
...
|RunSuccessLatency|執行成功的延遲|秒|平均值|已成功的工作流程執行延遲。|無維度|
|RunThrottledEvents|執行節流的事件|計數|總計|工作流程動作或觸發節流的事件數目。|無維度|
|RunStartThrottledEvents|Run Start Throttled Events|計數|總計|Number of workflow run start throttled events.|無維度|
|RunFailurePercentage|執行失敗百分比|百分比|總計|已失敗的工作流程執行百分比。|無維度|
|ActionsStarted|啟動的動作 |計數|總計|已啟動的工作流程動作數目。|無維度|
|ActionsCompleted|完成的動作 |計數|總計|已完成的工作流程動作數目。|無維度|
|ActionsSucceeded|成功的動作 |計數|總計|已成功的工作流程動作數目。|無維度|
...
|TriggerSuccessLatency|觸發程序成功延遲 |秒|平均值|已成功的工作流程觸發程序延遲。|無維度|
|TriggerThrottledEvents|觸發程序節流的事件|計數|總計|工作流程觸發程序節流的事件數目。|無維度|
|IntegrationServiceEnvironmentWorkflowProcessorUsage|Workflow Processor Usage for Integration Service Environment|百分比|平均值|Workflow processor usage for integration service environment.|無維度|
|IntegrationServiceEnvironmentWorkflowMemoryUsage|Workflow Memory Usage for Integration Service Environment|百分比|平均值|Workflow memory usage for integration service environment.|無維度|
|IntegrationServiceEnvironmentConnectorProcessorUsage|Connector Processor Usage for Integration Service Environment|百分比|平均值|Connector processor usage for integration service environment.|無維度|
|IntegrationServiceEnvironmentConnectorMemoryUsage|Connector Memory Usage for Integration Service Environment|百分比|平均值|Connector memory usage for integration service environment.|無維度|

## <a name="microsoftmachinelearningservicesworkspaces"></a>Microsoft.MachineLearningServices/workspaces

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|Completed Runs|Completed Runs|計數|總計|Number of runs completed successfully for this workspace|案例|
|Started Runs|Started Runs|計數|總計|Number of runs started for this workspace|案例|
|失敗的執行|失敗的執行|計數|總計|Number of runs failed for this workspace|案例|
...
|VipAvailability|資料路徑可用性|計數|平均值|每個持續時間 IP 位址可用性的平均值|連接埠|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|ApplicationRuleHit|Application rules hit count|計數|總計|Number of times Application rules were hit|Status, Reason, Protocol|
|NetworkRuleHit|Network rules hit count|計數|總計|Number of times Network rules were hit|Status, Reason, Protocol|

## <a name="microsoftnetworkapplicationgateways"></a>Microsoft.Network/applicationGateways

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|輸送量|輸送量|每秒位元組|總計|應用程式閘道每秒已服務的位元組數目|無維度|
...
|CapacityUnits|Current Capacity Units|計數|平均值|Capacity Units consumed|無維度|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|BitsInPerSecond|BitsInPerSecond|每秒計數|平均值|每秒輸入到 Azure 的位元數|PeeringType|
|BitsOutPerSecond|BitsOutPerSecond|每秒計數|平均值|每秒從 Azure 輸出的位元數|PeeringType|
...
|AverageRoundtripMs|Avg. Round-trip Time (ms)|毫秒|平均值|在來源與目的地之間所傳送連線能力監視探查的平均網路來回時間 (毫秒)|無維度|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|registration.all|註冊作業數|計數|總計|所有成功註冊作業 (建立、更新、查詢和刪除) 的計數。 |無維度|
|registration.create|註冊建立作業|計數|總計|所有成功註冊建立的計數。|無維度|
|registration.update|註冊更新作業|計數|總計|所有成功註冊更新的計數。|無維度|
|registration.get|註冊讀取作業|計數|總計|所有成功註冊查詢的計數。|無維度|
|registration.delete|註冊刪除作業|計數|總計|所有成功註冊刪除的計數。|無維度|
|incoming|傳入訊息|計數|總計|所有成功傳送 API 呼叫的計數。 |無維度|
|incoming.scheduled|傳送已排程的推播通知|計數|總計|Scheduled Push Notifications Canceled|無維度|
|incoming.scheduled.cancel|Scheduled Push Notifications Canceled|計數|總計|Scheduled Push Notifications Canceled|無維度|
|scheduled.pending|暫止已排程的通知|計數|總計|暫止已排程的通知|無維度|
|installation.all|安裝管理作業|計數|總計|安裝管理作業|無維度|
|installation.get|取得安裝作業|計數|總計|取得安裝作業|無維度|
...
|outgoing.allpns.success|成功通知|計數|總計|所有成功通知的計數。|無維度|
|outgoing.allpns.invalidpayload|承載錯誤|計數|總計|因為 PNS 傳回錯誤承載錯誤而失敗的推播計數。|無維度|
|outgoing.allpns.pnserror|外部通知系統錯誤|計數|總計|因為與 PNS 通訊發生問題 (不包括驗證問題) 而失敗的推播計數。|無維度|
|outgoing.allpns.channelerror|通道錯誤|計數|總計|因為通道無效、未與正確的應用程式相關聯、已節流處理或過期，導致失敗的推播計數。|無維度|
|outgoing.allpns.badorexpiredchannel|錯誤或過期的通道錯誤|計數|總計|因為註冊中的通道/權杖/registrationId 過期或無效，導致失敗的推播計數。|無維度|
|outgoing.wns.success|WNS 成功通知|計數|總計|所有成功通知的計數。|無維度|
|outgoing.wns.invalidcredentials|WNS 授權錯誤 (無效認證)|計數|總計|因為 PNS 不接受提供的認證或已封鎖認證，導致失敗的推播計數。 (Windows Live 無法識別認證)。|無維度|
|outgoing.wns.badchannel|WNS 不正確的通道錯誤|計數|總計|因為無法識別註冊中的 ChannelURI (WNS 狀態︰404 找不到)，導致失敗的推播計數。|無維度|
|outgoing.wns.expiredchannel|WNS 過期通道錯誤|計數|總計|因為 ChannelURIi 過期 (WNS 狀態︰ 410 不存在) 而失敗的推播計數。|無維度|
|outgoing.wns.throttled|WNS 節流通知|計數|總計|因為 WNS 正在節流此應用程式，導致失敗的推播計數 (WNS 狀態︰406 無法接受)。|無維度|
|outgoing.wns.tokenproviderunreachable|WNS 授權錯誤 (無法連線)|計數|總計|無法連線到 Windows Live。|無維度|
|outgoing.wns.invalidtoken|WNS 授權錯誤 (無效權杖)|計數|總計|提供給 WNS 的權杖無效 (WNS 狀態︰ 401 未經授權)。|無維度|
|outgoing.wns.wrongtoken|WNS 授權錯誤 (錯誤權杖)|計數|總計|提供給 WNS 的權杖有效，但卻是給另一個應用程式 (WNS 狀態︰403 禁止)。 如果註冊中的 ChannelURI 與另一個應用程式相關聯，可能會發生這種情形。 請檢查用戶端應用程式相關聯的應用程式，就是認證位於通知中樞的同一個應用程式。|無維度|
...
|outgoing.wns.channelthrottled|WNS 通道已節流處理|計數|總計|因為註冊中的 ChannelURI 已節流處理而捨棄通知 (WNS 回應標頭︰X-WNS-NotificationStatus:channelThrottled)。|無維度|
...
|outgoing.wns.dropped|WNS 已捨棄通知|計數|總計|因為註冊中的 ChannelURI 已節流處理而捨棄通知 (X-WNS-NotificationStatus: dropped 但不是 X-WNS-DeviceConnectionStatus: disconnected)。|無維度|
|outgoing.wns.pnserror|WNS 錯誤|計數|總計|因為與 WNS 通訊錯誤而未傳遞通知。|無維度|
|outgoing.wns.authenticationerror|WNS 驗證錯誤|計數|總計|因為與 Windows Live 通訊錯誤、無效認證或不正確的權杖，導致未傳遞通知。|無維度|
|outgoing.apns.success|APNS 成功通知|計數|總計|所有成功通知的計數。|無維度|
|outgoing.apns.invalidcredentials|APNS 授權錯誤|計數|總計|因為 PNS 不接受提供的認證或已封鎖認證，導致失敗的推播計數。|無維度|
|outgoing.apns.badchannel|APNS 不正確的通道錯誤|計數|總計|The count of pushes that failed because the token is invalid (APNS binary protocol status code: 8. APNS HTTP protocol status code: 400 with "BadDeviceToken").|無維度|
|outgoing.apns.expiredchannel|APNS 過期通道錯誤|計數|總計|由 APNS 意見反應通道宣告失效的權杖計數。|無維度|
|outgoing.apns.invalidnotificationsize|APNS 無效通知大小錯誤|計數|總計|The count of pushes that failed because the payload was too large (APNS binary protocol status code: 7).|無維度|
|outgoing.apns.pnserror|APNS 錯誤|計數|總計|因為與 APNS 通訊錯誤而失敗的推播計數。|無維度|
|outgoing.gcm.success|GCM 成功通知|計數|總計|所有成功通知的計數。|無維度|
|outgoing.gcm.invalidcredentials|GCM 授權錯誤 (無效認證)|計數|總計|因為 PNS 不接受提供的認證或已封鎖認證，導致失敗的推播計數。|無維度|
...
|outgoing.gcm.wrongchannel|GCM 不正確的通道錯誤|計數|總計|因為註冊中的 registrationId 未與目前的應用程式相關聯，導致失敗的推播計數 (GCM 結果：InvalidPackageName)。|無維度|
|outgoing.gcm.pnserror|GCM 錯誤|計數|總計|因為與 GCM 通訊錯誤而失敗的推播計數。|無維度|
|outgoing.gcm.authenticationerror|GCM 驗證錯誤|計數|總計|因為 PNS 不接受提供的認證、已封鎖認證，或應用程式中未正確設定 SenderId，導致失敗的推播計數 (GCM 結果︰MismatchedSenderId)。|無維度|
|outgoing.mpns.success|MPNS 成功通知|計數|總計|所有成功通知的計數。|無維度|
|outgoing.mpns.invalidcredentials|MPNS 無效認證|計數|總計|因為 PNS 不接受提供的認證或已封鎖認證，導致失敗的推播計數。|無維度|
|outgoing.mpns.badchannel|MPNS 不正確的通道錯誤|計數|總計|因為無法識別註冊中的 ChannelURI 而失敗的推播計數 (MPNS 狀態︰404 找不到)。|無維度|
|outgoing.mpns.throttled|MPNS 已將通知節流處理|計數|總計|因為 MPNS 正在節流此應用程式，導致失敗的推播計數 (MPNS 狀態︰406 無法接受)。|無維度|
|outgoing.mpns.invalidnotificationformat|MPNS 無效通知格式|計數|總計|因為通知的承載太大而失敗的推播計數。|無維度|
...
|outgoing.mpns.pnserror|MPNS 錯誤|計數|總計|因為與 MPNS 通訊錯誤而失敗的推播計數。|無維度|
|outgoing.mpns.authenticationerror|MPNS 驗證錯誤|計數|總計|因為 PNS 不接受提供的認證或已封鎖認證，導致失敗的推播計數。|無維度|
|notificationhub.pushes|所有外寄通知|計數|總計|通知中樞的所有外寄通知|無維度|
|incoming.all.requests|所有傳入要求|計數|總計|針對通知中樞傳入的要求總數|無維度|
|incoming.all.failedrequests|所有傳入的失敗要求|計數|總計|針對通知中樞傳入的失敗要求總數|無維度|
...
|Average_Total Bytes|Total Bytes|計數|平均值|Average_Total Bytes|Computer、ObjectName、InstanceName、CounterPath、SourceSystem|
...
|Average_Processes|程序|計數|平均值|Average_Processes|Computer、ObjectName、InstanceName、CounterPath、SourceSystem|
...
|Average_Uptime|Uptime|計數|平均值|Average_Uptime|Computer、ObjectName、InstanceName、CounterPath、SourceSystem|
|Average_Users|使用者人數|計數|平均值|Average_Users|Computer、ObjectName、InstanceName、CounterPath、SourceSystem|
|Average_Avg. Disk sec/Read|Avg. Disk sec/Read|計數|平均值|Average_Avg. Disk sec/Read|Computer、ObjectName、InstanceName、CounterPath、SourceSystem|
|Average_Avg. Disk sec/Write|Avg. Disk sec/Write|計數|平均值|Average_Avg. Disk sec/Write|Computer、ObjectName、InstanceName、CounterPath、SourceSystem|
...
|活動|活動|計數|平均值|活動|Source、EventLog、Computer、EventCategory、EventLevel、EventLevelName、EventID|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|ListenerConnections-Success|ListenerConnections-Success|計數|總計|Microsoft.Relay 的成功 ListenerConnections。|EntityName|
|ListenerConnections-ClientError|ListenerConnections-ClientError|計數|總計|Microsoft.Relay 的 ListenerConnections 上發生的 ClientError。|EntityName|
|ListenerConnections-ServerError|ListenerConnections-ServerError|計數|總計|Microsoft.Relay 的 ListenerConnections 上發生的 ServerError。|EntityName|
|SenderConnections-Success|SenderConnections-Success|計數|總計|Microsoft.Relay 的成功 SenderConnections。|EntityName|
|SenderConnections-ClientError|SenderConnections-ClientError|計數|總計|Microsoft.Relay 的 SenderConnections 上發生的 ClientError。|EntityName|
|SenderConnections-ServerError|SenderConnections-ServerError|計數|總計|Microsoft.Relay 的 SenderConnections 上發生的 ServerError。|EntityName|
...
|IncomingMessages|傳入訊息 (預覽)|計數|總計|Microsoft.ServiceBus 的傳入訊息。 (預覽)|EntityName|
|OutgoingMessages|傳出訊息 (預覽)|計數|總計|Microsoft.ServiceBus 的傳出訊息。 (預覽)|EntityName|
...
|ActiveMessages|佇列/主題中的作用中訊息計數。 (預覽)|計數|平均值|佇列/主題中的作用中訊息計數。 (預覽)|EntityName|
|DeadletteredMessages|Count of dead-lettered messages in a Queue/Topic. (預覽)|計數|平均值|Count of dead-lettered messages in a Queue/Topic. (預覽)|EntityName|
|ScheduledMessages|Count of scheduled messages in a Queue/Topic. (預覽)|計數|平均值|Count of scheduled messages in a Queue/Topic. (預覽)|EntityName|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|AllocatedCpu|AllocatedCpu|計數|平均值|Cpu allocated to this container in millicores|ApplicationName, ServiceName, CodePackageName, ServiceReplicaName|
|AllocatedMemory|AllocatedMemory|位元組|平均值|Memory allocated to this container in MB|ApplicationName, ServiceName, CodePackageName, ServiceReplicaName|
|ActualCpu|ActualCpu|計數|平均值|Actual CPU usage in millicores|ApplicationName, ServiceName, CodePackageName, ServiceReplicaName|
|ActualMemory|ActualMemory|位元組|平均值|Actual memory usage in MB|ApplicationName, ServiceName, CodePackageName, ServiceReplicaName|
...
|ApplicationStatus|ApplicationStatus|計數|平均值|Status of Service Fabric Mesh application|ApplicationName, Status|
|ServiceStatus|ServiceStatus|計數|平均值|Health Status of a service in Service Fabric Mesh application|ApplicationName, Status, ServiceName|
|ServiceReplicaStatus|ServiceReplicaStatus|計數|平均值|Health Status of a service replica in Service Fabric Mesh application|ApplicationName, Status, ServiceName, ServiceReplicaName|
|ContainerStatus|ContainerStatus|計數|平均值|Status of the container in Service Fabric Mesh application|ApplicationName, ServiceName, CodePackageName, ServiceReplicaName, Status|
|RestartCount|RestartCount|計數|平均值|Restart count of a container in Service Fabric Mesh application|ApplicationName, Status, ServiceName, ServiceReplicaName, CodePackageName|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|allocated_data_storage|Data space allocated|位元組|平均值|Data space allocated. Not applicable to data warehouses.|無維度|
|app_cpu_billed|App CPU billed|計數|總計|App CPU billed. Applies to serverless databases.|無維度|
|app_cpu_percent|App CPU percentage|百分比|平均值|App CPU percentage. Applies to serverless databases.|無維度|
|app_memory_percent|App memory used percentage|百分比|平均值|App memory used percentage. Applies to serverless databases.|無維度|
|blocked_by_firewall|遭到防火牆封鎖|計數|總計|遭到防火牆封鎖|無維度|
|cache_hit_percent|快取命中的百分比|百分比|最大值|Cache hit percentage. Applies only to data warehouses.|無維度|
|cache_used_percent|已用快取的百分比|百分比|最大值|Cache used percentage. Applies only to data warehouses.|無維度|
|connection_failed|失敗的連線|計數|總計|失敗的連線|無維度|
|connection_successful|成功的連線|計數|總計|成功的連線|無維度|
|cpu_percent|CPU 百分比|百分比|平均值|CPU 百分比|無維度|
|cpu_limit|CPU limit|計數|平均值|CPU limit. Applies to vCore-based databases.|無維度|
|cpu_used|CPU used|計數|平均值|CPU used. Applies to vCore-based databases.|無維度|
|deadlock|死結|計數|總計|Deadlocks. Not applicable to data warehouses.|無維度|
|diff_backup_size_bytes|Differential backup storage size|位元組|最大值|Cumulative differential backup storage size. Applies to General Purpose and Business Critical databases. Does not currently apply to Manage instance databases.|無維度|
|dtu_limit|DTU 限制|計數|平均值|DTU Limit. Applies to DTU-based databases.|無維度|
|dtu_consumption_percent|DTU 百分比|百分比|平均值|DTU percentage. Applies to DTU-based databases.|無維度|
|dtu_used|已使用 DTU|計數|平均值|DTU used. Applies to DTU-based databases.|無維度|
...
|dwu_consumption_percent|DWU 百分比|百分比|最大值|DWU percentage. Applies only to data warehouses.|無維度|
|dwu_limit|DWU 限制|計數|最大值|DWU limit. Applies only to data warehouses.|無維度|
|dwu_used|已使用 DWU|計數|最大值|DWU used. Applies only to data warehouses.|無維度|
|full_backup_size_bytes|Full backup storage size|位元組|最大值|Cumulative full backup storage size. Applies to General Purpose and Business Critical databases. Does not currently apply to Manage instance databases.|無維度|
|local_tempdb_usage_percent|本機 tempdb 百分比|百分比|平均值|Local tempdb percentage. Applies only to data warehouses.|無維度|
|log_backup_size_bytes|Log backup storage size|位元組|最大值|Cumulative log backup storage size. Applies to General Purpose and Business Critical databases. Does not currently apply to Manage instance databases.|無維度|
|log_write_percent|記錄 IO 百分比|百分比|平均值|Log IO percentage. Not applicable to data warehouses.|無維度|
|physical_data_read_percent|資料 IO 百分比|百分比|平均值|資料 IO 百分比|無維度|
|sessions_percent|工作階段百分比|百分比|平均值|Sessions percentage. Not applicable to data warehouses.|無維度|
|sqlserver_process_core_percent|SQL Server process core percent|百分比|最大值|This metric is a placeholder and not populated at this time.|無維度|
|sqlserver_process_memory_percent|SQL Server process memory percent|百分比|最大值|This metric is a placeholder and not populated at this time.|無維度|
|storage|Data space used|位元組|最大值|Total database size. Not applicable to data warehouses.|無維度|
|storage_percent|Data space used percent|百分比|最大值|Database size percentage. Not applicable to data warehouses or hyperscale databases.|無維度|
|tempdb_data_size|Tempdb Data File Size Kilobytes|計數|最大值|Tempdb Data File Size Kilobytes. Not applicable to data warehouses. This metric will be available for databases using the vCore purchasing model or 100 DTU and higher for DTU-based purchasing models.|無維度|
|tempdb_log_size|Tempdb Log File Size Kilobytes|計數|最大值|Tempdb Log File Size Kilobytes. Not applicable to data warehouses. This metric will be available for databases using the vCore purchasing model or 100 DTU and higher for DTU-based purchasing models.|無維度|
|tempdb_log_used_percent|Tempdb Percent Log Used|百分比|最大值|Tempdb Percent Log Used. Not applicable to data warehouses. This metric will be available for databases using the vCore purchasing model or 100 DTU and higher for DTU-based purchasing models.|無維度|
|workers_percent|背景工作角色百分比|百分比|平均值|Workers percentage. Not applicable to data warehouses.|無維度|
|xtp_storage_percent|記憶體內部 OLTP 儲存體百分比|百分比|平均值|In-Memory OLTP storage percent. Not applicable to data warehouses.|無維度|

## <a name="microsoftsqlserverselasticpools"></a>Microsoft.Sql/servers/elasticPools

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|allocated_data_storage|Data space allocated|位元組|平均值|Data space allocated|無維度|
|allocated_data_storage_percent|Data space allocated percent|百分比|最大值|Data space allocated percent|無維度|
|cpu_limit|CPU limit|計數|平均值|CPU limit. Applies to vCore-based elastic pools.|無維度|
|cpu_percent|CPU 百分比|百分比|平均值|CPU 百分比|無維度|
|cpu_used|CPU used|計數|平均值|CPU used. Applies to vCore-based elastic pools.|無維度|
|dtu_consumption_percent|DTU 百分比|百分比|平均值|DTU percentage. Applies to DTU-based elastic pools.|無維度|
|eDTU_limit|eDTU 限制|計數|平均值|eDTU limit. Applies to DTU-based elastic pools.|無維度|
|eDTU_used|已使用 eDTU|計數|平均值|eDTU used. Applies to DTU-based elastic pools.|無維度|
|log_write_percent|記錄 IO 百分比|百分比|平均值|記錄 IO 百分比|無維度|
|physical_data_read_percent|資料 IO 百分比|百分比|平均值|資料 IO 百分比|無維度|
|sessions_percent|工作階段百分比|百分比|平均值|工作階段百分比|無維度|
|storage_limit|Data max size|位元組|平均值|儲存體限制|無維度|
|storage_percent|Data space used percent||百分比|平均值|儲存體百分比|無維度|
|storage_used|Data space used|位元組|平均值|使用的儲存空間|無維度|
|sqlserver_process_core_percent|SQL Server process core percent|百分比|最大值|This metric is a placeholder and not populated at this time.|無維度|
|sqlserver_process_memory_percent|SQL Server process memory percent|百分比|最大值|This metric is a placeholder and not populated at this time.|無維度|
|tempdb_data_size|Tempdb Data File Size Kilobytes|計數|最大值|Tempdb Data File Size Kilobytes. Not applicable to data warehouses. This metric will be available for databases using the vCore purchasing model or 100 DTU and higher for DTU-based purchasing models.|無維度|
|tempdb_log_size|Tempdb Log File Size Kilobytes|計數|最大值|Tempdb Log File Size Kilobytes. Not applicable to data warehouses. This metric will be available for databases using the vCore purchasing model or 100 DTU and higher for DTU-based purchasing models.|無維度|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|BlobCapacity|Blob 容量|位元組|平均值|儲存體帳戶之 Blob 服務所使用的儲存體量 (位元組)。|BlobType, Tier|
|BlobCount|Blob 計數|計數|總計|儲存體帳戶之 Blob 服務中的 Blob 數目。|BlobType|       |BlobCount|Blob 計數|計數|平均值|儲存體帳戶之 Blob 服務中的 Blob 數目。|BlobType, Tier|
|ContainerCount|Blob 容器計數|計數|平均值|儲存體帳戶之 Blob 服務中的容器數目。|無維度|
|IndexCapacity|Index Capacity|位元組|平均值|The amount of storage used by ADLS Gen2 (Hierarchical) Index in bytes.|無維度|
|交易|交易|計數|總計|向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 使用 ResponseType 維度來取得不同類型回應的數目。|ResponseType、GeoType、ApiName/、Authentication|
|輸入|輸入|位元組|總計|輸入資料量 (位元組)。 此數目包括從外部用戶端輸入到 Azure 儲存體與 Azure 內的輸入。|GeoType、ApiName、Authentication|
|輸出|輸出|位元組|總計|輸出資料量 (位元組)。 此數目包括從外部用戶端輸出到 Azure 儲存體與 Azure 內的輸出。 因此，此數目未反映可收費的輸出。|GeoType、ApiName、Authentication|
|SuccessServerLatency|成功伺服器延遲|毫秒|平均值|Azure 儲存體用來處理成功要求的平均延遲 (毫秒)。 此值不包括在 AverageE2ELatency 中指定的網路延遲。|GeoType、ApiName、Authentication|
...
|FileCount|檔案計數|計數|平均值|儲存體帳戶之檔案服務中的檔案數目。|無維度|
|FileShareCount|檔案共用計數|計數|平均值|儲存體帳戶之檔案服務中的檔案共用數目。|無維度|
|交易|交易|計數|總計|向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 使用 ResponseType 維度來取得不同類型回應的數目。|ResponseType、GeoType、ApiName/、Authentication|
|輸入|輸入|位元組|總計|輸入資料量 (位元組)。 此數目包括從外部用戶端輸入到 Azure 儲存體與 Azure 內的輸入。|GeoType、ApiName、Authentication|
|輸出|輸出|位元組|總計|輸出資料量 (位元組)。 此數目包括從外部用戶端輸出到 Azure 儲存體與 Azure 內的輸出。 因此，此數目未反映可收費的輸出。|GeoType、ApiName、Authentication|
|SuccessServerLatency|成功伺服器延遲|毫秒|平均值|Azure 儲存體用來處理成功要求的平均延遲 (毫秒)。 此值不包括在 AverageE2ELatency 中指定的網路延遲。|GeoType、ApiName、Authentication|
...
|QueueCount|佇列計數|計數|平均值|儲存體帳戶之佇列服務中的佇列數目。|無維度|
|QueueMessageCount|佇列訊息計數|計數|平均值|儲存體帳戶之佇列服務中的佇列訊息大約數目。|無維度|
|交易|交易|計數|總計|向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 使用 ResponseType 維度來取得不同類型回應的數目。|ResponseType、GeoType、ApiName/、Authentication|
|輸入|輸入|位元組|總計|輸入資料量 (位元組)。 此數目包括從外部用戶端輸入到 Azure 儲存體與 Azure 內的輸入。|GeoType、ApiName、Authentication|
|輸出|輸出|位元組|總計|輸出資料量 (位元組)。 此數目包括從外部用戶端輸出到 Azure 儲存體與 Azure 內的輸出。 因此，此數目未反映可收費的輸出。|GeoType、ApiName、Authentication|
|SuccessServerLatency|成功伺服器延遲|毫秒|平均值|Azure 儲存體用來處理成功要求的平均延遲 (毫秒)。 此值不包括在 AverageE2ELatency 中指定的網路延遲。|GeoType、ApiName、Authentication|
...
|TableCount|資料表計數|計數|平均值|儲存體帳戶之資料表服務中的資料表數目。|無維度|
|TableEntityCount|資料表實體計數|計數|平均值|儲存體帳戶之資料表服務中的資料表實體數目。|無維度|
|交易|交易|計數|總計|向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 使用 ResponseType 維度來取得不同類型回應的數目。|ResponseType、GeoType、ApiName/、Authentication|
|輸入|輸入|位元組|總計|輸入資料量 (位元組)。 此數目包括從外部用戶端輸入到 Azure 儲存體與 Azure 內的輸入。|GeoType、ApiName、Authentication|
|輸出|輸出|位元組|總計|輸出資料量 (位元組)。 此數目包括從外部用戶端輸出到 Azure 儲存體與 Azure 內的輸出。 因此，此數目未反映可收費的輸出。|GeoType、ApiName、Authentication|
|SuccessServerLatency|成功伺服器延遲|毫秒|平均值|Azure 儲存體用來處理成功要求的平均延遲 (毫秒)。 此值不包括在 AverageE2ELatency 中指定的網路延遲。|GeoType、ApiName、Authentication|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|ServerSyncSessionResult|Sync Session Result|計數|平均值|Metric that logs a value of 1 each time the Server Endpoint successfully completes a Sync Session with the Cloud Endpoint|SyncGroupName, ServerEndpointName, SyncDirection|
|StorageSyncSyncSessionAppliedFilesCount|Files Synced|計數|總計|Count of Files synced|SyncGroupName, ServerEndpointName, SyncDirection|
|StorageSyncSyncSessionPerItemErrorsCount|檔案無法同步|計數|總計|Count of files failed to sync|SyncGroupName, ServerEndpointName, SyncDirection|
|StorageSyncBatchTransferredFileBytes|同步的位元組|位元組|總計|Total file size transferred for Sync Sessions|SyncGroupName, ServerEndpointName, SyncDirection|
|StorageSyncServerHeartbeat|Server Online Status|計數|最大值|Metric that logs a value of 1 each time the registered server successfully records a heartbeat with the Cloud Endpoint|ServerName|
|StorageSyncRecallIOTotalSizeBytes|雲端階層處理重新叫用|位元組|總計|Total size of data recalled by the server|ServerName|
...
|WarmStorageMaxProperties|Warm Storage Max Properties|計數|最大值|Maximum number of properties used allowed by the environment for S1/S2 SKU and maximum number of properties allowed by Warm Store for PAYG SKU|無維度|
|WarmStorageUsedProperties|Warm Storage Used Properties |計數|最大值|Number of properties used by the environment for S1/S2 SKU and number of properties used by Warm Store for PAYG SKU|無維度|
...
|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|DiskReadBytesPerSecond|Disk Read Bytes/Sec|每秒位元組|平均值|Average disk throughput due to read operations over the sample period.|無維度|
|DiskWriteBytesPerSecond|Disk Write Bytes/Sec|每秒位元組|平均值|Average disk throughput due to write operations over the sample period.|無維度|
|Disk Read Bytes|Disk Read Bytes|位元組|總計|Total disk throughput due to read operations over the sample period.|無維度|
|Disk Write Bytes|Disk Write Bytes|位元組|總計|Total disk throughput due to write operations over the sample period.|無維度|
|DiskReadOperations|Disk Read Operations|計數|總計|The number of IO read operations in the previous sample period. Note that these operations may be variable sized.|無維度|
|DiskWriteOperations|Disk Write Operations|計數|總計|The number of IO write operations in the previous sample period. Note that these operations may be variable sized.|無維度|
|Disk Read Operations/Sec|Disk Read Operations/Sec|每秒計數|平均值|The average number of IO read operations in the previous sample period. Note that these operations may be variable sized.|無維度|
|Disk Write Operations/Sec|Disk Write Operations/Sec|每秒計數|平均值|The average number of IO write operations in the previous sample period. Note that these operations may be variable sized.|無維度|
|DiskReadLatency|Disk Read Latency|毫秒|平均值|Total read latency. The sum of the device and kernel read latencies.|無維度|
|DiskWriteLatency|Disk Write Latency|毫秒|平均值|Total write latency. The sum of the device and kernel write latencies.|無維度|
|NetworkInBytesPerSecond|Network In Bytes/Sec|每秒位元組|平均值|Average network throughput for received traffic.|無維度|
|NetworkOutBytesPerSecond|Network Out Bytes/Sec|每秒位元組|平均值|Average network throughput for transmitted traffic.|無維度|
|Network In|Network In|位元組|總計|Total network throughput for received traffic.|無維度|
|Network Out|Network Out|位元組|總計|Total network throughput for transmitted traffic.|無維度|
|MemoryUsed|Memory Used|位元組|平均值|The amount of machine memory that is in use by the VM.|無維度|
|MemoryGranted|Memory Granted|位元組|平均值|The amount of memory that was granted to the VM by the host. Memory is not granted to the host until it is touched one time and granted memory may be swapped out or ballooned away if the VMkernel needs the memory.|無維度|
|MemoryActive|Memory Active|位元組|平均值|The amount of memory used by the VM in the past small window of time. This is the "true" number of how much memory the VM currently has need of. Additional, unused memory may be swapped out or ballooned with no impact to the guest's performance.|無維度|
|Percentage CPU|Percentage CPU|百分比|平均值|The CPU utilization. This value is reported with 100% representing all processor cores on the system. As an example, a 2-way VM using 50% of a four-core system is completely using two cores.|無維度|
|PercentageCpuReady|Percentage CPU Ready|毫秒|總計|Ready time is the time spend waiting for CPU(s) to become available in the past update interval.|無維度|
...
|DiskQueueLength|磁碟佇列長度|計數|平均值|磁碟佇列長度|執行個體|
...
|Handles|控制代碼計數|計數|平均值|控制代碼計數|執行個體|
|Threads|執行緒計數|計數|平均值|執行緒計數|執行個體|
...
|TotalAppDomains|應用程式網域總計|計數|平均值|應用程式網域總計|執行個體|
...
|MemoryWorkingSet|記憶體工作集|位元組|平均值|記憶體工作集|執行個體|
|AverageMemoryWorkingSet|平均記憶體工作集|位元組|平均值|平均記憶體工作集|執行個體|
|FunctionExecutionUnits|函式執行單位|MB / Milliseconds|總計|[Function Execution Units](https://github.com/Azure/Azure-Functions/wiki/Consumption-Plan-Cost-Billing-FAQ#how-can-i-view-graphs-of-execution-count-and-gb-seconds)|執行個體|
...
|TotalFrontEnds|前端總數|計數|平均值|前端總數|無維度|
|SmallAppServicePlanInstances|小型 App Service 方案背景工作角色|計數|平均值|小型 App Service 方案背景工作角色|無維度|
|MediumAppServicePlanInstances|中型 App Service 方案背景工作角色|計數|平均值|中型 App Service 方案背景工作角色|無維度|
|LargeAppServicePlanInstances|大型 App Service 方案背景工作角色|計數|平均值|大型 App Service 方案背景工作角色|無維度|

## <a name="microsoftwebhostingenvironmentsworkerpools"></a>Microsoft.Web/hostingEnvironments/workerPools

|計量|計量顯示名稱|單位|彙總類型|描述|維度|
|---|---|---|---|---|---|
|WorkersTotal|背景工作角色總數|計數|平均值|背景工作角色總數|無維度|
|WorkersAvailable|可用的背景工作角色|計數|平均值|可用的背景工作角色|無維度|
|WorkersUsed|已使用的背景工作角色|計數|平均值|已使用的背景工作角色|無維度|
|CpuPercentage|CPU 百分比|百分比|平均值|CPU 百分比|執行個體|
|MemoryPercentage|記憶體百分比|百分比|平均值|記憶體百分比|執行個體|
