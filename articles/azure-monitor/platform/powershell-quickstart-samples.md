...
> [!NOTE]
> 自 2016 年 9 月 25 日起，「Azure 監視器」是以前所謂「Azure Insights」的新名稱。 不過，命名空間及下列命令中仍包含 "insights" 一字。
...
`Get-AzLog` 支援許多其他參數。 如需詳細資訊，請參閱 `Get-AzLog` 參考。
...
若要檢視特定警示規則的歷程記錄，您可以使用 `Get-AzAlertHistory` Cmdlet，傳入警示規則的資源識別碼。
...
您可以使用 `Get-AzMetricDefinition` Cmdlet 檢視特定資源的所有計量的清單。
