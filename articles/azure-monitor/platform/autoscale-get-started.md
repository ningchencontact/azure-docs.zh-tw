...
1. 開啟 Azure 監視器中的 [自動調整規模]刀鋒視窗，然後選取您想要調整的資源。 (下列步驟會使用與某個 Web 應用程式相關聯的 App Service 方案。 您可以[在5分鐘內于 Azure 中建立第一個 ASP.NET web 應用程式。][4])
...
1. 您現在已建立第一個調整規模規則。 請注意，UX 建議了最佳做法，並指出「建議規則中至少有一個調整規模」。 方法如下：
...
恭喜您！ 您現在已成功建立第一個調整規模設定，可根據 CPU 使用量自動調整 Web 應用程式的規模。
