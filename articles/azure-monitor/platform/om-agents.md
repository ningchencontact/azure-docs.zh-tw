---
title: Operations Manager 連線到 Azure 監視器 |Microsoft Docs
...
ms.locfileid: "58369951"
---
# <a name="connect-operations-manager-to-azure-monitor"></a>Operations Manager 連線到 Azure 監視器

[!INCLUDE [azure-monitor-log-analytics-rebrand](../../../includes/azure-monitor-log-analytics-rebrand.md)]

若要維護中的現有投資[System Center Operations Manager](https://docs.microsoft.com/system-center/scom/key-concepts?view=sc-om-1807)並使用 Azure 監視器中的擴充的功能，您可以整合 Operations Manager 與 Log Analytics 工作區。 這可讓您利用 Azure 監視器中的記錄檔的機會，同時繼續使用 Operations Manager 連接到：
...
與 System Center Operations Manager 整合將值加入至服務作業策略的速度和效率的 Azure 監視器收集、 儲存及分析記錄資料，從 Operations Manager 中使用。 Azure 監視器記錄檔查詢說明相互關聯，來識別問題錯誤查明原因和杜絕支援現有問題管理程序的工作。 查詢引擎，以檢查效能、 事件和警示資料，使用豐富的儀表板和報告功能來公開此資料有意義的方式，示範 Azure 監視器的強度的彈性帶來對值得讚揚的 Operations Manager 的。

向 Operations Manager 管理群組回報的代理程式收集資料，從您的伺服器為基礎[Log Analytics 資料來源](agent-data-sources.md)和您已啟用您的工作區中的解決方案。 視啟用的解決方案，其資料會傳送直接從 Operations Manager 管理伺服器至服務，或者，因為代理程式管理系統上收集的資料量直接從代理程式傳送到 Log Analytics 工作區。 管理伺服器會直接將資料轉送到服務，永遠不會寫入作業或資料倉儲資料庫。 當管理伺服器無法使用 Azure 監視器的連線時，它會快取資料在本機之前重新建立通訊。 如果管理伺服器因發生規劃的維護或未規劃的中斷而離線，管理群組中的另一部管理伺服器會繼續使用 Azure 監視器的連線。  

下圖會顯示管理伺服器與代理程式之間的連線，在 System Center Operations Manager 管理群組和 Azure 監視器，包括方向與連接埠。   
...
* Azure 監視器只支援 System Center Operations Manager 1807、 Operations Manager 1801，Operations Manager 2016 Operations Manager 2012 SP1 UR6 或更新版本，以及 Operations Manager 2012 R2 UR2 或更新版本。 Operations Manager 2012 SP1 UR7 和 Operations Manager 2012 R2 UR3 中已加入 Proxy 支援。
...
|\*.ods.opinsights.azure.com| 443| 是|  
|*.azure-automation.net | 443| 是|  
|**Azure 監視器的 operations Manager 主控台**|||  
...
1. 在 [電腦搜尋] 對話方塊中，您可以搜尋 Operations Manager 監視的電腦或群組。 選取電腦或群組以登入 Azure 監視器中，按一下**新增**，然後按一下**確定**。
...
如果您的 proxy 伺服器需要驗證，請執行下列步驟來設定認證和設定需要傳播到管理群組中 Azure 監視器會回報的受控電腦。
...
建立連線，而且您設定哪些代理程式會收集並報告至 Azure 監視器的記錄檔資料之後，會在管理群組中，不一定是依照順序套用下列組態：
...
* 會建立兩個連接器。  第一個稱為**Microsoft.SystemCenter.Advisor.DataConnector**並轉送至 Azure 監視器產生從管理群組中的所有類別的執行個體的所有警示的訂用帳戶時，會自動設定。 第二個連接器**Advisor 連接器**，它會負責使用 Azure 監視器通訊及共用資料。
...
您可以覆寫這些兩個規則，請停用它們，以防止自動下載，或是修改管理伺服器與 Azure Monitorto 同步處理的頻率的頻率以判斷新的管理組件是否可用以及應該下載。 遵循[如何覆寫規則或監視器](https://technet.microsoft.com/library/hh212869.aspx)步驟，使用值 (秒) 修改 [頻率] 參數來變更同步處理排程，或修改 [已啟用] 參數來停用規則。 將目標設為覆寫 [Operations Manager 管理群組] 類別的所有物件。
