...
## <a name="metrics-and-dimensions-supported-for-logs"></a>記錄支援的計量和維度
 計量警示支援針對使用維度的計量發出警示。 您可以使用維度來將計量篩選到正確層級。 跨支援的解決方案列出來自 [Log Analytics 工作區](../../azure-monitor/platform/metrics-supported.md#microsoftoperationalinsightsworkspaces)的記錄所支援計量的完整清單。
