...
首先建立[記錄搜尋](../../azure-monitor/log-query/log-query-overview.md)，傳回您想要填入 Power BI 資料集的 Log Analytics 資料。  您接著可以將該查詢匯出到可以由 Power BI Desktop 使用的 [Power Query (M) 語言](https://msdn.microsoft.com/library/mt807488.aspx)。
...
2. 如果您使用記錄搜尋入口網站，請按一下 **Power BI**。  如果您使用 Analytics 入口網站，請選取**匯出** > **Power BI 查詢 (M)**。  這兩個選項都會將查詢匯出到稱為 **PowerBIQuery.txt** 的文字檔。 
    ...
    ![匯出記錄搜尋](media/powerbi/export-logsearch.png) ![匯出記錄搜尋](media/powerbi/export-analytics.png)
...
* 深入了解 [記錄檔搜尋](../../azure-monitor/log-query/log-query-overview.md) ，建置可以匯出至 Power BI 的查詢。
