...
如果 Log Analytics 工作區位於相同的 Azure 訂用帳戶中，或位於不同的訂用帳戶中，但在相同的 Azure Active Directory 中，請使用在[Azure 監視器中收集和分析 Log Analytics 工作區](activity-log-collect.md)中的步驟來收集 azure活動記錄。
