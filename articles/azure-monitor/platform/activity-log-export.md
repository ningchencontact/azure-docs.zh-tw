...
3. 使用 `Add-AzLogProfile` 來建立新的記錄設定檔：
    ...
    | serviceBusRuleId |否 |服務匯流排規則識別碼，您想要在其中建立事件中樞的服務匯流排命名空間。 這是以格式字串： `{service bus resource ID}/authorizationrules/{key name}`。 |
...
| level |事件的層級。 下列其中一個值：_關鍵_，_錯誤_，_警告_，_參考_，以及_詳細資訊_ |
