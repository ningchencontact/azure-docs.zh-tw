...
### <a name="metrics"></a>度量
[計量](data-platform-metrics.md)為數值，可描述系統在特定時間點的某個方面。 指标是定期收集的，使用一个时间戳、一个名称、一个值以及一个或多个定义标签进行标识。 可以使用各种算法聚合指标、将其与其他指标进行比较，以及分析不同时间的指标趋势。 
...
| 屬性  | 度量 | 記錄 |
|:---|:---|:---|
| 優點 | 轻型数据，支持警报等近实时方案。 非常适合用于快速检测问题。 | 使用丰富查询语言进行分析。 非常适合用于深入分析和识别根本原因。 |
| 資料 | 仅限数字值 | 文本或数字数据 |
| Structure | 标准属性集，包括采样时间、受监视的资源、数字值。 某些指标包含多个维度用于进一步的定义。 | 取决于日志类型的唯一属性集。 |
| 集合 | 定期收集。 | 可以在事件触发要创建的记录时偶发性地收集。 |
| 在 Azure 门户中查看 | 計量瀏覽器 | Log Analytics |
...
除了使用 Azure 中的工具來分析監視資料，您可能也有將監視資料轉送到外部工具的需求，例如安全性資訊和事件管理 (SIEM) 產品。 此轉送通常是透過 [Azure 事件中樞](/azure/event-hubs/)，直接從受監視的資源來完成。 可将某些源配置为直接向事件中心发送数据，同时，可以使用另一个进程（例如逻辑应用）来检索所需的数据。 有关详细信息，请参阅[将 Azure 监视数据流式传输到事件中心供外部工具使用](stream-monitoring-data-event-hubs.md)。
