...
description: Azure 監視器記錄分析服務常見問題的解答。
...
此 Microsoft 常見問題是 Azure 監視器 Log Analytics 工作區的常見問題清單。 若您有任何關於 Log Analytics 的其他問題，請前往[討論論壇](https://social.msdn.microsoft.com/Forums/azure/home?forum=opinsights)並張貼您的問題。 當問到常見問題時，我們會將其新增至此文章，以便其他人可以快速輕鬆地找到此問題。
...
答：若要確保代理程式可以與 Log Analytics 工作區通訊，請移至： [控制台]、[安全性 & 設定]、[ **Microsoft Monitoring Agent**]。
...
答：在 System Center Operations Manager 中，從 Log Analytics 受管理的電腦清單中移除電腦。 Operations Manager 會將代理程式的設定更新為不再向 Log Analytics 回報。 針對直接連線到 Log Analytics 的代理程式，您可以透過 [控制台] > [安全性和設定] > [Microsoft Monitoring Agent] 來阻止其通訊。
...
答：若要將工作區移到不同的訂用帳戶或資源群組，您必須先將工作區中的自動化帳戶取消連結。 若要取消連結自動化帳戶，必須先移除安裝在工作區中的以下解決方案：「更新管理」、「變更追蹤」，或是「停機期間啟動/停止 VM」。 移除這些解決方案之後，即可在自動化帳戶資源的左側窗格上選取 [連結的工作區]，然後按一下功能區上的 [取消與工作區的連結] 來將自動化帳戶取消連結。
...
[使用量](../../azure-monitor/platform/data-usage.md) 頁面會顯示整體使用方式。
...
[開始使用 Azure 監視器](../../azure-monitor/overview.md)以深入瞭解 Log Analytics，並在幾分鐘內啟動並執行。
