...
記錄搜尋警示規則適用於使用者根據組態和所使用的自訂分析查詢所決定的邏輯。 由於監視邏輯（包括要觸發警示規則的確切條件或原因）會封裝在分析查詢中，因此每個記錄警示規則可能會有所不同。 當符合或超過記錄搜尋警示規則的臨界值條件時，Azure 警示會針對特定基礎根本原因（或）案例進行評估。 因此，記錄警示稱為「不是狀態」。 而且只要提供自訂分析查詢的結果符合警示條件，記錄警示規則就會持續引發。 若未發出任何警示，請每次解決，因為監視失敗的確切根本原因邏輯會在使用者所提供的分析查詢中加上遮罩。 目前沒有任何機制可 Azure 監視器警示，以最終推算解決的根本原因。

讓我們看到相同的實際範例。 假設我們有一個名為 [ *Contoso-記錄*警示] 的記錄警示規則，如同[針對 [結果數目] 類型 [記錄警示] 所提供範例](#example-of-number-of-records-type-log-alert)中的設定，其中自訂警示查詢的設計是在記錄中尋找500結果碼。

- 在下午1:05，當 Azure 警示執行 Contoso 記錄警示時，記錄搜尋結果會產生零筆記錄，結果代碼為500。 因為0低於閾值，所以不會引發警示。
- 當 Azure 警示執行 Contoso 記錄警示時，在下午1:10 的下一個反復專案中，記錄搜尋結果提供五筆記錄，結果代碼為500。 由於五個超過閾值，而且會引發警示並觸發相關聯的動作。
- 在下午1:15，當 Contoso-記錄警示由 Azure 警示執行時，記錄搜尋結果會提供兩筆具有500結果碼的記錄。 由於兩個超過閾值，而引發警示時，會觸發相關聯的動作。
- 現在，于下午1:20 的下一個反復專案中，當 Azure 警示執行 Contoso 記錄警示時，會再次提供具有500結果碼的零筆記錄搜尋結果。 因為0低於閾值，所以不會引發警示。

但在上述案例中，于下午 1:15-Azure 警示無法判斷在1:10 看到的基礎問題是否持續存在，以及是否有 net new 失敗。 因為使用者所提供的查詢可能會將先前的記錄納入考慮，所以可以確保 Azure 警示。 由於警示的邏輯會封裝在警示查詢中，因此，在 1:15 PM 看到具有500結果代碼的兩筆記錄，在下午1:10 時可能會有或可能不會出現。 因此，若要在謹慎時發生錯誤，當 Contoso 記錄警示在下午1:15 執行時，會再次觸發設定的動作。 1:20 現在，當出現500結果碼的零筆記錄時，Azure 警示無法確定在 1:10 PM 和 1:15 PM 看到的500結果程式碼的原因現在已經解決，Azure 監視器警示可以安心地推算出不會因相同原因而發生的500錯誤問題再次。 因此，Contoso-記錄警示不會在 Azure 警示儀表板和/或傳送的通知中指出警示的解決方式，而變成已解決。 而是瞭解內嵌于分析查詢之邏輯的確切條件或原因的使用者，可以視需要[將警示標示為已關閉](alerts-managing-alert-states.md)。
