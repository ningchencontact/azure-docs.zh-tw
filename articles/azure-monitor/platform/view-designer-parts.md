...
description: 可以通过 Azure Monitor 中的视图设计器创建自定义视图，此类视图在 Azure 门户中显示，包含 Log Analytics 工作区中的多种基于数据的可视化效果。 此文章是自訂檢視中可用的視覺效果組件設定的參考指南。
...
在 Azure Monitor 中使用视图设计器即可在 Azure 门户中创建各种自定义视图，使 Log Analytics 工作区中的数据可视化。 此文章是自訂檢視中可用的視覺效果組件設定的參考指南。
...
| 檢視類型 | 描述 |
|:--- |:--- |
| [查詢清單](#list-of-queries-part) |显示日志查询的列表。 您可以選取每個查詢以顯示其結果。 |
| [數字與清單](#number-and-list-part) |标头具有单个数字，用于表示来自日志查询中的记录数。 清單會顯示查詢的前 10 筆結果，並包含表示數值資料行的相對值或一段時間內之變更的圖表。 |
| [兩個數字與清單](#two-numbers-and-list-part) |标头显示两个数字，用于表示来自单独日志查询中的记录数。 清單會顯示查詢的前 10 筆結果，並包含表示數值資料行的相對值或一段時間內之變更的圖表。 |
...
查询列表部件显示日志查询的列表。 您可以選取每個查詢以顯示其結果。 根據預設，檢視會包含單一查詢，您可以選取 [+ 查詢] 來新增額外的查詢。
...
标头具有单个数字，用于表示来自日志查询中的记录数。 清單會顯示查詢的前 10 筆結果，並包含表示數值資料行的相對值或一段時間內之變更的圖表。
...
| **清單** |**> 資料行標題** |
| 名稱 |圖表第一個資料行上方顯示的文字。 |
| Value |圖表第二個資料行上方顯示的文字。 |
| **清單** |**> 臨界值** |
| 啟用臨界值 |選取此連結以啟用閾值。 如需詳細資訊，請參閱[一般設定](#thresholds)。 |

## <a name="two-numbers-and-list-part"></a>兩個數字與清單組件
标头具有两个数字，用于表示来自单独日志查询中的记录数。 清單會顯示查詢的前 10 筆結果，並包含表示數值資料行的相對值或一段時間內之變更的圖表。
...
點選瀏覽會定義當您按一下標頭或檢視中的清單項目時，要採取哪個動作。  此操作将在 [Log Analytics](../../azure-monitor/log-query/portals.md) 中打开查询或启动另一个视图。

下表描述點選瀏覽的設定。

| 設定           | 描述 |
|:--|:--|
| 記錄搜尋 (自動) | 选择标题项时要运行的日志查询。  这是该项所基于的同一日志查询。
| 記錄搜尋        | 选择列表中的项时要运行的日志查询。  將查詢輸入 [瀏覽查詢] 方塊。   使用 *{selected item}* 來包含使用者所選項目的語法。  例如，如果查詢具有名為 *Computer* 的資料行且瀏覽查詢是 *{selected item}*，則會在您選取電腦時執行 *Computer="MyComputer"* 之類的查詢。 如果瀏覽查詢是 *Type=Event {selected item}*，則會執行 *Type=Event Computer="MyComputer"*。 |
