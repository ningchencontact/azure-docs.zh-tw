...
標頭有兩個數字，可顯示來自不同記錄查詢的記錄計數。 清單會顯示查詢的前 10 筆結果，並包含表示數值資料行的相對值或一段時間內之變更的圖表。
...
點選瀏覽會定義當您按一下標頭或檢視中的清單項目時，要採取哪個動作。  這會在[Log Analytics](../../azure-monitor/log-query/portals.md)中開啟查詢，或啟動另一個視圖。
