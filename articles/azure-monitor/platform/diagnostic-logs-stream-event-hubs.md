...
> [!WARNING]
> 啟用和串流來自計算資源 (例如，VM 或 Service Fabric) 的診斷記錄 [需要一組不同的步驟](../../azure-monitor/platform/diagnostics-extension-stream-event-hubs.md)。

事件中樞命名空間不一定要和資源發出記錄屬於相同的訂用帳戶，只要進行設定的使用者有這兩個訂用帳戶的適當 RBAC 存取權，而且這兩個訂用帳戶都屬於同一個 ADD 租用戶。
...
3. 如果您選取的資源上沒有任何設定，系統會提示您建立設定。 按一下「開啟診斷」。
   ...
   ![新增診斷設定 - 無現有的設定](media/diagnostic-logs-stream-event-hubs/diagnostic-settings-none.png)
...
您也可以使用 Windows Azure 診斷代理程式，從計算資源中串流診斷記錄。 [請參閱本文章](../../azure-monitor/platform/diagnostics-extension-stream-event-hubs.md)了解如何設定。
