...
計量和資源層級診斷記錄會提供 Azure 資源_內部_作業的相關資訊。 這些適用于大部分的 Azure 服務, 而監視解決方案和深入解析會收集特定服務的其他資料。
...
| Azure 監視器記錄 | 使用 Log Analytics 將平臺計量複製到記錄, 以進行趨勢和其他分析。 | [Azure 診斷 direct to Log Analytics](diagnostic-logs-stream-log-store.md) |
| 事件中樞 | 使用事件中樞將計量串流至其他位置。 |[將 Azure 監視資料串流至事件中樞以供外部工具取用](stream-monitoring-data-event-hubs.md) |

### <a name="diagnostic-logs"></a>診斷記錄
[診斷記錄](diagnostic-logs-overview.md)可讓您深入瞭解 Azure 資源的_內部_作業。  診斷記錄預設為不啟用。 您必須啟用它們, 並指定每個資源的目的地。 

診斷記錄的設定需求和內容會因資源類型而異, 而不是所有服務都會建立診斷記錄。 如需每個服務的詳細資訊和詳細設定程式的連結, 請參閱[Azure 診斷記錄支援的服務、架構和類別](diagnostic-logs-schema.md)。 如果這篇文章中未列出該服務, 則該服務目前不會寫入診斷記錄。

| 目的地 | 描述 | 參考資料 |
|:---|:---|:---|
| Azure 監視器記錄 | 將診斷記錄傳送至 Azure 監視器記錄, 以使用其他收集的記錄資料進行分析。 有些資源可以直接寫入 Azure 監視器, 而有些則會在匯入至 Log Analytics 工作區之前, 先寫入儲存體帳戶。 | [將 Azure 診斷記錄串流至 Azure 監視器中的 Log Analytics 工作區](diagnostic-logs-stream-log-store.md)<br>[使用 Azure 入口網站從 Azure 儲存體收集記錄](azure-storage-iis-table.md#use-the-azure-portal-to-collect-logs-from-azure-storage)  |
| 存放區 | 將診斷記錄檔傳送至 Azure 儲存體以進行封存。 | [封存 Azure 診斷記錄](archive-diagnostic-logs.md) |
| 事件中樞 | 使用事件中樞將診斷記錄串流至其他位置。 |[將 Azure 診斷記錄串流至事件中樞](diagnostic-logs-stream-event-hubs.md) |
...
| 目的地 | 描述 | 參考資料 |
|:---|:---|:---|
| 存放區 | 當您啟用診斷擴充功能時, 預設會將它寫入至儲存體帳戶。 | [在 Azure 儲存體中儲存和檢視診斷資料](diagnostics-extension-to-storage.md) |
