...
1. 若要取得您的混合式連線可與您的訂用帳戶，使用您的 Azure 帳戶登入。 HCM 不會繼續使用之外，您的 Azure 帳戶。 
1. 選擇訂用帳戶。
...
在 App Service **tcpping**命令列工具可以從 [進階工具 (Kudu)] 主控台中叫用。 這個工具可以指出您是否能夠存取 TCP 端點，但不會指出您是否能夠存取「混合式連線」端點。 當您在主控台中對「混合式連線」端點使用此工具時，您只能確認該端點使用「主機:連接埠」組合。  

如果您有命令列用戶端端點時，您可以測試從應用程式主控台的連線。 比方說，您可以使用 curl 來測試 web 伺服器端點的存取權。
