...
ms.locfileid: "53321574"
---
# <a name="how-to-monitor-apps-in-azure-app-service"></a>作法：監視 Azure App Service 中的應用程式
[App Service](https://go.microsoft.com/fwlink/?LinkId=529714) 在 [Azure 入口網站](https://portal.azure.com)中提供內建監視功能。
Azure 入口網站包括檢閱應用程式及 App Service 方案的**配額**和**計量**、設定**警示**，甚至是根據這些計量自動**調整**的功能。
...
如需有關不同 App Service SKU 之可用特定配額、限制及功能的詳細資訊，請參閱這裡：[Azure 訂用帳戶服務限制](../azure-subscription-service-limits.md#app-service-limits)

#### <a name="quota-enforcement"></a>強制配額
如果應用程式超過 **CPU (短期)**、**CPU (天)** 或**頻寬**配額，則應用程式會停止，直到重設配額為止。 在此期間，所有連入要求都會導致 **HTTP 403**。
![][http403]
...
* **平均記憶體工作集**
  * 應用程式使用的平均記憶體數量 (MiB)。
* **CPU 時間**
  * 應用程式所耗用的 CPU 數量 (以秒為單位)。 如需有關此計量的詳細資訊，請參閱：[CPU 時間與 CPU 百分比](#cpu-time-vs-cpu-percentage)
* **資料輸入**
  * 應用程式所耗用的連入頻寬量 (MiB)。
* **資料輸出**
  * 應用程式所耗用的連出頻寬量 (MiB)。
...
**CPU 百分比**很適合用於**基本**、**標準**和**進階**方案中裝載的應用程式，因為這些方案能夠相應放大。CPU 百分比可清楚指出所有執行個體的整體使用量。
...
**配額**。 UX 可讓您檢閱：(1) 配額名稱、(2) 其重設的間隔、(3) 其目前的限制，以及 (4) 目前的值。

![][metrics]
**計量**可以直接從資源頁面存取。 您也可以透過下列方式自訂圖表：(1) **按一下**圖表，然後選取 (2) [編輯圖表]。
您可以從這裡變更要顯示的 (3) [時間範圍]、(4) [圖表類型] 和 (5) [計量]。  

您可以從以下連結深入了解計量：[監視服務計量](../monitoring-and-diagnostics/insights-how-to-customize-monitoring.md)。
...
基本、標準或進階 App Service 方案中裝載的 App Service 應用程式支援**自動調整**。 自動調整可讓您設定用以監視 App Service 方案計量的規則。 這些規則可以增加或減少執行個體計數，視需要提供其他資源。 當應用程式過度佈建時，這些規則也可協助您節省成本。 您可以從以下連結深入了解如何自動調整規模：[如何調整規模](../monitoring-and-diagnostics/insights-how-to-scale.md)和 [Azure 監視器自動調整規模的最佳做法](../azure-monitor/platform/autoscale-best-practices.md)
