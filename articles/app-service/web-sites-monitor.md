...
在 Azure 入口網站中，您可以查看應用程式的*配額*和*計量*，並 App Service 方案，並設定以度量為基礎的*警示* *和自動*調整。
...
![Azure 入口網站中的計量圖表][metrics] 您可以直接從資源的 [**總覽**] 頁面存取度量。 在這裡，您會看到代表一些應用程式計量的圖表。
