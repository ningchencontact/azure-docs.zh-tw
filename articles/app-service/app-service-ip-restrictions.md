...
服務端點可讓您限制對選取的 Azure 虛擬網路子網路的存取。 若要限制存取特定的子網路，請使用一種虛擬網路中建立限制規則。 您可以挑選訂用帳戶、 VNet 和您想要允許或拒絕存取的子網路。 如果服務端點未已經啟用 Microsoft.Web 與您所選取的子網路，它會自動啟用，除非您核取方塊詢問您不要。 如果您有權限，以啟用子網路上的服務端點，或不，您想要在應用程式，但不是在子網路上啟用這種的情況多半會與相關。 如果您需要取得其他人，即可啟用子網路上的服務端點，您可以核取方塊，並讓您設定之服務端點的預期它在稍後啟用子網路上的應用程式。 
