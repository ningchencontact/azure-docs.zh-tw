...
在應用程式的左側導覽中, 按一下 [ **SSL 設定** > ] **[公開憑證 (.cer)**  > ] [**上傳公用憑證**]。
...
若要在您的應用程式程式碼中使用上傳或匯入的憑證`WEBSITE_LOAD_CERTIFICATES` , 請在<a target="_blank" href="https://shell.azure.com" >Cloud Shell</a>中執行下列命令, 讓其指紋可透過應用程式設定來存取:
...
即使您是直接在 .NET 程式碼中載入檔案, 程式庫仍會驗證是否已載入目前的使用者設定檔。 若要載入目前的使用者設定檔, `WEBSITE_LOAD_USER_PROFILE`請在<a target="_blank" href="https://shell.azure.com" >Cloud Shell</a>中使用下列命令來設定應用程式設定。

```azurecli-interactive
az webapp config appsettings set --name <app-name> --resource-group <resource-group-name> --settings WEBSITE_LOAD_USER_PROFILE=1
```

設定此設定之後, 下列C#範例會從應用程式存放庫`mycert.pfx`的`certs`目錄載入名為的憑證。
