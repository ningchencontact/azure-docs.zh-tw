...
3. 針對您新的應用程式輸入 [名稱]和 [說明]。 貼上您應用程式的 **URL** 作為 [網站]值。 然後, 針對 [**回呼 URL**], 輸入 App Service 應用程式的 URL, 並附加路徑`/.auth/login/aad/callback`。 例如： `https://contoso.azurewebsites.net/.auth/login/twitter/callback` 。 請確實使用 HTTPS 配置。
