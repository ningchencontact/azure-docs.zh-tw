...
透過防火牆裝置部署具有輸出流量的 ASE, 需要變更 ASE 子網上的路由。 路由會在 IP 層級運作。 如果您不小心定義路由, 可以將 TCP 回復流量強制從另一個位址到來源。 這稱為「非對稱式路由」, 它會中斷 TCP。

必須定義路由, 才能讓 ASE 的輸入流量以流量傳入的相同方式回復。 這適用于輸入管理要求, 而輸入應用程式要求則為 true。
