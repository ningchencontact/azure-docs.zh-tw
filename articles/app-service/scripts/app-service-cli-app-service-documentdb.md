---
title: "Azure CLI 指令碼範例-web 應用程式連接到 MongoDB (Cosmos DB) |Microsoft 文件"
description: "Azure CLI 指令碼範例-web 應用程式連接到 MongoDB (Cosmos DB)"
services: appservice
...
這個範例指令碼會使用 MongoDB API 和 Azure web 應用程式建立 Azure Cosmos DB 帳戶。 然後，它會連結 MongoDB 連接字串以使用應用程式設定的 web 應用程式。
...
此指令碼會使用下列命令來建立資源群組、 web 應用程式、 Cosmos 資料庫和所有相關的資源。 下表中的每個命令都會連結至命令特定的文件。
...
| [`az cosmosdb list-connection-strings`](/cli/azure/cosmosdb?view=azure-cli-latest#az_cosmosdb_list_connection_strings) | 列出指定的 Cosmos DB 帳戶的連接字串。 |
