...
![Linux 上 Azure App Service 中的 Python Django Web 應用程式](./media/tutorial-python-postgresql-app/run-python-django-app-in-azure.png)
...
> [!NOTE]
> 請記住 \<admin-username> 和 \<admin-password> 以便稍後使用。 您需要這些資訊，才能登入 Postgre 伺服器及其資料庫。
...
### <a name="configure-deployment-user"></a>設定部署使用者
...
如需有關如何在您程式碼中存取這些應用程式設定的詳細資訊，請參閱[存取環境變數](how-to-configure-python.md#access-environment-variables)。
