...
description: 本教學課程說明如何使用 Azure 入口網站建立內部標準負載平衡器。
...
負載平衡會將傳入要求分散於多部虛擬機器 (VM)，藉此提供高可用性和範圍。 您可以使用 Azure 入口網站建立標準負載平衡器，以及平衡多個 VM 間的內部流量。 本教學課程說明如何在標準定價層建立及設定內部負載平衡器、後端伺服器及網路資源。
...
1. 選取 [網路]  索引標籤，或選取 **[下一步：磁碟]** ，然後選取 **[下一步：網路]** 。 
   ...
   - **子網路**：**MyBackendSubnet**
   - **NIC 網路安全性群組**：選取 [基本]  。
   - [公用 IP]  > 選取 [新建]  並輸入下列值，然後選取 [確定]  ：
       - **Name**：**MyVM1-IP**
       - **SKU**：選取 [標準] 
   - **公用輸入連接埠**：選取 [允許選取的連接埠]  。
   - **選取輸入連接埠**：下拉並選取 [RDP (3389)] 
...
1. 選取 [新增]  。
   ...
   ![新增後端位址集區](./media/tutorial-load-balancer-standard-internal-portal/3-load-balancer-backend-02.png)
...
在本教學課程中，您建立了標準內部負載平衡器。 您還建立及設定了資源群組、後端伺服器、健康情況探查，以及負載平衡器的規則。 您已在後端 VM 上安裝 IIS，並使用測試 VM 在瀏覽器中測試負載平衡器。 
