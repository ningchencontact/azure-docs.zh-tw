...
現在您可以使用 [New-AzVM](/powershell/module/az.compute/new-azvm) 建立 VM。 下列範例會建立兩個 VM 及必要的虛擬網路元件 (如果尚未存在)。 在 VM 建立期間，先前建立的 NIC 會與 VM 相關聯，因為它們已被指派相同的虛擬網路 (*myVnet*) 和子網路 (*mySubnet*)：
