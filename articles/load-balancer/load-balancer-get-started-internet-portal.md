---
title: 建立公用基本負載平衡器 - Azure 入口網站 | Microsoft Docs
description: 了解如何使用 Azure 入口網站建立公用基本負載平衡器。
...
負載平衡會將傳入要求分散於多部虛擬機器，藉此提供高可用性和範圍。 您可使用 Azure 入口網站建立負載平衡器，以平衡虛擬機器的負載。 本快速入門示範如何建立網路資源、後端伺服器和公用基本負載平衡器。
...
在本節中，您會使用入口網站建立公用基本負載平衡器。 當您使用入口網站建立負載平衡器資源時，系統會在您建立公用 IP 時，將公用 IP 位址自動設定為負載平衡器的前端 (名為 LoadBalancerFrontend)。
...
本節中，您會建立一個虛擬網路、針對基本負載平衡器的後端集區建立兩部虛擬機器，然後在虛擬機器上安裝 IIS，協助測試負載平衡器。
...
1. 在螢幕的左上方，按一下 [新增] > [計算] > [Windows Server 2016 Datacenter]，然後輸入虛擬網路的下列值：
    ...
    - myResourceGroupLB - 針對 [資源群組]，選取 [使用現有的]，然後選取 myResourceGroupLB。
...
4. 按一下 [SERVICEPRINCIPAL] 。
 ...
 ![建立虛擬網路](./media/load-balancer-get-started-internet-portal/8-load-balancer-nsg-rules.png)
...
5. 在 [伺服器管理員] 中，按一下 [管理]，然後按一下 [新增角色及功能]。
...
2. 將公用 IP 位址複製並貼到您瀏覽器的網址列。 IIS Web 伺服器的預設頁面會顯示在瀏覽器上。
  ...
  ![IIS Web 伺服器](./media/load-balancer-get-started-internet-portal/9-load-balancer-test.png)
...
在本快速入門中，您已建立資源群組、網路資源和後端伺服器。 接著，您可使用這些資源來建立負載平衡器。 若要深入了解負載平衡器及其相關聯的資源，請繼續閱讀教學課程文章。
