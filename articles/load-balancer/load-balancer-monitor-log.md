...
每個 Resource Manager 資源都會自動啟用稽核記錄。 您需要啟用事件和健全狀況探查記錄，才能開始收集可透過這些記錄取得的資料。 使用下列步驟以啟用記錄功能。
...
> [!NOTE]
> 稽核記錄不需要個別的儲存體帳戶。 將儲存體用於事件和健全狀況探查記錄將會產生服務費用。

## <a name="audit-log"></a>稽核記錄檔

預設會產生稽核記錄檔。 記錄會在 Azure 的 [事件記錄] 存放區中保留 90 天。 閱讀 [檢視事件和稽核記錄](../monitoring-and-diagnostics/insights-debugging-with-events.md) 一文，進一步了解這些記錄。
...
* [使用 Power BI 視覺化您的 Azure 稽核記錄](https://blogs.msdn.com/b/powerbi/archive/2015/09/30/monitor-azure-audit-logs-with-power-bi.aspx)部落格文章。
