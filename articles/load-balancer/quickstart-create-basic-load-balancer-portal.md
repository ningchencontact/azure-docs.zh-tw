---
title: 快速入門：建立公用基本 Load Balancer-Azure 入口網站
...
ms.locfileid: "74048693"
---
# <a name="quickstart-create-a-basic-load-balancer-by-using-the-azure-portal"></a>快速入門：使用 Azure 入口網站建立基本 Load Balancer
...
1. 在 [建立虛擬網路] 窗格中，輸入或選取下列值：
   ...
   - **名稱**：輸入 MyVnet。
   - **ResourceGroup**：下拉 [選取現有的]，然後選取 [MyResourceGroupLB]。 
...
1. 在 [新增健康情況探查] 頁面上，輸入或選取下列值：
   ...
   - **通訊協定**：下拉並選取 [HTTP]。 
   - **連接埠**：輸入 80。 
   - **路徑**：接受預設 URI 的 */* 。 您可以使用任何其他 URI 來取代此值。 
   - **間隔**：輸入 15。 間隔是探查嘗試之間的秒數。
   - **狀況不良臨界值**：輸入 2。 這個值是將 VM 視為狀況不良之前，所發生的連續探查失敗次數。
