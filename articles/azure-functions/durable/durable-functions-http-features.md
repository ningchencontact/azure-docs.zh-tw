...
先前所述之 HTTP 回應的設計目的，是協助搭配長期函式實作長時間執行 HTTP 非同步 API。 This pattern is sometimes referred to as the *polling consumer pattern*. 用戶端/伺服器流量運作方式如下：
