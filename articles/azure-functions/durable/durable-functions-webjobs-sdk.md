...
在2.x 版的 Webjob SDK 中，主機是 `IHost`的執行，而在2.x 版中，您會使用 `JobHost` 物件。

[連結 Durable Functions] 範例可在 Webjob SDK 2.x 版本中取得：下載或複製 Durable Functions 存放[庫](https://github.com/azure/azure-functions-durable-extension/)，並移至 [*範例]\\webjobssdk\\[連結*] 資料夾。
...
Visual Studio 也有 WebJob 專案範本，您可以藉由選取**Cloud** > **Azure WebJob （.NET Framework）** 來使用它。 此範本會安裝許多套件，但您可能不需要其中的某些套件。
...
若要使用 Durable Functions 擴充功能，請在 `UseDurableTask` 方法中的 `JobHostConfiguration` 物件上呼叫 `Main`：
...
範例專案中的協調流程用戶端函式會啟動協調器函式，然後進入迴圈，每2秒呼叫一次 `GetStatusAsync`：
...
1. 使用 .NET Core 設定架構，將儲存體連接字串和 Application Insights 檢測金鑰放在*appsettings json*檔案中。 以下是範例：
