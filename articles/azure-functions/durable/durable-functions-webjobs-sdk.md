...
這篇文章說明如何開發 WebJobs SDK 2.x 專案。 如果您正在開發的 WebJobs SDK 3.x 版專案，本節可協助您了解這些差異。

導入的主要變更是使用.NET Core，而不是.NET Framework。 若要建立 WebJobs SDK 3.x 版專案，指示都相同，但有下列例外：

1. 建立 .NET Core 主控台應用程式。 在 Visual Studio**新的專案**對話方塊中，選取 **.NET Core** > **主控台應用程式 (.NET Core)**。 專案檔會將 `TargetFramework` 指定為 `netcoreapp2.0`。

1. 選擇發行前版本的 WebJobs SDK 版本 3.x 的下列套件：

    * `Microsoft.Azure.WebJobs.Extensions`
    * `Microsoft.Azure.WebJobs.Logging.ApplicationInsights`

1. 取得儲存體連接字串和從 Application Insights 檢測金鑰*appsettings.json*檔案中的，使用.NET Core 組態架構。 變更`Main`方法程式碼，若要這樣做。 以下是範例：
