...
Durable Functions 會提供內建 API，以簡化您所撰寫來與長時間執行之函式執行互動的程式碼。 [快速入門範例](durable-functions-create-first-csharp.md)所顯示的是簡單的 REST 命令，此命令可用來啟動新的協調器函式執行個體。 一旦有執行個體啟動，擴充功能就會公開 Webhook HTTP API 來查詢協調器函式的狀態。 下列範例顯示的是 REST 命令，這些命令可用來啟動協調器，以及查詢其狀態。 為了清楚起見，此範例中省略了部分詳細資料。
