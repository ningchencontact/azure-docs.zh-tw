...
> [!NOTE]
> 在中C#撰寫先行編譯的長期函數和在腳本中C#撰寫先行編譯的長期函式之間有些許差異。 在先行C#編譯函式中，持久性參數必須以個別的屬性裝飾。 例如`[OrchestrationTrigger]` ， `DurableOrchestrationContext`參數的屬性。 在先行C#編譯的持久性函式中，如果未正確裝飾參數，執行時間就無法將變數插入函式中，而且會發生錯誤。 如需更多範例，請參閱[GitHub 上的 azure 函式-持久性擴充功能範例](https://github.com/Azure/azure-functions-durable-extension/blob/master/samples)。
