...
協調器函式會排程活動函式，並透過內部佇列訊息收到其回應。 在 Azure Functions 取用方案中，執行的函式應用程式時[Azure Functions 縮放控制器](../functions-scale.md#how-the-consumption-and-premium-plans-work)監視這些佇列。 視需要加入新的計算執行個體。 調升規模到多個 Vm，可能會在一部 VM，而協調器函式呼叫可能在數個不同的 Vm 執行的活動函式上執行協調器函式。 Durable functions 的縮放行為的相關資訊，請參閱[效能和規模](durable-functions-perf-and-scale.md)。
