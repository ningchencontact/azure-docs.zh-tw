...
最大輪詢延遲可透過`maxQueuePollingInterval` [主機. json](../functions-host-json.md#durabletask)檔案中的屬性來設定。 將此設定為較高的值可能會導致訊息處理延遲較高。 只有在閒置一段時間後，才會預期較高的延遲。 將此設定為較低的值，可能會因為儲存體交易增加而導致儲存成本較高。
...
Durable Functions 所使用的佇列、資料表和 blob 會在設定的 Azure 儲存體帳戶中建立。 使用 **host.json** 檔案中的 `durableTask/azureStorageConnectionStringName` 設定，可以指定要使用的帳戶。
