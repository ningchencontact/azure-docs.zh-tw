...
可用的連接數目很有限，部分原因函式應用程式中執行時，才[沙箱環境](https://github.com/projectkudu/kudu/wiki/Azure-Web-App-sandbox)。 其中一個沙箱加諸於您的程式碼的限制是輸出的連線數目限制目前 600 的作用中 （1,200 總計） 連線，每個執行個體。 當您到達此限制時，functions 執行階段會將下列的訊息寫入至記錄檔： `Host thresholds exceeded: Connections`。 如需詳細資訊，請參閱 <<c0> [ 函式的服務限制](functions-scale.md#service-limits)。

這項限制是每個執行個體。 當[縮放控制器會將函式應用程式執行個體](functions-scale.md#how-the-consumption-and-premium-plans-work)若要處理更多的要求，每個執行個體有獨立的連線限制。 這表示沒有全域連線限制，且您可以在所有作用中的執行個體遠超過 600 個作用中的連線。
