...
ms.locfileid: "33762457"
---
# <a name="durable-functions-publishing-to-azure-event-grid-preview"></a>發佈至 Azure 事件方格 (預覽) 的 Durable Functions

本文說明如何設定 Azure Durable Functions，將協調流程生命週期事件 (例如已建立、已完成和失敗) 發佈至自訂 [Azure 事件方格主題](https://docs.microsoft.com/en-us/azure/event-grid/overview)。 

以下是適合使用此功能的一些案例：

* **類似 DevOps 案例的藍色/綠色部署**：您可能想知道在實作[並存部署策略](https://docs.microsoft.com/en-us/azure/azure-functions/durable-functions-versioning#side-by-side-deployments)前是否有任何工作正在執行中。
...
* 安裝 [Azure 儲存體模擬器](https://docs.microsoft.com/en-us/azure/storage/common/storage-use-emulator)。
* 安裝 [Azure CLI 2.0](https://docs.microsoft.com/en-us/cli/azure/?view=azure-cli-latest) 或使用 [Azure Cloud Shell](https://docs.microsoft.com/en-us/azure/cloud-shell/overview)。
...
* [事件方格快速入門：建立自訂事件 - PowerShell](https://docs.microsoft.com/en-us/azure/event-grid/custom-event-quickstart-powershell)
* [事件方格快速入門：建立自訂事件 - Azure 入口網站](https://docs.microsoft.com/en-us/azure/event-grid/custom-event-quickstart-portal)

### <a name="create-a-resource-group"></a>建立資源群組

使用 `az group create` 命令建立資源群組。 事件方格目前不支援所有區域。 如需支援區域的相關資訊，請參閱[事件方格概觀](https://docs.microsoft.com/en-us/azure/event-grid/overview)。 
...
* **EventGridTopicEndpoint** - 事件方格主題的端點。
...
確定[儲存體模擬器](https://docs.microsoft.com/en-us/azure/storage/common/storage-use-emulator)運作正常。 在執行之前，最好先執行 `AzureStorageEmulator.exe clear all` 命令。
...
選取 `Add Event Grid Subscription`。 此作業會為您建立的事件方格主題新增事件方格訂用帳戶。 如需詳細資訊，請參閱 [Azure 事件方格概念](https://docs.microsoft.com/en-us/azure/event-grid/concepts)。
...
* **data**：Durable Functions 專屬參數。
    * **hubName**：[TaskHub](https://docs.microsoft.com/en-us/azure/azure-functions/durable-functions-task-hubs) 名稱。
    * **functionName**：協調器函式名稱。
    * **instanceId**：Durable Functions 執行個體識別碼。
    * **原因**：與追蹤事件相關聯的其他資料。 如需詳細資訊，請參閱 [Durable Functions 中的診斷 (Azure Functions)](https://docs.microsoft.com/en-us/azure/azure-functions/durable-functions-diagnostics)
