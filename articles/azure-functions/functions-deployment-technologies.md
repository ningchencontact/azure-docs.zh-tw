...
| 部署技術 | Windows 耗用量 | Windows Premium (預覽) | Windows 專用  | Linux 使用量 (預覽) | Linux 專用 |
|-----------------------|:-------------------:|:-------------------------:|:-----------------:|:---------------------------:|:---------------:|
| 外部套件 URL<sup>1</sup> |✔|✔|✔|✔|✔|
| Zip 部署 |✔|✔|✔| |✔|
| Docker 容器 | | | | |✔|
...
>__使用時機:__ [外部套件 URL] 是在使用方式方案 (預覽) 中的 Linux 上執行 Azure Functions 唯一支援的部署方法。 當您更新函式應用程式所參考的封裝檔案時, 您必須[手動同步處理觸發](#trigger-syncing)程式, 以告知 Azure 您的應用程式已變更。
...
| | Windows 耗用量 | Windows Premium (預覽) | Windows 專用 | Linux 使用量 (預覽) | Linux Premium (預覽)| Linux 專用 |
|-|:-----------------: |:-------------------------:|:-----------------:|:---------------------------:|:---------------:|:---------------:|
| C# | | | | | |
