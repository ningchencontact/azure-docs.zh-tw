...
__其用途：__ 可讓您參考包含函式應用程式的遠端封裝 (.zip) 檔案。 從提供的 URL，下載檔案，並在中執行的應用程式[執行從套件](run-functions-from-deployment-package.md)模式。
...
__其用途：__ 可讓您將包含函式應用程式至 Azure 的 zip 檔案。 （選擇性） 您也可以指定將您的應用程式的開頭[執行從套件](run-functions-from-deployment-package.md)模式。
...
__其用途：__ 指定 Azure Functions 會提取及執行的容器映像。
...
__其用途：__ 封裝和部署您的 Windows 應用程式至任何 IIS 伺服器-包括在 Windows 上執行的 Azure 函式應用程式。
...
__其用途：__ 可讓您將函式應用程式連線到 git 存放庫，使得該存放庫中的程式碼的任何更新會觸發部署。 如需詳細資訊，看看[Kudu Wiki](https://github.com/projectkudu/kudu/wiki/VSTS-vs-Kudu-deployments)。
...
__其用途：__ 可讓您將程式碼從本機電腦推送至 Azure Functions 使用 Git。
...
__其用途：__ 可讓您同步處理您的內容從 Dropbox 與 OneDrive Azure Functions。
...
__其用途：__ 可讓您直接傳送檔案至 Azure Functions。
...
__其用途：__ 使用入口網站為基礎的編輯器可讓您直接編輯函式應用程式上的檔案 (基本上每當您按一下在部署**儲存**)。
...
#### <a name="portal-editing-availability"></a>編輯可用性的入口網站
...
<sup>1</sup>入口網站啟用編輯只有 HTTP 和計時器觸發程序使用 「 專用計劃在 Linux 上的函式。
...
## <a name="other-relevant-information"></a>其他相關資訊
...
> [Zip 部署適用於 Azure Functions](deployment-zip-push.md) 
> [從套件檔案中執行您的 Azure Functions](run-functions-from-deployment-package.md)
> [函式應用程式在 Azure Functions 中的自動化資源部署](functions-infrastructure-as-code.md)
