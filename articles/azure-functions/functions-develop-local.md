...
|環境                              |語言         |說明|
|-----------------------------------------|------------|---|
| [命令提示字元或終端機](functions-run-local.md) | [C# 指令碼 (.csx)](functions-reference-csharp.md)、[JavaScript](functions-reference-node.md) | [Azure Functions Core Tools] 提供核心執行階段和範本來建立函式，以便進行本機開發。 2.x 版支援在 Linux、MacOS 和 Windows 上進行開發。 所有環境都依賴 Core Tools 執行本機 Functions 執行階段。|
