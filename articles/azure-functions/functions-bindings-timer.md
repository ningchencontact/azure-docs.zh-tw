...
> [!CAUTION]
> 建議不要在生產環境中將 **runOnStartup** 設定為 `true`。 使用此設定會在極度無法預期的情況下執行程式碼。 在特定的生產環境設定中，對於取用量方案中託管的應用程式，這些額外的執行會導致成本大幅增加。 例如，使用**runOnStartup**啟用每當相應函式應用程式時，會叫用觸發程序。 請確定在生產環境中啟用 **runOnStartup** 之前，您完全了解函式的實際執行行為。   
