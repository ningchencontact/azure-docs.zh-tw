...
> [!NOTE]
> 目前無法在 Azure Functions 中設定控制佇列資料分割數目。 [正在設法支援此設定選項](https://github.com/Azure/azure-functions-durable-extension/issues/73)。

一般而言，協調器函式是輕巧的設計，並不需要大量運算能力。 因此，不需要建立大量的控制佇列分割區來獲得很高的輸送量。 相反地，大部分繁重的工作都在無狀態活動函式中進行，這還可以無限制地向外延展。

## <a name="auto-scale"></a>自動調整規模

由於所有 Azure Functions 都在使用情況方案中執行，Durable Functions 支援透過 [Azure Functions 縮放控制器](https://docs.microsoft.com/azure/azure-functions/functions-scale#runtime-scaling)來自動調整規模。 「縮放控制器」會監視工作項目佇列的長度和每個控制佇列，然後據以新增或移除虛擬機器執行個體。 如果控制佇列長度隨著時間而增加，縮放控制器會持續新增 VM 執行個體，直到達到控制佇列資料分割計數為止。 如果工作項目佇列長度隨著時間而增加，縮放控制器會持續新增虛擬機器執行個體，直到符合負載為止，不會考慮控制佇列資料分割計數。
