...
您可以藉由在啟動之後連接至函式主機，來對 IntelliJ 中的函式進行偵錯。  使用上述步驟在本機執行 Azure 函式，然後在 [執行] 功能表中，選取 [連接至本機處理序]。  您應該會看到連接埠 5005 上的處理序可供使用。  連接之後，您會有可在函式應用程式內叫用和偵錯的中斷點。
