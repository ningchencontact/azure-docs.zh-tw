...
| 值  | 描述  |
|---------|---------|
| **`1`**  | 建議用於 Windows 上執行的函數應用程式。 從函式應用程式中 `d:\home\data\SitePackages` 資料夾內的套件檔案執行。 如果不是[使用 zip deploy 進行部署](#integration-with-zip-deployment)，此選項會要求資料夾也必須有名為 `packagename.txt`的檔案。 此檔案只會包含資料夾內套件檔案的名稱，且不含任何空白字元。 |
