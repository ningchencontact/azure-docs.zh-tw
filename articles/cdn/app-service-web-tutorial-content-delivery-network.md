...
ms.locfileid: "34161239"
---
# <a name="tutorial-add-azure-cdn-to-an-azure-app-service-web-app"></a>教學課程：將 Azure CDN 新增至 Azure App Service Web 應用程式

本教學課程說明如何將 Azure CDN 新增至 [Azure App Service 中的 Web 應用程式](../app-service/app-service-web-overview.md)。 [Azure 內容傳遞網路 (CDN)](cdn-overview.md) 會在策略性放置的位置上快取靜態 Web 內容，以提供最大輸送量來將內容傳遞給使用者。 Azure CDN 也可降低您的 Web 應用程式的伺服器負載。 
...
若要完成本教學課程的自訂網域步驟，您需要擁有自訂網域並且具有網域提供者的 DNS 登錄存取權。 例如，若要為 contoso.com 和 www.contoso.com 新增 DNS 項目，您必須有權設定 contoso.com 根網域的 DNS 設定。

如果您還沒有網域名稱，請考慮遵循 [App Service 網域教學課程](../app-service/custom-dns-web-site-buydomains-web-app.md)的作法，使用 Azure 入口網站來購買網域。 
...
您必須建立 CNAME 記錄，以將自訂網域對應至 CDN 端點。 CNAME 記錄是將來源網域對應至目的地網域的 DNS 功能。 舉例來說，您必須將 cdn.contoso.com 或 static.contoso.com 對應至 contoso.azureedge.net。

如果您沒有自訂網域，請考慮遵循 [App Service 網域教學課程](../app-service/custom-dns-web-site-buydomains-web-app.md)的作法，使用 Azure 入口網站來購買網域。 
...
瀏覽至您網域註冊機構的網站，並找出用於建立 DNS 記錄的區段。 例如，此區段可能會命名為**網域名稱**、**DNS** 或 **名稱伺服器管理**。
...
在瀏覽器中使用您的自訂網域 (例如 cdn.contoso.com/index.html) 瀏覽至 *index.html*檔案，來驗證結果與您直接前往 &lt;endpointname&gt;azureedge.net/index.html 的結果相同。
