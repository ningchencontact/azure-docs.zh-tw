...
% | 百分比符號的意義取決於其使用方式。<br/><br/> `%{HTTPVariable}`:這個語法會識別 HTTP 變數。<br/>`%{HTTPVariable%Pattern}`:此語法會使用百分比符號來識別 HTTP 變數和作為分隔符號。<br />`\%`:將百分比符號進行轉義可讓它當做常值使用, `\%20`或表示 URL 編碼 (例如)。
\* | 星號可讓前置字元比對零或多次。
空白字元 | 空白字元通常會被視為常值字元。
