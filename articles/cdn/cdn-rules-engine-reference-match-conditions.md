...
- 國家/地區比對：國家/地區 」 比對條件設為所需的國家/地區的集。
   - 允許：設定為 「 國家/地區 」 比對條件**Se Neshoduje**僅允許指定的國家/地區存取儲存在 URL 路徑萬用字元比對條件所定義之位置的內容。
   - 封鎖：若要設定 「 國家/地區 」 比對條件**相符項目**封鎖指定的國家/地區存取儲存在 URL 路徑萬用字元比對條件所定義之位置的內容。
