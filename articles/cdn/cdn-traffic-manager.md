---
title: 使用流量管理員在多個 Azure CDN 端點之間進行容錯移轉
...
1. 遵循**建立新的 CDN 設定檔**中的步驟，建立**來自 Verizon 的 Azure CDN 標準**和[來自 Akamai 的 Azure CDN 標準](cdn-create-new-endpoint.md#create-a-new-cdn-profile)設定檔。
...
1. 請移至自訂網域網域提供者的網站，例如 GoDaddy，然後建立兩個 DNS CNAME 項目。 
    ...
    b.這是另一個 C# 主控台應用程式。 針對第二個 CNAME 項目，將沒有 cdnverify 子網域的自訂網域對應至 CDN 端點。 此項目會將自訂網域對應至流量管理員。 
...
2.  從您的 Azure CDN 設定檔中，選取第一個 CDN 端點 (Akamai)。 選取 [**新增自訂網域**]，然後輸入*cdndemo101. dustydogpetcare. online*。 確認用來驗證自訂網域的核取記號為綠色。 
    ...
    > [!NOTE]
    > 若要在**來自 Akamai 的 AZURE CDN**設定檔上啟用 SSL，您必須將自訂網域直接 cname 至您的端點。 尚不支援啟用 SSL 的 cdnverify。 
    >
