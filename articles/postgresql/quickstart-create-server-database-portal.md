...
6. 輸入 `\q`，然後選取 Enter 鍵結束 psql。 完成之後，您可以關閉 Cloud Shell。

您已在 Cloud Shell 中透過 psql 連線到適用於 PostgreSQL 的 Azure 資料庫伺服器，並已建立空白的使用者資料庫。 繼續進行下一節，以使用另一個常見工具 pgAdmin 連線。
