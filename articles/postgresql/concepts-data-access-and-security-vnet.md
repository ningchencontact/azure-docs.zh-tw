---
title: 適用於 PostgreSQL 的 Azure 資料庫-單一伺服器中的虛擬網路 (VNet) 服務端點總覽
description: 瞭解虛擬網路 (VNet) 服務端點如何適用于適用於 PostgreSQL 的 Azure 資料庫單一伺服器。
author: bolzmj
ms.author: mbolz
...
若要建立虛擬網路規則, 必須先有[虛擬網路][vm-virtual-network-overview](VNet) 和[虛擬網路服務端點][vm-virtual-network-service-endpoints-overview-649d], 規則才能參考。 下圖說明虛擬網路服務端點如何與適用於 PostgreSQL 的 Azure 資料庫搭配運作：
