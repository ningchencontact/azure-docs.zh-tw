---
title: 在 Azure Database for PostgreSQL-單一伺服器的防火牆規則
description: 本文描述適用於 Azure Database for PostgreSQL-單一伺服器的防火牆規則。
author: rachel-msft
ms.author: raagyema
...
ms.locfileid: "65073577"
---
# <a name="firewall-rules-in-azure-database-for-postgresql---single-server"></a>在 Azure Database for PostgreSQL-單一伺服器的防火牆規則
適用於 PostgreSQL 伺服器的防火牆的 azure 資料庫可防止所有存取對您資料庫伺服器，直到您指定哪些電腦擁有權限。 此防火牆會根據每一個要求的來源 IP 位址來授與伺服器存取權。
...
* **允許清單的變更尚未生效：** 適用於 PostgreSQL 的 Azure 資料庫伺服器防火牆設定變更可能會延遲最多 5 分鐘才能生效。

* **登入未獲授權或使用不正確的密碼：** 如果適用於 PostgreSQL 的 Azure 資料庫伺服器上的登入沒有權限，或所使用的密碼不正確，與適用於 PostgreSQL 的 Azure 資料庫伺服器的連線就會遭到拒絕。 建立防火牆設定只是讓用戶端有機會嘗試連線到您的伺服器；每個用戶端仍然必須提供必要的安全性認證。
