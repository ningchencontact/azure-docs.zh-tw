...
| **連接數目上限**        |                   |
| 基本的 50 個計算單位     | 55 個連線    |
| 基本的 100 個計算單位    | 105 個連線   |
| 標準的 100 個計算單位 | 150 個連線   |
| 標準的 200 個計算單位 | 250 個連線   |
| 標準的 400 個計算單位 | 480 個連線   |
| 標準的 800 個計算單位 | 950 個連線   |
| **計算單位數目上限**      |                   |
| 基本服務層         | 100 個計算單位 |
| 標準服務層      | 800 個計算單位 |
| **儲存體上限**            |                   |
...
Azure 系統需要五個連線，以用於監控適用於 PostgreSQL 伺服器的 Azure 資料庫。 到達太多連接時，您可能會收到下列錯誤：
> 嚴重錯誤︰很抱歉，已經有太多用戶端
