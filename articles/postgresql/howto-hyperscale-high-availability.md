---
title: 設定適用於 PostgreSQL 的 Azure 資料庫超大規模資料庫（Citus）伺服器群組的高可用性
...
在伺服器群組建立期間，或在 Azure 入口網站中伺服器群組的 [**設定**] 索引標籤中，可以啟用 HA。 在任一情況下，使用者介面看起來都很類似。 將 [**高可用性**] 滑杆拖曳至 [開啟] 或 [關閉]：
