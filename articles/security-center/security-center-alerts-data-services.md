...
|**Tor 異常**|表示已成功從稱為作用中結束節點 Tor (匿名 proxy) 的 IP 位址存取此帳戶。 此警示的嚴重性會考慮所使用的驗證類型 (如果有的話), 以及這是否為這類存取的第一個案例。可能的原因可能是攻擊者使用 Tor 存取了您的儲存體帳戶, 或合法的使用者已使用 Tor 存取您的儲存體帳戶。|
...
如需有關存放裝置警示的詳細資訊, 請參閱[Azure 儲存體的 Advanced 威脅防護](../storage/common/storage-advanced-threat-protection.md)一文, 以及查看保護警示一節。
