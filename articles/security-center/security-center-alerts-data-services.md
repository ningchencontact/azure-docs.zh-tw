...
|警示|描述|
|---|---|
|**從不尋常的位置存取**|表示 Azure 儲存體帳戶的存取模式有所變更。 與最近的活動相較之下，有人從 IP 位址存取此帳戶視為不熟悉。 可能是攻擊者已取得帳戶的存取權，或合法的使用者已從新的或不尋常的地理位置進行連線。 後者的範例是從新的應用程式或開發人員進行遠端維護。|
