...
|應啟用安全傳輸至儲存體帳戶|啟用儲存體帳戶的安全傳輸。 安全傳輸這個選項會強制您的儲存體帳戶僅接受來自安全連線 (HTTPS) 的要求。 使用 HTTPS 可確保伺服器與服務之間的驗證, 並保護傳輸中的資料免于網路層的攻擊, 例如中間人攻擊、竊聽及會話劫持。|高|20|儲存體帳戶|
