...
|資源類型|安全分數|建議|描述|
|----|----|----|----|
|Machine|40|應該啟用虛擬機器的網路安全性群組|啟用網路安全性群組以控制虛擬機器的網路存取。|
|Subnet|35|應該啟用子網路層級上的網路安全性群組|啟用網路安全性群組以控制子網路中所部署資源的網路存取。|
|Machine|30|在 Just-in-time 網路存取控制應該套用在虛擬機器上|套用 Just-in-Time VM 存取控制，以永久鎖定對所選連接埠的存取，並讓已獲授權的使用者能夠透過相同的機制且在一段有限的時間內開啟這些連接埠。|
|Machine|20|限制透過網際網路面向端點的存取|藉由限制現有允許規則的存取，強化網際網路對應 VM 的網路安全性群組。|
|Machine|10|新增新一代防火牆|新增「新一代防火牆」(NGFW) 解決方案，以加強保護網際網路對應 VM。|
|Machine|5|只透過網路閘道防火牆來路由傳送流量|為了完成新一代防火牆解決方案的部署，傳送至受保護網際網路對應 VM 的流量應該只透過新一代防火牆解決方案進行路由傳送。|
|VNet|5|啟用 DDoS 保護標準|針對在這些虛擬網路中具有公用 IP 的應用程式，並未以 DDOS 保護服務標準提供保護。 建議啟用該功能，以降低網路遭受大流量攻擊與通訊協定攻擊的風險。|
