...
- **標準**層將免費層的功能擴展到在私人雲端和其他公用雲端中執行的工作負載，為您的混合式雲端工作負載提供統一的安全性管理和威脅防護。 標準層也新增了先進的威脅偵測功能，其使用內建行為分析和機器學習來識別攻擊和零時差惡意探索、存取和應用程式控制，以降低暴露于網路攻擊和惡意程式碼的風險，以及個. 此外，標準層會為您的虛擬機器新增弱點掃描。 您可以免費試用標準層。 資訊安全中心 Standard 支援 Azure 資源，包括 Vm、虛擬機器擴展集、App Service、SQL server 和儲存體帳戶。 如果您有 Azure 資訊安全中心 Standard，可以根據資源類型退出宣告支援。 
