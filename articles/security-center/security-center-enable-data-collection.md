...
- **最小** - 確保這個集合所涵蓋的僅有可能代表發生成功入侵和重大事件的事件，數量很少。 比方說，這個集合包含使用者成功和失敗登入 （事件識別碼 4624、 4625），但它不包含符號外很重要的稽核，但不是偵測才有意義且其相對高的磁碟區。 這個集合的大部分資料量是登入事件和流程建立事件 (事件識別碼 4688)。
- **一般** - 提供此集合中的完整使用者稽核記錄。 比方說，這個集合所包含的使用者登入和使用者簽出 （事件識別碼 4634）。 納入了各種稽核動作，例如安全性群組變更、主要網域控制站 Kerberos 作業，以及業界組織建議的其他事件。
