...
|應啟用安全傳輸至儲存體帳戶|在您的儲存體帳戶中審查安全傳輸的需求。 安全傳輸這個選項會強制您的儲存體帳戶僅接受來自安全連線 (HTTPS) 的要求。 使用 HTTPS 可確保伺服器與服務之間的驗證。 它也會保護傳輸中的資料免于網路層的攻擊, 例如中間人、竊聽及會話劫持。|
...
|只應啟用您 Redis 快取的安全連線|Audit 僅允許透過 SSL 連線至 Azure Cache for Redis 的連線。 使用安全連線可確保伺服器與服務之間的驗證。 它也會保護傳輸中的資料免于網路層的攻擊, 例如中間人、竊聽及會話劫持。|
...
|應該啟用 SQL 資料庫上的透明資料加密|稽核 SQL 資料庫的透明資料加密狀態。|
...
|\[預覽]:即時網路存取控制應套用在虛擬機器上|Azure 資訊安全中心會依建議監視可能的網路即時存取。|
|\[預覽]:應該在虛擬機器上啟用適應性應用程式控制|Azure 資訊安全中心監視可能的應用程式白名單設定。|
...
|\[預覽]:您的訂用帳戶應指定最多3位擁有者|我們建議您指定最多三個訂用帳戶擁有者, 以降低受到遭入侵之擁有者入侵的可能性。|
|\[預覽]:您的訂用帳戶應該已指派一個以上的擁有者|我們建議您指定多個訂用帳戶擁有者, 以確保系統管理員存取權是重複的。|
...
|\[預覽]:應從您的訂用帳戶移除已淘汰的帳戶|已取代帳戶應該從您的訂用帳戶中移除。 已封鎖已淘汰的帳戶進行登入。|
