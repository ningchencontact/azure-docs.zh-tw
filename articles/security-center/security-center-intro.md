...
- **可操作的建議** – 運用已排定優先順序和可操作的安全性建議，在攻擊者入侵之前修正安全性弱點。
...
### <a name="policy-compliance"></a>原則合規性
原則合規性是由指派之所有原則的合規性因素所決定。 管理群組、訂用帳戶或工作區的整體合規性分數是指派的加權平均值。 單一指派中原則數目的加權平均因素，以及套用指派的資源數目。
