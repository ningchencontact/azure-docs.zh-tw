...
|應該在 SQL 受控實例上啟用 Advanced data security|此動作會在選取的 SQL 受控實例上啟用 SQL Advanced Data Security (ADS)。 <br>**注意**： <ul><li>針對所選 SQL 受控實例的每個區域和資源群組, 將會建立用於儲存掃描結果的儲存體帳戶, 並由該區域中的所有實例共用。</li><li> 廣告的費用為每個 SQL 受控實例 $15。</li></ul>|
...
|應該啟用 Key Vault 中的診斷記錄|此動作會在金鑰保存庫上啟用診斷記錄。 診斷記錄和計量會儲存在選取的工作區中。|
