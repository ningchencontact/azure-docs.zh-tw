...
* **變更可能遭到濫用以略過 UAC 的登錄機碼**：此警示表示已變更可能遭到濫用以略過 UAC (使用者帳戶控制) 的登錄機碼，攻擊者通常用於在遭盜入侵的主機上從不具權限 (標準使用者) 移到具權限 (例如系統管理員) 的存取。
...
* **來自多個來源的可疑連入 SSH 網路活動**：網路流量分析偵測到異常的連入 SSH 通訊。 具體來說，取樣的網路資料會顯示唯一 IP 連線到您的電腦，這在此環境中會被視為異常情況。 這項活動可能表示嘗試從多部主機 (殭屍網路) 對您的 SSH 端點進行暴力密碼破解攻擊。
