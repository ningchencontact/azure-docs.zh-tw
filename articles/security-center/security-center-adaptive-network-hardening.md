...
1. 在 資訊安全中心中，選取**Networking** -> **調適性網路強化**。 Vm 網路列在三個不同的索引標籤：
   ...
   * **資源未經掃描便傳遞**:無法執行的自適性網路強化的演算法，因為下列原因之一的 Vm:
...
1. 從**狀況不良的資源**索引標籤上，選取 VM。 會列出警示和建議的強化規則。
   ...
   ![堅固功能規則](./media/security-center-adaptive-network-hardening/hardening-rules.png)
...
* A**拒絕所有流量**規則是會列在這裡，「 都拒絕 」 規則的唯一類型，而且無法修改。 可以不過，可以將它刪除 (請參閱[刪除規則](#delete-rule))。
  > [!NOTE]
  > A**拒絕所有流量**規則條件時，建議，如此一來執行演算法，資訊安全中心不會識別應允許，現有的 NSG 組態為基礎的流量。 因此，建議的規則是拒絕指定的連接埠的所有流量。 這種類型的規則的名稱會顯示為 「 產生系統 」。 之後強制執行這項規則時，它在 NSG 中的實際名稱會是通訊協定、 流量方向、 「 拒絕 」 和一個隨機數字所組成的字串。

*若要修改的自適性網路強化的規則：*

1. 若要修改某些參數的規則，在**規則**索引標籤上，按一下規則的資料列結尾的三個點 （...） 上，按一下**編輯規則**。
...
1. 在 **編輯規則**視窗中，輸入詳細資料，然後按一下**儲存**。

   > [!NOTE]
   > 按一下後**儲存**、 您已成功新增規則，以及列出的其他建議的規則。 不過，您不具有在 NSG 上套用。 若要啟用它，您必須在清單中，選取的規則，按一下**強制執行**（為下一個步驟所述）。
...
1. 在 **規則**索引標籤上，按一下規則的資料列結尾的三個點 （...） 上，按一下**刪除規則**。
