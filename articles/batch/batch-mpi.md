...
多重執行個體工作可讓您在多個計算節點上同時執行 Azure Batch 工作。 這些工作可以在 Batch 中實現高效能運算案例，例如訊息傳遞介面 (MPI) 應用程式。 在本文中, 您將瞭解如何使用[Batch .net][api_net]程式庫來執行多重實例工作。
...
4. 下載一般資源檔之後，主要工作和子工作會執行您在多重執行個體設定中指定的 **協調命令** 。 協調命令通常用來準備執行工作所需的節點。 這可能包括啟動背景服務 (如[Microsoft MPI][msmpi_msdn] `smpd.exe`), 以及確認節點已準備好處理節點間的訊息。
5. 主要工作及所有子工作順利完成協調命令「之後」，主要工作會在主要節點上執行**應用程式命令**。 應用程式命令是多重執行個體工作自有的命令列，而且只有主要工作能執行。 在以[MS mpi][msmpi_msdn]為基礎的解決方案中, 這是您使用`mpiexec.exe`執行已啟用 MPI 之應用程式的位置。

> [!NOTE]
> 雖然它的功能不同, 但「多重實例工作」並不是唯一的工作類型, 例如[StartTask][net_starttask]或[JobPreparationTask][net_jobprep]。 多重實例工作只是已設定多重實例設定的標準 Batch 工作 (在 Batch .NET 中為[CloudTask][net_task] )。 在本文中，我們將它稱為 **多重執行個體工作**。
>
>
...
若要執行具有多重執行個體工作的 MPI 應用程式，您必須先在集區的計算節點上安裝 MPI 實作 (例如 MS-MPI 或 Intel MPI)。 這是使用[StartTask][net_starttask]的好時機, 它會在節點加入集區或重新開機時執行。 此程式碼片段會建立 StartTask, 將 MS-MPI 安裝套件指定為[資源檔][net_resourcefile]。 資源檔下載至節點後，便會執行啟動工作的命令列。 在此案例中，命令列會執行 MS-MPI 的自動安裝。
...
既然我們已討論過集區需求和 MPI 套件安裝，現在讓我們建立多重執行個體工作。 在此程式碼片段中, 我們會建立標準[CloudTask][net_task], 然後設定其[MultiInstanceSettings][net_multiinstance_prop]屬性。 如先前所述，多重執行個體工作不是獨特的工作類型，而只是已設定多重執行個體設定的標準 Batch 工作。
...
請注意此協調命令中使用 `start` 。 這是必要的，因為 `smpd.exe` 應用程式不會在執行後立即傳回。 如果沒有使用[start][cmd_start]命令, 此協調命令就不會傳回, 因此會封鎖應用程式命令執行。
...
Batch 會在配置給多重實例工作的計算節點上, 建立多個實例工作特有的數個[環境變數][msdn_env_var]。 您的協調和應用程式命令列可以參考這些環境變數，它們執行的指令碼和程式也可以。
...
如需這些和其他 Batch 計算節點環境變數的完整詳細資料 (包括其內容和可見度), 請參閱[計算節點環境變數][msdn_env_var]。

> [!TIP]
> Batch Linux MPI 程式碼範例含有幾個如何使用這些環境變數的範例。 [協調-cmd][coord_cmd_example] Bash 腳本會從 Azure 儲存體下載一般應用程式和輸入檔案、在主要節點上啟用網路檔案系統 (NFS) 共用, 並將其他配置給多重實例工作的節點設定為 NFS 用戶端。
>
>
...
多重實例工作的[TaskConstraints][net_taskconstraints] (例如[MaxTaskRetryCount][net_taskconstraint_maxretry]、 [MaxWallClockTime][net_taskconstraint_maxwallclock]和[RetentionTime][net_taskconstraint_retention]屬性) 會被視為標準工作的, 並套用至主要和所有子工作。 不過, 如果您在將多重實例工作新增到作業之後變更[RetentionTime][net_taskconstraint_retention]屬性, 這項變更只會套用到主要工作。 所有的子工作會繼續使用原始的[RetentionTime][net_taskconstraint_retention]。

如果最近的工作是多重執行個體工作的一部分，計算節點的最近工作清單會反映子工作的識別碼。

## <a name="obtain-information-about-subtasks"></a>取得子工作的相關資訊
若要使用 Batch .NET 程式庫來取得子工作的資訊, 請呼叫[CloudTask. cloudtask.listsubtasks][net_task_listsubtasks]方法。 這個方法會傳回所有子工作的相關資訊，以及已執行工作的計算節點的相關資訊。 您可以根據這項資訊判斷每項子工作的根目錄、集區識別碼、其目前狀態、結束代碼等等。 您可以搭配使用這項資訊與[PoolOperations. computenode.getnodefile][poolops_getnodefile]方法, 以取得子工作的檔案。 請注意，這個方法不會傳回主要工作 (識別碼 0) 的相關資訊。

> [!NOTE]
> 除非另有說明, 否則在多重實例[CloudTask][net_task]本身運作的 Batch .net 方法*僅*適用于主要工作。 例如, 當您在多重實例工作上呼叫[CloudTask. 呼叫 cloudtask.listnodefiles][net_task_listnodefiles]方法時, 只會傳回主要工作的檔案。
>
>
...
GitHub 上的[MultiInstanceTasks][github_mpi]程式碼範例示範如何使用多重實例工作在 Batch 計算節點上執行[MS-MPI][msmpi_msdn]應用程式。 請遵循[準備](#preparation)和[執行](#execution)中的步驟來執行範例。

### <a name="preparation"></a>準備工作
1. 遵循[如何編譯及執行簡單的 MS MPI 程式][msmpi_howto]中的前兩個步驟。 這可滿足下一個步驟的必要條件。
2. 建立[MPIHelloWorld][helloworld_proj]範例 MPI 程式的*發行*版。 這是多重執行個體工作將在計算節點上執行的程式。
3. 建立包含 `MPIHelloWorld.exe` (您在步驟 2 所建置) 和 `MSMpiSetup.exe` (您在步驟 1 所下載) 的 zip 檔案。 您會在下一個步驟中，將此 zip 檔案上傳為應用程式套件。
4. 使用[Azure 入口網站][portal]建立名為 "MPIHelloWorld" 的批次[應用程式](batch-application-packages.md), 並將您在上一個步驟中建立的 zip 檔案指定為應用程式套件的版本 "1.0"。 如需詳細資訊，請參閱[上傳及管理應用程式](batch-application-packages.md#upload-and-manage-applications)。
...
1. 從 GitHub 下載[azure 批次範例][github_samples_zip]。
...
5. *選擇性*：請先使用[Azure 入口網站][portal]或[Batch Explorer][batch_labs]來檢查範例集區、作業和工作 ("MultiInstanceSamplePool"、"MultiInstanceSampleJob"、"MultiInstanceSampleTask"), 然後再刪除資源。

> [!TIP]
> 如果您沒有 Visual Studio, 可以免費下載[Visual Studio Community][visual_studio] 。
>
>
...
* Microsoft HPC & Azure Batch Team blog 會[在 Azure Batch 上討論適用于 Linux 的 MPI 支援][blog_mpi_linux], 並包含使用[OpenFOAM][openfoam]搭配 Batch 的相關資訊。 您可以[在 GitHub 上找到 OpenFOAM 範例][github_mpi]的 Python 程式碼範例。
