...
4. 下載一般資源檔之後，主要工作和子工作會執行您在多重執行個體設定中指定的 **協調命令** 。 協調命令通常用來準備執行工作所需的節點。 這可能包括啟動背景服務（如[MICROSOFT MPI][msmpi_msdn]的 `smpd.exe`），並確認節點已準備好處理節點間的訊息。
...
5. *選擇性*：使用 [ [Azure 入口網站][portal]] 或 [ [Batch Explorer][batch_labs] ] 來檢查範例集區、作業和工作（"MultiInstanceSamplePool"、"MultiInstanceSampleJob"、"MultiInstanceSampleTask"），然後才刪除資源。
