...
您可能想要保留一份工作產生的記錄檔，或失敗應用程式所產生的損毀傾印檔案。 在這類情況下使用**作業釋放工作**，將此資料壓縮並上傳至[Azure 儲存體][azure_storage]帳戶。
...
作業釋放工作最多可以執行15分鐘，然後再由 Batch 服務終止。 如需詳細資訊，請參閱[REST API 參考檔](https://docs.microsoft.com/rest/api/batchservice/job/add#jobreleasetask)。
> 
> 
...
在此程式碼片段中，`myBatchClient` 是[BatchClient][net_batch_client]的實例，而 `myPool` 是 Batch 帳戶內的現有集區。
...
若要查看作用中的作業準備和發行工作，請查看 GitHub 上的[JobPrepRelease][job_prep_release_sample]範例專案。 此主控台應用程式會做這些事：
...
當您執行範例應用程式時，可以使用[Azure 入口網站][portal]來查看作業和其工作的屬性，或甚至下載作業的工作修改過的共用文字檔。
