...
節點可以執行節點作業系統環境所支援的任何可執行檔或指令碼。 可執行檔或\*腳本包括適用\*于 Windows 的\*.exe、.cmd、.bat 和 PowerShell 腳本，以及適用于 Linux 的二進位檔、shell 和 Python 腳本。
...
* **工作**：**在工作專案目錄中**，會為節點上執行的每個工作建立一個目錄。 它是透過參考`AZ_BATCH_TASK_DIR`環境變數來存取。
