...
您可以指派優先順序給在 Batch 中建立的作業。 Batch 服務會使用作業的優先順序值，以決定帳戶內的作業排程順序 (這不會與 [排程的作業](#scheduled-jobs)混淆)。 優先順序值可以介於 -1000 到 1000，-1000 表示最低優先順序，1000 表示最高優先順序。 若要更新作業的優先順序, 請呼叫[更新作業][rest_update_job] operation (Batch REST), or modify the [CloudJob.Priority][net_cloudjob_priority]屬性的屬性 (Batch .net)。
...
如需此功能的更深入詳細資料, 請參閱 Azure Batch 和[對 taskdependencies][github_sample_taskdeps] code sample in the [azure-batch-samples][github_samples] GitHub 存放庫[中](batch-task-dependencies.md)的工作相依性。
...
您可以填入這些實體的「環境設定」  屬性，以在工作或作業層級設定自訂環境變數。 例如, 請參閱 Batch .net 中的[將工作新增至作業][rest_add_task] operation (Batch REST API), or the [CloudTask.EnvironmentSettings][net_cloudtask_env]和[CloudJob cloudjob.commonenvironmentsettings][net_job_env]屬性。

您的用戶端應用程式或服務可以藉由使用 [[取得工作][rest_get_task_info] operation (Batch REST) or by accessing the [CloudTask.EnvironmentSettings][net_cloudtask_env]的相關資訊] 屬性 (Batch .net) 來取得工作的環境變數, 這兩者都是服務定義的和自訂的。 在計算節點上執行的程序可以在節點上存取這些和其他環境變數，例如，藉由使用熟悉的 `%VARIABLE_NAME%` (Windows) 或 `$VARIABLE_NAME` (Linux) 語法。
...
您可以使用 [[新增憑證][rest_add_cert] operation (Batch REST) or [CertificateOperations.CreateCertificate][net_create_cert] ] 方法 (batch .net) 將憑證新增至 Batch 帳戶。 然後，您可以將憑證與新的或現有的集區產生關聯。 當憑證與集區相關聯時，Batch 服務會在集區中的每個節點上安裝憑證。 當節點啟動時，在啟動任何工作之前 (包括開始工作和作業管理員工作)，Batch 服務會安裝適當的憑證。
...
* `stderr`和`stdout`
    ...
    在執行期間，應用程式可能會產生診斷輸出，以便用來排解疑難問題。 如前面的[檔案和目錄](#files-and-directories)一節所述，Batch 服務會將標準輸出和標準錯誤輸出寫入至計算節點上工作目錄中的 `stdout.txt` 和 `stderr.txt` 檔案。 您可以使用 Azure 入口網站或其中一個 Batch SDK 來下載這些檔案。 例如, 您可以使用 Batch .net 程式庫and [CloudTask.GetNodeFile][net_getfile_task]中的[ComputeNode][net_getfile_node] , 來抓取這些檔案和其他檔案以供疑難排解之用。
...
> [!IMPORTANT]
> 若要透過 RDP 或 SSH 連接到節點，您必須先在節點上建立使用者。 若要這樣做, 您可以使用 Azure 入口網站、在 batch .net 中[將使用者帳戶新增至節點][rest_create_user] by using the Batch REST API, call the [ComputeNode.CreateComputeNodeUser][net_create_user]方法, 或在 batch Python 模組中呼叫[add_user][py_add_user]方法。
>
>
