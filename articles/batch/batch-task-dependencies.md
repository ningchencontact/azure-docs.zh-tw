...
若要在 Batch 應用程式中使用工作相依性，您必須先將作業設定成使用工作相依性。 在 Batch .NET 中, 將其[UsesTaskDependencies][net_usestaskdependencies]屬性設定為, 以`true`在您的[CloudJob][net_cloudjob]上啟用它:
...
若要建立相依於一或多個父系工作完成的工作，您可以指定此工作「相依於」其他工作。 在 Batch .NET 中, 設定[CloudTask][net_cloudtask]。具有[對 taskdependencies][net_taskdependencies]類別之實例的[DependsOn][net_dependson]屬性:
...
> [!NOTE]
> 根據預設，當工作處於**已完成**狀態且其**結束代碼**是 `0` 時，才會將其視為已順利完成。 在 Batch .NET 中, 這表示[CloudTask][net_cloudtask]。的[狀態][net_taskstate]屬性值`Completed`和 CloudTask 的[TaskExecutionInformation][net_taskexecutioninformation]。[ExitCode][net_exitcode]屬性值為`0`。 如需變更方式的相關資訊，請參閱[相依性動作](#dependency-actions)一節。
> 
> 
...
在一對一關聯性中，工作相依於一項父系工作的成功完成。 若要建立相依性, 請提供單一工作識別碼給[對 taskdependencies][net_taskdependencies]。當您填入[CloudTask][net_cloudtask]的[DependsOn][net_dependson]屬性時, [OnId][net_onid]靜態方法。
...
在一對多關聯性中，工作相依於多項父工作的完成。 若要建立相依性, 請將工作識別碼集合提供給[對 taskdependencies][net_taskdependencies]。當您填入[CloudTask][net_cloudtask]的[DependsOn][net_dependson]屬性時, [OnIds][net_onids]靜態方法。
...
若要建立相依性, 請將範圍中的第一個和最後一個工作識別碼提供給[對 taskdependencies][net_taskdependencies]。當您填入[CloudTask][net_cloudtask]的[DependsOn][net_dependson]屬性時, [OnIdRange][net_onidrange]靜態方法。
...
[對 taskdependencies][github_taskdependencies]範例專案是 GitHub 上的其中一個[Azure Batch 程式碼範例][github_samples]。 此 Visual Studio 解決方案示範︰
