...
在本文中，您會了解如何使用 Azure 入口網站上傳和管理應用程式套件。 然後，您會了解如何使用 [Batch .NET][api_net] 程式庫將套件安裝在集區的計算節點上。
...
如您所見，欄位符合**新的應用程式** 視窗中，但**APPLICATION-ID**方塊已停用。 依照和新增應用程式相同的方式，指定新套件的 [版本]  ，瀏覽至您的**應用程式套件** .zip 檔案，然後按一下 [確定]  以上傳套件。
