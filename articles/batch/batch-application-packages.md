...
>
> 用於建立和管理應用程式套件的 api 是[Batch Management .net][api_net_mgmt] library. The APIs for installing application packages on a compute node are part of the [Batch .NET][api_net]程式庫的一部分。 其他語言的類似功能在其可用 Batch API 中。 
>
...
如果您想要在計算節點上查看應用程式套件的檔案結構, 請在入口網站中流覽至您的 Batch 帳戶。 從您的 Batch 帳戶, 流覽  至 [集區]。 選取包含您感興趣之計算節點的集區。
...
在 Batch .net 中, 當您建立新的集區或針對現有的集區時, 請指定一個或多個[CloudPool][net_cloudpool] .[ApplicationPackageReferences][net_cloudpool_pkgref] 。 [ApplicationPackageReference][net_pkgref] 類別能指定要安裝在集區之計算節點上的應用程式識別碼和版本。
...
在此範例中, 現有集區的*blender*應用程式2.7 版已設定為它的其中一個[CloudPool][net_cloudpool] .[ApplicationPackageReferences][net_cloudpool_pkgref]。 若要將集區的節點更新為 2.76b 版，請將 [ApplicationPackageReference][net_pkgref] 指定為新版本並認可變更。
...
您可以使用[ApplicationOperations][net_appops] .[ListApplicationSummaries][net_appops_listappsummaries]方法, 在 Batch 帳戶中列出應用程式及其套件。
...
* 中的[批次 REST API][api_rest] also provides support to work with application packages. For example, see the [applicationPackageReferences][rest_add_pool_with_packages]元素會[將集區新增至帳戶][rest_add_pool] , 以取得如何使用 REST API 指定要安裝之封裝的相關資訊。 如需如何使用批次 REST API 取得應用程式資訊的詳細資訊, 請參閱[應用程式][rest_applications]。
