...
ms.locfileid: "65826398"
---
# <a name="what-is-azure-time-series-insights"></a>什麼是 Azure 時間序列深入解析？

Azure 時間序列深入解析是針對儲存、視覺化及查詢大量時間序列資料 (例如 IoT 裝置所產生的時間序列資料) 而建置。  如果您想要儲存、管理、查詢或以視覺化方式呈現雲端的時間序列資料，時間序列深入解析可能很適合您。  
...
如果您要建置應用程式，以供內部取用或供外部客戶使用，時間序列深入解析可作為編製索引、儲存及彙總時間序列資料的後端。 您可以使用[用戶端 SDK](tutorial-explore-js-client-lib.md) 打造頂級的自訂視覺效果和使用者體驗。 時間序列深入解析也配有數個用來啟用這些自訂案例的[查詢 API](how-to-shape-query-json.md)。  
