...
ms.locfileid: "65406318"
---
# <a name="what-is-azure-time-series-insights"></a>什麼是 Azure 時間序列深入解析？

Azure 時間序列深入解析 (TSI) 是針對儲存、視覺化及查詢大量時間序列資料 (例如 IoT 裝置所產生的時間序列資料) 而建置。  如果您想要儲存、管理、查詢或以視覺化方式呈現雲端的時間序列資料，時間序列深入解析可能很適合您。  

![時間序列深入解析流程圖](media/overview/time-series-insights-flowchart.png)

時間序列深入解析有四個主要工作：

1. 其能夠和 Azure IoT 中樞和 Azure 事件中樞這類雲端閘道完全整合。 其可輕鬆地連線到這些事件來源，若訊息和結構在無錯誤的資料列和資料行中有資料，即剖析 JSON。 它會聯結中繼資料與遙測資料，而且將單欄式存放區中的資料編製索引。
1. TSI 會管理資料的儲存體。 為了確保隨時能夠輕鬆存取資料，它會將資料儲存在記憶體和 SSD 中長達 400 天。 您可以視需求在幾秒內以互動方式查詢數十億個事件。
1. TSI 會透過 TSI 總管提供立即可用的視覺效果。  
1. TSI 可在 TSI 總管中提供查詢服務，也可以用 API 查詢，API 方便整合，可將時間序列資料嵌入到自訂應用程式中。  

如果您要建置應用程式，以供內部取用或供外部客戶使用，TSI 可作為編製索引、儲存及彙總時間序列資料的後端。 您可以使用[用戶端 SDK](tutorial-explore-js-client-lib.md) 打造頂級的自訂視覺效果和使用者體驗。 TSI 也配有數個用來啟用這些自訂案例的[查詢 API](how-to-shape-query-json.md)。  

時間序列資料代表資產或處理程序如何隨著時間改變。 因此，時間序列資料會根據時間戳記編製索引，而且時間是最具意義的軸，這類資料會隨著時間軸進行組織。 時間序列資料通常會依照順序送達，而且結果通常會被視為插入資料庫，而不是更新資料庫。

在大型磁碟區中，儲存、編製索引、查詢、分析並以視覺化方式呈現時間序列資料可能具有挑戰性。
但是，Azure 時間序列深入解析會擷取每個新事件並儲存為一個資料列，而且會隨著時間有效地測量變更，可讓您回顧過去並取用深入解析及預測未來變更。
...
- 深入了解[規劃您時間序列深入解析](time-series-insights-environment-planning.md)的環境。
