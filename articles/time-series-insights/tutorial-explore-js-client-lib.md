...
為了協助 Web 開發人員查詢時間序列深入解析 (TSI) 中儲存的資料並將其視覺化，我們開發出以 JavaScript D3 為基礎的 TSI 用戶端程式庫。 本教學課程將引導您使用託管的範例應用程式來探索 TSI 用戶端程式庫及程式設計模型。

本教學課程會詳細說明如何使用程式庫、如何存取 TSI 資料，以及如何使用圖表控制項來轉譯並視覺化資料。 您也將了解如何嘗試以不同種類的圖表來將資料視覺化。 在本教學課程結束時，您將能夠使用用戶端程式庫將 TSI 功能納入自己的 Web 應用程式。
...
> * 閱讀 [TSI 用戶端參考文件](https://github.com/microsoft/tsiclient/blob/master/docs/API.md)。
...
本教學課程使用您瀏覽器的**開發人員工具**功能。 新式網頁瀏覽器 ([Microsoft Edge](/microsoft-edge/devtools-guide)、[Chrome](https://developers.google.com/web/tools/chrome-devtools/)、[FireFox](https://developer.mozilla.org/en-US/docs/Learn/Common_questions/What_are_browser_developer_tools) 和 [Safari](https://developer.apple.com/safari/tools/) 等等) 通常都可透過 `F12` 快速鍵來存取 **Web 檢查工具檢視**。 或是，您可以在網頁上按一下滑鼠右鍵並選取**檢查元素**，來存取此檢視。

## <a name="time-series-insights-sample-application"></a>時間序列深入解析範例應用程式

本教學課程通篇使用免費的託管時間序列深入解析應用程式範例，來瀏覽應用程式背後的原始程式碼及 TSI JavaScript 用戶端程式庫。 您將藉此學習如何以 JavaScript 與 TSI 互動，以及如何透過圖表和圖形來將資料視覺化。

1. 瀏覽至[時間序列深入解析應用程式範例](https://insights.timeseries.azure.com/clientsample)。 下列登入提示會顯示：

   [![TSI 用戶端範例登入提示](media/tutorial-explore-js-client-lib/tcs-sign-in.png)](media/tutorial-explore-js-client-lib/tcs-sign-in.png#lightbox)

1. 選取 [登入]，以輸入或選取您的認證。 請使用企業組織帳戶 (Azure Active Directory) 或個人帳戶 (Microsoft 帳戶或 MSA)。

   [![TSI 用戶端範例認證提示](media/tutorial-explore-js-client-lib/tcs-sign-in-enter-account.png)](media/tutorial-explore-js-client-lib/tcs-sign-in-enter-account.png#lightbox)

1. 登入之後，您會看到許多以 TSI 資料填寫的圖表類型。 您的使用者帳戶和**登出**選項會顯示在右上角：

   [![登入之後的 TSI 用戶端範例主頁面](media/tutorial-explore-js-client-lib/tcs-main-after-signin.png)](media/tutorial-explore-js-client-lib/tcs-main-after-signin.png#lightbox)
...
1. 展開 `<head>` 和 `<body>` 元素，並觀察下列各區段：
   ...
   * 在 `<head>` 元素底下，您可以找到可讓應用程式執行下列項目的頁面中繼資料和相依性：
     ...
     * 用來參照 TSI 用戶端 JavaScript 程式庫 **tsiclient.js** 的 `<script>` 元素。 頁面會使用此程式庫來呼叫 TSI 服務 API 並轉譯頁面上的圖表控制項。
     ...
     > * TSI 用戶端 JavaScript 程式庫的原始程式碼可從 [tsiclient 存放庫](https://github.com/Microsoft/tsiclient/tree/tutorial/pages/tutorial)取得。
...
1. 展開 `<div class="chartsWrapper">` 元素，您會發現更多子 `<div>` 元素。 這些元素會用來定位每個圖表控制項範例。 請注意其中有好幾對 `<div>` 元素，每個圖表範例各會使用一對：
...
1. 現在，展開位於 `<div class="chartsWrapper">` 元素正下方的 `<script type="text/javascript">` 元素。 請留意葉面層級 JavaScript 區段的開頭處，這部分可用來處理所有頁面邏輯 (驗證、呼叫 TSI 服務 API、轉譯圖表控制項等等)：
...
TSI 用戶端程式庫 (**tsclient.js**) 提供兩個重要 JavaScript 功能的抽象概念：

* **用來呼叫 TSI 查詢 API 的包裝函式方法**：可讓您使用彙總運算式來查詢 TSI 資料的 REST API。 這些方法會組織在程式庫的 `TsiClient.Server` 命名空間下。

* **用於建立和填入數種圖表控制項類型的方法**：用來在網頁中轉譯 TSI 彙總資料的方法。 這些方法會組織在程式庫的 `TsiClient.UX` 命名空間下。

透過這些簡化方式，開發人員可以更輕易地建置由 TSI 資料支援的 UI 圖形和圖表元件。

### <a name="authentication"></a>Authentication

[時間序列深入解析應用程式範例](https://insights.timeseries.azure.com/clientsample)是具有 ADAL OAuth 2.0 使用者驗證支援的單頁應用程式：

1. 使用 ADAL 進行驗證時，用戶端應用程式必須在 Azure Active Directory 中註冊。 事實上，註冊單頁應用程式是為了使用 [OAuth 2.0 隱含授與流程](https://docs.microsoft.com/azure/active-directory/develop/v1-oauth2-implicit-grant-flow)。
1. 因此，應用程式必須在執行階段指定一些註冊屬性。 其中包括用戶端 GUID (`clientId`) 和重新導向 URI (`postLogoutRedirectUri`)。
1. 之後，應用程式會從 Azure Active Directory 要求**存取權杖**。 系統會對特定服務/API 識別碼發出一組有限權限的存取權杖 (`https://api.timeseries.azure.com`)。 這些權杖權限是由系統代表所登入的使用者來發出。 服務/API 的識別碼是應用程式 Azure Active Directory 註冊中所包含的另一個屬性。
1. ADAL 將存取權杖傳回給應用程式後，系統在存取 TSI 服務 API 時就會以**持有人權杖**的形式來傳遞存取權杖。

   [!code-javascript[head-sample](~/samples-javascript/pages/tutorial/index.html?range=147-204&highlight=3-7,34-37)]

> [!TIP]
> 若要深入了解由 Microsoft 支援的 ADAL 程式庫，請參閱 [ADAL 參考文件](https://docs.microsoft.com/azure/active-directory/develop/active-directory-authentication-libraries#microsoft-supported-client-libraries)。

### <a name="control-identification"></a>控制項識別

在提供的範例中，`<div>` 元素會在父代 `<body>` 元素中加以排列，以針對頁面上所有轉譯的圖表控制項提供合適的版面配置。

每個 `<div>` 元素各自都會針對圖表控制項的位置和視覺效果屬性指定相關屬性。 HTML 元素的 `id` 屬性會作為繫結至特定控制項的唯一識別碼，以便進行轉譯及更新視覺化的資料。

### <a name="aggregate-expressions"></a>彙總運算式

TSI 用戶端程式庫 API 會使用彙總運算式：

* 彙總運算式可讓您建構一或多個**搜尋字詞**。

* 這些用戶端 API 可提供類似於其他示範應用程式 ([時間序列深入解析總管](https://insights.timeseries.azure.com/demo)) 的功能，其使用搜尋範圍、WHERE 述詞、量值和 split-by 值。

* 大多數的用戶端程式庫 API 採用彙總運算式陣列，以供服務用來建置 TSI 資料查詢。

### <a name="call-pattern"></a>呼叫模式

圖表控制項的填入和轉譯會遵循一般模式。 這種一般模式出現在整個應用程式範例中，並可在您使用用戶端程式庫時提供協助：
...
1. 使用 `TsiClient.Server` API 呼叫 TSI 查詢以要求彙總資料：
...
1. 將 TSI 查詢所傳回的壓縮結果轉換為視覺效果的 JSON。

   ```javascript
   var transformedResult = tsiClient.ux.transformAggregatesForVisualization(result, aes);
   ```

1. 使用 `TsiClient.UX` API 建立圖表控制項，並將它繫結至頁面上的其中一個 `<div>` 元素。
...
請看一下用來轉譯一些標準圖表控制項的示範程式碼。 並注意用來建立這些控制項的程式設計模型和模式。 具體來說，請檢查 `// Example 3/4/5` 註解底下的 HTML 區段，其轉譯了具有 HTML `id` 值 `chart3`、`chart4` 和 `chart5` 的控制項。

回想一下[網頁原始碼和結構一節](#page-source-and-structure)中的**步驟 3**，圖表控制項會在網頁上成列排列，每個控制項各有一個敘述性標題資料列。 在此範例中，所填入的三個圖表皆位於標題為 `"Multiple Chart Types From the Same Data"` 的 `<div>` 元素底下，並且繫結至該標題下的三個 `<div>` 元素：
...
TSI 用戶端程式庫有數個額外功能，可讓您用來實作具創意性的資料視覺效果。

### <a name="states-and-events"></a>狀態和事件

其中一個進階功能範例是能夠將狀態轉換和離散事件新增至圖表。 這項功能可用於醒目提示事件、警示和建立狀態切換 (例如開啟/關閉)。
...
2. 接下來，為 `Incidents` 定義 `events5` 結構，其保有要追蹤的事件元素陣列。陣列結構的圖形與針對 `events4` 所概述的結構相同。
...
請看一下 `// Example 13/14/15` 註解周圍的程式碼。 此程式碼一開始會轉譯 `"Line Chart with Context Menu to Create Pie/Bar Chart"` 標題底下的折線圖，而此圖會繫結至 HTML `id` 值為 `chart13` 的 `<div>` 元素。

使用快顯功能表，折線圖就可讓您動態建立圓形圖和橫條圖，且繫結至識別碼為 `chart14` 和 `chart15` 的 `<div>` 元素。 此外，圓形圖和橫條圖也都會使用快顯功能表來啟用自己的功能：分別能夠將圓形圖的資料複製到橫條圖，以及將橫條圖的資料列印至瀏覽器的主控台視窗。
...
[![有快顯功能表可供使用筆刷建立圓形圖/橫條圖的折線圖](media/tutorial-explore-js-client-lib/tcs-line-chart-with-context-menu-to-create-pie-bar-chart-brushes.png)](media/tutorial-explore-js-client-lib/tcs-line-chart-with-context-menu-to-create-pie-bar-chart-brushes.png#lightbox)
...
> [!div class="nextstepaction"]
> [TSI API 參考文件](https://github.com/microsoft/tsiclient/blob/master/docs/API.md)
