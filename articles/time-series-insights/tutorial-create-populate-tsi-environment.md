...
TSI 環境是收集及儲存裝置資料之處。 儲存在 TSI 環境中之後，您可以使用 [TSI 總管](time-series-quickstart.md)和 [TSI 查詢 API](/rest/api/time-series-insights/ga-query-api) 來查詢和分析資料。 IoT 中樞是所有裝置 (模擬或實體) 用來安全地連線至 Azure 雲端並傳輸資料的連線點。 [TSI 概觀](time-series-insights-overview.md)提到，Azure IoT 中樞也可作為將資料串流至 TSI 環境中的事件來源。 本教學課程會使用 [IoT 解決方案加速器](/azure/iot-accelerators/)產生範例遙測資料並串流至 IoT 中樞。
...
1. 在 [時間序列深入解析環境] 頁面上，填入必要參數：
   ...
   **訂用帳戶** | 訂用帳戶是 Azure 資源的容器。 請選擇要建立 TSI 環境的訂用帳戶。
...
### <a name="tsi-configuration-define-an-event-source"></a>TSI 組態：定義事件來源
...
1. 在 TSI 環境頁面上選取 [事件來源]，然後按一下 [+ 新增]。

   [![TSI 環境概觀](media/tutorial-create-populate-tsi-environment/ap-add-env-event-source-add.png)](media/tutorial-create-populate-tsi-environment/ap-add-env-event-source-add.png#lightbox)
...
1. 在 [新增事件來源] 頁面上輸入必要參數。
   ...
   **時間戳記屬性名稱** | 指定為 `timestamp`。
   ...
   [![TSI 環境的新事件來源](media/tutorial-create-populate-tsi-environment/ap-add-env-event-source-add-event-source.png)](media/tutorial-create-populate-tsi-environment/ap-add-env-event-source-add-event-source.png#lightbox)

## <a name="run-device-simulation-to-stream-data-into-tsi"></a>執行裝置模擬以將資料串流至 TSI 中
...
1. 返回 TSI 環境的資源群組 [概觀] 頁面。 選取 TSI 環境。
...
1. 在 TSI 環境的 [概觀] 頁面上，按一下 [時間序列深入解析總管 URL]，以開啟 TSI 總管。
...
1. TSI 總管會使用您的 Azure 入口網站帳戶進行載入和驗證。 在初始檢視中，您可以看到 TSI 環境中確實已填入模擬遙測資料的圖表區域。 若要篩選為較小的時間範圍，請選取左上方的下拉式清單。 接著，輸入足以跨越裝置模擬持續時間的時間範圍。 然後，按一下搜尋放大鏡。
...
1. 縮小時間範圍，可讓圖表放大顯示傳輸至 IoT 中樞與 TSI 環境的資料明顯暴增的區域。 同時也請留意右上方的**串流處理完成**文字，此處會顯示找到的事件總數。 您也可以拖曳 [間隔大小] 滑桿，來控制圖表的繪圖細微性。
