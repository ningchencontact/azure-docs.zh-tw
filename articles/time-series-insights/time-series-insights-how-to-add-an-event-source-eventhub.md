...
> [!NOTE]
> 這篇文章中所述的步驟適用於包括時間序列深入解析 GA，以及時間序列深入解析預覽環境。
...
應用程式會使用取用者群組從 Azure 事件中樞提取資料。 若要可靠地從事件中樞讀取資料，提供專用取用者群組，僅供此時間序列深入解析環境。
...
1. 為 [匯入選項]  選取適當的值：
   - 若您的其中一個訂用帳戶中有現有的事件中樞，請選取 [來自可用訂閱的使用事件中樞]  。 此選項是最簡單的方法。
     ...
     | 事件序列化格式 | 目前，JSON 是目前唯一可用的序列化格式。 此格式必須是事件訊息，或無法讀取資料。 |
