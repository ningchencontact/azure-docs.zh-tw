...
Application Insights 快照集偵錯工具可讓您從即時 Web 應用程式中自動收集偵錯快照集。 快照集會顯示擲回例外狀況時原始程式碼和變數的狀態。 本文說明偵錯工具的運作方式，並提供常見問題的解決方案。

## <a name="how-does-application-insights-snapshot-debugger-work"></a>Application Insights 快照集偵錯工具如何運作

Application Insights 快照集偵錯工具是 Application Insights 遙測管線的一部分 (ITelemetryProcessor 的執行個體)。 快照集收集器會監視您的程式碼中擲回的例外狀況 (AppDomain.FirstChanceException)，以及透過 `TelemetryClient.TrackException` 回報給 Application Insights 的例外狀況遙測資料。 一旦您成功將快照集收集器套件新增至專案中，且它偵測到遙測管線中有一個事件，便會在自訂資料中傳送名為 'AppInsightsSnapshotCollectorLogs' 和 'SnapshotCollectorEnabled' 的自訂事件。 同時，它也會啟動名為 'SnapshotUploader.exe' (1.2.0 版或更新版本) 或 'MinidumpUploader.exe' (1.2.0 之前的版本) 的程序，將收集到的快照集料檔案上傳至 Application Insights。  上傳程式程序啟動時，將會傳送名為 'UploaderStart' 的自訂事件。 其後，快照集收集器便會開始其正常的監視行為。

快照集收集器監視 Application Insights 例外狀況遙測資料時，會使用設定中定義的參數 (例如ThresholdForSnapshotting、MaximumSnapshotsRequired、MaximumCollectionPlanSize、ProblemCounterResetInterval) 來判斷何時要收集快照。 符合所有規則時，收集器就會要求下一個擲回相同位置的例外狀況快照。 同時，將會傳送自訂資料中名為「AppInsightsSnapshotCollectorLogs」和「RequestSnapshots」的自訂事件。 由於編譯器會將「版本」程式碼最佳化，因此在收集的快照集中可能無法顯示區域變數。 要求快照集時，快照集收集器會嘗試針對擲回例外狀況的方法取消最佳化。 在此期間，將會傳送在自訂資料中名為「AppInsightsSnapshotCollectorLogs」和「ProductionBreakpointsDeOptimizeMethod」的 Application Insights 自訂事件。  收集了下一個例外狀況的快照集後，將可使用區域變數。 收集了快照集後，它會將程式碼重新最佳化。 
...
如果[快照集收集器 NuGet 套件](https://www.nuget.org/packages/Microsoft.ApplicationInsights.SnapshotCollector)是 1.1.0 版或更新版本，則會將快照集收集器記錄傳送至您的 Application Insight 帳戶。 請確定 *ProvideAnonymousTelemetry* 未設定為 False (此值預設為 True)。
...
如果您可以在「如何找到快照集收集器記錄」步驟中找到記錄，代表快照集收集器已正確新增至您的專案中，因此您可以忽略此步驟。
...
在快照集收集器記錄檔中搜尋 *RequestSnapshots* (在搜尋方塊中輸入 ```RequestSnapshots```)。  如果找不到此項目，請檢查您的組態。 例如，*ThresholdForSnapshotting* 表示開始收集快照集之前可發生特定例外狀況的數目上限。

### <a name="step-5-make-sure-that-snapshot-is-not-disabled-due-to-memory-protection"></a>步驟 5：確定快照集未因記憶體保護而停用
為了確保您的應用程式效能，只有記憶體緩衝區夠大時，才會擷取快照集。 在快照集收集器記錄檔中搜尋「CannotSnapshotDueToMemoryUsage」。 在事件的自訂資料中，會有詳細的原因。 若您的應用程式是在 Azure Web 應用程式中執行，可能會受到嚴格限制。 因為符合某些記憶體規則時，Azure Web 應用程式會重新啟動您的應用程式。 您可以嘗試將服務方案相應增加為具有更多記憶體的機器，來解決此問題。

### <a name="step-6-make-sure-snapshots-were-captured"></a>步驟 6：確定已擷取快照集
在快照集收集器記錄檔中搜尋 ```RequestSnapshots```。  如果找不到此項目，請檢查您的組態。 例如，*ThresholdForSnapshotting* 表示收集快照集之前的特定例外狀況數目。

### <a name="step-7-make-sure-snapshots-are-uploaded-correctly"></a>步驟 7：確定已正確上傳快照集
在快照集收集器記錄檔中搜尋 ```UploadSnapshotFinish```。  若找不到此項目，請傳送含有 Application Insights 檢測金鑰的電子郵件給 snapshothelp@microsoft.com。 若此事件存在，請開啟其中一個記錄檔，並複製自訂資料中的「SnapshotId」值。 然後搜尋該值。 這會找到與快照集對應的例外狀況。 然後按一下例外狀況並開啟偵錯快照集。 (如果沒有對應的例外狀況，可能是因為高容量而取樣了例外狀況遙測資料。 請嘗試另一個 snapshotId。)
...
如果您的情況有所不同，則可能是 Bug。 請傳送含有 Application Insights 檢測金鑰的電子郵件給 snapshothelp@microsoft.com，並附上程式碼片段。
