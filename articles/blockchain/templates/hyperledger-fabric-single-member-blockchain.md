...
若要開始，請選取 [Hyperledger Fabric 單一成員區塊鏈]，然後按一下 [建立] 以開啟精靈中的 [基本概念] 刀鋒視窗。

[範本部署] 會引導您設定多節點網路。 部署流程分成三個步驟：基本資料、網路設定和 Fabric 設定。

### <a name="basics"></a>基本概念

在 [基本概念] 刀鋒視窗中，為任何部署指定標準參數的值。 例如，訂用帳戶、資源群組及基本虛擬機器屬性。
...
**密碼 (驗證類型 = 密碼)**|每個所部署虛擬機器的管理員帳戶密碼。 密碼必須包含下列其中三個字元類型：1 個大寫字元、1 個小寫字元、1 個數字與 1 個特殊字元。<br /><br />所有 VM 一開始都有相同的密碼，但您可以在佈建之後變更密碼。|12 - 72 個字元|NA
...
接下來，在 [網路大小和效能] 中指定聯盟網路大小的輸入。 例如，成員資格數目、排序者和對等節點。 選擇基礎結構選項和虛擬機器大小。
