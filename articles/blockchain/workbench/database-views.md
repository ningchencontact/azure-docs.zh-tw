...
| UploadedByUserExternalId         | nvarchar(255) | 否          | 應用程式上傳者的使用者外部識別碼。 By default, this ID is the user from the Azure Active Directory for the consortium.                                                                                                |
...
| UserExternalId             | nvarchar(255) | 否          | 與角色相關聯之使用者的外部識別碼。 By default, this ID is the user from the Azure Active Directory for the consortium.                                                                     |
...
| UserExternalId           | nvarchar(255) | 否          | 與連線相關聯之使用者的外部識別碼。 By default, this ID is the user from the Azure Active Directory for the consortium. |
| UserProvisioningStatus   | int           | 否          |識別使用者佈建程序的目前狀態。 可能的值包括： <br />0 – API 已建立使用者<br />1 – 已在資料庫中建立與使用者相關聯的金鑰<br />2 – 已完整佈建使用者 |
...
| ContractDeployedByUserId                 | int            | 否          | 合約部署者的使用者外部識別碼。 By default, this ID is the guid representing the Azure Active Directory ID for the user.                                                                                                          |
| ContractDeployedByUserExternalId         | nvarchar(255)  | 否          | 合約部署者的使用者外部識別碼。 By default, this ID is the guid representing the Azure Active Directory ID for the user.                                                                                                         |
...
| ContractDeployedByUserFirstName          | nvarchar(50)  | 是         | 合約部署者的使用者名字 |
| ContractDeployedByUserLastName           | nvarchar(255) | 是         | 合約部署者的使用者姓氏 |
| ContractDeployedByUserExternalId         | nvarchar(255) | 否          | 合約部署者的使用者外部識別碼。 By default, this ID is the guid that represents their identity in the consortium Azure Active Directory.                                                                                                                                                |
| ContractDeployedByUserEmailAddress       | nvarchar(255) | 是         | 合約部署者的使用者電子郵件地址 |
...
| ContractActionExecutedByUserExternalId   | nvarchar(255) | 是         | 合約動作執行者的使用者外部識別碼 By default, this ID is the guid that represents their identity in the consortium Azure Active Directory. |
...
| StateName                          | nvarchar(50)  | 是         | If this property contains the state of the contract, it is the display name for the state. 如果與狀態沒有關聯，則值將為 Null。 |
| StateDisplayName                   | nvarchar(255) | 否          | If this property contains the state, it is the display name for the state. 如果與狀態沒有關聯，則值將為 Null。 |
| StateValue                         | nvarchar(255) | 是         | If this property contains the state, it is the state value. 如果與狀態沒有關聯，則值將為 Null。 |
...
| StateName                          | nvarchar(50)  | 是         | If this property contains the state, it the display name for the state. 如果與狀態沒有關聯，則值將為 Null。 |
| StateDisplayName                   | nvarchar(255) | 否          | If this property contains the state, it is the display name for the state. 如果與狀態沒有關聯，則值將為 Null。 |
| StateValue                         | nvarchar(255) | 是         | If this property contains the state, it is the state value. 如果與狀態沒有關聯，則值將為 Null。 |
...
|--------------------|---------------|-------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ID                 | int           | 否          | 使用者的唯一識別碼 |
| ExternalID         | nvarchar(255) | 否          | 使用者的外部識別碼。 By default, this ID is the guid representing the Azure Active Directory ID for the user. |
...
此檢視會詳細顯示核心工作流程中繼資料以及工作流程的函式和參數。 Designed for reporting, it also contains metadata about the application associated with the workflow. 此檢視包含來自多個基礎資料表的資料，以利報告工作流程的資料。 對於每個工作流程，此檢視會包含下列資料：
...
| WorkflowStartStateValue           | int           | 否          | The value of the state |
| WorkflowStartStatePercentComplete | int           | 否          | 一個文字說明，可提示用戶端如何在 UI 中轉譯此狀態。 支援的狀態包括成功和失敗 |

## <a name="vwworkflowfunction"></a>vwWorkflowFunction

此檢視會詳細顯示核心工作流程中繼資料以及工作流程的函式和參數。 Designed for reporting, it also contains metadata about the application associated with the workflow. 此檢視包含來自多個基礎資料表的資料，以利報告工作流程的資料。 對於每個工作流程函式，此檢視會包含下列資料：
...
| WorkflowFunctionIsConstructor        | bit           | 否          | Identifies if the workflow function is the constructor for the workflow |
| WorkflowFunctionParameterId          | int           | 否          | 函式參數的唯一識別碼 |
| WorkflowFunctionParameterName        | nvarchar(50)  | 否          | 函式參數的名稱 |
| WorkflowFunctionParameterDisplayName | nvarchar(255) | 否          | 要在使用者介面中顯示的函式參數名稱 |
| WorkflowFunctionParameterDataTypeId  | int           | 否          | 與工作流程函式參數相關聯之資料類型的唯一識別碼 |
...
| WorkflowStateValue           | nvarchar(50)  | 否          | Value of the state |
