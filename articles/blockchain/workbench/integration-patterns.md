...
此外，在某些情況下 (例如，與供應鏈或媒體有關的情況)，也可能需要文件的整合。 雖然 Azure Blockchain Workbench 未提供可直接處理文件的 API 呼叫，但文件可以納入 Azure Blockchain 應用程式中。 此節也會說明該模式。

此節將說明經認可能夠在您的端對端解決方案中實作各類整合的模式。
...
此節探討的模式將著重在會將交易傳送至分散式總帳，以及會從 Azure Blockchain Workbench 的*鏈結關閉* SQL 資料庫查詢交易相關資料的 REST API 層面。
...
此節探討的模式將著重在會將交易傳送至分散式總帳，以及會顯示基礎分散式總帳所公開之事件訊息的訊息型 API 層面。
...
例如，如果運送藥品的貨車溫度飆升至 110 度，則可能會影響到藥品的藥效，且若未偵測到此問題而從供應鏈中移除，可能會造成公共安全問題。 如果某駕駛加速至時速 100 英哩，產生的感應器資訊可能致使保險業者取消其保險。 對於租用的車輛，GPS 資料可以在駕駛的行車路徑超出其租用合約所涵蓋的地理位置時予以指出，而收取罰金。
...
## <a name="prerequisites-for-implementing-integration-patterns-using-the-rest-and-message-apis"></a>使用 REST 和訊息 API 實作整合模式的先決條件
