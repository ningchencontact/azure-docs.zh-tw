...
ms.locfileid: "56234912"
---
# <a name="how-to-troubleshoot-azure-cache-for-redis"></a>如何針對 Azure Redis 快取問題進行疑難排解
本文提供下列類別的「Azure Redis 快取」問題疑難排解指引。

* [用戶端疑難排解](#client-side-troubleshooting) - 本節針對連線到「Azure Redis 快取」之應用程式所造成的問題，提供識別及解決問題的指引。
* [伺服器端疑難排解](#server-side-troubleshooting) - 本節針對在「Azure Redis 快取」伺服器端上造成的問題，提供識別及解決問題的指引。
...
2. 增加用戶端和「Azure Redis 快取伺服器」的 VM 大小以取得更高的頻寬功能，藉此縮短較大回應的資料傳輸時間。 只在伺服器或用戶端上取得更多頻寬可能並不夠。 請測量頻寬使用量，並與您目前擁有的 VM 大小的功能做比較。
...
#### <a name="resolution"></a>解決方案
* 檢閱 [Azure Redis 快取建議程式](cache-configure.md#azure-cache-for-redis-advisor)中所提及的任何建議和警示。
...
* 檢閱 [Azure Redis 快取效能](cache-faq.md#azure-cache-for-redis-performance)圖表，查看您是否可能接近目前層級的上限閾值。 如有必要，[調整](cache-how-to-scale.md)為具有更多 CPU 容量的較大快取層。 如果您已使用進階層，建議您[利用叢集來相應放大](cache-how-to-premium-clustering.md)
...
1. 確定「Azure Redis 快取」和用戶端應用程式位在 Azure 中的相同區域。 例如，當快取位在美國東部，但用戶端位在美國西部，而要求未在 `synctimeout` 間隔內完成時，您可能會收到逾時，或者當您從本機開發電腦進行偵錯時，您可能會收到逾時。 
   
    強烈建議您讓快取和用戶端位在相同的 Azure 區域中。 如果您有包含跨區域呼叫的案例，您應該將 `synctimeout` 間隔設定為大於預設值 1000 毫秒間隔的值，方法是在連接字串中加入 `synctimeout` 屬性。 下列範例示範 `synctimeout` 為 2000 毫秒的 StackExchange.Azure Redis 快取連接字串程式碼片段。
