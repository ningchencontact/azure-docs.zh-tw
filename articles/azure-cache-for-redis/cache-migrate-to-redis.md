...
>[!NOTE]
>受控快取服務及 In-Role Cache 已於 2016 年 11 月 30 日起[淘汰](https://azure.microsoft.com/blog/azure-managed-cache-and-in-role-cache-services-to-be-retired-on-11-30-2016/)。 如果您有任何 In-Role Cache 部署想要移轉至 Azure Cache for Redis，您可以依照本文中的步驟操作。

## <a name="migration-steps"></a>移轉步驟
要將受控快取服務應用程式移轉為使用 Azure Cache for Redis，必須執行下列步驟。

* 將受控快取服務功能對應至 Azure Cache for Redis
* 選擇快取供應項目
* 建立快取
* 設定快取用戶端
  * 移除受控快取服務設定
...
* 移轉受控快取服務程式碼
  ...
  * 存取快取中的基本資料型別
  * 使用快取中的 .NET 物件
* 將 ASP.NET 工作階段狀態和輸出快取移轉至 Azure Cache for Redis 

## <a name="map-managed-cache-service-features-to-azure-cache-for-redis"></a>將受控快取服務功能對應至 Azure Cache for Redis
Azure 受控快取服務與 Azure Cache for Redis 類似，但兩者在實作某些功能時會使用不同的方式。 本節將說明其中的一些差異，並提供有關在 Azure Cache for Redis 中實作受控快取服務功能的指引。
