...
本主題說明如何執行一般工作，例如建立、更新及調整 Azure Cache for Redis 執行個體，如何重新產生存取金鑰，以及如何檢視您的快取的相關資訊。 如需 Azure Cache for Redis 的 PowerShell Cmdlet 完整清單，請參閱 [Azure Cache for Redis Cmdlet](https://docs.microsoft.com/powershell/module/azurerm.rediscache/?view=azurermps-6.6.0)。
...
使用 [New-AzureRmRedisCache](https://docs.microsoft.com/powershell/module/azurerm.rediscache/new-azurermrediscache?view=azurermps-6.6.0) Cmdlet 建立新的 Azure Cache for Redis 執行個體。

> [!IMPORTANT]
> 您第一次使用 Azure 入口網站在訂用帳戶中建立 Azure Cache for Redis 時，入口網站會為該訂用帳戶註冊 `Microsoft.Cache` 命名空間。 如果您嘗試使用 PowerShell 在訂用帳戶中建立第一個 Azure Cache for Redis，您必須先使用下列命令註冊該命名空間；否則 Cmdlet (例如 `New-AzureRmRedisCache` 和 `Get-AzureRmRedisCache`) 會失敗。
> 
...
如需 `databases` 屬性的詳細資訊，請參閱 [預設的 Azure Cache for Redis 伺服器組態](cache-configure.md#default-redis-server-configuration)。 如需使用 [New-AzureRmRedisCache](https://docs.microsoft.com/powershell/module/azurerm.rediscache/new-azurermrediscache?view=azurermps-6.6.0) Cmdlet 建立快取的詳細資訊，請參閱先前的[建立 Azure Cache for Redis](#to-create-a-redis-cache) 一節。

## <a name="to-update-an-azure-cache-for-redis"></a>更新 Azure Cache for Redis
使用 [Set-AzureRmRedisCache](https://docs.microsoft.com/powershell/module/azurerm.rediscache/Set-AzureRmRedisCache?view=azurermps-6.6.0) Cmdlet 更新 Azure Cache for Redis 執行個體。
...
修改 `Size`、`Sku` 或 `ShardCount` 屬性時，可使用 `Set-AzureRmRedisCache` 來調整 Azure Cache for Redis 執行個體。 
...
若要刪除 Azure Cache for Redis，請使用 [Remove-AzureRmRedisCache](https://docs.microsoft.com/powershell/module/azurerm.rediscache/remove-azurermrediscache?view=azurermps-6.6.0) Cmdlet。
...
您可以使用 `Import-AzureRmRedisCache` Cmdlet 將資料匯入 Azure Cache for Redis 執行個體中。
...
您可以使用 `Export-AzureRmRedisCache` Cmdlet 從 Azure Cache for Redis 執行個體匯出資料。
...
您可以使用 `Reset-AzureRmRedisCache` Cmdlet 將 Azure Cache for Redis 執行個體重新啟動。
...
* [MSDN 上的 Azure Cache for Redis Cmdlet 文件](https://docs.microsoft.com/powershell/module/azurerm.rediscache/?view=azurermps-6.6.0)
