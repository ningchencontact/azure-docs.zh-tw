...
* 解決方案：   
請勿在 patch 和 update （put）要求中包含 `BaremetalTenantId`。 或者，請確定要求中的 `BaremetalTenantId` 是相同的。
...
當您嘗試使用已變更的 `PoolId` 屬性來更新或修補容量集區時，就會發生此錯誤。

* 原因：   
您正嘗試更新容量集區 `PoolId` 屬性。 @No__t_0 屬性是唯讀屬性，而且無法變更。
* 解決方案：   
請勿在 patch 和 update （put）要求中包含 `PoolId`。  或者，請確定要求中的 `PoolId` 是相同的。

***無法變更 CreationToken。***

當您嘗試在建立磁片區之後變更檔案路徑（`CreationToken`）時，就會發生此錯誤。 建立磁片區時，必須設定檔案路徑（`CreationToken`），而且稍後無法變更。
...
當其他錯誤處理在資源上執行動作無法處理錯誤時，就會顯示此錯誤。   其中包含文字「錯誤」。 @No__t_0 可以是（`getting`、`creating`、`updating` 或 `deleting`）的任何一個。  @No__t_0 是 `resourceTypeName` （例如 `netAppAccount`、`capacityPool`、`volume` 等等）。
...
當您嘗試變更 `FileSystemId` 時，就會發生此錯誤。  變更 `FileSystemdId` 不是支援的操作。 

* 原因：   
建立磁片區時，會設定檔案系統的識別碼。 `FileSystemId` 之後就無法變更。
* 解決方案：   
請勿在 patch 和 update （put）要求中包含 `FileSystemId`。  或者，請確定 `FileSystemId` 在要求中是相同的。

***識別碼為 ' {string} ' 的 ActiveDirectory 不存在。***

@No__t_0 部分是您在 Active Directory 連接的 [`ActiveDirectoryId`] 屬性中輸入的值。

* 原因：   
當您建立具有 Active Directory 設定的帳戶時，您已輸入應為空白 `ActiveDirectoryId` 的值。
* 解決方案：   
請勿在建立（put）要求中包含 `ActiveDirectoryId`。
...
* 解決方案：   
請務必在命令列上設定所有必要的和 nonconflicting 參數。 例如，您無法同時設定 `UnixReadOnly` 和 `UnixReadWrite` 參數。
* 因應措施：   
請參閱上述解決方案。

***Vlan {2} {1} 的 IP 範圍 {0} 已在使用中***
...
* 解決方案：   
請勿在 patch 和 update （put）要求中包含 `MountTargets`。  或者，請確定要求中的 `MountTargets` 是相同的。
...
***物件： {value} 的專案數： ExportPolicy。規則 [RuleIndex] 超出最大範圍。***
...
* 原因：   
建立磁片區時，會設定 `OwnerId` 屬性。 之後就無法變更屬性。
* 解決方案：   
請勿在 patch 和 update （put）要求中包含 `OwnerId`。 或者，請確定要求中的 `OwnerId` 是相同的。
...
當您更新容量集區大小，且大小小於該容量集區中所有磁片區的 `usedBytes` 值總計時，就會發生此錯誤。  當您建立新的磁片區，或調整現有磁片區的大小時，新的磁片區大小超過容量集區中的可用空間時，也可能會發生此錯誤。
...
當您建立資源時，就會發生此錯誤，而且您會在 name 屬性中填入其他值，而不是 `resourceId` 的 name 屬性。

* 原因：   
當您建立資源時，name 屬性中的值無效。
* 解決方案：   
將 [名稱] 屬性保留空白，或讓它使用與 [名稱] 屬性相同的值（在 `resourceId` 中的最後一個反斜線 "/" 和問號 "？" 之間）。
...
* 原因：   
您嘗試在 [磁片區 `protocolType`] 屬性中設定不正確值。
* 解決方案：   
在 `protocolType` 中設定有效的字串。
* 因應措施：   
將 `protocolType` 設定為 null。

***無法變更通訊協定類型***

當您嘗試更新或修補磁片區的 `ProtocolType` 時，就會發生此錯誤。  變更 ProtocolType 不是支援的操作。

* 原因：   
建立磁片區時，會設定 `ProtocolType` 屬性。  無法更新。
* 解決方案：   
無。
* 因應措施：   
使用新的通訊協定類型建立另一個磁片區。
...
* 原因：   
您嘗試建立資源，但達到配額限制（例如：每個訂用帳戶 `NetAppAccounts` 或每個 `NetAppAccount` `CapacityPools`）。
* 解決方案：   
增加配額限制。
* 因應措施：   
刪除相同類型的未使用資源，然後重新建立。
...
當您嘗試在建立磁片區之後變更 `subnetId` 時，就會發生此錯誤。  建立磁片區時必須設定 `SubnetId`，之後無法變更。

* 原因：   
您嘗試在建立磁片區之後變更 `subnetId`，這不是支援的操作。 
* 解決方案：   
如果不需要變更 `subnetId`，請考慮從要求中移除參數，以關閉錯誤訊息。
* 因應措施：   
如果您需要變更 `subnetId`，您可以使用新的 `subnetId` 建立新的磁片區，然後將資料移轉到新的磁片區。

***SubnetId 的格式無效。***

當您嘗試建立新的磁片區，但 `subnetId` 不是子網的 `resourceId` 時，就會發生此錯誤。

* 原因：   
當您嘗試建立新的磁片區，但 `subnetId` 不是子網的 `resourceId` 時，就會發生此錯誤。 
* 解決方案：   
檢查 `subnetId` 的值，以確定它包含所使用之子網的 `resourceId`。
* 因應措施：   
無。 請參閱上述解決方案。 
...
* 原因：   
您嘗試建立磁片區，而您選取的子網未委派給 `Microsoft.NetApp/volumes`。
* 解決方案：   
選取委派給 `Microsoft.NetApp/volumes` 的另一個子網。
* 因應措施：   
將正確的委派新增至子網。
