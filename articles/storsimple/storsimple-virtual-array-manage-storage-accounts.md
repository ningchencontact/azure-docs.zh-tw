...
4. 在儲存體帳戶認證的 [屬性] 刀鋒視窗中，執行下列動作︰
   ...
   2. 您可以選擇重新產生儲存體帳戶認證的存取金鑰。 如需詳細資訊，請參閱[管理儲存體帳戶存取金鑰](../storage/common/storage-account-keys-manage.md)。 提供新的儲存體帳戶認證金鑰。 而每個 Azure 儲存體帳戶，都有主要存取金鑰。
