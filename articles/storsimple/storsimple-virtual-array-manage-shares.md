...
* **固定在本機**:在這些共用中的資料隨時保持在陣列上，並不會溢出到雲端。
* **分層**:在這些共用中的資料可能流向雲端。 當您建立階層式共用時，大約 10% 的空間會佈建在本機層，而 90% 的空間會佈建在雲端。 舉例來說，如果您佈建 1 TB 的共用，當資料分層時，有 100 GB 會位於本機空間，900 GB 會用於雲端。 這也意味著，如果裝置的可用空間用盡，您就無法佈建階層式共用 (因為本機層上需要的 10% 無法使用)。
