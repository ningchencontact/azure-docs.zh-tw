...
* **改善的雲端中斷恢復功能**:此版本包含災害復原、 備份、 還原及分層發生雲端連線中斷的情況相關的數個 bug 修正。 
* **改善還原效能**:此版本包含 bug 修正，大幅減少還原作業的完成時間。
* **自動空間回收最佳化**:精簡佈建的磁碟區上刪除資料時，未使用的儲存體區塊需要回收。 此版本已經改善雲端空間回收程序，相較於之前的版本，此版本可讓未使用的空間更快變成可用空間。
* **新的虛擬磁碟映像**:新的 VHD、 VHDX 和 VMDK 現在已可透過 Azure 傳統入口網站。 您可以下載這些映像來佈建新的 Update 0.1 裝置。
* **改善的入口網站中的作業狀態精確度**:在舊版的軟體中，作業狀態在入口網站中報告不夠細緻。 本版已經修正這個問題。
* **網域加入體驗**:加入網域並重新命名裝置相關的錯誤修正。
...
| **6.** |階層式磁碟區或共用 |不支援使用 StorSimple 階層式磁碟區之應用程式的位元組範圍鎖定。 如果啟用位元組範圍鎖定，則 StorSimple 分層無法運作。 |建議的方法包括︰ <br></br>關閉應用程式邏輯中的位元組範圍鎖定。<br></br>選擇將這個應用程式的資料放在固定在本機的磁碟區中，而非階層式磁碟區。<br></br>警告  ：如果使用固定在本機的磁碟區，並啟用位元組範圍鎖定，請注意，即使在還原完成之前，固定在本機的磁碟區可以在線上。 在這種情況下，如果正在還原，則必須等待還原完成。 |
