...
| --- | --- | --- | --- | --- |
| 1 |主機效能 |在較早的版本中，我們觀察到在建立本機固定磁碟區期間和將分層磁碟區轉換成本機固定磁碟區期間，主機端會出現效能問題。 此版本中已經修正這些問題，因此已經改善在建立及轉換磁碟區期間的主機效能。 |是 |否 |
| 2 |固定在本機的磁碟區 |在極少數情況下，系統可能會在建立本機固定磁碟區時當機。 此版本已經修正這個錯誤。 |是 |否 |
| 3 |分層 |將 StorSimple Cloud Appliance (8010 與 8020) 的中繼資料分層上傳至雲端時，偶爾會發生當機。 此版本已經修正這個問題。 |否 |是 |
...
| 7 |Windows Management Instrumentation (WMI) |在舊版的軟體中，有幾個 web proxy 失敗，發生例外狀況 」\<ManagementException > 提供者載入失敗 」。 這個錯誤屬於 WMI 記憶體流失錯誤，現在已經修正。 |是 |否 |
| 8 |Update |在某些罕見的情況下，使用者在較早版本的軟體中嘗試掃描或安裝更新時，會收到 "CisPowershellHcsscripterror"。 此版本已經修正這個問題。 |是 |是 |
