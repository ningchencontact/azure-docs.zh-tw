...
Azure Key Vault 是一項雲端服務，可作為安全的祕密存放區。 您也可以安全地儲存金鑰、密碼、憑證和其他祕密。 如需 Key Vault 的詳細資訊，您可以檢閱[概觀](key-vault-overview.md)。 您可以透過 Azure 入口網站建立和管理 Azure 金鑰保存庫。 在本快速入門中，您會建立金鑰保存庫。 一旦完成該作業，您就會儲存祕密。
...
5. 在 [建立金鑰保存庫] 區段上提供下列資訊：
    - **Name**：在本快速入門中，我們使用 **Contoso-vault2**。 您必須在測試中提供唯一的名稱。
    - **訂用帳戶**：選擇您將使用於本快速入門的訂用帳戶。
...
> 若要了解如何自使用受控服務身分識別的 Web 應用程式讀取 Key Vault 中的祕密，請繼續進行以下教學課程[將 Azure Web 應用程式設定為從 Key Vault 讀取祕密](quick-create-net.md)。
