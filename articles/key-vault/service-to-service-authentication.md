...
1. 登入 Visual Studio 並使用**工具**&nbsp;>&nbsp;**選項** 來開啟**選項**。
...
1. 輸入*az account get---- https://vault.azure.net* 來確認存取。 如果您收到錯誤，請檢查正確的 Azure CLI 版本是否已正確安裝。
...
在您登入 Azure 之後，`AzureServiceTokenProvider` 會使用服務主體來取得用於本機開發的權杖。
...
1. 建立服務主體憑證，並自動將它儲存在您的 Key Vault 中。 使用 Azure CLI [az ad sp create-rbac--keyvault \<keyvaultname >--cert \<certificatename >--建立-cert--skip-指派](/cli/azure/ad/sp?view=azure-cli-latest#az-ad-sp-create-for-rbac)命令：
...
| 連接字串選項 | 案例 | 註解|
|:--------------------------------|:------------------------|:----------------------------|
| `RunAs=Developer; DeveloperTool=AzureCli` | 本機開發 | `AzureServiceTokenProvider` 使用 AzureCli 來取得權杖。 |
