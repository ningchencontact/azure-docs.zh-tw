...
依照慣例，SDK 會自動將 `/negotiate` 附加至 URL，並使用它來開始進行協商。
...
* `CORSCredentials`-設定為 `true` 以允許 "withCredentials" 要求
...
設定完成後，已驗證的 HTTP 要求會包含 `x-ms-client-principal-name` 和 `x-ms-client-principal-id` 標頭，分別包含已驗證身分識別的使用者名稱和使用者識別碼。

您可以在*SignalRConnectionInfo*系結設定中使用這些標頭來建立已驗證的連接。 以下是使用 `x-ms-client-principal-id` C#標頭的範例 negotiate 函式。
