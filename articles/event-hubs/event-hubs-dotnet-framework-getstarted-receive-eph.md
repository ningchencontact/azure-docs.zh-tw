---
title: 使用 .NET Framework 接收事件 - Azure 事件中樞 | Microsoft Docs
...
在此快速入門中，您已建立可從事件中樞接收訊息的 .NET Framework 應用程式。 若要了解如何使用 .NET Framework 將事件傳送到事件中樞，請參閱[從事件中樞傳送事件 - .NET Framework](event-hubs-dotnet-framework-getstarted-send.md)。
