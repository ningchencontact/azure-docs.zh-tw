...
| 值 | 描述 |
|---|---|
| 名稱 | 您想要提供給 Webhook 的名稱。 它只能包含字母和數字必須是長度為 5 到 50 個字元。 |
...
| 影響範圍 | Webhook 的運作範圍。 如果未指定，範圍是在登錄中的所有事件。 可以針對存放庫或標記中指定使用 「 存放庫： 標記 」 的格式或 「 儲存機制: *"儲存機制下所有標籤。 |
