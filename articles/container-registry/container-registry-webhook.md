...
| Location | 針對[異地](container-registry-geo-replication.md)複寫的登錄, 請指定登錄複本的 Azure 區域。 
...
| `Scope` | Webhook 的運作範圍。 如果未指定, 則範圍適用于登錄中的所有事件。 您可以針對存放庫或標籤使用 "repository: tag" 格式, 或針對存放庫底下的所有標籤指定 "repository: *"。 |
