...
建置完成後，如果您想要完成下列選擇性步驟，請記下**組建識別碼** (例如 "aa6")。
...
現在，請使用 `docker run` 在本機執行容器。 請將 **\<build-id\>** 取代為在上一個步驟的輸出中找到的組建識別碼 (例如 "aa6")。
...
如果您想要執行下列選擇性步驟以執行新建置的容器，並查看更新的版本號碼，請記下映像更新觸發之組建的 **BUILD ID** 值 (在上述輸出中，其值為 "aa8")。
