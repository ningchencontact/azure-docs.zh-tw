...
| SKU | 描述 |
| --- | ----------- |
| **基本** | 了解 Azure Container Registry 之開發人員的成本最佳化進入點。 基本登錄具有與 Standard 和 Premium 相同的程式設計功能（例如 Azure Active Directory[驗證整合](container-registry-authentication.md#individual-login-with-azure-ad)、[映射刪除][container-registry-delete]和[webhook][container-registry-webhook]）。 不過，內含儲存體和映像輸送量最適合較低的使用方式情節。 |
| **標準** | 「標準」登錄提供與「基本」相同的功能，並且提高內含儲存體和映像輸送量。 「標準」登錄應該滿足大部分實際執行案例的需求。 |
