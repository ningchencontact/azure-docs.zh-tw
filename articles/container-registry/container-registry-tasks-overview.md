...
「ACR 工作」已設計為容器生命週期原始物件。 例如，您可以將「ACR 工作」整合到 CI/CD 解決方案中。 執行[az login][az-login] with a [service principal][az-login-service-principal]之後, 您的 CI/CD 解決方案就可以發出[az acr build][az-acr-build]命令來啟動映射組建。
