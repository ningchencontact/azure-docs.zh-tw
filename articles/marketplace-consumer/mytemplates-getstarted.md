...
> [!NOTE]
> Microsoft 建議您透過[受管理的應用程式](../managed-applications/overview.md)來建立服務目錄應用程式，而不是在入口網站中使用私人範本。 您可以讓服務目錄應用程式可供您組織中的使用者使用。

本文將逐步引導您使用 Azure 入口網站來新增、管理及共用私人**範本**。

## <a name="guidance"></a>指引
下列建議可協助您在使用您的方案時充分利用**範本**：

* **範本**是一項封裝資源，其中包含 Resource Manager 範本和其他中繼資料。 其操作方式類似 Marketplace 中的項目。 主要差別在於它是私人項目 (相對於公用 Marketplace 項目)。
...
2. 匯出 Resource Manager 範本後，使用 [儲存範本] 按鈕將它儲存到 [範本] 儲存機制。 在 [這裡](../azure-resource-manager/resource-manager-export-template.md)尋找匯出範本的完整詳細資料。
   <br /><br />
   ![資源群組匯出](media/rg-export-portal1.PNG)
3. 選取 [儲存至範本]  命令按鈕。
   <br /><br />
4. 輸入以下資訊：
   
   * 名稱 – 範本物件的名稱 (注意︰此欄位是以 Azure Resource Manager 為基礎的名稱。 適用所有命名限制，一旦建立便無法變更)。
   * 說明 – 關於此範本的簡短摘要。
     
     ![儲存範本](media/save-template-portal1.PNG)
5. 按一下 [檔案] 。
   
   > [!NOTE]
   > 入口網站會在匯出的 Resource Manager 範本有誤時顯示通知，但您仍可將此 Resource Manager 範本儲存至 [範本]。 請確保在重新部署已匯出的 Resource Manager 範本之前，先檢查並修正所有的 Resource Manager 範本問題。
   > 
   > 

### <a name="method-2-add-a-new-template-resource-from-browse"></a>方法 2︰從瀏覽加入新的範本資源
您也可以在 [瀏覽] > [範本] 中使用 [+新增] 命令按鈕，從頭新增**範本**。 提供 [名稱]、[說明] 和 Resource Manager 範本 JSON。

![新增範本](media/add-template-portal1.PNG)

> [!NOTE]
> Microsoft.Gallery 是以租用戶為基礎的 Azure 資源提供者。 [範本] 資源會繫結至建立它的使用者。 但不會繫結至任何特定的訂用帳戶。 在部署範本時選擇訂用帳戶。
> 
> 

## <a name="view-template-resources"></a>檢視範本資源
在 [瀏覽 > 範本] 可以看見您可用的所有**範本**。 可用的範圍包括您所建立的範本，以及利用各種權限層級與您共用的範本。 如需詳細資訊，請參閱[存取控制](#access-control-for-a-tenant-resource-provider)。

![檢視範本](media/view-template-portal1.PNG)

按一下清單中的項目，即可檢視 **範本** 的詳細資料。

![檢視範本](media/view-template-portal2c.png)

## <a name="edit-a-template-resource"></a>編輯範本資源
以滑鼠右鍵按一下瀏覽清單上的項目，或選擇 [編輯] 命令按鈕，即可起始**範本** 的編輯流程。

![編輯範本](media/edit-template-portal1a.PNG)
...
![編輯範本](media/edit-template-portal2a.PNG)

儲存範本後，您就會看到確認通知。

![編輯範本](media/edit-template-portal3b.png)

## <a name="deploy-a-template-resource"></a>部署範本資源
您可以部署任何您有**讀取**權限的**範本**。 填妥 Resource Manager 範本參數的值以繼續進行部署。
...
按一下滑鼠右鍵或在檢視特定項目時，選取瀏覽項目上的 [共用]。

![共用範本](media/share-template-portal1a.png)

 您現在可以選擇角色及使用者或群組，以提供特定 **範本**的存取權。 可用的角色為 [擁有者]、[讀取者] 和 [參與者]。

![共用範本](media/share-template-portal2b.png)

![共用範本](media/share-template-portal3b.png)

按一下 [選取] 和 [確定]。 您現在可以看到您加入至資源的使用者或群組。

![共用範本](media/share-template-portal4b.png)
