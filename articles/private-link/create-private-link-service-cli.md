...
在此階段，您的私用連結服務已成功建立，並已準備好接收流量。 請注意，上述範例只是為了示範如何使用 Azure CLI 來建立私用連結服務。  我們尚未設定負載平衡器後端集區或後端集區上的任何應用程式來接聽流量。 如果您想要查看端對端流量流程，強烈建議您在 Standard Load Balancer 後方設定應用程式。  
...
使用 [az network vnet subnet create](/cli/azure/network/vnet/subnet#az-network-vnet-subnet-create)在虛擬網路中建立子網。 這個範例會在名為*myResourcegroup*的資源群組中名為*myPEVnet*的虛擬網路中，建立名為 *mySubnet* 的子網： 
...
--location westcentralus 
```
您可以使用私人連結服務上的 `az network private-link-service show` 取得*私人-連線資源識別碼*。 識別碼看起來會像這樣：   
