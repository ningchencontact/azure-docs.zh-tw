...
- Azure PowerShell 1.5.0 or higher. 如果尚未安裝，請依照[這些指示](/powershell/azure/install-az-ps)操作。
...
- **Path**: Output folder path. 這是選擇性參數。 If not specified, the package is created in current directory.
...
For Linux policies, include the property **AttributesYmlContent** in your configuration and overwrite the values accordingly. The Guest Configuration agent automatically creates the YaML file used by InSpec to store attributes. 請看下方範例。
