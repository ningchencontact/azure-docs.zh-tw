...
預設會以 JSON 提供 Azure CLI 的結果。 Azure PowerShell 中的結果預設為**PSCustomObject** ，但使用 `ConvertTo-Json` Cmdlet 可以快速地將其轉換為 JSON。 針對其他 Sdk，可以將查詢結果設定為輸出_ObjectArray_格式。
