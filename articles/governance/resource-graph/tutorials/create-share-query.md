...
Azure Resource Graph Explorer 可讓您直接在 Azure 入口網站中儲存 Resource Graph 查詢。 查詢分成兩種類型：_私人_和_共用_。 私人查詢會儲存在您的 Azure 入口網站設定中。 共用查詢是一種 Resource Manager 資源，可以使用角色型存取控制 (RBAC) 來管理，並使用資源鎖定加以保護。 這兩種類型的查詢都會在待用時加密。
