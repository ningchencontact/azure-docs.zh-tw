...
- \[預覽\]：部署 VM 擴充功能，以稽核允許沒有密碼的帳戶進行遠端連線的 Linux VM
- \[預覽\]：稽核沒有密碼的 Linux VM 帳戶
- \[預覽\]：稽核允許沒有密碼的帳戶進行遠端連線的 Linux VM
- 稽核傳統儲存體帳戶的使用
- 稽核傳統虛擬機器的使用
- 稽核不是使用受控磁碟的 VM
...
- \[預覽\]：稽核函數應用程式的僅限 HTTPS 存取權限
- \[預覽\]：稽核 Web 應用程式的僅限 HTTPS 存取權限
- \[預覽\]：稽核 API 應用程式的僅限 HTTPS 存取權限
