...
description: NIST SP 800-53 R4 藍圖範例對於 Azure 原則的控制措施對應。
author: DCtheGeek
ms.author: dacoulte
ms.date: 06/24/2019
ms.topic: sample
...
以下是與 **NIST SP 800-53 (第 4 版)** 控制措施的對應。 使用右側的導覽區可直接跳到特定的控制項對應。 許多對應的控制項都是以 [Azure 原則](../../../policy/overview.md)方案進行實作的。 若要檢閱完整方案，請在 Azure 入口網站中開啟 [原則]  ，然後選取 [定義]  頁面。 然後，找出並選取 **\[預覽\]：稽核 NIST SP 800-53 R4 控制措施並部署特定的 VM 延伸模組，以支援稽核需求**內建原則方案。
...
- \[預覽\]：稽核 Web 應用程式的 CORS 資源存取限制

## <a name="ac-5-separation-of-duties"></a>AC-5 權責區分

只有一個 Azure 訂用帳戶擁有者時，無法建立系統管理備援。 相反地，若有過多 Azure 訂用帳戶擁有者，因擁有者帳戶遭到入侵而產生缺口的可能性就會增加。 此藍圖指派了兩項 [Azure 原則](../../../policy/overview.md)定義，可稽核 Azure 訂用帳戶的擁有者數目，藉以協助您維護適當數量的 Azure 訂用帳戶擁有者。 這個藍圖還指派四個 Azure 原則定義，協助您控制 Windows 虛擬機器上 Administrators 群組的成員資格。 管理訂用帳戶擁有者與虛擬機器系統管理員權限可協助您實作適當的權責區分。
...
- \[預覽\]：稽核函式應用程式的遠端偵錯狀態
- \[預覽\]：稽核 Web 應用程式的遠端偵錯狀態
- \[預覽\]：稽核 API 應用程式的遠端偵錯狀態
- \[預覽\]：稽核 Linux VM 是否不允許沒有密碼即從帳戶進行遠端連線
- \[預覽\]：部署 VM 擴充功能來稽核 Linux VM 是否不允許沒有密碼即從帳戶進行遠端連線
- 稽核不受限制的儲存體帳戶網路存取
...
- \[預覽\]：在 Azure 資訊安全中心中監視未稽核的 SQL 伺服器
- 稽核診斷設定
...
- \[預覽\]：稽核 Linux VM 是否沒有無需密碼的帳戶
- \[預覽\]：部署 VM 擴充功能以稽核 Linux VM 是否沒有無需密碼的帳戶
- \[預覽\]：稽核 Linux VM 是否已將 passwd 檔案權限設定為 0644
- \[預覽\]：稽核 Windows VM 是否使用可還原的加密來儲存密碼
- \[預覽\]：部署 VM 擴充功能以稽核 Linux VM 是否已將 passwd 檔案權限設定為 0644
...
- \[預覽\]：稽核 Windows VM 是否無法重複使用先前的 24 個密碼
- \[預覽\]：稽核 Windows VM 的密碼最長使用期限是否為 70 天
- \[預覽\]：稽核 Windows VM 的密碼最短使用期限是否為 1 天
- \[預覽\]：稽核 Windows VM 是否已啟用密碼複雜性設定
- \[預覽\]：稽核 Windows VM 是否將最短密碼長度限制為 14 個字元
- \[預覽\]：稽核 Windows VM 是否使用可還原的加密來儲存密碼
- \[預覽\]：部署 VM 擴充功能以稽核 Windows VM 是否無法重複使用先前的 24 個密碼
- \[預覽\]：部署 VM 擴充功能以稽核 Windows VM 的密碼最長使用期限是否為 70 天
- \[預覽\]：部署 VM 擴充功能以稽核 Windows VM 的密碼最短使用期限是否為 1 天
- \[預覽\]：部署 VM 擴充功能以稽核 Windows VM 是否已啟用密碼複雜性設定
- \[預覽\]：部署 VM 擴充功能以稽核 Windows VM 是否將最短密碼長度限制為 14 個字元
- \[預覽\]：部署 VM 擴充功能以稽核 Windows VM 是否使用可還原的加密來儲存密碼
...
- 在沒有「進階資料安全性」的情況下稽核 SQL 伺服器
- 在 SQL 伺服器上部署進階資料安全性
- \[預覽\]：稽核 Azure 資訊安全中心內虛擬機器擴展集的 OS 弱點
...
- \[預覽\]：監視公開於網際網路的虛擬機器，以取得強化網路安全性群組流量的建議
...
- \[預覽\]：稽核 Azure 資訊安全中心內的虛擬機器擴展集上是否有任何遺失的系統更新
- \[預覽\]：稽核 Azure 資訊安全中心內虛擬機器擴展集的 OS 弱點
...
- \[預覽\]：稽核 Azure 資訊安全中心內的虛擬機器擴展集上是否有端點保護解決方案
...
您已檢閱過 NIST SP 800-53 R4 藍圖的控制項對應，接下來請瀏覽下列文章以了解架構及如何部署此範例：

> [!div class="nextstepaction"]
> [NIST SP 800-53 R4 藍圖 - 概觀](./index.md)
> [NIST SP 800-53 R4 藍圖 - 部署步驟](./deploy.md)
