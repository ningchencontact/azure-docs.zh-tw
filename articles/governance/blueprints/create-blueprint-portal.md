...
1. 新增訂用帳戶的角色指派：以滑鼠左鍵按一下 [訂用帳戶] 底下的 [+ 新增成品...] 資料列，即可在瀏覽器右側開啟 [新增成品] 視窗。 [成品類型] 請選取 [角色指派]。 請選取 [角色] 底下的 [參與者]，然後在 [新增使用者、應用程式或群組] 欄位勾選代表**動態參數**的核取方塊。 按一下 [新增] 將此成品新增至藍圖。
...
1. 新增訂用帳戶的資源群組：以滑鼠左鍵按一下 [訂用帳戶] 底下的 [+ 新增成品...] 資料列。 [成品類型] 請選取 [資源群組]。 將 [資源群組名稱] 和 [位置] 欄位留白，但是請務必勾選每個屬性的核取方塊，將其設定為**動態參數**。 按一下 [新增] 將此成品新增至藍圖。

1. 在資源群組下新增範本：以滑鼠左鍵按一下 [+ 新增成品...]  資料列。 [成品類型] 請選擇「Azure Resource Manager 範本 」，並將 [成品顯示名稱] 設為「StorageAccount」，將 [描述] 留白。 在編輯器方塊的 [範本] 索引標籤中，貼上以下的 Resource Manager 範本。 貼上範本之後，按一下 [參數] 索引標籤，並注意是否有自動偵測到範本參數 **storageAccountType** 並填入預設值 **Standard_LRS**，但設定為**動態參數**。 取消勾選核取方塊，並注意下拉式清單是否僅有 **allowedValues** 底下的 Resource Manager 範本所含的值。 勾選方塊，將其設回**動態參數**。 按一下 [新增] 將此成品新增至藍圖。
...
1. 在資源群組下新增角色指派：以滑鼠左鍵按一下 [資源群組] 項目底下的 [+ 新增成品...] 資料列。 [成品類型] 請選取 [角色指派]。 在 [角色] 底下，選取 [擁有者]，並取消勾選 [新增使用者、應用程式或群組] 欄位，然後搜尋並選取所要新增的使用者、應用程式或群組。 此成品會在每次指派此藍圖時使用設定相同的**靜態參數**。 按一下 [新增] 將此成品新增至藍圖。
