...
description: 在本教學課程中，您將瞭解如何建立、布建、驗證、更新、刪除和取消布建 ExpressRoute 線路。
...
1. 您可以選取建立新資源的選項來建立 ExpressRoute 循環。 按一下 [建立資源] > [網路] > [ExpressRoute]，如下圖所示：
...
2. 按一下 [ExpressRoute] 之後，將會看到 [建立 ExpressRoute 線路] 頁面。 在此頁面中填入值時，請確認所指定的 SKU 層 ([標準] 或 [進階]) 以及資料計量計費模型 ([無限制] 或 [計量付費]) 正確無誤。
   ...
   * 決定計費類型。 您可以指定 [已計量] 以採用計量付費數據傳輸方案，選取 [無限制] 以採用無限行動數據方案。 請注意，您可以將計費類型從 [計量付費] 變更為 [無限制]。

     > [!IMPORTANT]
     > 您無法將類型從 [**無限制**] 變更為 [**計量**付費]。
...
### <a name="3-view-the-circuits-and-properties"></a>3. 查看線路和屬性
...
### <a name="4-send-the-service-key-to-your-connectivity-provider-for-provisioning"></a>4. 將服務金鑰傳送給您的連線提供者以進行布建

在此頁面上，[提供者狀態] 會提供服務提供者目前佈建狀態的相關資訊。 提供 Microsoft 端的狀態。 如需線路佈建狀態的詳細資訊，請參閱 [工作流程](expressroute-workflows.md#expressroute-circuit-provisioning-states) 文章。
...
### <a name="5-periodically-check-the-status-and-the-state-of-the-circuit-key"></a>5. 定期檢查線路金鑰的狀態和狀態
...
### <a name="6-create-your-routing-configuration"></a>6. 建立路由設定
...
### <a name="7-link-a-virtual-network-to-an-expressroute-circuit"></a>7. 將虛擬網路連結至 ExpressRoute 線路
