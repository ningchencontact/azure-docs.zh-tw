...
- 下載並使用從 PowerShell Gallery 安裝 Az.DataMigration 模組[Install-module PowerShell cmdlet](https://docs.microsoft.com/powershell/module/powershellget/Install-Module?view=powershell-5.1)
...
下列範例會為 **AdventureWorks** 資料庫建立 `AzDataMigrationDatabaseInfo` 專案，並將其新增至清單 (提供給建立物件所需的參數使用)。
...
現在建立檔案共用的物件，表示本機 SMB 網路共用的 Azure 資料庫移轉服務可放置來源資料庫備份來使用新增 AzDmsFileShare cmdlet。
...
下一個步驟是選取的來源和目標資料庫使用的新 AzDmsSelectedDB cmdlet，如下列範例所示：
