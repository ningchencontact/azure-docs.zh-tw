...
本文說明如何在離線狀態從 SQL Server 移轉至 Azure SQL Database 受控執行個體。 若要進行線上移轉，請參閱[使用 DMS 將 SQL Server 線上移轉至 Azure SQL Database 受控執行個體](tutorial-sql-server-managed-instance-online.md)。
