...
- 在此案例中，會在相同的 Azure VNet 中建立 Azure SQL Database 受控實例和 Azure 資料庫移轉服務實例，但會使用不同的子網。  
- 此案例中使用的 VNet 也會使用[ExpressRoute](https://docs.microsoft.com/azure/expressroute/expressroute-introduction)或[VPN](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-about-vpngateways)連線到內部部署網路。
...
- Azure SQL Database 受控執行個體和 Azure 資料庫移轉服務所使用的 Vnet 位於不同的訂用帳戶中。

![受控執行個體與內部部署網路分開的網路拓撲](media/resource-network-topologies/mi-isolated-workload.png)

**需求**

- Azure 資料庫移轉服務針對此案例使用的 VNet 也必須使用（ https://docs.microsoft.com/azure/expressroute/expressroute-introduction) 或[VPN](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-about-vpngateways)連線到內部部署網路。
- 在用於 Azure SQL Database 受控實例和 Azure 資料庫移轉服務的 VNet 之間設定[vnet 網路對等互連](https://docs.microsoft.com/azure/virtual-network/virtual-network-peering-overview)。

## <a name="cloud-to-cloud-migrations-shared-vnet"></a>雲端到雲端的遷移：共用 VNet

如果來源 SQL Server 裝載于 Azure VM，並與 Azure SQL Database 受控實例和 Azure 資料庫移轉服務共用相同的 VNET，請使用此拓撲。
...
## <a name="cloud-to-cloud-migrations-isolated-vnet"></a>雲端到雲端遷移：隔離的 VNet

如果您的環境需符合一或多個下列情況案例，請使用此網路拓撲：

- Azure SQL Database 受控實例會布建在隔離的 VNet 中。
- 如果角色型存取控制（RBAC）原則已就緒，您必須限制使用者存取裝載 Azure SQL Database 受控實例的相同訂用帳戶。
- 用於 Azure SQL Database 受控執行個體和 Azure 資料庫移轉服務的 Vnet 位於不同的訂用帳戶中。
