...
**問：什麼是 Azure 資料庫移轉服務？**
Azure 資料庫移轉服務是完全受控的服務，設計用來啟用多個資料庫來源的無縫移轉到 Azure 資料平台，最少停機時間。 服務目前已正式運作後，與進行中的開發工作著重於：

* 可靠性和效能。
* 反覆新增來源目標組。
* 持續投資無衝突的移轉。

**問：哪些來源/目標組 Azure 資料庫移轉服務目前支援嗎？**
此服務目前支援各種不同的來源/目標配對或移轉案例。 如需每個可用移轉案例狀態的完整清單，請參閱 [Azure 資料庫移轉服務支援的移轉案例狀態](https://docs.microsoft.com/azure/dms/resource-scenario-status)一文。

其他移轉案例處於預覽階段，需要提交提名，以透過 DMS 預覽站台。 預覽狀態，且登入參與的其中一個這些產品的案例的完整清單，請參閱 < [DMS 預覽網站](https://aka.ms/dms-preview/)。

**問：Azure 資料庫移轉服務支援哪些版本的 SQL Server 做為來源？**
從 SQL Server 移轉時，Azure 資料庫移轉服務支援的來源是透過 SQL Server 2017 的 SQL Server 2005。

**問：使用 Azure 資料庫移轉服務時，離線和線上移轉之間的差異為何？**
您可以使用 Azure 資料庫移轉服務來執行離線和線上移轉。 具有*離線*移轉，需要關閉應用程式啟動時啟動移轉。 具有*線上*移轉停機時間僅限於結尾的移轉移交時間。 建議您先測試離線移轉來決定停機時間是否在容忍範圍內；如果無法容忍，則請進行線上移轉。
...
**問：Azure 資料庫移轉服務如何比較 Database Migration Assistant (DMA) 或 SQL Server Migration Assistant (SSMA) 等其他 Microsoft 資料庫移轉工具？**
...
**問：Azure 資料庫移轉服務如何比較 Azure Migrate 供應項目？**
...
**問：使用 Azure 資料庫移轉服務的必要條件有哪些？**
有數個以確保 Azure 資料庫移轉服務能順利執行時執行資料庫移轉所需的必要條件。 某些必要條件會套用到服務所支援的所有案例 (來源目標組)，而有些必要條件則是特定案例獨有的。
...
**問：如何尋找 IP 位址的 Azure 資料庫移轉服務，讓我可以建立用來存取移轉來源資料庫的防火牆規則的允許清單？**
...
**問：若要使用 Azure 資料庫移轉服務執行的資料庫移轉所需的步驟大致為何？**
在典型的簡單資料庫移轉期間，您必須：

1. 建立目標資料庫。
2. 評估您的來源資料庫。
    * 同質性的移轉，請使用，您現有的資料庫評估[DMA](https://www.microsoft.com/download/details.aspx?id=53595)。
    * 異質性 （來源 compete） 移轉，評估您現有的資料庫與[SSMA](https://aka.ms/get-ssma)。 您也可以使用 SSMA 轉換的資料庫物件，並將結構描述移轉到您的目標平台。
3. 建立 Azure 資料庫移轉服務的執行個體。
4. 建立移轉專案，以指定的來源資料庫、 目標資料庫，以及要移轉的資料表。
5. 啟動完整載入。
6. 挑選後續驗證。
7. 將生產環境手動轉換至新的雲端型資料庫。

## <a name="troubleshooting-and-optimization"></a>疑難排解和最佳化

**問：我設定 DMS，在移轉專案，我無法連線到我的來源資料庫。我該怎麼辦？**
...
**問：無法使用或已停止，為何我的 Azure 資料庫移轉服務？**
如果使用者明確地停止 Azure 資料庫移轉服務 (DMS) 或服務未使用達 24 小時期間內，服務將會處於 已停止，或自動暫停的狀態。 在每個情況下，服務都將無法使用並處於停止的狀態。  若要繼續使用中的移轉，請重新啟動服務。

**問：是否有任何建議的 Azure 資料庫移轉服務的效能最佳化？**
