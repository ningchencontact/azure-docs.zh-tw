...
這項服務目前支援各種移轉案例。 如需每個可用移轉案例狀態的完整清單，請參閱 [Azure 資料庫移轉服務支援的移轉案例狀態](https://docs.microsoft.com/azure/dms/resource-scenario-status)一文。 其他移轉案例處於預覽階段，需要提交提名，以透過 DMS 預覽站台。 預覽版本，以註冊加入其中一個這些供應項目中的案例的完整清單，請參閱 < [DMS 預覽網站](https://aka.ms/dms-preview/)。
