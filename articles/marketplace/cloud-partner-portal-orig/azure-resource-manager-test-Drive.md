...
Azure Resource Manager (Azure Resource Manager) 範本是 Azure 資源所組成的程式碼範本，可讓您視需要用來建立最能代表您解決方案的項目。 若不熟悉 Resource Manager 範本，請參閱[了解 ARM 範本](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-overview)與[製作 ARM 範本](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-authoring-templates)，以確定您知道如何建置並測試您自己的範本。
...
務必花足夠的時間評估展示您產品的最佳方式。 特別是如何利用所需的必要資源完成工作，因為這樣可讓您輕鬆地建立 Resource Manager 範本架構。

繼續討論上面所講的防火牆範例，架構可能是您需要為您的服務使用公用 IP URL，並為您防火牆所保護的網站使用另一個公用 IP URL。 每個 IP 都是部署在虛擬機器上並使用網路安全性群組 + 網路介面彼此連線。

一旦建立需資源套件的架構，您便可以開始撰寫及建置試用產品 Resource Manager 範本。
