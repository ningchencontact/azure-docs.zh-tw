...
在下列案例中新增多個 SKU：
- 您想要支援不同的計費模型，例如自備授權 (BYOL) 或預付型方案 (PAYG)。
...
1. 在 [供應項目設定] 欄位中設定下列**供應項目身分識別**。
    ...
    **名稱**
    ...
    您供應項目的顯示名稱。 此名稱會顯示在 Azure Marketplace 與 Azure 入口網站中。 它最多不能超過 50 個字元。 為供應項目名稱使用下列指導方針︰
    -  包含產品的可辨識品牌名稱。 
...
使用 Marketplace 檢視來設定針對 [Azure Marketplace](https://azuremarketplace.microsoft.com) 上與 [Azure Portal](https://portal.azure.com/) 上之供應項目顯示的欄位。
...
-   Azure 設計具有簡單的調色盤。 保持標誌上具有最少的主要和次要色彩數目。
...
主圖標誌為選用項。 發行者可以選擇不上傳主圖標誌。 不過，上傳標誌之後，無法將它刪除。 夥伴必須遵循 Azure Marketplace 的主圖圖示指導方針。
...
## <a name="to-publish-the-offer"></a>發行供應項目
