...
| **欄位名稱**    | **注意事項**   |  
| :---------------- | :-----------| 
| 供應項目識別碼  | 必要，無法在建立後變更。 最多50個字元，且只能包含小寫、英數位元、連字號或底線。 |
| 供應專案別名  | 必要。 |
...
| 您要如何讓潛在客戶與供應專案清單互動？ （呼叫動作）  | 如果不是透過 Microsoft 銷售，則為必要。 預設值：免費試用，選項：「立即取得」、「免費試用」、「聯絡我」。 |
| 試用版 URL  | 若已選取 [免費試用]，則為必要專案，如同客戶應該與供應專案清單互動的方式。 |
| 供應專案 URL  | 若已選取 [立即取得]，則為必要專案，如同客戶應該與供應專案清單互動的方式 |
| 聲道  | 選用。 預設值：未加入宣告 CSP （轉銷商）頻道。  |
| 試用產品 | 選用。 預設值：未啟用試用產品。  |
| 試用產品的類型 | 啟用試用產品時為必要。 預設值：未選取。 選項： Azure Resource Manager、適用于 Business Central 的 Dynamics 365、Dynamics 365 for Customer Engagement、Dynamics 365 for Operations、邏輯應用程式、Power BI。  |
| 潛在客戶管理-連接到 CRM 系統 | 如果是透過 Microsoft 銷售，或將供應專案列示為「聯絡我」，則為必要專案。 預設值：沒有已連線的 CRM 系統。 CRM 選項： Azure 資料表、Azure blob、Dynamics CRM online、HTTPs 端點、Marketo、Salesforce  |
...
| **欄位名稱**    | **注意事項**   | 
| :---------------- | :-----------|  
| 類別和子類別 | 必要1和最大值3。 預設值：未選取。 |
| 產業和 subindustries | 選用。 每個 L1 產業內最多2個 L1 產業和最多2個 subindustries，預設值：未選取 |
| 應用程式版本  | 選用。 預設值：無。 |
| 使用標準合約  | 選用。 預設值：未選取。  | |
| 使用規定  | 如果未選取標準合約，則為必要。  |
...
| 商店標誌（小型、中型、大型、寬、主圖）  | 需要小型（48 X 48）和大型（216 X 216）;其他大小選用但建議使用：中型（90 x 90）、寬（255 x 115）、主圖（815 x 290）。 必須是 PNG 格式。 |
...
| **欄位名稱**    | **注意事項**   | 
| :---------------- | :-----------| 
| AAD/MSA 電子郵件 + 描述 | 必要，最小1和最大值10（如果手動輸入），或在上傳 CSV 檔案時為20。 |
...
| **欄位名稱**    | **注意事項**   |  
| :---------------- | :-----------| 
| 計畫描述   | 如果是透過 Microsoft 銷售，則為必要。 最多500個字元。 | |
