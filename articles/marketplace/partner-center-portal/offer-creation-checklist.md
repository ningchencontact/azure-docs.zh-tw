...
| 聲道  | 選擇性。 預設值：未選擇加入的 CSP （轉銷商） 通道。  |
| 試用產品 | 選擇性。 預設值：啟用任何測試磁碟機。  |
...
| 產業和 subindustries | 選擇性。 2 個 L1 產業的最大和最大 2 subindustries 內每個 L1 產業，預設值：未選取任何人 |
| 應用程式版本  | 選擇性。 預設值：無。 |
| 使用標準的合約  | 選擇性。 預設值： 未選取。  | |
| 使用規定  | 如果標準的合約未選取必要的。  |
...
| 計劃對象  | 選擇性。 預設值：公用方案。 選項:公用、 私用的租用戶識別碼 |
