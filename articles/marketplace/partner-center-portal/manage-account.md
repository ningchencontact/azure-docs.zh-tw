...
您在稅務表單中提交的資訊會決定適當的稅金預繳匯率。 此預繳利率僅適用于您在美國所進行的銷售;在非美國位置進行的銷售不受限於預繳。 預繳匯率有所不同，但對於大部分在美國以外註冊的開發人員而言，預設速率為30%。 如果您的國家（地區）同意具有美國的收入稅務條約，您可以選擇降低此速率。
...
裝置管理設定只適用于 UWP 發行。 [詳細資訊](https://docs.microsoft.com/windows/uwp/publish/manage-account-settings-and-profile#additional-settings-and-info)。
...
2. 輸入新 Azure AD 的目錄資訊：
    ...
    - **全域管理員使用者帳戶資訊**：您要用於新全域管理員帳戶的名字、姓氏、使用者名稱和密碼。
