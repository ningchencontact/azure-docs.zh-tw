...
提供可存取您應用程式的有效 URL (從 HTTP 或 HTTPs 開始), 以向客戶列出您的供應專案, 其中包含免費試用版的連結。  例如： `https://contoso.com/trial/saas-app` 。 供應專案列出免費試用版是由您的服務所建立、管理及設定, 而且沒有由 Microsoft 管理的訂用帳戶。
...
手動新增最多十 (10) 個電子郵件地址, 或為現有的 Microsoft 帳戶 (MSA) 或 Azure Active Directory (AAD) 帳戶上傳 CSV 檔案, 以協助在即時發行之前驗證您的供應專案。 藉由新增這些帳戶, 您就可以在將供應專案發佈至 marketplace 之前, 定義允許預覽存取權的物件。 如果您的供應專案已上線, 您仍然可以定義預覽物件, 以測試您的供應專案是否有任何變更或更新。
...
- **方案名稱:** 當決定要在您的供應專案內選取哪一個方案時, 客戶會看到此名稱。 為此供應專案中的每個方案建立唯一的供應專案名稱。 方案名稱是用來區分可能屬於相同供應專案一部分的軟體方案 (例如 供應專案名稱:Windows Server;計畫Windows Server 2016、Windows Server 2019)。
