---
title: Dynamics 365 for Customer Engagement 供應項目設定 索引標籤 |Azure Marketplace
description: 在 AppSource Marketplace 上設定 Dynamics 365 for Customer Engagement 應用程式供應項目的供應項目設定。
...
下表說明此索引標籤中的欄位。必要欄位由星號 (*) 標示。

|    欄位         |       描述                                                            |
|  ---------       |     ---------------                                                          |
| **供應項目識別碼\***   | 供應項目的唯一識別碼 (位於發行設定檔中)。 此識別碼會顯示在產品的 URL 與深入解析報告中。 識別碼長度上限為 50 個字元，可使用小寫英數字元和連字號 (-)。 (識別碼結尾不能使用連字號。)**注意：** 供應項目上線後即無法變更，因為它會繫結至供應項目的 Marketplace 基底 URL。  |
...
使用[技術資訊索引標籤](./cpp-technical-info-tab.md)，設定供應項目的應用程式和套件資訊。
