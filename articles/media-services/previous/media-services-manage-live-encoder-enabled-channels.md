...
即時編碼通道只要其狀態透過 API 轉換為「執行中」，即會開始計費。   您也可以在 Azure 入口網站或 Azure 媒體服務總管工具 (https://aka.ms/amse)) 中檢視狀態。
...
6. 發行與程式相關聯的資產。   
    ...
    >[!NOTE]
    >创建 AMS 帐户后，会将一个处于“已停止”状态的**默认**流式处理终结点添加到帐户。 您想要串流內容的串流端點必須處於 [執行中] 狀態。 
...
### <a name="allowed-ip-addresses"></a>允許的 IP 位址
可以定义允许连接到预览终结点的 IP 地址。 如果沒有指定 IP 位址，將會允許任何 IP 位址。 允許的 IP 位址可以指定為單一 IP 位址 (例如 ‘10.0.0.1’)、使用 IP 位址和 CIDR 子網路遮罩的 IP 範圍 (例如 ‘10.0.0.1/22’)，或是使用 IP 位址和小數點十進位子網路遮罩的 IP 範圍 (例如 '10.0.0.1(255.255.252.0)')。
...
在建立通道時會透過預設 slate 資產識別碼屬性指定要使用的映像。 将对静态图像进行拉伸以适合显示图像大小。 
...
設定 **封存時間範圍** 長度，即可指定您想要保留程式之錄製內容的時數。 此值最小可以設定為 5 分鐘，最大可以設定為 25 個小時。 封存時間範圍長度也會指出用戶端可以從目前即時位置及時往回搜尋的最大時間量。 程式在超過指定的時間量後還是可以執行，但是會持續捨棄落後時間範圍長度的內容。 此属性的值还决定了客户端清单能够增加多长时间。

每个节目都与存储流式处理内容的资源相关联。 資產會對應到「Azure 儲存體」帳戶中的區塊 Blob 容器，而資產中的檔案則會儲存為該容器中的 Blob。 若要发布节目，以便客户查看该流，必须为关联的资源创建按需定位符。 擁有此定位器，可讓您建置可提供給用戶端的串流 URL。
...
* **啟動中**。 正在啟動通道。 在此狀態中不允許更新或串流。 如果發生錯誤，通道會回到已停止狀態。
* **執行中**。 通道能夠處理即時串流。
* **停止中**。 正在停止通道。 在此狀態期間允許任何更新或串流。
* **刪除中**。 正在删除通道。 在此狀態期間允許任何更新或串流。

下表顯示通道狀態如何對應至計費模式。 

| 頻道狀態 | 入口網站 UI 指標 | 是否计费？ |
| --- | --- | --- |
| 啟動中 |啟動中 |無 (暫時性狀態) |
| 正在运行 |准备就绪（没有正在运行的节目）<br/>或<br/>串流 (至少一個正在執行的程式) |是 |
| 停止中 |停止中 |否（暂时状态） |
| 已停止 |已停止 |否 |
...
* 通道或其关联的节目正在运行时，无法更改输入协议。 如果您需要不同的通訊協定，則應該為每個輸入通訊協定建立個別的通道。
