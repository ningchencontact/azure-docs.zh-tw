...
| 分析       | 數字     | 必要項 | 應該是拼接的時間 (以呈現時間表示)。 單位是小數秒。                                     |
...
| 分析       | 數字     | 必要項 | 事件或拼接的呈現時間。  呈現時間與期間應該與類型 1 或 2 的串流存取點 (SAP) 一致，如 [ISO-14496-12] 附錄 I 中所定義。針對 HLS 輸出，時間與期間應該與區段界限一致。 呈現時間與相同事件串流中不同事件訊息的期間不能重疊。 單位是小數秒。
