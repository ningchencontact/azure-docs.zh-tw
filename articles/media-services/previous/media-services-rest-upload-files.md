...
在媒體服務中，您會將數位檔案上傳到到資產。 [資產](https://docs.microsoft.com/rest/api/media/operations/asset)實體可以包含影片、音訊、影像、縮圖集合、文字播放軌和隱藏式輔助字幕檔案（以及這些檔案的相關中繼資料）。 將檔案上傳到資產之後，您的內容就會安全地儲存在雲端，以進行進一步的處理和串流。 
...
3. 檢查 **AzureMediaServices.postman_environment.json** 檔案。 您將會看到集合中幾乎所有作業都執行著 "test" 指令碼。 指令碼會採用一些回應所傳回的值，並設定適當的環境變數。
    ...
    ```
4. 在 [ **Postman** ] 視窗的左側，按一下 [ **1]。取得 AAD 驗證權杖** -> **取得服務主體的 Azure AD token**。
