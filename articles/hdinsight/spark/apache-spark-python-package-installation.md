...
> [!NOTE]  
> 叢集中有兩個 Python 安裝。 Spark 會使用位於 `/usr/bin/anaconda/bin` 的 Anaconda Python 安裝，並且會預設為 Python 2.7 環境。 若要使用 Python 3.x 並安裝 PySpark3 核心中的套件，請使用該環境的 `conda` 可執行檔路徑，並使用 `-n` 參數來指定環境。 例如，`/usr/bin/anaconda/envs/py35/bin/conda install -c conda-forge ggplot -n py35` 命令會使用 `ggplot` 通道將 `conda-forge` 套件安裝至 Python 3.5 環境。
