...
1. 開啟 IntelliJ IDEA。 開啟 [**新增專案**] 視窗。
   ...
   b.這是另一個 C# 主控台應用程式。 從主視窗中選取 [**具有失敗工作的調試範例（預覽）（Scala）的 Spark 專案**]。
...
2. 在 [**新增專案**] 視窗中，執行下列步驟：
   ...
   b.這是另一個 C# 主控台應用程式。 在 [**專案 SDK** ] 下拉式清單中，選取 [ **JAVA 1.8** for **Spark 2.3.2** cluster]。
...
1. 開啟**Microsoft Azure 儲存體總管**，找出失敗作業的叢集 HDInsight 帳戶，從對應的位置下載失敗的作業資源： **\hdp\spark2-events\\。 spark-失敗\\\<> 本機資料夾的應用程式識別碼**。 [**活動**] 視窗會顯示下載進度。
