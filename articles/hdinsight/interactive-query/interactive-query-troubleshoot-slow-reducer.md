...
如果輸入資料表中的資料分割較少 (例如小於 10), 而輸出分割區的數目是, 而變數設定為`true`, 這會導致資料以全域方式排序, 並使用每個分割區的單一歸納器寫入。 即使可用的歸納器數目較大, 還是有少數歸納器可能會因為資料扭曲而延遲, 而且無法達到最大平行處理原則。 當變更為`false`時, 多個歸納器可能會處理單一分割區, 而且會寫出多個較小的檔案, 進而加快插入速度。 這可能會影響進一步的查詢, 因為有較小的檔案。
