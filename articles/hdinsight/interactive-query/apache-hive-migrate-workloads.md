...
2. 針對您的 HDI 3.6 叢集啟動腳本動作，並以「前端節點」做為執行的節點類型。 將下列 URI 貼到標示為「Bash 腳本 URI」的文字方塊中： https://hdiconfigactions.blob.core.windows.net/hivemetastoreschemaupgrade/launch-schema-upgrade.sh 。在標示為「引數」的文字方塊中，輸入所**複製**Hive 中繼存放區的 servername、資料庫、使用者名稱和密碼，並以空格分隔。 指定 servername 時，請勿包含 ". database.windows.net"。
...
針對您的叢集啟動腳本動作，並以「前端節點」做為執行的節點類型。 將下列 URI 貼到標示為「Bash 腳本 URI」的文字方塊中： https://hdiconfigactions.blob.core.windows.net/dasinstaller/LaunchDASInstaller.sh

等待5到10分鐘，然後使用此 URL 啟動資料分析 Studio： HTTPs://\<clustername >. hdinsightclustername>.azurehdinsight.net. net/das/
