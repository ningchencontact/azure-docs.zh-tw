...
[Apache Spark](http://spark.apache.org) 是一個開放原始碼平行處理架構，可支援記憶體內部處理，大幅提升巨量資料分析應用程式的效能。 如需深入了解 HDInsight 中的 Spark，請參閱 [Azure HDInsight 上的 Spark 簡介](../spark/apache-spark-overview.md)。 
...
* **[Apache Spark](http://spark.apache.org/)**：平行處理架構，可支援記憶體內部處理，大幅提升巨量資料分析應用程式的效能。 Spark 適用於 SQL、串流資料和機器學習。 請參閱[什麼是 HDInsight 中的 Apache Spark？](../spark/apache-spark-overview.md)
...
您可以使用[指令碼動作](../hdinsight-hadoop-script-actions-linux.md)來安裝其他語言。

### <a name="java-virtual-machine-jvm-languages"></a>Java 虛擬機器 (JVM) 語言
Java 虛擬機器 (JVM) 上可以執行許多 Java 以外的語言。 不過，如果您要執行這些語言，您可能必須在叢集上安裝其他元件。
...
* [使用 Power Query 將 Excel 連線到 Hadoop](apache-hadoop-connect-excel-power-query.md)：了解如何使用 Microsoft Power Query for Excel，將 Excel 連線到儲存 HDInsight 叢集資料的 Azure 儲存體帳戶。 必須具有 Windows 工作站。 

* [使用 Microsoft Hive ODBC 驅動程式將 Excel 連接到 Hadoop](apache-hadoop-connect-excel-hive-odbc-driver.md)：了解如何使用 Microsoft Hive ODBC 驅動程式從 HDInsight 匯入資料。 必須具有 Windows 工作站。 
