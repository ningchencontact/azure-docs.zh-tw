...
您也可以從 Excel 使用 Microsoft Power Query for Excel，連接與 HDInsight 叢集和其他資料來源 (包括其他 (非 HDInsight) Hadoop 叢集) 相關聯的資料。 如需安裝和使用 Power Query 的詳細資訊，請參閱[Excel 連接到 HDInsight 使用 Power Query](../hdinsight-connect-excel-power-query.md)。
...
1. 輸入或選取下列值：
   ...
   |  密碼 |輸入 HDInsight 叢集使用者的密碼。 選取此核取方塊**儲存密碼 （加密）** 。|
...
3. 從下拉式清單中，選取您在上一節中建立的資料來源名稱，然後按**確定**。

4. 第一次使用，如**ODBC 驅動程式**此時會開啟對話方塊。 選取  **Windows**從左側功能表中。 然後選取**Connect**來開啟**Navigator**視窗。
