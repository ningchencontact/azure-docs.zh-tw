...
1. 啟動 Beeline 時，您必須為 HDInsight 叢集上的 HiveServer2 提供連接字串：
    ...
    * 透過 Azure 虛擬網路連線時，您必須提供叢集前端節點的完整網域名稱 (FQDN)。 由於此連線是直接連線到叢集節點，因此會使用連接埠 `10001` 進行連線：
        ...
        ```
    * 連線至已加入 Azure Active Directory (AAD) 的「企業安全性套件」(ESP) 叢集時，您必須一併指定網域名稱 `<AAD-Domain>`，以及具備叢集存取權限的網域使用者帳戶名稱 `<username>`：
...
若要找出前端節點的完整網域名稱，請利用[使用 Apache Ambari REST API 管理 HDInsight](../hdinsight-hadoop-manage-ambari-rest-api.md#example-get-the-fqdn-of-cluster-nodes) 文件中的資訊。
...
如果您搭配使用 Tez 和 Hive，請參閱下列文件：[在以 Linux 為基礎的 HDInsight 上使用 Apache Ambari Tez 檢視](../hdinsight-debug-ambari-tez-view.md)。
