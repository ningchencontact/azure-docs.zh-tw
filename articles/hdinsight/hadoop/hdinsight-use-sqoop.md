...
[Apache Sqoop](https://sqoop.apache.org/docs/1.99.7/user.html)是一種專為在 Hadoop 叢集和關係資料庫之間傳輸資料而設計的工具。 此工具可讓您從 SQL Server、MySQL 或 Oracle 等關聯式資料庫管理系統 (RDBMS)，將資料匯入至 Hadoop 分散式檔案系統 (HDFS)，使用 MapReduce 或 Apache Hive 轉換 Hadoop 中的資料，然後將資料匯回 RDBMS。 在本文中，您使用的是關係資料庫的 SQL Server 資料庫。
...
* 名為 `hivesampletable`的 Hive 資料表，其參考位於 `/hive/warehouse/hivesampletable`的資料檔案。 此資料表包含某些行動裝置資料。
...
2. 輸入下列屬性：
    ...
    |Resource group |從下拉式清單中選取您的資源群組，或建立一個新的|
    ...
    Azure SQL Server 名稱將會 `<ClusterName>dbserver`。 資料庫名稱將會 `<ClusterName>db`。 預設的儲存體帳戶名稱將會是 `e6qhezrh2pdqu`。
