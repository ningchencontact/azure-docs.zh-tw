...
ms.locfileid: "67433272"
---
# <a name="upload-data-for-apache-hadoop-jobs-in-hdinsight"></a>在 HDInsight 中上傳 Apache Hadoop 作業的資料

Azure HDInsight 在 Azure 儲存體和 Azure Data Lake Storage (Gen1 和 Gen2) 上提供了功能完整的 Hadoop 分散式檔案系統 (HDFS)。 Azure 儲存體和 Data Lake Storage Gen1 和 Gen2 是設計作為 HDFS 的延伸，以便為客戶提供順暢的體驗。 它們可讓 Hadoop 生態系統中的完整元件集直接在它管理的資料上運作。 Azure 儲存體、Data Lake Storage Gen1 和 Gen2 是不同的檔案系統，但經過最佳化後，都非常適合儲存資料以及計算儲存的資料。 使用 Azure 儲存體的優點的相關資訊，請參閱[使用 Azure 儲存體與 HDInsight](hdinsight-hadoop-use-blob-storage.md)，[使用 HDInsight 與 Data Lake 儲存體 Gen1](hdinsight-hadoop-use-data-lake-store.md)，和[與使用 Data Lake 儲存體 Gen2HDInsight](hdinsight-hadoop-use-data-lake-storage-gen2.md)。

## <a name="prerequisites"></a>必要條件

開始進行之前，請注意下列需求：

* Azure HDInsight 叢集。 如需相關指示，請參閱 <<c0> [ 開始使用 Azure HDInsight](hadoop/apache-hadoop-linux-tutorial-get-started.md)或是[建立 HDInsight 叢集](hdinsight-hadoop-provision-linux-clusters.md)。
* 下列文章的知識：
...
如需詳細資訊，請參閱 <<c0> [ 搭配 HDInsight 使用 Sqoop](hadoop/hdinsight-use-sqoop.md)。
