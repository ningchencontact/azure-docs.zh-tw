...
![架構圖表](./media/apache-storm-sensor-data-analysis/devicesarchitecture.png)
...
![拓撲圖](./media/apache-storm-sensor-data-analysis/sensoranalysis.png)
...
3. 開啟網路瀏覽器，在網址列輸入 `http://localhost:3000/`。 將會顯示與下圖類似的頁面：
    ...
    ![Web 儀表板](./media/apache-storm-sensor-data-analysis/emptydashboard.png)
...
2. 若要以本機模式啟動拓撲，請使用下列命令：
   ...
   啟動之後，拓撲會從事件中樞讀取項目，並將它們傳送到在本機電腦上執行的儀表板。 您應該會看見 Web 儀表板中出現數個線條，如下圖所示：
    ...
    ![儀表板與資料](./media/apache-storm-sensor-data-analysis/datadashboard.png)
...
2. 在 [自訂部署] 區段輸入以下值：
    ...
    ![HDInsight 參數](./media/apache-storm-sensor-data-analysis/parameters.png)
...
![Vnet 及叢集的資源群組](./media/apache-storm-sensor-data-analysis/groupblade.png)
...
1. 選擇下列範例之一，為您的 HBase 叢集取出 Zookeeper 仲裁：
    ...
    >
    > 此範例需要 Azure PowerShell。 如需進一步了解 Azure PowerShell 之使用，請參閱 [Azure PowerShell 使用者入門](https://docs.microsoft.com/powershell/scripting/Getting-Started-with-Windows-PowerShell?view=powershell-6)
...
5. 拓撲啟動後，以瀏覽器開啟您發佈至 Azure 的網站，然後使用 `node app.js` 命令將資料傳送至事件中樞。 您應該會看見顯示資訊的 Web 儀表板更新。
    ...
    ![儀表板](./media/apache-storm-sensor-data-analysis/datadashboard.png)
...
若需更多搭配 HDInsight 的 Storm 拓撲範例，請參閱 [Storm on HDInsight 的範例拓撲](apache-storm-example-topology.md)。

如需關於 Apache Storm 的詳細資訊，請參閱 [Apache Storm](https://storm.incubator.apache.org/) 網站 (英文)。

如需關於 HBase on HDInsight 的詳細資訊，請參閱 [HBase 與 HDInsight 概觀](../hbase/apache-hbase-overview.md)。

如需關於 Socket.io 的詳細資訊，請參閱 [socket.io](http://socket.io/) 網站 (英文)。

如需關於 D3.js 的詳細資訊，請參閱 [D3.js：資料驅動型文件](http://d3js.org/)(英文)。

如需關於以 Java 建立拓撲的詳細資訊，請參閱 [為 Apache Storm on HDInsight 開發 Java 拓撲](apache-storm-develop-java-topology.md)。

如需關於以 .NET 建立拓撲的詳細資訊，請參閱 [使用 Visual Studio 為 Apache Storm on HDInsight 開發 C# 拓撲](apache-storm-develop-csharp-visual-studio-topology.md)。
