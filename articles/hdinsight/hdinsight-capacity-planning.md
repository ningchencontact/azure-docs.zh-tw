...
您可以相應放大叢集以符合尖峰負載需求，然後當不再需要這些額外的節點時，將其相應縮小。 [自動調整功能](hdinsight-autoscale-clusters.md)可讓您自動調整您的叢集將會根據預先決定的度量和時間。 如需有關手動調整您的叢集的詳細資訊，請參閱[調整 HDInsight 叢集](hdinsight-scaling-best-practices.md)。
