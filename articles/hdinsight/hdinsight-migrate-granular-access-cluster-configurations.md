...
先前，具有擁有者、參與者或讀者[RBAC 角色](https://docs.microsoft.com/azure/role-based-access-control/rbac-and-directory-admin-roles)的叢集使用者可以透過 HDInsight API 取得秘密，因為他們可供擁有 `*/read` 許可權的任何人使用。 密碼會定義為值，可用來取得比使用者角色應允許更高的存取權。 其中包括叢集閘道 HTTP 認證、儲存體帳戶金鑰和資料庫認證之類的值。

從2019年9月3日開始，存取這些密碼將需要 `Microsoft.HDInsight/clusters/configurations/action` 許可權，這表示他們無法再由具有「讀取者」角色的使用者存取。 具有此許可權的角色是「參與者」、「擁有者」和「新的 HDInsight 叢集操作員」角色（如下所示）。
...
- [API](#api)：使用 `/configurations` 或 `/configurations/{configurationName}` 端點的使用者。
...
- [適用于 JAVA 的 SDK](#sdk-for-java)：使用 `update` 的使用者，或從 `ConfigurationsInner` 類別 `get` 的方法。
- [適用于 Go 的 SDK](#sdk-for-go)：使用 `Get` 的使用者，或從 `ConfigurationsClient` 結構 `Update` 方法。
...
- `ClusterOperationsExtensions.GetClusterConfigurations`**不會再傳回敏感的參數**，例如儲存體金鑰（核心網站）或 HTTP 認證（閘道）。
    - 若要取出所有設定（包括機密參數），請使用 `ClusterOperationsExtensions.ListConfigurations` 繼續進行。  請注意，具有「讀取者」角色的使用者將無法使用這個方法。 這可讓您更精確地控制哪些使用者可以存取叢集的機密資訊。
...
- [`ConfigurationOperationsExtensions.Get`](https://docs.microsoft.com/dotnet/api/microsoft.azure.management.hdinsight.configurationsoperationsextensions.get?view=azure-dotnet) **不會再傳回敏感的參數**，例如儲存體金鑰（核心網站）或 HTTP 認證（閘道）。
    - 若要取出所有設定（包括機密參數），請使用[`ConfigurationOperationsExtensions.List`](https://docs.microsoft.com/dotnet/api/microsoft.azure.management.hdinsight.configurationsoperationsextensions.list?view=azure-dotnet)繼續進行。  請注意，具有「讀取者」角色的使用者將無法使用這個方法。 這可讓您更精確地控制哪些使用者可以存取叢集的機密資訊。 
...
- [`ConfigurationsOperations.get`](https://docs.microsoft.com/python/api/azure-mgmt-hdinsight/azure.mgmt.hdinsight.operations.configurationsoperations#get-resource-group-name--cluster-name--configuration-name--custom-headers-none--raw-false----operation-config-) **不會再傳回敏感的參數**，例如儲存體金鑰（核心網站）或 HTTP 認證（閘道）。
    - 若要取出所有設定（包括機密參數），請使用[`ConfigurationsOperations.list`](https://docs.microsoft.com/python/api/azure-mgmt-hdinsight/azure.mgmt.hdinsight.operations.configurationsoperations#list-resource-group-name--cluster-name--custom-headers-none--raw-false----operation-config-)繼續進行。  請注意，具有「讀取者」角色的使用者將無法使用這個方法。 這可讓您更精確地控制哪些使用者可以存取叢集的機密資訊。 
...
- [`ConfigurationsClient.get`](https://godoc.org/github.com/Azure/azure-sdk-for-go/services/preview/hdinsight/mgmt/2018-06-01-preview/hdinsight#ConfigurationsClient.Get) **不會再傳回敏感的參數**，例如儲存體金鑰（核心網站）或 HTTP 認證（閘道）。
    - 若要取出所有設定（包括機密參數），請使用[`ConfigurationsClient.list`](https://godoc.org/github.com/Azure/azure-sdk-for-go/services/preview/hdinsight/mgmt/2018-06-01-preview/hdinsight#ConfigurationsClient.List)繼續進行。  請注意，具有「讀取者」角色的使用者將無法使用這個方法。 這可讓您更精確地控制哪些使用者可以存取叢集的機密資訊。 
...
新增此角色指派最簡單的方式是使用 Azure CLI 中的 `az role assignment create` 命令。

> [!NOTE]
> 此命令必須由具有「擁有者」角色的使用者執行，因為只有他們可以授與這些許可權。 `--assignee` 是您想要指派 HDInsight 叢集操作員角色之使用者的服務主體或電子郵件地址的名稱。 如果您收到許可權不足的錯誤，請參閱下列常見問題。
...
叢集設定現在位於更細微的角色型存取控制之下，而且需要 `Microsoft.HDInsight/clusters/configurations/*` 許可權才能存取它們。 若要取得此許可權，請將「HDInsight 叢集操作員」、「參與者」或「擁有者」角色指派給嘗試存取設定的使用者或服務主體。
...
除了擁有擁有者角色以外，執行命令的使用者或服務主體必須具有足夠的 AAD 許可權，才能查詢受託人的物件識別碼。 此訊息表示 AAD 許可權不足。 請嘗試以 `–assignee-object-id` 取代 `-–assignee` 引數，並提供受託人的物件識別碼做為參數，而不是名稱（如果是受控識別，則為主體識別碼）。 如需詳細資訊，請參閱[az role 指派建立檔](https://docs.microsoft.com/cli/azure/role/assignment?view=azure-cli-latest#az-role-assignment-create)的選擇性參數一節。

如果仍然無法解決問題，請洽詢您的 AAD 系統管理員以取得正確的許可權。

### <a name="what-will-happen-if-i-take-no-action"></a>如果我沒有採取任何動作，會發生什麼事？

從2019年9月3日開始，`GET /configurations` 和 `POST /configurations/gateway` 呼叫將不會再傳回任何資訊，而 `GET /configurations/{configurationName}` 呼叫將不會再傳回敏感性參數，例如儲存體帳戶金鑰或叢集密碼。 對應的 SDK 方法和 PowerShell Cmdlet 也是如此。
