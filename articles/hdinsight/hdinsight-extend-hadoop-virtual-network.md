...
* 了解 TCP/IP 網路。 如果您不熟悉 TCP/IP 網路，則應該與之前在生產網路中修改的人員合作。
...
3. 您使用網路安全性群組、使用者定義路由或虛擬網路設備來限制傳入或傳出虛擬網路的流量嗎？
    ...
    * 網路安全性群組
        ...
        取代`RESOURCEGROUP`的資源群組包含虛擬網路，然後輸入命令名稱：
...
3. 尋找虛擬網路的 Azure 指派 DNS 尾碼。 此值與 `0owcbllr5hze3hxdja3mqlrhhe.ex.internal.cloudapp.net` 類似。 如需尋找 DNS 尾碼的相關資訊，請參閱[範例：自訂 DNS](#example-dns) 一節。
...
強制通道是一種使用者定義路由設定，其中來自子網路的所有流量都會強制流向特定網路或位置，例如內部部署網路。 HDInsight「不」支援流向內部部署網路的強制通道。 如果您使用 Azure 防火牆或網路虛擬設備，裝載在 Azure 中，您可以使用流量路由傳送到其進行監視，並允許所有連出流量的 Udr。
...
1. 使用下列命令來建立名為 `hdisecure`的新網路安全性群組。 取代`RESOURCEGROUP`包含 Azure 虛擬網路的資源群組。 取代`LOCATION`群組建立所在的位置 （區域）。
