...
了解如何透過 Azure 虛擬網路，直接連線到 HDInsight 上的 Apache Kafka。 此文件提供使用下列設定連線到 Kafka 的相關資訊：
...
* 在與 HDInsight 上之 Kafka 相同的虛擬網路中執行用戶端。 [開始在 HDInsight 上使用 Apache Kafka](apache-kafka-get-started.md) 文件中使用的就是此設定。 用戶端會直接在 HDInsight 叢集節點或相同網路中的另一部虛擬機器上執行。

* 將私人網路 (例如，您的內部部署網路) 連線至虛擬網路。 此設定可讓您內部部署網路中的用戶端直接使用 Kafka。 若要啟用此設定，請執行下列工作：

    1. 建立虛擬網路。
    2. 建立 VPN 閘道以使用站對站設定。 此文件中使用的設定會連線到內部部署網路中的 VPN 閘道裝置。
    3. 在虛擬網路中建立 DNS 伺服器。
...
* 使用 VPN 閘道與 VPN 用戶端，將個別機器連線至虛擬網路。 若要啟用此設定，請執行下列工作：
    ...
    如需詳細資訊，請參閱[使用 VPN 用戶端連線到 Apache Kafka](#vpnclient) 一節。
...
1. 遵循[使用點對站連線的自我簽署憑證](../../vpn-gateway/vpn-gateway-certificates-point-to-site.md)文件中的步驟執行。 此文件會建立閘道所需的憑證。
...
* [對 HDInsight 上的 Apache Kafka 使用鏡像](apache-kafka-mirroring.md)
