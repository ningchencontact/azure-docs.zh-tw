...
Apache HBase 區域伺服器上的重新開機作業無法完成。 從區域伺服器啟動失敗的背景工作節點上 `/var/log/hbase` 目錄中的 `region-server.log`，您可能會看到類似如下的錯誤訊息：
...
1. 由於這種突然關機，雖然區域伺服器進程已終止，但與進程相關聯的埠可能無法釋出，最終會導致 `AddressBindException`。
