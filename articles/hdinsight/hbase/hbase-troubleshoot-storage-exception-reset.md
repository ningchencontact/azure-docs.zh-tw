...
雖然沒有名為的資料夾 blob `/hbase/data/default/ThatTable` 坐在儲存體中。 WASB 驅動程式發現 blob 檔案上方存在，因此不允許建立名為 `/hbase/data/default/ThatTable` 的任何 blob，因為它假設父資料夾已存在，因此建立資料表將會失敗。

## <a name="resolution"></a>解析度

1. 從 Apache Ambari UI，重新開機使用中的 HMaster。 這可讓兩個待命 HMaster 的其中一個變成作用中的，而新的作用中 HMaster 將會重載中繼資料資料表資訊。 因此，您不會在 HMaster UI 中看到 `already-deleted` 資料表。
