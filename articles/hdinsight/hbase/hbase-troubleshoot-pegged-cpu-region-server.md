...
1. 在叢集中的所有節點上安裝 jdk 1.8，如下所示：
    ...
    * 或者，您可以登入每個個別節點，並 `sudo add-apt-repository ppa:openjdk-r/ppa -y && sudo apt-get -y update && sudo apt-get install -y openjdk-8-jdk`執行命令。

1. 移至 Ambari UI-`https://<clusterdnsname>.azurehdinsight.net`。

1. 流覽至**HBase-> 的 [> advanced-> advanced** ] `hbase-env configs`，並將變數 `JAVA_HOME` 變更為 [`export JAVA_HOME=/usr/lib/jvm/java-8-openjdk-amd64`]。 儲存設定變更。
