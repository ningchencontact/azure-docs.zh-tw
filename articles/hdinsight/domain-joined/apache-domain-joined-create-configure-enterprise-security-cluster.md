...
1. 在 [**同步**處理] 頁面上，選取 [**全部** **] > [確定]** 以啟用完整同步處理。
...
1. 在裝載 Azure AD DS （`HDIFabrikam-AADDSVNET`）的虛擬網路與將裝載已啟用 ESP 的 HDInsight 叢集（`HDIFabrikam-HDIVNet`）的虛擬網路之間建立對等關聯性。 使用下列 PowerShell 程式碼，對等互連兩個虛擬網路。
