...
1. 編輯 **pom.xml** 檔案，並在 `<dependencies>` 區段內加入下列程式碼：
   ...
   > [!IMPORTANT]  
   > 版本號碼必須符合隨附於 HDInsight 叢集的 HBase 版本。 可以使用下表来查找正确的版本号。
   >
   >
...
有許多方法可將檔案上傳至 HDInsight 叢集，如 [在 HDInsight 中將 Apache Hadoop 工作的資料上傳](hdinsight-upload-data.md)中所述。 下列步驟會使用 Azure PowerShell。
...
5. 檔案上傳之後，使用以下程式碼建立使用 **hbaseapp** 的資料表：
    ...
    此命令會在 HDInsight 叢集中建立名為 **people** 的新資料表。 此命令在控制台窗口中不显示任何输出。
