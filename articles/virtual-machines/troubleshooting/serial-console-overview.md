...
ms.locfileid: "75451232"
---
# <a name="azure-serial-console"></a>Azure 序列主控台

Azure 入口網站中的序列主控台可讓您存取執行 Linux 或 Windows 的虛擬機器（Vm）和虛擬機器擴展集實例的文字型主控台。 此序列式連線可連線至 ttyS0 或 VM 的 COM1 序列埠，或是虛擬機器擴展集執行個體，提供不依存於網路或作業系統狀態的存取能力。 您只能使用 Azure 入口網站來存取序列主控台，而且只允許對 VM 或虛擬機器擴展集具有「參與者」或更高版本存取角色的使用者。
