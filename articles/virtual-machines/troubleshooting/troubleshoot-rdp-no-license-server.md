...
ms.locfileid: "50088248"
---
# <a name="remote-desktop-license-server-is-not-available-when-you-connect-to-azure-vm"></a>當您連線到 Azure VM 時無法使用遠端桌面授權伺服器

本文可協助解決下列問題：因為沒有遠端桌面授權伺服器可用來提供授權而無法連線到 Azure 虛擬機器 (VM)。

## <a name="symptoms"></a>徵兆

當您嘗試連線到 VM 時，會遇到下列案例：

- 虛擬機器 (VM) 螢幕擷取畫面顯示作業系統已完全載入且正在等待認證。
- 當您嘗試建立 Microsoft 遠端桌面通訊協定 (RDP) 連線時，會收到下列錯誤訊息：

  - **遠端工作階段中斷，因為沒有提供授權的遠端桌面授權伺服器可以使用。**

  - **沒有可用的遠端桌面授權伺服器。遠端桌面服務將停止運作，因為這部電腦已經過了寬限期而且從未連線到任何有效的 Windows Server 2008 授權伺服器。請按一下這個訊息來開啟 RD 工作階段主機伺服器設定，以便使用授權診斷。**
...
1. 使用系統管理工作階段連線到 VM：
  ...
  如果您無法使用系統管理工作階段連線到 VM，您可以使用[序列主控台](serial-console-windows.md)來存取 VM，如下所示：
...
2. 檢查 VM 是否已啟用遠端桌面工作階段主機角色。 如果已啟用此角色，請確定它會正確運作。 開啟提升權限的 CMD 執行個體，並遵循下列步驟：
  ...
  2. 使用下列命令來檢查原則並視需要重新設定：
    ...
    如果將 **LicensingMode** 值設定為 4 以外的任何其他值 (每位使用者)，則會將值設定為 4：

    ```
    reg add "HKLM\SYSTEM\CurrentControlSet\Control\Terminal Server\RCM\Licensing Core" /v LicensingMode /t REG_DWORD /d 4
    ```

    如果 **SpecifiedLicenseServers** 值不存在，或它具有不正確的授權伺服器資訊，則需變更它，如下所示：
  ...
  4. 如果您沒有 CAL，請移除遠端桌面工作階段主機角色。 然後，將 RDP 設回標準，並且只允許兩個並行的 RDP 連線連至 VM。

    ```
    dism /ONLINE /Disable-feature /FeatureName:Remote-Desktop-Services
    ```

    如果 VM 具有遠端桌面授權角色且不會使用它，您也可以移除該角色。

    ```
    dism /ONLINE /Disable-feature /FeatureName:Licensing
    ```

  5. 確定 VM 可以連線到遠端桌面授權伺服器。 您可以測試 VM 和授權伺服器之間與連接埠 135 的連線。 
