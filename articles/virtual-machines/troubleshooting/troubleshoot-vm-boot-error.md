...
* 如果您收到錯誤**錯誤 15:找不到檔案, 或找**不到初始 RAM 磁碟或**initrd/initramfs**檔案, 請遵循下列步驟。
   ...
   * 如果遺失的檔案是`/boot/grub/menu.lst`, 則此錯誤適用于較舊的 OS 版本 (**RHEL**6.x、 **Centos** 6.x 和**Ubuntu 14.04**), 因此命令可能會延遲。 您必須加速舊的伺服器並進行測試, 以確保提供正確的命令。
