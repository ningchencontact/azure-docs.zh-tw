...
#### <a name="step-1-turn-on-remote-deskop"></a>步驟 1：開啟遠端桌面
...
4. 檢查登錄機碼值，如下所示：
   ...
   3. 如果命令傳回 0，表示終端伺服器已停用。 那麼請啟用終端伺服器，如下所示：

      ```
      reg add "HKLM\SYSTEM\CurrentControlSet\Control\Terminal Server" /v TSEnabled /t REG_DWORD /d 1 /f
      ```

   4. 如果伺服器在終端伺服器陣列 (RDS 或 Citrix) 上，終端伺服器模組會設為清空模式。 檢查終端伺服器模組的目前模式。

      ```
      reg query "HKLM\SYSTEM\CurrentControlSet\Control\Terminal Server" /v TSServerDrainMode
      ```

   5. 如果命令傳回 1，表示終端伺服器模組設為清空模式。 那麼請將模組設為工作模式，如下所示：
   ...
   7. 如果命令會傳回 1，表示您無法連線到終端伺服器。 那麼請啟用連線，如下所示：
   ...
   9. 如果命令傳回 0，表示 RDP 接聽程式已停用。 那麼請啟用接聽程式，如下所示：

      ```
      reg add "HKLM\SYSTEM\CurrentControlSet\Control\Terminal Server\Winstations\RDP-Tcp" /v fEnableWinStation /t REG_DWORD /d 1 /f
      ```

   10. 檢查是否可以連線到 RDP 接聽程式。

      ```
      reg query "HKLM\SYSTEM\CurrentControlSet\Control\Terminal Server\Winstations\RDP-Tcp" /v fLogonDisabled
      ```

   11. 如果命令會傳回 1，表示您無法連線到 RDP 接聽程式。 那麼請啟用連線，如下所示：
...
7. 若要從 CMD 執行個體離開，請輸入 `exit`，然後按兩次 **Enter**。

8. 輸入 `restart` 來重新啟動 VM。
...
3. 啟動復原 VM 的遠端桌面連線。
4. 開啟提升權限的命令提示字元工作階段 (**以系統管理員身分執行**)。 執行下列指令碼。 在此指令碼中，我們假設指派給已連結 OS 磁碟的磁碟機代號是 F。請將此磁碟機代號取代為 VM 的適當值。
...
3. 如果 VM 已加入網域，請檢查下列登錄機碼，以查看是否有會停用 RDP 的群組原則。 
...
4. 將磁碟從救援 VM 中斷連結。
5. [從磁碟建立新的 VM](../windows/create-vm-specialized.md)。
