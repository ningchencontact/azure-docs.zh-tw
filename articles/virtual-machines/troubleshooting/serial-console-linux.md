...
不正確的防火牆規則 |  如果您已設定 iptables 來封鎖 SSH 連線，您可以使用序列主控台而不需要 SSH 互動與您的 VM。 更多詳細資料，請參閱[iptables 相關自定義 man 頁面](https://linux.die.net/man/8/iptables)。 同樣地，如果您的 firewalld 封鎖 SSH 存取，您可以透過序列主控台存取虛擬機器，並重新設定 firewalld。 更多詳細資料可在[firewalld 文件](https://firewalld.org/documentation/)。
