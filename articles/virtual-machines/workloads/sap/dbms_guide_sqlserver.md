...
* 使用可用性群組接聽程式時，資料庫 VM 需要連接到專用的負載平衡器。 為了避免 Azure 在這兩個 Vm 都已關閉的情況下指派新的 IP 位址，您應該在 Always On 設定中，將靜態 IP 位址指派給這些 Vm 的網路介面（定義靜態 IP 位址會在[本文中說明][virtual-networks-reserved-private-ip]）
