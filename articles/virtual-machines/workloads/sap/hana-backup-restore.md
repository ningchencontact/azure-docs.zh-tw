...
若要取得最新的快照集腳本和檔, 請參閱[GitHub](https://github.com/Azure/hana-large-instances-self-service-scripts/tree/master/snapshot_tools_v4.1)。 當您從[GitHub](https://github.com/Azure/hana-large-instances-self-service-scripts/tree/master/snapshot_tools_v4.1)下載快照集腳本套件時, 您會取得三個檔案。 其中一個檔案會記載在 PDF 中, 以取得所提供的功能。 下載工具組之後, 請依照「取得快照集工具」中的指示進行。
...
若要產生公開金鑰, 請參閱在[Azure 上的 SAP Hana 的 Microsoft 快照集工具](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.1/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.1.pdf)中的「啟用與存放裝置的通訊」。
...
若要設定和使用使用者帳戶, 請參閱[GitHub](https://github.com/Azure/hana-large-instances-self-service-scripts/tree/master/snapshot_tools_v4.1)中的「啟用與 SAP Hana 的通訊」。
...
從 [GitHub](https://github.com/Azure/hana-large-instances-self-service-scripts/tree/master/snapshot_tools_v4.1) 下載最新版本的指令碼。 腳本的安裝方式隨著腳本的版本4.1 而變更。 如需詳細資訊, 請參閱[Azure 上的 SAP Hana 的 Microsoft 快照集工具](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.1/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.1.pdf)中的「啟用與 SAP Hana 的通訊」。

如需確切的命令順序, 請參閱[Azure 上 SAP Hana 的 Microsoft 快照集工具](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.1/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.1.pdf)中的「輕鬆安裝快照集工具 (預設)」。 我們建議使用預設安裝。 

若要從版本3.x 升級至 4.1, 請參閱[Azure 上的 SAP Hana 的 Microsoft 快照集工具](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.1/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.1.pdf)中的「升級現有的安裝」。 若要卸載4.1 工具組, 請參閱[Microsoft snapshot tools for SAP Hana On Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.1/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.1.pdf)中的 < 卸載快照集工具」。

別忘了在[Azure 上的 SAP Hana 的 Microsoft 快照集工具](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.1/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.1.pdf)中, 執行「完成快照集工具的完整設定」中所述的步驟。

「這些快照集工具是什麼？」中會說明安裝的腳本和檔案的用途。 在[適用于 Azure 上的 SAP Hana 的 Microsoft 快照集工具](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.1/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.1.pdf)中。

設定快照集工具之前, 請確定您也已正確設定 HANA 備份位置和設定。 如需詳細資訊, 請參閱[Azure 上 SAP Hana 的 Microsoft 快照集工具](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.1/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.1.pdf)中的「SAP Hana 設定」。

在[Azure 上的 SAP Hana 的 Microsoft 快照集工具](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.1/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.1.pdf)中的「設定檔-hanabackupcustomerdetails.txt .txt」中會描述快照集工具集的設定。
...
下一個測試步驟是根據您放入*hanabackupcustomerdetails.txt*設定檔中的資料, 檢查存放裝置的連線能力。 然後執行測試快照集。 執行`azure_hana_backup`命令之前, 您必須執行此測試。 如需這項測試的命令順序, 請參閱[Azure 上 SAP Hana 的 Microsoft 快照集工具](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.1/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.1.pdf)中的 < 檢查存放裝置的連線-testStorageSnapshotConnection ""。
...
如需確切的命令語法和功能, 請參閱[azure 上的 SAP Hana 的 Microsoft 快照集工具](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.1/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.1.pdf)中的 < 執行快照集備份-azure_hana_backup。 
...
在您第一次成功執行儲存體快照集之後, 請刪除在步驟6中執行的測試快照集。 如需詳細資訊, 請參閱[Azure 上 SAP Hana 的 Microsoft 快照集工具](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.1/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.1.pdf)中的 < 移除測試快照集 removeTestStorageSnapshot。 
...
若要取得快照集的更多詳細資料`azure_hana_snapshot_details`, 請使用腳本。 如果嚴重損壞修復位置中有作用中的伺服器, 您可以在任一位置執行此腳本。 此指令碼提供下列輸出，這些輸出會依包含快照集的每個磁碟區細分： 
   * 磁碟區中所有快照集的大小
...
如需命令和輸出的語法, 請參閱[azure 上 SAP Hana 的 Microsoft 快照集工具](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.1/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.1.pdf)中的 < 列出快照集 azure_hana_snapshot_details。 
...
如需腳本的詳細資訊, 請參閱[azure 上的 SAP Hana 的 Microsoft 快照集工具](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.1/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.1.pdf)中的 < 刪除快照集 azure_hana_snapshot_delete。
...
如需使用新的工具集來還原快照集的可能性, 請參閱手動復原指南中的「如何還原快照」, 這[是 Azure 中從儲存體快照集的 SAP Hana](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/guides/Manual%20recovery%20of%20snapshot%20with%20HANA%20Studio.pdf)。
...
1. 開啟 Azure 支援要求, 並包含有關還原特定快照集的指示:
   ...
   - 還原期間︰SAP Hana 在 Azure 服務上, 可能會要求您參加會議呼叫以協調、驗證及確認是否還原正確的儲存體快照集。 

   - 還原之後：在還原儲存體快照集時, Azure 服務上的 SAP Hana 會通知您。
...
從儲存體快照集復原 SAP Hana 資料檔案的另一種可能性, 記載于[Azure 上從儲存體快照集的 SAP Hana 的手動修復指南](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/guides/Manual%20recovery%20of%20snapshot%20with%20HANA%20Studio.pdf)中的步驟7。

若要從快照集備份進行還原, 請參閱[從儲存體快照集在 Azure 上 SAP Hana 的手動復原指南](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/guides/Manual%20recovery%20of%20snapshot%20with%20HANA%20Studio.pdf)。 
...
若要還原至特定時間點, 請參閱手動復原指南中的[從儲存體快照集在 Azure 上進行 SAP Hana](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/guides/Manual%20recovery%20of%20snapshot%20with%20HANA%20Studio.pdf)的「將資料庫復原到下列時間點」。 
