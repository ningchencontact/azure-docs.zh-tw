...
Oracle 支援在以 Oracle Linux 為基礎的虛擬機器映射上，于 Azure 中執行 Oracle DB 12.1 和更新版本的 Standard 和 Enterprise 版本。  為了在 Azure 上 Oracle DB 生產工作負載的最佳效能，請務必適當地調整 VM 映射的大小，並使用進階 SSD 或 Ultra SSD 受控磁碟。 如需有關如何使用 Oracle 已發佈之 VM 映像在 Azure 中快速地讓 Oracle DB 啟動並執行的指示，[請嘗試 Oracle DB 快速入門逐步解說](oracle-database-quick-create.md)。
