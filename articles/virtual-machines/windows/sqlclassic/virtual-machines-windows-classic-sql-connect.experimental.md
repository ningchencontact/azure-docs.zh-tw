...
雖然此連接字串可讓用戶端透過網際網路連線，但這不表示任何人都可以連接您的 SQL Server。 外部客户端必须有正确的用户名和密码。 為了增加安全性，請勿使用知名的 1433 連接埠做為公用虛擬機器端點。 請盡可能考慮在您的端點加入 ACL 來限制流量，只開放給您允許的用戶端。 如需有關在端點中使用 ACL 的指示，請參閱 [在端點上管理 ACL](/previous-versions/azure/virtual-machines/windows/classic/setup-endpoints#manage-the-acl-on-an-endpoint)。
...
虛擬網路也可讓您將 Azure VM 加入網域。 加入網域是將 Windows 驗證搭配 SQL Server 使用的唯一方法。 其他连接方案需要使用用户名和密码进行 SQL 身份验证。
