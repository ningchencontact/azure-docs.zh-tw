...
Azure 受控磁碟可免除個別管理儲存體帳戶的需求，進而簡化儲存體管理。  您也可以將現有的 Azure VM 移轉至受控磁碟，以便受惠於可用性設定組中更佳的 VM 可靠性。 它會確保的可用性設定組不同的 Vm 磁碟是充分的隔離性與彼此避免單一失敗點。 它會自動以不同的儲存體縮放單位 (戳記) 將不同 VM 的磁碟放在一個可用性設定組中，以限制硬體和軟體失敗所引起之單一儲存體縮放單位失敗的影響。
根據您的需求，您可選擇兩種類型的儲存體選項︰

- [高階管理磁碟](premium-storage.md)是實線狀態磁碟機 (SSD) 儲存媒體提供高效能、 低延遲磁碟執行 O 為大量工作負載的虛擬機器的支援。 您可以將這類磁碟移轉至進階受控磁碟，以利用這類磁碟的速度和效能。
...
|---------------------|-------|-------|-------|-------|-------|-------|-------|-------|
| 磁碟大小           | 128 GB| 512 GB| 128 GB| 256 GB|512 GB | 1024 GB (1 TB)    | 2048 GB (2 TB)    | 4095 GB (4 TB)    | 
