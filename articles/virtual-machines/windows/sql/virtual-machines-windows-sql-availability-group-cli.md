...
這篇文章說明如何使用[Azure SQL VM CLI](https://docs.microsoft.com/mt-mt/cli/azure/ext/sqlvm-preview/sqlvm?view=azure-cli-2018-03-01-hybrid)部署在 Windows 容錯移轉叢集 (WSFC)，並將 SQL Server Vm 加入至叢集，以及建立內部負載平衡器和 Always On 可用性群組接聽程式。  Always On 可用性群組的實際的部署仍然是以手動方式透過 SQL Server Management Studio (SSMS)。 
...
手動建立可用性群組之後, 您可以建立接聽程式使用[az sql vm ag 接聽程式](https://docs.microsoft.com/cli/azure/sql/vm/group/ag-listener?view=azure-cli-latest#az-sql-vm-group-ag-listener-create)。 
...
沒有多一層的複雜性時部署到裝載於 Azure、 SQL Server Vm 的可用性群組，如資源現已管理資源提供者與`virtual machine group`。 因此，當新增或移除可用性群組的複本，是 SQL Server Vm 的相關資訊來更新接聽程式的中繼資料的額外的步驟中。 因此，當其他 SQL Server VM 的複本加入可用性群組中，您也必須使用[az sqlvm aglistener 新增-sqlvm](/cli/azure/ext/sqlvm-preview/sqlvm/aglistener?view=azure-cli-2018-03-01-hybrid#ext-sqlvm-preview-az-sqlvm-aglistener-add-sqlvm)命令，將 SQL Server VM 加入至接聽程式的中繼資料。 同樣地，當從可用性群組移除複本，您也必須使用[az sqlvm ag 接聽程式移除-sqlvm](/cli/azure/ext/sqlvm-preview/sqlvm/aglistener?view=azure-cli-2018-03-01-hybrid#ext-sqlvm-preview-az-sqlvm-aglistener-remove-sqlvm)來移除接聽程式的該 SQL Server VM 的中繼資料。 

### <a name="adding-a-replica"></a>加入複本
...
1. 加入 SQL Server VM 的中繼資料執行接聽程式：
    ```cli
...
### <a name="removing-a-replica"></a>移除複本
若要從可用性群組移除複本，執行下列作業：
