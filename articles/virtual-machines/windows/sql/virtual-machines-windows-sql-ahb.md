...
## <a name="limitations"></a>限制
 ...
 - 轉換授權模型的功能目前僅適用於以隨用隨付 SQL Server VM 映像啟動的狀況。 如果您從入口網站使用自備授權映像來啟動，則無法將該映像轉換成隨用隨付。
  - 目前的授權模型才支援變更為使用 Resource Manager 模型部署的虛擬機器。 不支援使用傳統模型所部署的 Vm。 
   - 目前，變更授權模型才會啟用公用雲端安裝。
   - 目前，只能在具有單一 NIC （網路介面） 的虛擬機器上支援此程序。 在有多個 NIC 的虛擬機器，您應該先移除其中一個 Nic （藉由使用 Azure 入口網站） 之前嘗試程序。 否則，您將會類似下列的錯誤：「 虛擬機器 '\<vmname\>' 有一個以上的 NIC 相關聯。 」 雖然您可以將 NIC 新增回 VM 之後變更的授權模式,，透過 SQL 組態刀鋒視窗中，例如自動修補和備份，完成的作業將不再支援。
...
## <a name="register-sql-resource-provider-to-your-subscription"></a>註冊訂用帳戶的 SQL 資源提供者 
...
若要向 SQL 資源提供者註冊您的 SQL Server VM，您必須向訂用帳戶註冊資源提供者。 您可以使用 Azure 入口網站、 Azure CLI 或 PowerShell 來這麼做。 
...
SQL 資源提供者註冊訂用帳戶之後, 您就可以向資源提供者註冊您的 SQL Server VM。 您可以使用 Azure CLI 和 PowerShell。 
...
## <a name="change-licensing-model"></a>變更授權模型

一旦您的 SQL Server VM 已向資源提供者，您可以變更使用 Azure 入口網站、 Azure CLI 或 PowerShell，在授權模型。 
...
當您嘗試使用 SQL 資源提供者尚未註冊的 SQL Server VM 上的授權模式變更時，就會發生此錯誤。 您必須註冊資源提供者，以您[訂用帳戶](#register-sql-resource-provider-to-your-subscription)，再將您的 SQL Server VM 向 SQL[資源提供者](#register-sql-server-vm-with-sql-resource-provider)。 

## <a name="next-steps"></a>後續步驟

如需詳細資訊，請參閱下列文章： 

* [在 Windows VM 上的 SQL Server 概觀](virtual-machines-windows-sql-server-iaas-overview.md)
* [SQL Server on Windows VM 的常見問題集](virtual-machines-windows-sql-server-iaas-faq.md)
* [Windows VM 的定價指導上的 SQL Server](virtual-machines-windows-sql-server-pricing-guidance.md)
* [SQL Server on Windows VM 的版本資訊](virtual-machines-windows-sql-server-iaas-release-notes.md)
