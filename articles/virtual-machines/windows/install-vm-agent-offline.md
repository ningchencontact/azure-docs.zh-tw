...
2.  將 OS 磁碟附加至另一個 VM (亦稱為「疑難排解」VM) 當做資料磁碟。 如需詳細資訊，請參閱[在 Azure 入口網站中將資料磁碟附加至 Windows VM](attach-managed-disk-portal.md)。
...
7.  如果 VM 代理程式無法運作，請備份目前的設定。

    >[!NOTE]
    >如果 VM 未安裝代理程式，請繼續步驟 8。 
...
8.  使用疑難排解 VM 上現有的檔案作為 VM 代理程式安裝的儲存機制。 完成下列步驟：
    ...
    2. 編輯登錄檔。 在每個檔案中，將項目值 **SYSTEM** 變更為 **BROKENSYSTEM** (如下列影像所示)，然後儲存檔案。
...
9.  從 C:\windowsazure\packages 將 VM 代理程式資料夾複製到 &lt;您連接的 OS 磁碟&gt;：\windowsazure\packages。

    ![將 VM 代理程式檔案複製到 OS 磁碟](./media/install-vm-agent-offline/copy-package.png)
      
    >[!NOTE]
    >不要複製 **logs** 資料夾。 在服務啟動之後，會產生新的記錄檔。

10.  選取 [BROKENSYSTEM]。 在功能表上，選取 [檔案] > **[上傳 Hive]**。

11.  選取 [BROKENSOFTWARE]。 在功能表上，選取 [檔案] > **[上傳 Hive]**。

12.  卸離 OS 磁碟，然後使用 OS 磁碟重新建立 VM。
...
- [Azure 虛擬機器代理程式概觀](agent-user-guide.md)
- [適用於 Windows 的虛擬機器擴充功能和功能](extensions-features.md)
