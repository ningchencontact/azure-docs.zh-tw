...
3. 您在部署範本之後，請使用您慣用的方法來確認 VM 加密狀態：
     ...
     -  選取 VM，然後按一下 [設定] 標題之下的 [磁碟]，以在入口網站中驗證加密狀態。 在圖中的 [加密] 之下，您會看到它是否已啟用。 
           ![Azure 入口網站啟用磁片加密](../media/disk-encryption/disk-encryption-fig2.png)
...
您可以使用 PowerShell 或[透過 Azure 入口網站](attach-disk-ps.md)，[將新的磁碟新增至 Windows VM](attach-managed-disk-portal.md)。 
...
- **透過 Resource Manager 範本停用加密：** 
    ...
    1. 從**在[執行中的 Windows VM 上停用磁碟加密](https://github.com/Azure/azure-quickstart-templates/tree/master/201-decrypt-running-windows-vm)** 範本，按一下 [部署至 Azure]。
