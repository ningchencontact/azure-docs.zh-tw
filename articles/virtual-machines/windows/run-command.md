...
下列是使用 [Invoke-AzureRmVMRunCommand](/powershell/module/azurerm.compute/invoke-azurermvmruncommand) Cmdlet 在 Azure VM 中執行 PowerShell 指令碼的範例。 此 Cmdlet 預期 `ScriptPath` 變數中所參考的指令碼，會位於 Cmdlet 執行所在位置的本機環境。
