...
本文示範如何透過 Azure 入口網站將新的受控資料磁碟連結到 Windows 虛擬機器。 這麼做之前，請先檢閱下列提示：
...
5. 在下拉式清單的新磁碟，選取**建立磁碟**。
6. 在**建立受管理的磁碟**頁面上，輸入磁碟的名稱並視需要調整其他設定。 完成之後，請按一下 [建立]。
7. 在**磁碟**頁面上，按一下**儲存**儲存新的磁碟設定的 vm。
6. Azure 會建立磁碟，並將其附加到虛擬機器後，新的磁碟會列出在下方的虛擬機器的磁碟設定**資料磁碟**。
...
2. 磁碟管理可以辨識您有新的、 未初始化的磁碟和**初始化磁碟**視窗出現。
3. 請確定已選取新磁碟，並按一下 [確定] 將磁碟初始化。
4. 新磁碟隨即顯示為**未配置**。 以滑鼠右鍵按一下磁碟上的任何位置，並選取 [新增簡單磁碟區]。 **新增簡單磁碟區精靈**隨即開啟。
