...
如果您選擇在本機安裝和使用 PowerShell，而不是從 Azure Cloud shell，則本教學課程需要 Azure PowerShell 模組。 執行 `Get-Module -ListAvailable Az` 找出版本。 如果您需要升級，請參閱[安裝 Azure PowerShell 模組](/powershell/azure/install-az-ps)。 如果您在本機執行 PowerShell，則也需要執行 `Connect-AzAccount` 以建立與 Azure 的連線。
