---
title: Azure 中常見的 Windows 虛擬機器錯誤碼 | Microsoft Docs
description: 了解當您在 Azure 中佈建及管理 Windows 虛擬機器時，所遇到的一些常見錯誤碼
services: virtual-machines-windows
documentationcenter: ''
author: xujing-ms
manager: jeconnoc
editor: ''
tags: azure-resource-manager
ms.assetid: ''
ms.service: virtual-machines-windows
ms.devlang: na
ms.topic: article
ms.tgt_pltfrm: vm-windows
ms.workload: infrastructure
ms.date: 11/03/2017
ms.author: xujing;genli
ms.openlocfilehash: 365fdcc8c7bf4ae8ec68b98734971d324032797f
ms.sourcegitcommit: 5b2ac9e6d8539c11ab0891b686b8afa12441a8f3
ms.translationtype: HT
ms.contentlocale: zh-TW
ms.lasthandoff: 04/06/2018
ms.locfileid: "30913210"
---
# <a name="understand-common-error-messages-when-you-manage-windows-virtual-machines-in-azure"></a>了解當您在 Azure 中管理 Windows 虛擬機器時的常見錯誤訊息

本文說明當您在 Azure 中建立或管理 Windows 虛擬機器 (VM) 時，可能遇到的一些最常見錯誤碼和訊息。

[!INCLUDE [virtual-machines-common-error-messages](../../../includes/virtual-machines-common-error-messages.md)]
