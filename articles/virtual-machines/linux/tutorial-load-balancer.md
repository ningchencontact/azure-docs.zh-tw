...
使用 [az vm availability-set create](/cli/azure/vm/availability-set) 建立可用性設定組。 下列範例會建立名為 myAvailabilitySet  的可用性設定組：
...
若要檢視連線到負載平衡器之虛擬 NIC 的 VM 清單，請使用 [az network lb address-pool show](/cli/azure/network/lb/address-pool)。 查詢和篩選虛擬 NIC 的識別碼，如下所示：
...
若要確認虛擬 NIC 已連線到後端位址集區，請從上一步驟再次使用 [az network lb address-pool show](/cli/azure/network/lb/address-pool)。
