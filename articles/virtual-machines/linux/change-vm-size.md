...
ms.locfileid: "67671729"
---
# <a name="resize-a-linux-virtual-machine-using-azure-cli"></a>使用 Azure CLI 調整 Linux 虛擬機器大小 

佈建虛擬機器 (VM) 之後，您可以調整 VM 向上或向下變更[VM 大小][vm-sizes]。 在某些情況下，您必須先解除配置 VM。 如果無法在裝載 VM 的硬體叢集上取得所需的大小，您可能需要解除配置 VM。 此文章詳述如何使用 Azure CLI 來調整 Linux VM 的大小。 
...
如需提高延展性，可執行多個 VM 執行個體並相應放大。如需詳細資訊，請參閱 <<c0> [ 在虛擬機器擴展集自動調整 Linux 機器][scale-set]。 
