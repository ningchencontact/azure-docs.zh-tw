...
ms.locfileid: "74036885"
---
# <a name="resize-a-linux-virtual-machine-using-azure-cli"></a>使用 Azure CLI 調整 Linux 虛擬機器大小 

布建虛擬機器（VM）之後，您可以藉由變更[vm 大小][vm-sizes]來相應增加或減少 vm。 在某些情況下，您必須先解除配置 VM。 如果無法在裝載 VM 的硬體叢集上取得所需的大小，您可能需要解除配置 VM。 此文章詳述如何使用 Azure CLI 來調整 Linux VM 的大小。 
...
1. 使用 [az vm list-vm-resize-options](/cli/azure/vm) 檢視裝載 VM 之硬體叢集上可用的 VM 大小清單。 下列範例會針對資源群組 `myVM` 區域中名為 `myResourceGroup` 的 VM 列出 VM 大小：
...
3. 如果未列出所需的 VM 大小，您需要先使用 [az vm deallocate](/cli/azure/vm) 解除配置 VM。 此程序可讓 VM 的大小調整為區域支援的任何可用大小，然後啟動。 下列步驟會解除配置、調整大小，然後啟動 `myVM` 資源群組中名為 `myResourceGroup` 的 VM：
