...
如果您需要 X11 伺服器遠端連線到 NV VM，建議使用 [x11vnc](http://www.karlrunge.com/x11vnc/)，因為它可讓圖形進行硬體加速。 M60 裝置的 BusID 必須手動加入至 X11 組態檔 (通常是 `etc/X11/xorg.conf`)。 新增類似下列的 `"Device"` 區段：
