...
若要確認 VM 的受控磁碟位於可用性區域，請使用 [az vm show](/cli/azure/vm) 命令來傳回磁碟識別碼。在此範例中，磁碟會儲存在變數中並使用於稍後的步驟。 
...
使用 [az vm list-ip-addresses](/cli/azure/vm) 命令，以傳回 *myVM* 中公用 IP 位址資源的名稱。 在此範例中，名稱會儲存在變數中並使用於稍後的步驟。
