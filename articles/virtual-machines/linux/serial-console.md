...
序列主控台文字只會佔用部分的螢幕大小 (通常在使用文字編輯器之後) | 這是未知的螢幕大小在使用序列連線時的已知問題。 建議您安裝 xterm 或其他提供「調整大小」命令的類似公用程式。 執行「調整大小」即可修正此問題。
...
1. 序列主控台並沒有和 SSH 一樣多的頻寬 - 它是純文字的連線，因此很難在序列主控台中進行重度使用 GUI 的互動。
