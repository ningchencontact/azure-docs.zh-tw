...
---
# <a name="how-the-azure-reservation-discount-is-applied-to-virtual-machines"></a>如何將 Azure 保留折扣套用至虛擬機器

購買 Azure 保留的虛擬機器執行個體之後，保留折扣會自動套用至符合保留之屬性和數量的虛擬機器。 保留會涵蓋虛擬機器的計算成本。

保留折扣適用于您從 Azure Marketplace 購買的基礎 Vm。
...
保留折扣是「*使用或遺失-it*」。 因此, 如果您有任何一小時沒有相符的資源, 就會遺失該小時的保留數量。 您無法繼續執行未使用的保留時數。
...
1. 任何高於保留線的使用量都會依一般隨用隨付費率計費。 您不用支付低於保留線的使用費用，因為此費用已涵蓋於所購買的保留內。
...
## <a name="discount-can-apply-to-different-sizes"></a>折扣可適用于不同的大小
...
## <a name="discount-applies-to-matching-servicetype-only"></a>折扣僅適用于相符的 ServiceType

保留折扣僅適用于 VM 使用量, 其中的`ServiceType` `AdditionalInfo`值符合所購買的保留。 保留折扣應用程式會忽略用於 Vm 的計量, 而且`ServiceType`只會評估。 瞭解您購買 VM 的服務類型。 您可以將非 premium 儲存體 VM 保留專案交換為 premium 儲存體保留, 或以相反方式進行。
...
如需實例大小彈性的詳細資訊, 請參閱[使用保留的 VM 實例的虛擬機器大小彈性](../virtual-machines/windows/reserved-vm-instance-size-flexibility.md)。
