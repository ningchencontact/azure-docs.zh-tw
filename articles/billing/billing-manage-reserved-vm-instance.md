---
title: 管理 Azure 保留的 VM 執行個體 | Microsoft Docs
...
ms.locfileid: "37064414"
---
# <a name="manage-reserved-instances-in-azure"></a>管理 Azure 中的保留執行個體
...
3. 選取保留執行個體。
4. 選取 [設定] > [組態]。
5. 變更範圍。 如果您從共用變更為單一範圍，您只能選取擁有者是您的訂用帳戶。 只能選取與保留執行個體屬於相同計費內容的訂用帳戶。 計費內容取決於您購買保留執行個體時選取的訂用帳戶。 該範圍僅適用於預付型方案供應項目的 MS-AZR-0003P 訂用帳戶與企業供應項目的 MS-AZR-0017P 訂用帳戶。 對於企業合約，開發/測試訂用帳戶沒有取得保留執行個體折扣的資格。
