...
您可以在 Azure 入口網站使用預算[預算功能](../cost-management/tutorial-acm-create-budgets.md)在成本管理中。 或者，您可以建立使用 REST Api、 Powershell cmdlet 的預算，或使用 CLI。 下列程序會使用 REST API。 在呼叫 REST API 之前，您將會需要授權權杖。 若要建立授權權杖，您可以使用 [ARMClient](https://github.com/projectkudu/ARMClient) \(英文\) 專案。 **ARMClient** 可讓您將自己驗證至 Azure Resource Manager，並取得呼叫 API 的權杖。
...
10. 選取 [raw] \(原始\) 按鈕選項。
11. 在文字方塊中，貼上下方範例預算定義，不過，您必須取代**subscriptionid**， **budgetname**，並**actiongroupname**參數搭配您訂用帳戶識別碼、 您的預算，唯一的名稱和您在同時擁有 URL 和要求本文中建立動作群組名稱：
