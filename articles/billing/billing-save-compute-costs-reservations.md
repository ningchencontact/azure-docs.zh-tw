---
title: 什麼是 Azure 保留？ | Microsoft Docs
...
---
# <a name="what-are-azure-reservations"></a>什麼是 Azure 保留？

Azure 保留可協助您透過預先支付一年或三年期的虛擬機器或 SQL 資料庫計算容量來節省成本。 預先支付費用可讓您在所使用的資源上取得折扣。 Azure 保留可以大幅降低虛擬機器或 SQL 資料庫的計算成本。透過預付一年或三年期的承諾用量費用，即可節省高達預付型方案價格的 72%。 保留會提供計費折扣，且不會影響虛擬機器或 SQL 資料庫的執行階段狀態。
...
如果您有長期執行的虛擬機器或 SQL 資料庫，則購買保留可提供您最符合成本效益的選項。 例如，如果您持續在未使用保留的情況下執行某個服務的四個執行個體，便必須以預付型方案費率支付費用。 如果您針對那些資源購買保留，便能立即取得保留折扣。 那些資源將不再以預付型方案費率計費。
...
- 
  [隨用隨付](https://azure.microsoft.com/offers/ms-azr-0003p/)訂用帳戶供應項目類型 (MS-AZR-003P)。 您必須具有訂用帳戶的「擁有者」角色才能購買保留。 如果要在 Enterprise 註冊中購買保留，企業系統管理員必須在 EA 入口網站中啟用保留購買。 此設定預設為啟用狀態。
- 雲端解決方案提供者 (CSP) 合作夥伴可以使用 Azure 入口網站或[合作夥伴中心](https://docs.microsoft.com/partner-center/azure-reservations)來購買 Azure 保留。 

Azure 保留折扣僅適用於與 Enterprise、預付型方案，或 CSP 訂用帳戶類型相關聯的虛擬機器或 SQL 資料庫。
...
保留折扣會套用至符合您於購買保留時所選取之屬性的資源使用量。 屬性包含相符的 VM、SQL 資料庫或其他資源所執行的範圍。 例如，如果您想要針對美國西部區域的四個標準 D2 虛擬機器取得保留折扣，請選取這些 VM 執行所在的訂用帳戶。 如果虛擬機器是在註冊/帳戶內的不同訂用帳戶中執行，則選取共用的範圍。 共用的範圍可讓保留折扣套用到整個訂用帳戶。 您可以在購買保留之後變更範圍。 如需詳細資訊，請參閱[在 Azure 中管理保留](billing-manage-reserved-vm-instance.md)。

保留折扣僅適用於與 Enterprise 或預付型方案訂用帳戶類型相關聯的虛擬機器或 SQL 資料庫。 在具有其他優惠類型的訂用帳戶中執行的虛擬機器或 SQL 資料庫，將無法收到保留折扣。 針對 Enterprise 註冊，企業開發/測試訂用帳戶不具備獲得保留權益的資格。

若要深入了解保留如何影響虛擬機器或 SQL 資料庫的計費，請參閱[了解保留折扣的套用方式](billing-understand-vm-reservation-charges.md)。

## <a name="what-happens-when-the-reservation-term-expires"></a>保留期限到期時會發生什麼事？

在保留期限結束時，計費折扣會到期，而虛擬機器、SQL 資料庫或其他資源將會改以預付型方案費率計費。 Azure 保留並不會自動更新。 若要繼續獲得計費折扣，您必須針對可使用保留的服務購買新的保留。
...
- [了解保留折扣的套用方式](billing-understand-vm-reservation-charges.md)
- [了解預付型方案訂用帳戶的保留使用量](billing-understand-reserved-instance-usage.md)
- [了解 Enterprise 註冊的保留使用量](billing-understand-reserved-instance-usage-ea.md)
- [Windows 軟體的成本不包括在保留內](billing-reserved-instance-windows-software-costs.md)
- [合作夥伴中心雲端解決方案提供者 (CSP) 計畫中的 Azure 保留](https://docs.microsoft.com/partner-center/azure-reservations)
