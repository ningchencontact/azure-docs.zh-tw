---
title: 使用 Azure 保留容量節省 SQL 資料倉儲費用的成本
description: 瞭解如何節省保留容量 SQL 資料倉儲費用的成本，以節省金錢。
services: billing
author: yashesvi
manager: yashar
ms.service: billing
...
---
# <a name="save-costs-for-sql-data-warehouse-charges-with-reserved-capacity"></a>節省保留容量 SQL 資料倉儲費用的成本

您可以藉由在一或三年期間內 cDWU 使用量的保留，來節省 Azure SQL 資料倉儲成本。 若要購買 SQL 資料倉儲保留容量，您需要選擇 Azure 區域和條款。 接著，將 SQL 資料倉儲 SKU 新增到您的購物車，並選擇您想要購買的 cDWU 單位數量。
...
您可以在[Azure 入口網站](https://portal.azure.com/#blade/Microsoft_Azure_Reservations/ReservationsBrowseBlade)中購買 Azure SQL 資料倉儲保留容量。 預先支付保留期，[或按月付款](billing-monthly-payments-reservations.md)。 若要購買保留容量：
...
2. 選取 [所有服務] > [保留]。
3. 選取一個訂用帳戶。 使用 [訂用帳戶] 清單選擇用來支付保留容量的訂用帳戶。 訂用帳戶的付款方法會收取保留容量的費用。 訂用帳戶類型必須是 Enterprise 合約 (供應項目號碼：MS-AZR-0017P 或 MS-AZR-0148P) 或隨用隨付 (供應項目號碼：MS-AZR-0003P 或 MS-AZR-0023P)。
