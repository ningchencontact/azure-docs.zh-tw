...
ms.locfileid: "57570057"
---
# <a name="show-directions-from-a-to-b"></a>顯示從甲地到乙地的指示

本文說明如何提出路線要求，並在地圖上顯示路線。

有兩種方式可以這麼做。 第一個方式是透過服務模組查詢 [Azure 地圖服務路線 API](https://docs.microsoft.com/rest/api/maps/route/getroutedirections)。 第二種方式是利用[擷取 API](https://fetch.spec.whatwg.org/)提出搜尋要求以[Azure 地圖服務路線 API](https://docs.microsoft.com/rest/api/maps/route/getroutedirections)。 下文將討論這兩種方式。
...
最後一個區塊的程式碼會利用[擷取 API](https://fetch.spec.whatwg.org/)提出搜尋要求以[Azure 地圖服務路線 API](https://docs.microsoft.com/rest/api/maps/route/getroutedirections)。 它會接著剖析傳入的回應。 對於成功的回應，收集與路由中的每個點的緯度和經度資訊建立連接這些點的行陣列。 然後它會將所有線條全都新增到 dataSource，以在地圖上呈現路線。 如需相關指示，您可以查看[在地圖上新增線條](./map-add-shape.md#addALine)。
