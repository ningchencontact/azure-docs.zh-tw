...
| `['get', string]` | 值 | 從目前功能的屬性取得屬性值。 如果要求的屬性遺失，則傳回 null。 |
| `['get', string, object]` | 值 | 從提供之物件的屬性取得屬性值。 如果要求的屬性遺失，則傳回 null。 |
| `['has', string]` | 布林值 | 判斷功能的屬性是否具有指定的屬性。 |
...
| 運算是 | 傳回類型 | 描述 |
|------------|-------------|-------------|
| `['! ', boolean]` | 布林值 | 邏輯否定。 如果輸入 `false`，則傳回 `true`，如果輸入 `true`，則傳回 `false`。 |
| `['!= ', value, value]` | 布林值 | 如果輸入值不相等，則傳回 `true`，否則 `false`。 |
| `['<', value, value]` | 布林值 | 如果第一個輸入嚴格小於第二個輸入，則會傳回 `true`，否則會 `false`。 引數必須是字串或兩個數字。 |
| `['<=', value, value]` | 布林值 | 如果第一個輸入小於或等於第二個輸入，則會傳回 `true`，否則會傳回 `false`。 引數必須是字串或兩個數字。 |
| `['==', value, value]` | 布林值 | 如果輸入值相等，則傳回 `true`，否則傳回 `false`。 引數必須是字串或兩個數字。 |
| `['>', value, value]` | 布林值 | 如果第一個輸入嚴格大於第二個輸入，則會傳回 `true`，否則會 `false`。 引數必須是字串或兩個數字。 |
| `['>=' value, value]` | 布林值 | 如果第一個輸入大於或等於第二個輸入，則傳回 `true`，否則傳回 `false`。 引數必須是字串或兩個數字。 |
| `['all', boolean, boolean, …]` | 布林值 | 如果所有輸入都 `true`，則會傳回 `true`，否則會傳回 `false`。 |
| `['any', boolean, boolean, …]` | 布林值 | 如果有任何輸入 `true`，則會傳回 `true`，否則會傳回 `false`。 |
...
| `['to-boolean', value]` | 布林值 | 將輸入值轉換為布林值。 當輸入是空字串、`0`、`false`、`null`或 `NaN`時，會產生 `false` 結果。否則其 `true`。 |
...
| 運算是 | 傳回類型 | 描述 |
|------------|-------------|-------------|
| `['concat', string, string, …]` | 字串 | 將多個字串串連在一起。 每個值都必須是字串。 如有需要，請使用 `to-string` 類型運算式將其他實數值型別轉換成字串。 |
| `['downcase', string]` | 字串 | 將指定的字串轉換為小寫。 |
...
| `['var', name: string]` | any | 參考使用 `let` 運算式建立的變數。 |
