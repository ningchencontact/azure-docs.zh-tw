...
除了可從 IoT Central 網站建立和管理 IoT Central 應用程式外，您也可以從 Azure 入口網站管理 IoT Central。 本文將逐步解說可能的內容以及如何操作。

## <a name="create-iot-central-applications"></a>建立 IoT Central 應用程式
若要建立新的應用程式，請巡覽至[Azure 入口網站](https://ms.portal.azure.com)並按一下左側主導覽功能表中的 [建立資源]。 

![管理入口網站：導覽功能表](media\howto-manage-iot-central-from-portal\image0.png)

在搜尋列鍵入 "IoT Central" 一詞。

![管理入口網站：搜尋](media\howto-manage-iot-central-from-portal\image0a.png)

按一下搜尋結果中的 IoT Central 應用程式明細項目。

![管理入口網站：搜尋結果](media\howto-manage-iot-central-from-portal\image0b.png)

按一下 [建立] 按鈕以查看您需要填寫的表單。

![管理入口網站：IoT Central 資源](media\howto-manage-iot-central-from-portal\image0c.png)

填寫表單中的所有欄位。 此表單類似於您需要填寫以從 IoT Central 網站建立應用程式的表單。 若要深入了解如何填寫每個欄位，請參閱[建立 IoT Central 應用程式](quick-deploy-iot-central.md)快速入門。 
...
如果您已有 Azure IoT Central 應用程式，則可以將其刪除並移至 Azure 入口網站中的其他訂用帳戶或資源群組。 您無法查看 Azure 入口網站中的試用版應用程式，因為沒有訂用帳戶支援這些試用版。

若要開始使用，請按一下左側主導覽功能表中的 [所有資源]。 使用搜尋方塊鍵入您應用程式的名稱，並在您的資源清單中找到該名稱。 然後，按一下您想要管理的 IoT Central 應用程式。
...
若要將應用程式移動至不同的資源群組，請按一下資源群組旁邊的**變更**連結。 在出現的對話方塊中，選擇要將此應用程式遷移至的資源群組。
