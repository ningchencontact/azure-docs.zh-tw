...
1. 如果您尚未建立任何規則，您會看到下列畫面：
    ...
    ![還沒有規則](media/howto-create-event-rules-experimental/Rules_Landing_Page.png)
...
1. 按一下 [事件] 圖格，以建立事件監視規則。
    ...
    ![規則類型](media/howto-create-event-rules-experimental/Rule_Types.png)
...
1. 若要立即為透過此範本所建立的所有裝置啟用規則，請切換 [為此範本的所有裝置啟用規則]。
    ...
    ![規則詳細資訊](media/howto-create-event-rules-experimental/Rule_Detail.png)
...
1. 從量測下拉式清單中選擇想要監視的事件。 此範例選取了**風扇馬達錯誤**事件。
   ...
   ![條件](media/howto-create-event-rules-experimental/Condition_Filled_Out.png)
...
1. (選擇性) 您也可以將 [計數] 設定為 [彙總]，並提供對應的閾值。
    ...
    ![新增事件條件](media/howto-create-event-rules-experimental/Aggregate_Condition_Filled_Out.png)
...
1. 選擇 [動作] 旁的 [+]。 在此，您會看到可用動作的清單。
    ...
    ![新增動作](media/howto-create-event-rules-experimental/Add_Action.png)

1. 選擇 [傳送電子郵件] 動作，在 [收件者] 欄位中輸入有效的電子郵件地址，然後提供當此規則觸發時要在電子郵件本文中出現的附註。

    > [!NOTE]
    > 只有已新增至應用程式且至少已登入一次的使用者才會收到電子郵件。 深入了解 Azure IoT Central 中的[使用者管理](howto-administer-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)。

   ![設定動作](media/howto-create-event-rules-experimental/Configure_Action.png)
...
- 觸發規則時，[Microsoft Flow 動作](howto-add-microsoft-flow-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)會啟動 Microsoft Flow 中的工作流程 
- 而 [Webhook 動作](howto-create-webhooks-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)會在觸發規則時，通知其他服務
...
- [在規則中新增 Microsoft Flow 動作](howto-add-microsoft-flow-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)
- [在規則中新增 Webhook 動作](howto-create-webhooks-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)
- [如何管理您的裝置](howto-manage-devices-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)
