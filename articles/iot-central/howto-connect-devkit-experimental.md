...
1. Azure IoT Central 應用程式是從**範例 Devkits** 應用程式範本建立而來。 如需詳細資訊，請參閱[建立應用程式快速入門](quick-deploy-iot-central-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)。
...
1. 從 Device Explorer 中新增 [真實裝置]，按一下 [+ 新增] > [實際] 以新增實際裝置。
    ...
    * 輸入裝置名稱，或使用建議的名稱
    ...
    ![新增裝置](media/howto-connect-devkit-experimental/add-device.png)

1. 取得所新增裝置的連線詳細資料 (例如**範圍識別碼、裝置識別碼和主要金鑰**)，方法是按一下裝置頁面上的 [連線]。

    ![連線詳細資料](media/howto-connect-devkit-experimental/device-connect.png)
...
1. 開啟網頁瀏覽器並巡覽至 [http://192.168.0.1/start](http://192.168.0.1/start)。 以下網頁隨即顯示：
    ...
    ![裝置設定頁面](media/howto-connect-devkit-experimental/configpage.png)
...
1. 選擇 [設定裝置] 之後，您會看到這個頁面：
    ...
    ![已設定裝置](media/howto-connect-devkit-experimental/deviceconfigured.png)
...
1. 使用 [裝置總管]，瀏覽至您所新增真實 MXChip 裝置的 [量值] 頁面：
    ...
    ![瀏覽至真實裝置](media/howto-connect-devkit-experimental/realdevicenew.png)

1. 在 [量值] 頁面上，您可以看到來自 MXChip 裝置的遙測：

    ![從真實裝置檢視遙測](media/howto-connect-devkit-experimental/devicetelemetrynew.png)

1. 在 [屬性] 頁面上，您可以檢視裝置所報告的最後一個印模號碼和裝置位置：

    ![檢視裝置屬性](media/howto-connect-devkit-experimental/devicepropertynew.png)

1. 在 [設定] 頁面上，您可以在 MXChip 裝置上更新設定：

    ![檢視裝置設定](media/howto-connect-devkit-experimental/devicesettingsnew.png)

1. 在 [儀表板] 頁面上，您可以看到位置地圖

    ![檢視裝置儀表板](media/howto-connect-devkit-experimental/devicedashboardnew.png)
