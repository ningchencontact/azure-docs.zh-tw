...
1. 在 IoT Central 應用程式中，新增並驗證您要用來產生裝置憑證_的中繼或根 x.509 憑證_：
    ...
    - 流覽至 **[系統管理] > [裝置連線] [> 憑證（x.509）** ]，然後新增您用來產生分葉裝置憑證的 x.509 根或中繼憑證。
      ...
      ![連線設定](media/overview-iot-central-get-connected-pnp/connection-settings.png)
    ...
    - 驗證憑證擁有權可確保憑證的上傳者擁有憑證的私密金鑰。 若要驗證憑證：
        ...
        - 上傳已簽署的驗證憑證，然後選取 [**驗證**]。
          ...
          ![連線設定](media/overview-iot-central-get-connected-pnp/verify-cert.png)
...
![連線設定](media/overview-iot-central-get-connected-pnp/device-connection-flow1.png)
...
1. 設定您的連線設定：
    ...
    ![連線設定 SAS](media/overview-iot-central-get-connected-pnp/connection-settings-sas.png)
...
## <a name="individual-enrollment-based-device-connectivity"></a>以註冊為基礎的個別裝置連線能力
