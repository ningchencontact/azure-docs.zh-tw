---
title: 教學課程：在 Azure IoT Central 中定義新的裝置類型
...
若要完成本教學課程，您需要根據**自訂應用程式**範本建立的 Azure IoT Central 應用程式。 如果您沒有應用程式，請完成[建立 Azure IoT Central 應用程式](quick-deploy-iot-central.md)快速入門，確定您選擇**自訂應用程式**範本。
...
若要將新的裝置範本新增至應用程式，您必須移至 [裝置範本]  頁面。 若要這樣做，請選取左側窗格上的 [裝置範本]  。
