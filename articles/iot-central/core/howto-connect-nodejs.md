...
記下 [**裝置**連線] 頁面上的裝置連線資訊： [**範圍識別碼**]、[**裝置識別碼**] 和 [**主要金鑰**]。 您稍後會在本操作指南中，將這些值新增至您的裝置程式碼：
...
1. 將下列變數宣告新增至該檔案：
    ...
    使用您先前所記下的值，來更新預留位置 `{your Scope ID}`、`{your Device ID}`和 `{your Primary Key}`。 在此範例中，您會將 `targetTemperature` 初始化為零，您可以使用裝置的目前讀取或裝置對應項的值。
