...
若要深入了解 Azure IoT 中心的裝置連線能力，請參閱[裝置連線能力](overview-iot-central-get-connected.md)。
...
- **裝置功能模型**會指定裝置的功能，例如它所傳送的遙測、定義裝置狀態的屬性，以及裝置所回應的命令。 裝置功能會組織成一或多個介面。 如需裝置功能模型的詳細資訊，請參閱[IoT 隨插即用](../../iot-pnp/overview-iot-plug-and-play.md)檔。
...
在 Azure IoT Central 應用程式中，您可以[建立和執行作業](../core/howto-run-a-job.md?toc=/azure/iot-central/preview/toc.json&bc=/azure/iot-central/preview/breadcrumb/toc.json)來管理已連線的裝置。 這些作業可讓您對裝置屬性或設定進行大量更新，或執行命令。 例如，您可以建立作業來增加多個冷飲自動販賣機器的風扇速度。
...
既然您已瞭解 Azure IoT Central 的架構，建議的下一個步驟是瞭解 Azure IoT Central 中的[裝置連線能力](overview-iot-central-get-connected.md)。
