...
1. 在您的裝置程式碼中使用這些連線詳細資料 (**裝置身分識別、裝置名稱和裝置主要金鑰**) 來佈建裝置並加以連線，並立即開始看到資料流通。 如果您正在使用 MxChip 裝置，請遵循[這裡的逐步指示](howto-connect-devkit-experimental.md#add-a-real-device)，從＜準備 DevKit 裝置＞一節開始。   
    ...
    *   **Node.js：** 如果您想要使用 Node.js，請[使用這裡的逐步指示](tutorial-add-device-experimental.md#prepare-the-client-code)，從＜準備用戶端程式碼＞一節開始。
...
如果您正在使用 **MxChip** 裝置進行連線，請遵循[這裡的逐步指示](howto-connect-devkit-experimental.md#add-a-real-device)，從＜準備 DevKit 裝置＞一節開始。   
...
- [將一般 Node.js 用戶端連線至 Azure IoT Central 應用程式](howto-connect-nodejs-experimental.md)
- [將 Raspberry Pi 裝置連線到 Azure IoT Central 應用程式](howto-connect-raspberry-pi-python.md)
- [將 DevDiv kit 裝置連線到 Azure IoT Central 應用程式](howto-connect-devkit-experimental.md)
...
- [準備及連線 DevKit 裝置](howto-connect-devkit-experimental.md)
