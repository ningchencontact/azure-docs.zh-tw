...
在您的 Azure IoT Central 應用程式中, 從**MXChip**裝置範本新增真實裝置, 並記下裝置連線詳細資料:**範圍識別碼、裝置識別碼和主要金鑰**:
...
使用 Visual Studio Code 開啟`iot-central-firmware`資料夾中`MXCHIP/mxchip_advanced`的資料夾:

![Visual Studio Code](media/howto-connect-devkit/vscodeview.png)

若要查看如何將遙測傳送至 Azure IoT Central 應用程式, 請開啟 `src`資料夾中的遙測檔案:

- 此函式 `TelemetryController::buildTelemetryPayload` 會使用裝置上感應器的資料，建立 JSON 遙測承載。

- 函式會`sendTelemetry`在**AzureIOTClient**中呼叫,將JSON承載傳送至AzureIoTCentral應用程式所使用的IoT中樞。`TelemetryController::sendTelemetryPayload`

若要查看如何向 Azure IoT Central 應用程式報告屬性值, 請開啟 `src`資料夾中的遙測檔案:

- 函式會大約每隔30秒傳送位置報告屬性。 `TelemetryController::loop` 它會使用`sendReportedProperty` **AzureIOTClient**檔中的函式。

- 當裝置加速計偵測到按兩下時, 此函式會傳送 dieNumber 報告屬性。 `TelemetryController::loop` 它會使用`sendReportedProperty` **AzureIOTClient**檔中的函式。
...
- **DmCountdown**函數是**倒數**命令的處理常式。 它會變更裝置 LED 的色彩, 並使用報告屬性將倒數值傳送回 IoT Central 應用程式。 報告屬性的名稱與命令相同。 函式會使用`sendReportedProperty` **AzureIOTClient**原始檔案中的函式。
...
| Display name | 欄位名稱 | 傳回類型 | 輸入欄位顯示名稱 | 輸入功能變數名稱 | 輸入欄位類型 |
| ------------ | ---------- | ----------- | ------------------------ | ---------------- | ---------------- |
| 回顯         | 回應       | 文字        | 要顯示的值         | displayedValue   | 文字             |
| 倒數    | 倒數  | 號      | 計數來源               | countFrom        | 號           |
