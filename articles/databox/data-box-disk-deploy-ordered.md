...
6.  在 [訂單]  中，指定 [訂單詳細資料]  。 輸入或選取下列資訊。
    ...
    針對受控磁碟指定的儲存體帳戶不能當成暫存的儲存體帳戶來使用。 資料箱服務會將 VHD 上傳至暫存儲存體帳戶，然後再將其轉換為受控磁碟並移至資源群組。 如需詳細資訊，請參閱[確認資料上傳至 Azure](data-box-disk-deploy-upload-verify.md#verify-data-upload-to-azure)。
