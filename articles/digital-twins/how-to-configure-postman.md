...
此文章說明如何設定 Azure Active Directory (Azure AD) 應用程式以使用 OAuth 2.0 隱含授權流程。 接著，將討論如何設定 Postman REST 用戶端，以向您的管理 API 提出權杖關聯的 HTTP 要求。
...
設定您的 Azure AD 應用程式以使用 OAuth 2.0 隱含授權流程。
...
1. 將**回覆 URL** 設定為 [`https://www.getpostman.com/oauth2/callback`](https://www.getpostman.com/oauth2/callback)。

      ![Azure AD 回覆 URL][2]

1. 複製並保存 Azure AD 應用程式的**應用程式識別碼**。 您將在下列內容中用到它。

## <a name="configure-the-postman-client"></a>設定 Postman 用戶端

接下來，安裝並設定 Postman 以取得 Azure AD 權杖。 之後，使用取得的權杖來向 Azure Digital Twins 提出已驗證的 HTTP 要求：
