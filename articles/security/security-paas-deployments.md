...
**詳細資料**：遺失金鑰和認證是相當常見的問題。 您可以使用集中式的解決方案，在硬體安全性模組 (Hsm) 中儲存金鑰和祕密。 [Azure Key Vault](../key-vault/key-vault-whatis.md)透過加密驗證金鑰、 儲存體帳戶金鑰、 資料加密金鑰、.pfx 檔案，以及使用 Hsm 所保護的金鑰的密碼來保護您的金鑰和祕密。
