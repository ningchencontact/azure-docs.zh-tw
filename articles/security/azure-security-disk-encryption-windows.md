...
ms.locfileid: "39392575"
---
#  <a name="enable-azure-disk-encryption-for-windows-iaas-vms"></a>啟用 Windows IaaS VM 適用的 Azure 磁碟加密 

您可以啟用許多磁碟加密案例，步驟可能會因案例而有所不同。 下列各節涵蓋更詳細的 Windows IaaS VM 案例。 您必須先完成 [Azure 磁碟加密必要條件](/articles/security/azure-security-disk-encryption-prerequisites.md)，才能您使用磁碟加密。 
...
2. 選取訂用帳戶、資源群組、資源群組位置、參數、法律條款及協議。 按一下 [建立] 以在新 IaaS VM 上啟用加密。
...
您可以使用用戶端憑證驗證 (不管是否搭配 KEK)。 指令碼需要完成 [Azure 磁碟加密必要條件](azure-security-disk-encryption-prerequisites.md)。 使用 PowerShell 指令碼之前，您應該已經有上傳至金鑰保存庫並部署到 VM 的憑證。 如果您也使用 KEK，KEK 應已存在。 如需詳細資訊，請參閱先決條件一文的 [Azure AD 的憑證式驗證](azure-security-disk-encryption-prerequisites.md#bkmk_Cert)一節。
