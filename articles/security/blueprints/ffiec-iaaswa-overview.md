---
title: Azure 安全性與合規性藍圖 - 適用於 FFIEC 的 IaaS Web 應用程式
description: Azure 安全性與合規性藍圖 - 適用於 FFIEC 的 IaaS Web 應用程式
services: security
author: meladie
...
ms.locfileid: "71257192"
---
# <a name="azure-security-and-compliance-blueprint-iaas-web-application-for-ffiec-financial-services"></a>Azure 安全性與合規性藍圖：適用于 FFIEC 金融服務的 IaaS Web 應用程式
...
整個解決方案都是建立在 Azure 儲存體的基礎上，而客戶會從 Azure 入口網站進行設定。 Azure 儲存體會使用儲存體服務加密來加密所有資料，以維護待用資料的機密性。 異地備援儲存體可確保資料會複寫到數百英哩遠的次要資料中心，並在該資料中心儲存為三份複本，防止客戶主要資料中心內的不良事件導致資料遺失。
...
![適用於 FFIEC 的 IaaS Web 應用程式參考架構圖表](images/ffiec-iaaswa-architecture.png "適用於 FFIEC 的 IaaS Web 應用程式參考架構圖表")
...
請參閱使用此解決方案部署之[網路安全性群組](https://github.com/Azure/fedramp-iaas-webapp/blob/master/nestedtemplates/virtualNetworkNSG.json)的設定。 組織可以使用[此文件](../../virtual-network/virtual-network-vnet-plan-design-arm.md)當作指南來編輯上述的檔案，以設定網路安全性群組。
...
此參考架構的資料流程圖可供[下載](https://aka.ms/ffiec-iaaswa-tm)，或可以在以下位置找到。 此模型可協助客戶在進行修改時，了解系統基礎結構中的潛在風險要點。

![適用於 FFIEC 的 IaaS Web 應用程式威脅模型](images/ffiec-iaaswa-threat-model.png "適用於 FFIEC 的 IaaS Web 應用程式威脅模型")
...
[Azure 安全性與合規性藍圖 - FFIEC IaaS Web 應用程式實作對照表](https://aka.ms/ffiec-iaaswa-cim)提供 IaaS Web 應用程式架構處理了哪些 FFIEC 要求的相關資訊，包括詳細說明此實作如何符合每個涵蓋目標的要求。
