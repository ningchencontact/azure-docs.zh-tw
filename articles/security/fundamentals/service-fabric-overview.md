...
您在透過 Azure 入口網站、Azure Resource Manager 範本或獨立 JSON 範本建立叢集時，會設定憑證安全性。 您可以指定用於憑證變換的主要憑證和選用次要憑證。 您指定的主要和次要憑證，應該不同於您為[用戶端對節點安全性](../../service-fabric/service-fabric-cluster-security.md)指定的系統管理員用戶端憑證和唯讀用戶端憑證。
