...
- [Azure 事件中樞](/azure/event-hubs/event-hubs-what-is-event-hubs)
- [Azure 記錄整合](azure-log-integration-overview.md)
...
* 如果您將提供遙測資訊，請將核取方塊保留為選取。 如果您不想要將使用情況資訊傳送給 Microsoft，請清除核取方塊。
   ...
   如需 Azure 記錄整合和安裝方式的詳細資訊，請參閱[使用 Azure 診斷記錄和 Windows 事件轉送進行 Azure 記錄整合](azure-log-integration-get-started.md)。
...
* 最新的 PowerShell 版本。
   ...
   至少擁有 PowerShell 5.0 之後，您可以遵循[安裝 Azure PowerShell](/powershell/azure/install-az-ps)中的指示，繼續安裝最新版本。
...
1. 透過執行指令碼 LoadAzLogModule.ps1，匯入 AzLog Cmdlet。 輸入 `.\LoadAzLogModule.ps1` 命令 (請注意該命令中的 ".\")。您應該會看到如下的結果：</br>
   ...
   ![載入的模組清單](./media/azure-log-integration-keyvault-eventhub/loaded-modules.png)
...
1. 成功驗證之後，您就會登入。 由於您在後續步驟中需要使用訂用帳戶識別碼和訂用帳戶名稱，因此請記下這些資訊。
...
1. 建立資源群組：
   ...
   ![建立資源群組之後的輸出](./media/azure-log-integration-keyvault-eventhub/create-rg.png)
1. 建立將用於追蹤狀態資訊的儲存體帳戶：
...
1. 取得所有可能的 Azure 位置，並將名稱新增至可用於後續步驟的變數：
    ...
    如果您在`$locations`此時輸入，則會看到位置名稱，而不包含 get-azlocation 所傳回的其他資訊。
...
> [!NOTE]
> 嘗試建立記錄設定檔時，您可能會收到錯誤訊息。 接著，您可以查看 AzLogProfile 和 Remove-AzLogProfile 的檔。 如果您執行 AzLogProfile，您會看到記錄檔設定檔的相關資訊。 您可以透過輸入 ```Remove-AzLogProfile -name 'Log Profile Name'``` 命令，刪除現有記錄設定檔。
>
>![資源管理員設定檔錯誤](./media/azure-log-integration-keyvault-eventhub/rm-profile-error.png)
...
1. 設定和讀取密碼以產生其他記錄項目：
   ...
   ![傳回的密碼](./media/azure-log-integration-keyvault-eventhub/keyvaultsecret.png)
...
- [Azure 記錄整合常見問題集](azure-log-integration-faq.md)
- [開始使用 Azure 記錄整合](azure-log-integration-get-started.md)
- [將記錄從 Azure 資源整合到 SIEM 系統](azure-log-integration-overview.md)
