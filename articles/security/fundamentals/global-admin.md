...
此外，為系統管理員啟用 [Azure 多重要素驗證 (MFA)](../../active-directory/authentication/multi-factor-authentication.md) 可以加強防護使用者登入和交易的安全。 Azure MFA 也可協助 IT 降低遭入侵的認證能夠存取公司資料的可能性。
...
在繼續前先檢閱 [MFA](../../active-directory/authentication/howto-mfa-mfasettings.md) 的運作方式。
...
使用[如何要求使用者或群組進行雙步驟驗證](../../active-directory/authentication/howto-mfa-userstates.md)中所列的其中一個程序，開始使用 Azure MFA。 您可以選擇對所有登入強制執行雙步驟驗證, 也可以建立條件式存取原則, 只在對您很重要時才需要雙步驟驗證。
