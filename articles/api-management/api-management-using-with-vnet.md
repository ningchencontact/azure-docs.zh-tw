...
| * / 80, 443 |輸出 |TCP |VIRTUAL_NETWORK / INTERNET|Azure 儲存體、Azure 服務匯流排和 Azure Active Directory 上的相依性 (如果適用)。|外部和內部 | 
...
**透過網路虛擬設備進行路由傳送**：使用 UDR 搭配預設路由 (0.0.0.0/0)，透過在 Azure 上執行的網路虛擬裝置，針對從 API 管理子網路流向網際網路的流量進行路由傳送的設定，將可完全防止 API 管理與所需服務之間的通訊。 不支援這樣的設定。 
