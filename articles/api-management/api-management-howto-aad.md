...
10. 在瀏覽器中開啟其他索引標籤。 
11. 移至 [Azure 入口網站](https://portal.azure.com)。
12. 選取 ![箭號](./media/api-management-howto-aad/arrow.png).
13. 輸入 **active**。 [Azure Active Directory] 窗格隨即出現。
...
25. 選取您的應用程式，然後選取 [讀取目錄資料] 和 [登入及讀取使用者設定檔] 核取方塊。
...
31. 切換回您的 API 管理應用程式。 
    ...
    >  
    >
32. [新增識別提供者] 視窗也包含 [允許的租用戶] 文字方塊。 請在該處指定 Azure AD 執行個體的網域，您將會對它授與 API 管理服務執行個體的 API 存取權。 您可以使用換行符號、空格或逗號來分隔多個網域。
...
2. 輸入 Azure AD 中其中一位使用者的認證，然後選取 [登入]。

   ![以使用者名稱和密碼來登入][api-management-aad-signin]

3. 如果需要其他資訊，可能會出現註冊表單的提示。 完成註冊表單，然後選取 [註冊]。
