...
在您讓 Azure AD 執行個體中的使用者可以存取之後，即可在 API 管理中新增 Azure AD 群組。 之後，您就可以更輕鬆地管理群組中開發人員與產品之間的關聯。
 ...
 > [!IMPORTANT]
 > 若要加入外部 Azure AD 群組, 您必須先遵循上一節中的程式, 在 [身分識別] 索引標籤上設定 Azure AD 實例。 此外, 應用程式必須被授與具有`Directory.Read.All`許可權 Azure AD 圖形 API 的存取權。 
...
## <a name="a-idlog_in_to_dev_portalsign-in-to-the-developer-portal-by-using-an-azure-ad-account"></a><a id="log_in_to_dev_portal"/>使用 Azure AD 帳戶來登入開發人員入口網站
