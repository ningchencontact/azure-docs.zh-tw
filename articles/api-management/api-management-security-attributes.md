...
| API 呼叫加密| 有 | 管理平面呼叫都會經過[Azure Resource Manager](../azure-resource-manager/index.yml)透過 TLS。 需要有效的 JSON Web 權杖 (JWT)。  資料平面的呼叫可以保護使用 TLS 和其中一個支援的驗證機制 （例如，用戶端憑證或 JWT）。
 |
