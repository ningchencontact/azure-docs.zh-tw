...
| ------------ | --------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| check-header | 根元素。                                                                                                                                 | 有      |
| value        | 允許的 HTTP 標頭值。 指定多個值元素時，如果其中任何一個值相符，則會將檢查視為成功。 | 無       |

### <a name="attributes"></a>屬性

| 名稱                       | 描述                                                                                                                                                            | 必要項 | 預設值 |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------- |
| failed-check-error-message | 如果標頭不存在或具有無效值，要在 HTTP 回應本文中傳回的錯誤訊息。 此訊息必須正確逸出任何特殊字元。 | 有      | N/A     |
| failed-check-httpcode      | 標頭不存在或具有無效值時所要傳回的 HTTP 狀態碼。                                                                                        | 有      | N/A     |
| header-name                | 要檢查的 HTTP 標頭名稱。                                                                                                                                  | 有      | N/A     |
| ignore-case                | 可以設定為 True 或 False。 如果設定為 True，則會在標頭值與一組可接受的值進行比較時，忽略大小寫。                                    | 有      | N/A     |
...
> [!CAUTION]
> 由於節流架構的分散式本質，速率限制是永遠不會完全正確。 設定之間的差異，實際允許的數目不同要求根據要求量和速率、 後端延遲，以及其他因素。
...
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| set-limit | 根元素。                                                                                                                                                                                                                                                                                            | 有      |
| API       | 新增一或多個這些元素，以對強加呼叫頻率限制 api 產品內。 產品和 API 呼叫頻率限制會獨立套用。 API 可以透過 `name` 或 `id` 參考。 如果同時提供兩個屬性，則會使用 `id` 而忽略 `name`。                    | 無       |
| 作業 | 新增一或多個這些元素，以對強加呼叫頻率限制對 API 內的作業。 產品、API 和作業呼叫頻率限制會獨立套用。 作業可以透過 `name` 或 `id` 參考。 如果同時提供兩個屬性，則會使用 `id` 而忽略 `name`。 | 無       |

### <a name="attributes"></a>屬性

| 名稱           | 描述                                                                                           | 必要項 | 預設值 |
| -------------- | ----------------------------------------------------------------------------------------------------- | -------- | ------- |
| name           | 要套用速率限制的 API 名稱。                                                | 有      | N/A     |
| calls          | 在 `renewal-period` 中指定的時間週期內允許的呼叫總數上限。 | 有      | N/A     |
| renewal-period | 重設配額的時間週期 (以秒為單位)。                                              | 有      | N/A     |
...
`rate-limit-by-key` 原則藉由將指定時間週期內的呼叫頻率限制為指定次數，以防止每個金鑰的 API 使用量暴增。 密钥的值可以是任意字符串，通常使用策略表达式来提供密钥。 可以新增選擇性增量條件，以指定哪些要求應該計入限制。 觸發此原則時，呼叫者會收到 `429 Too Many Requests` 回應狀態碼。
...
| 名稱                | 描述                                                                                           | 必要項 | 預設值 |
| ------------------- | ----------------------------------------------------------------------------------------------------- | -------- | ------- |
| 呼叫               | 在 `renewal-period` 中指定的時間週期內允許的呼叫總數上限。 | 有      | N/A     |
| counter-key         | 用於頻率限制原則的金鑰。                                                             | 有      | N/A     |
| increment-condition | 此布林運算式指定要求是否應該計入配額 (`true`)。        | 無       | N/A     |
| renewal-period      | 重設配額的時間週期 (以秒為單位)。                                              | 有      | N/A     |
...
| ip-filter action="allow &#124; forbid"    | 指定允許或不允許指定的 IP 位址和範圍進行呼叫。 | 有                                                | N/A     |
...
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| quota     | 根元素。                                                                                                                                                                                                                                                                                | 有      |
| API       | 新增一或多個這些元素，以對強加產品內的 Api 呼叫配額。 產品和 API 呼叫配額會獨立套用。 API 可以透過 `name` 或 `id` 參考。 如果同時提供兩個屬性，則會使用 `id` 而忽略 `name`。                    | 無       |
| 作業 | 新增一或多個這些元素，以對強加呼叫配額 API 內的作業。 產品、API 和作業呼叫配額會獨立套用。 作業可以透過 `name` 或 `id` 參考。 如果同時提供兩個屬性，則會使用 `id` 而忽略 `name`。 | 無       |

### <a name="attributes"></a>屬性

| 名稱           | 描述                                                                                               | 必要項                                                         | 預設值 |
| -------------- | --------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | ------- |
| name           | 套用配額的 API 或作業名稱。                                             | 有                                                              | N/A     |
...
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| validate-jwt        | 根元素。                                                                                                                                                                                                                                                                                                                                         | 有      |
| audiences           | 包含可呈現在權杖上之可接受的受眾宣告清單。 如果存在多個受眾值，則會嘗試每個值，直到全部試完 (即表示驗證失敗) 或其中一個值成功為止。 必須指定至少一個受眾。                                                                     | 無       |
| issuer-signing-keys | 用來驗證已簽署權杖的 Base64 編碼安全性金鑰清單。 如果存在多個安全性金鑰，則會嘗試每個金鑰，直到全部試完 (即表示驗證失敗) 或其中一個金鑰成功 (很適合用於權杖變換) 為止。 金鑰元素具有用來與 `kid` 宣告進行比對的選擇性 `id` 屬性。               | 無       |
| decryption-keys     | 用來將權杖解密的 Base64 編碼金鑰清單。 如果有多個安全性金鑰存在，則系統會嘗試每個金鑰，直到試完所有金鑰 (即表示驗證失敗) 或某個金鑰成功為止。 金鑰元素具有用來與 `kid` 宣告進行比對的選擇性 `id` 屬性。                                                 | 無       |
| issuers             | 可接受之簽發權杖的主體清單。 如果存在多個簽發者值，則會嘗試每個值，直到全部試完 (即表示驗證失敗) 或其中一個值成功為止。                                                                                                                                         | 無       |
| openid-config       | 此元素用於指定可從中取得簽署金鑰和簽發者之符合規範的 Open ID 設定端點。                                                                                                                                                                                                                        | 無       |
| required-claims     | 包含應存在於權杖上才會被視為有效的宣告清單。 當 `match` 屬性設定為 `all` 時，原則中的每個宣告值都必須存在於權杖，才能驗證成功。 當 `match` 屬性設定為 `any` 時，至少一個宣告必須存在於權杖，才能驗證成功。 | 無       |
| zumo-master-key     | Azure 行動服務所簽發之權杖的主要金鑰                                                                                                                                                                                                                                                                                                 | 無       |

### <a name="attributes"></a>屬性

| 名稱                            | 描述                                                                                                                                                                                                                                                                                                                                                                                                                                            | 必要項                                                                         | 預設值                                                                           |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| clock-skew                      | 時間範圍。 用來指定權杖簽發者和 API 管理執行個體的系統時鐘之間最大預期時間差異。                                                                                                                                                                                                                                                                                                               | 無                                                                               | 0 秒                                                                         |
| failed-validation-error-message | 如果 JWT 未通過驗證，在 HTTP 回應主體中傳回的錯誤訊息。 此訊息必須正確逸出任何特殊字元。                                                                                                                                                                                                                                                                                                 | 無                                                                               | 預設錯誤訊息視驗證問題而定，例如「JWT 不存在」。 |
| failed-validation-httpcode      | JWT 未通過驗證時所要傳回的 HTTP 狀態碼。                                                                                                                                                                                                                                                                                                                                                                                         | 無                                                                               | 401                                                                               |
| header-name                     | 保留權杖的 HTTP 標頭名稱。                                                                                                                                                                                                                                                                                                                                                                                                         | 必须指定 `header-name`、`query-parameter-name`、`token-value` 中的一个。 | N/A                                                                               |
| query-parameter-name            | 保留權杖的查詢參數名稱。                                                                                                                                                                                                                                                                                                                                                                                                     | 必须指定 `header-name`、`query-parameter-name`、`token-value` 中的一个。 | N/A                                                                               |
| token-value                     | 一个表达式，返回的字符串包含 JWT 令牌                                                                                                                                                                                                                                                                                                                                                                                                     | 必须指定 `header-name`、`query-parameter-name`、`token-value` 中的一个。 | N/A                                                                               |
| id                              | `key` 元素的 `id` 屬性可讓您指定要與權杖中的 `kid` 宣告 (如果存在) 進行比對的字串，以找出適合用於簽章驗證的金鑰。                                                                                                                                                                                                                                           | 無                                                                               | N/A                                                                               |
| match                           | `claim` 元素的 `match` 屬性可指定原則中的每個宣告值是否都必須存在於權杖，才能驗證成功。 可能的值包括：<br /><br /> - `all` - 原則中的每個宣告值都必須存在於權杖，才能驗證成功。<br /><br /> - `any` - 至少一個宣告必須存在於權杖，才能驗證成功。                                                       | 無                                                                               | 全部                                                                               |
| require-expiration-time         | 布尔值。 指定權杖中是否需有逾期宣告。                                                                                                                                                                                                                                                                                                                                                                               | 無                                                                               | true                                                                              |
| require-scheme                  | 權杖結構描述的名稱，例如"Bearer"。 當已設定此屬性時，原則將會確定指定的結構描述存在於授權標頭值中。                                                                                                                                                                                                                                                                                    | 無                                                                               | N/A                                                                               |
| require-signed-tokens           | 布林值。 指定是否需要簽署權杖。                                                                                                                                                                                                                                                                                                                                                                                           | 無                                                                               | true                                                                              |
| 分隔符號                       | 字串。 指定用於從多重值宣告中擷取一組值的分隔符號 (例如 ",")。                                                                                                                                                                                                                                                                                                                                          | 無                                                                               | N/A                                                                               |
| url                             | 可從中取得 Open ID 設定中繼資料的 Open ID 設定端點 URL。 回應應該根據 URL 所定義的規格：`https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata`。 對於 Azure Active Directory，使用下列 URL：`https://login.microsoftonline.com/{tenant-name}/.well-known/openid-configuration` 代替您的目錄租用戶名稱，例如 `contoso.onmicrosoft.com`。 | 有                                                                              | N/A                                                                               |
output-token-variable-name|字串。 成功进行令牌验证后，将作为 [`Jwt`](api-management-policy-expressions.md) 类型的对象接收令牌值的上下文变量的名称|無|N/A
