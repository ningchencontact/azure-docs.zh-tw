...
<table><tr><td><b>屬性名稱</b></td><td><b>資料類型</b></td><td><b>註解</b></td></tr><tr><td>timestamp</td><td>DateTime</td><td>上次修改項目的時間。 伺服器會在插入項目時以及每次更新項目時產生此欄位。 此屬性值會在輸入發佈作業時遭到忽略。</td></tr><tr><td>ID</td><td>URI</td><td>項目的絕對 URL (唯讀)。 它是項目的唯一可定址 URI。  此屬性值會在輸入發佈作業時遭到忽略。</td></tr><tr><td>type</td><td>String</td><td>資產的類型 (唯讀)。</td></tr><tr><td>Etag</td><td>String</td><td>對應到項目版本的字串，在執行會更新目錄中項目的作業時可用於開放式並行存取控制。 "*" 可用來比對任何值。</td></tr></table>
...
<tr><td></td><td>標籤</td><td>string</td><td>描述資產的標記。</td></tr>
...
<tr><td></td><td>sourceType</td><td>string</td><td>描述資料來源的類型。  例如: SQL Server、Oracle Database 等等。  </td></tr>
<tr><td></td><td>objectType</td><td>string</td><td>描述資料來源中的物件類型。 例如: SQL Server 的資料表、View。</td></tr>

<tr><td>DataSourceLocation</td><td></td><td></td><td></td></tr>
<tr><td></td><td>通訊協定</td><td>string</td><td>必要項。 描述用來與資料來源進行通訊的通訊協定。 例如：適用於 SQl Server 的 "tds"、適用於 Oracle 的 "oracle" 等等。如需目前支援的通訊協定清單，請參閱<a href="https://docs.microsoft.com/azure/data-catalog/data-catalog-dsr">資料來源參考規格 - DSL 結構</a>。</td></tr>
<tr><td></td><td>位址</td><td>字典&lt;字串, 物件&gt;</td><td>必要項。 位址是一組專屬於通訊協定的資料，用來識別所參考的資料來源。 資料將範圍設定為特定通訊協定的位址，表示如果不知道通訊協定就沒有意義。</td></tr>
...
### <a name="roles"></a>角色
有三個角色:**系統管理員**、**擁有**者和**參與者**。  每個角色有其範圍和權限，於下表中摘要說明。
...
> **擁有者** 角色僅適用於根項目。
> 
> 根據預設，在目錄中建立項目時，其 **參與者** 會設定為目前已驗證的使用者。 如果每個人都能更新項目，則第一次發佈項目時，應該在 **roles** 屬性中，將**參與者**設定為 &lt;Everyone&gt; 特殊安全性主體 (請參閱下列範例)。 **參與者**無法變更，而且在項目存留期間都維持不變 (即使**系統管理員**或**擁有者**都沒有權限變更**參與者**)。 **參與者**的明確設定唯一支援的值是&lt;Everyone:&gt;**參與者**只能是建立專案或&lt;每個人&gt;的使用者。
> 
> 
...
> [!NOTE]
> 在 PUT 中, 不需要在主體中指定專案裝載:PUT 只能用來更新角色和 (或) 許可權。
> 
> 
