...
**Azure 資料目錄**讓 Adventure Works 財務團隊合作建立關鍵的資料來源，因此每位團隊成員可以輕鬆地尋找並使用資料，它們需要和可以與整個團隊分享其知識。
...
當您在選擇試驗專案時，請選取其案例最引人注目且能解決現有商務問題的團隊。 例如，商務分析師會從 SQL Server Database 建立報表。 問題在於，它們就以為已經知道資料來源的幾個同事交談後才。 最後，在浪費時間嘗試尋找要使用哪些資料來源後, 他們找到一個 Excel 活頁簿，其中包含的每個資料來源的描述。 雖然 Excel 活頁簿已充分說明所需的資料表，它們就能很快找到這些資料來源如果他們已註冊，而在標註**Azure 資料目錄**。
...
**Azure 資料目錄**所解決的其中一個商務問題是將**資料產生者**連接至**資料取用者**。 其解決方式是作為企業資料來源相關資訊的中央存放庫。 David 使用資料目錄註冊了 Adventure Works 和 SQL Server 資料來源。 使用群眾外包會探索此資料來源的任何使用者可以分享她的意見，在資料上，除了使用這些探索到的資料。 比方說，Nancy 在搜尋目錄後找到資料來源，並分享其關於資料的專業知識。  現在，組織中的其他人可藉由搜尋資料目錄從所分享的知識獲益。
...
在此範例中，商務分析師 Nancy 使用 SQL Server Database 中的資料為其團隊建立報告。 問題在於，它們就以為已經知道資料來源的幾個同事交談後才。 它們就能很快找到這些資料來源如果他們已註冊，然後在中央位置這類標註**Azure 資料目錄**。
