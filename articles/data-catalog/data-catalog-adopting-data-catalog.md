...
**Azure 資料目錄**所解決的其中一個商務問題是將**資料產生者**連接至**資料取用者**。 其解決方式是作為企業資料來源相關資訊的中央存放庫。 David 使用資料目錄註冊了 Adventure Works 和 SQL Server 資料來源。 透過群眾外包，任何找到此資料來源的使用者就能分享其對於資料的意見，而不必使用其找到的資料。 比方說，Nancy 在搜尋目錄後找到資料來源，並分享其關於資料的專業知識。  現在，組織中的其他人可藉由搜尋資料目錄從所分享的知識獲益。
...
> [!NOTE]
> 我們已撰寫使用 **Azure 資料目錄** API 的範例工具，來將 Excel 活頁簿移轉至資料目錄。 若要了解資料目錄 API 和範例工具，請[下載臨機操作的活頁簿程式碼範例](https://azure.microsoft.com/documentation/samples/data-catalog-dotnet-excel-register-data-assets/)，並查看 [Azure 資料目錄 REST API](https://msdn.microsoft.com/library/azure/mt267593.aspx) 文件。
>
>
