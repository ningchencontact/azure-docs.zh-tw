...
Twilio 定價的相關資訊可在[Twilio 定價][twilio_pricing]取得。 Azure 客戶享有[特殊供應][special_offer]專案: 免費點數為1000文字或1000輸入分鐘。 若要註冊此供應專案或取得詳細資訊, 請造訪[https://ahoy.twilio.com/azure][special_offer]。
...
註冊 Twilio 帳戶時，您會收到帳戶識別碼和驗證權杖。 兩者皆為呼叫 Twilio API 所需。 為了防止未經授權存取您的帳戶，您妥善保管驗證權杖。 您的帳戶識別碼和驗證權杖可在[Twilio 主控台][twilio_console]中, 分別在標示為**帳戶 SID**和**驗證權杖**的欄位中查看。
...
如需傳入**呼叫. creator**方法之參數的詳細資訊, 請參閱[https://www.twilio.com/docs/api/rest/making-calls][twilio_rest_making_calls]。
...
**ApiVersion** 參數會出現在 Twilio 語音要求 (而非 SMS 要求) 中。 若要查看 Twilio 語音和 SMS 要求的可用要求參數，請分別參閱 <https://www.twilio.com/docs/api/twiml/twilio_request> 和 <https://www.twilio.com/docs/api/twiml/sms/twilio_request>。 **RoleName** 環境參數會隨附在 Azure 部署中。 (如果您想要新增自訂環境變數, 以便從**getenv**中挑選它們, 請參閱 [[其他角色設定][misc_role_config_settings]] 的 [環境變數] 區段。)
...
如需在 Azure 中搭配 JAVA 使用 Twilio 的詳細資訊, 請參閱[如何在 azure 上的 JAVA 應用程式中使用 Twilio 撥][howto_phonecall_java]打電話。
