---
title: 瞭解適用C#于的 IoT 安全性代理程式本機設定檔 Azure 資訊安全中心 |Microsoft Docs
description: 深入瞭解 IoT 安全性服務的 Azure 資訊安全中心、的安全性代理程式本機設定檔C#。
...
| readRemoteConfigurationTimeout | 時間範圍 | 從 IoT 中樞提取遠端設定的時間週期。 如果代理程式無法在指定的時間內提取設定，作業將會超時。|
| schedulerInterval | 時間範圍 | 內部排程器間隔。 |
| producerInterval | 時間範圍 | 事件產生者背景工作間隔。 |
| consumerInterval | 時間範圍 | 事件取用者背景工作間隔。 |
