...
ms.locfileid: "58541913"
---
# <a name="configure-security-agents"></a>設定安全性代理程式
...
這篇文章提供如何設定與 ASC 的代理程式用於 iot 的說明。
...
IoT 安全性代理程式的每個 ASC 與相關屬性位於 azureiotsecurity 模組的所需的屬性區段內的代理程式組態物件。 

若要修改的組態，建立和修改此物件內 azureiotsecurity 模組對應項身分識別。 如果代理程式組態物件不存在於 azureiotsecurity 模組對應項中，所有的安全性代理程式屬性值會設定為預設值。 
...
所有的自訂屬性必須設定內部 azureiotsecurity 模組對應項中的代理程式組態物件。 

設定屬性，將會覆寫預設值。 若要設定屬性，加入想要的值的組態物件中的屬性索引鍵。 
