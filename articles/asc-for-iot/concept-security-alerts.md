...
| Name | 嚴重性 | 資料來源 | 描述 | 建議的補救步驟|                  
|----------|---------------|-------------|-----------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|**高**嚴重性|  |  |  |
|   二進位命令列   | 高 | 代理程式 | 偵測到從命令列呼叫/執行的 LA Linux 二進位檔。 此程式可能是合法的活動，或表示您的裝置遭到入侵。|   使用執行它的使用者來檢查命令，並檢查這是否為合法預期在裝置上執行的內容。 如果不是，請向您的資訊安全小組呈報警示。 |
|   停用防火牆   | 高                                      | 代理程式       | 偵測到內部主機防火牆的可能操作。 惡意執行者通常會在嘗試竊取資料時停用內部主機防火牆。|  請與執行命令的使用者進行檢查，以確認這在裝置上是否為合法的預期活動。 如果不是，請向您的資訊安全小組呈報警示。 |
|   埠轉送偵測   | 高| 代理程式  | 偵測到外部 IP 位址的埠轉送起始。|  如果這是您預期會在裝置上看到的合法活動，請與執行命令的使用者進行檢查。 如果沒有，請將警示呈報給資訊安全性小組。 |
|   已偵測到可能嘗試停用 Auditd 記錄   | 高 | 代理程式       | Linux Auditd 系統提供一種方式來追蹤系統上的安全性相關資訊。 系統會盡可能記錄有關系統上所發生事件的最多資訊。 這是要徑任務環境的重要資訊，用來判斷違反安全性原則的人員以及他們執行的動作。 停用 Auditd 記錄可能會讓您無法探索系統上使用的安全性原則違規。 |如果這是具有商業理由的合法活動，請洽詢裝置擁有者。 如果不是，此事件可能會由惡意執行者隱藏活動。 立即將事件呈報給您的資訊安全小組。|
|   反向 shell   | 高 | 代理程式 | 在% {遭入侵的主機} 上分析主機資料時，偵測到可能的反向 shell。 反向 shell 通常用來取得遭入侵的機器，以回呼惡意執行者所控制的機器。|  如果這是您預期會在裝置上看到的合法活動，請與執行命令的使用者進行檢查。 如果沒有，請將警示呈報給資訊安全性小組。  |
|  成功的 Bruteforce 嘗試   | 高  | 代理程式       | 已識別多次失敗的登入嘗試，接著成功登入。 嘗試進行 Bruteforce 攻擊的裝置可能已成功。 | 審查 SSH Bruteforce 警示和裝置上的活動。 <br>如果活動是惡意的：<br> 針對遭盜用的帳戶推出密碼重設。<br> 調查並補救（如果找到）裝置是否有惡意程式碼。  |
|  成功的本機登入   |   高                           | 代理程式       | 已偵測到成功的本機登入裝置 | 請確定已登入的使用者是已授權的合作物件。  |
|   Web shell   | 高 | 代理程式 | 偵測到可能的 web shell。 惡意執行者通常會將 web shell 上傳至遭入侵的電腦，以取得持續性或進一步攻擊。|  如果這是您預期會在裝置上看到的合法活動，請與執行命令的使用者進行檢查。 如果沒有，請將警示呈報給資訊安全性小組。 |
|**中**嚴重性|  |  |  |
|   類似于偵測到通用 Linux bot 的行為 | 中| 代理程式 | 通常會偵測到與通用 Linux 殭屍網路相關聯的進程執行。| 如果這是您預期會在裝置上看到的合法活動，請與執行命令的使用者進行檢查。 如果沒有，請將警示呈報給資訊安全性小組。 |
|  與偵測到的 Fairware 勒索軟體類似的行為  | 中 | 代理程式       | 執行 rm-套用至使用主機資料分析偵測到之可疑位置的 rf 命令。 因為 rm-rf 會以遞迴方式刪除檔案，所以通常只會用於個別的資料夾。 在此情況下，它會用於可能會移除大量資料的位置。 Fairware 勒索軟體已知會在此資料夾中執行 rm-rf 命令。 |請與執行命令的使用者進行檢查，這是您預期會在裝置上看到的合法活動。 如果沒有，請將警示呈報給資訊安全性小組。
|  與偵測到勒索軟體類似的行為  | 中 | 代理程式       | 執行類似于已知勒索軟體的檔案，可能會讓使用者無法存取其系統或個人檔案，而且可能會要求 ransom 付款以重新取得存取權。|如果這是您預期會在裝置上看到的合法活動，請與執行命令的使用者進行檢查。 如果沒有，請將警示呈報給資訊安全性小組。
...
|   偵測到可疑的 nohup 命令使用 | 中 | 代理程式       | 在主機上偵測到可疑的 nohup 命令使用。 惡意執行者通常會從臨時目錄執行 nohup 命令，並有效地允許其可執行檔在背景中執行。 在臨時目錄中的檔案上看到此命令執行不是預期的行為，也不是平常的行為。 |如果這是您預期會在裝置上看到的合法活動，請與執行命令的使用者進行檢查。 如果沒有，請將警示呈報給資訊安全性小組。
|   偵測到可疑的 useradd 命令使用  | 中      | 代理程式       | 在裝置上偵測到可疑的 useradd 命令使用。 |如果這是您預期會在裝置上看到的合法活動，請與執行命令的使用者進行檢查。 如果沒有，請將警示呈報給資訊安全性小組。
|  透過 TCP 通訊端公開的 Docker daemon  | 中 | 代理程式 | 電腦記錄指出您的 Docker daemon （dockerd）會公開 TCP 通訊端。 根據預設值，Docker 設定在啟用 TCP 通訊端時，不會使用加密或驗證。 預設 Docker 設定可讓任何人存取相關埠，以啟用 Docker daemon 的完整存取權。|如果這是您預期會在裝置上看到的合法活動，請與執行命令的使用者進行檢查。 如果沒有，請將警示呈報給資訊安全性小組。
...
|  可能覆寫一般檔案 | 中  | 代理程式 | 在裝置上覆寫常見的可執行檔。 所謂的惡意執行者會覆寫一般檔案，以隱藏其動作或做為持續性的方法。 |如果這是您預期會在裝置上看到的合法活動，請與執行命令的使用者進行檢查。 如果沒有，請將警示呈報給資訊安全性小組。
|   偵測到具有特殊許可權的容器 | 中 | 代理程式       | 電腦記錄表示有許可權的 Docker 容器正在執行。 具有許可權的容器具有主機資源的完整存取權。 如果遭到入侵，惡意執行者可以使用具特殊許可權的容器來取得主機電腦的存取權。|如果容器不需要在特殊許可權模式下執行，請從容器中移除許可權。
...
|  x.509 裝置憑證指紋不符  | 中           | IoT Hub     | x.509 裝置憑證指紋不符合設定。 |審查裝置上的警示。 不需要進一步的動作。 |
|  x.509 憑證已過期  | 中 | IoT Hub     | X.509 裝置憑證已過期。 |  這可能是具有過期憑證或嘗試模擬合法裝置的合法裝置。 如果合法裝置目前正確通訊，這可能是模擬嘗試。 ||
|**低**嚴重性|  |  |  |
...
|----------|------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------|
|**中**嚴重性|  |  |  |
|  新增至 IoT 中樞的憑證  | 中                                     |已將名為 \'% {DescCertificateName} \' 的憑證新增至 IoT 中樞 \'% {DescIoTHubName} \'。 如果此動作是由未經授權的合作物件所進行，則可能表示惡意活動。| 1. 確認憑證是由授權方所新增。 <br> 2. 如果授權的合作物件未新增，請移除該憑證，並將警示呈報給組織安全性小組。  |
|  從 IoT 中樞刪除的憑證  | 中                             | 已從 IoT 中樞 \'% {DescIoTHubName} \' 刪除名為 \'% {DescCertificateName} \' 的憑證。 如果此動作是由未經授權的合作物件所進行，它可能表示有惡意活動。| 1. 確認憑證已被授權方移除。 <br> 2. 如果授權的合作物件未移除憑證，請將憑證加回，並將警示呈報給組織安全性小組。 |
| 偵測到不成功的嘗試將憑證新增至 IoT 中樞   | 中    | 嘗試將憑證 \'% {DescCertificateName} \' 新增至 IoT 中樞 \'% {DescIoTHubName} \' 時發生失敗。 如果此動作是由未經授權的合作物件所進行，則可能表示惡意活動。|   請確定變更憑證的許可權只會授與授權的合作物件。  |
|  偵測到不成功的嘗試，無法從 IoT 中樞刪除憑證 | 中  | 嘗試從 IoT 中樞 \'% {DescIoTHubName} \' 刪除憑證 \'% {DescCertificateName} \' 失敗。 如果此動作是由未經授權的合作物件所進行，則可能表示惡意活動。 |請確定變更憑證的許可權只會授與授權的合作物件。
|**低**嚴重性|  |  |  |
