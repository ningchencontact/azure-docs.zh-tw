...
### <a name="node-rename"></a>案例：重新命名已註冊的節點需要再次登出/註冊
...
### <a name="missing-write-permissions"></a>案例：上架失敗並出現訊息-無法啟用解決方案
...
確定您有正確的權限可讓虛擬機器上線。 檢閱[讓機器上線所需的權限](../automation-role-based-access-control.md#onboarding)，嘗試讓解決方案再次上線。 如果您收到 `The solution cannot be enabled on this VM because the permission to read the workspace is missing`的錯誤，請確定您有 `Microsoft.OperationalInsights/workspaces/read` 的許可權，才能找出 VM 是否上架至工作區。
...
* 更新管理
  * 從您的自動化帳戶移除更新部署（排程）
* 於下班時間開始/停止 VM
  * 在 [**設定**] > [**鎖定**] 底下，移除自動化帳戶中解決方案元件的任何鎖定。
