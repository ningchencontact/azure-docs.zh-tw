...
### <a name="rp-register"></a>案例：無法為訂用帳戶註冊自動化資源提供者

#### <a name="issue"></a>問題

在您的自動化帳戶中使用解決方案時，可能會收到下列錯誤。
...
您可以藉由在 Azure 入口網站中完成下列步驟來註冊自動化資源提供者：

1. 按一下 Azure 服務清單底部的 [所有服務]，然後選取 [一般] 服務群組中的 [訂用帳戶]。
2. 選取您的訂用帳戶。
3. 按一下 [_設定_] 下的 [**資源提供者**]。
4. 從資源提供者清單中，確認是否已註冊**Microsoft。 Automation**資源提供者已註冊。
5. 如果未列出提供者，請向底下列出的步驟註冊**Microsoft. Automation**提供者[ ](/azure/azure-resource-manager/resource-manager-register-provider-errors)。

### <a name="mw-exceeded"></a>案例：更新管理排程失敗，發生錯誤 MaintenanceWindowExceeded

#### <a name="issue"></a>問題

更新的預設維護視窗是120分鐘。 您可以將維護期間增加到最多6個小時或360分鐘。

#### <a name="resolution"></a>解析度

編輯任何失敗的排程更新部署，並增加維護時間範圍。

如需維護時段的詳細資訊，請參閱[安裝更新](../automation-update-management.md#install-updates)。
...
### <a name="updates-nodeployment"></a>案例：沒有部署的更新安裝

### <a name="issue"></a>問題

當您在更新管理中註冊 Windows 機器時，您會看到 [更新安裝但不部署]。

### <a name="cause"></a>原因

在 Windows 上，更新會在可用時立即自動安裝。 如果您未排程將更新部署到電腦上，這可能會造成混淆。

### <a name="resolution"></a>解析度

Windows 登錄機碼， `HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU`預設值為 "4"-**自動下載並安裝**。

針對更新管理用戶端，我們建議您將此機碼設為 "3"-**自動下載，但不要自動安裝**。

如需詳細資訊，請參閱設定[自動更新](https://docs.microsoft.com/en-us/windows/deployment/update/waas-wu-settings#configure-automatic-updates)。
...
|`0x80072EE2`|與已設定的 WSUS 伺服器交談的網路連線問題或問題。 檢查 WSUS 設定，並確定可從用戶端存取。|
...
## <a name="patches-are-not-installed"></a>未安裝修補程式

### <a name="machines-do-not-install-updates"></a>機器不會安裝更新

* 請嘗試直接在機器上執行更新。 如果機器無法更新，請參閱[疑難排解指南中的潛在錯誤清單](https://docs.microsoft.com/azure/automation/troubleshoot/update-management#hresult)。
* 如果在本機執行更新，請遵循[＜從更新管理中移除 VM＞](https://docs.microsoft.com/azure/automation/automation-update-management#remove-a-vm-from-update-management)的指示，來嘗試移除機器上的代理程式，並將其重新安裝。

### <a name="i-know-updates-are-available-but-they-dont-show-as-needed-on-my-machines"></a>我知道有可用的更新，但它們在我的機器上並未顯示為必要

* 通常，如果機器設定為從 WSUS/SCCM 取得更新，但 WSUS/SCCM 未核准更新，就會發生此問題。
* 您可以[對本文件的「藉由編輯登錄來設定自動更新」一節中的登錄機碼交互參照 "UseWUServer" 登錄機碼](https://support.microsoft.com/help/328010/how-to-configure-automatic-updates-by-using-group-policy-or-registry-s)，藉以檢查機器是否設定了 WSUS/SCCM
...
* 依分類 (「重大與安全性更新」) 將更新部署至 Linux 時有重要事項須留意，尤其是針對 CentOS。 這些[限制記載在更新管理概觀頁面上](https://docs.microsoft.com/azure/automation/automation-update-management#linux-2)
