...
2. 選取 [+ 建立資源]。
3. 搜尋**自動化**。 在搜尋結果中，選取 [自動化]。<br><br> ![在 Azure Marketplace 中搜尋並選取 [自動化與控制]](media/automation-create-standalone-account/automation-marketplace-select-create-automationacct.png)<br> 
4. 在下一個畫面上，選取 [建立]。
  ...
  >
5. 在 [加入自動化帳戶] 窗格的 [名稱] 方塊中，輸入新自動化帳戶的名稱。
6. 如果您有多個訂用帳戶，請在 [訂用帳戶] 方塊中，指定您想要用於新帳戶的訂用帳戶。 
7. 對於 [資源群組]，輸入或選取新的或現有的資源群組。 
8. 對於 [位置]，選取一個 Azure 資料中心位置。
9. 對於 [建立 Azure 執行身分帳戶] 選項，確定已選取 [是]，然後選取 [建立]。
  ...
  > 若未建立服務主體，則不會指派參與者角色。
  >
10. 若要追蹤自動化帳戶建立的進度，請在功能表中選取 [通知]。

### <a name="resources-included"></a>包含的資源
順利建立自動化帳戶時，系統會自動為您建立幾項資源。 下表摘要說明執行身分帳戶的資源。
