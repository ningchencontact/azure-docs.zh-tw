...
1. 在 [參數] 步驟中，每個複合資源具有的參數都已公開，如此才能提供參數。 如果參數具有描述，則會顯示在參數欄位旁邊。 如果欄位是 **PSCredential** 型別參數，設定的下拉式清單會提供目前自動化帳戶中的 **Credential** 物件清單。 也可以使用 [ **+ 新增認證**] 選項。 一旦已提供所有必要參數，按一下 [儲存並編譯]。
