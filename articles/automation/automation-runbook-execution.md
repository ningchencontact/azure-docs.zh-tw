...
|需要提高許可權的腳本|Hybrid Runbook Worker|沙箱不允許提高許可權。 若要解決此問題, 請使用混合式 Runbook 背景工作角色, 而您`Invoke-Command`可以在執行需要提高許可權的命令時, 關閉 UAC 並使用|
...
撰寫處理多個訂用帳戶的 runbook 時, 您的 runbook 需要使用[enable-azurermcoNtextautosave](/powershell/module/azurerm.profile/disable-azurermcontextautosave)指令程式, 以確保您的驗證內容不會從可能在相同沙箱中執行的另一個 runbook 抓取。 接著, 您必須`-AzureRmContext` `AzureRM`在 Cmdlet 上使用參數, 並將您的內容傳遞給它。
...
[$ErrorActionPreference](/powershell/module/microsoft.powershell.core/about/about_preference_variables#erroractionpreference)喜好設定變數會決定 PowerShell 如何回應非終止錯誤。 終止錯誤不會受到的`$ErrorActionPreference`影響, 它們一律會終止。 藉由`$ErrorActionPreference`使用, 一般非終止的錯誤 ( `PathNotFound`例如從`Get-ChildItem` Cmdlet) 將會停止 runbook 完成。 下列範例示範如何使用`$ErrorActionPreference`。 最後`Write-Output`一行永遠不會執行, 因為腳本將會停止。
...
在 PowerShell 腳本中使用[Try Catch](/powershell/module/microsoft.powershell.core/about/about_try_catch_finally) , 協助您處理終止錯誤。 藉由使用 Try Catch, 您可以攔截特定例外狀況或一般例外狀況。 Catch 語句應該用來追蹤錯誤, 或用來嘗試處理錯誤。 下列範例會嘗試下載不存在的檔案。 它會攔截例外狀況, 如果有另一個例外狀況, 則會傳回最後一個值。 `System.Net.WebException`
...
[Throw](/powershell/module/microsoft.powershell.core/about/about_throw)可以用來產生終止錯誤。 在 runbook 中定義您自己的邏輯時, 這會很有用。 如果符合應停止腳本的特定準則, 您可以使用`throw`來停止腳本。 下列範例會顯示使用`throw`所需的函式參數的電腦。
...
|:--- |:--- |
| 已完成 |工作已成功完成。 |
| 已失敗 |針對 [圖形化和 PowerShell 工作流程 Runbook](automation-runbook-types.md)，此 Runbook 無法編譯。 針對 [PowerShell 指令碼 Runbook](automation-runbook-types.md)，此 Runbook 無法啟動，或作業發生例外狀況。 |
| 處理失敗，正在等候資源 |工作失敗，因為其達到 [公平共用](#fair-share) 的三次上限，且每次從相同的檢查點或啟動 Runbook 開始。 |
| 已佇列 |工作正在等候取得自動化背景工作中的資源，以便可啟動。 |
...
| 已停止 |工作完成之前已由使用者停止。 |
| 正在停止 |系統正在停止作業。 |
| 擱置 |工作已由使用者、系統或 Runbook 中的命令暫停。 如果 Runbook 沒有檢查點，它會從 Runbook 的開頭開始。 如果它有檢查點，則可重新啟動並從其最後一個檢查點繼續。 只有在發生例外狀況時，系統才會暫止 Runbook。 根據預設，ErrorActionPreference 會設定為 [繼續]，代表作業會在發生錯誤時繼續執行。 如果此喜好設定變數設定為 [停止]，作業會在發生錯誤時暫停。 只適用於 [圖形化和 PowerShell 工作流程 Runbook](automation-runbook-types.md) 。 |
| 正在暫止 |因使用者要求，系統正在嘗試暫停工作。 Runbook 必須達到其下一個檢查點才能暫停。 如果它已通過其最後一個檢查點，則可在暫停之前完成。 只適用於 [圖形化和 PowerShell 工作流程 Runbook](automation-runbook-types.md) 。 |
