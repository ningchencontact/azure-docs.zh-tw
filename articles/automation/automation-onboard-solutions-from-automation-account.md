...
選取 [更新管理] 下的 [更新管理]。 選取的 Log Analytics 工作區與上一個步驟中使用的工作區相同。 按一下 [啟用]，讓更新管理解決方案上線。 啟用解決方案最多需要 15 分鐘。
...
瀏覽至您的自動化帳戶，然後選取 [一般] 下的 [已儲存搜尋]。 下表顯示這些解決方案所使用的兩個已儲存的搜尋：
