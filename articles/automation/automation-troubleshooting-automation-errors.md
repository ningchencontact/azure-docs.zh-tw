...
**錯誤：** Runbook 失敗，錯誤為「已對作業進行了三次嘗試，但失敗」。
...
**錯誤的原因：** 這是故意設計的行為，原因是針對 Azure 自動化中程序的「公平共用」監視，也就是如果 Runbook 的執行時間超過三小時，就會自動暫停。 不過，傳回的錯誤訊息不會提供「接下來該怎麼辦」的選項。 有許多原因能造成 Runbook 暫停， 而大多數是因為發生錯誤。 舉例來說，Runbook 中未攔截到的例外狀況、網路失敗，或是執行 Runbook 的 Runbook 背景工作角色當機，都會造成 Runbook 暫停，並讓 Runbook 在恢復運作時從上一個檢查點開始。
...
**錯誤：** 節點回報**失敗**狀態，並包含「從伺服器 https://``<url>``//accounts/``<account-id>``/Nodes(AgentId=``<agent-id>``)/GetDscAction 取得動作的嘗試失敗，因為找不到有效的設定 ``<guid>``」錯誤。
...
**錯誤：** 您的 DSC 編譯作業已暫停，錯誤訊息：「編譯已順利完成，但是沒有產生任何節點 configuration.mofs」。

**錯誤的原因：** 當 DSC 設定中緊接在 **Node** 關鍵字後面的運算式評估為 `$null` 時，則不會產生任何節點設定。
...
**錯誤：** DSC 代理程式輸出「找不到具有指定內容值的執行個體」。

**錯誤的原因：** 您已經將 WMF 版本升級，且擁有損毀的 WMI。

**疑難排解秘訣：** 若要修正問題，請依照 [DSC 已知問題和限制](https://msdn.microsoft.com/powershell/wmf/5.0/limitation_dsc)一文中的指示執行。
...
## <a name="common-errors-when-onboarding-solutions"></a>將解決方案上線時的常見錯誤

將解決方案上線時，您可能會遇到錯誤。 以下是您可能會遇到的常見錯誤清單。

### <a name="computergroupqueryformaterror"></a>ComputerGroupQueryFormatError

**錯誤的原因：**

這個錯誤碼表示未正確地將用來將目標設定為解決方案的已儲存搜尋電腦群組查詢格式化。 您可能已變更查詢，或者系統可能已變更該查詢。
...
如果您已遵循上述的疑難排解步驟，但仍找不到答案，您可以檢閱以下額外的支援選項：
