...
當用戶端使用 Webhook 啟動 Runbook 時，其無法覆寫 Webhook 中定義的參數值。 若要從用戶端接收資料，Runbook 可以接受稱為 **$WebhookData** 的單一參數。 此參數的類型為 [object]，其中包含用戶端在 POST 要求中包含的資料。
