...
2. 移動資源 (例如 Runbook、作業等) 時，會在作業期間鎖定來源群組和目標群組。 群組上的寫入和刪除作業將會封鎖，直到移動完成。 
3. 在移轉完成之後，應該更新任何參考現有訂用帳戶中資源或訂用帳戶識別碼的 Runbook 或變數。  
...
1. 從您的自動化帳戶，按一下頁面頂端的 [移動]。<br> ![移動選項](media/automation-migrate-account-subscription/automation-menu-move.png)<br>
2. 選擇將自動化帳戶移至另一個資源群組或其他訂用帳戶。
3. 在 [移動資源] 窗格上，請注意會顯示與您的自動化帳戶和資源群組相關的資源。 從下拉式清單中選取 [訂用帳戶] 和 [資源群組] ，或選取 [建立新的資源群組] 選項並且在提供的欄位中輸入新的資源群組名稱。 
4. 檢閱並選取核取方塊，以確認您「了解工具和指令碼需要更新，才能在移動資源後使用新的資源識別碼」，然後按一下 [確定]。<br> ![移動資源頁面](media/automation-migrate-account-subscription/automation-move-resources-blade.png)<br>   
