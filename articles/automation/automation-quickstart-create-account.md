...
1. 輸入帳戶資訊。 針對 [建立 Azure 執行身分帳戶] 選擇 [是]，就會自動啟用簡化對 Azure 進行驗證的構件。 請務必注意，在建立自動化帳戶時，名稱在選擇後即不可變更。 自動化帳戶可為指定租用戶管理其所有區域和訂用帳戶上的資源。 完成後，按一下 [建立] 可啟動自動化帳戶部署。
