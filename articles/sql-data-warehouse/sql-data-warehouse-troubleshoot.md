...
| TempDB 空間問題 | [監視 TempDB](sql-data-warehouse-manage-monitor.md#monitor-tempdb)空間使用量。  TempDB 空間不足的常見原因如下：<br>-配置給查詢的資源不足，導致資料溢出到 TempDB。  請參閱[工作負載管理](resource-classes-for-workload-management.md) <br>-統計資料遺失或過期，因而造成過多的資料移動。  如需如何建立統計資料的詳細資訊，請參閱[維護資料表統計資料][Statistics]<br>-每個服務層級配置了 TempDB 空間。  將[您的 SQL 資料倉儲調整][Scaling your SQL Data Warehouse]為較高的 DWU 設定，會配置更多 TempDB 空間。|
