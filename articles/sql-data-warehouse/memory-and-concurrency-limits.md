...
Gen2 在每個查詢提供比 Gen1 還多 2.5x 倍的記憶體。 此額外的記憶體可協助 Gen2 提供快速效能。  Gen2 效能等級的範圍為 DW500c 到 DW30000c。 
...
> [!NOTE]
> Gen2 上的 smallrc 資源類別會在服務等級增加時動態新增記憶體；其在 DW1000c 只支援最多 32 個並行查詢，並在 DW500c 只支援最多 20 個並行查詢。  在執行個體調整到超過 DW1500c 之後，smallrc 所使用的並行位置和記憶體會隨著服務等級增加而增加。 
>
>
