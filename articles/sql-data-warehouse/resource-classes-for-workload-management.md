...
- 調整服務層級不提供按比例變更為相同的資源類別配置的記憶體。

**在 Gen2 上**，動態資源類別是真正的動態位址先前所述的點。  針對 small-medium-large-xlarge 資源類別配置記憶體百分比的新規則是 3-10-22-70，**不論服務等級為何**。  下表是記憶體配置百分比與可執行的並行存取查詢數目下限詳細資料，不論服務等級為何。
