...
-建立或卸除資料表的 ALTER TABLE...SWITCH、 SPLIT 或 MERGE PARTITION-ALTER INDEX DISABLE-DROP INDEX-建立、 更新或卸除統計資料-TRUNCATE TABLE-ALTER AUTHORIZATION-建立登入-建立、 ALTER 或 DROP USER-建立、 ALTER 或 DROP PROCEDURE-建立或卸除檢視表-插入值-選取從系統檢視和 Dmv-說明-DBCC
...
2. 若要查看記憶體密集 CCI 作業 （載入、 複製資料表、 重建索引等） 在給定的資源類別的非分割 CCI 資料表上的最佳資源類別。 此預存程序會使用資料表結構描述來找出所需授與的記憶體。

### <a name="dependencies--restrictions"></a>相依性及限制

- 這個預存程序不被設計來計算分割的 cci 資料表的記憶體需求。
- 這個預存程序不會考慮 CTAS/INSERT-SELECT 的 SELECT 部分考慮記憶體需求，而會假設它是一個 SELECT。
- 此預存程序會使用暫存資料表，可在此預存程序建立所在的工作階段中使用。
- 這個預存程序仰賴目前的供應項目 （例如，硬體組態、 DMS 組態），，如果其中有任何變更，此預存程序不會正確運作。  
- 這個預存程序仰賴現有的並行存取限制供應項目，如果這些變更，此預存程序不會正確運作。  
- 這個預存程序仰賴現有的資源類別供應項目，如果這些變更，此預存程序不會正確運作。  
...
>
>例如，在 DW100，可供授與的最高記憶體為 400 MB，但資料表結構描述的寬度卻足以跨過 400 MB 的需求。
