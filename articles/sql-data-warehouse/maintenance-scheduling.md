...
您會使用維護排程來選擇時段，方便您接收新功能、升級與修補程式。 您可以在七天期間內選擇主要和次要維護時段。 範例中的主要時段是星期六 22:00 到星期日 01:00，而次要時段為星期三 19:00 到 22:00。 如果 SQL 資料倉儲無法在主要維護時段執行維護，它將在次要維護時段再次試著進行維護。 在主要和次要 windows 期間發生服務維護。
...
所有維護事件都會提前 24 小時預先通知。 若要將執行個體的停機時間降至最低，先確定資料倉儲在您選擇的維護期間之前沒有長時間執行的交易。 維護開始時，將取消所有作用中的工作階段。 非認可的交易將會回復，而且您的資料倉儲將會遇到的連線短暫中斷。 您將在資料倉儲上的維護完成之後，立即收到通知。
