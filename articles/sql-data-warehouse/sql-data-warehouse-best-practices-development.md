...
另請參閱[管理資料表統計資料][Manage table statistics], [CREATE STATISTICS][CREATE STATISTICS]、[更新統計資料][更新統計資料]
...
另請參閱[資料表總覽][Table overview], [Table distribution][Table distribution]、[選取]資料表散發、選取資料表[分佈], [CREATE TABLE][CREATE TABLE], [CREATE TABLE AS SELECT][CREATE TABLE AS SELECT]
...
另請參閱[瞭解交易][Understanding transactions], [Optimizing transactions][Optimizing transactions]、[資料表分割][資料表分割]、 [TRUNCATE TABLE][TRUNCATE TABLE] , [ALTER TABLE][ALTER TABLE]、 [create table as select (CTAS)][create table as select (CTAS)]
...
另請參閱[資料表總覽][Table overview], [Table data types][Table data types]、 [CREATE TABLE][CREATE TABLE]

## <a name="optimize-clustered-columnstore-tables"></a>將叢集資料行存放區資料表最佳化
叢集資料行存放區索引是將資料儲存在 SQL 資料倉儲中最有效率的方式之一。  根據預設，SQL 資料倉儲中的資料表會建立為「叢集資料行存放區」。  為了讓資料行存放區資料表的查詢獲得最佳效能，良好的區段品質很重要。  當資料列在記憶體不足的狀態下寫入資料行存放區資料表時，資料行存放區區段品質可能會降低。  壓縮的資料列群組中的資料列數目可以測量區段品質。  如需偵測和改善叢集資料行存放區資料表之區段品質的逐步指示, 請參閱資料行存放區[索引品質][Causes of poor columnstore index quality] in the [Table indexes][Table indexes]不佳的原因一文。  由於高品質的資料行存放區區段很重要，因此最好使用中型或大型資源類別中的使用者識別碼來載入資料。 使用較低的[資料倉儲單位](what-is-a-data-warehouse-unit-dwu-cdwu.md)，表示您想要將更大型的資源類別指派給正在載入的使用者。
...
另請參閱[資料表索引][Table indexes], [Columnstore indexes guide][Columnstore indexes guide]、重建資料行存放區[索引][重建]資料行存放區索引
