...
您需要在專用的 Linux 機器（VM 或內部部署）上部署代理程式，以支援 Fortinet 和 Azure Sentinel 之間的通訊。 下圖說明 Azure 中 Linux VM 的事件設定。
...
## <a name="step-2-forward-fortinet-logs-to-the-syslog-agent"></a>步驟2：將 Fortinet 記錄轉送到 Syslog 代理程式
...
1. 在您的 Fortinet 應用裝置上開啟 CLI，然後執行下列命令：
 ...
 若要在 Fortinet 事件的 Azure 監視器 Log Analytics 中使用相關的架構，請搜尋 `CommonSecurityLog`。
