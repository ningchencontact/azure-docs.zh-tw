---
title: 收集 Azure Sentinel 預覽版中的 Fortinet 資料 |Microsoft Docs
description: 了解如何收集在 Azure Sentinel Fortinet 資料。
services: sentinel
...
若要連接到 Azure Sentinel 的 Fortinet 設備，您需要部署專用的電腦上的代理程式 (VM 或內部部署上) 以支援應用裝置與 Azure Sentinel 之間的通訊。 您可以在自動或手動部署代理程式。 只有使用您專用的電腦是否為新的 VM，您會在 Azure 中建立自動部署。 
