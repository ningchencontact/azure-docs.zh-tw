---
title: 收集 Azure Sentinel 預覽版中的 Palo Alto 資料 |Microsoft Docs
description: 了解如何收集 Azure Sentinel 中的 Palo Alto 資料。
services: sentinel
...
您可以連接 Azure Sentinel 至任何 Palo Alto 網路設備為 Syslog CEF 儲存記錄檔。 與 Azure Sentinel 整合可讓您輕鬆分析和查詢執行的記錄檔資料之間的 Palo Alto 從。 如需有關如何內嵌 Azure Sentinel CEF 資料的詳細資訊，請參閱 <<c0> [ 連線 CEF 設備](connect-common-event-format.md)。

> [!NOTE]
> - 資料會儲存在您在執行 Azure Sentinel 的工作區的地理位置。

## <a name="step-1-connect-your-palo-alto-appliance-using-an-agent"></a>步驟 1：連接您的 Palo Alto 設備使用代理程式

若要連接您的 Palo Alto 設備 Azure Sentinel，您需要部署專用的電腦上的代理程式 (VM 或內部部署上) 以支援應用裝置與 Azure Sentinel 之間的通訊。 您可以在自動或手動部署代理程式。 只有使用您專用的電腦是否為新的 VM，您會在 Azure 中建立自動部署。 
...
## <a name="step-2-forward-palo-alto-logs-to-the-syslog-agent"></a>步驟 2：將 Palo Alto 記錄檔轉送到 Syslog 代理程式

設定 Palo Alto 網路轉送至 Azure 透過 Syslog 代理程式工作區的 CEF 格式的 Syslog 訊息：
1.  移至[常見事件格式 (CEF) 的設定指南](https://docs.paloaltonetworks.com/resources/cef)並下載您的設備類型 pdf。 請遵循所有指南中的指示來設定您的 Palo Alto 設備收集 CEF 事件。 

1.  移至[設定 Syslog 監視](https://aka.ms/asi-syslog-paloalto-forwarding)並遵循步驟 2 和 3 來設定從您的 Palo Alto 應用裝置至 Azure 的 Sentinel 的 CEF 事件轉送。
...
在本文件中，您已了解如何連接到 Azure Sentinel 的 Palo Alto 設備。 若要深入了解 Azure Sentinel，請參閱下列文章：
