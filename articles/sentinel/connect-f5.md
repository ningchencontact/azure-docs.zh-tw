---
title: 將 F5 資料連線到 Azure Sentinel 預覽 |Microsoft Docs
...
ms.locfileid: "68679288"
---
# <a name="connect-your-f5-appliance"></a>連接您的 F5 設備

> [!IMPORTANT]
> Azure Sentinel 目前為公開預覽狀態。
> 此預覽版本是在沒有服務等級協定的情況下提供，不建議用於生產工作負載。 可能不支援特定功能，或可能已經限制功能。 如需詳細資訊，請參閱 [Microsoft Azure 預覽版增補使用條款](https://azure.microsoft.com/support/legal/preview-supplemental-terms/)。

您可以將記錄檔儲存為 Syslog CEF, 以將 Azure Sentinel 連接到任何 F5 設備。 與 Azure Sentinel 的整合可讓您從 F5 輕鬆地對記錄檔資料執行分析和查詢。 如需有關 Azure Sentinel 如何內嵌 CEF 資料的詳細資訊, 請參閱[連接 CEF 設備](connect-common-event-format.md)。

> [!NOTE]
> 資料將會儲存在您執行 Azure Sentinel 之工作區的地理位置中。

## <a name="step-1-connect-your-f5-appliance-using-an-agent"></a>步驟 1:使用代理程式連接您的 F5 設備

若要將您的 F5 設備連接到 Azure Sentinel, 您需要在專用電腦 (VM 或內部部署) 上部署代理程式, 以支援設備和 Azure Sentinel 之間的通訊。
...
1. 在 Azure Sentinel 入口網站中, 按一下 [**資料連線器**], 然後依序選取 [ **F5** ] 和 [**開啟連接器] 頁面**。 
...
## <a name="step-2-forward-f5-logs-to-the-syslog-agent"></a>步驟 2:將 F5 記錄轉送到 Syslog 代理程式

設定 F5 以透過 Syslog 代理程式將 Syslog 訊息以 CEF 格式轉送至您的 Azure 工作區:

請移至 F5 設定[應用程式安全性事件記錄](https://aka.ms/asi-syslog-f5-forwarding), 並遵循指示以使用下列指導方針來設定遠端記錄:
  - 將**遠端存放裝置類型**設定為**CEF**。
  - 將**通訊協定**設定為**UDP**。
  - 將**ip 位址**設定為 SYSLOG 伺服器 ip 位址。
  - 將**埠號碼**設定為**514**, 或將代理程式設定為使用的埠。
  - 將設備設定為您在 Syslog 代理程式中設定的**功能**(根據預設, 代理程式會將此項設定為**local4**)。
...
在本檔中, 您已瞭解如何將 F5 設備連接到 Azure Sentinel。 若要深入了解 Azure Sentinel，請參閱下列文章：
