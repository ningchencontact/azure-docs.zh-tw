...
- 建立您自己的書簽：在搜尋過程中，您可能會遇到看起來不尋常或可疑的相符或發現、儀表板或活動。 若要標記這些專案，讓您在未來可以返回它們，請使用書簽功能。 書簽可讓您儲存專案，以供稍後用來建立調查的事件。 如需書簽的詳細資訊，請參閱[在搜尋中使用書簽](hunting.md)。
