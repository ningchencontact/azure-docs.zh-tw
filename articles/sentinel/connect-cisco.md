---
title: 將 Cisco 資料連接至 Azure 的 Sentinel Preview |Microsoft Docs
description: 了解如何將 Cisco 資料連接至 Azure 的 Sentinel。
services: sentinel
...
## <a name="step-2-forward-cisco-asa-logs-to-the-syslog-agent"></a>步驟 2：將 Cisco ASA 記錄檔轉送到 Syslog 代理程式

Cisco ASA 不支援 CEF，，因此當 Syslog 和 Azure Sentinel 代理程式知道如何剖析它們，如同它們是 CEF 記錄傳送記錄檔。 設定 Cisco ASA 將 Syslog 訊息轉送到您的 Azure 工作區，透過 Syslog 代理程式：

移至[外部的 Syslog 伺服器來傳送 Syslog 訊息](https://aka.ms/asi-syslog-cisco-forwarding)，並遵循指示來設定連線。 使用這些參數，當系統提示您：
- 設定**連接埠**514 或您在代理程式中設定的連接埠。
- 設定**syslog_ip**代理程式的 IP 位址。
- 設定**記錄功能**設施中的代理程式設定。 根據預設，代理程式會設定為 4 設施。

若要使用 Log Analytics 中的 Cisco 事件相關的結構描述，搜尋`CommonSecurityLog`。
...
在本文件中，您已了解如何連接至 Azure 的 Sentinel 的 Cisco ASA 裝置。 若要深入了解 Azure Sentinel，請參閱下列文章：
