...
您需要在專用的 Linux 機器（VM 或內部部署）上部署代理程式，以支援 Cisco ASA 和 Azure Sentinel 之間的通訊。 下圖說明 Azure 中 Linux VM 的事件設定。
...
## <a name="step-2-forward-cisco-asa-logs-to-the-syslog-agent"></a>步驟2：將 Cisco ASA 記錄轉送到 Syslog 代理程式
...
若要在 Log Analytics 中針對 Cisco 事件使用相關的架構，請搜尋 `CommonSecurityLog`。
