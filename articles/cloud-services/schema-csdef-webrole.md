...
|enableNativeCodeExecution|boolean|選用。 預設值是 `true`；預設會啟用機器碼執行和完全信任。 將此屬性設為 `false` 會停用 Web 角色的機器碼執行，並改用 Azure 部分信任。|  
|vmsize|string|選用。 設定此值可變更對角色所配置的虛擬機器大小。 預設值為 `Small`。 如需詳細資訊，請參閱[雲端服務的虛擬機器大小](cloud-services-sizes-specs.md)。|  
...
|hostHeader|string|選用。 指定可讓您使用不同主機名稱在單一 IP 位址/連接埠號碼組合上裝載多個網站的主機名稱。|  
