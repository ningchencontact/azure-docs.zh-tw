...
1. [**IoT Central**](../iot-central/core/overview-iot-central.md)：這是一個 IoT 應用程式平台，能簡化 IoT 解決方案的建立，有助於降低 IoT 管理、作業與開發的負擔及費用。 若要開始，您可以選取適用您裝置類型的範本，然後建立及測試裝置操作員將使用的基本 IoT Central 應用程式。 IoT Central 應用程式也可讓您監視裝置，並佈建新的裝置。 此服務適用於簡單而不需要深入自訂服務的解決方案。
