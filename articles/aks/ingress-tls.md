...
本文說明如何部署[NGINX 輸入控制器][nginx-ingress]in an Azure Kubernetes Service (AKS) cluster. The [cert-manager][cert-manager]專案用來自動產生和設定[let 's Encrypt][可讓加密]憑證。 最後，會有兩個應用程式在 AKS 叢集中執行，且均可透過單一 IP 位址來存取。
...
- [建立輸入控制器用來自動產生具有靜態公用 IP 位址的 TLS 憑證的 let 's Encrypt][aks-ingress-static-tls]
