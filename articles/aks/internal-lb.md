...
> [!NOTE]
> Azure Load Balancer 有兩種 SKU -「基本」  和「標準」  。 根據預設，*基本*SKU 可在服務資訊清單用來在 AKS 上建立負載平衡器。 如需詳細資訊，請參閱 < [Azure 負載平衡器 SKU 的比較][azure-lb-comparison]。
...
使用您建立內部負載平衡器部署[kubectl 套用][kubectl-apply]並指定您的 YAML 資訊清單名稱：
...
當您建立 AKS 叢集時，您可以指定進階網路設定。 這種方法可讓您將叢集部署到現有的 Azure 虛擬網路和子網路。 其中一個案例是將 AKS 叢集部署到已連線至內部部署環境的私人網路，並執行只能在內部存取的服務。 如需詳細資訊，請參閱設定您自己的虛擬網路子網路，具有[Kubenet][use-kubenet] or [Azure CNI][advanced-networking]。
...
> [!NOTE]
> 您可能需要對 AKS 叢集的服務主體，授與 Azure 虛擬網路資源部署所在資源群組的「網路參與者」  角色。 檢視與服務主體[az aks show][az-aks-show]，例如`az aks show --resource-group myResourceGroup --name myAKSCluster --query "servicePrincipalProfile.clientId"`。 若要建立角色指派，請使用[az 角色指派建立][az-role-assignment-create]命令。
