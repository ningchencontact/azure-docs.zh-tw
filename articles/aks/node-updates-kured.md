...
這篇文章會示範如何使用開放原始碼[kured （KUbernetes 重新啟動精靈）][kured]監看的 Linux 節點，需要重新開機，則會自動處理 執行中 pod 和節點的重新排定重新開機程序。
...
您也可以設定適用於 `kured` 的其他參數，例如，與 Prometheus 或 Slack 整合。 如需有關其他設定參數的詳細資訊，請參閱[kured 安裝 docs][kured-install]。

## <a name="update-cluster-nodes"></a>更新叢集節點

根據預設，在 AKS 中的 Linux 節點會檢查更新每個晚上。 如果您不想等待，則可手動執行更新以檢查 `kured` 能夠正確執行。 首先，遵循步驟來[透過 ssh 連線到您的 AKS 節點的其中一個][aks-ssh]。 SSH 連線至 Linux 節點之後，檢查有更新，並將它們套用到，如下所示：
...
您可以監視使用節點的狀態[kubectl 取得節點][kubectl-get-nodes]命令。 下列範例輸出會在節點準備好進行重新啟動程序時，顯示狀態為 *SchedulingDisabled* 的節點：
...
在更新程序完成之後，您可以檢視節點使用的狀態[kubectl 取得節點][kubectl-get-nodes]命令搭配`--output wide`參數。 這個額外的輸出可讓您查看 *KERNEL-VERSION* 基礎節點中的差異，如下列範例輸出所示。 *Aks nodepool1 28993262-0*已在上一個步驟和顯示的核心版本中更新*4.15.0-1039-azure*。 節點*aks nodepool1 28993262-1* ，尚未更新的顯示的核心版本*4.15.0-1037-azure*。
