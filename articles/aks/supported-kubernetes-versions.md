...
一般情況下，使用者應該要執行其所執行次要版本的最新修補程式版本，例如，如果您的生產叢集在*1.12.14*上，而*1.12.15*是*1.12*系列可用的最新可用修補程式版本，則您應該儘快升級至*1.12.15* ，以確保您的叢集已完全修補並受到支援。

## <a name="kubernetes-version-support-policy"></a>Kubernetes 版本支援原則

> [!NOTE]
> 自2019年12月9日起，AKS 將會移至支援最新（N）-2 版的 Kubernetes。 這項變更是為了符合 Kubernetes 版本支援的上游視窗，並確保使用的是最新且最安全的版本。 若要深入瞭解，請參閱[這裡的公告](https://azure.microsoft.com/updates/azure-kubernetes-service-will-be-retiring-support-for-kubernetes-versions-1-11-and-1-12/)。
...
如果上游 Kubernetes 已發行1.12.3，而1.11.6 和 AKS 發行了這些修補程式版本，則最舊的修補程式版本會被取代並移除，而且支援的版本清單會變成：
...
AKS 會發行每週服務更新，其摘要說明[github](https://github.com/Azure/AKS/releases)上的服務已發行的新 Kubernetes 版本、服務變更和元件更新。
...
當您在入口網站或 Azure CLI 中部署 AKS 叢集時，叢集一律會設定為 N-1 次要版本和最新的修補程式。 例如，如果 AKS 支援*1.13. a*， *1.12. a* + *1.12. b*， *1.11.* a + *1.11. b*， *1.10. a* + *1.10 b*，新叢集的預設版本為*1.12. b*。
...
輸出類似下列範例，其中顯示 Kubernetes 版本*1.14.6*是可用的最新版本：
...
- 如果支援的 AKS 版本為*1.13. a*、 *1.12. b* + *1.12 .c*、 *1.11. d* + *1.11. e*和 1.10. *f* + *1.10. g* ，而且您是在*1.9. h*或*1.9. i*，您的支援範圍外。
