...
若要確認已成功建立憑證，請使用 `kubectl describe certificate tls-secret` 命令。
...
如果您需要建立額外的憑證資源，可以使用下列範例資訊清單來執行該動作。 建立名為*certificates.yaml* ，並更新*dnsNames*並*網域*您在上一個步驟中建立的 DNS 名稱。 如果您使用僅供內部使用的輸入控制器，請為您的服務指定內部 DNS 名稱。
