...
建立新的資源群組和 AKS 叢集，然後使用您的金鑰來加密 OS 磁片。
...
## <a name="add-a-node-pool-to-an-existing-aks-cluster-and-encrypt-the-os-disk-with-a-customer-managed-key"></a>將節點集區新增至現有的 AKS 叢集，並使用客戶管理的金鑰將 OS 磁片加密

根據預設，新的 nodepools 不會使用加密的磁片。  您可以使用下列命令，將新的節點集區新增至現有的叢集，並使用您自己的金鑰來加密 OS 磁片。
