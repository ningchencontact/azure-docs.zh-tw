...
| Docker 橋接器位址 | 用來作為節點上 Docker 橋接器 IP 位址的 IP 位址 (採用 CIDR 標記法)。 此 CIDR 會與節點上的容器數目相關聯。 預設值為 172.17.0.1/16。 |
...
使用 AKS 引擎所建立的 Kubernetes 叢集同時支援[kubenet][kubenet] and [Azure CNI][cni-networking]外掛程式。 因此，這兩個網路案例都會受到 AKS 引擎支援。
