...
本文說明如何安裝 Istio。 Istio `istioctl` 用戶端二進位檔會安裝到您的用戶端電腦上，且 Istio 元件會安裝到 AKS 上的 Kubernetes 叢集中。

> [!NOTE]
> 這些指示參考 Istio 版本 `1.1.3`。
>
> Istio `1.1.x` 版本已由 Istio 團隊針對 Kubernetes @no__t 版本（`1.12`、`1.13`）進行測試。 您可以在[GitHub Istio 版本][istio-github-releases]和[Istio 新聞][istio-release-notes]中每個版本的相關資訊中找到其他 Istio 版本。
...
本文中詳述的步驟假設您已建立 AKS 叢集（已啟用 RBAC 的 Kubernetes `1.11` 和更新版本），並已建立與叢集的 @no__t 1 連線。 如果您需要這些專案的協助，請參閱[AKS 快速入門][aks-quickstart]。

您將需要[Helm][helm] ，才能遵循這些指示並安裝 Istio。 建議您在叢集中正確安裝和設定 `2.12.2` 或更新版本。 如果您需要安裝 Helm 的協助，請參閱[AKS Helm 安裝指引][helm-install]。 所有 Istio pod 也必須排程在 Linux 節點上執行。
...
在 PowerShell 中，使用 `Invoke-WebRequest` 下載最新的 Istio 版本，然後使用 `Expand-Archive` 進行解壓縮，如下所示：
...
@No__t-0 用戶端二進位檔會在您的用戶端電腦上執行，並可讓您與 Istio 服務網格互動。 在用戶端作業系統之間，安裝步驟稍有不同。 選擇下列其中一個符合您慣用環境的安裝步驟：
...
使用下列命令，在 Linux 或[適用于 linux 的 Windows 子系統][install-wsl]上以 bash 為基礎的 shell 中安裝 Istio `istioctl` 用戶端二進位檔。 這些命令會將 `istioctl` 用戶端二進位檔複製到 `PATH` 中的標準使用者程式位置。
...
若要在 Windows 上以**Powershell**為基礎的 shell 中安裝 Istio `istioctl` 用戶端二進位檔，請使用下列命令。 這些命令會將 `istioctl` 用戶端二進位檔複製到 Istio 資料夾，然後透過您的 `PATH`，讓它立即可用（在目前的 shell 中）並永久（跨 shell 重新開機）。 您不需要更高的（系統管理員）許可權來執行這些命令，也不需要重新開機 shell。
...
Istio 會使用[自訂資源定義（CRDs）][kubernetes-crd]來管理其執行時間設定。 我們必須先安裝 Istio CRDs，因為 Istio 元件有相依性。 使用 Helm 和 `istio-init` 圖表，將 Istio CRDs 安裝到 AKS 叢集中的 @no__t 1 命名空間：
...
既然我們已確認成功完成作業，讓我們來確認是否已安裝正確的 Istio CRDs 數目。 您可以針對您的環境執行適當的命令，以確認已安裝所有 53 Istio CRDs。 此命令應該會傳回 `53` 的數位。
...
以您的密碼取代 `REPLACE_WITH_YOUR_SECURE_PASSWORD` 權杖，然後執行下列命令：
...
既然我們已成功建立 AKS 叢集中的 Grafana 和 Kiali 秘密，就可以安裝 Istio 元件了。 使用 [Helm] 和 [`istio`] 圖表，將 Istio 元件安裝到 AKS 叢集中的 @no__t 1 命名空間。 針對您的環境使用適當的命令。

> [!NOTE]
> 我們會在安裝過程中使用下列選項：
> - `global.controlPlaneSecurityEnabled=true`-已啟用控制平面的相互 TLS
> - `mixer.adapters.useAdapterCRDs=false`-移除混音器介面卡 CRDs 上的監看，使其淘汰，這將可改善效能
...
@No__t-0 Helm 圖會部署大量物件。 您可以從上方的 `helm install` 命令的輸出查看清單。 Istio 元件的部署可能需要4到5分鐘的時間才能完成，視您的叢集環境而定。
...
先確認是否已建立預期的服務。 使用[kubectl get svc][kubectl-get]命令來查看執行中的服務。 查詢 `istio-system` 命名空間，其中的 Istio 和附加元件是由 @no__t 1 Helm 圖表所安裝：
...
- `istio-*` 個服務
- `jaeger-*`、`tracing` 和 @no__t 2 附加元件追蹤服務
- `prometheus` 附加元件計量服務
...
接著，確認是否已建立所需的 Pod。 使用[kubectl get][kubectl-get] pod 命令，然後重新查詢 @no__t 1 命名空間：
...
應該有兩個具有 `Completed` 狀態的 @no__t 0 pod。 這些 pod 負責執行在先前步驟中建立 CRDs 的工作。 所有其他 pod 都應該顯示 `Running` 的狀態。 如果您的 Pod 沒有這些狀態，請等候一兩分鐘直到其成為該狀態。 如果有任何 pod 回報問題，請使用[kubectl 描述 pod][kubectl-describe]命令來檢查其輸出和狀態。
...
Istio 的分析和監視儀表板是由[Grafana][grafana]提供。 將用戶端電腦上的本機埠 `3000` 轉送至 AKS 叢集中執行 Grafana 的 pod 上的埠 `3000`：
...
[Prometheus][prometheus]會提供 Istio 的計量。 將用戶端電腦上的本機埠 `9090` 轉送至 AKS 叢集中執行 Prometheus 的 pod 上的埠 `9090`：
...
Istio 內的追蹤是由[Jaeger][jaeger]提供。 將用戶端電腦上的本機埠 `16686` 轉送至 AKS 叢集中執行 Jaeger 的 pod 上的埠 `16686`：
...
服務網格可檢視性儀表板是由[Kiali][kiali]提供。 將用戶端電腦上的本機埠 `20001` 轉送至 AKS 叢集中執行 Kiali 的 pod 上的埠 `20001`：
...
若要從您的 AKS 叢集中移除 Istio，請使用下列命令。 @No__t-0 命令會移除 `istio` 和 @no__t 2 圖表，而 `kubectl delete ns` 命令將會移除 `istio-system` 命名空間。
