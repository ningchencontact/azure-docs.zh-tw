...
在 Kubernetes 叢集中, 當建立資源時, 會使用「許可控制站」攔截對 API 伺服器的要求。 然後, 「許可控制」可以針對一組規則來*驗證*資源要求,或將資源變動以變更部署參數。
...
當您啟用 pod 安全性原則時, AKS 會建立兩個名為 [特殊*許可權*] 和 [*受限*] 的預設原則 請勿編輯或移除這些預設原則。 相反地, 請建立您自己的原則, 以定義您想要控制的設定。 讓我們先看看這些預設原則對 pod 部署的影響為何。

若要查看可用的原則, 請使用[kubectl get psp][kubectl-get]命令, 如下列範例所示。 做為預設*限制*原則的一部分, 使用者會被拒絕使用特殊許可權 pod 擴大, 而使用者*MustRunAsNonRoot*。
...
*受限制*的 pod 安全性原則會套用至 AKS 叢集中任何已驗證的使用者。 此指派是由 ClusterRoles 和 ClusterRoleBindings 所控制。 使用[kubectl get clusterrolebindings][kubectl-get]命令, 並搜尋*default: 限制:* binding:
...
讓我們先測試當您使用的安全性內容`privileged: true`來排程 pod 時, 會發生什麼事。 此安全性內容會擴大 pod 的許可權。 在上一節中, 顯示預設 AKS pod 安全性原則時,*受限制*的原則應該會拒絕此要求。
...
Kubernetes 排程器會接受 pod 要求。 不過, 如果您使用`kubectl get pods`來查看 pod 的狀態, 就會發生錯誤:
...
Kubernetes 排程器會接受 pod 要求。 不過, 如果您使用`kubectl get pods`來查看 pod 的狀態, 則會有與上一個範例不同的錯誤:
...
已成功排程 pod。 當您使用[kubectl get][kubectl-get] pod 命令檢查 pod 的狀態時, pod 正在執行:
