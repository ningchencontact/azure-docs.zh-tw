...
在 shell 提示字元中，使用 `wget` 來查看您是否可以存取預設的 NGINX 網頁。 此時，將逾時值設定為 2 秒。 網路原則現在會封鎖所有輸入流量，因此無法載入頁面，如下列範例所示：
...
在 shell 提示字元中，使用 `wget` 來查看您是否可以存取預設的 NGINX 網頁：
...
在 shell 提示字元中，使用 `wget` 來查看您是否可以存取預設的 NGINX 網頁。 網路原則會封鎖輸入流量，因此無法載入頁面，如下列範例所示：
...
在 shell 提示字元中，使用 `wget` 來查看網路原則允許流量：
