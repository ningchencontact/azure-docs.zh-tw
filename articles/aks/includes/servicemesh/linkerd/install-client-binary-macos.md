...
@No__t_0 用戶端二進位檔會在您的用戶端電腦上執行，並可讓您與 Linkerd 服務網格互動。 使用下列命令，將 Linkerd `linkerd` 用戶端二進位檔安裝在 MacOS 上以 bash 為基礎的 shell 中。 這些命令會將 `linkerd` 用戶端二進位檔複製到 `PATH` 中的標準使用者程式位置。
