...
在 Kubernetes 中，「服務」會按邏輯將 Pod 分組，以允許透過 IP 位址或 DNS 名稱和特定連接埠上的直接存取。 您也可以使用*負載平衡器*來分散流量。 您也可以透過*輸入控制器*執行更複雜的應用程式流量路由。 Pod 的網路流量的安全性和篩選可以透過 Kubernetes*網路原則*(在 AKS 中為預覽狀態) 來進行。
