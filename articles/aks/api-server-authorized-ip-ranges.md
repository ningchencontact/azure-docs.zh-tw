...
本文 assumess 您正在使用[kubenet][kubenet]的叢集。  使用以[Azure Container 網路介面（CNI）][cni-networking]為基礎的叢集，您將不需要用來保護存取的必要路由表。  您將需要手動建立路由表。  如需詳細資訊，請參閱[管理路由表](https://docs.microsoft.com/azure/virtual-network/manage-route-table)。
