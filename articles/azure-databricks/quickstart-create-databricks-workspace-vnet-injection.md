...
2. 在 [**建立虛擬網路**] 下, 套用下列設定: 
    ...
    |設定|建議值|說明|
    |-------|---------------|-----------|
    |名稱|databricks-quickstart|選取虛擬網路的 [名稱]。|
    |位址空間|10.1.0.0/16|使用 CIDR 標記法的虛擬網路位址範圍。|
    |訂閱|\<您的訂用帳戶\>|選取您要使用的 Azure 訂用帳戶。|
    |資源群組|databricks-quickstart|選取 [新建], 然後為您的帳戶輸入新的資源組名。|
...
3. 部署完成後, 流覽至您的虛擬網路, 然後選取 [**設定**] 底下的 [**位址空間**]。 在顯示 [*新增其他位址範圍*] 的方塊中`10.179.0.0/16` , 插入並選取 [**儲存**]。
...
2. 在 [ **Azure Databricks 服務**] 底下, 套用下列設定:
    ...
    |設定|建議值|描述|
    |-------|---------------|-----------|
    |工作區名稱|databricks-quickstart|選取您 Azure Databricks 工作區的 [名稱]。|
    |訂閱|\<您的訂用帳戶\>|選取您要使用的 Azure 訂用帳戶。|
    ...
    |公用子網名稱|public-subnet|使用預設的公用子網名稱。|
    |公用子網 CIDR 範圍|10.179.64.0/18|此子網的 CIDR 範圍應介於/18 到/26 之間。|
    |私人子網名稱|private-subnet|使用預設的私人子網名稱。|
...
4. 返回您的 Azure Databricks 工作區, 然後選取您所建立的叢集。 然後流覽至**SPARK UI**頁面上的 [執行] 索引標籤。 請注意, 驅動程式和執行程式的位址是在私人子網範圍內。 在此範例中, 驅動程式為 10.179.0.6, 而執行程式為10.179.0.4 和10.179.0.5。 您的 IP 位址可能不同。
