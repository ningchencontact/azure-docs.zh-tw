...
您可以先使用 keytool 新增 CA 憑證，再將 JDK 壓縮並新增到 Azure 專案的 **approot** 資料夾，或者也可以執行使用了 keytool 來新增憑證的 Azure 啟動工作。 (此範例假設您會在壓縮 JDK 之前新增 CA 憑證。)另外，範例中會使用特定 CA 憑證，但是若要取得不同的 CA 憑證並將之儲存到 cacerts 存放區，步驟會很類似。
...
1. 在設定為 JDK **jdk\jre\lib\security** 資料夾的 Administrator 命令提示字元下，執行下列命令來查看已安裝哪些憑證：
    ...
    系統會提示您輸入存放區密碼。 預設密碼為 **changeit**。 (如果您想要變更密碼，請參閱 keytool 文件，網址為 <http://docs.oracle.com/javase/7/docs/technotes/tools/windows/keytool.html>。)這個範例假設 MD5 指紋為 67:CB:9D:C0:13:24:8A:82:9B:B2:17:1E:D1:1B:EC:D4 的憑證未列在其中，而您想要將它匯入 (Twilio API service 需要這個特定的憑證)。
...
3. 透過下列命令匯入憑證：
    ...
    如果系統提示您信任具有 MD5 指紋 67:CB:9D:C0:13:24:8A:82:9B:B2:17:1E:D1:1B:EC:D4 的憑證，請輸入 **y** 回應。
...
如果您需要新增 Baltimore CyberTrust 根憑證，它的序號是 02:00:00:b9、SHA1 指紋是 d4:de:20:d0:5e:66:fc:53:fe:1a:50:88:2c:78:db:28:52:ca:e4:74。 您可以從 <https://cacert.omniroot.com/bc2025.crt> 下載它，並儲存到副檔名為 **.cer** 的本機檔案，然後使用先前步驟中所示的 **keytool** 匯入。
