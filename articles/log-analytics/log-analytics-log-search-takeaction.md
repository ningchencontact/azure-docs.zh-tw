...
* [Log Analytics 工作區](log-analytics-queries.md)

## <a name="to-initiate-runbook-from-log-search"></a>從記錄搜尋初始化 Runbook

若要對事件採取動作，並從記錄搜尋結果初始化 Runbook，首先請建立記錄搜尋，然後就可以從結果中視需要叫用 Runbook。  您可以利用 [Azure 入口網站](log-analytics-queries.md)中的記錄搜尋功能達到此目的。  在此範例中，我們會透過這項功能的基本示範，從 Azure 入口網站執行記錄搜尋。
...
* 檢閱 [Log Analytics 記錄檔搜尋參考資料](log-analytics-queries.md) ，以檢視 Log Analytics 中提供的所有搜尋欄位和 Facet。
