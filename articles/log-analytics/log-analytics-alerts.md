...
Log Analytics 中的警示會識別您的 Log Analytics 儲存機制中的重要資訊。  本文將討論一些設計決策，必須以要查詢資料的收集頻率、透過可能因網路延遲或處理容量所造成之資料擷取的隨機延遲，及認可 Log Analytics 存放庫內的資料為基礎來制定這些決策。  還會提供警示規則在 Log Analytics 中的運作方式，並說明不同類型的警示規則之間的差異。
...
- 收集頻率會決定機器上的 OMS 代理程式將資料傳送至 Log Analytics 的頻率。 例如，如果收集頻率為 10 分鐘，且系統中沒有其他延遲，則傳輸資料的時間戳記在新增至存放庫並可在 Log Analytics 進行搜尋之前，可能會是介於 0 到 10 分鐘以前的。
...
- **時間範圍**。  指定查詢的時間範圍。  查詢只會傳回在此目前時間範圍內建立的記錄。  這可能是介於 5 分鐘與 24 小時之間的任何值。 範圍寬度必須足以容納擷取中的合理延遲。 時間間隔必須是您需要能夠處理的最長延遲時間的兩倍。<br> 例如，如果您需要對 30 分鐘延遲的可靠警示，範圍就必須是一小時。  
    ...
    - **遺漏警示**. 假設擷取延遲在某些情況下為 60 分鐘，但大部分的情況下是 15 分鐘。  如果時間間隔設定為 30 分鐘，當延遲是 60 分鐘時就會遺失警示，因為當執行警示查詢時，資料將無法提供搜尋。 
