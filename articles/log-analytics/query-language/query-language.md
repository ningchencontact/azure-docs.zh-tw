...
- [在 Azure 入口網站開始使用 Log Analytics](get-started-analytics-portal.md)是一堂在 Azure 入口網站中撰寫查詢和使用結果的課程。
-  [在 Log Analytics 中開始使用查詢](get-started-queries.md)是一堂使用 Log Analytics 資料撰寫查詢的課程。

## <a name="concepts"></a>概念
- [分析 Azure 監視器中的 Log Analytics 資料](../log-analytics-queries.md)提供記錄查詢的簡短概觀，並說明建置 Log Analytics 資料結構的方式。
- [在 Log Analytics 中檢視及分析資料](../log-analytics-log-search-portals.md)說明您要在其中建立和執行 Log Analytics 查詢的入口網站。

## <a name="reference"></a>參考

- [查詢語言參考](/azure/kusto/query)是適用於資料總管查詢語言的完整語言參考。
- [Log Analytics 查詢語言](data-explorer-difference.md)說明資料總管查詢語言版本間的差異。
- [Log Analytics 記錄中的標準屬性](../log-analytics-standard-properties.md)說明所有 Log Analytics 資料的標準屬性。
- [在 Log Analytics 中執行跨資源記錄搜尋](../log-analytics-cross-workspace-search.md)說明如何撰寫查詢，這類查詢會使用來自多個 Log Analytics 工作區與 Application Insights 應用程式的資料。
