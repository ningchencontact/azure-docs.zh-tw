...
* 造訪「Log Analytics 意見反應」頁面，以檢閱已提交的構想和錯誤[http://aka.ms/opinsightsfeedback](https://aka.ms/opinsightsfeedback)，或提出新的構想和錯誤。  
...
## <a name="important-configuration-files"></a>重要組態檔案
 ...
 >[!NOTE]
 >如果您是從 Azure 入口網站中的[資料功能表 Log Analytics [進階設定]](log-analytics-data-sources.md#configuring-data-sources) 為工作區設定了集合，則為效能計數器和 Syslog 編輯組態檔會遭到覆寫。 若要停用所有代理程式的組態，請從 Log Analytics [進階設定] 停用集合，若是單一代理程式，則請執行下列命令：  
...
| 4 | 套件類型無效或 Proxy 設定無效；omsagent-rpm.sh 套件只能安裝在以 RPM 為基礎的系統上，至於 omsagent-deb.sh 套件，則只能安裝在以 Debian 為基礎的系統上。 建議您使用來自[最新版本](log-analytics-quick-collect-linux-computer.md#install-the-agent-for-linux)的通用安裝程式。 也請進行[檢閱](#issue:-unable-to-connect-through-proxy-to-log-analytics)，以驗證 Proxy 設定。 |
...
| 4 | 提供給 omsadmin 指令碼的 Proxy 無效。 請確認 Proxy，並查看 [HTTP Proxy 的使用文件](log-analytics-concept-hybrid.md#network-firewall-requirements)。 |
| 5 | 從 Log Analytics 服務收到 403 HTTP 錯誤。 如需詳細資訊，請參閱 omsadmin 指令碼的完整輸出。 |
| 6 | 從 Log Analytics 服務收到非 200 HTTP 錯誤。 如需詳細資訊，請參閱 omsadmin 指令碼的完整輸出。 |
| 7 | 無法連線至 Log Analytics 服務。 如需詳細資訊，請參閱 omsadmin 指令碼的完整輸出。 |
...
偵錯記錄功能可讓您查看 Log Analytics 服務的分批上傳 (依類型、資料項目總，以及傳送所花費的時間分隔)：
...
### <a name="resolution"></a>解決方案
* 確認 Log Analytics 工作區 (適用於 Syslog) 中的組態具有所有設備和正確的記錄檔層級。 檢閱[在 Azure 入口網站中設定 Syslog 集合](log-analytics-data-sources-syslog.md#configure-syslog-in-the-azure-portal)
...
2. 確認上架至 Log Analytics 服務是否成功，做法是檢查下列檔案是否存在：`/etc/opt/microsoft/omsagent/<workspace id>/conf/omsadmin.conf`。  如果不成功，請使用 omsadmin.sh 命令列[指示](https://github.com/Microsoft/OMS-Agent-for-Linux/blob/master/docs/OMS-Agent-for-Linux.md#onboarding-using-the-command-line)重新上架。
...
8. 在某些情況下，當 Log Analytics Linux 代理程式無法與 Log Analytics 服務通訊時，代理程式上的資料會備份為整個緩衝區大小：50 MB。 應該執行下列命令重新啟動代理程式：`/opt/microsoft/omsagent/bin/service_control restart`。
...
2. 遵循[指示](log-analytics-quick-collect-linux-computer.md)來安裝代理程式。
