...
我們預計在 2018 年 8 月前解決這兩個入口網站之間的差距。 我們會根據客戶的意見反應，來溝通終止 OMS 入口網站的時間表。 我們很高興移至 Azure 入口網站，並預期此次轉換將會很簡單。 但我們了解任何變更均有其難度，而且可能會對您造成困擾。 如有任何問題、 意見反應或考量，請傳送至LAUpgradeFeedback@microsoft.com。 本文其餘部分將介紹關鍵案例、 目前的差距以及此次轉換的藍圖。 
...
- 使用 OMS 入口網站所建立的更新排程，可能不會反映在 Azure 入口網站更新管理儀表板的排定更新部署、或更新工作歷程記錄。 這個落差預計在 2018 年 6 月底以前解決。
- 自訂記錄檔預覽功能只能透過 OMS 入口網站啟用。 自 2018 年 6 月底起，這項功能將會在所有的工作空間中自動啟用。
...
自 6 月 25 日開始並持續到 7 月為止，會開始將 OMS 入口網站的存取控制權限自動轉換為 Azure 入口網站權限的作業。 完成轉換後，OMS 入口網站使用者管理區段會將使用者路由傳送至 Azure 中的存取控制 (IAM)。 
