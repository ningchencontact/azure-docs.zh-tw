...
1. 開啟[Azure 入口網站](https://portal.azure.com/)按一下**瀏覽** > **記錄分析 (OMS)**開啟記錄分析 (OMS)。  
2. 在**記錄分析 (OMS)**，按一下 **新增**開啟**OMS 工作區**。  
   ![OMS](./media/log-analytics-sccm/sccm-azure01.png)
3. 在**OMS 工作區**，提供下列資訊，然後按一下**確定**。
...
5. 在**資源群組**，按一下以開啟先前建立的資源群組&lt;資源群組名稱&gt;設定。  
   ![資源群組設定](./media/log-analytics-sccm/sccm-azure03.png)
6. 在&lt;資源群組名稱&gt;設定，請按一下 存取控制 (IAM) 來開啟&lt;資源群組名稱&gt;使用者。  
   ![資源群組的使用者](./media/log-analytics-sccm/sccm-azure04.png)  
7. 在&lt;資源群組名稱&gt;使用者，按一下**新增**開啟**新增存取**。
8. 在**新增存取**，按一下 **選取角色**，然後選取**參與者**角色。  
...
4. 如果您成功完成其他所有程序，則 [OMS 連線組態] 畫面上的資訊會自動出現在此頁面。 連接設定的資訊應該會出現為您**Azure 訂用帳戶**， **Azure 資源群組**，和**Operations Management Suite 工作區**。  
...
> [!NOTE]
> 您必須將 OMS 連線至階層中的最上層網站。 如果您將 OMS 連線至獨立主要站台，並將管理中心網站新增到您的環境，您必須刪除並重新建立新的階層中的 OMS 連線。
>
>
...
1. 在 [組態管理員] 中，瀏覽至**雲端服務**，然後選取**OMS Connector**開啟**OMS 連線內容**頁面。
