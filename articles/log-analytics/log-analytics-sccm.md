...
2. 在 Azure 入口網站中，[為 Configuration Manager (已註冊的 Web 應用程式) 提供存取 OMS 的權限](#provide-configuration-manager-with-permissions-to-oms)。
...
下列程序可為 Azure 入口網站提供存取 OMS 的權限。 具體來說，您必須授與參與者角色給資源群組中的使用者，以允許 Azure 入口網站將 Configuration Manager 連線至 OMS。
...
1. 開啟 [Azure 入口網站](https://portal.azure.com/)，然後按一下 [瀏覽] > [Log Analytics (OMS)] 以開啟 [Log Analytics (OMS)]。  
2. 在 [Log Analytics (OMS)] 上按一下 [新增] 以開啟 [OMS 工作區]。  
   ![OMS](./media/log-analytics-sccm/sccm-azure01.png)
3. 在 [OMS 工作區] 上提供下列資訊，然後按一下 [確定]。
   ...
   * **定價層**  
     ...
     >
     >
4. 按一下 [瀏覽] > [資源群組]，開啟 [資源群組]。
5. 在 [資源群組] 中按一下您先前建立的資源群組，開啟 [&lt;資源群組名稱&gt;] 設定。  
   ![資源群組設定](./media/log-analytics-sccm/sccm-azure03.png)
6. 在 [&lt;資源群組名稱&gt;] 設定中按一下 [存取控制 (IAM)]，開啟 [&lt;資源群組名稱&gt; 使用者]。  
   ![資源群組使用者](./media/log-analytics-sccm/sccm-azure04.png)  
7. 在 [&lt;資源群組名稱&gt; 使用者] 中按一下 [新增]，開啟 [新增存取]。
8. 在 [新增存取] 中按一下 [選取角色]，然後選取 [參與者] 角色。  
...
3. 在 [Azure Active Directory] 畫面上對 OMS 設定連線設定 (方法是提供 [租用戶]、[用戶端 ID] 和 [用戶端秘密金鑰])，然後選取 [下一步]。  
...
1. 在 OMS 入口網站中，[從 OMS 下載代理程式安裝檔案](log-analytics-windows-agent.md)。
2. 使用下列其中一個方法，在執行 Configuration Manager 服務連線點網站系統角色的電腦上安裝並設定代理程式：
   * [使用安裝程式安裝代理程式](log-analytics-windows-agent.md)
   * [使用命令列安裝代理程式](log-analytics-windows-agent.md)
   * [使用 Azure 自動化中的 DSC 安裝代理程式](log-analytics-windows-agent.md)
