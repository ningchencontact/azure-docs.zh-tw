...
在下列範例中，您會建立一個計量量值警示規則，此規則會為查詢中值超過 90% 閾值的每個電腦物件建立警示。
...
5. 使用下列資訊設定警示規則：  
   ...
   c. [警示頻率] 指定應執行查詢的頻率。  針對此範例，指定 5 分鐘，這會在我們指定的時段內發生。  
   d. 選取 [計量度量]，並在 [彙總值] 中輸入 *90*，在 [觸發警示的依據]  中輸入 *3*。  
   e. 在 [動作] 下方停用電子郵件通知。
6. 按一下 [儲存]  完成警示規則。 此警示規則會立即開始執行。<br><br> ![警示規則範例](media/log-analytics-tutorial-response/log-analytics-alert-01.png)
...
6. 按一下 [儲存]  完成警示規則。 此警示規則會立即開始執行。<br><br> ![使用電子郵件通知的警示規則](media/log-analytics-tutorial-response/log-analytics-alert-02.png)
