...
### <a name="create-an-azure-ad-key-vault-application"></a>建立 Azure AD Key Vault 應用程式

要部署 Blockchain Workbench，必須要註冊 Azure AD Key Vault 應用程式。

1. 在 Azure 入口網站的左側導覽窗格中，選取 [Azure Active Directory] 服務。 選取 [應用程式註冊] > [新增應用程式註冊]。
2. 提供應用程式的 [名稱] 和 [登入 URL]。 您可以使用預留位置值，因為這些值在部署期間將會變更。

    ![建立 Key Vault 應用程式註冊](media/blockchain-workbench-deploy/key-vault-app-create.png)

    | 設定  | 值  |
    |---------|---------|
    | Name | `Blockchain Key Vault app` |
    | 應用程式類型 | Web 應用程式/API |
    | 登入 URL | `https://keyvaultclient |

5. 選取 [建立] 以註冊 Azure AD Key Vault 應用程式。
...
11. 完成 [Azure 監視器] 設定。
    ...
    | 設定 | 說明  |
    |---------|--------------|
    | 監視 | 選擇您是否要讓 Azure 監視器用來監視區塊鏈網路。 |
    | 連線至現有的 OMS 執行個體 | 選擇您要使用現有的 Operations Management Suite 執行個體，還是要建立新的。 
...
現在您已部署的 Azure Blockchain Workbench，下一個步驟是要[在 Azure Blockchain Workbench 中管理使用者](blockchain-workbench-manage-users.md)。