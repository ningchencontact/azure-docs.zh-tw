...
description: 如何在 Azure Blockchain Workbench 中建立區塊鏈應用程式。
...
如需組態檔內容的詳細資訊，請參閱 [Azure Blockchain Workbench 組態參考](blockchain-workbench-configuration-overview.md)。
...
|基底類別函式  | 目的  | 呼叫時機  |
|---------|---------|---------|
| ContractCreated() | 通知 Blockchain Workbench 已建立合約 | 在結束合約建構函式之前 |
| ContractUpdated() | 通知 Blockchain Workbench 已更新合約狀態 | 在結束合約函式之前 |
...
Blockchain Workbench 的合約必須繼承自 **WorkbenchBase** 基底類別。 在宣告合約時，必須以引數形式傳遞應用程式名稱和工作流程名稱。
...
合約必須繼承自 **WorkbenchBase** 基底類別，並傳入組態檔中所定義的 **ApplicationName** 參數和 **Name** 工作流程。 在此情況下，應用程式名稱和工作流程名稱會相同。
...
在結束建構函式前，呼叫 `ContractCreated()` 函式。 此函式會通知 Blockchain Workbench 已建立合約。
...
在結束函式前，呼叫 `ContractUpdated()` 函式。 此函式會通知 Blockchain Workbench 已更新合約狀態。 如果您想要復原在函式中所做的狀態變更，請呼叫 revert()。 還原會捨棄自上一次呼叫 ContractUpdated() 後所做的狀態變更。
...
在應用程式中新增應用程式成員以起始合約並對其採取動作。 若要新增應用程式成員，您必須是 [Blockchain Workbench 系統管理員](blockchain-workbench-manage-users.md#manage-blockchain-workbench-administrators)。

1. 選取 [應用程式] > [Hello, Blockchain!]。
2. 與應用程式相關聯的成員數目會顯示在頁面右上角。 若為新的應用程式，成員數目會是零。
3. 選取頁面右上角的 [成員] 連結。 隨即會顯示目前的應用程式成員清單。
4. 在成員資格清單中，選取 [新增成員]。
5. 選取或輸入您想要新增的成員名稱。 只有存在於 Blockchain Workbench 租用戶的 Azure AD 使用者才會列出。 如果找不到使用者，則需要[新增 Azure AD 使用者](blockchain-workbench-manage-users.md#add-azure-ad-users)。
6. 選取成員的 [角色]。 針對第一個成員，選取 [要求者] 作為角色。
7. 選取 [新增] 以在應用程式中新增與角色相關聯的成員。
8. 在應用程式中新增另一個具有**回應者**角色的成員。

如需如何在 Blockchain Workbench 中管理使用者的詳細資訊，請參閱[在 Azure Blockchain Workbench 中管理使用者](blockchain-workbench-manage-users.md)
