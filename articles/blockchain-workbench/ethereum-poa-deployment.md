...
![自備操作員](./media/ethereum-poa-deployment-guide/bring-you-own-operator.png)
...
![部署架構](./media/ethereum-poa-deployment-guide/deployment-architecture.png)
...
權威證明的核心是非集中式控管。 控管 DApp 是一組預先部署的智慧合約和 Web 應用程式，用來控管網路的授權。
授權可細分為管理員身分識別和驗證器節點。
管理員有權將共識參與委派給一組驗證器節點。 管理員也可經由投票讓其他管理員加入或退出網路。

![控管 dapp](./media/ethereum-poa-deployment-guide/governance-dapp.png)
...
此程序需要可支援部署多個虛擬機器擴展集和受控磁碟的 Azure 訂用帳戶。 如有必要，請[建立免費的 Azure 帳戶](https://azure.microsoft.com/free/)以開始作業。
...
範例部署如下所示：![基本刀鋒視窗](./media/ethereum-poa-deployment-guide/basic-blade.png)
...
範例部署如下所示：![部署區域](./media/ethereum-poa-deployment-guide/deployment-regions.png)
...
每個參數的詳細描述如下：
  ...
  驗證器節點儲存體效能|支援每個已部署驗證器節點的受控磁碟類型。|標準或進階|標準
  驗證器節點的虛擬機器大小|驗證器節點所使用的虛擬機器大小。|標準 A、標準 D、標準 D-v2、標準 F 系列、標準 DS 與標準 FS|標準 D1 v2

範例部署如下所示：![網路大小和效能](./media/ethereum-poa-deployment-guide/network-size-and-performance.png)
...
範例部署如下所示：![以太坊設定](./media/ethereum-poa-deployment-guide/ethereum-settings.png)

#### <a name="azure-monitor"></a>Azure 監視器

[Azure 監視器] 刀鋒視窗可讓您設定網路的 Azure 監視器資源。 Azure 監視器會收集並顯示您的網路中實用的計量和記錄，讓您能夠快速檢查網路健康情況或偵錯問題。

  參數名稱|說明|允許的值|預設值
  ---|---|---|---
監視|啟用 Azure 監視器的選項|啟用或停用|啟用
連線至現有的 Log Analytics|建立新的 Log Analytics 執行個體作為 Azure 監視器的一部分，或加入現有的執行個體|建立新的或加入現有的|新建
Azure 監視器位置 (連線至現有的 Azure 監視器 = 建立新的)|將部署新 Azure 監視器的區域|所有 Azure 監視器區域|NA
現有的 Log Analytics 工作區識別碼 (連線至現有的 Azure 監視器 = 加入現有的)|現有 Log Analytics 執行個體的工作區識別碼||NA
現有的 Log Analytics 主要金鑰 (連線至現有的 Azure 監視器 = 加入現有的)|用來連線至現有 Azure 監視器執行個體的主要金鑰||NA


範例部署如下所示：![Azure 監視器](./media/ethereum-poa-deployment-guide/azure-monitor.png)
...
![部署電子郵件](./media/ethereum-poa-deployment-guide/deployment-email.png)
...
若要擴充您的聯盟，您必須先連接實體網路。
使用以公用 IP 為基礎的部署時，第一個步驟將可順暢執行。 如果部署於 VPN 後方，請參閱[連接 VNet 閘道](#connecting-vnet-gateways)一節，而在[步驟 2](#step-2-new-admin-deployment) 中執行網路連線。

#### <a name="step-1-add-the-new-admin"></a>步驟 1：新增管理員

1.  收集新管理員的公用以太坊位址。

2.  瀏覽至 [控管 DApp]，並以太坊位址和適當的別名建立新的管理員

3.  通知其他現有的成員有新的要求，讓他們可經由投票新增此管理員

4.  一旦選票達到 51% 以上，成員即會新增為管理員

![新增管理員](./media/ethereum-poa-deployment-guide/adding-admin.png)

#### <a name="step-2-new-admin-deployment"></a>步驟 2：新管理員的部署
...
#### <a name="step-3-delegate-validators"></a>步驟 3：委派驗證器

新的管理員必須為驗證器授與代表他們參與的權限。 此作業無法自動完成，因為只有成員可控制其金鑰。\*

_\*第一個成員在網路上的節點可自動新增，因為我們可以將其驗證器節點預先編譯到網路上的創世區塊中。_

新的管理員必須執行下列步驟：

1.  開啟在*他們的*部署中部署的控管 DApp

2.  瀏覽至 [驗證器] 索引標籤

3.  選取每個已部署的驗證器，然後按一下 [新增]

![新增驗證器](./media/ethereum-poa-deployment-guide/adding-validators.png)

#### <a name="connecting-vnet-gateways"></a>連接 VNet 閘道

在使用私人網路的情況下，會透過 VNet 閘道連線連接不同的成員。 在成員可以加入網路並查看交易流量之前，現有的成員必須在 VPN 閘道上執行最後的設定，以接受連線。 這表示加入成員的以太坊節點須在連線建立後才能執行。 建議在聯盟中您建立備援網路連線 (網格)，以降低單一失敗點的可能性。

新成員部署後，現有成員必須設定對新成員的 VNet 閘道連線，以完成雙向連線。 為此，現有成員將需要：

1.  連線方成員的 VNet 閘道資源識別碼 (請參閱部署輸出)
...
![監視器類別](./media/ethereum-poa-deployment-guide/monitor-categories.png)

選取 [節點概觀] 會將您導向至入口網站，以檢視每個節點的基礎結構統計資料。

![節點統計資料](./media/ethereum-poa-deployment-guide/node-stats.png)

選取 [網路統計資料] 會引導您檢視以太坊網路統計資料。

![網路統計資料](./media/ethereum-poa-deployment-guide/network-stats.png)
...
基於安全考量，網路群組安全性規則依預設會拒絕 SSH 連接埠存取。 若要存取 PoA 網路中的虛擬機器擴展集執行個體，您必須將此規則變更為 [允許]\"\"

1.  首先，從 Azure 入口網站進入已部署資源群組的 [概觀] 區段。

    ![SSH 概觀](./media/ethereum-poa-deployment-guide/ssh-overview.png)
...
4.  將 [動作]\"\" 變更為 [允許]
    ...
    ![SSH 啟用允許](./media/ethereum-poa-deployment-guide/ssh-enable-allow.png)

5.  按一下 [儲存]\"\" (變更可能需要數分鐘才會套用)

現在，您可以透過 SSH 和您提供的管理員使用者名稱和密碼/SSH 金鑰，從遠端連線至驗證器節點的虛擬機器。
範本部署輸出參數 'SSH\_TO\_FIRST\_VL\_NODE\_REGION1' 中會列出要存取第一個驗證器節點所需執行的 SSH 命令 (部署範例：ssh - p 4000 poaadmin\@leader4vb.eastus.cloudapp.azure.com)。 若要前往其他交易節點，則將通訊埠編號加上 1 (例如，第一個交易節點在連接埠 4000 上)。
...
![搜尋 Azure 流量管理員](./media/ethereum-poa-deployment-guide/traffic-manager-search.png)

為設定檔指定唯一名稱，並選取在 PoA 部署期間所建立的資源群組。

![建立流量管理員](./media/ethereum-poa-deployment-guide/traffic-manager-create.png)

在部署完成後，選取資源群組中的執行個體。 用來存取流量管理員的 DNS 名稱可在 [概觀] 索引標籤中找到

![找出流量管理員 DNS](./media/ethereum-poa-deployment-guide/traffic-manager-dns.png)

選取 [端點] 索引標籤，然後按一下 [新增] 按鈕。 然後，將 [目標資源類型] 變更為 [公用 IP 位址]。 接著，取得第一個區域的負載平衡器所具備的公用 IP 位址。

![路由流量管理員](./media/ethereum-poa-deployment-guide/traffic-manager-routing.png)
...
### <a name="debug-a-smart-contract-truffle-develop"></a>對智慧合約進行偵錯 (Truffle 開發)
...
### <a name="i-notice-a-bunch-of-transactions-on-the-network-that-i-didnt-send-where-are-these-coming-from"></a>我發現網路上有許多我未傳送的交易。 這些交易來自何處？

將[個人 API](https://web3js.readthedocs.io/en/1.0/web3-eth-personal.html) 解除鎖定並不安全。 Bot 帳戶會接聽已解除鎖定的以太坊帳戶，並嘗試清空資金。 Bot 會假設這些帳戶含有真實的以太幣，並嘗試率先竊取餘額。 請勿在網路上啟用個人 API。 您應手動使用 MetaMask 之類的電子錢包預先簽署交易，或依照「以程式設計方式與智慧合約互動」一節中的說明，以程式設計方式預先簽署。

### <a name="how-to-ssh-onto-a-vm"></a>如何透過 SSH 連線至 VM？

基於安全考量，我們會鎖定 SSH 連接埠。 請遵循[本指南以啟用 SSH 連接埠](#_Accessing_Nodes_via)。
...
這非常適合作為讓網路監理者得以進行稽核的機制。 若要使用此機制，請從「擴增聯盟」的步驟 2 開始執行即可。
