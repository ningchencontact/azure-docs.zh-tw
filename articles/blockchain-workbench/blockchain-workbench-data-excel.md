...
3. 選取 [從 Azure]，然後選取 [從 Azure SQL 資料庫]。
   ...
   ![連線到 Azure SQL 資料庫](media/blockchain-workbench-data-excel/connect-sql-db.png)
...
4. 在 [SQL Server 資料庫] 對話方塊中：
   ...
   ![提供資料庫伺服器與資料載](media/blockchain-workbench-data-excel/provide-server-db.png)
...
5. 在 [SQL Server 資料庫] 對話方塊導覽列中，選取 [資料庫]。 輸入 [使用者名稱] 與 [密碼]，然後選取 [連線]。
   ...
   ![提供用於存取資料庫的認證](media/blockchain-workbench-data-excel/provide-credentials.png)
...
[Excel 導覽器] 對話方塊會在您與資料庫連線後開啟。 您可以使用導覽器查看資料庫中的資料庫與檢視。 檢視是針對報告設計的，其名稱前會加上 **vw**。
   ...
   ![Excel 導覽器檢視預覽](media/blockchain-workbench-data-excel/excel-navigator.png)
...
2. 選取 [載入] 以擷取檢視中的所有資料並放到 Excel 活頁簿中。
   ...
   ![從檢視載入的資料](media/blockchain-workbench-data-excel/view-data.png)
