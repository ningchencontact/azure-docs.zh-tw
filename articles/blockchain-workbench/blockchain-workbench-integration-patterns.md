...
REST API 和傳訊整合不僅可讓您從外部系統將交易傳送至 Azure Blockchain Workbench 應用程式所包含的智慧合約，也可根據應用程式內發生的變更將事件通知傳送至外部系統。

在資料整合案例中，Azure Blockchain Workbench 包含一組資料庫檢視，其中整合了區塊鏈和中繼資料中與應用程式和智慧合約有關的中交易資料。

此外，在某些情況下 (例如，與供應鏈或媒體有關的情況)，也可能需要文件的整合。 雖然 Azure Blockchain Workbench 未提供可直接處理文件的 API 呼叫，但文件可以納入 Azure Blockchain 應用程式中。 本節也會說明該模式。

本節將說明經認可能夠在您的端對端解決方案中實作各類整合的模式。

## <a name="rest-api-based-integration"></a>以 REST API 為基礎的整合

Azure Blockchain Workbench 產生的 Web 應用程式所包含的功能，可透過 REST API 來公開。 這包括 Azure Blockchain Workbench 上傳、應用程式的設定和管理、將交易傳送至分散式總帳，以及應用程式中繼資料和總帳資料的查詢。

REST API 主要用於互動式用戶端，例如 Web、行動和 Bot 應用程式。

本節探討的模式將著重在會將交易傳送至分散式總帳，以及會從 Azure Blockchain Workbench 的*鏈結關閉* SQL 資料庫查詢交易相關資料的 REST API 層面。

### <a name="sending-transactions-to-a-distributed-ledger-from-an-external-system"></a>從外部系統將交易傳送至分散式總帳

Azure Blockchain Workbench REST API 可讓您傳送已驗證的要求，以對分散式總帳執行交易。

![將交易傳送至分散式總帳](media/blockchain-workbench-integration-patterns/send-transactions-ledger.png)
...
-   REST API 將要求封裝為訊息，並將其傳送至服務匯流排。 在此處會擷取、簽署訊息，然後傳送至適當的分散式總帳。
...
Azure Blockchain Workbench REST API 可讓您傳送已驗證的要求，以查詢對分散式總帳執行之智慧合約的相關詳細資料。

![查詢中繼資料](media/blockchain-workbench-integration-patterns/querying-metadata.png)
...
傳訊整合有助於與無法使用或不適用互動式登入的系統、服務和裝置進行互動。 傳訊整合著重在兩種類型的訊息，此類訊息會要求對分散式總帳執行交易，且在交易已發生時必須由該總帳公開事件。

傳訊整合著重於執行和監視與使用者建立、合約建立和合約的交易執行有關的交易，且主要由*無周邊*後端系統所使用。

本節探討的模式將著重在會將交易傳送至分散式總帳，以及會顯示基礎分散式總帳所公開之事件訊息的訊息型 API 層面。
...
![單向事件傳遞](media/blockchain-workbench-integration-patterns/one-way-event-delivery.png)
...
![直接傳遞](media/blockchain-workbench-integration-patterns/direct-delivery.png)
...
![未知的訊息格式](media/blockchain-workbench-integration-patterns/unknown-message-format.png)
...
![將控制權轉換至外部程序](media/blockchain-workbench-integration-patterns/transition-external-process.png)

#### <a name="return-of-control-from-the-smart-contract"></a>從智慧合約交回控制權

根據自訂外部系統的能力，該系統有可能無法以 Azure Blockchain Workbench 預期的標準格式之一來傳遞訊息。 外部系統產生其中一種訊息的能力，將會決定應採取下列兩個傳回路徑中的哪一個。

##### <a name="direct-delivery-of-an-azure-blockchain-workbench-in-the-expected-format"></a>Azure Blockchain Workbench 以預期的格式進行直接傳遞

![](media/blockchain-workbench-integration-patterns/direct-delivery.png)

在此模型中，與合約的通訊和後續的狀態變更將會在上述程序之後發生，其間：

-   在外部程式碼執行完成或達到特定里程碑時，就會傳送事件到連線至 Azure Blockchain Workbench 的服務匯流排。

-   系統若無法直接調整而撰寫符合 API 預期的訊息，就會轉換訊息。

-   訊息的內容會在封裝後傳送至智慧合約的特定函式。 這是代表與外部系統相關聯的使用者執行的作業。
...
在此模型中，採用標準格式的訊息無法直接傳送，與合約的通訊和後續的狀態變更將會在上述程序之後發生，其間：
...
6. 訊息的內容會在封裝後傳送至智慧合約的特定函式。 這是代表與外部系統相關聯的使用者執行的作業。
...
例如，如果運送藥品的貨車溫度飆升至 110 度，則可能會影響到藥品的藥效，且若未偵測到此問題而從供應鏈中移除，可能會造成公共安全問題。 如果某駕駛加速至時速 100 英哩，產生的感應器資訊可能致使保險業者取消其保險。 對於租用的車輛，GPS 資料可以在駕駛的行車路徑超出其租用合約所涵蓋的地理位置時加以指出，而收取罰金。
...
在 Azure 平台上，與裝置的整合通常透過 IoT 中樞來完成。 IoT 中樞可讓您根據內容來路由訊息，並啟用前述的功能類型。

![IoT 訊息](media/blockchain-workbench-integration-patterns/iot.png)
...
![資料整合](media/blockchain-workbench-integration-patterns/data-integration.png)
...
2. Azure Blockchain Workbench 應用程式的適當智慧合約已定義函式，以接受來自外部系統或裝置的事件。
3. 智慧合約的應用程式組態檔包含將指派給系統或裝置的角色。
4. 智慧合約的應用程式組態檔可識別已定義的角色在哪些狀態下可呼叫此函式。
...
提供與外部系統整合的能力，在許多案例中都是關鍵需求之一。 能夠在開發與外部系統整合的程式碼之前或期間驗證智慧合約，會比較妥當。

使用 Azure Active Directory (Azure AD) 可大幅增進開發人員生產力和獲利時程。 具體來說，與外部系統的程式碼整合，可能需要相當程度的時間。 使用 Azure AD 和 Azure Blockchain Workbench 的 UX 自動產生功能，可讓開發人員從該外部系統登入 Workbench，並透過 UX 填入該外部系統預期的值。 這可讓您在為外部系統撰寫整合程式碼之前或期間，快速開發和驗證概念證明環境中的概念。
