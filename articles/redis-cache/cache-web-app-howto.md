---
title: 使用 Redis 快取建立 ASP.NET Web 應用程式 | Microsoft Docs
description: 在本快速入門中，您會學到如何使用 Redis 快取建立 ASP.NET Web 應用程式
...
開啟 Visual Studio，然後按一下 [檔案][新增]、[專案]。

![建立專案](./media/cache-web-app-howto/cache-create-project.png)

在 [新增專案] 對話方塊中，採取下列步驟：

1. 在 [範本] 清單中展開 [Visual C#] 節點
1. 選取 [雲端]
...
![選取專案範本](./media/cache-web-app-howto/cache-select-template.png)
...
> [!TIP]
> 金鑰輪替期間會使用次要存取金鑰作為重新產生主要存取金鑰時的替代金鑰。
>
...
在本節中，您要更新應用程式以支援會對 Azure Redis 快取顯示簡單測試的新檢視。
...
當您在本機執行應用程式時，CacheSecrets.config 中的資訊將會用來連線至 Azure Redis 快取執行個體。 稍後您會將此應用程式部署至 Azure。 屆時，您將會在 Azure 中設定應用程式設定，以供應用程式用來擷取快取連線資訊，而不是這個檔案。 由於 CacheSecrets.config 不會隨著應用程式部署至 Azure，因此您只會在本機測試應用程式時使用它。 請以最安全的方式保存這項資訊，以防止有人惡意存取快取資料。

在 [方案總管] 中，按兩下 Web.config 檔案來加以開啟。

![Web.config](./media/cache-web-app-howto/cache-web-config.png)

在 web.config 檔案中尋找 `<appSetting>` 元素，並新增下列 `file` 屬性。 如果您使用不同檔案名稱或位置，請以這些值取代範例中顯示的值。

* 取代前： `<appSettings>`
* 取代後： ` <appSettings file="C:\AppSecrets\CacheSecrets.config">`

ASP.NET 執行階段會將外部檔案的內容與 `<appSettings>` 元素的標記合併。 如果找不到指定的檔案，則執行階段會略過檔案屬性。 您的密碼 (您的快取的連接字串) 不會包含在應用程式的原始程式碼中。 將 Web 應用程式部署至 Azure 時將不會部署 CacheSecrests.config 檔案。

### <a name="configure-the-application-to-use-stackexchangeredis"></a>設定應用程式以使用 StackExchange.Redis

若要將應用程式設定為使用適用於 Visual Studio 的 [StackExchange.Redis](https://github.com/StackExchange/StackExchange.Redis) NuGet 套件，請按一下 [工具] > [NuGet 套件管理員] > [套件管理員主控台]。
...
### <a name="update-the-homecontroller-and-layout"></a>更新 HomeController 和版面配置

在 [方案總管] 中展開 [控制器] 資料夾，然後開啟 [HomeController.cs] 檔案。

在檔案頂端新增下列兩個 `using` 陳述式，以支援快取用戶端和應用程式設定。
...
將下列方法新增至 `HomeController` 類別，以支援會對新快取執行一些命令的新 `RedisCache` 動作。
...
### <a name="add-a-new-rediscache-view"></a>新增 RedisCache 檢視

在 [方案總管] 中，展開 [檢視] 資料夾，然後在 [主資料夾] 資料夾上按一下滑鼠右鍵。 選擇 [新增] > [檢視...]。

在 [新增檢視] 對話方塊中，輸入 **RedisCache** 作為檢視名稱，然後按一下 [新增]。

以下列程式碼取代 RedisCache.cshtml 檔案中的程式碼：
...
在 Visual Studio 的功能表上，按一下 [偵錯] > [開始偵錯] 以在本機建置和啟動要測試和偵錯的應用程式。

在瀏覽器中，按一下導覽列上的 [Azure Redis 快取測試]。

在下列範例中，您會看到 `Message` 金鑰先前有快取值，此值是在入口網站中使用 Redis 主控台所設定的。 應用程式更新了該快取值。 應用程式也已執行 `PING` 和 `CLIENT LIST` 命令。
...
在本機成功測試應用程式後，您會將應用程式部署至 Azure 並在雲端執行。

### <a name="publish-the-app-to-azure"></a>將應用程式發佈至 Azure

在 Visual Studio 的 [方案總管] 中，以滑鼠右鍵按一下專案節點，然後選擇 [發佈]。

![發佈](./media/cache-web-app-howto/cache-publish-app.png)

按一下 [Microsoft Azure App Service]，選擇 [建立新的]，然後按一下 [發佈]。

![發佈至 App Service](./media/cache-web-app-howto/cache-publish-to-app-service.png)

在 [建立 App Service] 對話方塊中進行下列變更：

| 設定 | 建議值 | 說明 |
| ------- | :---------------: | ----------- |
| **應用程式名稱** | 使用預設值 | 應用程式名稱會是應用程式在部署至 Azure 時的主機名稱。 如有必要，該名稱中會新增時間戳記尾碼，以便成為唯一名稱。 |
| **訂用帳戶** | 選擇 Azure 訂用帳戶 | 系統會就任何相關的裝載費用，對此訂用帳戶收費。 如果您有多個 Azure 訂用帳戶，請確認您已選取所需的訂用帳戶。|
| **資源群組** | 使用您用來建立快取的相同資源群組。 例如，TestResourceGroup。 | 資源群組可協助您以群組方式管理所有資源。 稍後當您想要刪除應用程式時，直接刪除該群組即可。 |
| **App Service 方案** | 按一下 [新增] 並建立名為 TestingPlan 的新 App Service 方案。 <br />使用您在建立快取時所使用的相同**位置**。 <br />選擇**免費**大小。 | App Service 方案會針對要搭配執行的 Web 應用程式定義一組計算資源。 |

![App Service 對話方塊](./media/cache-web-app-howto/cache-create-app-service-dialog.png)

設定 App Service 裝載設定之後，按一下 [建立] 來為應用程式建立新的 App Service。

在 Visual Studio 中監視 [輸出]  視窗，以查看發佈至 Azure 的狀態。 成功完成發佈後，系統會記錄 App Service 的 URL，如下所示：

![發佈輸出](./media/cache-web-app-howto/cache-publishing-output.png)

### <a name="add-the-app-setting-for-the-cache"></a>新增快取的應用程式設定

新 App Service 發佈完成後，請新增應用程式設定。 這項設定會用來儲存快取連線資訊。 在 Azure 入口網站頂端的搜尋列中輸入應用程式名稱，以尋找您剛才建立的新 App Service。

![尋找 App Service](./media/cache-web-app-howto/cache-find-app-service.png)

新增名為 **CacheConnection** 的應用程式設定，以供應用程式用來連線至快取。 在 CacheSecrets.config 檔案中使用針對 `CacheConnection` 所設定的相同值。 該值包含快取的主機名稱和存取金鑰。

![新增應用程式設定](./media/cache-web-app-howto/cache-add-app-setting.png)

### <a name="run-the-app-in-azure"></a>在 Azure 中執行應用程式

在瀏覽器中瀏覽至 App Service 的 URL。 該 URL 會顯示在 Visual Studio [輸出] 視窗中的發佈作業結果內。 此外，也會在 Azure 入口網站中您所建立的 App Service [概觀] 頁面上提供該 URL。

按一下導覽列上的 [Azure Redis 快取測試] 以測試快取存取。
...
在這個接下來的教學課程中，您會在更加真實的案例中使用 Azure Redis 快取，來改善應用程式的效能。 您會將此應用程式更新為在 ASP.NET 和資料庫中使用另行快取模式來快取排行榜結果。
