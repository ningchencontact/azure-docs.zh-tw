...
您可以使用 Azure PowerShell 模組中的 `New-AzResourceGroupDeployment` API，將範本部署為 Resource Manager 升級至叢集。 範例命令可能像這樣：
...
您也可以使用 `New-AzOperationalInsightsWorkspace` 命令，透過 PowerShell 部署 log analytics 資源。 若要使用此方法，請確定您已安裝 [Azure PowerShell](https://docs.microsoft.com/powershell/azure/install-Az-ps)。 請使用此指令碼建立新的 Log Analytics 工作區，並將 Service Fabric 解決方案新增至該工作區： 
