...
- I-> P:複本升階至主要複本將閒置次要複本。 這會在閒置次要複本為主要複本的正確候選對象時，於特殊重新設定下發生。
- I-> N:刪除閒置次要複本。
- S-> P:複本升階為主要作用中次要資料庫。 這可能是因為叢集資源管理員起始的主要複本容錯移轉或主要複本移動。 例如，為了回應應用程式升級或負載平衡等情況。
- S-> N:刪除作用中次要複本。
- P-> S:降級的主要複本的程序。 這可能是因為叢集資源管理員起始的主要複本移動。 例如，為了回應應用程式升級或負載平衡等情況。
- P-> N:刪除主要複本。
