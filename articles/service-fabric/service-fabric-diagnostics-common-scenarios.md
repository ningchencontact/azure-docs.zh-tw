...
2. 按一下刀鋒視窗底部標題為「摘要」的圖形
    ...
    ![Log Analytics 解決方案](media/service-fabric-diagnostics-common-scenarios/oms-solution-azure-portal.png)
...
5. 按一下頂端的 [新增警示規則]，現在，每當有根據這項查詢的事件抵達時，您就會在所選擇的通訊方法中收到警示。
    ...
    ![Log Analytics 新警示](media/service-fabric-diagnostics-common-scenarios/oms-create-alert.png)
...
若要追蹤應用程式中的 Reliable Services 或 Reliable Actors 效能，也請新增「Service Fabric 動作項目」、「動作項目方法」、「服務」和「服務方法」等計數器。 您可以使用和上述案例類似的方式新增這些計數器，以下是要在 Log Analytics 中新增的可靠服務和動作項目效能計數器範例：
