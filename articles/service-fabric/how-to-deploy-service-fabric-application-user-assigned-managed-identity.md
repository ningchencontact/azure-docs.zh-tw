...
若要使用使用者指派的身分識別來啟用應用程式, 請先將**identity**屬性新增至**userAssigned**類型的應用程式資源, 以及參考的使用者指派的身分識別。 然後在**應用程式**資源的 [**屬性**] 區段內新增**managedIdentities**區段, 其中包含每個使用者指派身分識別的易記名稱清單, 以 principalId 對應。
