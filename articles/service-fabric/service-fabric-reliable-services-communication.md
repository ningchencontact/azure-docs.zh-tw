...
在这两种情况下，都会返回侦听器的集合。 這可讓您的服務透過多個接聽程式，可能使使用不同的通訊協定，在多個端點上接聽。 例如，您可能有 HTTP 接聽程式和個別的 WebSocket 接聽程式。 每個接聽程式都會獲得一個名稱及所產生的名稱集合：當用戶端要求服務執行個體或資料分割的接聽位址時，系統會以 JSON 物件的形式呈現位址配對。
...
名為「命名服務」  的系統服務會在 Service Fabric 叢集上執行。 命名服务是服务及其地址（服务的每个实例或副本正在其上侦听）的注册机构。 當 `ICommunicationListener(C#) / CommunicationListener(Java)` 的 `OpenAsync(C#) / openAsync(Java)` 方法完成時，它的傳回值會在命名服務中註冊。 這個在命名服務中發佈的傳回值是一個字串，其值完全可以是任何項目。 這個字串值是用戶端向命名服務要求服務的位址時將會看見的內容。
...
通常用戶端程式碼不需要直接搭配 ServicePartitionResolver 使用。 它在创建后即会传递给 Reliable Services API 中的通信客户端工厂。 Factory 會在內部使用解析程式來產生可用來與服務通訊的用戶端物件。
...
* **不可重試**的例外狀況會進一步分類為**暫時性**和**非暫時性**。
  * **暂时性**异常是只会重试而不会重新解析服务终结点地址的异常。 這類例外狀況包括暫時性網路問題或服務錯誤回應，但不包括指出服務端點位址不存在的錯誤回應。
...
* [有可靠服務的 ASP.NET Core](service-fabric-reliable-services-communication-aspnetcore.md)
