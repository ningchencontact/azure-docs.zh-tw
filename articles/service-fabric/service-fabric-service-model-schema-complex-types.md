---
title: Azure Service Fabric 服務模型 XML 架構複雜類型
...
ConfigPackage Settings.xml 的節名稱。
...
## <a name="configpackagepoliciestype-complextype"></a>ContainerHostPoliciesType complexType
...
在包含 Settings.xml 檔案的 PackageRoot 底下，宣告以 Name 屬性命名的資料夾。 此檔案包含程序可以在執行階段讀回的使用者定義、成對的索引鍵/值設定等區段。 在升級期間，如果只有 ConfigPackage 版本已變更，則不會重新啟動執行中程序。 相反地，回呼會通知程序組態設定已變更，因此它們可以動態方式重新載入。
...
使用 clusterManifest.xml 中指定的預設存放庫認證，而不是 ApplicationManifest.xml 內部 "RepositoryCredentials" 標記中指定的認證。
...
使用 MSI 權杖驗證 (或自訂指定的端點) 取得用於驗證的權杖。
...
容器的隔離模式。 有效值為 default、process 或 hyperv (僅支援 Windows 容器)。
...
指示如果刪除容器，是 (true) 否 (false) 會自動移除容器。  如果設為 false，SF 會手動移除容器。
...
描述服務封裝和容器網路和之間的關聯，以及 (選擇性) 服務封裝端點和容器網路端點之間的關聯。
...
容器網路的名稱 (case-insensitive)。 必須是已建立容器網路的名稱，或保留的名稱，包括 "Open" 或 "NAT"。
...
端點的通訊協定。 複寫服務資訊清單中的值。 HTTPS 端點必須有宣告於應用程式資訊清單中的 EndpointCertificate 和 EndpointBindingPolicy。 此通訊協定無法於後續在應用程式升級期間變更。 
...
URI 配置。 複寫服務資訊清單中的值。 例如 "http"、"https" 或 "ftp"。
...
#### <a name="protocol"></a>通訊協定
端點的通訊協定：http、https、tcp　或 udp。 HTTPS 端點必須有宣告於應用程式資訊清單中的 EndpointCertificate 和 EndpointBindingPolicy。 此通訊協定無法於後續在應用程式升級期間變更。 
...
#### <a name="passwordencrypted"></a>PasswordEncrypted
指定密碼為加密還是純文字。 元素已被取代。 請使用 'Type' 屬性指示加密密碼。
...
核心記憶體限制 (位元組)。  必須是正整數。  請注意，這是 Linux 特定項目，因此若設定此項目，Windows 上的 Docker 將會發生錯誤。
...
/dev/shm 的大小 (位元組)。 如果省略，系統會使用 64MB。  必須是正整數。  請注意，這是 Linux 特定項目，但是如果指定，Docker 不會發生錯誤，  而是會直接忽略。
...
指示是否應向服務封裝中的程式碼封裝公開網狀架構執行階段特定資訊。
...
指示是否應使用 Service Fabric 複寫的區塊儲存體，在服務封裝中提供容器的磁碟區。
