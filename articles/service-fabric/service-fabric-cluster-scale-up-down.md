...
如果您的叢集有多個節點類型, 請針對您想要調整的每個節點類型/虛擬機器擴展集重複這項操作 (輸入或輸出)。 請先考慮一定要有的節點數目，再設定自動調整規模。 主要節點類型一定要有的節點數目下限是由您已選擇的可靠性層級決定。 深入了解 [可靠性層級](service-fabric-cluster-capacity.md)。
...
> [!NOTE]
> 在相應減少案例中, 除非您的節點類型具有金色或銀[級的耐久性等級][durability], 否則您需要使用適當的節點名稱呼叫[remove-servicefabricnodestate 指令程式](https://docs.microsoft.com/powershell/module/servicefabric/remove-servicefabricnodestate)。 針對銅級持久性, 不建議一次相應減少一個以上的節點。
> 
> 
...
> [!NOTE]
> 新增 Vm 需要一些時間, 因此不會預期會有瞬間的加入。 因此, 請事先規劃新增容量, 以允許超過10分鐘的時間, 讓複本/服務實例能夠使用 VM 容量。
> 
...
Service fabric 系統服務會在叢集中的主要節點類型上執行。 向下調整主要節點類型時, 絕對不要將實例數目相應減少為小於[可靠性層級](service-fabric-cluster-capacity.md)所保證的大小。 
...
這三個步驟套用至節點之後，即可從擴展集移除節點。 如果您使用的是除了[銅][durability]以外的持久性層級, 則在移除擴展集實例時, 會為您完成這些步驟。
