...
| ---------------- |  ----------------------------  | ---------------------------------------------------------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| 金色             | 5                              | 單一客戶專用的完整節點 SKU (例如 L32s、GS5、G5、DS15_v2、D15_v2) | 可以延遲到 Service Fabric 叢集核准為止 | 每一 UD 可以暫停 2 小時，讓複本有額外時間從先前的失敗中復原 |
| 銀           | 5                              | 單一核心或更高版本的 Vm, 具有至少 50 GB 的本機 SSD                      | 可以延遲到 Service Fabric 叢集核准為止 | 無法延遲很長的期間                                                    |
| 銅色           | 1                              | 至少具有 50 GB 本機 SSD 的 Vm                                              | Service Fabric 叢集不會延遲           | 無法延遲很長的期間                                                    |
...
- 採用更安全的方式來進行 VM SKU 變更 (相應增加/減少)：變更虛擬機器擴展集的 VM SKU 需要一些步驟和考慮。 以下為您可以遵循以避免發生常見問題的程序。
    - **針對非主要節點類型:** 建議您建立新的虛擬機器擴展集, 修改服務放置條件約束以包含新的虛擬機器擴展集/節點類型, 然後將舊的虛擬機器擴展集實例計數減少為零, 一次一個節點 (這是為了讓確定移除節點並不會影響叢集的可靠性)。
...
- **要在 Azure 中執行任何生產工作負載的 VM 實例數目:** 您必須指定最小的主要節點類型大小5和銀級的可靠性層級。  
...
- **VM SKU:** 主要節點類型是系統服務執行所在的位置, 因此您為它選擇的 VM SKU 必須考慮您打算放入叢集中的整體尖峰負載。 我用比喻來闡明我的意思 - 將主要節點類型想像成您的「肺」，它供應氧氣給您的腦，如果腦沒有足夠的氧氣，您的身體就會出問題。 
...
- 建議的 VM SKU 是標準 D2_V2 或對等, 最少為 50 GB 的本機 SSD。
- 支援的最低使用 VM SKU 為 Standard_D2_V3 或標準 D1_V2, 或對等的最小值為 50 GB 的本機 SSD。 
...
**VM 實例的數目:** 對於具狀態的生產工作負載, 建議您以最低和目標複本計數5來執行它們。 這表示在穩定狀態下，最後每個容錯網域和升級網域中會有一個複本 (來自複本集)。 主要節點類型的整個可靠性層級概念，是為系統服務指定此設定。 因此，相同的考量也適用於您的具狀態服務。

因此，對於生產工作負載，如果您執行具狀態工作負載，建議的最小非主要節點類型大小為 5。

**VM SKU:** 這是您的應用程式服務執行所在的節點類型, 因此您為它選擇的 VM SKU 必須考慮您打算放入每個節點的尖峰負載。 節點類型的容量需求取決於您打算在叢集中執行的工作負載，因此，我們無法為您的特定工作負載提供確切的指引，但以下是可協助您開始進行的概括性指引
...
**VM 實例的數目:** 對於無狀態的生產工作負載, 支援的最小非主要節點類型大小為2。 這可讓您執行應用程式的兩個無狀態執行個體，在遺失 VM 執行個體的情況下，您的服務仍可繼續運作。 

**VM SKU:** 這是您的應用程式服務執行所在的節點類型, 因此您為它選擇的 VM SKU 必須考慮您打算放入每個節點的尖峰負載。 節點類型的容量需求取決於您打算在叢集中執行的工作負載。 我們無法提供適用於您特定工作負載的確切指引。  不過，以下是可協助您開始使用的概括性指引。

生產工作負載 

- 建議的 VM SKU 是標準 D2_V2 或對等。 
