...
使用 Service Fabric CLI 的 Service Fabric 應用程式升級會使用 [sfctl application upgrade](https://docs.microsoft.com/azure/service-fabric/service-fabric-sfctl-application#sfctl-application-upgrade) 命令搭配下列必要和選擇性參數。

### <a name="required-parameters"></a>必要參數

| 參數 | 描述 |
| --- | --- |
| application-id  |正在升級的應用程式識別碼。 <br> 這通常是不含 'fabric:'URI 配置的完整應用程式名稱。 從 6.0 版開始，階層的名稱會以 '\~' 字元分隔。 例如，如果應用程式名稱是 ' fabric：/myapp/app1 '，則應用程式識別在 6.0 + 中會是 ' myapp @ no__t-0app1 '，而在舊版中會是 ' myapp/app1 '。|
...
upgrade-domain-timeout | 執行 *FailureAction* 之前，每個升級網域必須完成的時間長度。 它會先解譯為代表 ISO 8601 持續時間的字串。 如果失敗，則會解譯為代表總毫秒數的數字。 預設值為永不 (無限)，您應該針對您的應用程式適當地加以自訂。 預設值：P10675199DT02H48M05.4775807S。 |
upgrade-timeout | 執行 *FailureAction* 之前，每個升級網域必須完成的時間長度。 它會先解譯為代表 ISO 8601 持續時間的字串。 如果失敗，則會解譯為代表總毫秒數的數字。 預設值為永不 (無限)，您應該針對您的應用程式適當地加以自訂。 預設值：P10675199DT02H48M05.4775807S。|
