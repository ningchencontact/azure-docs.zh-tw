---
title: Azure Service Fabric 服務模型 XML 架構元素
...
將用於執行服務程式碼套件的使用者帳戶。  使用者帳戶必須在 [主體] 區段中宣告。 通常最好是使用本機系統帳戶 (而不是系統管理員帳戶) 執行安裝程式進入點。
...
指定帳戶的類型： LocalUser、DomainUser、NetworkService、LocalService、ManagedServiceAccount 或 LocalSystem。  預設值為 LocalUser。 本機使用者帳戶會建立在應用程式部署所在的電腦上。 根據預設，這些帳戶不會具有這裡指定的相同名稱。 相反地，它們會動態產生並具有隨機密碼。 支援的本機系統帳戶類型為 LocalUser、NetworkService、LocalService 和 LocalSystem。 有 Azure Active Directory 可供使用的 Windows Server 部署可支援網域帳戶。
