...
## <a name="configpackage-element-type-configpackagedescriptiontype"></a>ConfigPackage 元素 (類型 ConfigPackageDescriptionType) 
...
|內容|0 個元素，0 個屬性|
|已定義|在 [ConfigPackagePoliciesType complexType](service-fabric-service-model-schema-complex-types.md#configpackagepoliciestype-complextype) 區域中|
|name|ConfigPackage|
|minOccurs|0|
|maxOccurs|無限制|
...
## <a name="configpackage-element-type-configpackagetype"></a>ConfigPackage 元素 (類型 ConfigPackageType) 
可在包含 Settings.xml 檔案的 PackageRoot 底下，宣告以 Name 屬性命名的資料夾。 此檔案包含程序可以在執行階段讀回的使用者定義、成對的索引鍵/值設定等區段。 在升級期間，如果只有 ConfigPackage 版本已變更，則不會重新啟動執行中程序。 相反地，回呼會通知程序組態設定已變更，因此它們可以動態方式重新載入。
...
可設定要保留的主控台重新導向輸出檔案數目。  必須為正整數，預設值為 "2"。
|屬性|值|
|---|---|
...
可設定主控台重新導向輸出檔案的大小上限。  必須為大於 128KB 的正整數，預設值為 "20480"。
|屬性|值|
|---|---|
...
|內容|0 個元素，0 個屬性|
|已定義|在 [NetworkPoliciesType complexType](service-fabric-service-model-schema-complex-types.md#networkpoliciestype-complextype) 區域中|
...
在 PackageRoot (包含要由程序在執行階段取用的靜態資料檔案) 底下宣告以 Name 屬性命名的資料夾。 當服務資訊清單中列出的資料套件有任何一個已進行升級，Service Fabric 就會回收主機和支援套件中指定的所有 EXE 和 DLLHOST。
...
|內容|0 個元素，0 個屬性|
|已定義|在 [ContainerNetworkPolicyType complexType](service-fabric-service-model-schema-complex-types.md#containernetworkpolicytype-complextype) 區域中|
...
## <a name="environmentvariable-element-type-environmentvariableoverridetype"></a>EnvironmentVariable 元素 (類型 EnvironmentVariableOverrideType) 
環境變數。
...
|內容|0 個元素，0 個屬性|
|已定義|在 [EnvironmentOverridesType complexType](service-fabric-service-model-schema-complex-types.md#environmentoverridestype-complextype) 區域中|
...
## <a name="environmentvariable-element-type-environmentvariabletype"></a>EnvironmentVariable 元素 (類型 EnvironmentVariableType) 
環境變數。
...
|內容|0 個元素，0 個屬性|
|已定義|在 [EnvironmentVariablesType complexType](service-fabric-service-model-schema-complex-types.md#environmentvariablestype-complextype) 區域中|
|name|EnvironmentVariable|
|minOccurs|0|
|maxOccurs|無限制|
...
|內容|0 個元素，0 個屬性|
|已定義|在 [ServicePackageContainerPolicyType complexType](service-fabric-service-model-schema-complex-types.md#servicepackagecontainerpolicytype-complextype)、[ContainerHostPoliciesType complexType](service-fabric-service-model-schema-complex-types.md#containerhostpoliciestype-complextype) 區域中|
|name|PortBinding|
|minOccurs|0|
|maxOccurs|無限制|
...
可指定原則以決定如何向複本公開 Service Fabric 執行階段。
...
可指定帳戶的類型：LocalUser、DomainUser、NetworkService、LocalService、ManagedServiceAccount 或 LocalSystem。  預設值為 LocalUser。 本機使用者帳戶會建立在應用程式部署所在的電腦上。 根據預設，這些帳戶不會具有這裡指定的相同名稱。 相反地，它們會動態產生並具有隨機密碼。 支援的本機系統帳戶類型為 LocalUser、NetworkService、LocalService 和 LocalSystem。 有 Azure Active Directory 可供使用的 Windows Server 部署可支援網域帳戶。
|屬性|值|
|---|---|
|name|AccountType|
|use|選用|
