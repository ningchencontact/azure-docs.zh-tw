...
* **HealthReportSendInterval**：從報告新增至用戶端到傳送給健康狀態資料存放區之間的延遲。 用於將報告批次處理為單一訊息，而不是針對每份報告傳送一則訊息。 批次處理可改善效能。 預設值：30 秒。
* **HealthReportRetrySendInterval**：健康狀態用戶端將累積的健康狀態報告重新傳送至健康狀態資料存放區的間隔。 預設值：30 秒。
* **HealthOperationTimeout**：報告訊息傳送至健康狀態資料存放區的逾時期間。 如果訊息逾時，健康狀態用戶端就會不斷重試，直到健康狀態存放區確認報告已處理。 預設值：兩分鐘。
