...
Clusterconfig.x509.multimachine.json [> .zip](https://go.microsoft.com/fwlink/?LinkId=730690)獨立叢集套件\<下載的*clusterconfig.unsecure.multimachine.json」* 檔案範例，包含使用[群組受管理的服務帳戶（WindowsServer）](https://technet.microsoft.com/library/hh831782.aspx)來設定 Windows 安全性的範本。（& i）：  
...
> [!NOTE]
> ClustergMSAIdentity 值的格式為 "mysfgmsa@mydomain"。
...
此模型已被淘汰。 建議使用 gMSA，如上所述。 使用[ServiceFabric >\<版本](https://go.microsoft.com/fwlink/?LinkId=730690)下載的範例*clusterconfig.x509.multimachine.json* 。 zip 獨立叢集套件包含用來設定 Windows 安全性的範本（template）。  Windows 安全性於 **Properties** 區段中設定︰ 
