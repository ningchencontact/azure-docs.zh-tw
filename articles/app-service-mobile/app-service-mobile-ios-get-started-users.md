---
title: 在 iOS 上新增驗證
description: 瞭解如何使用 Azure Mobile Apps 透過身分識別提供者（例如 AAD、Google、Facebook、Twitter 和 Microsoft）驗證 iOS 應用程式的使用者。
...
1. 在 Mac 上，開啟 Xcode 中的 QSTodoListViewController.m，並新增下列方法：
    ...
    如果您不使用 Google 作為識別提供者，請將 google 變更為 microsoftaccount、twitter、facebook 或 windowsazureactivedirectory。 如果您使用 Facebook，則必須在您的應用程式中將[facebook 網域列入][1]允許清單。
...
4. 開啟 `QSAppDelegate.m` 檔案並新增下列程式碼：
   ...
   將下列程式碼新增於 `#pragma mark - Core Data stack` 行的正前方。  以您在步驟1中使用的 urlScheme 值取代_appname_ 。
...
3. 開啟 `AppDelegate.swift` 檔案，並將下列程式碼行新增至 `AppDelegate` 類別：
    ...
    以您在步驟1中使用的 urlScheme 值取代_appname_ 。
...
App Service 驗證會使用 Apples Inter-App Communication。  如需此主題的詳細資訊，請參閱[Apple 檔][2]
<!-- URLs. -->
