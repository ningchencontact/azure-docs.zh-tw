---
title: 啟用離線同步處理（Xamarin iOS）
description: 瞭解如何使用 App Service 行動應用程式來快取和同步處理 Xamarin iOS 應用程式中的離線資料。
...
ms.locfileid: "74668258"
---
# <a name="enable-offline-sync-for-your-xamarinios-mobile-app"></a>為您的 Xamarin.iOS 行動裝置 App 啟用離線同步處理
...
3. 重建並執行用戶端應用程式。 應用程式的運作方式與啟用離線同步處理之前相同。不過，本機資料庫現在已填入可在離線案例中使用的資料。
