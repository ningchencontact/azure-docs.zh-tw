...
本教學課程是根據您完成教學課程 [建立 Xamarin iOS 應用程式] 時所建立之 Mobile Apps 的 Xamarin.Forms 快速入門方案。 Xamarin.Forms 的快速入門方案包含程式碼來支援離線同步處理，只需要啟用即可。 在本教學課程中，您將會更新快速入門方案來開啟 Azure Mobile Apps 的離線功能。 我們也會在應用程式中反白顯示離線特有的程式碼。 如果您不要使用下載的快速入門方案，則必須將資料存取擴充套件新增至您的專案。 有关服务器扩展包的详细信息，请参阅 [Work with the .NET backend server SDK for Azure Mobile Apps][1]（使用适用于 Azure 移动应用的 .NET 后端服务器 SDK）。
...
專案中使用 C# 前置處理器指示詞來包含離線同步處理程式碼。 定義 **OFFLINE\_SYNC\_ENABLED** 符號後，組建中會包含這些程式碼路徑。 对于 Windows 应用，还必须安装 SQLite 平台。
...
* **TodoItemManager** 中的 **todoTable** 欄位是 **IMobileServiceSyncTable** 類型，而非 **IMobileServiceTable**。 此類別使用本機資料庫來進行所有建立、讀取、更新和刪除 (CRUD) 資料表作業。 您可以在 **IMobileServiceSyncContext** 上呼叫 **PushAsync**，以決定何時將這些變更推送到行動應用程式後端。 當呼叫 **PushAsync** 時，透過追蹤和推送所有用戶端應用程式修改之資料表中的變更，同步處理內容可協助保存資料表關聯性。
    ...
    此示例使用默认同步处理程序的简单错误处理。 實際的應用程式會使用自訂 **IMobileServiceSyncHandler** 實作來處理各種錯誤，例如網路狀況和伺服器衝突。
...
2. 重建並執行用戶端應用程式。 應用程式在啟動後會嘗試與行動應用程式後端同步處理。 验证调试控制台中是否未记录任何异常。
3. (選擇性) 使用 SQL Server 物件總管或 REST 工具 (例如 Fiddler 或 [Postman][6]) 檢視已更新的資料。 请注意，数据已在后端数据库和本地存储之间进行同步。
