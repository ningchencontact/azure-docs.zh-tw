...
### <a name="step-1-check-whether-nic-is-misconfigured"></a>步骤 1：檢查 NIC 是否設定不正確
...
### <a name="step-2-check-whether-network-traffic-is-blocked-by-nsg-or-udr"></a>步骤 2：檢查 NSG 或 UDR 是否封鎖網路流量
...
### <a name="step-3-check-whether-network-traffic-is-blocked-by-vm-firewall"></a>步骤 3：檢查 VM 防火牆是否封鎖網路流量

停用防火牆，然後測試結果。 如果未解決此問題，請確認防火牆設定，然後重新啟用防火牆。

### <a name="step-4-check-whether-vm-app-or-service-is-listening-on-the-port"></a>步骤 4：檢查 VM 應用程式或服務是否正在接聽連接埠
...
### <a name="step-5-check-whether-the-problem-is-caused-by-snat"></a>步骤 5：檢查問題是否因 SNAT 而造成
...
### <a name="step-6-check-whether-traffic-is-blocked-by-acls-for-the-classic-vm"></a>步骤 6：檢查傳統 VM 的 ACL 是否封鎖流量
...
### <a name="step-7-check-whether-the-endpoint-is-created-for-the-classic-vm"></a>步骤 7：檢查是否已為傳統 VM 建立端點
...
### <a name="step-8-try-to-connect-to-a-vm-network-share"></a>步骤 8：嘗試連線至 VM 網路共用
...
### <a name="step-9-check-inter-vnet-connectivity"></a>步骤 9：檢查內部 Vnet 連線
