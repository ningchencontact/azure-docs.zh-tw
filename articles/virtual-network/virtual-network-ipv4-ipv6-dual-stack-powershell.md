...
使用 [New-AzLoadBalancer](/powershell/module/az.network/new-azloadbalancer) 建立基本負載平衡器。 下列範例會建立公用基本負載平衡器*myLoadBalancer*您在建立使用 IPv4 和 IPv6 前端 IP 組態、 後端集區、 健康情況探查、 負載平衡規則和 NAT 規則上述的步驟：
