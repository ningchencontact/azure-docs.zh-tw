...
4. 此规则允许入站 Internet 流量抵达 Web 服务器。 此規則不會變更路由的行為。 這個規則只會允許指向 IIS01 的流量通過。 因此，如果來自網際網路的流量將 Web 伺服器做為其目的地，此規則會允許流量，並停止再處理其他規則。 (在優先順序為 140 的規則中，其他所有輸入網際網路流量皆會遭到封鎖)。 如果您只要處理 HTTP 流量，則可將此規則進一步限制為只允許目的地連接埠 80。
