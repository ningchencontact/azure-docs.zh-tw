...
本文說明如何 Azure CLI 使用基本 Load Balancer 部署雙重堆疊 (IPv4 + IPv6) 應用程式, 其中包含具有雙重堆疊子網的雙重堆疊虛擬網路、具有雙重 (IPv4 + IPv6) 前端設定的基本 Load Balancer、具有 Nic 的 Vm具有雙重 IP 設定、雙重網路安全性群組規則和雙重公用 Ip。
