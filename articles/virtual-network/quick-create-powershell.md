---
title: 建立 Azure 虛擬網路 - PowerShell | Microsoft Docs
description: 快速了解如何使用 PowerShell 來建立虛擬網路。 虛擬網路可讓 Azure 資源 (例如虛擬機器) 彼此及與網際網路進行私密通訊。
...
從 *myVm1* VM 上的 PowerShell，輸入 `ping myvm2`。 Ping 會失敗，因為 Ping 使用網際網路控制訊息通訊協定 (ICMP)，而 Windows 防火牆預設並不允許 ICMP 通過。
