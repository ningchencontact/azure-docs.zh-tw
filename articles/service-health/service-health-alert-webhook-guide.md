---
title: 使用 webhook 傳送 Azure 服務健康狀態通知
...
請參閱 `ServiceHealth` webhook 承載[的範例](../azure-monitor/platform/activity-log-alerts-webhook.md)。

您可以查看 `context.eventSource == "ServiceHealth"`來確認它是服務健康狀態警示。 以下是最相關的屬性：
- **activityLog。狀態**
- **activityLog. level**
...
服務健康狀態警示可以通知您跨多個區域和服務的問題。 若要取得完整的詳細資料，您必須剖析 `impactedServices`的值。
