...
假設您使用相當小的資料樣本 (相對於完整資料集範例 (n = 11748))，模型精確度相當高 (為 85%)，而 RMSE 在預測計程車車資價格方面大約會有 +- 4.00 美元的誤差。 下一個提高精確度的可能步驟是，返回此筆記本的第二個資料格，然後從每月 2,000 筆記錄提高樣本大小，接著再次執行整個實驗，以使用更多資料來將模型重新定型。
