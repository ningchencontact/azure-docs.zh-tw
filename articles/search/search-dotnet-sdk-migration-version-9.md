...
這是因為在複雜欄位的情況下，這些屬性現在必須 `null`。 如果您有可讀取這些屬性的程式碼，就必須準備好處理 `null`。 請注意，`Field` 的所有其他屬性一律都是可為 null，而且在複雜欄位的情況下，其中部分也會 `null`，尤其是下列內容：
...
已 `internal``Field` 的無參數的函式。 從現在開始，每個 `Field` 都需要在結構時使用明確的名稱和資料類型。
...
已移除 `ExtensibleEnum` 的基類。 所有衍生自它的類別現在都是結構，例如 `AnalyzerName`、`DataType`和 `DataSourceType`。 其 `Create` 方法也已移除。 您可以直接移除對 `Create` 的呼叫，因為這些類型可從字串隱含轉換。 如果這會導致編譯器錯誤，您可以透過轉型為區分類型來明確叫用轉換運算子。 例如，您可以變更程式碼，如下所示：
...
`FacetResults` 和 `HitHighlights` 類別已移除。 Facet 結果現在會輸入為 `IDictionary<string, IList<FacetResult>>`，並將焦點放在 `IDictionary<string, IList<string>>`。 解決這項變更所引進之組建錯誤的快速方法，就是在使用已移除類型的每個檔案頂端加入 `using` 別名。 例如：
...
`AutocompleteParameters` 的 `AutocompleteMode` 屬性已不再是可為 null。 如果您有將此屬性指派給 `null`的程式碼，您可以直接將它移除，而且屬性會自動初始化為預設值。

`IndexAction` 的參數順序已變更，現在已自動產生此函數。 我們建議使用 factory 方法 `IndexAction.Upload`、`IndexAction.Merge`等等，而不是使用此函數。

### <a name="removed-preview-features"></a>已移除的預覽功能

如果您要從版本 8.0-preview 升級到第9版，請注意，已移除客戶管理金鑰的加密，因為這項功能仍處於預覽狀態。 具體而言，`Index` 和 `SynonymMap` 的 `EncryptionKey` 屬性已經移除。
...
如果您使用「動態類型」 `Search`、`Suggest`或會傳回 `Document`類型實例的 `Get` Api，請注意，它們現在會將空的 JSON 陣列還原序列化為 `object[]` 而不是 `string[]`。
