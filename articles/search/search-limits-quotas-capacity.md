...
| 每份檔的所有複雜集合的元素上限&nbsp;<sup>2</sup> |3000 |3000 |3000 |3000 |3000 |3000 |3000 |3000 |
...
<sup>2</sup>每份檔的複雜集合中有非常大量的元素，目前會導致高儲存體使用率。 這是已知的問題。 在此同時，限制為3000是所有服務層級的安全上限。 此限制只會針對使用最早正式推出（GA） API 版本的索引編制作業強制執行，而此版本支援複雜類型欄位（`2019-05-06`）。 若不中斷可能使用舊版預覽 API 版本（支援複雜類型欄位）的用戶端，我們將不會對使用這些預覽 API 版本的索引作業強制執行這項限制。 請注意，預覽 API 版本不適合用于生產案例，我們強烈建議客戶移至最新的 GA API 版本。

<a name="document-limits"></a>

## <a name="document-limits"></a>文件限制 

截至 2018 年 10 月，在任何區域中任何計費層 (基本、S1、S2、S3、S3 HD) 建立的任何新的服務，都不再有任何文件限制。 雖然大部分的區域自2017年11月/12 月起都有無限制的檔計數，但有幾個區域會繼續在該日期之後強加檔限制。 根據您建立搜尋服務的時間和地點，您可能正在執行仍受限於文件限制的服務。
...
> [!NOTE]
> 雖然沒有任何 SKU 特定檔限制，但每個索引仍然受限於最大的安全限制，以確保服務的穩定性。 這是來自 Lucene 的限制。 每個 Azure 認知搜尋檔都會在內部編制為一或多個 Lucene 檔的索引。 每個搜尋檔的 Lucene 檔數目取決於複雜集合欄位中的元素總數。 每個元素都會以個別的 Lucene 檔編制索引。 例如，在複雜集合欄位中包含3個元素的檔，將會以 4 Lucene 檔的形式編制索引，而針對該檔本身，則會以3個為元素。 Lucene 檔的最大數目大約是每個索引25000000000。
...
> [!NOTE]
> 如[索引限制](#index-limits)所述，從支援複雜類型（`2019-05-06`）的最新 GA API 版本開始，索引子也會在每份檔的所有複雜集合上強制執行3000個元素的上限。 這表示如果您已使用先前的 API 版本建立索引子，則不會受限於此限制。 為了保留最大相容性，使用先前的 API 版本建立的索引子，然後使用 `2019-05-06` 或更新版本的 API 版本進行更新，仍會從限制中**排除**。 客戶應該留意具有非常龐大複雜集合（如先前所述）的負面影響，而且我們強烈建議使用最新的 GA API 版本來建立任何新的索引子。
