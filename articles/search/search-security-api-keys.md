...
---
# <a name="create-and-manage-api-keys-for-an-azure-cognitive-search-service"></a>建立和管理 Azure 認知搜尋服務的 api 金鑰
...
> [!NOTE]  
>  在要求 URI 中傳遞敏感性資料 (例如 `api-key`) 被視為不佳的安全性做法。 基於這個理由，Azure 認知搜尋只接受查詢索引鍵做為查詢字串中的 `api-key`，而且您應該避免這麼做，除非您的索引內容應該可公開使用。 一般規則是建議將 `api-key` 當作要求標頭來傳遞。  
...
3. 選取服務，然後在 [總覽] 頁面上，按一下 [**設定**] >[**金鑰**] 來查看管理和查詢金鑰。
...
> [!Note]
> [在中查詢 Azure 認知搜尋索引中C#](search-query-dotnet.md)可以找到顯示查詢金鑰使用方式的程式碼範例。
