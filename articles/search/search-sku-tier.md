...
+ 準備一個小型、具代表性的資料集。
+ [在入口網站中建立初始索引](search-create-index-portal.md)，並記下其大小。 功能和屬性會影響存放裝置。 例如，新增建議工具（自動提示）將會增加儲存需求。 使用相同的資料集，您可能會嘗試建立多個版本的索引，每個欄位各有不同的屬性，以查看儲存體需求的差異。 如需詳細資訊，請參閱[建立基本索引中的「儲存體影響](search-what-is-an-index.md#index-size)」。
