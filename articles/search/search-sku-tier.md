---
title: 在搜尋服務布建期間選擇定價層或 SKU
titleSuffix: Azure Cognitive Search
description: Azure 認知搜尋可以在這些 Sku 中布建：免費、基本和標準，而標準則適用于各種資源設定和容量層級。
manager: nitinme
author: HeidiSteen
ms.author: heidist
...
ms.locfileid: "72794259"
---
# <a name="choose-a-pricing-tier-for-azure-cognitive-search"></a>選擇 Azure 認知搜尋的定價層

當您建立 Azure 認知搜尋服務時，會在服務存留期固定的定價層（或 SKU）上[建立資源](search-create-service-portal.md)。 層包括免費、基本、標準和儲存體優化。 標準和儲存體優化提供數個設定和容量。
...
如需詳細資訊，請參閱[定價頁面](https://azure.microsoft.com/pricing/details/search/)上的「 [Azure 認知搜尋的服務限制](search-limits-quotas-capacity.md)」一文，以及當您布建服務時，入口網站頁面上的各層。
...
不同于可「暫停」以避免費用的虛擬機器或其他資源，Azure 認知搜尋服務一律可供您專屬使用的硬體使用。 因此，建立服務是一種可計費事件，會在您建立服務時啟動，並在您刪除服務時結束。 
...
如果服務位於不同的區域，則會收取輸出資料的費用。 這些費用並不是您 Azure 認知搜尋帳單的一部分。 這裡有提到，因為如果您使用資料或 AI 擴充的索引子，從不同的區域提取資料，您會看到整體帳單中反映的成本。

### <a name="ai-enrichment-with-cognitive-services"></a>具有認知服務的 AI 擴充
...
| [內建認知技能](cognitive-search-predefined-skills.md) | 以與您直接使用認知服務執行工作時相同的費率來計費。 |
...
若要瞭解 Azure 認知搜尋作業，最重要的計費概念是*搜尋單位*（SU）。 由於「Azure 認知搜尋」相依于索引和查詢的複本和資料分割，因此只有一或多個資料分割才有意義。 相反地，會將兩者合併來計費。
...
- 將所有服務合併成一個資源群組，例如 Azure 認知搜尋、認知服務，以及解決方案中使用的任何其他 Azure 服務。 在 Azure 入口網站中，尋找資源群組，並使用**成本管理**命令，以深入瞭解實際和預估的支出。
...
在 Azure 認知搜尋中，容量會結構化為*複本* *和分割*區。
...
若要判斷索引的大小，您必須[建置一個](search-create-index-portal.md)索引。 Azure 認知搜尋中的資料結構主要是[反向索引](https://en.wikipedia.org/wiki/Inverted_index)結構，其具有與來源資料不同的特性。 針對反向索引，大小和複雜度取決於內容，而不一定是您饋送至其中的資料量。 具有高冗余的大型資料來源可能會產生比包含高度變數內容的較小資料集更小的索引。 因此，很少可以根據原始資料集的大小來推斷索引大小。
...
> [!NOTE]
> 如果檔包含無關的資料，則儲存需求可能會擴大。 在理想的情況下，檔只包含搜尋體驗所需的資料。 二進位資料無法搜尋，且應分別儲存（可能是在 Azure 資料表或 blob 儲存體中）。 接著，您應該在索引中加入一個欄位，以保存外部資料的 URL 參考。 個別檔的大小上限為 16 MB （如果您要在一個要求中大量上傳多個檔，則會較少）。 如需詳細資訊，請參閱[Azure 認知搜尋中的服務限制](search-limits-quotas-capacity.md)。
>
...
如果您預計從一開始就有很大的查詢量，您應該考慮較高的標準層，並以更強大的硬體做為後盾。 接著，您可以讓分割區和複本離線，或甚至切換至較低層的服務（如果未發生這些查詢磁片區）。 如需如何計算查詢輸送量的詳細資訊，請參閱[Azure 認知搜尋效能和優化](search-performance-optimization.md)。
...
從免費層開始，並使用您資料的子集來建立初始索引，以瞭解其特性。 Azure 認知搜尋中的資料結構是反向的索引結構。 反向索引的大小和複雜性取決於內容。 請記住，重複性高的內容所產生的索引，往往會比差異性高的內容所產生的索引小。 因此，內容特性，而不是資料集的大小，會決定索引儲存需求。
