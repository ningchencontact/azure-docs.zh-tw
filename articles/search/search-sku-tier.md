...
description: Azure 搜尋服務可以在這些 Sku 中布建:「免費」、「基本」、「標準」和「標準」在各種資源設定和容量層級都有提供。
services: search
author: HeidiSteen
manager: nitinme
tags: ''
...
最低費用是第一個搜尋單位 (一個複本 x 一個資料分割)。 此最小值在服務的存留期間內已修正, 因為服務無法在此設定的任何專案上執行。 除了最小值之外, 您還可以單獨新增複本和分割區。 透過複本和分割區增量增加的容量, 將會根據下列公式來增加您的帳單: [(複本 x 分割區 x 速率)](#search-units), 而您需支付的費用取決於您選取的定價層。
...
SU 是服務所使用之*複本*和資料分割的乘積: **(R x P = SU)** 。

每個服務都以一個 SU (一個複本乘以一個分割區) 作為最小值。 任何服務的最大值為 36 su。 可以透過多種方式達到此上限:6個數據分割 x 6 個複本, 或3個分割區 x 12 個複本, 例如。 通常會使用小於總容量 (例如, 3 個複本、3個數據分割服務, 以9個 su 計費)。 如需有效的組合, 請參閱資料[分割和複本組合](search-capacity-planning.md#chart)圖表。
...
容量和執行服務的成本是直接相關的。 階層會限制兩個層級: 儲存體和資源。 您應該考慮這兩個原因, 因為您第一次達到的限制是有效的限制。
...
### <a name="step-1-develop-rough-estimates-by-using-the-free-tier"></a>步驟 1:使用免費層開發粗略估計

估計容量的一種方法是從免費層開始。 請記住, 免費服務最多可提供三個索引、50 MB 的儲存空間, 以及2分鐘的編制索引時間。 使用這些條件約束估計預計的索引大小可能會很困難。 以下是您可以採取的方法:

+ [建立免費服務](search-create-service-portal.md)。
+ 準備小型、具代表性的資料集 (例如5000檔和 10% 樣本大小)。
+ [建立初始索引](search-create-index-portal.md)並記下其在入口網站中的大小 (例如 30 MB)。

如果此範例為代表, 且整個資料來源為 10%, 如果所有檔都已編制索引, 則 30 MB 的索引會變成大約 300 MB。 有了這個初稿號碼, 您可以將這兩個索引的數量加倍 (開發和生產)。 這可提供您儲存體需求的總計 600 MB。 基本層可輕鬆滿足這項需求, 因此您將從這裡開始。

### <a name="step-2-develop-refined-estimates-by-using-a-billable-tier"></a>步驟 2:使用可計費層來開發精簡的估價

有些客戶會想要從專用資源開始, 以容納較大的取樣和處理時間, 然後在開發期間開發索引數量、大小和查詢磁片區的實際估計。 一開始, 服務是根據最佳猜測的估計來布建。 然後, 隨著開發專案的成熟, 小組通常會知道現有的服務已超過或低於預測生產工作負載的容量。
...
1. 在入口網站中[監視儲存體、服務限制、查詢量和延遲](search-monitor-usage.md)。 入口網站會顯示每秒查詢數、節流查詢, 以及搜尋延遲。 所有這些值都可協助您決定是否選取正確的層級。 您也可以藉由啟用[搜尋流量分析](search-traffic-analytics.md), 設定深度監視值, 例如點選連結分析。
...
+ 瞭解如何建立有效率的索引, 並瞭解哪些重新整理方法的影響最小。 使用[搜尋流量分析](search-traffic-analytics.md)來取得查詢活動的深入解析。
...
在您有索引大小的初始估計之後, 請在本文討論的其中一個層級上布建可[計費服務](search-create-service-portal.md):基本、標準或儲存體已優化。 放寬資料大小的任何人工條件約束, 並[重建您的索引](search-howto-reindex.md), 以包含您想要搜尋的所有資料。
