...
**SearchServiceName**: (字串) 搜尋服務名稱**SearchId**: (guid) 搜尋查詢的唯一識別碼 (位於搜尋回應中) **IndexName**: (字串) 搜尋要查詢的服務索引**QueryTerms**: (字串) 搜尋使用者**ResultCount**: (int) 所傳回的檔數目 (出現在搜尋回應中) **ScoringProfile**: (字串) 所使用評分設定檔的名稱 (如果有的話)
