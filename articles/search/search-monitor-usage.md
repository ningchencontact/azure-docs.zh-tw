...
Azure 監視器記錄和 Blob 儲存體皆以免費共用服務的形式提供, 因此您可以在 Azure 訂用帳戶的存留期免費試用。 Application Insights 可供免費註冊和使用，只要應用程式資料大小在特定限制範圍內即可 (如需詳細資訊，請參閱[價格頁面](https://azure.microsoft.com/pricing/details/monitor/))。
...
| 平均 |ssNoversion |64 |度量時間間隔中原始範例的平均值 |
| 最小值 |ssNoversion |37 |度量時間間隔中原始範例的最小值 |
| 最大值 |ssNoversion |78 |度量時間間隔中原始範例的最大值 |
| 總計 |ssNoversion |258 |度量時間間隔中原始範例的總和值 |
| 計數 |ssNoversion |4 |用來產生度量的原始樣本數 |
