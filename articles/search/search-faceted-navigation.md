...
雖然此行為隨時可能會變更，但如果您今天遇到這項行為，您可以透過人為因而誇大的次數來解決此問題，>\<數位，以強制執行每個分區的完整報告。 如果計數: 的值大於或等於欄位中唯一值的數目，您就能保證結果正確。 不過，當文件計數很高的時候，則會有效能的負面影響，因此請謹慎使用此選項。
...
1. 若要使用示範應用程式的對應功能，請從 [Bing 地圖服務開發人員中心](https://www.bingmapsportal.com/)取得 Bing 地圖服務的金鑰。 將它貼在 `index.cshtml` 頁面中的現有金鑰上。 系統不會使用 `BingApiKey` 檔案中的 `Web.config` 設定。 
