---
title: 設定資料輸入複寫，以將資料複寫至適用於 MariaDB 的 Azure 資料庫。
...
## <a name="create-a-mariadb-server-to-be-used-as-replica"></a>建立 MariaDB 伺服器做為複本

1. 建立新的適用於 MariaDB 的 Azure 資料庫伺服器

   建立新的 MariaDB 伺服器 (例如 "replica.mariadb.database.azure.com")。 若要了解如何建立伺服器，請參閱[使用 Azure 入口網站建立適用於 MariaDB 的 Azure 資料庫伺服器](quickstart-create-mariadb-server-database-using-azure-portal.md)。 此伺服器是複寫中資料的「複本」伺服器。

   > [!IMPORTANT]
   > 適用於 MariaDB 的 Azure 資料庫伺服器必須建立於一般用途或記憶體最佳化定價層。
   > 

2. 建立相同的使用者帳戶和對應權限

   使用者帳戶不會從主要伺服器複寫到複本伺服器。 如果您預計提供複本伺服器存取權給使用者，則必須在此新建的適用於 MariaDB 伺服器的 Azure資料庫中，手動建立所有帳戶與對應權限。

## <a name="configure-the-master-server"></a>設定主要伺服器
下列步驟會針對裝載在內部部署的 MariaDB 伺服器、虛擬機器中的 MariaDB 伺服器或由其他雲端提供者所代管的資料庫服務，準備及設定資料輸入複寫。 此伺服器是「資料輸入複寫」中的「主要」伺服器。 
...
1. 開啟二進位記錄
   ...
   如果變數 [`log_bin`](https://mariadb.com/kb/en/library/replication-and-binary-log-server-system-variables/#log_bin) 傳回的值是 “ON"，表示伺服器上的二進位記錄已啟用。 

   如果 `log_bin` 傳回 “OFF” 值，請開啟二進位記錄，方法是編輯 my.cnf 以確保 `log_bin=ON`，然後將伺服器重新開機使變更生效。

2. 主要伺服器設定

   「資料輸入複寫」要求主要伺服器和複本伺服器之間的參數 `lower_case_table_names` 一致。 此參數在適用於 MariaDB 的 Azure 資料庫中預設為 1。 
...
5. 取得二進位記錄檔的檔案名稱和位移
   ...
   執行 [`show master status`](https://mariadb.com/kb/en/library/show-master-status/) 命令，以判斷目前二進位記錄檔的檔案名稱和位移。
...
1. 從主要伺服器傾印所有資料庫
   ...
   您可使用 mysqldump 從主要伺服器傾印資料庫。 如需詳細資料，請參閱[傾印和還原](howto-migrate-dump-restore.md)。 您不需要傾印 MySQL 程式庫和測試程式庫。

2. 將主要伺服器設為讀取/寫入模式

   傾印資料庫後，請將主要的 MariaDB 伺服器重新變更為讀取/寫入模式。
...
3. 將傾印檔案還原至新的伺服器
   ...
   將傾印檔案還原至在適用於 MariaDB 的 Azure 資料庫服務中建立的伺服器。 請參閱[傾印和還原](howto-migrate-dump-restore.md)，了解如何將傾印檔案還原至 MariaDB 伺服器。 如果傾印檔案太大，請先在與複本伺服器所在區域相同的區域中，將檔案上傳至 Azure 內的虛擬機器。 從虛擬機器還原至適用於 MariaDB 的 Azure 資料庫伺服器。
...
1. 設定主要伺服器
   ...
   *使用 SSL 的複寫*
   ...
   在主要伺服器 (裝載於 “companya.com” 網域) 與複本伺服器 (裝載於適用於 MariaDB 的 Azure 資料庫) 之間設定「使用 SSL 的複寫」。 此已儲存的程序可在複本伺服器上執行。 
   ...
   *不使用 SSL 的複寫*
   ...
   在主要伺服器 (裝載於 “companya.com” 網域) 與複本伺服器 (裝載於適用於 MariaDB 的 Azure 資料庫) 之間設定「不使用 SSL 的複寫」。 此已儲存的程序可在複本伺服器上執行。
