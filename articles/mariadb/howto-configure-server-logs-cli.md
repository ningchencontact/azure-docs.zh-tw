...
若要列出伺服器的可用記錄檔，請執行 [az mariadb server-logs list](/cli/azure/mariadb/server-logs#az-mariadb-server-logs-list) 命令。
...
使用 [az mariadb server-logs download](/cli/azure/mariadb/server-logs#az-mariadb-server-logs-download) 命令，即可下載您伺服器適用的個別記錄檔。
...
- 深入了解[適用於 MariaDB 的 Azure 資料庫中的伺服器記錄檔](concepts-server-logs.md)。
