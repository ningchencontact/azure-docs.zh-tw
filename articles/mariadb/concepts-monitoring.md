...
您可以在伺服器上啟用慢速查詢記錄。 這些記錄檔中也會有透過 Azure 診斷記錄 Azure 監視器記錄檔、 事件中樞和儲存體帳戶。 若要深入了解記錄，請瀏覽 [伺服器記錄](concepts-server-logs.md)頁面。
...
[查詢效能深入解析](concepts-query-performance-insight.md)可搭配查詢存放區提供可從 Azure 入口網站存取的視覺效果。 這些圖表可讓您識別影響效能的關鍵查詢。 查詢效能深入解析處於公開預覽狀態，並可**智慧型效能**向您的 Azure 資料庫 MariaDB 伺服器的入口網站頁面的區段。
