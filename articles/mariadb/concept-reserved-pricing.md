---
title: 預付具有保留容量的計算-適用於 MariaDB 的 Azure 資料庫
description: 預付以保留容量適用於 MariaDB 的 Azure 資料庫計算資源
author: kummanish
ms.author: manishku
ms.service: mariadb
ms.topic: conceptual
...
適用於 MariaDB 的 Azure 資料庫現在可協助您節省與隨用隨付價格相比的計算資源還要划算成本。 有了適用於 MariaDB 的 Azure 資料庫保留容量，您可以在適用于 mariadb 伺服器上預先承諾一年的時間，以取得計算成本的大量折扣。 若要購買適用於 MariaDB 的 Azure 資料庫保留容量，您必須指定 Azure 區域、部署類型、效能層級和期限。 </br>

您不需要將保留指派給特定的適用於 MariaDB 的 Azure 資料庫伺服器。 已執行適用於 MariaDB 的 Azure 資料庫或新部署的，將會自動獲得保留定價的權益。 藉由購買保留，您會預先支付一年中計算成本的費用。 一旦您購買保留，符合保留屬性的 Azure database for 適用于 mariadb 計算費用將不再以隨用隨付費率計費。 保留專案並不涵蓋與適用于 mariadb 資料庫伺服器相關聯的軟體、網路或儲存體費用。 在保留期限結束時，計費權益會到期，而適用於 MariaDB 的 Azure 資料庫會以隨用隨付價格計費。 保留不會自動更新。 如需定價資訊，請參閱[適用於 MariaDB 的 Azure 資料庫保留容量](https://azure.microsoft.com/pricing/details/mariadb/)供應專案。 </br>

您可以在[Azure 入口網站](https://portal.azure.com/)中購買適用於 MariaDB 的 Azure 資料庫保留容量。 若要購買保留容量：

* 您至少必須是一個企業或個別訂用帳戶的擁有者角色，且具有隨用隨付費率。
* 針對企業訂用帳戶，必須在 [EA 入口網站](https://ea.azure.com/)中啟用**新增保留執行個體**。 或者，如果該設定已停用，則您必須是訂用帳戶上的 EA 系統管理員。
* 針對雲端解決方案提供者（CSP）計畫，只有系統管理員代理程式或銷售代理人可以適用於 MariaDB 的 Azure 資料庫保留容量購買。 </br>
...
2. 選取 [所有服務] > [保留]。
3.  選取 [**新增**]，然後在 [購買保留] 窗格中選取 [**適用於 MariaDB 的 Azure 資料庫**]，為您的適用于 mariadb 資料庫購買新的保留。
4.  填寫必要欄位。 符合您所選屬性的現有或新資料庫，可以取得保留容量折扣。 取得折扣的適用於 MariaDB 的 Azure 資料庫伺服器實際數目，取決於選取的範圍和數量。


![保留定價的總覽](media/concepts-reserved-pricing/mariadb-reserved-price.png)
...
| 欄位 | 描述 |
| :------------ | :------- |
| Subscription   | 用來支付保留容量適用於 MariaDB 的 Azure 資料庫的訂用帳戶。 訂用帳戶的付款方法會收取適用於 MariaDB 的 Azure 資料庫保留容量保留期的前期成本。 訂用帳戶類型必須是 enterprise 合約（供應專案號碼： MS-AZR-0017P-Ms-azr-0017p 或 MS-MS-AZR-0017P-Ms-azr-0148p）或具有隨用隨付定價的個別合約（供應專案號碼： MS-MS-AZR-0017P-Ms-azr-0003p 或 MS-MS-AZR-0017P-Ms-azr-0023p）。 針對企業訂用帳戶，費用會從註冊的承諾用量金額餘額扣除或作為超額部分收費。 針對使用隨用隨付定價的個別訂用帳戶，費用會依訂用帳戶的信用卡或發票付款方法計費。
...
| 地區 | 適用於 MariaDB 的 Azure 資料庫保留容量保留所涵蓋的 Azure 區域。
