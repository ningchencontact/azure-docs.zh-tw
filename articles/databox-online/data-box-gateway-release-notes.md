...
| **2.** |由於此版本有錯誤，您可能會在 error.xml 看到錯誤碼 110 的執行個體 (具有無法辨識的項目名稱)。 | 
| **3.** |由於此版本有錯誤，您可能會在特定檔案上傳期間看到錯誤碼 2003 的執行個體。 | 
| **4.** |在此版本中，您一次只可以重新整理一個共用。 | 
...
| **3.** |重新命名 |不支援將物件重新命名。 |如果這項功能對您的工作流程極為重要，請連絡 Microsoft 支援服務。 |
| **4.** |複製| 如果唯讀檔案已複製到裝置，則不會保留唯讀屬性。 | |
