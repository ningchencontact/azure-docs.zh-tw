...
6. 在 [指定 vCenter Server 詳細資料]  中，執行下列動作：
    ...
    - 在 [收集範圍]  中，選取 VM 探索的範圍。 收集器只能探索指定範圍內的 VM。 範圍可以設定為特定資料夾、資料中心或叢集。 它不應包含超過 1500 個 VM。 [深入了解](how-to-scale-assessment.md)如何探索較大的環境。
       ...
       > [!NOTE]
       > **集合範圍**僅會列出主機和叢集的資料夾。 無法直接選取 VM 的資料夾作為集合範圍。 不過，您可以使用可存取個別 VM 的 vCenter 帳戶進行探索。 [深入了解](https://docs.microsoft.com/azure/migrate/how-to-scale-assessment#set-up-permissions)如何將範圍設為 VM 的資料夾。
