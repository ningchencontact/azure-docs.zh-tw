...
2. 搜尋**Azure 移轉**，並選取服務**Azure 移轉 （預覽）**搜尋結果中。 接著，按一下 [建立]。
3. 指定專案名稱，以及專案的 Azure 訂用帳戶。
4. 建立新的資源群組。
5. 指定要在其中建立專案，然後按一下 位置**建立**。 在這個預覽版中，您只能在「美國中西部」區域建立 Azure Migrate 專案。 不過，您仍能針對任何目標 Azure 位置規劃移轉。 指定給專案的位置只用於儲存收集在內部部署的 Vm 所傳來的中繼資料。 
...
2. 提供設備的語言、時區和密碼喜好設定。
3. 在桌面上，按一下 [執行收集器] 捷徑。
4. 在 Azure 移轉行程中，開啟**設定先決條件**。
    - 接受授權條款，並閱讀第三方資訊。
    - 收集器會確認 VM 是否能夠存取網際網路。
    - 如果 VM 能夠透過 Proxy 存取網際網路，請按一下 [Proxy 設定]，然後指定 Proxy 位址和接聽連接埠。 如果 Proxy 需要驗證，請指定認證。

    > [!NOTE]
    > Proxy 位址必須在表單 http://ProxyIPAddress 或 http://ProxyFQDN 輸入。 支援僅有 HTTP proxy。

    - 收集器會檢查 collectorservice 正在執行。 根據預設，收集器 VM 上會安裝此服務。
...
5. 在**指定 vCenter 伺服器詳細資料**，執行下列動作：
    ...
    - 在 [收集範圍] 中，選取 VM 探索的範圍。 收集器只能探索指定範圍內的 VM。 範圍可以設定為特定資料夾、資料中心或叢集。 它不應該包含超過 1000 個 VM。 
    - 在 [用來分組的標籤類別] 中，選取 [無]。
6. 在**指定移轉專案**、 指定的 Azure 移轉專案識別碼，以及您從入口網站複製的索引鍵。 如果您之前未複製，請從收集器 VM 開啟 Azure 入口網站。 在專案的 [概觀] 頁面中，按一下 [探索機器]，然後複製值。  
7. 在**檢視收集進度**，監視探索，並檢查 收集到的 Vm 所傳來的中繼資料不在範圍內。 收集器會提供概略的探索時間。
...
> [!NOTE]
> Azure Migrate 所提供的成本估計適用於執行內部部署 VM 以作為 Azure 基礎結構即服務 (IaaS) VM。 Azure 移轉不會考慮任何平台即服務 (PaaS) 或軟體即服務 (SaaS) 成本。 
...
- [深入了解](how-to-scale-assessment.md)如何探索及評估大型的 VMware 環境。
- 了解如何建立使用高信心評估群組[機器相依性對應](how-to-create-group-machine-dependencies.md)
