...
在本文中，Contoso 會使用 Azure Site Recovery 服務，將其 SmartHotel 應用程式前端 VM 遷移至 Azure。 Contoso 也會將應用程式資料庫遷移至 Azure SQL Database 受控執行個體。
...
[文章 11：在 Visual Studio Team Services 上重構 Team Foundation Server](contoso-migration-tfs-vsts.md) | Contoso 會藉由將其內部部署 Team Foundation Server 部署遷移至 Azure 中的 Visual Studio Team Services 來對該部署進行遷移。 | 可用
...
您可以從 [GitHub](https://github.com/Microsoft/SmartHotel360) 下載本文中所用的範例 SmartHotel 應用程式。
...
- 這個應用程式橫跨兩層 VM (**WEBVM** 和 **SQLVM**)，而且位於 VMware ESXi 6.5 版主機 (**contosohost1.contoso.com**)。 
- VMware 環境是由 VM 上執行的 vCenter Server 6.5 (**vcenter.contoso.com**) 進行管理。
- Contoso 會將應用程式資料庫 (**SmartHotelDB**) 遷移到 Azure SQL Database 受控執行個體。
- Contoso 會將內部部署 VMware VM 遷移到 Azure VM。
...
服務 | 說明 | 成本
--- | --- | ---
[資料庫管理服務](https://docs.microsoft.com/azure/dms/dms-overview) | 資料庫管理服務能夠從多個資料庫來源無縫移轉到 Azure 資料平台，將停機時間降到最低。 | 深入了解[支援的區域](https://docs.microsoft.com/azure/dms/dms-overview#regional-availability)和[資料庫管理服務定價](https://azure.microsoft.com/pricing/details/database-migration/)。
...
Contoso 會完成下列步驟，以將 SmartHotel 應用程式的 Web 和資料層遷移至 Azure：
...
> * **步驟 2：準備資料庫管理服務**：Contoso 必須註冊資料庫移轉提供者、建立執行個體，然後建立資料庫管理服務專案。 Contoso 也必須為資料庫管理服務設定共用存取簽章 (SAS) 統一資源識別項 (URI)。 SAS URI 可提供 Contoso 儲存體帳戶資源的委派存取權，以便 Contoso 對儲存體物件授與有限的權限。 Contoso 會設定 SAS URI，以便資料庫管理服務存取儲存體帳戶容器 (此為服務在上傳 SQL Server 備份檔案時的上傳目的地)。
...
> * **步驟 6：使用資料庫管理服務遷移資料庫**：Contoso 會遷移資料庫。
...
Contoso 會設定虛擬網路，如下所示： 

1. Contoso 會在主要美國東部 2 區域中建立新的虛擬網路 (**VNET-SQLMI-EU2**)。 它會將此虛擬網路新增至 **ContosoNetworkingRG** 資源群組。
2. Contoso 會指派 10.235.0.0/24 位址空間。 Contoso 會確保範圍不會與其企業中的任何其他網路重疊。
2. Contoso 會將兩個子網路新增到此網路：
...
6. 部署虛擬網路和子網路之後，Contoso 會將網路對等互連，如下所示：
...
7. Contoso 可設定自訂 DNS 設定。 DNS 會先指向 Contoso 的 Azure 網域控制站。 而後指向 Azure DNS。 Contoso Azure 網域控制站的位置如下所示：
...
1. Contoso 會建立 UDR 資料表。 Contoso 會在 **ContosoNetworkingRG** 資源群組中建立路由表。

    ![路由表](media/contoso-migration-rehost-vm-sql-managed-instance/mi-route-table.png)

2. 若要符合受控執行個體需求，在部署路由表 (**MIRouteTable**) 之後，Contoso 會新增位址前置詞為 0.0.0.0/0 的路由。 [下一個躍點類型] 選項會設定為 [網際網路]。
...
1. 因為受控執行個體會提供一個商務應用程式，所以 Contoso 會在公司的主要美國東部 2 區域中部署受控執行個體。 Contoso 會將受控執行個體新增至 **ContosoRG** 資源群組。
2. Contoso 會選取執行個體的定價層、大小計算和儲存體。 深入了解[受控執行個體定價](https://azure.microsoft.com/pricing/details/sql-database/managed/)。
...
## <a name="step-2-prepare-the-database-management-service"></a>步驟 2︰準備資料庫管理服務

若要準備資料庫管理服務，Contoso 需要執行一些作業：

- 在 Azure 中註冊資料庫管理服務提供者。
- 將 Azure 儲存體存取權提供給資料庫管理服務，以便上傳用來遷移資料庫的備份檔案。 為了提供 Azure 儲存體的存取權，Contoso 會建立 Azure Blob 儲存體容器。 Contoso 會產生 Blob 儲存體容器的 SAS URI。 
- 建立資料庫管理服務專案。

接著，Contoso 會完成下列步驟：

1. Contoso 會在其訂用帳戶之下註冊資料庫移轉提供者。
    ![資料庫管理服務 - 註冊](media/contoso-migration-rehost-vm-sql-managed-instance/dms-subscription.png)

2. Contoso 會建立 Blob 儲存體容器。 Contoso 會產生 SAS URI，以便資料庫管理服務存取它。

    ![資料庫管理服務 - 產生 SAS URI](media/contoso-migration-rehost-vm-sql-managed-instance/dms-sas.png)

3. Contoso 會建立資料庫管理服務執行個體。 

    ![資料庫管理服務 - 建立執行個體](media/contoso-migration-rehost-vm-sql-managed-instance/dms-instance.png)

4. Contoso 會將資料庫管理服務執行個體置於 **VNET-PROD-DC-EUS2** 虛擬網路的 **PROD-DC-EUS2** 子網路中。
    - Contoso 會將資料庫管理服務放在該處，因為此服務必須在可透過 VPN 閘道存取內部部署 SQL Server VM 的虛擬網路中。
    - **VNET-PROD-EUS2** 會與 **VNET-HUB-EUS2** 對等互連，並且能夠使用遠端閘道。 [使用遠端閘道] 選項確保資料庫管理服務可以視需要進行通訊。

        ![資料庫管理服務 - 設定網路](media/contoso-migration-rehost-vm-sql-managed-instance/dms-network.png)

*需要其他協助？*

- 了解如何[設定資料管理服務](https://docs.microsoft.com/azure/dms/quickstart-create-data-migration-service-portal)。
...
Contoso 會按照下列方式設定 Site Recovery：

1. 因為 VM 是 SmartHotel 應用程式的 Web 前端，所以 Contoso 會將 VM 容錯移轉到其現有的生產網路 (**VNET-PROD-EUS2**) 和子網路 (**PROD-FE-EUS2**)。 此網路和子網路都位於主要美國東部 2 區域中。 Contoso 會在[部署 Azure 基礎結構](contoso-migration-infrastructure.md)時設定此網路。
2. Contoso 會建立儲存體帳戶 (**contosovmsacc20180528**)。 Contoso 會使用一般用途的帳戶。 Contoso 會選取標準儲存體和本地備援儲存體複寫。

    ![Site Recovery - 建立儲存體帳戶](media/contoso-migration-rehost-vm-sql-managed-instance/asr-storage.png)

3. 網路和儲存體帳戶準備就緒之後，Contoso 會建立保存庫 (**ContosoMigrationVault**)。 Contoso 會將此保存庫放在主要美國東部 2 區域的 **ContosoFailoverRG** 資源群組中。
...
若要針對 Site Recovery 準備 VMware，Contoso 必須完成下列工作：
...
- Contoso 會在 Azure 主控台中設定複寫時指定此帳戶。
...
容錯移轉至 Azure 之後，Contoso 希望能夠連線到 Azure 中複寫的 VM。 若要連線到 Azure 中複寫的 VM，Contoso 必須在移轉之前，在內部部署 VM 上完成一些工作： 

1. 如需透過網際網路存取，Contoso 會在容錯移轉之前，在內部部署 VM 上啟用 RDP。 Contoso 會確定已針對 [公用] 設定檔新增 TCP 和 UDP 規則，且在 [Windows 防火牆] > [允許的應用程式] 中已針對所有設定檔允許 RDP。
2. 如需透過站對站 VPN 存取，Contoso 會在內部部署機器上啟用 RDP。 Contoso 會在 [Windows 防火牆] > [允許的應用程式與功能] 中，允許 [網域和私人] 網路使用 RDP。
3. Contoso 會將內部部署 VM 的作業系統 SAN 原則設定為 **OnlineAll**。

Contoso 在執行容錯移轉時，也必須檢查下列項目：

- 觸發容錯移轉時，VM 上不應該有擱置的 Windows 更新。 如果 Windows 更新擱置，在更新完成前，Contoso 無法登入虛擬機器。
- 容錯移轉之後，Contoso 應勾選 [開機診斷] 以檢視 VM 的螢幕擷取畫面。 如果 Contoso 無法檢視開機診斷，則應檢查 VM 是否正常執行，然後檢閱[疑難排解祕訣](http://social.technet.microsoft.com/wiki/contents/articles/31666.troubleshooting-remote-desktop-connection-after-failover-using-asr.aspx)。

## <a name="step-5-replicate-the-on-premises-vms-to-azure-by-using-site-recovery"></a>步驟 5：使用 Site Recovery 將內部部署 VM 複寫至 Azure

在執行移轉至 Azure 之前，Contoso 必須設定複寫，然後為其內部部署 VM 啟用複寫。

### <a name="set-a-replication-goal"></a>設定複寫目標

1. 在保存庫中的保存庫名稱 (**ContosoVMVault**) 下，Contoso 會設定複寫目標 ([開始使用] > [Site Recovery] > [準備基礎結構])。
2. Contoso 會指定其機器都位於內部部署環境、都是 VMware VM，而且 Contoso 會想要複寫至 Azure。
...
若要繼續，Contoso 必須確認它已完成部署規劃。 為了確認，Contoso 會選取 [是，我已完成]。 在此部署中，Contoso 只會移轉單一 VM，所以不需要部署規劃。

### <a name="set-up-the-source-environment"></a>設定來源環境

Contoso 現在必須設定其來源環境。 若要設定其來源環境，Contoso 會下載 OVF 範本。 Contoso 會使用此範本，將組態伺服器與其相關聯的元件部署為高可用性、內部部署 VMware VM。 伺服器上的元件包括︰
...
- 處理序伺服器，可作為複寫閘道。 處理序伺服器：
    ...
    - 使用快取、壓縮和加密，將複寫資料最佳化。
    - 將複寫日期傳送至 Azure 儲存體。
- 處理序伺服器也會在 Contoso 想要複寫的 VM 上安裝行動服務。 處理序伺服器會在內部部署 VMware VM 上執行自動探索。
- 建立並啟動組態伺服器 VM 之後，Contoso 會在保存庫中註冊伺服器。

若要設定來源環境：

1. Contoso 會從 Azure 入口網站 ([準備基礎結構] > [來源] > [組態伺服器]) 下載 OVF 範本。
    
    ![新增組態伺服器](./media/contoso-migration-rehost-vm-sql-managed-instance/add-cs.png)

2. Contoso 會將範本匯入 VMware 中，以建立和部署 VM。

    ![部署 OVF 範本](./media/contoso-migration-rehost-vm-sql-managed-instance/vcenter-wizard.png)

3.  當 Contoso 第一次啟動 VM 時，VM 會在 Windows Server 2016 安裝程序中啟動。 Contoso 會接受授權合約，並輸入系統管理員密碼。
4. 安裝完成後，Contoso 會以系統管理員身分登入 VM。 Contoso 第一次登入 VM 時，Azure Site Recovery 設定工具會自動執行。
5. 在 Site Recovery 設定工具中，Contoso 會輸入一個名稱，以在保存庫中註冊組態伺服器。
6. 此工具會檢查 VM 與 Azure 的連線。 建立連線之後，Contoso 會選取 [登入] 以登入 Azure 訂用帳戶。 認證必須能夠存取在其中註冊組態伺服器的保存庫。 

    ![註冊組態伺服器](./media/contoso-migration-rehost-vm-sql-managed-instance/config-server-register2.png)

7. 此工具會執行一些設定工作，而後重新開機。 Contoso 會再次登入機器。 組態伺服器管理精靈會自動啟動。
8. 在此精靈中，Contoso 會選取要接收複寫流量的 NIC。 這項設定在完成之後即無法變更。
9. Contoso 會選取訂用帳戶、資源群組，以及要在其中註冊組態伺服器的復原服務保存庫。

    ![選取復原服務保存庫](./media/contoso-migration-rehost-vm-sql-managed-instance/cswiz1.png)

10. Contoso 會下載並安裝 MySQL Server 和 VMWare PowerCLI。 然後，Contoso 會驗證伺服器設定。
11. 驗證之後，Contoso 會指定 vCenter Server 執行個體或 vSphere 主機的 FQDN 或 IP 位址。 Contoso 會保留預設的連接埠，並且在 Azure 中輸入 vCenter Server 執行個體的顯示名稱。
12. Contoso 必須指定先前所建立的帳戶，以便 Site Recovery 自動探索可用於複寫的 VMware VM。 
13. Contoso 會輸入認證，以便在啟用複寫時自動安裝行動服務。 對於 Windows 機器，此帳戶需具備 VM 的本機系統管理員權限。 
...
8. Site Recovery 會使用指定的設定，連線至 VMware 伺服器。 Site Recovery 會探索 VM。

### <a name="set-up-the-target"></a>設定目標

Contoso 現在必須設定目標複寫環境：

1. 在 [準備基礎結構] > [目標] 中，Contoso 會選取目標設定。
2. Site Recovery 會確認在指定的目標中有儲存體帳戶和網路。

### <a name="create-a-replication-policy"></a>建立複寫原則

設定好來源和目標之後，Contoso 會建立複寫原則，並將讓此原則與組態伺服器建立關聯：

1. 在 [準備基礎結構] > [複寫設定] > [複寫原則] >  [建立和關聯] 中，Contoso 會建立 **ContosoMigrationPolicy** 原則。
2. Contoso 會使用預設設定：
...
現在，Contoso 可以開始複寫 WebVM。

1. 在 [複寫應用程式] > [來源] > [複寫] 中，Contoso 會選取來源設定。
2. Contoso 表示想要啟用虛擬機器、選取 vCenter Server 執行個體，以及設定組態伺服器。

    ![啟用複寫 - 來源](./media/contoso-migration-rehost-vm-sql-managed-instance/enable-replication1.png)
 
3. Contoso 會指定目標設定，包括 Azure VM 在容錯移轉之後所在的資源群組和網路。 Contoso 會指定要儲存所複寫資料的儲存體帳戶。

    ![啟用複寫 - 目標](./media/contoso-migration-rehost-vm-sql-managed-instance/enable-replication2.png)

4. Contoso 會選取 **WebVM** 進行複寫。 Site Recovery 會在複寫啟用時在每部 VM 上安裝行動服務。 

    ![啟用複寫 - 選取 VM](./media/contoso-migration-rehost-vm-sql-managed-instance/enable-replication3.png)

5. Contoso 會確認已選取正確的複寫原則。 然後，針對 **WEBVM** 啟用複寫。 Contoso 會在 [作業] 中追踨複寫進度。 執行 [完成保護] 作業之後，機器即準備好進行容錯移轉。
6. 在 Azure 入口網站的 [基本資訊] 中，Contoso 可以看到複寫至 Azure 的 VM 結構：
...
## <a name="step-6-migrate-the-database-by-using-the-database-management-service"></a>步驟 6：使用資料庫管理服務遷移資料庫

Contoso 必須建立資料庫管理服務專案，然後遷移資料庫。

### <a name="create-a-database-management-service-project"></a>建立資料庫管理服務專案

1. Contoso 會建立資料庫管理服務專案。 Contoso 會選取 [SQL Server] 來源伺服器類型。 Contoso 會選取 [Azure SQL Database 受控執行個體] 作為目標。

     ![資料庫管理服務 - 新增移轉專案](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-project.png)

2. 移轉精靈隨即開啟。

### <a name="migrate-the-database"></a>遷移資料庫 

1. 在移轉精靈中，Contoso 會指定內部部署資料庫所在的來源 VM。 Contoso 會提供用於存取資料庫的認證。

    ![資料庫管理服務 - 來源詳細資料](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-wizard-source.png)

2. Contoso 會選取要遷移的資料庫 (**SmartHotel.Registration**)：

    ![資料庫管理服務 - 選取來源資料庫](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-wizard-sourcedb.png)

3. 針對目標，Contoso 會輸入 Azure 中的受控執行個體名稱。 Contoso 會輸入受控執行個體的存取認證。

    ![資料庫管理服務 - 目標詳細資料](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-target-details.png)

4. 在 [新增活動] > [執行移轉] 中，Contoso 會指定下列設定來執行移轉：
    - 來源和目標認證。
    - 要遷移的資料庫。
    - Contoso 在內部部署 VM 上建立的網路共用。 資料庫管理服務會將來源備份擷取到此共用。 
        - 執行來源 SQL Server 執行個體的服務帳戶必須擁有此共用的寫入權限。
        - 必須使用共用的 FQDN 路徑。
    - SAS URI，此 URI 可供資料庫管理服務存取儲存體帳戶容器，而此服務會將備份檔案上傳至該容器以供移轉。

        ![資料庫管理服務 - 設定移轉設定](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-migration-settings.png)

5. Contoso 會儲存移轉，然後執行它。
6. 在 [概觀] 中，Contoso 會監視移轉狀態。

    ![資料庫管理服務 - 監視](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-monitor1.png)

7. 完成移轉後，Contoso 會確認受控執行個體上有目標資料庫。

    ![資料庫管理服務 - 驗證資料庫移轉](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-monitor2.png)

## <a name="step-7-migrate-the-vm-by-using-site-recovery"></a>步驟 7：使用 Site Recovery 遷移 VM

Contoso 會執行快速測試容錯移轉，然後遷移 VM。

### <a name="run-a-test-failover"></a>執行測試容錯移轉

在遷移 WEBVM 之前，測試容錯移轉有助於確保一切運作正常。 Contoso 會完成下列步驟：

1. Contoso 會針對最新的可用時間點 (**最近處理的**) 執行一次測試容錯移轉。
2. Contoso 會選取 [先將機器關機再開始容錯移轉]。 選取這個選項後，Site Recovery 嘗試先關閉來源 VM，再觸發容錯移轉。 即使關機失敗，仍會繼續容錯移轉。 
3. 執行測試容錯移轉： 
    1. 系統會執行先決條件檢查，以確保移轉所需的全部條件都已準備就緒。
    2. 容錯移轉會處理資料，以便建立 Azure VM。 如果選取了最新的復原點，則會根據資料建立復原點。
    3.  使用先前步驟中處理的資料來建立 Azure VM。
3. 容錯移轉完成後，Azure VM 複本會出現在 Azure 入口網站中。 Contoso 會確認一切運作正常：VM 為適當的大小、其已連線到正確的網路，而且正在執行中。 
4. 確認測試容錯移轉之後，Contoso 會清除容錯移轉。 然後，記錄並儲存任何觀察到的結果。 

### <a name="migrate-the-vm"></a>移轉 VM

1. 驗證測試容錯移轉如預期般正常運作之後，Contoso 會針對移轉建立一個復原方案。 Contoso 會將 WEBVM 新增至此方案：

     ![建立復原計畫 - 選取項目](./media/contoso-migration-rehost-vm-sql-managed-instance/recovery-plan.png)

2. Contoso 會對此方案執行容錯移轉。 Contoso 會選取最新的復原點。 Contoso 指定 Site Recovery 應該在觸發容錯移轉之前，先嘗試關閉內部部署 VM。
...
4. 在 Azure 中確認 VM 之後，Contoso 會完成移轉程序、停止 VM 複寫，以及停止 VM 的 Site Recovery 計費。
...
在移轉程序的最後一個步驟中，Contoso 會將應用程式的連接字串更新為指向在 Contoso 受控執行個體上執行的遷移後資料庫。

1. 在 Azure 入口網站中，Contoso 會選取 [設定] > [連接字串] 來尋找連接字串。

    ![連接字串](./media/contoso-migration-rehost-vm-sql-managed-instance/failover4.png)  

2. Contoso 會使用 SQL Database 受控執行個體的使用者名稱與密碼來更新此字串。
3. 設定此字串之後，Contoso 會取代其應用程式的 web.config 檔案中目前的連接字串。
4. 更新該檔案並加以儲存後，Contoso 會在 WEBVM 上重新啟動 IIS。 若要重新啟動 IIS，Contoso 會在命令提示字元視窗中執行 `IISRESET /RESTART`。
5. 重新啟動 IIS 後，應用程式會使用在 SQL Database 受控執行個體上執行的資料庫。
6. 此時，Contoso 可以關閉內部部署 SQLVM 機器。 已完成移轉。
...
移轉完成時，SmartHotel 應用程式會在 Azure VM 上執行，而 SmartHotel 資料庫可適用於 Azure SQL Database 受控執行個體。  
...
- 安全性小組會檢閱用來控制 VM 存取權的網路安全性群組。 網路安全性群組有助於確保只可以傳遞該應用程式允許的流量。
- Contoso 安全性小組也會考慮使用 Azure 磁碟加密和 Azure KeyVault 來保護磁碟上的資料。
- 安全性小組會在受控執行個體上啟用威脅偵測。 如果偵測到威脅，威脅偵測會將警示傳送至 Contoso 的安全性小組/服務台系統。 深入了解[受控執行個體的威脅偵測](https://docs.microsoft.com/azure/sql-database/sql-database-managed-instance-threat-detection)。
...
Contoso 會使用 Azure 備份服務來備份 WEBVM 上的資料。 深入了解 [Azure 備份](https://docs.microsoft.com/azure/backup/backup-introduction-to-azure-backup?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)。
...
在本文中，Contoso 使用 Site Recovery 服務將應用程式前端 VM 遷移至 Azure，以重新裝載 SmartHotel 應用程式。 Contoso 會使用 Azure 資料庫管理服務，將內部部署資料庫遷移至 Azure SQL Database 受控執行個體。

## <a name="next-steps"></a>後續步驟

在這系列的下一篇文章中，Contoso 會使用 Azure Site Recovery 服務，[在 Azure VM 上重新裝載 SmartHotel 應用程式](contoso-migration-rehost-vm.md)。
