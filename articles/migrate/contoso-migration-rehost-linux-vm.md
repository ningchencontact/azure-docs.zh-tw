...
這份文件是一系列文章中的其中一篇，描述 Contoso 這家虛構的公司如何將其內部部署資源遷移至 Microsoft Azure 雲端。 整個系列文章中包含背景資訊以及一組案例，用以說明如何設定移轉基礎結構，以及執行不同類型的移轉。 案例會變得越來越複雜，而我們也將不定期增加其他文章。
...
[文章 5：重新裝載至 Azure VM](contoso-migration-rehost-vm.md) | 說明 Contoso 如何將其 SmartHotel 應用程式 VM 遷移至 Azure VM。 | 可用
...
文章 7：將 Linux 應用程式重新裝載至 Azure VM | 說明 Contoso 如何使用 Azure Site Recovery 遷移其 osService Linux 應用程式。 | 本文。
...
- Contoso 不想在這個應用程式上投注資源。  這對企業很重要，但以其目前的狀況而言，他們只想安全地移至雲端。
