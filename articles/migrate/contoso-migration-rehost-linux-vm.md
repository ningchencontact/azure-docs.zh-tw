...
這份文件是一系列文章中的第七篇，描述 Contoso 這家虛構的公司如何將其內部部署資源遷移至 Microsoft Azure 雲端。 整個系列文章中包含背景資訊以及一組案例，用以說明如何設定移轉基礎結構，以及執行不同類型的移轉。 案例會變得越來越複雜，我們會不定期增加其他文章。
...
文章 7：將 Linux 應用程式重新裝載至 Azure VM (本文) | 說明 Contoso 如何使用 Azure Site Recovery 遷移其 osService Linux 應用程式。
...
IT 領導小組與其商務合作夥伴密切合作，了解他們從此次移轉想要實現什麼目標：

- **解決業務成長**：Contoso 的業務量日益增多，對內部部署系統和基礎結構造成了壓力。
- **限制風險**：服務台應用程式對 Contoso 的業務影響甚大。 他們想要在亳無風險的情況下，將它移至 Azure。
- **擴充**：他們不想要立即變更應用程式。 他們只想要確保應用程式穩定。
...
- Contoso 不想投資這個應用程式。  這對企業很重要，但以其目前的格式而言，他們只想安全地移至雲端。
...
2. 準備好 Azure 和內部部署元件之後，則會為 VM 設定和啟用複寫功能。
3. 在複寫功能正常運作之後，他們會將 VM 容錯移轉至 Azure 以便進行遷移。
...
以下是您 (和 Contoso) 應該準備好的事項。

**需求** | **詳細資料**
--- | ---
**Azure 訂用帳戶** | 您應該早就在本系列文章的前幾篇文章中，建立了訂用帳戶。 如果您沒有 Azure 訂用帳戶，請建立[免費帳戶](https://azure.microsoft.com/pricing/free-trial/)。<br/><br/> 如果您建立免費帳戶，您就是訂用帳戶的管理員，並可執行所有動作。<br/><br/> 如果您使用現有訂用帳戶，而且您不是系統管理員，則需要與系統管理員合作，讓其指派擁有者或參與者權限給您。<br/><br/> 如果您需要更細微的權限，請檢閱[此文章](../site-recovery/site-recovery-role-based-linked-access-control.md)。 
...
> * **步驟 2：針對 Site Recovery 準備內部部署 VMware**：他們會準備要用於探索 VM 和安裝代理程式的帳戶，以及準備在容錯移轉後連線至 Azure VM。
> * **步驟 3：複寫 VM**：他們會設定來源和目標移轉環境、建立複寫原則，並開始將 VM 複寫至 Azure 儲存體。
> * **步驟 4：使用 Site Recovery 遷移 VM**：他們會執行測試容錯移轉，確定一切都能正常運作，然後執行完整的容錯移轉，以便將 VM 遷移至 Azure。
...
- 容錯移轉資源所在的 VNet (Contoso 將使用他們已經部署的生產 VNet)
- 新的 Azure 儲存體帳戶，用來保存複寫的資料。 
- Azure 中的一個復原服務保存庫。

Contoso 已經在 [Azure 基礎結構部署](contoso-migration-infrastructure.md)期間建立 VNet ，所以他們只需要建立儲存體帳戶和保存庫。

1. Contoso 會在美國東部 2 區域中建立 Azure 儲存體帳戶 (contosovmsacc20180528)。

    - 儲存體帳戶與復原服務保存庫必須位於相同的區域。
    - 他們會使用一般用途的帳戶，配備標準儲存體和 LRS 複寫。

    ![Site Recovery 儲存體](./media/contoso-migration-rehost-linux-vm/asr-storage.png)

2. 網路和儲存體帳戶準備就緒之後，Contoso 會建立保存庫 (ContosoMigrationVault)，然後將它放在美國東部 2 號主要區域的 **ContosoFailoverRG** 資源群組中。
...
Contoso 會按照下列方式準備內部部署 VMware 基礎結構：

- 在 vCenter Server 或 vSphere ESXi 主機上建立一個帳戶，將 VM 探索自動化。
- 建立一個帳戶，其允許在您想要複寫的 VMware VM 上自動安裝行動服務。
- 準備內部部署 VM，以便在移轉之後於建立時連線到 Azure VM。
...
容錯移轉至 Azure 之後，Contoso 希望能夠連線到 Azure 中複寫的 VM。 若要這麼做，他們還需執行一些作業：

- 若要透過網際網路存取，請在移轉前，先在內部部署 Linux VM 上啟用 SSH。  若為 Ubuntu，可以使用下列命令來完成此動作：**Sudo apt-get ssh install -y**。
- 執行移轉 (容錯移轉) 之後，應勾選 [開機診斷] 來檢視 VM 的螢幕擷取畫面。
- 如果這個不起任何作用，則應檢查 VM 是否正常執行，並且檢閱這些[疑難排解祕訣](http://social.technet.microsoft.com/wiki/contents/articles/31666.troubleshooting-remote-desktop-connection-after-failover-using-asr.aspx)。
...
Contoso 必須先設定及啟用複寫，才可以將 Web VM 遷移至 Azure。
...
Contoso 必須設定其來源環境。 若要這樣做，他們需要下載 OVF 範本，然後用它來將 Site Recovery 組態伺服器部署為高可用性、內部部署 VMware VM。 當組態伺服器設定好並正在執行之後，他們將它註冊到保存庫。
...
6. 此工具會檢查 VM 是否可連線到 Azure。 建立連線之後，他們會登入 Azure 訂用帳戶。 認證必須能夠存取您要在其中註冊組態伺服器的保存庫。
...
11. 他們會接著下載並安裝 MySQL Server 和 VMWare PowerCLI。 
...
14. 註冊完成後，Contoso 會在 Azure 入口網站中確認組態伺服器和 VMware 伺服器都已列在保存庫中的 [來源] 頁面上。 探索可能需要 15 分鐘以上的時間。 
...
Contoso 現在可以開始複寫 **OSTICKETWEB** VM。
...
### <a name="create-and-customize-a-recovery-plan"></a>建立並自訂復原方案
 ...
 驗證測試容錯移轉如預期般正常運作之後，Contoso 會針對移轉建立一個復原方案。 
...
4.  在 [+群組] > [新增受保護項目] 中，他們會將 **OSTICKETWEB** 新增至 [群組 2：開始]。  Contoso 需要將這些項目放在兩個不同的群組中。
...
Contoso 已準備好在復原方案上執行容錯移轉，以便遷移 VM。
...
在移轉程序的最後一個步驟中，Contoso 會將應用程式的連接字串更新為指向在 **OSTICKETMYSQL** VM 上執行的應用程式資料庫。 
...
- 他們會更新其內部文件以顯示新位置，以及 OSTICKETWEB 和 OSTICKETMYSQL 的 IP 位址。
- 他們會檢閱與 VM 互動的任何資源，並更新任何相關的設定或文件，以反映新的組態。
- Contoso 使用了 Azure Migrate 服務搭配相依性對應，評定要進行遷移的 VM。 基於此目的，他們應該從 VM 中移除 Microsoft Monitoring Agent 以及他們所安裝的 Dependency Agent。
...
- 他們也會考慮使用磁碟加密和 Azure KeyVault 來保護 VM 磁碟上的資料。
...
Contoso 會使用 Azure 備份服務來備份 VM 上的資料。 [深入了解](https://docs.microsoft.com/azure/backup/backup-introduction-to-azure-backup?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)。
