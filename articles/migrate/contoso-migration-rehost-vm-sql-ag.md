...
本文說明 Contoso 如何在 Azure 中重新裝載 SmartHotel 應用程式。 他們會將應用程式前端 VM 移轉至 Azure VM，並將應用程式資料庫移轉至 Azure SQL Server VM，在具有 SQL Server AlwaysOn 可用性群組的 Windows Server 容錯移轉叢集中執行。

這份文件是系列文章中的其中一篇，說明 Contoso 這家虛構的公司如何將內部部署資源移轉至 Microsoft Azure 雲端。 整個系列文章中包含背景資訊和說明如何設定移轉基礎結構的案例、評估內部部署資源的移轉，以及執行不同類型的移轉。 案例會變得越來越複雜，而我們也將不定期增加其他文章。
...
[文章 5：在 Azure VM 上重新裝載應用程式](contoso-migration-rehost-vm.md) | 說明 Contoso 如何單純憑藉 Site Recovery 來移轉 SmartHotel 應用程式 VM。 | 可用
文章 6：在 Azure VM 和 SQL Server AlwaysOn 可用性群組上重新裝載應用程式 | 說明 Contoso 如何移轉 SmartHotel 應用程式。 Contoso 使用 Site Recovery 來移轉應用程式 VM，並使用資料庫移轉服務，將應用程式資料庫移轉至受到 SQL Server 可用性群組保護的 SQL Server 叢集。 | 本文
...
[文章 9：在 Azure Web 應用程式和 Azure SQL 資料庫上重構應用程式](contoso-migration-refactor-web-app-sql.md) | 說明 Contoso 如何將 SmartHotel 應用程式移轉至 Azure Web 應用程式，以及如何將應用程式資料庫移轉至 Azure SQL Server 執行個體 | 可用
[文章 10：在 Azure Web Apps 和 Azure MySQL 上重構 Linux 應用程式](contoso-migration-refactor-linux-app-service-mysql.md) | 說明 Contoso 如何將 Linux osTicket 應用程式移轉至多個網站中的 Azure Web Apps (與 GitHub 整合以進行持續傳遞)。 他們會將應用程式資料庫移轉至 Azure MySQL 執行個體。 | 可用
[文章 11：在 VSTS 上重構 TFS](contoso-migration-tfs-vsts.md) | 說明 Contoso 如何將其內部部署 Team Foundation Server (TFS) 部署移轉至 Azure 中的 Visual Studio Team Services (VSTS)，以進行移轉。 | 可用
[文章 12：在 Azure 容器和 Azure SQL Database 上重新建構應用程式](contoso-migration-rearchitect-container-sql.md) | 說明 Contoso 如何將其 SmartHotel 應用程式移轉和重新建構至 Azure。 他們會將應用程式的 Web 層重新建構為 Windows 容器及 Azure SQL Database 中的應用程式資料庫。 | 可用
[文章 13：在 Azure 中重建應用程式](contoso-migration-rebuild.md) | 示範 Contoso 如何使用各種 Azure 功能和服務 (包括 App Services、Azure Kubernetes、Azure Functions、認知服務及 Cosmos DB) 重建 SmartHotel 應用程式。 | 可用



在此文章中，Contoso 會將在 VMware VM 上執行的兩層 Windows .NET SmartHotel 應用程式移轉到 Azure。 如果您想使用此應用程式，您可以從 [GitHub](https://github.com/Microsoft/SmartHotel360) 下載其開放原始碼。
...
- Contoso 不想在這個應用程式上投注資源。  這對企業很重要，但就其目前的形式而言，他們只想安全地將其移至雲端。
- 應用程式的內部部署資料庫有可用性方面問題。 他們想要將其部署於 Azure 中，作為具有容錯移轉能力的高可用性叢集。
...
**服務** | **說明** | **成本**
--- | --- | ---
[資料庫管理服務](https://docs.microsoft.com/azure/dms/dms-overview) | Contoso 會使用 DMS 來移轉應用程式資料層。 DMS 會透過站對站 VPN 連線至內部部署 SQLVM 機器，而移轉 DMS 能夠從多個資料庫來源無縫移轉至 Azure 資料平台，將停機時間降到最低。 | 了解 DMS 的[支援區域](https://docs.microsoft.com/azure/dms/dms-overview#regional-availability)，並取得[定價詳細資料](https://azure.microsoft.com/pricing/details/database-migration/)。
[Azure Site Recovery](https://docs.microsoft.com/azure/site-recovery/) | Contoso 會使用 Site Recovery 執行應用程式前端 VM 的隨即轉移。 Site Recovery 可協調和管理 Azure VM 以及內部部署 VM 和實體伺服器的移轉和災害復原。  | 複寫至 Azure 的期間會產生 Azure 儲存體費用。  發生容錯移轉時，系統會建立 Azure VM 並產生費用。 [深入了解](https://azure.microsoft.com/pricing/details/site-recovery/)費用和定價。

 

## <a name="migration-process"></a>移轉程序

- Contoso 會將應用程式 VM 移轉至 Azure。
...
- 他們會使用資料移轉服務 (DMS)，將資料庫移轉至 Azure 中的 SQL Server 叢集。
...
> [!div class="checklist"]
> * **步驟 1：在 Azure 中建立 SQL Server VM**：為了達到高可用性，Contoso 想要在 Azure 中部署叢集資料庫。 他們部署了兩個 SQL Server VM，和一個 Azure 內部負載平衡器。
> * **步驟 2：部署叢集**：在部署 SQL Server VM 之後，他們開始準備 Azure SQL Server 叢集。  他們會將其資料庫移轉至這個預先建立的叢集中。
> * **步驟 3：準備 DMS**：為了準備 DMS，他們會註冊資料庫移轉提供者、建立 DMS 執行個體，並建立專案。 他們會設定共用存取簽章 (SAS) 統一資源識別項 (URI)。 DMS 會使用 SAS URI 來存取服務將 SQL Server 備份檔案上傳到的儲存體帳戶容器。
> * **步驟 4：為 Azure 進行 Site Recovery 的準備工作**：他們會建立一個 Azure 儲存體帳戶來保存複寫的資料，並建立 Site Recovery 保存庫。
> * **步驟 5：為內部部署 VMware 進行 Site Recovery 的準備工作**：他們會準備帳戶以便探索 VM 和安裝代理程式，並準備內部部署 VMware，使其可在容錯移轉後連線至 Azure VM。
> * **步驟 6：複寫 VM**：他們會設定複寫設定，並啟用 VM 複寫。
> * **步驟 7：使用 DMS 移轉資料庫**：他們會移轉資料庫。
> * **步驟 8：使用 Site Recovery 移轉 VM**：他們會先執行測試容錯移轉，確定一切都能正常運作，然後再執行完整的容錯移轉，以移轉 VM。


## <a name="step-1-prepare-a-sql-server-alwayson-availability-group-cluster"></a>步驟 1：準備 SQL Server AlwaysOn 可用性群組叢集

Contoso 想要藉由在 Azure 的叢集中部署資料庫，以進行事先規劃。 然後，他們可以將此叢集用於其他資料庫。 

- SQL Server VM 將部署在 ContosoRG 資源群組中 (用於生產資源)。
- 除了唯一名稱以外，這兩個 VM 所使用的設定皆相同。
- 內部負載平衡器將部署在 ContosoNetworkingRG 中 (用於網路資源)。
- VM 會執行具有 SQL Server 2017 Enterprise Edition 的 Windows Server 2016。 Contoso 不具備此作業系統的授權，因此他們將使用 Azure Marketplace 中以 Azure EA 承諾用量計費的形式提供授權的映像。
...
## <a name="step-2-deploy-the-failover-cluster"></a>步驟 2：部署容錯移轉叢集
...
在設定叢集之前，Contoso 為每個機器的作業系統磁碟建立快照集。
...
1. Contoso 使用容錯移轉叢集管理員中的**仲裁設定精靈**來設定雲端見證。
...
在 AlwaysOn 啟用後，Contoso 可以設定將會保護 SmartHotel 資料庫的 AlwaysOn 可用性群組。
...
Contoso 此時想要部署位於叢集節點前面的內部負載平衡器。 負載平衡器會接聽流量，並將其導向至適當的節點。
...
部署內部負載平衡器後，Contoso 必須加以設定。 他們建立後端位址集區、設定健康情況探查，並設定負載平衡規則。

### <a name="add-a-backend-pool"></a>新增後端集區

為了將流量分散至叢集中的各個 VM，Contoso 設定了後端位址集區，其中包含將會從負載平衡器接收網路流量之 VM NIC 的 IP 位址。
...
Contoso 建立了健康情況探查，讓負載平衡器能夠監視應用程式的健康情況。 此探查會根據 VM 對健康情況檢查的回應，以動態方式從負載平衡器輪替中新增或移除 VM。
...
此時，Contoso 必須使用負載平衡器規則來定義如何將流量分散至 VM。
...
2. Contoso 設定前端接聽程式，以在 TCP 1433 上接收傳入的 SQL 用戶端流量。
3. 他們指定流量所將路由到的後端集區，以及 VM 用來接聽流量的連接埠。
4. Contoso 啟用浮動 IP (伺服器直接回傳)。 SQL AlwaysOn 一律需要此功能。
...
1.  Contoso 在[部署 Azure 基礎結構](contoso-migration-rehost-vm-sql-ag.md)時，即已建立可用於 Site Recovery 的網路/子網路。
    ...
    - SmartHotel 應用程式是一個生產應用程式，而 WEBVM 將會移轉至美國東部 2 主要區域中的 Azure 生產網路 (VNET-PROD-EUS2)。
    - WEBVM 將會置於 ContosoRG 資源群組中 (用於生產資源)，並且置於生產子網路中 (PROD-FE-EUS2)。

2. Contoso 在主要區域中建立 Azure 儲存體帳戶 (contosovmsacc20180528)。
...
1. Contoso 在 vCenter 層級建立一個角色。
2. 接著，Contoso 為該角色指派必要的權限。
...
容錯移轉之後，Contoso 想要連線至 Azure VM。 為此，他們在移轉之前執行下列作業：
...
- [了解](https://docs.microsoft.com/azure/site-recovery/vmware-azure-tutorial-prepare-on-premises#prepare-an-account-for-mobility-service-installation)如何建立行動服務推送安裝的帳戶。


## <a name="step-5-replicate-the-on-premises-vms-to-azure-with-site-recovery"></a>步驟 5：使用 Site Recovery 將內部部署 VM 複寫至 Azure

Contoso 必須先設定並啟用複寫，才能執行移轉至 Azure 的作業。
...
Contoso 必須設定其來源環境。 為此，他們下載 OVF 範本，然後用它將 Site Recovery 組態伺服器部署為高可用性、內部部署的 VMware VM。 當組態伺服器開始執行之後，他們將它註冊到保存庫。
...
3. Contoso 選取要複寫的 WebVM、檢查複寫原則並啟用複寫。 Site Recovery 會在複寫啟用時在 VM 上安裝行動服務。
 
    ![啟用複寫](./media/contoso-migration-rehost-vm-sql-ag/enable-replication3.png)

4. 他們在 [作業] 中追踨複寫進度。 執行 [完成保護] 作業之後，機器即準備好進行容錯移轉。
5. 在 Azure 入口網站的 [基本資訊] 中，Contoso 可以看到 VM 複寫至 Azure 的結構。
...
## <a name="step-5-install-the-database-migration-assistant-dma"></a>步驟 5：安裝 Database Migration Assistant (DMA)

Contoso 會使用 DMA 將 SmartHotel 資料庫移轉至 Azure VM **SQLAOG1**。 他們依照下列方式設定 DMA：
...
## <a name="step-6-migrate-the-database-with-dma"></a>步驟 6：使用 DMS 移轉資料庫
...
DMS 會透過 Contoso 資料中心與 Azure 之間的站對站 VPN 連線來連線至內部部署 SQL Server VM，然後再移轉資料庫。

## <a name="step-7-protect-the-database"></a>步驟 7：保護資料庫

在應用程式資料庫執行於 **SQLAOG1** 的情況下，Contoso 現在可以使用 AlwaysOn 可用性群組加以保護。 他們使用 SQL Management Studio 設定 AlwaysOn，然後使用 Windows 叢集指派接聽程式。 
...
2. 在 [指定選項] 中，他們將可用性群組命名為 **SHAOG**。 在 [選取資料庫] 中，他們選取 SmartHotel 資料庫。
...
在設定 SQL 部署的最後一個步驟中，Contoso 將內部負載平衡器設定作為叢集上的接聽程式，並使接聽程式上線。 他們使用指令碼來執行此作業。
...
在所有設定完成後，Contoso 此時在使用移轉後資料庫的 Azure 中已有可運作的可用性群組。 他們藉由在 SQL Management Studio 中連線至內部負載平衡器來確認這一點。
...
3. 容錯移轉完成後，Azure VM 複本會出現在 Azure 入口網站中。 Contoso 會檢查 VM 為適當的大小、其已連線到正確的網路，而且正在執行中。 
4. 確認之後，Contoso 可以清除容錯移轉，並記錄與儲存任何觀察到的結果。 

### <a name="run-a-failover"></a>執行容錯移轉

1. 驗證測試容錯移轉如預期般正常運作之後，Contoso 會建立移轉的復原計畫，並將 WEBVM 新增至計畫。
...
在移轉程序的最後一個步驟中，Contoso 會將應用程式的連接字串更新為指向在其 SHAOG 接聽程式上執行的移轉後資料庫。 此組態會在目前執行於 Azure 中的 WEBVM 上變更。  此組態位於ASP 應用程式的 web.config 中。 
...
移轉之後，SmartHotel 應用程式會在 Azure VM 上執行，而 SmartHotel 資料庫會位於 Azure SQL 叢集中。
...
- 他們也考慮使用 Azure 磁碟加密和 KeyVault 來保護磁碟上的資料。
- 他們應評估透明資料加密 (TDE)，然後在執行於新的 SQL AOG 的 SmartHotel 資料庫上加以啟用。 [深入了解](https://docs.microsoft.com/sql/relational-databases/security/encryption/transparent-data-encryption?view=sql-server-2017)。
...
Contoso 會使用 Azure 備份服務來備份 WEBVM、SQLAOG1 和 SQLAOG2 上的資料。 [深入了解](https://docs.microsoft.com/azure/backup/backup-introduction-to-azure-backup?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)。
...
在本文中，Contoso 使用 Site Recovery 服務將應用程式前端 VM 移轉至 Azure，以重新裝載 SmartHotel 應用程式。 他們將應用程式資料庫移轉至佈建在 Azure 中的 SQL Server 叢集，並在 SQL Server AlwaysOn 可用性群組中加以保護。
