...
3. 在 [尋找資料夾] 中，指定已解壓縮的 VHD 所在的資料夾。 然後按一下 [確定](英文)。
1. 在 [選取虛擬機器] 中，按 [下一步]。
2. 在 [選擇匯入類型] 中，按一下 [複製虛擬機器 (建立新的唯一識別碼)]。 然後按一下 [確定](英文)。
3. 在 [選擇目的地] 中，保留預設設定。 按一下 [下一步]。
4. 在 [儲存資料夾] 中，保留預設設定。 按一下 [下一步]。
