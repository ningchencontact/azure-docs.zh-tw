...
為了熟悉並更加了解其中所涉及的技術，他們將會評估和遷移小型的內部部署旅遊應用程式。 這個應用程式分為兩層，其中一個 VM 上有 Web 應用程式在執行，第二個 VM 上則有 SQL Server 資料庫。 應用程式部署在 VMware 中，環境則是由 vCenter Server 來管理。 他們會使用 Data Migration Assistant (DMA) 和 Azure Migrate 服務來進行評估。
...
讓我們繼續進行下一個案例，以將內部部署 VM 隨即移轉至 Azure。
