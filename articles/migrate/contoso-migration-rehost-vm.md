...
這份文件是系列文章中的其中一篇，說明 Contoso 這家虛構的公司如何將內部部署資源移轉至 Microsoft Azure 雲端。 整個系列文章中包含背景資訊和說明如何設定移轉基礎結構的案例、評估內部部署資源的移轉，以及執行不同類型的移轉。 案例的複雜性會提高。 我們會隨著時間經過新增文章。
...
文章 5：在 Azure VM 上重新裝載應用程式 | Contoso 會使用 Site Recovery 服務，將其 SmartHotel 應用程式 VM 遷移至 Azure VM。 | 本文
...
- **限制風險**：SmartHotel 應用程式對 Contoso 的業務影響甚大。 Contoso 想要在亳無風險的情況下，將應用程式移至 Azure。
...
容錯移轉之後，Contoso 會想要連線到 Azure VM。 若要這樣做，Contoso 管理員必須在移轉前執行下列作業：
...
7. 註冊完成後，在 Azure 入口網站中重複檢查組態伺服器和 VMware 伺服器都已列在保存庫中的 [來源] 頁面上。 探索可能需要 15 分鐘以上的時間。 
