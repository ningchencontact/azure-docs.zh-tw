...
1.  為每個租用戶建立一個使用者，並使用 [RBAC](https://docs.microsoft.com/azure/role-based-access-control/role-assignments-portal)，將唯讀權限指派給屬於特定租用戶的所有虛擬機器。 然後，使用這些認證進行探索。 RBAC 會確保對應的 vCenter 使用者只能存取特定租用戶的虛擬機器。
2. 您要為不同的租用戶使用者設定 RBAC，如以下 User#1 和 User#2 的範例所述：

    - 在 [使用者名稱] 和 [密碼] 中，指定收集器要用來探索虛擬機器的唯讀帳戶認證
    - Datacenter1 - 提供 User#1 和 User#2 唯讀權限。 請勿將這些使用權限傳播給所有子物件，因為您是要在個別虛擬機器上設定權限。

      - VM1 (Tenant#1) (User#1 的唯讀權限)
      - VM2 (Tenant#1) (User#1 的唯讀權限)
      - VM3 (Tenant#2) (User#2 的唯讀權限)
      - VM4 (Tenant#2) (User#2 的唯讀權限)

   - 如果您使用 User#1 認證進行探索，則只會探索 VM1 和 VM2。
