...
**從 Hyper-V 移轉至 Azure**：Azure Migrate 目前僅支援評估 VMware 虛擬機器至 Azure 的移轉。 Hyper-V 支援已列在 Azure Migrate 藍圖規劃中。 在此過渡期間，您可使用 ASR 部署規劃工具。 一旦 Azure Migrate 啟用 Hyper-V 支援，您即可使用 Azure Migrate 規劃移轉 Hyper-V 的工作負載。

**從 VMware/Hyper-V 移轉至 Azure 的災難復原**：如果您預計使用 Azure Site Recovery (ASR) 在 Azure 上執行災難復原，請使用 ASR 部署規劃工具進行 DR 規劃。 ASR 部署規劃工具可針對您的內部部署環境進行深層的 ASR 特定評估， 此外，還可針對如何成功執行複寫、虛擬機器容錯移轉等 DR 作業提供必要的 ASR 建議。  
...
針對相依性視覺化，如果您在虛擬機器上安裝代理程式，則相依性代理程式收集的資料會儲存在美國由使用者訂用帳戶建立的 OMS 工作區中。 一旦使用者刪除訂用帳戶中的 OMS 工作區，該資料也會一併刪除。 [深入了解](https://docs.microsoft.com/azure/migrate/concepts-dependency-visualization)。
