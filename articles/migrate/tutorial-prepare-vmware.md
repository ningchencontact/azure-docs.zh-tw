...
- **設定評量帳戶**。 Azure Migrate 需要存取 vCenter Server，才能探索用於評量的 VM。 您需要唯讀帳戶來存取 Azure Migrate。
...
Azure Migrate 需要存取 vCenter Server，才能探索用於評估和無代理程式移轉的 VM。 若只是要進行評量，請設定一個可使用 vCenter Server 的唯讀帳戶。
