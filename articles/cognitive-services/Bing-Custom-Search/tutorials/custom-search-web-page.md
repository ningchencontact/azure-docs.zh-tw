...
5.  按一下 [確定]，指定 URL，以及是否要包括基底頁面的子頁面：
...
1.  在 [定義編輯器] 中，按一下 [使用中] 索引標籤，輸入要在搜尋中包括的一或多個網站的 URL。

    ![定義編輯器 [使用中] 索引標籤的螢幕擷取畫面](../media/customSrchEditor.png)

2.  若要確認您的執行個體會傳回結果，請在右邊的預覽窗格中輸入查詢。 Bing 只會針對已編製索引的公用網站來傳回結果。

## <a name="add-blocked-entries"></a>加入已封鎖的項目
若要排除來自特定網站或 URL 的結果，請將它們加入到 [已封鎖] 索引標籤。

1. 在 [定義編輯器] 中，按一下 [已封鎖] 索引標籤，輸入要從搜尋中排除的一或多個網站的 URL。
...
若要將特定網頁釘選到搜尋結果的頂端，請將網頁和查詢字詞加入到 [已釘選] 索引標籤。[已釘選] 索引標籤包含一份網頁與查詢字詞配對的清單，指定對於特定查詢要顯示為第一個結果的網頁。 使用者的查詢字詞必須完全符合已釘選的查詢字詞，網頁才能釘選到頂端。

1. 在 [定義編輯器] 中，按一下 [已釘選] 索引標籤，輸入網頁以及應可讓網頁傳回為第一個結果的查詢字詞。

    ![定義編輯器 [已釘選] 索引標籤的螢幕擷取畫面](../media/pinnedCustomSrch.png)

2. 若要確認您的執行個體會將指定的網頁傳回為第一個結果，請在右邊的預覽窗格中輸入您釘選的查詢字詞。
...
3. 選取色彩佈景主題。
    ...
    ![[託管的 UI] 選取配置步驟的螢幕擷取畫面](./media/custom-search-hosted-ui-select-color-theme.png)

4. 指定其他設定選項。

    ![[託管的 UI] 其他設定步驟的螢幕擷取畫面](./media/custom-search-hosted-ui-additional-configurations.png)

5. 貼上您的**訂用帳戶金鑰**。 請參閱[試用認知服務](https://azure.microsoft.com/try/cognitive-services/?api=bing-custom-search-api)。
...
4. 從 [託管的 UI] 索引標籤，向下捲動到名為 [取用 UI] 的區段。 複製 JavaScript 程式碼片段。

    ![[託管的 UI] 儲存按鈕的螢幕擷取畫面](./media/custom-search-hosted-ui-consuming-ui.png)
