...
2. 建立以 `ApiKeyServiceClientCredentials` 做為參數的新 `TextAnalyticsClient` 物件，然後產生 `MultiLanguageInput` 物件的清單，內有您想要分析的文件。
...
2. 建立以 `ApiKeyServiceClientCredentials` 做為參數的新 `TextAnalyticsClient` 物件，然後產生 `LanguageInput` 物件的清單，內有您的文件。
...
2. 建立以 `ApiKeyServiceClientCredentials` 做為參數的新 `TextAnalyticsClient` 物件，然後產生 `MultiLanguageBatchInput` 物件的清單，內有您的文件。
