...
將下列類別新增到 **Program** 類別。 您可以使用此類別來讓項目的內部內容識別碼與檢閱識別碼產生關聯。
...
> [!Note]
> 在實務上，您可以對將接收手動檢閱結果的 URL 設定回呼 URL `CallbackEndpoint` (透過 HTTP POST 要求)。 您可以修改此方法以檢查擱置審核項目的狀態。
