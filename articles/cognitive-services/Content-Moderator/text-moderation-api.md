...
ms.locfileid: "35368738"
---
# <a name="text-moderation"></a>文字仲裁

您可以使用 Content Moderator 的電腦輔助文字仲裁和[人在迴路](Review-Tool-User-Guide/human-in-the-loop.md) \(human-in-the-loop\) 功能來仲裁文字內容。

企業可以使用文字仲裁服務，根據其原則和閾值來封鎖、核准或審核內容。 文字仲裁服務可用來針對需要合作夥伴、員工及客戶產生文字內容的環境，加強人工仲裁。 這些環境包括聊天室、討論區、聊天機器人、電子商務目錄、文件等。 

此 API 會掃描傳入的文字 (上限為 1024 個字元) 是否有粗話、將可能不想要的文字分類 (預覽版)、自動校正文字，以及偵測潛在的「個人識別資訊」(PII)。 它也會依據自訂字詞清單進行比對。 自動校正功能可協助捕捉刻意拼錯的單字。 處理內容之後，服務會傳回一個詳細的回應。 您可以使用此回應在檢閱工具中建立人工審核項目，或是將其移除等。
...
Content Moderator 的電腦輔助**文字分類功能**支援的語言**僅限英文**，可協助偵測可能的不想要內容。 所標幟的內容可能是依據上下文而被視為不當的內容。 除了傳達每個類別的可能性之外，它也可能建議對內容進行人工審核。 此功能使用定型模型來識別可能的濫用、毀謗性或歧視性語言。 這包括要供審核的俚語、縮寫單字、冒犯性及刻意拼錯的單字。 
...
雖然預設的全域字詞清單適用於大多數案例，但您可能會想要依據業務需求特定的字詞來進行過濾。 例如，您可能會想要從使用者的文章中篩選掉任何競爭的品牌名稱。 您的所允許文字內容閾值可能與預設清單不同。
