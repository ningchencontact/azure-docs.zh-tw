---
title: 快速入門：Bing 實體搜尋 SDK，C#
titleSuffix: Azure Cognitive Services
description: 設定實體搜尋 SDK 主控台應用程式與 C#。
...
ms.locfileid: "52307353"
---
# <a name="quickstart-bing-entity-search-sdk-with-c"></a>快速入門：搭配使用 Bing 實體搜尋 SDK 與 C#

Bing 實體搜尋 API 包含用於實體搜尋和剖析結果的 REST API 功能。

[C# Bing 實體搜尋 SDK 範例的原始程式碼](https://github.com/Azure-Samples/cognitive-services-dotnet-sdk-samples/tree/master/BingSearchv7/BingEntitySearch)可從 Git Hub 取得。
...
若要使用 Bing 實體搜尋 SDK 來設定主控台應用程式，請在 Visual Studio 中瀏覽至 [方案總管] 中的 `Manage NuGet Packages` 選項。  新增 `Microsoft.Azure.CognitiveServices.Search.EntitySearch` 套件。

安裝 [NuGet 實體搜尋套件](https://www.nuget.org/packages/Microsoft.Azure.CognitiveServices.Search.EntitySearch/1.2.0)也會一併安裝相依性，包括下列組件：
* Microsoft.Rest.ClientRuntime
* Microsoft.Rest.ClientRuntime.Azure
* Newtonsoft.Json

## <a name="entity-search-client"></a>實體搜尋用戶端
若要建立 `EntitySearchAPI` 用戶端的執行個體，請新增 using 指示詞：
```
...
下列主控台應用程式會查閱 "Satya Nadella" 查詢上的單一實體，並印出。
```
using System;
using System.Linq;
using System.Text;
...
## <a name="ambiguous-results"></a>模稜兩可的結果
下列程式碼會處理模稜兩可查詢 "William Gates" 的結果去除混淆。
```
...
下列程式碼會查閱單一存放區 "Microsoft Store"，並印出其電話號碼。
```
...
```
## <a name="entityscenario-list"></a>EntityScenario 清單
下列程式碼會查閱 "Seattle restaurants" 清單，並列印其名稱和電話號碼。
```
...
```
## <a name="error-results"></a>錯誤結果
下列程式碼會觸發錯誤要求，並顯示如何讀取錯誤回應。
```
