...
API 回應包含 [SearchResponse](https://docs.microsoft.com/rest/api/cognitiveservices/bing-entities-api-v7-reference#searchresponse) 物件。 若 Bing 找到相關的實體或地點，物件會包含 `entities` 欄位、`places` 欄位或兩者。 否則，回應物件不包含任一欄位。
> [!NOTE]
