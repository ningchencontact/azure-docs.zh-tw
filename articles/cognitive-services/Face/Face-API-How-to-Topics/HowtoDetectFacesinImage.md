---
title: 使用臉部 API 偵測影像中的人臉 | Microsoft Docs
titleSuffix: Microsoft Cognitive Services
description: 使用認知服務中的臉部 API 來偵測影像中的人臉。
...
---
# <a name="how-to-detect-faces-in-image"></a>如何偵測影像中的人臉

本指南會示範如何偵測影像中的人臉及臉部屬性，例如性別、年齡或所擷取的姿勢。 這些範例是在 C# 中使用臉部 API 用戶端程式庫所編寫的。 
...
## <a name="step1"></a> 步驟 1：授權 API 呼叫

每次呼叫臉部 API 時，一定會用到訂用帳戶金鑰。 這個金鑰必須透過查詢字串參數傳遞，或者在要求標頭中指定。 若要透過查詢字串傳遞訂用帳戶密鑰，請參閱所舉出的[臉部 - 偵測](https://westus.dev.cognitive.microsoft.com/docs/services/563879b61984550e40cbbe8d/operations/563879b61984550f30395236)要求 URL 範例：
...
或者，您也可以在 HTTP 要求標題中指定訂用帳戶金鑰：**ocp-apim-subscription-key: &lt;訂用帳戶金鑰&gt;**。若您使用用戶端程式庫，則會透過 FaceServiceClient 類別的建構函式傳入訂用帳戶金鑰。 例如︰
```CSharp
faceServiceClient = new FaceServiceClient("<Subscription Key>");
```

## <a name="step2"></a> 步驟 2：將影像上傳至服務並執行臉部偵測
...
## <a name="step3"></a> 步驟 3：了解和使用臉部特徵點
