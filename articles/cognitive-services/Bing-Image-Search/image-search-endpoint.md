...
description: 影像搜尋 API 包含三個端點。 端點1會從 web 傳回影像。 端點2會傳回 ImageInsights。 端點 3 會傳回發燒影像。
