---
title: 專案答案搜尋概觀 - Microsoft 認知服務 | Microsoft Docs
...
如果回應或結果包含 `contractualRules`、`attributions` 或 `provider` 欄位，您必須屬性化資料。 如果回應不包含上述任何欄位，則不需要任何屬性。 如果回應包含 `contractualRules` 欄位以及 `attributions` 和/或 `provider` 欄位，您必須使用契約規則來屬性化資料。  
...
如果契約規則包含 `targetPropertyName` 欄位，該規則只會套用至目標欄位。 否則，該規則會套用至包含 `contractualRules` 欄位的父物件。  
...
如果實體包含影像，而且您會顯示該影像，您必須提供提供者網站的點選連結。 如果實體包含 [MediaAttribution](reference.md#mediaattribution) 規則，請使用規則的 URL 建立點選連結。 否則，請使用包含在影像 `provider` 欄位中的 URL 建立點選連結。  
