...
本教學課程應用程式說明如何：
> [!div class="checklist"]
> * 在 JavaScript 中執行 Bing 新聞搜尋 API 呼叫
> * 將搜尋選項傳遞給 Bing 新聞搜尋 API
> * 顯示來自四種類別的新聞搜尋結果：任何類型、財經、健康或政治，時間範圍為 24 小時、過去一週、一個月或所有可用時間
> * 逐頁檢視搜尋結果
...
在本教學課程中，我們會討論原始程式碼的選定部分。 有完整的[原始程式碼](tutorial-bing-news-search-single-page-app-source.md)可供使用。 若要執行範例，請將原始程式碼複製並貼入文字編輯器，並將其儲存為 `bing.html`。
...
下圖顯示查詢文字方塊，以及定義搜尋學校資金相關新聞的選項。

![Bing 新聞搜尋選項](media/news-search-categories.png)
...
假設有查詢、選項字串和 API 金鑰，`BingNewsSearch` 函式會使用 `XMLHttpRequest` 物件向 Bing 新聞搜尋端點提出要求。
...
```
HTTP 要求成功完成時，JavaScript 會呼叫 `load` 事件處理常式 `handleBingResponse()` 函式，來處理成功傳遞至 API 的 HTTP GET 要求。 
...
```
主要的搜尋結果會傳回成 JSON 回應中的最上層 `value` 物件。 我們將這些結果傳遞給函式 `renderResults()`，它會逐一查看這些結果並呼叫個別函式將每個項目轉譯成 HTML。 產生的 HTML 將傳回給 `renderSearchResults()`，在其中它會插入頁面中的 `results` 區域。
...
在 JavaScript 程式碼中有一個物件 `searchItemRenderers` 包含 *renderers:* 函式，可為每種搜尋結果產生 HTML。
...
新聞轉譯器函式會：
> [!div class="checklist"]
> * 建立段落標籤，將其指派給 `news` 類別，並將其推送至 HTML 陣列。
...
> [Bing 新聞搜尋 API 參考](//docs.microsoft.com/rest/api/cognitiveservices/bing-news-api-v7-reference)