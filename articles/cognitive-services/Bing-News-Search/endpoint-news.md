...
**端點 1：** 根據使用者的搜尋查詢傳回新聞項目。 如果搜尋查詢是空的，則呼叫會傳回熱門新聞文章。 查詢 `?q=""` 選項也可搭配 `/news` URL 使用。 如需了解可用性，請參閱[支援的國家/地區與市場](supported-countries-markets.md#supported-markets-for-news-search-endpoint)。
```
GET https://api.cognitive.microsoft.com/bing/v7.0/news/search 
``` 

**端點 2：** 依照類別傳回熱門新聞項目。 您可以使用 `category=business``category=sports` 或 `category=entertainment`，特別要求熱門商業、運動或娛樂文章。  `category` 參數只能搭配 `/news` URL 使用。 指定類別時有一些正式需求；請參閱[查詢參數](https://docs.microsoft.com/rest/api/cognitiveservices/bing-news-api-v7-reference#query-parameters)文件中的 `category`。 如需了解可用性，請參閱[支援的國家/地區與市場](supported-countries-markets.md#supported-markets-for-news-endpoint)。 
```
GET https://api.cognitive.microsoft.com/bing/v7.0/news  
```

**端點 3：** 傳回社交網路上目前發燒的新聞主題。 若包含 `/trendingtopics` 選項，Bing 搜尋會忽略其他幾個參數，例如 `freshness` 和 `?q=""`。 如需了解可用性，請參閱[支援的國家/地區與市場](supported-countries-markets.md#supported-markets-for-news-trending-endpoint)。
```
