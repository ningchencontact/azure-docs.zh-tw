...
---
# <a name="how-to-use-the-anomaly-detector-api-on-your-time-series-data"></a>作法：在您的時間序列資料上使用異常偵測器 API  
...
根據預設，異常偵測的上限和下限會使用 `expectedValue`、`upperMargin` 和 `lowerMargin` 來計算。 如果您需要不同的界限，建議您將 `marginScale` 套用至 `upperMargin` 或 `lowerMargin`。 界限的計算方式如下：
