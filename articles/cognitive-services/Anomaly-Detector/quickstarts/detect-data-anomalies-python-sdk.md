...
description: 本快速入門會說明如何使用 Anomaly Detector API 來偵測資料序列中的異常狀況 (以批次或串流資料的方式)。
