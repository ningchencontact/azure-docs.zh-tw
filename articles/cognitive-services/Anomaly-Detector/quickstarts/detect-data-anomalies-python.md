...
3. 傳送要求使用`requests.post()`。 結合您的端點，並針對完整的異常偵測 URL 要求 URL，並包含您的標頭和 json 要求資料。 
...
3. 在資料集中尋找異常狀況的位置。 回應的`isAnomaly`欄位包含與指定的資料點是否為異常狀況相關的布林值。 逐一查看清單中，並列印的任何索引`True`值。 如果找不到任何，這些值會對應到的異常資料點索引。
