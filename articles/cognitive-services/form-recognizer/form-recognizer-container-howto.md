...
| 容器 | 最小值 | 建議 |
|-----------|---------|-------------|
| 表單辨識器 | 2核心，4 GB 記憶體 | 4核心，8 GB 記憶體 |
...
> [!IMPORTANT]
> 您必須指定 `Eula`、`Billing`和 `ApiKey`，以及 `FormRecognizer:ComputerVisionApiKey` 和 `FormRecognizer:ComputerVisionEndpointUri` 選項，才能執行容器;否則，容器將無法啟動。 如需詳細資訊，請參閱[帳單](#billing)。
