...
QnAMaker 執行階段是您在 Azure 入口網站中[建立 QnAMaker 服務](./set-up-qnamaker-service-azure.md)時所部署的 Azure App Service 組件之一。 執行階段會定期進行更新。 QnA Maker App Service 是在2019年4月網站延伸模組版本 (第5版 +) 的自動更新模式。 這已經過設計, 可在升級期間處理零停機時間。 您可以在上 https://www.qnamaker.ai/UserSettings 檢查目前的版本。 如果您的版本早于版本 5.x, 您必須重新開機 App Service, 以套用最新的更新以套用至 QnAMaker 安裝程式。
