...
**答**：沒有。 如果您的應用程式使用一般日常用語，則您不需自訂模型。 如果您的應用程式要用於背景噪音很少或完全沒有背景噪音的環境，則您也不需自訂模型。

您可以在入口網站部署基準和自訂模型，並對它們執行精確度測試。 您可以使用這項功能，來測量基準與自訂模型的精確度。

**问：如何知道我的資料集或模型處理何時完成？**
...
**问：我發現自己犯了錯誤，如何取消進行中的資料匯入或模型建立？**

**答**：目前，您無法復原原音或語言調節程序。 您可以在匯入的資料與模型處於終止狀態之後予以刪除。

**问：搜尋和聽寫模型與對話模型之間有何差異？**
...
**问：我可以壓縮文字檔來上傳更大的文字檔嗎？** 

**答**：沒有。 目前僅允許未壓縮的文字檔。

**问：資料報告指出有失敗的語句。有什麼問題？**
...
**问：我可以使用自訂語言模型，對自訂原音模型執行離線測試嗎？**

**答**：可以，只要在設定離線測試時，在下拉式功能表中選取自訂語音模型即可。

**问：我可以使用自訂原音模型對自訂語言模型執行離線測試嗎？**
