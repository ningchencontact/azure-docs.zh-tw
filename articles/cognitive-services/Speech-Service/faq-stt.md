...
若要增加自訂模型的平行存取，我們需要下列資訊：

- 部署模型的區域。
- 已部署模型的端點識別碼。

若要增加基底模型的平行存取，我們需要下列資訊：

- 服務的區域，

和

- 您 subrscription 的存取權杖（請參閱[這裡](https://docs.microsoft.com/azure/cognitive-services/speech-service/rest-speech-to-text#how-to-get-an-access-token)），
