...
**答**：基準模型已使用 Microsoft 所擁有的資料加以定型，並已部署在雲端中。 自訂模型可讓使用者將模型調整成更適合具有特定環境噪音或語言的特定環境。 製造商工廠、汽車和吵雜的街道會需要經過調整的原音模型，而生物學、物理學、無線電、產品名稱和自訂縮略字等特定主題則需要語言模型。
...
**答**：否，如果您的應用程式使用一般日常語，而沒有自訂詞彙或可能很罕見，則您不需要自訂模型。 此外，如果您的應用程式要用於背景噪音很少或完全沒有的環境，您也不需要自訂。 入口網站可讓使用者部署基準和自訂模型，並對它們執行精確度測試。 使用者可以使用這項功能，來測量基準與自訂模型的精確度。
...
例如，如果語言模型建立程序階段中目前有語言模型，則無法啟動程序。
不過，您可以同時處理原音模型和語言模型。 
...
**問**：如果我需要比預設值或入口網站所提供更高的並行處理，該怎麼辦？ 
...
**答**：這取決於用於您應用程式中的詞彙和片語與起始語言模型有何差異。 針對所有新的字組，盡可能提供許多使用字組的範例會很有用。 針對用於您應用程式中的常見片語，在語言資料中包含片語也很有用，因為它也會告知系統留意這些字詞。 在語言資料集中至少有 100 個語句是很常見的，而且通常會有數百個或更多語句。 此外，如果特定查詢類型必須比其他類型更常見，您可以將常見查詢的多個複本插入資料集。

**問**：我可以只上傳字詞清單嗎？

**答**：上傳字詞清單會將字詞納入詞彙中，但不會告知系統這些字詞的通常用法。
