...
「自訂語音」提供工具, 可讓您以視覺化方式檢查模型的辨識品質, 方法是比較音訊資料與對應的辨識結果。 從自訂語音入口網站, 您可以播放上傳的音訊, 並判斷提供的辨識結果是否正確。 這項工具可讓您快速檢查 Microsoft 的基準語音轉換文字模型或定型自訂模型的品質, 而不需要轉譯任何音訊資料。
...
2. 按一下 [**加入測試**]。
3. 選取 **[檢查品質 (僅限音訊資料)** ]。 為測試指定名稱、描述, 然後選取您的音訊資料集。
4. 選取最多兩個您想要測試的模型。
5. 按一下 [建立]。
