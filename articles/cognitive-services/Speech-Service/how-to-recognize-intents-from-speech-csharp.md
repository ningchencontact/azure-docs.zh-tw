...
在本指南中，您會使用語音 SDK 來開發C#主控台應用程式，透過裝置的麥克風從使用者語句衍生意圖。 您將學習如何：
...
應用程式會使用一次模式，因此會呼叫 `RecognizeOnceAsync()` 以開始辨識。 結果是 `IntentRecognitionResult` 物件，其中包含有關所辨識意圖的資訊。 您使用下列運算式來擷取 LUIS JSON 回應：
