...
1. 如同上傳資料集，請輸入名稱和描述，以便識別此模型。
    ...
    請謹慎選擇名稱。 您在這裡輸入的名稱，會是您在語音合成要求中指定語音作為 SSML 輸入一部分所使用的名稱。 只允許字母、數字和一些標點符號字元，例如 `-`、`_` 和 `(', ')`。
...
> [!NOTE]
> 法文、德文和義大利文語音訓練會從有 2,000 個以上語句的資料集開始。 中文-英文雙語模型也支援使用有 2,000 個以上語句的資料集。
