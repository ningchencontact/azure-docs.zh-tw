...
應用程式以 Microsoft 認知服務語音 SDK Maven 套件 1.0.0 版和 Android Studio 3.1 為基礎。
...
1. 在出現的視窗中，輸入適用於 Android 之語音 SDK 的名稱和版本 `com.microsoft.cognitiveservices.speech:client-sdk:1.0.0`。 然後選取 [確定]。
...
1. 開啟來源檔案 `MainActivity.java`。 將 `package` 陳述式後面的所有程式碼取代為下列內容。
