...
1. 選取 [語音識別 API] 索引標籤。
   ...
   ![語音 API-語音服務索引標籤](media/index/cognitive-services-speech-api-tab.png)

1. 在 [**語音服務**] 底下，選取 [**取得 API 金鑰**]。

   ![語音 API-取得 API 金鑰](media/index/speech-api-get-api-key.png)

1. 同意條款，並從下拉式功能表中選取您的地區設定。

   ![語音 API-同意條款](media/index/speech-api-agree-to-terms.png)
...
1. 選取入口網站左上方的 [建立資源]。
    ...
    ![語音 API-建立資源](media/index/speech-api-create-resource.png)

1. 在 [新增] 視窗中，搜尋 **speech**。

1. 在搜尋結果中，選取 [Speech]。

    ![語音 API-選取語音](media/index/speech-api-select-speech.png)

1. 在 [Speech] 底下選取 [建立] 按鈕。

    ![語音 API-[建立] 按鈕](media/index/speech-api-create-button.png)
...
1. 在 [建立] 底下輸入：
   ...
   * 選取 [建立]。
     ...
     ![語音 API-選取 [建立]](media/index/speech-api-select-create.png)

     建立和部署新的語音資源可能需要一些時間。 選取 [快速入門] 可查看新資源的相關資訊。

     ![語音 API-部署資源](media/index/speech-api-deploy-resource.png)
