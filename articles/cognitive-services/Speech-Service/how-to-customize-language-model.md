...
在 [建立語言模型] 頁面上輸入「名稱」和「描述」，可協助您持續追蹤此模型的相關資訊，例如使用的資料集。 接下來，從下拉式功能表中選取 [底語言模型]。 此模型會是自訂的起點。 有兩種基底語言模型可選擇。 搜尋與聽寫模型適用於應用程式上的語音導向作業，例如命令、搜尋查詢或聽寫。 對話模型適用於辨識以對話形式說出的語音。 這類型的語音通常會指向另一個人，並且發生在話務中心或會議中。 稱為「通用」的新模型也已公開推出。 「通用」的目標是處理所有案例，並在最終取代搜尋、聽寫和交談模型。
