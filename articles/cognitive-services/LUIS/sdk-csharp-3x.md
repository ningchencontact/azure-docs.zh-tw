---
title: 快速入門：適用於 .NET 的 Language Understanding (LUIS) 撰寫用戶端程式庫 3.x
titleSuffix: Azure Cognitive Services
description: 開始使用適用於 .NET 的 LUIS 用戶端程式庫 3.x。 請遵循下列步驟來安裝套件，並試用基本工作的程式碼範例。
...
ms.locfileid: "74808044"
---
# <a name="quickstart-language-understanding-luis-client-library-3x-for-net"></a>快速入門：適用於 .NET 的 Language Understanding (LUIS) 用戶端程式庫 3.x

開始使用適用於 .NET 的 Language Understanding (LUIS) 用戶端程式庫 3.x。 請遵循下列步驟來安裝套件，並試用基本工作的程式碼範例。  Language Understanding (LUIS) 可讓您將自訂機器學習智慧套用至使用者的對話、自然語言文字中，以預測整體意義，並找出相關的詳細資訊。

使用適用於 .NET 的 Language Understanding (LUIS) 用戶端程式庫 3.x 可以：
