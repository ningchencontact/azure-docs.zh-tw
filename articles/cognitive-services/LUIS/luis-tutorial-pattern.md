...
在本教學課程中，您將可以在提供較少語句範例的情況下，使用模式來提升意圖和實體的識別預測。 模式透過範本語句範例的方式來提供，其中包含用來識別實體及可忽略文字的語法。 模式是運算式比對與機器學習的組合。  範本語句範例搭配意圖語句，可讓 LUIS 更容易理解哪些語句符合意圖。 
...
這些語句太相似，因此若不提供更多的語句範例，便無法判斷各個語句中內容相關的唯一性。 藉由為意圖新增模式，便無須提供許多語句範例，LUIS 便可學習意圖的常見語句模式。 
...
當 LUIS 將預測結果傳回用戶端應用程式之後，意圖名稱就可用來作為用戶端應用程式中的函式名稱，而員工實體可用來作為該函式的參數。
