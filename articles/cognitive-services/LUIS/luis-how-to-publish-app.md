...
您一律可以先[測試](luis-interactive-test.md)後再發佈應用程式。 
...
在 [Endpoint url settings] \(端點 URL 設定\) 中，[Enable Bing spell checker] \(啟用 Bing 拼字檢查工具\) 核取方塊允許 LUIS 在預測之前更正錯字。 建立 **[Bing 拼字檢查金鑰](https://azure.microsoft.com/try/cognitive-services/?api=spellcheck-api)**。 建立金鑰之後，會將兩個 querystring 參數新增至發佈頁面上的端點 URL。 
