...
1. 使用下列設定來設定訂用帳戶：
   ...
   |Subscription|您的訂用帳戶|選取其中一個與您的帳戶相關聯的訂用帳戶。|
   |Location|美國西部 |此資源的 Azure 區域。|
   |定價層|**S0**|此資源的預設定價層。|
   |Resource group|`my-cognitive-service-resource-group`|為所有認知服務資源，建立新的資源群組。 當您完成使用資源後，可以刪除資源群組來清除您的訂用帳戶。 |
   | | | |
