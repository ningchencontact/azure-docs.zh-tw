...
description: 透過本快速入門，開始使用適用於 .NET 的 LUIS 用戶端程式庫。 請遵循下列步驟來安裝套件，並試用基本工作的程式碼範例。
...
ms.locfileid: "75447754"
---
# <a name="quickstart-language-understanding-luis-authoring-2x-client-library-for-net"></a>快速入門：適用於 .NET 的 Language Understanding (LUIS) 撰寫 2.x 用戶端程式庫

開始使用適用於 .NET 的 Language Understanding (LUIS) 撰寫 2.x 用戶端程式庫。 請遵循下列步驟來安裝套件，並試用基本工作的程式碼範例。  Language Understanding (LUIS) 可讓您將自訂機器學習智慧套用至使用者的對話、自然語言文字中，以預測整體意義，並找出相關的詳細資訊。
