...
Azure [訂用帳戶金鑰](luis-how-to-azure-subscription.md) 會在 Azure 入口網站中產生。 您要在**[發佈](manage-keys.md)** 頁面上指派金鑰給 LUIS 應用程式。 您不需要知道實際的金鑰值。 LUIS 會使用訂用帳戶名稱來進行指派。 
...
LUIS 建議使用現有[端點語句](label-suggested-utterances.md)中的語句來加強模型。 在舊版中，這名為 **sample**。 在新版中，名稱已從 sample 變更為 **suggest**。 這在 LUIS 網站中稱為**[檢閱端點語句](https://docs.microsoft.com/azure/cognitive-services/LUIS/label-suggested-utterances)** \(英文\)。
...
[預先建置的實體](Pre-builtEntities.md)已經變更。 確定您使用 V2 預先建置的實體。 這包括使用 [datetimeV2](pre-builtentities.md?#use-a-prebuilt-datetimev2-entity)，而不是 datetime。 
