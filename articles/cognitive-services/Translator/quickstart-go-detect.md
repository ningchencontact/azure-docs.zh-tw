---
title: 快速入門：從文字識別語言 (Go) - 翻譯工具文字 API
...
ms.locfileid: "67705622"
---
# <a name="quickstart-use-the-translator-text-api-to-detect-text-language-using-go"></a>快速入門：搭配使用翻譯工具文字 API 與 Go 來偵測文字語言

在本快速入門中，您將了解如何搭配使用 Go 與翻譯工具文字 REST API 來偵測所提供文字的語言。
...
使用您慣用的 IDE 或編輯器建立新的 Go 專案。 然後，將下列程式碼片段複製到您的專案中名為 `detect-language.go` 的檔案。
...
接下來，針對要求本文建立匿名結構，並使用 `json.Marshal()` 來將它編碼為 JSON。 將此程式碼新增到 `detect` 函式。
...
將此程式碼新增到 `detect` 函式以將 JSON 回應解碼，然後將結果格式化並加以列印。
