...
將以下幾行新增至 `Transliterate` 類別。 首先，這樣會從環境變數讀取訂用帳戶金鑰和端點。 然後，您會注意到，連同 `api-version`，已將兩個額外參數附加到 `url`。 這些參數用來設定輸入語言，以及用於直譯的指令碼。 在此範例中，它會設為日文 (`jpan`) 和英文 (`latn`)。 
