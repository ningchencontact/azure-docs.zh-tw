---
title: 快速入門：翻譯文字 (Go) - 翻譯工具文字 API
titleSuffix: Azure Cognitive Services
description: 在本快速入門中，您將會搭配使用翻譯工具文字 API 與 Go，在 10 分鐘內將文字翻譯成另一種語言。
...
ms.locfileid: "55218354"
---
# <a name="quickstart-use-the-translator-text-api-to-translate-a-string-using-go"></a>快速入門：搭配使用翻譯工具文字 API 與 Go 來翻譯字串

在本快速入門中，您將了解如何搭配使用 Go 與翻譯工具文字 REST API，將文字字串從英文翻譯成義大利文和德文。
...
使用您慣用的 IDE 或編輯器建立新的 Go 專案。 然後，將下列程式碼片段複製到您的專案中名為 `translate-text.go` 的檔案。
...
讓我們來建立可翻譯文字的函式。 此函式將會接受單一引數：您的翻譯工具文字訂用帳戶金鑰。
...
接下來，讓我們來建構 URL。 使用 `Parse()` 和 `Query()` 方法來建置 URL。 您將會注意到已使用 `Add()` 方法新增參數。 在此範例中，您會將英文翻譯成德文和義大利文：`de` 和 `it`。

將此程式碼複製到 `translate` 函式。
...
接下來，針對要求本文建立匿名結構，並使用 `json.Marshal()` 來將它編碼為 JSON。 將此程式碼新增到 `translate` 函式。
...
將此程式碼新增到 `translate` 函式以將 JSON 回應解碼，然後將結果格式化並加以列印。
