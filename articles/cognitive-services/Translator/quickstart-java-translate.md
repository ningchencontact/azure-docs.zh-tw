...
接著，在此資料夾中建立名為 `Translate.java` 的檔案。

## <a name="import-required-libraries"></a>匯入必要的程式庫

開啟 `Translate.java` 並新增下列 Import 陳述式：
...
將以下幾行新增至 `Translate` 類別。 您會注意到，連同 `api-version` ，已將兩個額外參數附加到 `url`。 這些參數用來設定翻譯輸出。 在此範例中，它會設為德文 (`de`) 和義大利文 (`it`)。 請務必更新訂用帳戶金鑰值。
...
將以下這一行新增至 `Translate` 類別，以具現化 `OkHttpClient`：
...
接下來，我們將建置 POST 要求。 您可以視需要變更要翻譯的文字。 文字必須逸出。
