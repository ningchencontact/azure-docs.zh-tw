...
視圖定義成品必須命名為**viewDefinition** ，並**放在建立**受控應用程式定義的 .zip 封裝中，與**createUiDefinition**相同的層級。 若要瞭解如何建立 .zip 套件併發布受控應用程式定義，請參閱[發佈 Azure 受控應用程式定義](publish-managed-app-definition-quickstart.md)
...
|頁首|否|[總覽] 頁面的標頭。|
|說明|否|受控應用程式的描述。|
|指令|否|[總覽] 頁面的其他工具列按鈕陣列，請參閱[命令](#commands)。|
...
|namespace|否|判斷正確的計量提供者時所要使用的其他資訊。|
...
|指令|否|CustomResources 視圖的其他工具列按鈕陣列，請參閱[命令](#commands)。|
|columns|否|自訂資源的資料行陣列。 如果未定義，預設會顯示 `name` 資料行。 此資料行必須具有 `"key"` 和 `"displayName"`。 針對 [金鑰]，提供要在視圖中顯示之屬性的索引鍵。 如果是 nested，請使用點做為分隔符號，例如，`"key": "name"` 或 `"key": "properties.property1"`。 在 [顯示名稱] 中，提供要在視圖中顯示之屬性的顯示名稱。 您也可以提供 `"optional"` 屬性。 設定為 true 時，預設會在視圖中隱藏資料行。|
