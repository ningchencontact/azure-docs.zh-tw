...
ms.locfileid: "34305288"
---
# <a name="create-and-deploy-an-azure-managed-application-with-azure-cli"></a>使用 Azure CLI 建立及部署 Azure 受控應用程式

本文將簡介如何使用受控應用程式。 您可以為您組織中的使用者，將受控應用程式定義新增至內部目錄。 然後，將受控應用程式部署至您的訂用帳戶。 為簡化指示，我們已為受控應用程式建置檔案。 有一個檔案會定義解決方案的基礎結構。 第二個檔案會定義使用者介面，以透過入口網站部署受控應用程式。 這些檔案皆可透過 GitHub 取得。 您可以從[建立服務目錄應用程式](publish-service-catalog-app.md)教學課程中，了解如何建置這些檔案。

當您完成時，您會有三個資源群組，其中包含受控應用程式的不同部分。

| 資源群組 | Contains | 說明 |
| -------------- | -------- | ----------- |
| appDefinitionGroup | 受控應用程式定義。 | 發行者會建立此資源群組和受控應用程式定義。 只要有受控應用程式定義存取權，任何人都可以對其進行部署。 |
| applicationGroup | 受控應用程式執行個體。 | 取用者會建立此資源群組和受控應用程式執行個體。 取用者可以透過此執行個體更新受控應用程式。 |
| infrastructureGroup | 受控應用程式資源。 | 建立受控應用程式時，系統會自動建立此資源群組。 發行者可以存取此資源群組，以管理應用程式。 取用者具有資源群組的有限存取權。 |
...
定義受控應用程式時，您會代表取用者選取管理資源的使用者、群組或應用程式。 此身分識別會根據指派的角色，取得受控資源群組的權限。 一般來說，您會建立 Azure Active Directory 群組來管理資源。 不過，在本文中您會使用自己的身分識別。
...
若要包含部署的受控應用程式，您需要資源群組。 使用 **westcentralus** 作為位置。

```azurecli-interactive
az group create --name applicationGroup --location westcentralus
```

## <a name="deploy-the-managed-application"></a>部署受控應用程式
...
* 如需檔案的範例，請參閱[受控應用程式範例](https://github.com/Azure/azure-managedapp-samples/tree/master/samples)。
