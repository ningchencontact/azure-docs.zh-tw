...
ms.locfileid: "51613707"
---
# <a name="supported-resources-for-metric-alerts-in-azure-monitor"></a>Azure 監視器中計量警示所支援的資源

Azure 監視器現已可支援[新的計量警示類型](monitoring-overview-alerts.md)，此類型具有遠優於[傳統計量警示](monitoring-overview-alerts-classic.md)的優點。 計量適用於 [Azure 服務的大型清單](monitoring-supported-metrics.md)。 新版警示則可支援該資源類型 (不斷增長) 的子集。 本文將列出該子集。


您也可以在擷取來作為計量的常用 Log Analytics 記錄上使用新版計量警示。 如需詳細資訊，請檢視[記錄的計量警示](monitoring-metric-alerts-logs.md)。

## <a name="portal-powershell-cli-rest-support"></a>入口網站、PowerShell、CLI、REST 支援
目前，您只能在 Azure 入口網站、[REST API](https://docs.microsoft.com/rest/api/monitor/metricalerts/) 或 [Resource Manager 範本](monitoring-create-metric-alerts-with-templates.md)中建立新版計量警示。 即將支援使用 PowerShell 和 Azure CLI 2.0 版與更新版本來設定新版警示。
...
* 深入了解新的[警示體驗](monitoring-overview-alerts.md)。
