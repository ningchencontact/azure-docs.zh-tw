...
您可以參閱[使用 Azure 監視器來建立、檢視及管理警示](alert-metric.md)，深入了解如何建立警示規則。

警示可跨數個 Azure 監視服務使用。 如需如何和何時使用每個服務的資訊，請參閱[監視 Azure 應用程式和資源](../azure-monitor/overview.md)。 下表提供可在整個 Azure 使用的警示規則類型清單。 它也列出整合警示體驗所支援的項目。
