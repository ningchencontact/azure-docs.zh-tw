---
title: 在新的 Azure 監視器警示體驗中使用活動記錄警示
description: 如何從 Azure 監視器下的 [警示 (預覽)] 索引標籤建立活動記錄警示。 本文詳細說明此功能的新使用者經驗。
...
3. 在 [定義警示條件] 下提供下列資訊，然後按一下 [完成]。
    ...
    **警示邏輯**：
     ...
     - **事件層級** - 事件的安全性層級。**詳細資訊、資訊、警告、錯誤**或**重大**。
     - **狀態**：事件的狀態。**已啟動、失敗**或**成功**。
