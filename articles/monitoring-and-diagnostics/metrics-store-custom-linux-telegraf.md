...
1. 在 VM 窗格中，瀏覽至 [身分識別] 索引標籤。請確定您的 VM 將系統指派的身分識別設定為 [開啟]。 
    ...
    ![Telegraf VM 身分識別預覽](./media/metrics-store-custom-linux-telegraf/connect-to-VM.png)
 
## <a name="connect-to-the-vm"></a>連接至 VM 

建立與 VM 的 SSH 連線。 在 VM 的 [概觀] 頁面上，選取 [連線] ****  按鈕。 

![Telegraf VM 概觀頁面](./media/metrics-store-custom-linux-telegraf/connect-VM-button2.png)
...
```
Telegraf 的設定檔會定義 Telegraf 的作業。 根據預設，範例組態檔會安裝在 **/etc/telegraf/telegraf.conf** 路徑。 範例組態檔會列出所有可能的輸入與輸出外掛程式。不過，我們將建立自訂組態檔，並透過執行下列命令讓代理程式使用它： 
...
1. 瀏覽到新的 [監視器] ****  索引標籤。 然後選取 [計量] ****。  
     ...
     ![監視器 - 計量 (預覽)](./media/metrics-store-custom-linux-telegraf/metrics.png)

1. 在資源選取器中選取您的 VM。

     ![計量圖表](./media/metrics-store-custom-linux-telegraf/metric-chart.png)

1. 選取 **Telegraf/CPU** 命名空間，然後選取 **usage_system** 計量。 您可以根據此計量的維度來選擇篩選條件，或分割它們。  

     ![選取命名空間和計量](./media/metrics-store-custom-linux-telegraf/VM-resource-selector.png)
