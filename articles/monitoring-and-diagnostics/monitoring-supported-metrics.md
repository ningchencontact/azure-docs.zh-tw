...
|---|---|---|---|---|---|
|qpu_metric|QPU|計數|平均|QPU。 範圍 0-100 (S1)、0-200 (S2) 和 0-400 (S4)|ServerResourceType|
|memory_metric|記憶體|位元組|平均|記憶體。 範圍 0-25 GB (S1)、0-50 GB (S2) 和 0-100 GB (S4)|ServerResourceType|
|TotalConnectionRequests|連線要求的總計|計數|平均|連線要求的總計。 這些是達到的。|ServerResourceType|
...
|CurrentUserSessions|目前的使用者工作階段|計數|平均|目前建立的使用者工作階段數目。|ServerResourceType|
|QueryPoolBusyThreads|查詢集區忙碌執行緒|計數|平均|查詢執行緒集區中的忙碌執行緒數目。|ServerResourceType|
|CommandPoolJobQueueLength|命令集區作業佇列長度|計數|平均|命令執行緒集區佇列中的作業數目。|ServerResourceType|
|ProcessingPoolJobQueueLength|處理集區作業佇列長度|計數|平均|處理執行緒集區佇列中的非 I/O 作業數目。|ServerResourceType|
|CurrentConnections|連接：目前的連接|計數|平均|目前已建立的用戶端連接數目。|ServerResourceType|
|CleanerCurrentPrice|記憶體︰清除工具目前價格|計數|平均值|記憶體目前的價格 ($/位元組/時間)，並正規化為 1000。|ServerResourceType|
|CleanerMemoryShrinkable|記憶體︰可壓縮的清除器記憶體|位元組|平均|背景清除器將清除的記憶體數量，以 KB 為單位。|ServerResourceType|
|CleanerMemoryNonshrinkable|記憶體︰不可壓縮的清除器記憶體|位元組|平均值|背景清除器將不會清除的記憶體數量，以 KB 為單位。|ServerResourceType|
|MemoryUsage|記憶體：記憶體使用量|位元組|平均|伺服器處理序用於計算清除器記憶體價格的記憶體使用量。 等於計數器 Process\PrivateBytes 加上記憶體對應的資料大小，忽略 xVelocity 記憶體內部分析引擎 (VertiPaq) 在超出 xVelocity 引擎記憶體限制外對應或配置的任何記憶體。|ServerResourceType|
|MemoryLimitHard|記憶體︰固定記憶體限制|位元組|平均值|組態檔中的固定記憶體限制。|ServerResourceType|
|MemoryLimitHigh|記憶體︰記憶體上限|位元組|平均|來自組態檔的記憶體上限。|ServerResourceType|
|MemoryLimitLow|記憶體︰記憶體下限|位元組|平均|來自組態檔的記憶體下限。|ServerResourceType|
|MemoryLimitVertiPaq|記憶體︰記憶體限制 VertiPaq|位元組|平均|來自組態檔的記憶體內部限制。|ServerResourceType|
|Quota|記憶體︰配額|位元組|平均值|目前的記憶體配額，以位元組為單位。 記憶體配額也就是指授與使用的記憶體，或是保留的記憶體。|ServerResourceType|
|QuotaBlocked|記憶體︰封鎖的配額|計數|平均|在釋放其他記憶體配額之前，目前已封鎖的配額要求數目。|ServerResourceType|
|VertiPaqNonpaged|記憶體︰未分頁的 VertiPaq|位元組|平均|工作集中已封鎖來供記憶體內部引擎使用的記憶體位元組。|ServerResourceType|
|VertiPaqPaged|記憶體︰分頁的 VertiPaq|位元組|平均|可供記憶體內部資料使用的分頁記憶體位元組。|ServerResourceType|
|RowsReadPerSec|處理︰每秒讀取的資料列|每秒計數|平均|從所有關聯式資料庫讀取資料列的速率。|ServerResourceType|
|RowsConvertedPerSec|處理︰每秒轉換的資料列|每秒計數|平均|處理期間資料列轉換的速率。|ServerResourceType|
|RowsWrittenPerSec|處理︰每秒寫入的資料列|每秒計數|平均|處理期間資料列寫入的速率。|ServerResourceType|
|CommandPoolBusyThreads|執行緒︰命令集區的忙碌執行緒數|計數|平均|命令執行緒集區中的忙碌執行緒數。|ServerResourceType|
|CommandPoolIdleThreads|執行緒︰命令集區的閒置執行緒數|計數|平均值|命令執行緒集區中的閒置執行緒數。|ServerResourceType|
|LongParsingBusyThreads|執行緒︰完整剖析的忙碌執行緒數|計數|平均|完整剖析執行緒集區中的忙碌執行緒數目。|ServerResourceType|
|LongParsingIdleThreads|執行緒︰完整剖析的閒置執行緒數|計數|平均值|完整剖析執行緒集區中的閒置執行緒數目。|ServerResourceType|
|LongParsingJobQueueLength|執行緒︰完整剖析的作業佇列長度|計數|平均|完整剖析執行緒集區佇列中的作業數目。|ServerResourceType|
|ProcessingPoolBusyIOJobThreads|執行緒︰處理集區的忙碌 I/O 作業執行緒數|計數|平均值|處理執行緒集區中執行 I/O 作業的執行緒數目。|ServerResourceType|
|ProcessingPoolBusyNonIOThreads|執行緒︰處理集區的忙碌非 I/O 執行緒數|計數|平均|處理執行緒集區中執行非 I/O 作業的執行緒數目。|ServerResourceType|
|ProcessingPoolIOJobQueueLength|執行緒：處理集區 I/O 作業佇列長度|計數|平均值|處理執行緒集區佇列中的 I/O 作業數目。|ServerResourceType|
|ProcessingPoolIdleIOJobThreads|執行緒︰處理集區的閒置 I/O 作業執行緒數|計數|平均|處理執行緒集區中 I/O 作業的閒置執行緒數目。|ServerResourceType|
|ProcessingPoolIdleNonIOThreads|執行緒︰處理集區的閒置非 I/O 執行緒數|計數|平均|處理執行緒集區中專供非 I/O 作業使用的閒置執行緒數目。|ServerResourceType|
...
|ShortParsingBusyThreads|執行緒︰簡短剖析的忙碌執行緒數|計數|平均|簡短剖析執行緒集區中的忙碌執行緒數目。|ServerResourceType|
|ShortParsingIdleThreads|執行緒︰簡短剖析的閒置執行緒數|計數|平均|簡短剖析執行緒集區中的閒置執行緒數目。|ServerResourceType|
...
|mashup_engine_memory_metric|M 引擎記憶體|位元組|平均|混搭引擎處理序的記憶體使用量|ServerResourceType|
...
|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|TotalJob|工作總數|計數|總計|工作總數|RunbookName 狀態|
...
|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|connectedclients|連線的用戶端|計數|最大值||無維度|
|totalcommandsprocessed|總作業數|計數|總計||無維度|
|cachehits|快取點擊|計數|總計||無維度|
|cachemisses|快取遺漏|計數|總計||無維度|
|getcommands|取得|計數|總計||無維度|
|setcommands|設定|計數|總計||無維度|
|evictedkeys|收回的金鑰|計數|總計||無維度|
|totalkeys|索引鍵總計|計數|最大值||無維度|
|expiredkeys|到期的金鑰|計數|總計||無維度|
...
|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|Percentage CPU|Percentage CPU|百分比|平均|虛擬機器目前使用的配置計算單位百分比。|無維度|
...
|Disk Read Bytes/Sec|磁碟讀取|每秒位元組|平均|監視期間從磁碟讀取的平均位元組。|無維度|
|Disk Write Bytes/Sec|磁碟寫入|每秒位元組|平均值|監視期間寫入磁碟的平均位元組。|無維度|
|Disk Read Operations/Sec|Disk Read Operations/Sec|每秒計數|平均|磁碟讀取 IOPS。|無維度|
|Disk Write Operations/Sec|Disk Write Operations/Sec|每秒計數|平均|磁碟寫入 IOPS。|無維度|

## <a name="microsoftcognitiveservicesaccounts"></a>Microsoft.CognitiveServices/accounts

|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|TotalCalls|呼叫總數|計數|總計|呼叫總數。|無維度|
|SuccessfulCalls|成功的呼叫|計數|總計|成功的呼叫數。|無維度|
|TotalErrors|錯誤總數|計數|總計|具有錯誤回應 (HTTP 回應碼 4xx 或 5xx) 的呼叫總數。|無維度|
|BlockedCalls|已封鎖的呼叫|計數|總計|超過速率或配額限制的呼叫數目。|無維度|
|ServerErrors|伺服器錯誤|計數|總計|具有服務內部錯誤 (HTTP 回應碼 5xx) 的呼叫數目。|無維度|
|ClientErrors|用戶端錯誤|計數|總計|具有用戶端錯誤 (HTTP 回應碼 4xx) 的呼叫數目。|無維度|
|DataIn|資料輸入|位元組|總計|內送資料大小，以位元組為單位。|無維度|
|DataOut|資料輸出|位元組|總計|輸出資料大小，以位元組為單位。|無維度|
|Latency|Latency|毫秒|平均值|延遲，以毫秒為單位。|無維度|
|CharactersTranslated|轉譯字元數|計數|總計|傳入文字要求中的字元總數。|無維度|
|SpeechSessionDuration|語音工作階段持續時間|秒|總計|語音工作階段的總持續時間 (秒)。|無維度|
|TotalTransactions|總交易|計數|總計|總交易數|無維度|

## <a name="microsoftcomputevirtualmachines"></a>Microsoft.Compute/virtualMachines

|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|Percentage CPU|Percentage CPU|百分比|平均|目前虛擬機器正在使用中的已配置計算單位百分比|無維度|
|Network In|Network In|位元組|總計|虛擬機器 (連入流量) 在所有網路介面上接收到的位元組數目|無維度|
|Network Out|Network Out|位元組|總計|虛擬機器 (連出流量) 在所有網路介面上送出的位元組數目|無維度|
...
|Disk Write Operations/Sec|Disk Write Operations/Sec|每秒計數|平均|磁碟寫入 IOPS|無維度|
|剩餘 CPU 信用額度|剩餘 CPU 信用額度|計數|平均|可用於高載的信用額度總數|無維度|
...
|剩餘 CPU 信用額度|剩餘 CPU 信用額度|計數|平均|可用於高載的信用額度總數|無維度|
|CPU Credits Consumed|CPU Credits Consumed|計數|平均|由虛擬機器取用的信用額度總數|無維度|
...
|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|SuccessfulRuns|成功的執行|計數|總計|成功執行的數目。|無維度|
|FailedRuns|失敗的執行|計數|總計|失敗的執行次數。|無維度|

## <a name="microsoftdatafactoryfactories"></a>Microsoft.DataFactory/factories

|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|PipelineFailedRuns|失敗的管線執行計量|計數|總計||無維度|
|PipelineSucceededRuns|成功的管線執行計量|計數|總計||無維度|
|ActivityFailedRuns|失敗的活動執行計量|計數|總計||無維度|
|ActivitySucceededRuns|成功的活動執行計量|計數|總計||無維度|
|TriggerFailedRuns|失敗的觸發程序執行計量|計數|總計||無維度|
|TriggerSucceededRuns|成功的觸發程序執行計量|計數|總計||無維度|
...
|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|cpu_percent|CPU 百分比|百分比|平均|CPU 百分比|無維度|
|compute_limit|計算單位限制|計數|平均值|計算單位限制|無維度|
|compute_consumption_percent|計算單位百分比|百分比|平均|計算單位百分比|無維度|
|memory_percent|記憶體百分比|百分比|平均值|記憶體百分比|無維度|
|io_consumption_percent|IO 百分比|百分比|平均|IO 百分比|無維度|
|storage_percent|儲存體百分比|百分比|平均|儲存體百分比|無維度|
|storage_used|已使用儲存體|位元組|平均|已使用儲存體|無維度|
|storage_limit|儲存體限制|位元組|平均|儲存體限制|無維度|
|active_connections|作用中的連接總數|計數|平均|作用中的連接總數|無維度|
|connections_failed|失敗的連接總數|計數|平均|失敗的連接總數|無維度|
...
|storage_used|已使用儲存體|位元組|平均|已使用儲存體|無維度|
|storage_limit|儲存體限制|位元組|平均|儲存體限制|無維度|
|active_connections|作用中的連接總數|計數|平均值|作用中的連接總數|無維度|
|connections_failed|失敗的連接總數|計數|平均|失敗的連接總數|無維度|
...
|devices.totalDevices|裝置總計|計數|總計|向 IoT 中樞註冊的裝置數目|無維度|
|devices.connectedDevices.allProtocol|連接的裝置|計數|總計|連接至 IoT 中樞的裝置數目|無維度|
|d2c.telemetry.egress.success|已傳遞的遙測訊息|計數|總計|訊息成功寫入端點的次數 (總計)|無維度|
|d2c.telemetry.egress.dropped|捨棄的訊息|計數|總計|由於傳遞端點已無作用而卸除的訊息數目|無維度|
|d2c.telemetry.egress.orphaned|被遺棄的訊息|計數|總計|不符合任何路由 (包括後援路由) 的訊息計數|無維度|
|d2c.telemetry.egress.invalid|無效的訊息|計數|總計|因為與端點不相容而未傳遞的訊息計數|無維度|
|d2c.telemetry.egress.fallback|符合後援條件的訊息|計數|總計|寫入後援端點的訊息數目|無維度|
|d2c.endpoints.egress.eventHubs|傳遞至事件中樞端點的訊息|計數|總計|訊息成功寫入事件中樞端點的次數|無維度|
|d2c.endpoints.latency.eventHubs|事件中樞端點的訊息延遲|毫秒|平均|訊息輸入到 IoT 中樞與訊息輸入到事件中樞端點之間的平均延遲，以毫秒為單位|無維度|
|d2c.endpoints.egress.serviceBusQueues|傳遞至服務匯流排佇列端點的訊息|計數|總計|訊息成功寫入服務匯流排佇列端點的次數|無維度|
|d2c.endpoints.latency.serviceBusQueues|服務匯流排佇列端點的訊息延遲|毫秒|平均|訊息輸入到 IoT 中樞與訊息輸入到服務匯流排佇列端點之間的平均延遲，以毫秒為單位|無維度|
|d2c.endpoints.egress.serviceBusTopics|傳遞至服務匯流排主題端點的訊息|計數|總計|訊息成功寫入服務匯流排主題端點的次數|無維度|
|d2c.endpoints.latency.serviceBusTopics|服務匯流排主題端點的訊息延遲|毫秒|平均|訊息輸入到 IoT 中樞與訊息輸入到服務匯流排主題端點之間的平均延遲，以毫秒為單位|無維度|
|d2c.endpoints.egress.builtIn.events|傳遞至內建端點 (訊息/事件) 的訊息|計數|總計|訊息成功寫入內建端點 (訊息/事件) 的次數|無維度|
|d2c.endpoints.latency.builtIn.events|內建端點 (訊息/事件) 的訊息延遲|毫秒|平均值|訊息輸入到 IoT 中樞與訊息輸入到內建端點 (訊息/事件) 之間的平均延遲，以毫秒為單位 |無維度|
|d2c.endpoints.egress.storage|傳遞至儲存體端點的訊息|計數|總計|訊息成功寫入儲存體端點的次數|無維度|
...
|d2c.endpoints.egress.storage.blobs|寫入儲存體的 Blob|計數|總計|寫入儲存體端點的 Blob 數目|無維度|
|d2c.twin.read.success|裝置的成功對應項讀取|計數|總計|裝置起始的所有成功對應項讀取的計數。|無維度|
|d2c.twin.read.failure|裝置的失敗對應項讀取|計數|總計|裝置起始的所有失敗對應項讀取的計數。|無維度|
|d2c.twin.read.size|裝置的對應項讀取回應大小|位元組|平均|裝置起始的所有成功對應項讀取的平均值、最小值和最大值。|無維度|
...
|d2c.twin.update.size|裝置的對應項更新大小|位元組|平均|裝置起始的所有成功對應項更新的大小平均值、最小值和最大值。|無維度|
|c2d.methods.success|成功直接方法叫用|計數|總計|所有成功直接方法呼叫的計數。|無維度|
|c2d.methods.failure|失敗直接方法叫用|計數|總計|所有失敗直接方法呼叫的計數。|無維度|
|c2d.methods.requestSize|直接方法叫用的要求大小|位元組|平均|所有成功直接方法要求的平均值、最小值和最大值。|無維度|
|c2d.methods.responseSize|直接方法叫用的回應大小|位元組|平均|所有成功直接方法回應的平均值、最小值和最大值。|無維度|
|c2d.twin.read.success|後端的成功對應項讀取|計數|總計|後端起始的所有成功對應項讀取的計數。|無維度|
|c2d.twin.read.failure|後端的失敗對應項讀取|計數|總計|後端起始的所有失敗對應項讀取的計數。|無維度|
|c2d.twin.read.size|後端的對應項讀取回應大小|位元組|平均|後端起始的所有成功對應項讀取的平均值、最小值和最大值。|無維度|
...
|dailyMessageQuotaUsed|已使用的訊息總數|計數|平均|今日已使用的總訊息數。 這是一個累計值，會在每天的 00:00 UTC 重設為零。|無維度|
...
|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|elasticPool.requestedUsageRate|要求的使用方式的速率|百分比|平均|要求的使用方式的速率|無維度|

## <a name="microsoftdeviceselasticpoolsiothubtenants"></a>Microsoft.Devices/ElasticPools/IotHubTenants

|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|tenantHub.requestedUsageRate|要求的使用方式的速率|百分比|平均|要求的使用方式的速率|無維度|
|deviceDataUsage|總 devicedata 使用量|計數|總計|連接到 iot 中樞的任何裝置間傳輸的位元組|無維度|
|d2c.telemetry.ingress.allProtocol|遙測訊息傳送嘗試|計數|總計|要嘗試傳送至您 IoT 中樞的裝置到雲端遙測訊息數目|無維度|
|d2c.telemetry.ingress.success|已傳送的遙測訊息|計數|總計|成功傳送至您 IoT 中樞的裝置到雲端遙測訊息數目|無維度|
|c2d.commands.egress.complete.success|完成的命令|計數|總計|裝置成功完成的雲端到裝置命令數目|無維度|
|c2d.commands.egress.abandon.success|放棄的命令|計數|總計|裝置放棄的雲端到裝置命令數目|無維度|
...
|d2c.endpoints.egress.builtIn.events|傳遞至內建端點 (訊息/事件) 的訊息|計數|總計|訊息成功寫入內建端點 (訊息/事件) 的次數|無維度|
|d2c.endpoints.latency.builtIn.events|內建端點 (訊息/事件) 的訊息延遲|毫秒|平均|訊息輸入到 IoT 中樞與訊息輸入到內建端點 (訊息/事件) 之間的平均延遲，以毫秒為單位 |無維度|
|d2c.endpoints.egress.storage|傳遞至儲存體端點的訊息|計數|總計|訊息成功寫入儲存體端點的次數|無維度|
|d2c.endpoints.latency.storage|儲存體端點的訊息延遲|毫秒|平均|訊息輸入到 IoT 中樞與訊息輸入到儲存體端點之間的平均延遲，以毫秒為單位|無維度|
|d2c.endpoints.egress.storage.bytes|寫入儲存體的資料|位元組|總計|寫入儲存體端點的資料量，以位元組為單位|無維度|
|d2c.endpoints.egress.storage.blobs|寫入儲存體的 Blob|計數|總計|寫入儲存體端點的 Blob 數目|無維度|
|d2c.twin.read.success|裝置的成功對應項讀取|計數|總計|裝置起始的所有成功對應項讀取的計數。|無維度|
|d2c.twin.read.failure|裝置的失敗對應項讀取|計數|總計|裝置起始的所有失敗對應項讀取的計數。|無維度|
...
|twinQueries.success|成功對應項查詢|計數|總計|所有成功對應項查詢的計數。|無維度|
|twinQueries.failure|失敗對應項查詢|計數|總計|所有失敗對應項查詢的計數。|無維度|
|twinQueries.resultSize|對應項查詢結果大小|位元組|平均|所有成功對應項查詢的結果大小平均值、最小值和最大值。|無維度|
|jobs.createTwinUpdateJob.success|成功建立的對應項更新作業|計數|總計|所有成功建立的對應項更新作業計數。|無維度|
|jobs.createTwinUpdateJob.failure|建立失敗的對應項更新作業|計數|總計|所有建立失敗的對應項更新作業計數。|無維度|
|jobs.createDirectMethodJob.success|成功建立的方法叫用作業|計數|總計|所有成功建立的直接方法叫用作業計數。|無維度|
|jobs.createDirectMethodJob.failure|建立失敗的方法叫用作業|計數|總計|所有建立失敗的直接方法叫用作業計數。|無維度|
...
|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|MetadataRequests|中繼資料要求|計數|計數|中繼資料要求的計數。 Cosmos DB 會維護每個帳戶，可讓您列舉集合、 資料庫等的系統中繼資料集合和其設定，免費提供。|GlobalDatabaseAccountName、 資料庫名稱、 集合名稱、 地區、 StatusCode|
|MongoRequestCharge|Mongo 要求電量|計數|總計|使用 mongo 要求單位|GlobalDatabaseAccountName、 DatabaseName、 CollectionName、 地區、 CommandName、 ErrorCode|
|MongoRequests|Mongo 要求|計數|計數|Mongo 提出的要求數目|GlobalDatabaseAccountName、 DatabaseName、 CollectionName、 地區、 CommandName、 ErrorCode|
|TotalRequestUnits|總要求單位|計數|總計|要求單位取用|GlobalDatabaseAccountName、 資料庫名稱、 集合名稱、 地區、 StatusCode|
|TotalRequests|要求總數|計數|計數|進行的要求數目|GlobalDatabaseAccountName、 資料庫名稱、 集合名稱、 地區、 StatusCode|
...
|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|SuccessfulRequests|成功要求 （預覽）|計數|總計|Microsoft.EventHub 的成功要求。 (預覽)|EntityName|
|ServerErrors|伺服器錯誤。 (預覽)|計數|總計|Microsoft.EventHub 的伺服器錯誤。 (預覽)|EntityName|
|UserErrors|使用者錯誤。 (預覽)|計數|總計|Microsoft.EventHub 的使用者錯誤。 (預覽)|EntityName|
|QuotaExceededErrors|超出配額的錯誤。 (預覽)|計數|總計|Microsoft.EventHub 超出配額的錯誤。 (預覽)|EntityName|
|ThrottledRequests|節流的要求。 (預覽)|計數|總計|Microsoft.EventHub 的節流要求。 (預覽)|EntityName|
|IncomingRequests|傳入要求 （預覽）|計數|總計|Microsoft.EventHub 的傳入要求。 (預覽)|EntityName|
|IncomingMessages|內送訊息 （預覽）|計數|總計|Microsoft.EventHub 的內送郵件。 (預覽)|EntityName|
|OutgoingMessages|外寄訊息 （預覽）|計數|總計|Microsoft.EventHub 的外寄郵件。 (預覽)|EntityName|
|IncomingBytes|傳入的位元組。 (預覽)|位元組|總計|Microsoft.EventHub 的傳入位元組。 (預覽)|EntityName|
|OutgoingBytes|傳出的位元組。 (預覽)|位元組|總計|Microsoft.EventHub 的傳出位元組。 (預覽)|EntityName|
|ActiveConnections|ActiveConnections （預覽）|計數|總計|Microsoft.EventHub 的使用中連線總數。 (預覽)|EntityName|
|ConnectionsOpened|已開啟的連線。 (預覽)|計數|總計|Microsoft.EventHub 的已開啟連線。 (預覽)|EntityName|
|ConnectionsClosed|已關閉的連線。 (預覽)|計數|總計|Microsoft.EventHub 的已關閉連線。 (預覽)|EntityName|
|CaptureBacklog|擷取待辦項目。 (預覽)|計數|總計|Microsoft.EventHub 的擷取待辦項目。 (預覽)|EntityName|
|CapturedMessages|已擷取的訊息。 (預覽)|計數|總計|Microsoft.EventHub 的已擷取訊息。 (預覽)|EntityName|
|CapturedBytes|已擷取的位元組。 (預覽)|位元組|總計|Microsoft.EventHub 的已擷取位元組。 (預覽)|EntityName|
|INREQS|傳入的要求|計數|總計|命名空間的連入傳送要求總計|無維度|
|SUCCREQ|成功的要求|計數|總計|命名空間的成功要求總數|無維度|
|FAILREQ|失敗的要求|計數|總計|命名空間的失敗要求總數|無維度|
|SVRBSY|伺服器忙線錯誤|計數|總計|命名空間的伺服器忙線錯誤總數|無維度|
|INTERR|內部伺服器錯誤|計數|總計|命名空間的內部伺服器錯誤總數|無維度|
|MISCERR|其他錯誤|計數|總計|命名空間的失敗要求總數|無維度|
|INMSGS|連入訊息數 (已過時)|計數|總計|命名空間的傳入訊息總數。 此計量已過時。 請改為使用「傳入訊息數」計量|無維度|
|EHINMSGS|傳入訊息|計數|總計|命名空間的內送訊息總數|無維度|
|OUTMSGS|傳出訊息數 (已過時)|計數|總計|命名空間的傳出訊息總數。 此計量已過時。 請改為使用「傳出訊息數」計量|無維度|
|EHOUTMSGS|外送訊息|計數|總計|命名空間的外送訊息總數|無維度|
...
|EHABL|封存待處理項目訊息|計數|總計|命名空間的事件中樞封存於待處理項目中訊息|無維度|
|EHAMSGS|封存訊息|計數|總計|命名空間中的事件中樞已封存訊息|無維度|
...
|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|ObservedMetricValue|觀察的計量值|計數|平均|執行時由自動調整規模計算的值|MetricTriggerSource|
|MetricThreshold|計量閾值|計數|平均值|當自動調整規模執行時設定的自動調整規模閾值。|MetricTriggerRule|
|ObservedCapacity|觀察的容量|計數|平均|執行時向自動調整規模回報的容量。|無維度|
...
|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|Latency|Latency|毫秒|平均值|應用程式開發介面呼叫持續時間|OperationName OperationResult|
...
|RunSuccessLatency|執行成功的延遲|秒|平均|已成功的工作流程執行延遲。|無維度|
...
|ActionSuccessLatency|動作成功延遲 |秒|平均|已成功的工作流程動作延遲。|無維度|
...
|TriggerSuccessLatency|觸發程序成功延遲 |秒|平均|已成功的工作流程觸發程序延遲。|無維度|
...
|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|BytesSentRate|傳送的位元組|計數|總計|網路介面傳送的位元組數目|無維度|
|BytesReceivedRate|已接收的位元組|計數|總計|網路介面接收的位元組數目|無維度|
|PacketsSentRate|傳送的封包|計數|總計|網路介面傳送的封包數目|無維度|
|PacketsReceivedRate|收到的封包|計數|總計|接收的網路介面的封包數目|無維度|

## <a name="microsoftnetworkloadbalancers"></a>Microsoft.Network/loadBalancers

|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|VipAvailability|VIP 可用性|計數|平均|以探查結果為基礎的 VIP 端點可用性|VipAddress，VipPort|
|DipAvailability|DIP 可用性|計數|平均|以探查結果為基礎的 DIP 端點可用性|ProtocolType，DipPort，VipAddress，VipPort，DipAddress|
|ByteCount|位元組計數|計數|總計|在期間內傳輸的位元組總數|VipAddress，VipPort，Direction|
|PacketCount|封包計數|計數|總計|在期間內傳輸的封包總數|VipAddress，VipPort，Direction|
|SYNCount|SYN 計數|計數|總計|在期間內傳輸的 SYN 封包總數|VipAddress，VipPort，Direction|
|SnatConnectionCount|SNAT 連線計數|計數|總計|在期間內建立的新 SNAT 連線總數|VipAddress，DipAddress，ConnectionState|
...
|DDoSTriggerSYNPackets|輸入觸發 DDoS 緩和 SYN 封包|每秒計數|最大值|輸入觸發 DDoS 緩和 SYN 封包|無維度|
|VipAvailability|Availability|計數|平均值|期間內的平均 IPAddress 可用性|Port|
...
|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|PacketsInDroppedVMProtection|為 VM 保護卸除輸入封包|每秒計數|平均值|為 VM 保護卸除輸入封包|無維度|
|PacketsOutDroppedVMProtection|為 VM 保護卸除輸出封包|每秒計數|平均值|為 VM 保護卸除輸出封包|無維度|
...
|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|TunnelAverageBandwidth|通道頻寬|每秒位元組|平均|通道的平均頻寬 (位元組/秒)|ConnectionName，RemoteIP|
|TunnelEgressBytes|通道輸出位元組數|位元組|總計|傳出通道的位元組|ConnectionName，RemoteIP|
|TunnelIngressBytes|通道輸入位元組數|位元組|總計|傳入通道的位元組|ConnectionName，RemoteIP|
|TunnelEgressPackets|通道輸出封包數|計數|總計|通道的傳出封包計數|ConnectionName，RemoteIP|
|TunnelIngressPackets|通道輸入封包數|計數|總計|通道的連入封包計數|ConnectionName，RemoteIP|
|TunnelEgressPacketDropTSMismatch|通道輸出 TS 不相符封包捨棄|計數|總計|從通道流量選取器不相符的傳出封包卸除計數|ConnectionName，RemoteIP|
|TunnelIngressPacketDropTSMismatch|通道輸入 TS 不符合封包捨棄|計數|總計|從通道流量選取器不相符的連入封包卸除計數|ConnectionName，RemoteIP|

## <a name="microsoftnetworkexpressroutecircuits"></a>Microsoft.Network/expressRouteCircuits

|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|BitsInPerSecond|BitsInPerSecond|每秒計數|平均|每秒輸入到 Azure 的位元數|無維度|
...
|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|SearchLatency|搜尋延遲|秒|平均|搜尋服務的平均搜尋延遲|無維度|
|SearchQueriesPerSecond|每秒搜尋查詢|每秒計數|平均|搜尋服務的每秒搜尋查詢|無維度|
...
|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|SuccessfulRequests|成功要求 （預覽）|計數|總計|命名空間 （預覽） 成功要求總數|EntityName|
...
|IncomingMessages|內送訊息 （預覽）|計數|總計|Microsoft.ServiceBus 的內送郵件。 (預覽)|EntityName|
|OutgoingMessages|外寄訊息 （預覽）|計數|總計|Microsoft.ServiceBus 的外寄郵件。 (預覽)|EntityName|
|ActiveConnections|ActiveConnections （預覽）|計數|總計|Microsoft.ServiceBus 的使用中連線總數。 (預覽)|EntityName|
|ConnectionsOpened|已開啟的連線。 (預覽)|計數|總計|Microsoft.ServiceBus 的已開啟連線。 (預覽)|EntityName|
|ConnectionsClosed|已關閉的連線。 (預覽)|計數|總計|Microsoft.ServiceBus 的已關閉連線。 (預覽)|EntityName|
...
|---|---|---|---|---|---|
|cpu_percent|CPU 百分比|百分比|平均值|CPU 百分比|無維度|
|physical_data_read_percent|資料 IO 百分比|百分比|平均|資料 IO 百分比|無維度|
...
|workers_percent|背景工作角色百分比|百分比|平均|背景工作角色百分比|無維度|
|sessions_percent|工作階段百分比|百分比|平均|工作階段百分比|無維度|
|dtu_limit|DTU 限制|計數|平均|DTU 限制|無維度|
|dtu_used|已使用 DTU|計數|平均|已使用 DTU|無維度|
...
|---|---|---|---|---|---|
|cpu_percent|CPU 百分比|百分比|平均|CPU 百分比|無維度|
|database_cpu_percent|CPU 百分比|百分比|平均|CPU 百分比|DatabaseResourceId|
|physical_data_read_percent|資料 IO 百分比|百分比|平均|資料 IO 百分比|無維度|
|database_physical_data_read_percent|資料 IO 百分比|百分比|平均|資料 IO 百分比|DatabaseResourceId|
|log_write_percent|記錄 IO 百分比|百分比|平均值|記錄 IO 百分比|無維度|
|database_log_write_percent|記錄 IO 百分比|百分比|平均|記錄 IO 百分比|DatabaseResourceId|
|dtu_consumption_percent|DTU 百分比|百分比|平均|DTU 百分比|無維度|
|database_dtu_consumption_percent|DTU 百分比|百分比|平均|DTU 百分比|DatabaseResourceId|
|storage_percent|儲存體百分比|百分比|平均|儲存體百分比|無維度|
|workers_percent|背景工作角色百分比|百分比|平均|背景工作角色百分比|無維度|
|database_workers_percent|背景工作角色百分比|百分比|平均|背景工作角色百分比|DatabaseResourceId|
|sessions_percent|工作階段百分比|百分比|平均|工作階段百分比|無維度|
|database_sessions_percent|工作階段百分比|百分比|平均值|工作階段百分比|DatabaseResourceId|
|eDTU_limit|eDTU 限制|計數|平均|eDTU 限制|無維度|
|storage_limit|儲存體限制|位元組|平均|儲存體限制|無維度|
|eDTU_used|已使用 eDTU|計數|平均|已使用 eDTU|無維度|
|storage_used|已使用儲存體|位元組|平均|已使用儲存體|無維度|
|database_storage_used|已使用儲存體|位元組|平均|已使用儲存體|DatabaseResourceId|
|xtp_storage_percent|記憶體內部 OLTP 儲存體百分比|百分比|平均|記憶體內部 OLTP 儲存體百分比|無維度|

## <a name="microsoftsqlservers"></a>Microsoft.Sql/servers

|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|dtu_consumption_percent|DTU 百分比|百分比|平均|DTU 百分比|ElasticPoolResourceId|
|database_dtu_consumption_percent|DTU 百分比|百分比|平均值|DTU 百分比|DatabaseResourceId, ElasticPoolResourceId|
|storage_used|已使用儲存體|位元組|平均|已使用儲存體|ElasticPoolResourceId|
|database_storage_used|已使用儲存體|位元組|平均|已使用儲存體|DatabaseResourceId, ElasticPoolResourceId|
...
|交易|交易|計數|總計|向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 使用 ResponseType 維度來取得不同類型回應的數目。|ResponseType，GeoType，ApiName|
...
|SuccessServerLatency|成功伺服器延遲|毫秒|平均|Azure 儲存體用來處理成功要求的平均延遲 (毫秒)。 此值不包括在 AverageE2ELatency 中指定的網路延遲。|GeoType，ApiName|
|SuccessE2ELatency|成功 E2E 延遲|毫秒|平均|向儲存體服務或所指定 API 作業發出之成功要求的平均端對端延遲 (毫秒)。 此值包括 Azure 儲存體內讀取要求、傳送回應及接收回應認可的必要處理時間。|GeoType，ApiName|
|Availability|Availability|百分比|平均|儲存體服務或所指定 API 作業的可用性百分比。 可用性的計算方式是將 TotalBillableRequests 值除以適用要求數目，包括產生意外錯誤的要求。 所有意外錯誤都會導致儲存體或所指定 API 作業的可用性降低。|GeoType，ApiName|

## <a name="microsoftstoragestorageaccountsblobservices"></a>Microsoft.Storage/storageAccounts/blobServices

|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|BlobCapacity|Blob 容量|位元組|平均|儲存體帳戶之 Blob 服務所使用的儲存體量 (位元組)。|BlobType|
|BlobCount|Blob 計數|計數|平均值|儲存體帳戶之 Blob 服務中的 Blob 數目。|BlobType|
|ContainerCount|Blob 容器計數|計數|平均|儲存體帳戶之 Blob 服務中的容器數目。|無維度|
|交易|交易|計數|總計|向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 使用 ResponseType 維度來取得不同類型回應的數目。|ResponseType，GeoType，ApiName|
|輸入|輸入|位元組|總計|輸入資料量 (位元組)。 此數目包括從外部用戶端輸入到 Azure 儲存體與 Azure 內的輸入。|GeoType，ApiName|
|輸出|輸出|位元組|總計|輸出資料量 (位元組)。 此數目包括從外部用戶端輸出到 Azure 儲存體與 Azure 內的輸出。 因此，此數目未反映可收費的輸出。|GeoType，ApiName|
|SuccessServerLatency|成功伺服器延遲|毫秒|平均|Azure 儲存體用來處理成功要求的平均延遲 (毫秒)。 此值不包括在 AverageE2ELatency 中指定的網路延遲。|GeoType，ApiName|
...
|TableCount|資料表計數|計數|平均|儲存體帳戶之資料表服務中的資料表數目。|無維度|
...
|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|QueueCapacity|佇列容量|位元組|平均|儲存體帳戶之佇列服務所使用的儲存體量 (位元組)。|無維度|
...
|FileCount|檔案計數|計數|平均|儲存體帳戶之檔案服務中的檔案數目。|無維度|
|FileShareCount|檔案共用計數|計數|平均|儲存體帳戶之檔案服務中的檔案共用數目。|無維度|
|交易|交易|計數|總計|向儲存體服務或所指定 API 作業傳送的要求數。 此數目包括成功與失敗的要求，以及產生錯誤的要求。 使用 ResponseType 維度來取得不同類型回應的數目。|ResponseType，GeoType，ApiName|
|輸入|輸入|位元組|總計|輸入資料量 (位元組)。 此數目包括從外部用戶端輸入到 Azure 儲存體與 Azure 內的輸入。|GeoType，ApiName|
|輸出|輸出|位元組|總計|輸出資料量 (位元組)。 此數目包括從外部用戶端輸出到 Azure 儲存體與 Azure 內的輸出。 因此，此數目未反映可收費的輸出。|GeoType，ApiName|
|SuccessServerLatency|成功伺服器延遲|毫秒|平均|Azure 儲存體用來處理成功要求的平均延遲 (毫秒)。 此值不包括在 AverageE2ELatency 中指定的網路延遲。|GeoType，ApiName|
...
|InputEvents|輸入事件|計數|總計|輸入事件|無維度|
|InputEventBytes|輸入事件位元組|位元組|總計|輸入事件位元組|無維度|
|LateInputEvents|延遲輸入事件|計數|總計|延遲輸入事件|無維度|
|OutputEvents|輸出事件|計數|總計|輸出事件|無維度|
|ConversionErrors|資料轉換錯誤|計數|總計|資料轉換錯誤|無維度|
|Errors|執行階段錯誤|計數|總計|執行階段錯誤|無維度|
|DroppedOrAdjustedEvents|次序錯誤事件|計數|總計|次序錯誤事件|無維度|
|AMLCalloutRequests|函式要求|計數|總計|函式要求|無維度|
|AMLCalloutFailedRequests|失敗的函式要求|計數|總計|失敗的函式要求|無維度|
|AMLCalloutInputEvents|函式事件|計數|總計|函式事件|無維度|

## <a name="microsoftwebserverfarms"></a>Microsoft.Web/serverfarms

|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|CpuPercentage|CPU 百分比|百分比|平均|CPU 百分比|執行個體|
|MemoryPercentage|記憶體百分比|百分比|平均|記憶體百分比|執行個體|
|DiskQueueLength|磁碟佇列長度|計數|平均|磁碟佇列長度|執行個體|
|HttpQueueLength|Http 佇列長度|計數|平均|Http 佇列長度|執行個體|
...
|AverageResponseTime|平均回應時間|秒|平均|平均回應時間|執行個體|
...
|FunctionExecutionUnits|函式執行單位|計數|平均值|函式執行單位|執行個體|
|FunctionExecutionCount|函式執行計數|計數|平均|函式執行計數|執行個體|
...
|TotalFrontEnds|前端總數|計數|平均值|前端總數|執行個體|
|SmallAppServicePlanInstances|小型 App Service 方案背景工作角色|計數|平均值|小型 App Service 方案背景工作角色|執行個體|
|MediumAppServicePlanInstances|中型 App Service 方案背景工作角色|計數|平均值|中型 App Service 方案背景工作角色|執行個體|
|LargeAppServicePlanInstances|大型 App Service 方案背景工作角色|計數|平均值|大型 App Service 方案背景工作角色|執行個體|

## <a name="microsoftwebhostingenvironmentsworkerpools"></a>Microsoft.Web/hostingEnvironments/workerPools

|計量|計量顯示名稱|單位|彙總類型|說明|維度|
|---|---|---|---|---|---|
|WorkersTotal|背景工作角色總數|計數|平均|背景工作角色總數|執行個體|
|WorkersAvailable|可用的背景工作角色|計數|平均|可用的背景工作角色|執行個體|
|WorkersUsed|已使用的背景工作角色|計數|平均值|已使用的背景工作角色|執行個體|
|CpuPercentage|CPU 百分比|百分比|平均|CPU 百分比|執行個體|
|MemoryPercentage|記憶體百分比|百分比|平均|記憶體百分比|執行個體|
