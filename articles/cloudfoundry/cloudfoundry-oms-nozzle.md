...
如果您已透過 Marketplace 範本部署監視解決方案，請前往 Azure 入口網站並尋找該解決方案。 您可以在資源群組中找到您在範本中指定的解決方案。 按一下解決方案，流覽至 [log analytics 主控台]，預先設定的視圖會列出，其中包含最 Cloud Foundry 的系統 Kpi、應用程式資料、警示和 VM 健康情況計量。 
