...
[Azure 監視器](https://azure.microsoft.com/services/log-analytics/)是 Azure 中的服務。 它能協助您收集和分析從雲端及內部部署環境產生的資料。

Log Analytics Nozzle (為 Nozzle) 是 Cloud Foundry (CF) 元件，可將計量從[Cloud Foundry loggregator](https://docs.cloudfoundry.org/loggregator/architecture.html) firehose 轉送至 Azure 監視器記錄檔。 在 Nozzle 的協助之下，您可以收集、檢視及分析多個部署的系統健康情況和效能計量。

在本文件中，您會學習如何將 Nozzle 部署到 CF 環境，並從 Azure 監視器記錄主控台存取資料。
...
您可以手動方式或使用範本來建立 Log Analytics 工作區。 範本會部署預先設定的 KPI 檢視和警示 Azure 監視器記錄主控台的安裝。 
...
如果您已透過 Marketplace 範本部署監視解決方案，請前往 Azure 入口網站並尋找該解決方案。 您可以在資源群組中找到您在範本中指定的解決方案。 按一下 [方案] 中，瀏覽至 「 log analytics 主控台 」，預先設定的檢視未列出，請使用頂端的 Cloud Foundry 系統 Kpi、 應用程式資料、 警示和 VM 健康情況計量。 
...
| Type=CF_ValueMetric_CL Name_s=slowConsumerAlert                               | 結果數目 > 0   | 當 Nozzle 自 loggregator 收到緩慢取用者警示時，它會傳送**slowConsumerAlert** valuemetric 傳送給 Azure 監視器記錄檔。 |
...
如果您移除 Nozzle，OMS 入口網站中的資料並不會自動移除。 根據您設定的 Azure 監視器記錄檔保留過期。
...
從 PCF2.0，會傳輸到 Azure Log Analytics nozzle 系統計量轉寄站，VM 的效能計量，並將其整合到 Log Analytics 工作區中。 您不再需要 Log Analytics 代理程式，就能取得 VM 效能計量。 不過，您仍然可以使用 Log Analytics 代理程式來收集 Syslog 資訊。 Log Analytics 代理程式會以 Bosh 附加元件的形式安裝至您的 CF VM。 
