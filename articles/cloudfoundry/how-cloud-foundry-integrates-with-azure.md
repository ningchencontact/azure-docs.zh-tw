...
description: 描述如何 Cloud Foundry 可以使用 Azure 服務來加強企業經驗
...
ms.locfileid: "60198953"
---
# <a name="integrate-cloud-foundry-with-azure"></a>整合 Cloud Foundry 與 Azure

[Cloud Foundry](https://docs.cloudfoundry.org/) 是一個在雲端提供者的 IaaS 平台上執行的 PaaS 平台。 它可以跨雲端提供者提供一致的應用程式部屬體驗。 它也可以整合使用各種 Azure 服務的詳細資訊，以企業級的 HA，延展性，並節省成本。
有 [6 個 Cloud Foundry 的子系統](https://docs.cloudfoundry.org/concepts/architecture/) \(英文\)，可有彈性地線上調整，包括：路由、 驗證，應用程式生命週期管理、 服務管理傳訊和監控。 針對每個子系統中，您可以設定要使用對應的 Azure 服務的 Cloud Foundry。 
...
Bosh 使用 Azure CPI （雲端提供者介面） 的磁碟建立和刪除的常式。 預設會使用非受控磁碟。 客戶必須手動建立儲存體帳戶，然後在 CF 資訊清單檔中設定那些帳戶。 這是因為每個儲存體帳戶的磁碟數目的限制。
現在有[受控磁碟](https://azure.microsoft.com/services/managed-disks/)可供使用，可為虛擬機器提供受控的安全可靠磁碟儲存體。 客戶不再需要處理儲存體帳戶來調整規模及達到 HA。 Azure 會自動安排磁碟。 無論是新的或現有的部署，Azure CPI 會處理建立或在 CF 部署期間移轉的受控磁碟。 它支援使用 PCF 1.11。 您也可以探索開放原始碼的 Cloud Foundry [受控磁碟指引](https://github.com/cloudfoundry-incubator/bosh-azure-cpi-release/tree/master/docs/advanced/managed-disks) \(英文)\ 來參考相關資訊。 
...
預設會使用 Azure 基本負載平衡器來處理連入 CF API/應用程式要求，將它們轉送給 GoRouters。 Diego Brain、MySQL、ERT 等 CF 元件也可以使用負載平衡器來平衡流量負載，以達到 HA。 Azure 也提供一組完全受控的負載平衡解決方案。 如果您要尋找 TLS 終止 （「 SSL 卸載 」），或每個 HTTP/HTTPS 要求的應用程式層處理，請考慮應用程式閘道。 針對第 4 層的高可用性和延展性負載平衡，請考慮使用標準負載平衡器。
...
Azure Log Analytics Nozzle 是 Cloud Foundry 元件，可將計量從[Cloud Foundry loggregator firehose](https://docs.cloudfoundry.org/loggregator/architecture.html)要[Azure 監視器記錄](https://azure.microsoft.com/services/log-analytics/)。 在 Nozzle 的協助之下，您可以收集、檢視及分析多個部署的 CF 系統健康情況和效能計量。
按一下 [此處](https://docs.microsoft.com/azure/cloudfoundry/cloudfoundry-oms-nozzle)若要了解如何將 Azure Log Analytics Nozzle 部署到開放原始碼和 Pivotal Cloud Foundry 環境，並接著存取資料，從 Azure 監視器記錄主控台。 
> [!NOTE]
> PCF 2.0 中，從 Vm 的 BOSH 健康情況計量會轉送給 Loggregator Firehose 預設和已整合到 Azure 監視器記錄主控台。
...
雖然通常針對 Pivotal Cloud Foundry 生產環境建議使用 F 和 D VM 系列，但新的「高載」[B 系列](https://azure.microsoft.com/blog/introducing-b-series-our-new-burstable-vm-size/)帶來了新選項。 B 系列高載 Vm 非常適合不需要完整 CPU 效能的持續，例如 web 伺服器、 小型資料庫和開發與測試環境的工作負載。 這些工作負載通常具有高載的效能需求。 相較於 $0.05/小時 (F1)，它是 $0.012/小時 (B1)，如需詳細資料，請參閱 [VM 大小](https://docs.microsoft.com/azure/virtual-machines/linux/sizes-general)和[價格](https://azure.microsoft.com/pricing/details/virtual-machines/linux/)。 
#### <a name="managed-standard-disk"></a>受控標準磁碟： 
若要確保在生產環境中獲得可靠的效能，建議使用進階磁碟。  使用[受控磁碟](https://azure.microsoft.com/services/managed-disks/)時，標準儲存體也可以提供類似的可靠性，但效能不同。 不能是重視效能的工作負載，例如開發/測試或非關鍵的環境中，受管理的標準磁碟提供更低的成本與替代選項。  
...
#### <a name="use-azure-first-party-services"></a>使用 Azure 的第一個合作對象服務： 
...
第一次都可使用 azure 的整合功能[開放來源 Cloud Foundry](https://github.com/cloudfoundry-incubator/bosh-azure-cpi-release/tree/master/docs/advanced/)，才能在 Pivotal Cloud Foundry 上。 標示 * 的功能表示仍無法透過 PCF 取得。 不是在本文中涵蓋搭配 Azure Stack 的 cloud Foundry 整合。
