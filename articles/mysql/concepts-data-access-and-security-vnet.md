...
> [!NOTE]
> 針對適用於 MySQL 的 Azure 資料庫，這個功能已於已部署適用於 MySQL 的 Azure 資料庫的所有 Azure 公用雲端區域中，以公開預覽的形式提供。
...
不過，截至 2018 年 5 月，適用於 MySQL 的 Azure 資料庫服務還不是可直接指派給子網路的服務。
...
- 在公開預覽期間，不支援 VNet 移動作業。 若要移動虛擬網路規則，請先捨棄然後再重新建立該規則。
