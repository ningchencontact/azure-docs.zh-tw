...
1. 在 [Azure 入口網站](https://portal.azure.com/)中，選取您想要監視的「適用於 MySQL 的 Azure 資料庫」伺服器。
...
4. [新增規則] 頁面隨即開啟，如下所示。  填寫必要資訊：
   ...
   | 警示在 | 針對這類警示，請選擇 [計量]。 |
   | 訂用帳戶 | 此欄位會預先填入裝載「適用於 MySQL 的 Azure 資料庫」的訂用帳戶。 |
   | 資源群組 | 此欄位會預先填入「適用於 MySQL 的 Azure 資料庫」的資源群組。 |
   | 資源 | 此欄位會預先填入「適用於 MySQL 的 Azure 資料庫」的名稱。 |
