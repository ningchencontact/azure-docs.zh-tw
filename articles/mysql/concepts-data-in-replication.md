---
title: 資料複寫-適用於 MySQL 的 Azure 資料庫
...
- 如果主伺服器已啟用 SSL，請確定為網域提供的 SSL CA 憑證已包含在 `mysql.az_replication_change_master` 預存程式中。 請參閱下列[範例](https://docs.microsoft.com/azure/mysql/howto-data-in-replication#link-master-and-replica-servers-to-start-data-in-replication)和 `master_ssl_ca` 參數。
