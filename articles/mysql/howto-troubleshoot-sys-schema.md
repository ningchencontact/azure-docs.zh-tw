...
MySQL performance_schema 最先是在 MySQL 5.5 中導入的，其檢測功能適用於多種重要的伺服器資源，例如記憶體配置、預存程式、中繼資料鎖定等。不過，performance_schema 包含超過 80 個資料表，且通常需要聯結 performance_schema 內的資料表以及 information_schema 中的資料表，才能取得所需的資訊。 Sys_schema 是以 performance_schema 與 information_schema 為基礎而建置的，它提供了功能強大、適用於唯讀資料庫的[易用檢視](https://dev.mysql.com/doc/refman/5.7/en/sys-schema-views.html)集合，並且可在「適用於 MySQL 的 Azure 資料庫」5.7 版中完整啟用。

![sys_schema 的檢視](./media/howto-troubleshoot-sys-schema/sys-schema-views.png)

sys_schema 中有 52 個檢視，每個檢視分別具有下列其中一個前置詞：

- Host_summary 或 IO：I/O 相關延遲。
- InnoDB：InnoDB 緩衝區狀態和鎖定。
- 記憶體：依主機和使用者分類的記憶體使用量。
- 結構描述：結構描述相關資訊，例如自動增量、索引等等。
- 陳述式：SQL 陳述式的相關資訊；這可以是導致完整資料表掃描或長時間查詢的陳述式。
- 使用者：使用者所使用的資源 (依使用者分類)。 範例包括檔案 I/O、連線和記憶體。
- 等候：依主機或使用者分類的等候事件。
...
由於適用於 MySQL 的 Azure 資料庫會調整儲存體方面的 IO，因此將我已佈建的儲存體增加到 1 TB 時，我的 IO 延遲會減少為 571 毫秒，這代表效能提高了 26 倍！

![IO 延遲：1TB](./media/howto-troubleshoot-sys-schema/io-latency-1TB.png)

### <a name="sysschematableswithfulltablescans"></a>*sys.schema_tables_with_full_table_scans*

儘管經過仔細規劃，許多查詢仍可能導致完整資料表掃描。 如需關於索引類型和如何加以最佳化的詳細資訊，您可以參閱這篇文章：[如何對查詢效能進行疑難排解](./howto-troubleshoot-query-performance.md)。 完整資料表掃描會耗用大量資源，並降低資料庫效能。 要透過完整資料表掃描來尋找資料表，最快的方式是查詢 *sys.schema_tables_with_full_table_scans* 檢視。
...
排解資料庫效能問題時，識別您的資料庫內發生的事件可能會有幫助，而使用 *sys.user_summary_by_statement_type* 檢視正可做到這一點。

![依陳述式分類的摘要](./media/howto-troubleshoot-sys-schema/summary-by-statement.png)

在此範例中，適用於 MySQL 的 Azure 資料庫花費了 53 分鐘將 slog 查詢記錄排清 44579 次。 這不僅耗時，也需要許多 IO。 您可以在 Azure 入口網站中減少這項活動，方法是停用慢速查詢記錄，或降低慢速查詢記錄的頻率。

## <a name="database-maintenance"></a>資料庫維護

### <a name="sysinnodbbufferstatsbytable"></a>*sys.innodb_buffer_stats_by_table*

InnoDB 緩衝集區存在於記憶體中，是 DBMS 與儲存體之間的主要快取機制。 InnoDB 緩衝集區的大小與效能層緊密連結，且除非選擇了不同的產品 SKU，否則無法變更。 如同您作業系統中的記憶體，舊的頁面會遭到替換，以騰出空間給較新的資料。 若要了解哪些資料表耗用了大部分的 InnoDB 緩衝集區記憶體，您可以查詢 *sys.innodb_buffer_stats_by_table* 檢視。

![InnoDB 緩衝集區](./media/howto-troubleshoot-sys-schema/innodb-buffer-status.png)

從上圖可以明顯看出，除了系統資料表和檢視以外，mysqldatabase033 資料庫中的每個資料表 (分別裝載其中一個「我的 WordPress」網站) 記憶體中佔用了 16 KB (或 1 頁) 的資料。

### <a name="sysschemaunusedindexes--sysschemaredundantindexes"></a>*Sys.schema_unused_indexes* & *sys.schema_redundant_indexes*

索引是提升讀取效能的理想工具，但會造成插入和儲存方面的額外成本。 *Sys.schema_unused_indexes* 和 *sys.schema_redundant_indexes* 可讓您深入了解未使用或重複的索引。
...
總之，sys_schema 對效能調整和資料庫維護而言，都是很理想的工具。 請務必在您適用於 MySQL 的 Azure 資料庫中充分運用這項功能。 
