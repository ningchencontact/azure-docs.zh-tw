...
3. 在「適用於 MySQL 的 Azure 資料庫」表單中填寫下列資訊︰
    ...
    | *密碼* | 設定管理帳戶密碼 |
    | *確認密碼* | 確認管理帳戶密碼 |
    | *Location* | 東南亞 (選取 [北歐] 或 [美國西部]) |
    | *版本* | 5.7 (選擇適用於 MySQL 的 Azure 資料庫伺服器版本) |

4. 按一下 [定價層]  指定新資料庫的服務層級和效能等級。 選取 [一般用途]  索引標籤。Gen 5  、2 個虛擬核心  、5 GB  和 7 天  是**計算世代**、**虛擬核心**、**儲存體**和**備份保留期限**的預設值。 您可以讓這些滑桿保留原狀。 若要啟用異地備援儲存體中的伺服器備份，請從 [備份備援選項]  中選取 [異地備援]  。
