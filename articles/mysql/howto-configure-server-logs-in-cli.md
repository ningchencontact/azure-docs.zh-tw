...
1. 開啟慢速查詢記錄設定**緩慢\_查詢\_記錄**參數設為 ON。
...
若要列出可用的慢速查詢記錄檔以取得您的伺服器，請執行[az mysql server 記錄檔清單](/cli/azure/mysql/server-logs#az-mysql-server-logs-list)命令。
...
- 深入了解[適用於 MySQL 慢速查詢記錄中的 Azure 資料庫](concepts-server-logs.md)。
