...
---
# <a name="tutorial-provision-an-azure-database-for-mysql-server-using-azure-resource-manager-templates"></a>教學課程：使用 Azure Resource Manager 範本佈建「適用於 MySQL 的 Azure 資料庫」伺服器
...
若要取得「適用於 MySQL 的 Azure 資料庫」伺服器的 JSON 範本參考，請移至 Microsoft.DBforMySQL 伺服器範本參考 (https://docs.microsoft.com/en-us/azure/templates/microsoft.dbformysql/servers)。 下列範例 JSON 範本可用來建立在 VNet 服務端點上執行「適用於 MySQL 的 Azure 資料庫」的新伺服器。
```json
{
