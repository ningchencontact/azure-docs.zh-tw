...
* 如果應用程式必須超出其中一個延展性目標，您應建立多個儲存體帳戶，並將您的應用程式資料在這幾個儲存體帳戶中進行分割。 如果您要使用此模式，那麼請確定設計您的應用程式，以便日後可以增加更多儲存體帳戶以取得負載平衡。 在撰寫本文時，每個 Azure 訂用帳戶可以有最多 250 個儲存體帳戶，每個區域 （當部署與 Azure Resource Manager 模型）。  除了儲存資料、進行交易或傳輸資料等使用方式外，儲存體帳戶也不會有其他費用。
...
### <a name="subheading47"></a>磁碟分割命名慣例

Azure 儲存體使用範圍型的資料分割配置，調整和負載平衡系統。 （帳戶 + 容器與 blob） 資料分割索引鍵用來分割成數個範圍，這些範圍的資料是負載平衡系統。 這表示命名慣例，如語彙順序 (例如*mypayroll*， *myperformance*， *myemployees*等) 或使用時間戳記 (*log20160101*， *log20160102*， *log20160102*等) 將適用於正在可能共置於相同的資料分割伺服器，直到資料分割負載平衡作業會將它們出分割成較小的範圍。 比方說，在容器內的所有 blob 會都提供由單一伺服器上，直到這些 blob 的負載需要進一步重新平衡分割區範圍。 同樣地，一群名稱依語彙順序排列的少量負載帳戶可能接受單一伺服器的服務，直到其中一個或所有帳戶的負載要求它們分割到多個分割區伺服器。 每個負載平衡作業都可能在作業期間影響儲存體呼叫的延遲。 處理某個分割區流量突然暴增的系統的能力會受到單一分割區伺服器的延展性，直到負載平衡作業會重新平衡分割區索引鍵範圍。

您可以遵循一些最佳做法來降低這類作業的頻率。  

* 可能的話，請使用較大 Put Blob 或放置區塊大小 （大於 4 Mb 的標準帳戶和大於 256 KiB premium 帳戶） 來啟用高輸送量區塊 Blob (HTBB)。 HTBB 提供最高的效能內嵌不受影響的分割區命名。
* 詳細檢查帳戶、容器、Blob、資料表和佇列所使用的命名慣例。 請考慮前面加上 3 位數雜湊，使用最適合您需求的雜湊函式的 帳戶、 容器或 blob 名稱。  
* 如果使用時間戳記或數字識別碼組織資料，您必須確定您使用的不是僅附加在後 (或僅在前面加上) 的流量模式。 這些模式並不適合範圍型的分割區系統，而且可能導致所有流量進入單一分割區並限制系統進行有效的負載平衡。 比方說，如果您有每日作業，例如使用加上時間戳記的 blob 物件*yyyymmdd*，則該日常作業的所有流量會都導向至單一物件，都會由單一資料分割伺服器提供服務。 請查看每個 Blob 的限制和每個分割區的限制是否符合您的需求，考慮是否需要將這項作業拆分成多個 Blob。 同樣地，如果在資料表中儲存時間序列資料，則所有流量都可能導向到索引鍵命名空間的最後部分。 如果您必須使用時間戳記或數字識別碼，識別碼前面與 3 位數雜湊，或在時間戳記前置時間的秒數部分例如*ssyyyymmdd*。 如果定期執行列出和查詢作業，請選擇會限制查詢次數的雜湊函數。 其他情況，隨機的前置詞即足以應付。  
* 如需其他有關使用 Azure 儲存體中的資料分割配置的詳細資訊，請參閱[Azure 儲存體：具有高度一致性的高可用性雲端儲存體服務](https://sigops.org/sosp/sosp11/current/2011-Cascais/printable/11-calder.pdf) \(英文\)。

### <a name="networking"></a>網路功能

雖然 API 呼叫非常重要，但應用程式的實體網路限制經常會對效能產生重大影響。 下列說明了使用者可能會遇到的部分限制。  

#### <a name="client-network-capability"></a>用戶端的網路功能
...
#### <a name="subheading10"></a>增加執行緒的最小數目，如果使用同步程式碼搭配非同步工作

此程式碼會增加執行緒集區的執行緒數目下限：  
...
#### <a name="subheading11"></a>利用.NET 4.5 和較高的回收
...
#### <a name="subheading14"></a>節流和伺服器忙線錯誤
...
除了已經實證做法以外[所有服務](#allservices)先前所述，下列已經實證的做法尤其適用於 Blob 服務。  
...
#### <a name="subheading16"></a>頻寬及每個 blob 的作業
...
* Node.js：對要求選項或在 Blob 服務，請使用 parallelOperationThreadCount。
...
#### <a name="entities-per-second-storage-account"></a>每秒 （儲存體帳戶） 的實體
...
#### <a name="entities-per-second-partition"></a>每秒 （分割區） 的實體
...
##### <a name="limiting-the-amount-of-data-returned"></a>將傳回限制的資料量
...
您可以使用**更新訊息**作業來增加隱藏逾時或更新訊息的狀態資訊。 雖然這是功能強大，請記住，每個**更新訊息**作業計數算在延展性目標。 不過，相較於在工作的每個階段完成時，將工作從一個佇列傳遞到下一個佇列的工作流程，這會更有效率。 使用**更新訊息**作業可讓您的應用程式將工作狀態儲存到訊息，然後繼續工作，而不是每次步驟完成 requeuing 作業的下一個步驟的訊息。  
