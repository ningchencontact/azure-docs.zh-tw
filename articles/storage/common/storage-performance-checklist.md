...
每個使用 Azure 儲存體的應用程式開發人員都應該花時間閱讀這篇文章, 並檢查其應用程式是否遵循下列每個經過證實的作法。  
...
#### <a name="sub1bandwidth"></a>所有服務的頻寬擴充性目標
...
* 如果應用程式必須超出其中一個延展性目標，您應建立多個儲存體帳戶，並將您的應用程式資料在這幾個儲存體帳戶中進行分割。 如果您要使用此模式，那麼請確定設計您的應用程式，以便日後可以增加更多儲存體帳戶以取得負載平衡。 在撰寫本文時, 每個區域的每個 Azure 訂用帳戶最多可以有250個儲存體帳戶 (部署時使用 Azure Resource Manager 模型)。  除了儲存資料、進行交易或傳輸資料等使用方式外，儲存體帳戶也不會有其他費用。
...
Azure 儲存體使用範圍型的資料分割配置，調整和負載平衡系統。 分割區索引鍵 (account + container + blob) 是用來將資料分割成多個範圍, 而這些範圍會在系統間進行負載平衡。 這表示命名慣例 (例如, *mypayroll*、 *myperformance*、 *myemployees*等) 或使用時間戳 (*log20160101*、 *log20160102*、 *log20160102*等) 將會出借其本身可能會共置於相同的分割區伺服器上, 直到負載平衡作業將它們分割成較小的範圍。 例如, 容器內的所有 blob 都可以由單一伺服器提供服務, 直到這些 blob 上的負載需要進一步重新平衡分割區範圍為止。 同樣地，一群名稱依語彙順序排列的少量負載帳戶可能接受單一伺服器的服務，直到其中一個或所有帳戶的負載要求它們分割到多個分割區伺服器。 每個負載平衡作業都可能在作業期間影響儲存體呼叫的延遲。 系統處理資料分割流量突然激增的能力, 會受到單一分割區伺服器的擴充性限制, 直到負載平衡作業開始進行並重新平衡資料分割索引鍵範圍為止。

您可以遵循一些最佳做法來降低這類作業的頻率。  

* 可能的話, 請使用較大的 Put Blob 或 Put 區塊大小 (適用于標準帳戶, 大於 4 MiB, premium 帳戶則為大於 256 KiB) 來啟動高輸送量區塊 Blob (HTBB)。 HTBB 提供不受磁碟分割命名影響的高效能內嵌。
* 詳細檢查帳戶、容器、Blob、資料表和佇列所使用的命名慣例。 請考慮使用最符合您需求的雜湊函數, 在帳戶、容器或 blob 名稱前面加上3位數的雜湊。  
* 如果使用時間戳記或數字識別碼組織資料，您必須確定您使用的不是僅附加在後 (或僅在前面加上) 的流量模式。 這些模式並不適合範圍型的分割區系統，而且可能導致所有流量進入單一分割區並限制系統進行有效的負載平衡。 比方說, 如果您的日常作業使用具有時間戳記的 blob 物件 (例如*yyyymmdd*), 則該每日作業的所有流量都會導向至單一資料分割伺服器提供的單一物件。 請查看每個 Blob 的限制和每個分割區的限制是否符合您的需求，考慮是否需要將這項作業拆分成多個 Blob。 同樣地，如果在資料表中儲存時間序列資料，則所有流量都可能導向到索引鍵命名空間的最後部分。 如果您必須使用時間戳或數位識別碼, 請在識別碼前面加上3位數雜湊, 或在時間戳記前面加上*ssyyyymmdd*的秒數部分。 如果定期執行列出和查詢作業，請選擇會限制查詢次數的雜湊函數。 其他情況，隨機的前置詞即足以應付。  
* 如需 Azure 儲存體中使用之資料分割配置的詳細資訊[, 請參閱 Azure 儲存體:具有高度一致性的高可用性雲端儲存體服務](https://sigops.org/sosp/sosp11/current/2011-Cascais/printable/11-calder.pdf) \(英文\)。
...
如需關於 SAS 的詳細資訊，請參閱[使用共用存取簽章 (SAS) 對 Azure 儲存體資源授與有限存取權](storage-sas-overview.md)。  
...
如需條件式下載的詳細資訊, 請參閱[指定 Blob 服務作業的條件式標頭](/rest/api/storageservices/specifying-conditional-headers-for-blob-service-operations)。  
...
#### <a name="subheading10"></a>如果使用同步程式碼搭配非同步工作, 則增加執行緒的最小數目

此程式碼會增加執行緒集區中線程的最小數目:  
...
#### <a name="subheading11"></a>利用 .NET 4.5 和更高的垃圾收集
...
### <a name="subheading12"></a>未系結的平行處理
...
除了先前所述之[所有服務](#allservices)的已證明做法以外, 下列已證實的作法特別適用于 Blob 服務。  

### <a name="blob-specific-scalability-targets"></a>Blob 特定的擴充性目標
...
Blob 服務支援 head 要求, 其中可能包含有關 Blob 的中繼資料。 例如，如果您的應用程式需要相片的 EXIF 資料，它可以擷取相片並擷取該資料。 若要節省頻寬並改善效能，當應用程式上傳相片時，您的應用程式可以在 Blob 的中繼資料中儲存 EXIF 資料：您即可僅使用一個 HEAD 要求來擷取中繼資料中的 EXIF 資料，並節省大量頻寬以及每次讀取 Blob 時擷取 EXIF 資料所需的處理時間。 這在您僅需要中繼資料而非 Blob 完整內容的案例中，此操作會很有幫助。  每個 Blob 僅可儲存 8 KB 的中繼資料 (此服務不接受超過此值的儲存要求)，因此如果資料不符合該大小，您就可能無法使用此方法。  

### <a name="rapid-uploading"></a>快速上傳

若要快速上傳 blob, 第一個要回答的問題是: 您上傳一個 blob 或多個 blob 嗎？  使用下列指引，根據您的案例來判斷所要使用的正確方法。  
...
* Node.js：請在要求選項或 Blob 服務上使用 parallelOperationThreadCount。
* C++：使用 blob_request_options::set_parallelism_factor 方法。

#### <a name="subheading22"></a>快速上傳多個 Blob

若要快速上傳多個 Blob，請平行上傳 Blob。 因為這個方法會將上傳散佈到儲存體服務的多個資料分割，因此會比搭配平行區塊上傳一次上傳單一 Blob 的方法要快。 單一 Blob 僅支援 60 MB/秒 (大約是 480 Mbps) 的輸送量。 在撰寫本文時, 美國 LRS 帳戶最多支援 20 Gbps 的輸入, 這遠高於個別 blob 所支援的輸送量。  [AzCopy](#subheading18) 依預設會執行平行上傳，在此案例中我們建議使用此方法。  
...
### <a name="subheading24"></a>資料表特定的擴充性目標
...
#### <a name="entities-per-second-storage-account"></a>每秒實體數 (儲存體帳戶)
...
#### <a name="entities-per-second-partition"></a>每秒實體數 (分割區)
...
#### <a name="subheading26"></a>停用 Nagle
...
點查詢只會擷取一個實體。 做法是指定要擷取的實體分割索引鍵和資料列索引鍵。 這些查詢很有效率，您應盡可能地加以使用。  

###### <a name="partition-queries"></a>分割區查詢
...
##### <a name="subheading37"></a>將資料數列儲存在單一實體中
...
或者，您的應用程式可以儲存每小時 CPU 使用量做為單一實體的個別屬性：若要每小時更新，您的應用程式可以使用單一 **InsertOrMerge Upsert** 呼叫來更新最近一小時的值。 若要繪製資料, 應用程式只需要抓取單一實體, 而不是 24, 以提供有效率的查詢 (請參閱上面有關[查詢範圍](#subheading30)的討論)。
...
### <a name="subheading40"></a>停用 Nagle

請參閱討論 Nagle 演算法的資料表組態一節 — Nagle 演算法通常對佇列要求的效能有負面影響，您應將它停用。  

### <a name="subheading41"></a>訊息大小

佇列效能和擴充性會隨著訊息大小的增加而降低。 您應該只將接收端所需的資訊放在訊息中。  

### <a name="subheading42"></a>批次抓取
...
### <a name="subheading44"></a>更新訊息

您可以使用**更新訊息**作業來增加隱藏超時, 或更新訊息的狀態資訊。 雖然這項功能很強大, 但請記住, 每個**更新訊息**作業都會計入擴充性目標。 不過，相較於在工作的每個階段完成時，將工作從一個佇列傳遞到下一個佇列的工作流程，這會更有效率。 使用**更新訊息**作業可讓您的應用程式將工作狀態儲存到訊息, 然後繼續工作, 而不是在每次步驟完成時 requeuing 作業的下一個步驟的訊息。  
