...
如果您沒有金鑰保存庫，您可以從入口網站、 Powershell 或 CLI 來建立它：
...
如果您使用現有的 key vault，您需要在您的保存庫啟用虛刪除 」 和 「 不要清除，藉由執行下列 PowerShell 或 Azure CLI 命令：
...
您必須具有該金鑰保存庫的金鑰保存庫和一個索引鍵。 若要從金鑰保存庫指定您的金鑰，請依照下列步驟進行操作：
