...
例如，假設您的客戶成功提交更新，但是在更新傳播到次要區域之前主要區域會失敗。 當客戶要求讀回資料時，它們會接收到過時的資料，從次要地區，而不是更新的資料。 設計您的應用程式時，您必須決定是否可接受此情況，如果可以，您該如何傳達給客戶。 
...
RA-GRS 的運作方式是將交易從主要區域複寫到次要區域。 此複寫程序可保證次要區域中的資料是*最終一致*的。 這表示，主要區域中的所有交易最終都會出現在次要區域中，但有可能會延遲出現，而且不保證交易會以原本在主要區域中套用它們的相同順序到達次要區域。 如果您的交易不按順序到達次要區域，則您可能  要考慮讓次要區域中的資料處於不一致狀態，直到服務更新為止。

下表顯示範例的更新，使其成員的員工的詳細資料時，可能會發生什麼情況*系統管理員*角色。 基於此範例，這會要求您更新**員工**實體，並利用系統管理員總數的計數來更新**系統管理員角色**實體。 請注意，如何在次要區域中不按順序套用更新。
...
| T3       | 交易 C：<br> Update <br>administrator<br>角色實體，位於<br>primary |                    | T1                 | 交易 C 已寫入主要區域，<br> 但尚未複寫。  |
