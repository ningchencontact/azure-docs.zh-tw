...
區域備援儲存體 (ZRS) (預覽) 設計為可簡化高可用性應用程式的開發。 ZRS 提供儲存體物件在指定一年中至少 99.9999999999% (12 個 9) 的持久性。 ZRS 以同步方式將您的資料複寫到多個可用性區域。 針對不接受停機時間的情況，例如交易式應用程式，請考慮 ZRS。

即使單一區域無法使用或無法修復，ZRS 讓客戶仍然能夠讀取和寫入資料。 插入和更新資料會以同步方式進行，一致性極強。   

目前在下列區域提供 ZRS 預覽，其他區域也會陸續推出：

- 美國東部 2 
- 美國中部 
- 法國中部 (此區域目前為預覽狀態。 請參閱[目前法國提供 Microsoft Azure 預覽的 Azure 可用性區域](https://azure.microsoft.com/blog/microsoft-azure-preview-with-azure-availability-zones-now-open-in-france)以要求使用。)

### <a name="zrs-classic-accounts"></a>傳統 ZRS 帳戶

現有的 ZRS 功能是指傳統 ZRS。 傳統 ZRS 帳戶僅適用於一般用途 (general-purpose V1) 儲存體帳戶中的區塊 Blob。 

傳統 ZRS 是以非同步方式在一至兩個區域內的資料中心複寫資料。 除非 Microsoft 開始容錯移轉至次要區域，否則可能無法使用複本。 
...
ZRS 帳戶支援手動將傳統 ZRS 帳戶與該區域中的 LRS、GRS 或 RA-GRS 帳戶互相移轉。 您可以使用 AzCopy、Azure 儲存體總管、Azure PowerShell、Azure CLI 或 Azure 儲存體用戶端程式庫之一，執行這項手動移轉。
