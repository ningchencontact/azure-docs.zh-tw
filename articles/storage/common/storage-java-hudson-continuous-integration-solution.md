...
在本教學課程中，您將使用 Microsoft 提供之 Hudson CI 的 Azure 儲存體外掛程式。
...
* Hudson 連續整合解決方案。
  ...
  5. 完成初始設定之後，取消執行中的 Hudson WAR 實例，再次啟動 Hudson WAR，然後重新開啟 Hudson 儀表板 `http://localhost:8080/`，您將用來安裝和設定 Azure 儲存體外掛程式。
...
3. 在 [Microsoft Azure Storage Account Configuration] 區段中：
    ...
    a. 輸入您的儲存體帳戶名稱，您可以從[Azure 入口網站](https://portal.azure.com)取得。
   
    b.這是另一個 C# 主控台應用程式。 輸入您的儲存體帳戶金鑰，也可從[Azure 入口網站](https://portal.azure.com)中獲得。
   
    c. 如果您使用全域 Azure 雲端，請讓 [Blob 服務端點 URL] 使用預設值。 如果您使用不同的 Azure 雲端，請使用儲存體帳戶的[Azure 入口網站](https://portal.azure.com)中指定的端點。
...
7. 在 [容器名稱] 中，指定容器名稱。 （如果上傳組建成品時，容器不存在，則會建立容器）。您可以使用環境變數，因此在此範例中，請輸入 **$ {JOB_NAME}** 作為容器名稱。
...
每個工作只能建立一個將成品上傳至 Azure Blob 儲存體的建置後動作。 將成品上傳至 Azure Blob 儲存體的單一組建後動作，可以指定不同的檔案（包含萬用字元）和要**上傳之成品清單內的**檔案路徑，並使用分號作為分隔符號。 例如，若 Hudson 組建在您工作區的 **build** 資料夾中產生 JAR 檔和 TXT 檔，且您想將兩者都上傳至 Azure Blob 儲存體，請在 [要上傳的成品清單] 中使用下列值：**build/\*.jar;build/\*.txt**。 您也可以使用雙冒號語法來指定要在 Blob 名稱內使用的路徑。 例如，若您想要在 Blob 路徑中使用 **binaries** 上傳 JAR，並在 Blob 路徑中使用 **notices** 上傳 TXT，請在 **[List of Artifacts to upload]** 中使用下列值：**build/\*.jar::binaries;build/\*.txt::notices**。
