...
#### <a name="status-code-404-not-found-examples"></a>狀態碼404（找不到）範例
...
#### <a name="status-code-409-conflict-examples"></a>狀態碼409（衝突）範例
...
#### <a name="status-code-412-precondition-failed-examples"></a>狀態碼412（先決條件失敗）範例
...
首先，我們需要設定 Azure 儲存體記錄和計量，讓我們有服務端的資料可進行分析。 您可以用各種方式設定記錄和計量 - 透過 [Azure 入口網站](https://portal.azure.com)，或使用 PowerShell，或以程式設計的方式。 如需設定記錄和計量的詳細資訊，請參閱[啟用計量](storage-analytics-metrics.md#enable-metrics-using-the-azure-portal)和[啟用記錄](storage-analytics-logging.md#enable-storage-logging)。
...
## <a name="use-azcopy-to-copy-server-logs-to-a-local-directory"></a>使用 AzCopy 將伺服器記錄複製到本機目錄
...
| PercentTimeoutError 增加 (全部) |*StatusCode   == 500 |所有 |
