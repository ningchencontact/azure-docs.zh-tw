...
- **Azure Active Directory (Azure AD) 整合**適用於 blob 和佇列。 Azure AD 提供了角色型存取控制 (RBAC)，供您更細微地控制用戶端對儲存體帳戶資源的存取。 如需詳細資訊，請參閱 <<c0> [ 驗證對 Azure 儲存體使用 Azure Active Directory 的要求](storage-auth-aad.md)。
