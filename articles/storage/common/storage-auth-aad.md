---
title: 授權存取 Azure blob 和佇列使用 Azure Active Directory |Microsoft Docs
description: 授與 Azure blob 和佇列使用 Azure Active Directory 存取權。
services: storage
author: tamram
ms.service: storage
ms.topic: article
...
ms.locfileid: "67302423"
---
# <a name="authorize-access-to-azure-blobs-and-queues-using-azure-active-directory"></a>授權存取 Azure blob 和佇列使用 Azure Active Directory

使用 Azure Active Directory (AD) 授權要求 Blob 和佇列儲存體的 azure 儲存體支援。 使用 Azure AD，您可以使用角色型存取控制 (RBAC) 權限授與安全性主體，可能是使用者、 群組或應用程式服務主體。 Azure AD 傳回 OAuth 2.0 權杖來驗證的安全性主體。 語彙基元可用來授權存取在 Blob 或佇列儲存體資源要求。

授權使用者或應用程式使用由 Azure AD 的 OAuth 2.0 權杖共用金鑰驗證透過提供較高的安全性和易用性，以及共用存取簽章 (SAS)。 與 Azure AD 中，則不需要儲存帳戶存取金鑰與您的程式碼和風險潛在的安全性弱點。 雖然您可以繼續使用共用金鑰授權於應用程式，但使用 Azure AD 就不需要將帳戶存取金鑰和程式碼一起儲存。 您也可以繼續使用共用存取簽章 (SAS) 將細部存取權授與儲存體帳戶中的資源，但 Azure AD 提供類似功能，卻不必管理 SAS 權杖或擔心需要撤銷遭盜用的 SAS。 Microsoft 建議您 Azure 儲存體的應用程式時可能使用 Azure AD 授權。

使用 Azure AD 進行授權可供所有一般用途和 Blob 儲存體帳戶的所有公用區域和國家/地區雲端。 只有儲存體帳戶使用 Azure Resource Manager 部署模型支援建立 Azure AD 授權。
...
原生應用程式和 Azure Blob 或佇列服務提出要求的 web 應用程式可以也會授與 Azure AD 的存取權。 若要了解如何要求存取權杖，並使用它來授權要求的 blob 或佇列的資料，請參閱[到 Azure 儲存體與 Azure AD 授權存取，從 Azure 儲存體應用程式](storage-auth-aad-app.md)。
...
如需有關如何為 Azure 儲存體定義內建角色的詳細資訊，請參閱[了解角色定義](../../role-based-access-control/role-definitions.md#management-and-data-operations)。 建立自訂的 RBAC 角色的詳細資訊，請參閱[建立 azure 角色型存取控制的自訂角色](../../role-based-access-control/custom-roles.md)。
...
存取 blob 或佇列的資料，透過 Azure 入口網站、 PowerShell 或 Azure CLI 可授權，使用使用者的 Azure AD 帳戶，或使用的帳戶存取金鑰 （共用金鑰的授權）。
...
## <a name="azure-ad-authorization-over-smb-for-azure-files"></a>Azure AD 授權透過 SMB 的 Azure 檔案

Azure 檔案服務支援使用 Azure AD 進行授權透過 SMB 的已加入網域的僅限 Vm （預覽）。 若要深入了解 Azure 檔案，使用透過 SMB 的 Azure AD，請參閱[適用於 Azure 檔案 （預覽） 透過 SMB 的概觀的 Azure Active Directory 授權](../files/storage-files-active-directory-overview.md)。

## <a name="next-steps"></a>後續步驟

- [授權適用於 Azure 資源的存取權的 blob 和佇列與 Azure Active Directory 與受管理的身分識別](storage-auth-aad-msi.md)
