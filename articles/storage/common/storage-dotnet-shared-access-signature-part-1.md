...
* Azure 存储接受 SAS 的可选的 IP 地址或 IP 地址范围。 例如，您可以指定屬於組織的 IP 位址範圍。
...
證明 SAS 非常有用的一個常見案例，就是使用者在您的儲存體帳戶中讀取和寫入自己的資料。 在存储帐户存储用户数据的情形中，有两种典型的设计模式：
...
1. 輕量型服務可視需要驗證用戶端，然後產生 SAS。 在客户端接收 SAS 后，它们可以直接使用 SAS 定义的权限并且针对 SAS 允许的间隔访问存储帐户资源。 SAS 可減輕透過前端 Proxy 服務路由所有資料的需求。
...
* **服务 SAS。** 服务 SAS 只能委派对以下一个存储服务中的资源的访问权限：Blob、队列、表或文件服务。 如需有關建構服務 SAS 權杖的深入資訊，請參閱[建構服務 SAS](https://msdn.microsoft.com/library/dn140255.aspx) 和[服務 SAS 範例](https://msdn.microsoft.com/library/dn140256.aspx)。
...
SAS 權杖是*用戶端*所產生的字串 (如需程式碼範例，請參閱〈[SAS 範例](#sas-examples)〉一節)。 譬如 Azure 儲存體不會以任何方式，追蹤您透過儲存體用戶端程式庫所產生的 SAS 權杖。 可以在客户端上创建不限数量的 SAS 令牌。
...
* **权限。**  在 SAS 上指定的權限表示用戶端可以使用 SAS 來對儲存體資源執行哪些作業。 帳戶 SAS 和服務 SAS 的可用權限不同。
...
* **通訊協定。** 一个可选参数，它指定请求允许的协议。 可能的值為 HTTPS 和 HTTP (`https,http`)，其為預設值或僅限 HTTPS (`https`)。 請注意，僅 HTTP 是不允許的值。
...
| 名稱 | SAS 部分 | 描述 |
| --- | --- | --- |
| Blob URI |`https://myaccount.blob.core.windows.net/sascontainer/sasblob.txt` |Blob 的地址。 請注意，我們強烈建議您使用 HTTPS。 |
...
* **含預存存取原則的 SAS：** 預存存取原則會在資源容器 (Blob 容器、資料表、佇列或檔案共用) 中定義，且可用來管理一或多個共用存取簽章的限制。 當您將 SAS 與預存存取原則建立關聯時，SAS 會繼承為該預存存取原則所定義的限制 (開始時間、過期時間和權限)。

> [!NOTE]
> 目前，帳戶 SAS 必須是臨機操作 SAS。 帳戶 SAS 尚不支援預存的存取原則。

这两种形式之间的差异对于一个关键情形而言十分重要：吊销。 SAS URI 為 URL，因此無論其原始建立者為何，任何人只要取得 SAS 即可自由使用。 如果 SAS 是公开发布的，则世界上的任何人都可以使用它。 除非發生下述四種情況之一，否則 SAS 會將資源存取權授與具有 SAS 的任何人：

1. 已到達 SAS 上指定的過期時間。
2. 达到了对该 SAS 引用的存储访问策略指定的到期时间（如果引用存储访问策略并且该存储访问策略指定一个到期时间）。 發生此情況的原因，有可能是因為已超過指定的間隔時間，或是因為您已修改預存存取原則，將過期時間設定為過去的日期，這是撤銷 SAS 的一種方法。
...
2. **尽可能参照存储访问策略。**  預存存取原則提供了撤銷權限且無需重新產生儲存體帳戶金鑰的選項。 將到期日設在未來 (或無限) 的日期，並確定定期更新到期日以將到期日再往未來的日期移動。
3. **对临时 SAS 使用近期的到期时间。** 如此一來，即使 SAS 遭到入侵，亦僅會造成短期影響。 如果您無法參考預存存取原則，此做法格外重要。 短期到期時間亦可協助限制可寫入 Blob 的資料量，方法是限制可對其上傳的可用時間。
4. **如果需要，让客户端自动续订 SAS。** 用戶端應在到期日之前就更新 SAS，以便如果提供 SAS 的服務無法使用的話，還有時間可以進行重試。 如果您打算將 SAS 用於少量的即時短期操作 (預計可在到期期限內完成的操作)，則此建議可能沒有必要，因為沒有更新 SAS 的打算。 不過，如果您有定期透過 SAS 做出要求的用戶端，則到期的可能性便有可能發生。 主要考量是要平衡下列兩個需求：短期的 SAS (如先前所述)，與確保用戶端提早要求更新以避免成功更新之前因 SAS 過期而中斷。
...
7. **了解您帳戶的任何方式將會被收取費用，包括以 SAS 方式完成的部分。**  如果您提供 Blob 的寫入存取權，則使用者可能會選擇上傳 200GB 的 Blob。 若您也同時提供使用者讀取存取權，則他們可能會選擇下載 10 次，而您便會產生 2TB 的出口成本。 再次強調，提供有限的權限有助於減少惡意使用者採取的潛在動作。 使用短期 SAS 以便减少这一威胁（但要注意结束时间上的时钟偏移）。
...
10. **使用儲存體分析來監視您的應用程式。**  您可以使用記錄和度量來觀察由於 SAS 提供者服務中斷或不小心移除預存存取原則，而造成的任何驗證失敗急劇增加。 如需額外資訊，請參閱 [Azure 儲存體團隊部落格](https://blogs.msdn.com/b/windowsazurestorage/archive/2011/08/03/windows-azure-storage-logging-using-logs-to-track-storage-requests.aspx) (英文)。
...
* [共享访问签名第 2 部分：透過 Blob 儲存體來建立與使用 SAS](../blobs/storage-dotnet-shared-access-signature-part-2.md)
...
* [資料表與佇列 SAS 簡介](https://blogs.msdn.com/b/windowsazurestorage/archive/2012/06/12/introducing-table-sas-shared-access-signature-queue-sas-and-update-to-blob-sas.aspx)
