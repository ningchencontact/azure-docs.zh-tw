...
* **異地複寫：** Azure 儲存體可以進行異地複寫。 雖然這樣可支援地理位置復原和資料備援，但容錯移轉至異地複寫的位置會嚴重影響效能，且可能產生額外的成本。 因此，建議您明智地選擇地理區域複寫，且最好在資料的價值大於額外成本時才這樣做。
* **資料生命週期管理：** 任何檔案系統中的所有資料都會經歷自己的生命週期，從非常重要且經常存取到較不重要、較少存取，然後到封存或刪除。 Azure 儲存體會提供資料階層處理和生命週期管理原則，將資料依其生命週期階段適當分層。
...
> [!NOTE]
> Data Lake Storage Gen2 的公開預覽期間僅支援 `--type LRS`。 在整個預覽計畫期間，將會提供其他的備援選項。
...
&lt;FileSystem&gt; 可識別檔案系統 Azure Data Lake Storage 的路徑。
&lt;AccountName&gt; 可識別 Azure 儲存體帳戶名稱。 需要使用完整網域名稱 (FQDN)。

如果 &lt;FileSystem&gt; 和 &lt;AccountName&gt; 的值皆未指定，則會使用預設檔案系統。 對於預設檔案系統上的檔案，您可以使用相對路徑或絕對路徑。 例如，可使用下列其中一個路徑來參考 HDInsight 叢集隨附的 hadoop-mapreduce-examples.jar 檔案：
