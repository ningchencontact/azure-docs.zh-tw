...
ms.locfileid: "51281552"
---
# <a name="quickstart-create-a-storage-account-for-analytics"></a>快速入門：建立用於分析的儲存體帳戶

已啟用 Data Lake Storage Gen2 預覽版的儲存體帳戶[支援階層式命名空間服務](introduction.md)，它可提供專門打造的原生目錄型檔案系統，與 Hadoop 分散式檔案系統 (HDFS) 搭配運作。 透過 [ABFS 驅動程式](abfs-driver.md)即可從 HDFS 存取 Data Lake Storage Gen2 資料。

若要在儲存體帳戶上啟用 Data Lake Storage Gen2 功能，請[填妥預覽版問卷來要求存取權](https://aka.ms/adlsgen2signup)。 通過核准後，您就可以建立啟用 Data Lake Storage Gen2 的儲存體帳戶。 本快速入門會示範如何使用 [Azure 入口網站](https://portal.azure.com/)、[Azure PowerShell](https://docs.microsoft.com/powershell/azure/overview) 或透過 [Azure CLI](https://docs.microsoft.com/cli/azure?view=azure-cli-latest) 建立帳戶。
...
> [!NOTE]
> 階層命名空間目前可在所有公用區域中使用，但不可在私用區域中使用。 其目前無法在主權雲端中使用。
...
在本快速入門中，您已建立具備 Data Lake Storage Gen2 功能的儲存體帳戶。 若要了解如何在您的儲存體帳戶之間上傳和下載 Blob，請繼續進行 Blob 儲存體快速入門。
