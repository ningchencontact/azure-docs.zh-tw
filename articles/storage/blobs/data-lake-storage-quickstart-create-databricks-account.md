...
5. 在此程式碼區塊中，請將此程式碼區塊中的 `storage-account-name`、`appID`、`password` 和 `tenant-id` 預留位置值取代為您在建立服務主體時所收集到的值。 將 `file-system-name` 預留位置值設為您要為檔案系統指定的任何名稱。
