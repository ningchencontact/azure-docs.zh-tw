...
是的。 設定於帳戶層級的 [存取層]  屬性是套用至該帳戶中所有物件的預設層 (沒有明確設定的層)。 不過，blob 層級的階層處理功能可讓您在物件層級上設定存取層，而不管帳戶上設定的存取層為何。 中的 blob 都是三個存取層 （經常性存取、 非經常性存取或封存） 可能會存在於相同的帳戶。
...
資料分割 只有經常性和非經常性存取層可能會設定為預設帳戶存取層。 封存只能設定於物件層級。
