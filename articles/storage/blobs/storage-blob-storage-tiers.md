...
### <a name="cool-and-archive-early-deletion-effective-march-1-2018"></a>提前刪除非經常性存取和封存 (2018 年 3 月 1 日起生效)
...
在 30 天前和 180 天前遭到刪除或移出非經常性存取 (僅限 GPv2 帳戶) 或封存層的任何 Blob，分別會產生按比例計算的提早刪除費用 (2018 年 3 月 1 起生效)。 檢查 [存取層變更時間] blob 屬性 (該屬性提供最後一次存取層變更的戳記)，即可判斷 blob 放在非經常性存取或封存層中的時間長度。 如需詳細資訊，請參閱[非經常性存取或封存提早刪除](#cool-and-archive-early-deletion)一節。
