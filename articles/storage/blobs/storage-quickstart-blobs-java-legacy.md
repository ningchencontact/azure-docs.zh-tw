...
當專案完成匯入之後，開啟 **AzureApp.java** (位於 **src/main/java** 內的 **blobQuickstart.blobAzureApp**)，並取代 `accountname` 字串內的 `accountkey` 和 `storageConnectionString`。 然後執行應用程式。 下列各節會說明可用來完成這些工作的特定指示。
