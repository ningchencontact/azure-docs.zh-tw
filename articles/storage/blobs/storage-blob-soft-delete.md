...
在虛刪除的基底 Blob 上呼叫取消[刪除 Blob](/rest/api/storageservices/undelete-blob)作業，會將它和所有相關聯的虛刪除快照集還原為使用中。 在作用中的基底 blob 上呼叫 `Undelete Blob` 作業，會將所有相關聯的虛刪除快照集還原為使用中。 快照集還原為作用中後，看起來會像是使用者產生的快照集；它們並不會覆寫基底 Blob。

若要將 blob 還原至特定的虛刪除快照集，您可以在基底 blob 上呼叫 `Undelete Blob`。 然後，您可以將快照集複製到目前作用中的 Blob。 您也可以將快照集複製到新的 Blob。
...
### <a name="is-soft-delete-available-for-all-storage-account-types"></a>虛刪除是否適用于所有儲存體帳戶類型？
...
### <a name="can-i-turn-on-soft-delete-for-existing-storage-accounts"></a>我可以為現有的儲存體帳戶開啟虛刪除嗎？
...
### <a name="if-i-turn-off-soft-delete-will-i-still-be-able-to-access-soft-deleted-data"></a>如果我關閉虛刪除，仍然可以存取虛刪除的資料嗎？

是，在關閉虛刪除的情況下，您仍可存取及復原未到期的虛刪除資料。

### <a name="can-i-read-and-copy-out-soft-deleted-snapshots-of-my-blob"></a>我可以讀取和複製已虛刪除的 blob 快照集嗎？  

是，但您必須先對 Blob 呼叫「取消刪除」。

### <a name="is-soft-delete-available-for-all-blob-types"></a>虛刪除是否適用于所有 blob 類型？

是，虛刪除適用于區塊 blob、附加 blob 和分頁 blob。

### <a name="is-soft-delete-available-for-virtual-machine-disks"></a>虛刪除是否適用于虛擬機器磁片？  
...
### <a name="do-i-need-to-change-my-existing-applications-to-use-soft-delete"></a>我需要將現有的應用程式變更為使用虛刪除嗎？
