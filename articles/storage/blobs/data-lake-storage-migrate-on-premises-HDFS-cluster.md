...
4. 將 shell 變數`azjars`設定為`hadoop-azure`和`azure-storage` jar 檔案的位置。 您可以在 Hadoop 安裝目錄底下找到這些檔案。
    ...
    若要判斷這些檔案是否存在, 請使用下列命令`ls -l $<hadoop_install_dir>/share/hadoop/tools/lib/ | grep azure`:。 `<hadoop_install_dir>`將預留位置取代為您已安裝 Hadoop 的目錄路徑。 請務必使用完整路徑。
...
5. 建立您要用於資料複製的儲存體容器。 您也應該指定目的地目錄做為此命令的一部分。 此時, 這可能是虛擬的目的地目錄。
    ...
    * `<blob_service_endpoint>`將預留位置取代為您的 blob 服務端點名稱。

    * 以您帳戶的存取金鑰取代預留位置。`<account_key>`

    * 以您的容器名稱取代預留位置。`<container-name>`

    * 以您想要將資料複製到其中的目錄名稱取代預留位置。`<destination_directory>`
...
7. 將資料從 Hadoop HDFS 複製到資料箱 Blob 儲存體, 放入您稍早建立的容器中。 如果找不到您要複製到的目錄, 則命令會自動建立它。
    ...
    * 以包含檔案排除清單的檔案名取代預留位置。`<exlusion_filelist_file>`

    * 將`<source_directory>`預留位置取代為包含您要複製之資料的目錄名稱。

    * 以您想要將資料複製到其中的目錄名稱取代預留位置。`<destination_directory>`

    選項是用來`hadoop-azure*.jar`讓和相依`azure-storage*.jar`檔案可供使用`distcp`。 `-libjars` 某些叢集可能已經發生這種情況。
...
* 若要使用以 Azure 為基礎的 Hadoop 叢集, 請執行此 DistCp 命令:
    ...
    * `<source_account>`將和`<dest_account>`預留位置取代為來源和目的地儲存體帳戶的名稱。

    * `<source_container>`將和`<dest_container>`預留位置取代為來源和目的地容器的名稱。

    * `<source_path>`將和`<dest_path>`預留位置取代為來源和目的地目錄路徑。

    * 使用包含資料之儲存體帳戶的存取金鑰來取代預留位置。`<source_account_key>`
...
若要建立服務主體, 請[參閱如何:使用入口網站來建立可存取資源的 Azure AD 應用程式和服務主體](https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal)。
...
* 當您執行文章、應用程式識別碼和用戶端秘密值的[取得值](https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal#get-values-for-signing-in)一節中的步驟時, 就是文字檔。 您很快就會用到這些資料。
...
1. `copy-acls.py`下載腳本。 請參閱本文的[下載協助程式腳本, 並設定您的邊緣節點來執行它們](#download-helper-scripts)一節。
...
* `<application-id>`將和`<client-secret>`預留位置取代為您在建立服務主體時所收集的應用程式識別碼和用戶端密碼。
