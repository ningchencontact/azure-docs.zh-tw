...
| 功能/工具    | 詳細資訊    |
|--------|-----------|
| **Data Lake Storage Gen2 Api** | 部分支援 <br><br>在目前的版本中，您可以使用 Data Lake Storage Gen2 **REST** api 與目錄互動，並設定存取控制清單（acl），但沒有其他 sdk （例如 .Net、JAVA 或 Python）來執行這些工作。 若要執行其他工作（例如上傳和下載檔案），您可以使用 Blob Sdk。  |
...
| **Azure Blob 儲存體生命週期管理原則** | 支援所有存取層。 封存存取層目前為預覽狀態。 尚不支援刪除 blob 快照集。 |
| **Azure 內容傳遞網路（CDN）** | 尚不支援|
| **Azure 搜尋服務** |支援（預覽）|
| **Azure 儲存體總管** | 版本特定支援 <br><br>僅使用透過 `1.10.0``1.6.0` 的版本。 <br> 版本 `1.10.0` 可[免費下載](https://docs.microsoft.com/azure/vs-azure-tools-storage-explorer-relnotes)。 尚不支援版本 `1.11.0`。|
| **Blob 容器 Acl** |尚不支援|
| **Blobfuse** |尚不支援|
| **自訂網域** |尚不支援|
...
| **物件層級層** |支援非經常性存取和封存層。 封存層處於預覽狀態。 尚不支援其他所有存取層。|
