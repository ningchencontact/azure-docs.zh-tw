...
Blob 儲存體 Api 已停用，以避免可能發生的功能操作問題，因為 Blob 儲存體 Api 尚未與 Azure Data Lake Gen2 Api 互通。

> [!NOTE]
> 透過 Data Lake Storage 上的多重通訊協定存取公開預覽，blob Api 和 Data Lake Storage Gen2 Api 可以在相同的資料上運作。 若要深入瞭解，請參閱[Data Lake Storage 上的多重通訊協定存取](data-lake-storage-multi-protocol-access.md)。
...
透過 Data Lake Storage 上的多重通訊協定存取公開預覽，blob Api 和 Data Lake Storage Gen2 Api 可以在相同的資料上運作。
...
| **Azure Blob 儲存體生命週期管理原則** | 支援 Data Lake Storage preview[上的多重通訊協定存取](data-lake-storage-multi-protocol-access.md)。 只有預覽支援非經常性存取和封存存取層。 尚不支援刪除 blob 快照集。 |
| **Azure 內容傳遞網路（CDN）** | 尚不支援|
| **Azure 搜尋服務** |支援 Data Lake Storage preview[上的多重通訊協定存取](data-lake-storage-multi-protocol-access.md)。|
| **Azure 儲存體總管** | 版本特定支援 <br><br>僅使用 `1.6.0` 或更高版本。 <br>版本 `1.6.0` 可[免費下載](https://azure.microsoft.com/features/storage-explorer/)。|
| **Blob 容器 Acl** |尚不支援|
| **Blobfuse** |尚不支援|
| **自訂網域** |尚不支援|
| **檔案系統 Explorer** | 有限支援 |
