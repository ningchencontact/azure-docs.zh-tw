...
為了模擬失敗，您可使用 [Fiddler](#simulate-a-failure-with-fiddler) 或[靜態路由](#simulate-a-failure-with-an-invalid-static-route)。 任一方法會允許您模擬對[讀取權限異地備援](../common/storage-redundancy-grs.md#read-access-geo-redundant-storage) (RA-GRS) 儲存體帳戶的主要端點要求失敗，而導致從次要端點讀取應用程式。
...
請將下列範例程式碼貼至 `OnBeforeResponse` 函式中。 新程式碼會被轉為註解，以確保其所建立的邏輯不會立即實作。

完成後，選取 [檔案] 和 [儲存] 以儲存變更。
...
應用程式開始從主要端點讀取時，在主控台視窗中按下**任意鍵**以暫停應用程式。
...
由於您會控制範例，所以不需要中斷它即可模擬失敗。 只要藉由執行範例並輸入 **P**，確定檔案已上傳至儲存體帳戶。

![應用程式案例](media/storage-simulate-failure-ragrs-account-app/Java-put-list-output.png)

---

### <a name="simulate-failure"></a>模擬失敗

應用程式暫停後，將在 Fiddler 中儲存的自訂規則取消註解。

此範例程式碼會尋找 RA-GRS 儲存體帳戶要求，若路徑包含 `HelloWorld` 檔案的名稱，則會傳回 `503 - Service Unavailable` 的回應碼。

瀏覽至 Fiddler，然後選取 [規則] -> [自訂規則...]。

將下列幾行取消註解，並將 `STORAGEACCOUNTNAME` 取代為您儲存體帳戶的名稱。 選取 [檔案] -> [儲存]，以儲存變更。 
...
應用程式再次開始執行後，針對主要端點的要求即會開始發生失敗。 應用程式會嘗試重新連線至主要端點 5 次。 超過五次的失敗嘗試閥值後，其會從次要唯讀端點要求映像。 應用程式從次要端點成功取出映像 20 次時，即會嘗試連線至主要端點。 若仍然無法連線至主要端點，則應用程式會繼續從次要端點讀取。

此模式即是在上個教學課程中述及的[斷路器模式](https://docs.microsoft.com/azure/architecture/patterns/circuit-breaker)。
...
若要再次模擬主要端點功能，請移除邏輯以插入 `503` 錯誤。

若要暫停應用程式，請按下**任意鍵**。

瀏覽至 Fiddler，然後選取 [規則] 和 [自訂規則...]。 

註解或移除 `OnBeforeResponse` 函式中的自訂邏輯，並保留預設函式。
...
瀏覽至 Fiddler，然後選取 [規則] 和 [自訂規則...]。註解或移除 `OnBeforeResponse` 函式中的自訂邏輯，並保留預設函式。

選取 [檔案] 和 [儲存] 以儲存變更。

完成時，輸入 **G** 以測試下載。 應用程式會回報它現在再度使用主要管線。
...
藉由執行範例並輸入 **P**，確定檔案已上傳至儲存體帳戶。

---

### <a name="simulate-failure"></a>模擬失敗

在應用程式暫停的情況下，在 Windows 上以系統管理員身分啟動命令提示字元，或在 Linux 上以 root 身分執行終端機。

在命令提示字元或終端機上輸入下列命令，以取得儲存體帳戶主要端點網域的相關資訊。

```
nslookup STORAGEACCOUNTNAME.blob.core.windows.net
``` 
 使用您的儲存體帳戶名稱取代 `STORAGEACCOUNTNAME`。 將儲存體帳戶的 IP 位址複製到文字編輯器，供後續使用。
...
![繼續執行應用程式](media/storage-simulate-failure-ragrs-account-app/java-get-pipeline-example-v10.png)
