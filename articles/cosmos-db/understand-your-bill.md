...
Azure Cosmos DB 是一個完全受控的雲端原生資料庫服務，透過只針對已佈建輸送量與已取用儲存體收費以簡化計費流程。 相較于內部部署或 IaaS 裝載的替代方案，沒有額外的授權費用、硬體、公用程式成本或設施成本。 當您考慮 Azure Cosmos DB 的多重區域功能時，相較于現有的內部部署或 IaaS 解決方案，資料庫服務可大幅降低成本。
...
* 在720小時（24小時 * 30 天）的月份中，如果500小時布建的輸送量是 1200 RU/秒，而其餘的220小時布建輸送量是 22200 RU/秒，您的每月帳單會顯示： 500 x $ 0.096/hour + 220 x $ 1.776/小時 = $ 438.72/month。
...
* 您的帳單則會變更為： 1550 * $0.008 = $ 12.40/小時。  

* 在720小時的月份中，如果300小時布建的輸送量為 120-K RU/秒，而其餘的420小時布建輸送量為 155-K RU/秒，您的每月帳單會顯示： 300 x $ 9.60/小時 + 420 x $ 12.40/小時 = $2880 + $5208 = $ 8088/月。 
...
|其他 3 個區域的輸送量計費 - 美國東部、北歐和東亞       | 3 * 10K RU/秒 * 24 * 30    |每小時每 100 RU/秒 $0.008  |$1,728|
|美國西部容器的儲存體計費      | 250 GB    |$0.25/GB  |$62.50|
|其他 3 個區域的儲存體計費 - 美國東部、北歐和東亞      | 3 * 250 GB    |$0.25/GB  |$187.50|
|**總計**     |     |  |**$2,554**|

*我們也假設您每月從美國西部的容器輸出 100 GB 的資料，並將資料複寫到美國東部、北歐和東亞。您需支付每個資料傳輸費率的輸出費用。*
...
### <a name="billing-example-azure-cosmos-account-with-multi-master-database-level-throughput-including-dedicated-throughput-mode-for-some-containers"></a>計費範例：具有多宿主的 Azure Cosmos 帳戶、資料庫層級輸送量，包括某些容器的專用輸送量模式
...
| | |2個額外區域的輸送量計費：美國東部、北歐（所有區域都可寫入）  |`(2 + 1) * (60 K RU/sec /100 * $0.016) * 100 hours = $2,880`  |$2,880  |
...
| | |2個額外區域的輸送量計費：美國東部、北歐（所有區域都可寫入）  |`(2 + 1) * (120 K RU/sec /100 * $0.016) * 100 hours = $5,760`  |$5,760  |
|[201-300]  |D1:50K <br/>D2:70K <br/>C1:20K |美國西部容器的輸送量計費 (所有區域都可寫入)  |`D1: 50 K RU/sec/100 * $0.016 * 100 hours = $800` <br/>`D2: 70 K RU/sec/100 * $0.016 * 100 hours = $1,120` <br/>`C1: 20 K RU/sec/100 *$0.016 * 100 hours = $320` |$2240  |
| | |2個額外區域的輸送量計費：美國東部、北歐（所有區域都可寫入）  |`(2 + 1) * (140 K RU/sec /100 * $0.016-) * 100 hours = $6,720` |$6,720 |
...
| | |2個額外區域的輸送量計費：美國東部、北歐（所有區域都可寫入）  |`(1 + 1) * (90 K RU/sec /100 * $0.016) * 100 hours = $2,880`  |$2,880  |
...
| | |2個額外區域的輸送量計費：美國東部、北歐（所有區域都可寫入）  |`(1 + 1) * (40 K RU/sec /100 * $0.016) * 100 hours = $1,280`  |$1,280  |
...
| | |2個額外區域的輸送量計費：美國東部、北歐（所有區域都可寫入）  |`(1 + 1) * (120 K RU/sec /100 * $0.016) * 200 hours = $1,280`  |$7,680  |
...
| | |2個額外區域的輸送量計費：美國東部、北歐（所有區域都可寫入）  |`(1 + 1) * (70 K RU/sec /100 * $0.016) * 20 hours = $448`  |$224  |
|| |**每月總成本**  | |**$38,688**   |
...
|**輸送量成本** | | | |
|----|----|----|----|
|作業類型| 個要求/秒| 平均 RU/要求| 必要 RU|
|寫入| 100 | 5 | 500|
|讀取| 400| 1| 400|

總 RU/秒： 500 + 400 = 900 每小時成本： 900/100 * $0.008 = $0.072 預期的輸送量每月成本（假設31天）： $0.072 * 24 * 31 = $53.57

**每月總成本**

每月成本總計 = 儲存體每月成本 + 輸送量每月成本每月總成本 = $25.00 + $53.57 = $78.57

*定價可能因地區而異。如需最新的定價，請參閱[定價頁面](https://azure.microsoft.com/pricing/details/cosmos-db/)。*
...
* 輸送量的成本（隨用隨付）： 100000 RU/秒/100 * $ 0.008/小時 * 一年中的8760小時 = $70080 
