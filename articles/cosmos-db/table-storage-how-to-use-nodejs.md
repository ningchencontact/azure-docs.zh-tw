...
若要使用「Azure 儲存體」，您需要 Azure Storage SDK for Node.js，這包含一組能與「儲存體 REST」服務進行通訊的便利程式庫。
...
## <a name="add-an-azure-comsos-db-connection"></a>新增 Azure Cosmos DB 連線
...
在此回呼中，以及處理 **returnObject** (來自對伺服器發出之要求的回應) 之後，回呼必須叫用 **next** (如果存在) 以繼續處理其他篩選，否則就叫用 **finalCallback** 以結束服務叫用。
...
如果作業成功，`result` 就會包含所插入記錄的 [ETag](http://en.wikipedia.org/wiki/HTTP_ETag)，而 `response` 則會包含作業的相關資訊。
...
GitHub 上的 [azure-storage-node 存放庫](https://github.com/Azure/azure-storage-node/tree/master/examples/samples)中也有接續範例 (continuationsample.js)。 
