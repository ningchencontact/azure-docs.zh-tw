...
本快速入門會示範如何使用 Azure 入口網站建立 Azure Cosmos DB 帳戶、文件資料庫和集合。 接著，您要建置和執行以 [SQL Python API](sql-api-sdk-python.md) 為基礎的主控台應用程式。
...
讓我們快速檢閱應用程式中發生了什麼。 請開啟 DocumentDBGetStarted.py 檔案，您會發現這些程式碼行會建立 Azure Cosmos DB 資源。 
