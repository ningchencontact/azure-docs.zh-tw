...
假設您要建立一個全新且亮眼的社交網路，讓使用者可以張貼含有圖片、影片或甚至音樂等相關媒體的文章。 使用者可以評論貼文，並給予評等的點數。 他們可以看到貼文摘要，並可在主要的網站登陸頁面上進行互動。 這個方法乍聽之下似乎不複雜，但為了簡潔起見，我們先就此打住 (您可以深入探討受關聯性影響的自訂使用者摘要，但這已超出本文的目標範圍)。
...
最大的一階為「擴充使用者」。 它包含所有重要的使用者資訊，加上其他不需快速讀取或最終使用情況的相關資料 (例如登入過程)。 這些資料可以儲存在 Cosmos DB 外部、Azure SQL Database 中或 Azure 儲存體資料表中。
