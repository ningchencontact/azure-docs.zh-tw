...
ms.locfileid: "48248749"
---
# <a name="throughput-and-request-units-in-azure-cosmos-db"></a>Azure Cosmos DB 中的輸送量和要求單位

Azure Cosmos DB 資源會根據已佈建的輸送量和儲存體來計費。 Azure Cosmos DB 輸送量會以**每秒要求單位 (RU/秒)** 來表示。 Azure Cosmos DB 支援各種 API 來執行不同的作業，範圍可從簡單地讀取及寫入到複雜的圖表查詢。 每個要求都會根據為要求提供服務所需的計算量來取用要求單位。 作業的要求單位數目是先決的。 您可以使用回應標頭來追蹤 Azure Cosmos DB 中任何作業所取用的要求單位數。 為了提供可預測的效能，您應該保留每秒 100 RU 單位的輸送量。 您可以使用 Azure Cosmos DB [要求單位計算機](https://www.documentdb.com/capacityplanner)來預估您的輸送量需求。

在 Azure Cosmos DB 中，您可以佈建兩個資料粒度的輸送量： 

1. **Azure Cosmos DB 容器：** 為容器佈建的輸送量僅保留供該特定容器使用。 指派容器層級的輸送量 (RU/秒) 時，可以採用「固定」或「無限制」方式來建立容器。 

  固定大小之容器的輸送量上限為 10,000 RU/秒，且儲存體限制為 10 GB。 若要建立無限制的容器，您必須指定最小輸送量 1,000 RU/s 和[分割區索引鍵](partition-data.md)。 由於您的資料可能分散在多個分割區，因此您必須挑選基數高 (100 個到數百萬個相異值) 的分割區索引鍵。 藉由選取具有許多相異值的分割區索引鍵，Azure Cosmos DB 便可確保能夠一致地調整對集合、資料表及圖形的要求。 

2. **Azure Cosmos DB 資料庫：** 為資料庫佈建的輸送量會在該資料庫內的所有容器之間共用。 在資料庫層級佈建輸送量時，您可以選擇明確排除特定容器，而改為在容器層級為這些容器佈建輸送量。 資料庫層級輸送量會要求所有集合在建立時都必須指定分割區索引鍵。 在資料庫層級指派輸送量時，屬於此資料庫的容器在建立時都應該指定分割區索引鍵，因為每個集合都是「無限制」容器。  

Azure Cosmos DB 會根據所佈建的輸送量，配置實體分割區來裝載您的容器，並隨著資料的成長將資料分散到各個分割區。 下圖說明在不同層級佈建輸送量：

  ![佈建個別容器和一組容器的要求單位](./media/request-units/provisioning_set_containers.png)

> [!NOTE] 
> 在容器層級和資料庫層級佈建輸送量是不同的供應項目，在這任一方之間切換都需要將資料從來源移轉到目的地。 這表示您需要建立新資料庫或新集合，然後藉由使用[大量執行程式程式庫](bulk-executor-overview.md)或 [Azure Data Factory](../data-factory/connector-azure-cosmos-db.md) 來移轉資料。
...
要求單位是要求處理成本的標準化量值。 單一要求單位代表讀取 (透過自我連結或識別碼) 由 10 個唯一屬性值 (不包括系統屬性) 所組成之單一 1 KB 項目所需的處理能力。 建立 (插入)、取代或刪除相同項目的要求會耗用服務的更多處理，因此需要更多的要求單位。 
