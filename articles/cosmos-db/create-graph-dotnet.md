...
3. 如果您的圖形資料庫帳戶是在 2017 年 11 月 27 日之前建立的，請將 `endpoint` 值中的 `documents` 變更為 `graphs`。 如果您的圖形資料庫帳戶是在 2017 年 11 月 27 日當天或之後建立的，請將 `endpoint` 值中的 `documents` 變更為 `gremlin.cosmosdb`。
