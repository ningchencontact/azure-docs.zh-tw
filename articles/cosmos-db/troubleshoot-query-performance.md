...
使用 `QueryMetrics` 來疑難排解緩慢或昂貴的查詢。 
  ...
  * 將 `FeedOptions.PopulateQueryMetrics = true` 設定為在回應中具有 `QueryMetrics`。
  * `QueryMetrics` 類別具有多載的 `.ToString()` 函數，可叫用以取得 `QueryMetrics`的字串表示。 
  * 您可以利用計量來衍生下列深入解析，還有其他資訊： 
  
      * 查詢管線的任何特定元件是否花費太長的時間才能完成（以數百毫秒或更多的順序）。 

          * 查看 `TotalExecutionTime`。
          * 如果查詢的 `TotalExecutionTime` 小於端對端執行時間，則時間會花在用戶端或網路中。 再次檢查用戶端和 Azure 區域是否已共置。
      ...
      * 個別來回行程的優於方式（請參閱 `QueryMetrics`的字串表示中的 `Partition Execution Timeline`）。 
      * 查詢是否耗用過高的要求費用。 
...
您可以透過要求的[摘要選項](https://docs.microsoft.com/dotnet/api/microsoft.azure.documents.client.feedoptions?view=azure-dotnet)參數來調整查詢效能。 請嘗試設定下列選項：
  ...
  * 先將 `MaxDegreeOfParallelism` 設定為-1，然後比較不同值之間的效能。 
  * 先將 `MaxBufferedItemCount` 設定為-1，然後比較不同值之間的效能。 
  * 將 `MaxItemCount` 設定為-1。
...
多重值子查詢可以將述詞推送至每個 select-many 運算式之後，而不是在 `WHERE` 子句中的所有交叉聯結之後，藉以優化 `JOIN` 運算式。 如需詳細範例，請參閱[優化聯結運算式](https://docs.microsoft.com/azure/cosmos-db/sql-query-subquery#optimize-join-expressions)一文。

## <a name="optimize-order-by-expressions"></a>將 ORDER BY 運算式優化 
如果欄位為稀疏或未包含在索引原則中，`ORDER BY` 查詢效能可能會受到影響。

  * 若為稀疏欄位（例如時間），請盡可能減少搜尋空間與篩選準則。 
  * 針對單一屬性 `ORDER BY`，請在索引原則中包含屬性。 
  * 若為多個屬性 `ORDER BY` 運算式，請在要排序的欄位上定義[複合索引](https://docs.microsoft.com/azure/cosmos-db/index-policy#composite-indexes)。  
