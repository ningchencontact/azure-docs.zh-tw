...
*運算子深度* | **400** |  周遊中的特有步驟總數。 例如，```g.V().out()``` 的運算子計數為2： V （）和 out （），```g.V('label').repeat(out()).times(100)``` 的運算子深度為3： V （）、重複（）和 out （），因為 ```.times(100)``` 是 ```.repeat()``` 運算子的參數。
*平行處理原則的程度* | **32** | 在對儲存層發出的單一要求中，可查詢的儲存體分割區數目上限。 具有數百個數據分割的圖形會受到此限制的影響。
*重複限制* | **32** | ```.repeat()``` 運算子可執行的反覆項目數上限。 在大部分情況下，```.repeat()``` 步驟的每個反復專案都會執行廣度優先的回合，這表示任何一個迴圈最多隻能有32個頂點之間的躍點。
...
*每小時的資源權杖* | **100** | Gremlin 用戶端用來連線到區域中 Gremlin 帳戶的唯一資源權杖數目。 當應用程式超過每小時唯一的權杖限制時，將會在下一次驗證要求時傳回 `"Exceeded allowed resource token limit of 100 that can be used concurrently"`。
