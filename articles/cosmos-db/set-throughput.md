...
在資料庫上布建的輸送量可以由該資料庫內的容器共用。 最多25個容器可以共用在資料庫上布建的輸送量。 超過25個容器, 針對該資料庫內建立的每個新容器, 都可以與資料庫中已有的其他集合共用資料庫輸送量的一部分。 可以共用的輸送量數量取決於在資料庫中布建的容器數目。 

如果您的工作負載牽涉到刪除和重新建立資料庫中的所有集合, 建議您卸載空的資料庫, 然後在建立集合之前, 先重新建立新的資料庫。
