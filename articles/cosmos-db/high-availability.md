...
將區域新增至新的或現有的 Azure Cosmos 帳戶時, 您可以啟用區域冗余。 目前, 您只能使用 Azure 入口網站、PowerShell 和 Azure Resource Manager 範本來啟用區域冗余。 若要在您的 Azure Cosmos 帳戶上啟用區域冗余, 您`isZoneRedundant`應該針對`true`特定位置將旗標設定為。 您可以在 [位置] 屬性中設定此旗標。 例如, 下列 powershell 程式碼片段會啟用「東南亞」區域的區域冗余:
