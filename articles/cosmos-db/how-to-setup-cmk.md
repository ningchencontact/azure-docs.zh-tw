...
- 選取 [ **+ 新增存取原則**]
- 在 [**金鑰許可權**] 下拉式功能表中，選取 [**取得**]、[解除包裝**金鑰**] 和 [**換行金鑰**]：

    ![選取正確的許可權](./media/how-to-setup-cmk/portal-akv-add-ap-perm2.png)

- 在 [**選取主體**] 底下，選取 [**未選取**]。 然後，搜尋並選取 [ **Azure Cosmos DB** ] 主體。 最後，按一下底部的 [**選取**] （如果找不到**Azure Cosmos DB**主體，您可能需要在步驟1重新註冊**Microsoft DocumentDB**資源提供者）：
...
- 選取**產生/匯入**
- 提供新金鑰的名稱，選取 RSA 金鑰大小（建議最少3072，以獲得最佳安全性），然後選取 [**建立**]：
...
從 Azure 入口網站建立新的 Azure Cosmos DB 帳戶時，請在**加密**步驟中選擇 [**客戶管理的金鑰**]。 在 [**金鑰 URI** ] 欄位中，傳遞從步驟4複製的 Azure Key Vault 金鑰的 URI：
...
> [!IMPORTANT]
> 必須明確設定 `Location` 參數，才能使用 CMK 成功建立帳戶。
