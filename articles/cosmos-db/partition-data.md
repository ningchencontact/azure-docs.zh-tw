...
實體分割區是固定數量的保留 SSD 型儲存體。 系統會複寫每個實體分割區以獲得高可用性。 一或多個實體分割區會組成容器。 實體分割區管理可完全由 Azure Cosmos DB 管理，您不需要撰寫複雜的程式碼或管理分割區。 Azure Cosmos DB 容器在儲存體和輸送量方面並無限制。 

邏輯分割區是一種實體分割區內的分割區，會儲存所有與單一分割區索引鍵值相關聯的資料。 邏輯分割區的最大值為 10 GB。在下列圖表中，單一容器會有三個邏輯分割區。 每個邏輯分割區都會個別儲存一個分割區索引鍵、LAX、AMS 和 MEL 的資料。 每個 LAX、AMS 和 MEL 邏輯分割區無法成長超過 10 GB 的邏輯分割區上限。 

![資源的資料分割](./media/introduction/azure-cosmos-db-partitioning.png) 

當集合符合[資料分割必要條件](#prerequisites)時，資料分割對應用程式而言是透明的作業。 Azure Cosmos DB 支援快速的讀取與寫入、查詢、交易邏輯、一致性層級，以及透過方法/API 對單一容器資源進行更細微的存取控制。 此服務會處理跨實體和邏輯分割區所分散的資料，以及將查詢要求路由傳送至正確的分割區。 

## <a name="how-does-partitioning-work"></a>資料分割如何運作

資料分割如何運作？ 每個項目必須具備可唯一識別它的資料分割索引鍵和資料列索引鍵。 您的分割區索引鍵是存放資料的邏輯分割區，並為 Azure Cosmos DB 提供將資料分散到實體分割區的自然界限。 請記住，單一邏輯分割區的資料必須位在單一實體分割區內，但實體分割區是受 Azure Cosmos DB 管理。 

簡單地說，Azure Cosmos DB 中的資料分割運作方式如下︰

* 您使用每秒 **T** 要求的輸送量佈建 Azure Cosmos DB 容器。
...
Azure Cosmos DB 使用雜湊型資料分割。 當您寫入項目時，Azure Cosmos DB 會將資料分割索引鍵值進行雜湊處理，然後使用雜湊的結果來判斷要在其中儲存項目的分割區。 Azure Cosmos DB 會將資料分割索引鍵相同的所有項目儲存在相同的實體分割區中。 選擇資料分割索引鍵是在設計階段必須進行的一項重要決策。 您必須選擇具有各種不同的值並且有平均存取模式的屬性名稱。 如果實體分割區觸達其儲存空間限制，且分割區中的所有資料具有相同的分割區索引鍵，Azure Cosmos DB 會傳回「分割區索引鍵已觸達 10 GB 的大小上限」的錯誤，且分割區未分割，因此選擇分割區索引鍵是很重要的決策。

> [!NOTE]
> 資料分割索引鍵最好有許多相異值 (至少數百至數千個)。
>

Azure Cosmos DB 容器可以建立為固定或無限制。 固定大小的容器具有上限為 10 GB 和 10,000 RU/秒的輸送量。 若要建立無限制的容器，您必須指定最小輸送量 1,000 RU/秒，且必須指定分割區索引鍵。
...
對於要自動分割為 **p1** 和 **p2** 的實體分割區，如[資料分割如何運作](#how-does-partitioning-work)中所述，必須使用 1,000 RU/秒以上的輸送量來建立容器，且必須提供分割區索引鍵。 在 Azure 入口網站中建立容器時，選取 [無限制] 的儲存容量選項，以利用資料分割和自動調整。 

如果您在 Azure 入口網站中、或是以程式設計方式建立容器，而初始輸送量為 1,000 RU/秒以上，且您的資料包含分割區索引鍵，您可以在不變更容器的情況下利用資料分割，這包括 [固定] 大小容器，只要您在輸送量中至少使用 1,000 RU/秒來建立初始容器，且分割區索引鍵存在於資料中。

如果您未使用分割區索引鍵建立 [固定] 大小容器，或是使用小於 1,000 RU/秒的輸送量來建立 [固定] 大小容器，則容器無法如本文中所述自動分割。 若要將資料從像這樣的容器移轉到無限制的容器 (輸送量至少 1,000 RU/秒且包含分割區索引鍵的容器) 中，您必須使用[資料移轉工具](import-data.md)或[變更摘要文件庫](change-feed.md)將變更移轉。 
...
若要使用 Azure Cosmos DB 資料表 API 建立資料表，請使用 CreateIfNotExists 方法。 
