...
Azure Cosmos DB 是真正開放的資料庫服務，不會發明 JSON 文件的任何特殊資料類型 (例如日期時間) 或特定編碼。 Azure Cosmos DB 無須遵循任何特殊 JSON 慣例，即可編寫各種文件之間的關聯性。Azure Cosmos DB 的 SQL 語法提供有效率的階層式和關係查詢運算子，讓您可以用於查詢及保護文件，不僅無須任何特殊註釋，也無須使用不同的屬性來編寫文件之間的關聯性。  
