...
ms.locfileid: "67657382"
---
# <a name="data-modeling-in-azure-cosmos-db"></a>Azure Cosmos DB 中模型化的資料

雖然無結構描述的資料庫，像是 Azure Cosmos DB 中，可讓您極容易儲存和查詢非結構化及半結構化資料，您應該花一些時間來思考来最大化效能和延展性方面的服務和最低的資料模型成本。

要如何儲存資料？ 您的應用程式要如何擷取及查詢資料？ 大量讀取或寫入繁重，則是您的應用程式？

閱讀本文後，您將能夠回答下列問題：

* 什麼是資料模型化，以及為什麼應該關心？
* 何關聯式資料庫不同的 Azure Cosmos DB 中的模型化資料？
* 如何表達非關聯式資料庫中的資料關聯性？
* 何時內嵌資料，以及何時連結至資料？

## <a name="embedding-data"></a>內嵌資料

當您開始在 Azure Cosmos DB 中的資料模型化時嘗試將您的實體視為**自封式項目**表示為 JSON 文件。

進行比較，我們先來看如何我們可能會在此模型在關聯式資料庫中的資料。 下列範例示範人員可能如何儲存在關聯式資料庫中。

![關聯式資料庫模型](./media/sql-api-modeling-data/relational-data-model.png)

當使用關聯式資料庫，策略是標準化您的資料。 將您的資料正規化通常牽涉到取得實體，例如： 個人，分解成離散的元件。 在上述範例中，個人可擁有多個連絡詳細資料記錄，以及多個地址記錄。 連絡人詳細資料可進一步細分透過進一步擷取一般欄位，例如型別。 這同樣適用於位址，每一筆記錄可以是類型*首頁*或是*商務*。

將資料正規化的引導前提是在每個記錄資料上 **避免儲存多餘的資料** ，而是參考資料。 在此範例中，若要讀取人員以及其所有的連絡人詳細資料和地址，您需要在執行階段，來有效地撰寫回 （或反正規化） 資料使用聯結。
...
現在讓我們看看如何我們會建立與 Azure Cosmos DB 中的獨立實體相同的資料模型。
...
使用上述我們的方法有**反正規化**藉由將人員記錄**內嵌**的所有資訊都與到這個人，他們的連絡人詳細資料和地址，例如*單一JSON*文件。
此外，因為我們不受限於固定的結構描述，我們有彈性可進行一些動作，像是讓連絡詳細資料不同的圖形。

從資料庫擷取完整的人員記錄現在是**單一讀取作業**對單一容器和單一項目。 更新個人記錄，他們的連絡人詳細資料和地址，還有**單一寫入作業**針對單一項目。
...
雖然 Azure Cosmos DB 中的法則是反正規化所有項目，並將所有資料都內嵌到單一項目，這可能會導致某些情況下，應該予以避免。
...
如果我們要模型化一般的部落格或 CMS 系統，這可能是具有內嵌註解的文章實體的外觀。 此範例的問題在於註解陣列是 **unbounded**，表示任何單一文章可以具備的註解數目沒有 (實際) 的限制。 項目的大小可以成長無限期地大，這可能會造成問題。

項目的大小增加時透過網路，以及讀取和更新的項目，大規模地傳輸資料的能力會受到影響。

在此情況下，它會比較好，請考慮下列的資料模型。
...
此模型有內嵌在 [post] 容器，也就是使用一組固定的屬性陣列中的三個最新註解。 其他註解會分組中的批次的 100 個註解，並儲存為個別項目。 因為我們虛構的應用程式可讓使用者一次載入 100 個註解，因此已將批次大小選擇為 100。  

內嵌資料並不是個好主意的另一種情況時，內嵌的資料經常跨項目，而且會經常變更。
...
如果您查看書籍文件，在查看作者陣列時就會看到一些有趣的欄位。 沒有`id`也就是欄位的欄位，我們使用回參考作者文件，正規化的模型中的標準作法，但然後我們也有`name`和`thumbnailUrl`。 我們可能卡與`id`，保留要從使用 「 link 」，個別作者文件中取得它所需的任何其他資訊的應用程式，但因為作者的名稱和縮圖圖片中的，每本書籍，會顯示我們的應用程式顯示我們也可以反正規化到每個清單中的通訊錄伺服器儲存之間往返**某些**作者的資料。

當然，如果變更作者的名稱，或他們想要更新其相片，我們必須把更新他們曾經發行，但我們的應用程式以作者不常變更其名稱的假設每本書籍，這是可接受的設計決策。  
...
## <a name="distinguishing-between-different-document-types"></a>區別不同的文件類型

在某些情況下，您可能想要混合相同集合中，不同的文件類型此乃常見情況，當您想要多個相關的文件，以放入相同[分割區](partitioning-overview.md)。 比方說，您可以在此放置這兩個書籍和書籍評論中相同的集合並分割所`bookId`。 在這種情況下，您通常會想要新增到您的文件，才能加以區隔識別其類型的欄位。
