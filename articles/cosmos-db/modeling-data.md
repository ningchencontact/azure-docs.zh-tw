...
在我們更進一步深入探討之前，讓我們稍微後退一下，看看我們如何在關聯式資料庫中建立某個項目的模型，這應該是許多人已熟悉的主題。 下列範例示範人員可能如何儲存在關聯式資料庫中。

![關聯式資料庫模型](./media/sql-api-modeling-data/relational-data-model.png)

使用關聯式資料庫時，我們已多年被告知要進行正規化、正規化、正規化。

將您的資料正規化通常會牽涉到取得某個實體 (例如某個人)，然後將它分解為離散的資料片段。 在上述範例中，人員可以有多個連絡詳細資料記錄，以及多個地址記錄。 我們甚至透過進一步擷取一般欄位 (像是類別)，更進一步細分連絡詳細資料。 相同的位址，每一筆記錄有類型，如*首頁*或是*商務*。
...
雖然在文件資料庫中的經驗法則是對所有項目反正規化，並將所有資料內嵌至單一的文件，但是這可能會導致某些應該避免的情況。
