...
|Azure 目錄整合 (RBAC)| 您也可以使用 Azure 入口網站中的存取控制 (IAM), 提供或限制 Cosmos 帳戶、資料庫、容器和供應專案 (輸送量) 的存取權。 IAM 提供角色型存取控制，並與 Active Directory 整合。 您可以針對個人和群組使用內建角色或自訂角色。 如需詳細資訊, 請參閱[Active Directory 整合](role-based-access-control.md)文章。|
...
|區域複寫|即使在單一資料中心內，Azure Cosmos DB 也會自動複寫資料來達到高可用性，還可讓您選擇[一致性層級](consistency-levels.md)。 此複寫可保證所有單一區域帳戶和所有多重區域帳戶具有寬鬆一致性的 99.99%[可用性 SLA](https://azure.microsoft.com/support/legal/sla/cosmos-db) , 以及所有多重區域資料庫帳戶 99.999% 的讀取可用性。|
|自動化線上備份|Azure Cosmos 資料庫會定期備份, 並儲存在異地多餘的存放區中。 <br><br>請參閱[使用 Azure Cosmos DB 進行自動線上備份及還原](online-backup-and-restore.md)以深入了解。|
...
|HTTPS/SSL/TLS 加密|用戶端對服務的所有 Azure Cosmos DB 互動都支援 SSL/TLS 1.2。 此外, 所有內部資料中心和跨資料中心複寫都是強制執行 SSL/TLS 1.2。|
...
|安全性和資料保護認證| 如需最新的認證清單, 請參閱整體[Azure 合規性網站](https://www.microsoft.com/en-us/trustcenter/compliance/complianceofferings), 以及包含所有認證的最新[Azure 合規性檔](https://gallery.technet.microsoft.com/Overview-of-Azure-c1be3942)(搜尋 Cosmos)。 如需更專注的閱讀, 請參閱2018年4月25日文章 [Azure #CosmosDB:安全、私用、符合規範, 包括 SOC 1/2 Type 2、HITRUST、PCI DSS Level 1、ISO 27001、HIPAA、FedRAMP High 和其他許多專案。
