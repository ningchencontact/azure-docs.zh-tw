...
- **索引類型：** 範圍 （相等、 範圍或 ORDER BY 查詢） 或空間 （空間查詢）。
...
- **範圍索引**支援有效率的相等查詢、 聯結查詢、 範圍查詢 (使用 >，<>、 =、 < =、 ！ =)，和 ORDER BY 查詢。 根據預設，ORDER By 查詢也需要最大索引精確度 (-1)。 資料類型可以是 String 或 Number。
