...
Azure 原則的其中一個最大優點，就是能夠針對訂用帳戶中的資源或訂用帳戶的[管理群組](../azure-resource-manager/management-groups-overview.md)，提供相關見解和控制權。 此控制權可運用在許多不同方面，例如防止在錯誤的位置建立資源、強制執行通用且一致的標記使用方式，或稽核現有的資源以取得適當的組態和設定。 在所有情況下，原則都會產生資料，讓您了解環境的合規性狀態。
...
![原則合規性頁面](media/policy-compliance/compliance-page.png)

您可以將一個原則或計畫指派給不同的範圍，但請注意表格中每個指派的範圍，以及指派給該範圍的類型定義。 也會提供每個指派的不相容原則和不相容資源數目。 按一下表格中的原則或計畫可讓您更深入地查看該特定指派的合規性。

![原則合規性詳細資料](media/policy-compliance/compliance-details.png)

[Non-compliant resources] \(不相容的資源\) 索引標籤上的資源清單反映目前指派之現有資源的評估狀態，建立資源的要求所觸發的事件 (附加、稽核、拒絕、部署) 則會顯示在 [事件] 索引標籤下方。

![原則合規性事件](media/policy-compliance/compliance-events.png)

以滑鼠右鍵按一下您想要收集更多詳細資料的事件資料列，然後選取 [顯示活動記錄]。 活動記錄頁面隨即開啟，並會預先篩選至顯示指派和事件詳細資料的搜尋結果。 活動記錄檔提供有關這些事件的其他內容和資訊。

![原則合規性活動記錄](media/policy-compliance/compliance-activitylog.png)
...
![使用 Log Analytics 的原則合規性](media/policy-compliance/compliance-loganalytics.png)

## <a name="next-steps"></a>後續步驟

- 檢閱[原則定義結構](policy-definition.md)。
- 檢閱[了解原則效果](policy-effects.md)。
