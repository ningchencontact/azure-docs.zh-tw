---
title: Azure 原則 JSON 範例 - 允許來自某個資源群組的自訂 VM 映像 | Microsoft Docs
description: 此 JSON 範例原則會要求自訂映像必須來自某個已核准的資源群組。
...
此 JSON 範例原則會要求自訂映像必須來自某個已核准的資源群組。 您需指定已核准之資源群組的名稱。
...
您可以在「Azure 原則」文件中找到其他「Azure 原則」範本範例
