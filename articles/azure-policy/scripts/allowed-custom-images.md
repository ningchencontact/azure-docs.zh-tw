...
description: 此原則會要求只能在環境中部署已核准的自訂映像。
...
有數個工具可用來與 Resource Manager REST API 互動，例如 [ARMClient](https://github.com/projectkudu/ARMClient) 或 PowerShell。 在[原則定義結構](../policy-definition.md#aliases)的 [別名] 區段中，可以找到從 PowerShell 呼叫 REST API 的範例。
...
- 檢閱其他 [Azure 原則範例](../json-samples.md)
- 檢閱 [Azure 原則定義結構](../policy-definition.md)
- 如需虛擬機器的 Azure 原則範例，請參閱[將原則套用到 Windows VM](../../virtual-machines/windows/policy.md)