...
解除佈建裝置的確切步驟取決於所使用的證明機制。

搭配自我簽署的分葉憑證 (沒有憑證鏈結) 使用 TPM 證明或 X.509 證明的裝置，會透過個別的註冊項目佈建。 對於這些裝置，您可以刪除項目以永久撤銷裝置對佈建服務的存取權限，也可以停用項目以暫時撤銷其存取權限，然後刪除或停用身分識別登錄中對於佈建的 IoT 中樞所登錄的裝置。
...
- 若要解除佈建所有透過註冊群組佈建的裝置，必須先停用註冊群組，將其簽署憑證列入黑名單。 然後即可使用該註冊群組的佈建裝置清單，從個別的 IoT 中樞身分識別登錄中停用或刪除每個裝置。 從個別的 IoT 中樞停用或刪除所有裝置之後，可以選擇是否要刪除註冊群組。 不過請注意，如果您刪除註冊群組，而憑證鏈結之中一個或多個更上層裝置的簽署憑證有已啟用的註冊群組，這些裝置可以重新註冊。 
- 若要解除佈建註冊群組的其中一個裝置，必須先為分葉 (裝置) 憑證建立一個已停用的獨立註冊。 如此會撤銷該裝置的佈建服務存取權限，但是仍允許鏈結之中具有註冊群組簽署憑證的其他裝置進行存取。 然後您可以使用該註冊群組詳細資料中的佈建裝置清單來尋找佈建裝置的 IoT 中樞，再從該中樞的身分識別登錄停用或刪除裝置。 請勿刪除裝置已停用的個別註冊。 否則會使得裝置透過註冊群組重新註冊。 
