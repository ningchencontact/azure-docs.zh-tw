...
* **地理位置/地理延遲**：當裝置位置之間移動時，網路延遲已獲得改善，讓裝置移轉至更接近的 IoT 中樞。

* **多租用戶**：裝置可能會使用相同的 IoT 解決方案中，並且重新指派給新客戶或客戶站台。 系統可能會使用不同的 IoT 中樞為這個新客戶提供服務。

* **方案變更**:裝置無法移到新的或更新的 IoT 解決方案。 此重新指派可能需要裝置與連線至其他後端元件的新 IoT 中樞通訊。

* **隔離**:類似於方案的變更。 故障、遭入侵或已過期的裝置可能會重新指派給只能更新，讓裝置再次符合合規性要求的 IoT 中樞。 在裝置正常運作之後，就會移轉回其主要中樞。
...
* **重新佈建，並將資料移轉**:此原則是新的註冊項目的預設值。 此原則會在與註冊項目關聯的裝置提交新的要求 (1) 時採取動作。 視註冊項目設定而定，裝置可能會重新指派給其他 IoT 中樞。 如果裝置所屬的 IoT 中樞有所變更，將會移除初始 IoT 中樞中的裝置註冊。 該初始 IoT 中樞中已更新的裝置狀態資訊將會移轉至新的 IoT 中樞 (2)。 移轉期間，裝置的狀態將回報為**指派中**。

    ![使用裝置佈建服務佈建](./media/concepts-device-reprovisioning/dps-reprovisioning-migrate.png)

* **重新佈建和重設為初始組態**:此原則會在與註冊項目關聯的裝置提交新的佈建要求 (1) 時採取動作。 視註冊項目設定而定，裝置可能會重新指派給其他 IoT 中樞。 如果裝置所屬的 IoT 中樞有所變更，將會移除初始 IoT 中樞中的裝置註冊。 系統會將佈建服務執行個體在佈建裝置時收到的初始設定資料提供給新的 IoT 中樞 (2)。 移轉期間，裝置的狀態將回報為**指派中**。

    此原則通常會用於在不變更 IoT 中樞的情況下，恢復出廠預設值。

    ![使用裝置佈建服務佈建](./media/concepts-device-reprovisioning/dps-reprovisioning-reset.png)

* **永遠不會重新佈建**:永遠不會將裝置指派給不同的中樞。 提供此原則的目的是為了管理回溯相容性。
