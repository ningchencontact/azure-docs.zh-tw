...
* 內[**建**](#built-ins)：這些內建的觸發程式和動作都是「原生」，可 Azure Logic Apps 並協助您建立在自訂排程上執行的邏輯應用程式、與其他端點通訊、接收和回應要求，以及呼叫 azure 函式、azure API Apps （Web Apps）、您自己使用 Azure API 管理管理和發行的 api，以及可接收要求的嵌套邏輯應用程式。 您也可以使用內建動作，協助您組織及控制邏輯應用程式的工作流程，以及處理資料。
...
|   |   |   |   |   | 
|---|---|---|---|---| 
| ![API 圖示][biztalk-server-icon]<br/>**BizTalk**</br> **伺服器** | [![API 圖示][file-system-icon]<br/>**檔</br> 系統**][file-system-doc] | [**IBM DB2**][ibm-db2-icon]<br/>的 ![API 圖示][ibm-db2-doc] | [![API 圖示][ibm-informix-icon]<br/>**IBM**</br> **Informix**][ibm-informix-doc] | ![API 圖示][mysql-icon]<br/>**MySQL** | 
