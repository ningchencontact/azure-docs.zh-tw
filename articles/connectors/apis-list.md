...
* 內[**建**](#built-ins)：這些內建的觸發程式和動作都是「原生」，可 Azure Logic Apps 並協助您建立在自訂排程上執行的邏輯應用程式、與其他端點通訊、接收和回應要求，以及呼叫 azure 函式、azure API Apps （Web）應用程式）、您自己使用 Azure API 管理來管理及發佈的 Api，以及可接收要求的嵌套邏輯應用程式。 您也可以使用內建動作，協助您組織及控制邏輯應用程式的工作流程，以及處理資料。
...
* **受控連接器**：由 Microsoft 部署及管理，這些連接器提供存取雲端服務、內部部署系統或兩者（包括 Office 365、Azure Blob 儲存體、SQL Server、Dynamics、Salesforce、SharePoint）的觸發程式和動作。等等。 某些連接器特別支援企業對企業（B2B）通訊案例，而且需要連結至邏輯應用程式的[整合帳戶](../logic-apps/logic-apps-enterprise-integration-create-integration-account.md)。 使用特定連接器之前，您可能必須先建立由 Azure Logic Apps 管理的連接。 
...
|   |   |   |   | 
|---|---|---|---| 
| [@no__t 1API 圖示 @ no__t-2**排程**][recurrence-doc] | - 透過 [週期] 觸發程序，依照指定的排程執行邏輯應用程式 (範圍從基本到複雜週期)。 <p>- 透過 [延遲] 動作，在指定的持續時間內暫停邏輯應用程式。 <p>- 透過 [延遲直到] 動作，將邏輯應用程式暫停直到指定的日期和時間為止。 | [@no__t 1API 圖示 @ no__t-2**HTTP**][http-doc] | 利用適用於 HTTP、HTTP + Swagger 以及 HTTP + Webhook 的觸發程序和動作，透過 HTTP 與任何端點通訊。 | 
| [@no__t 1API 圖示 @ no__t-2**要求**][http-request-doc] | - 透過 [要求] 觸發程序，讓邏輯應用程式得以從其他應用程式或服務呼叫、在 Event Grid 資源事件上觸發，或在 Azure 資訊安全中心警示的回應上觸發。 <p>- 透過 [回應] 動作，將回應傳送到應用程式或服務。 | [@no__t 1API 圖示 @ no__t-2**批次**][batch-doc] | - 透過 [批次訊息] 觸發程序，分批處理訊息。 <p>- 透過 [將訊息傳送至批次] 動作，呼叫具有現有批次觸發程序的邏輯應用程式。 | 
| [![API 圖示 @ no__t-2**Azure Functions**][azure-functions-doc] | 呼叫 Azure 函式，以從邏輯應用程式執行自訂程式碼片段 (C# 或 Node.js)。 | [@no__t 1API 圖示 @ no__t-2**AZURE API 管理**][azure-api-management-doc] | 呼叫透過 Azure API 管理控管和發佈的自有 API 所定義的觸發程序和動作。 | 
| [@no__t 1API 圖示 @ no__t-2**Azure App 服務**][azure-app-services-doc] | 呼叫 API Apps 或 Web Apps (裝載於 Azure App Service)。 如果包含 Swagger，這些應用程式所定義的動作與觸發程序看起來像是任何其他第一級觸發程序和動作。 | [@no__t 1API 圖示 @ no__t-2**Azure @ no__t-4Logic Apps**][nested-logic-app-doc] | 呼叫透過 [要求] 觸發程序啟動的其他邏輯應用程式。 | 
||||| 
...
|   |   |   |   | 
|---|---|---|---| 
| [![Built 圖示 @ no__t-2**條件**][condition-doc] | 根據條件為 true 或 false，評估條件並執行不同的動作。 | [![Built-in 圖示 @ no__t-2**For each**][for-each-doc] | 對陣列中的每個項目執行相同的動作。 | 
| [![Built-in 圖示 @ no__t-2**Scope**][scope-doc] | 將動作分成數個「範圍」，這些範圍會在範圍中的動作執行完成後取得自己的狀態。 | [![Built-in 圖示 @ no__t-2**Switch**][switch-doc] | 將動作分成數個「案例」，這些案例會獲派唯一值 (預設案例除外)。 僅執行其指派值符合運算式、物件或語彙基元結果的案例。 如果沒有相符項目存在，請執行預設案例。 | 
| [![Built-in 圖示 @ no__t-2**Terminate**][terminate-doc] | 停止正在執行的邏輯應用程式工作流程。 | [![Built-in 圖示 @ no__t-2**直到**][until-doc] | 重複動作，直到符合指定的條件或某些狀態已改變為止。 | 
||||| 
...
| [![Built-in 圖示 @ no__t-2**變數**][variables-doc] | 執行變數相關作業： <p>- **附加到陣列變數**：插入一個值，作為變數所儲存陣列中的最後一個項目。 <br>- **附加到字串變數**：插入一個值，作為變數所儲存字串中的最後一個字元。 <br>- **遞減變數**：依常數值遞減變數。 <br>- **遞增變數**：依常數值遞增變數。 <br>- **初始化變數**：建立變數，並宣告其資料類型和初始值。 <br>- **設定變數**：將不同的值指派給現有的變數。 |
|  |  | 
...
|   |   |   |   | 
|---|---|---|---| 
| [![API 圖示 @ no__t-2**Azure 服務匯流排**][azure-service-bus-doc] | 利用 Logic Apps 中最常用的連接器，管理非同步訊息、工作階段和主題訂用帳戶。 | [![API 圖示 @ no__t-2**SQL Server**][sql-server-doc] | 連線到內部部署 SQL Server 或雲端中的 Azure SQL Database，以便管理記錄、執行預存程序，或執行查詢。 | 
| [@no__t 1API 圖示 @ no__t-2**Office 365 @ no__t-4Outlook**][office-365-outlook-doc] | 連線到 Office 365 電子郵件帳戶，以便建立和管理電子郵件、工作、行事曆事件和會議、連絡人、要求等等。 | [@no__t 1API 圖示 @ no__t-2**Azure Blob @ no__t-4Storage**][azure-blob-storage-doc] | 連線到儲存體帳戶，以便建立和管理 blob 內容。 | 
| [![API 圖示 @ no__t-2**SFTP**][sftp-doc] | 連線到可從網際網路存取的 SFTP 伺服器，以便使用您的檔案和資料夾。 | [@no__t 1API 圖示 @ no__t-2**SharePoint @ no__t-4Online**][sharepoint-online-doc] | 連線到 SharePoint Online，以便管理檔案、附件、資料夾等等。 | 
| [@no__t 1API 圖示 @ no__t-2**Dynamics 365 @ no__t-4CRM Online**][dynamics-365-doc] | 連線到 Dynamics 365 帳戶，以便建立和管理記錄、項目等等。 | [@no__t 1API 圖示 @ no__t-2**FTP**][ftp-doc] | 連線到可從網際網路存取的 FTP 伺服器，以便使用您的檔案和資料夾。 | 
| [@no__t 1API 圖示 @ no__t-2**Salesforce**][salesforce-doc] | 連線到 Salesforce 帳戶，以便建立和管理記錄、作業、物件等項目。 | [@no__t 1API 圖示 @ no__t-2**Twitter**][twitter-doc] | 連線到 Twitter 帳戶，以便管理推文、追蹤者、您的時間軸等等。 將您的推文儲存到 SQL、Excel 或 SharePoint。 | 
| [![API 圖示 @ no__t-2**Azure 事件中樞**][azure-event-hubs-doc] | 透過事件中樞取用和發佈事件。 例如，透過事件中樞從邏輯應用程式取得輸出，然後將輸出傳送給即時分析提供者。 | [![API 圖示 @ no__t-2**Azure Event**</br>**方格**][azure-event-grid-doc] | 當 Azure 資源或第三方資源變更時，監視 Event Grid 所發佈的事件。 | 
|||||
...
|   |   |   |   |   | 
|---|---|---|---|---| 
| ![API 圖示][biztalk-server-icon]<br/>**BizTalk**</br> **伺服器** | [@no__t 1API 圖示 @ no__t-2 檔案 **@ no__t-4 系統**][file-system-doc] | [@no__t 1API 圖示 @ no__t-2**IBM DB2**][ibm-db2-doc] | [@no__t 1API 圖示 @ no__t-2**IBM**</br> **Informix**][ibm-informix-doc] | ![API 圖示][mysql-icon]<br/>**MySQL** | 
| [![API 圖示 @ no__t-2**Oracle DB**][oracle-db-doc] | ![API 圖示][postgre-sql-icon]<br/>**PostgreSQL** | [@no__t 1API 圖示 @ no__t-2**SharePoint @ no__t-4 伺服器**][sharepoint-server-doc] | [@no__t 1API 圖示 @ no__t-2**SQL @ no__t-4 伺服器**][sql-server-doc] | ![API 圖示][teradata-icon]<br/>**Teradata** | 
|||||
...
|   |   |   |   | 
|---|---|---|---| 
| [@no__t 1API 圖示 @ no__t-2**AS2 @ no__t-4 解碼**][as2-doc] | [@no__t 1API 圖示 @ no__t-2**AS2 @ no__t-4 編碼**][as2-doc] | [@no__t 1API 圖示 @ no__t-2**EDIFACT @ no__t-4 解碼**][edifact-decode-doc] | [@no__t 1API 圖示 @ no__t-2**EDIFACT @ no__t-4 編碼**][edifact-encode-doc] | 
| [![API 圖示 @ no__t-2 一般檔案 **@ no__t-4 解碼**][flat-file-decode-doc] | [@no__t 1API 圖示 @ no__t-2 一般檔案 **@ no__t-4 編碼**][flat-file-encode-doc] | [@no__t 1API 圖示 @ no__t-2**整合 @ no__t-4account**][integration-account-doc] | [@no__t 1API 圖示 @ no__t-2**液體**</br> 個**轉換**][json-liquid-transform-doc] | 
| [@no__t 1API 圖示 @ no__t-2**X12 @ no__t-4 解碼**][x12-decode-doc] | [@no__t 1API 圖示 @ no__t-2**X12 @ no__t-4 編碼**][x12-encode-doc] | [@no__t 1API 圖示 @ no__t-2**XML**</br>**轉換**][xml-transform-doc] | [@no__t 1API 圖示 @ no__t-2**XML <br/>validation**][xml-validate-doc] |  
||||| 
...
|   |   |   | 
|---|---|---| 
| [@no__t 1API 圖示 @ no__t-2**IBM 3270**][ibm-3270-doc] | [@no__t 1API 圖示 @ no__t-2**IBM MQ**][ibm-mq-doc] | [@no__t 1API 圖示 @ no__t-2**SAP**][sap-connector-doc] |
|||| 
...
[azure-event-hubs-doc]: ./connectors-create-api-azure-event-hubs.md "連接到 Azure 事件中樞。在邏輯應用程式與事件中樞之間接收和傳送事件"
...
[microsoft-translator-doc]: ./connectors-create-api-microsofttranslator.md "連接到 Microsoft Translator。翻譯文字、偵測語言及更多" 
[office-365-outlook-doc]: ./connectors-create-api-office365-outlook.md "連接到您的 Office 365 帳戶。傳送和接收電子郵件、管理您的行事曆和連絡人等等"
[office-365-users-doc]: ./connectors-create-api-office365-users.md 
[office-365-video-doc]: ./connectors-create-api-office365-video.md "取得影片資訊、影片清單和頻道，以及 Office 365 影片的播放 URL"
[onedrive-doc]: ./connectors-create-api-onedrive.md "連接到您的個人 Microsoft OneDrive。上傳、刪除、列出檔案等"
[onedrive-for-business-doc]: ./connectors-create-api-onedriveforbusiness.md "連接到您的商務 Microsoft OneDrive。上傳、刪除、列出您的檔案等等"
[oracle-db-doc]: ./connectors-create-api-oracledatabase.md "連線至 Oracle 資料庫來新增、插入、刪除資料列等等"
[outlook.com-doc]: ./connectors-create-api-outlook.md "連接到您的 Outlook 信箱。管理您的電子郵件、行事曆、連絡人等等"
[project-online-doc]: ./connectors-create-api-projectonline.md "連接至 Microsoft Project Online。管理您的專案、工作、資源等"
...
[sftp-doc]: ./connectors-create-api-sftp.md "連接到您的 SFTP 帳戶。上傳、取得、刪除檔案等等"
...
[wunderlist-doc]: ./connectors-create-api-wunderlist.md "連接到 Wunderlist。管理您的工作和待辦事項清單、讓您的生活保持同步，以及更多工具"
