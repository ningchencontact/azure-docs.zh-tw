---
title: 連線至 IBM DB2-Azure Logic Apps
...
1. 在搜尋方塊中，輸入「db2」作為篩選條件。 例如，在 [動作] 清單中，選取下列動作：**取得資料表 （預覽）**
...
| **伺服器** | 是 | DB2 伺服器的位址或別名冒號連接埠號碼，例如「myDB2server.cloudapp.net:50000」 <p><p>**注意**：這個值是字串，代表 TCP/IP 位址或別名，在 IPv4 或 IPv6 格式，後面接著冒號和 TCP/IP 通訊埠編號。 |
| **資料庫** | 是 | 資料庫的名稱 <p><p>**注意**：此值就代表 DRDA 關聯式資料庫名稱 (RDBNAM) 的字串： <p>- DB2 for z/OS 接受 16 個位元組的字串 (其中，此資料庫也稱為「IBM DB2 for z/OS」位置)。 <br>- DB2 for i 接受 18 個位元組的字串 (其中，此資料庫也稱為「IBM DB2 for i」關聯式資料庫)。 <br>- DB2 for LUW 接受 8 個位元組的字串。 |
| **使用者名稱** | 是 | 資料庫的使用者名稱 <p><p>**注意**：這個值是的字串，其長度以特定的資料庫為基礎： <p><p>- DB2 for z/OS 接受 8 個位元組的字串。 <br>- DB2 for i 接受 10 個位元組的字串。 <br>- DB2 for Linux 或 UNIX 接受 8 個位元組的字串。 <br>- DB2 for Windows 接受 30 個位元組的字串。 |
| **密碼** | 是 | 資料庫的密碼 |
||||
...
| **連線名稱** | 是 | 連線的名稱，例如「MyLogicApp-DB2-connection」 | 
| **伺服器** | 是 | DB2 伺服器的位址或別名冒號連接埠號碼，例如「myDB2server:50000」 <p><p>**注意**：這個值是字串，代表 TCP/IP 位址或別名，在 IPv4 或 IPv6 格式，後面接著冒號和 TCP/IP 通訊埠編號。 |
| **資料庫** | 是 | 資料庫的名稱 <p><p>**注意**：此值就代表 DRDA 關聯式資料庫名稱 (RDBNAM) 的字串： <p>- DB2 for z/OS 接受 16 個位元組的字串 (其中，此資料庫也稱為「IBM DB2 for z/OS」位置)。 <br>- DB2 for i 接受 18 個位元組的字串 (其中，此資料庫也稱為「IBM DB2 for i」關聯式資料庫)。 <br>- DB2 for LUW 接受 8 個位元組的字串。 |
| **Authentication** | 是 | 連線的驗證類型，例如「基本」 <p><p>**注意**：選取此值，從清單中，其中包含基本或 Windows (Kerberos)。 |
| **使用者名稱** | 是 | 資料庫的使用者名稱 <p><p>**注意**：這個值是的字串，其長度以特定的資料庫為基礎： <p><p>- DB2 for z/OS 接受 8 個位元組的字串。 <br>- DB2 for i 接受 10 個位元組的字串。 <br>- DB2 for Linux 或 UNIX 接受 8 個位元組的字串。 <br>- DB2 for Windows 接受 30 個位元組的字串。 |
| **密碼** | 是 | 資料庫的密碼 |
| **閘道器** | 是 | 所安裝內部部署資料閘道的名稱 <p><p>**注意**：選取此值，從清單中，其中包含您的 Azure 訂用帳戶和資源群組內的所有已安裝的資料閘道。 |
||||
...
1. 指定所有必要屬性 (*) 的值。 選取資料表之後，此動作就會顯示該資料表中記錄特有的相關屬性。
   ...
   | **區域 ID** | 是 | 所要新增區域的識別碼，例如「99999」 |
   | **區域的描述** | 是 | 所要新增區域的說明，例如「Area 99999」 |
   | **區域 ID** | 是 | 所要新增區域的識別碼，例如「102」 |
   |||| 
...
1. 指定所有必要屬性 (*) 的值。 選取資料表之後，此動作就會顯示該資料表中記錄特有的相關屬性。
   ...
   | **區域 ID** | 是 | 新區域的識別碼，例如「99999」 |
   | **區域的描述** | 是 | 新區域的說明，例如「Updated 99999」 |
   | **區域 ID** | 是 | 新區域的識別碼，例如「102」 |
   ||||
...
技術的詳細資訊，例如觸發程序、 動作和限制，如所述的連接器的 OpenAPI (以前稱為 Swagger) 檔案，請參閱 <<c0> [ 連接器的參考頁面](/connectors/db2/)。
