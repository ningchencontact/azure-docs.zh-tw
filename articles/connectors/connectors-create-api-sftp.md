...
SFTP 連接器會處理所檔案*50 MB 或更小*，並且不支援[訊息區塊處理](../logic-apps/logic-apps-handle-large-messages.md)。 對於較大的檔案，使用[SFTP-SSH 連接器](../connectors/connectors-sftp-ssh.md)。 SFTP 連接器與 SFTP-SSH 連接器之間的差異，請檢閱[比較 SFTP-SSH 和 SFTP](../connectors/connectors-sftp-ssh.md#comparison) SFTP-SSH 文件中。
...
## <a name="how-sftp-triggers-work"></a>SFTP 觸發工作的方式

SFTP 觸發程序工作輪詢 SFTP 檔案系統，並尋找任何自上次輪詢後已變更的檔案。 某些工具可讓您在檔案變更時保留時間戳記。 在這些情況下，您必須停用此功能，以便讓您的觸發程序可以運作。 以下是一些常見的設定：
