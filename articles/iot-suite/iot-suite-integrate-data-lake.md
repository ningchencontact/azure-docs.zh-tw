...
* [部署 Azure Data Lake Store](/data-lake-store/data-lake-store-get-started-portal)
  ...
  * 在您的帳戶中[建立資料夾](/data-lake-store/data-lake-store-get-started-portal#createfolder)，並將它命名為 "streaming"。
...
1. 在 Azure 入口網站中，按一下 [Cloud Shell] 按鈕。
    ...
    ![入口網站啟動圖示](media/iot-suite-integrate-data-lake/portal-launch-icon.png)
...
1. 按一下 [建立資源]，從 Marketplace 選取 [物聯網]，然後按一下 [串流分析作業]。
    ...
    ![新增串流分析作業](media/iot-suite-integrate-data-lake/new-stream-analytics-job.png)
...
1. 按一下頁面底部的 [新增] 。
    ...
    ![建立串流分析作業](media/iot-suite-integrate-data-lake/create-stream-analytics-job.png)
...
1. 在 [概觀] 頁面上，按一下 [輸入]。
    ...
    ![[概觀] 頁面](media/iot-suite-integrate-data-lake/stream-analytics-overview.png)

1. 按一下 [新增串流輸入]，然後從下拉式清單中選取 [IoT 中樞]。

    ![新增輸入](media/iot-suite-integrate-data-lake/stream-analytics-add-input.png)

1. 在 [新的輸入] 索引標籤上，輸入 **IoTHub** 作為 [輸入別名]。

1. 從 [取用者群組] 下拉式清單中，選取您稍早建立的取用者群組。 我們在此處是使用 [streamanalyticsjob]。

    ![選取輸入](media/iot-suite-integrate-data-lake/stream-analytics-new-input.png)

1. 按一下 [檔案] 。

1. 在 [概觀] 頁面上，按一下 [輸出]。

    ![新增 Data Lake Store](media/iot-suite-integrate-data-lake/stream-analytics-overview-2.png)

1. 按一下 [新增]，然後從下拉式清單中選取 [Data Lake Store]。

    ![新增輸出](media/iot-suite-integrate-data-lake/stream-analytics-output.png)
...
1. 在 [路徑前置詞模式] 欄位中，輸入 **/streaming/{date}/{time}**。 保留 YYYY/MM/DD 的預設日期格式及 HH 的時間格式。
    ...
    ![提供資料夾結構](media/iot-suite-integrate-data-lake/stream-analytics-new-output.png)

1. 按一下 [授權]。

    您必須以 Data Lake Store 授權，以提供串流分析作業寫入檔案系統的存取權。

    ![針對 Data Lake Store 授權串流分析](media/iot-suite-integrate-data-lake/stream-analytics-out-authorize.png)
...
1. 在查詢編輯器中，使用您先前所定義的值來取代 [YourOutputAlias] 和 [YourInputAlias] 預留位置。
    ...
    ![串流分析查詢](media/iot-suite-integrate-data-lake/stream-analytics-query.png)
...
1. 在 [概觀] 索引標籤上，按一下 [啟動]。
    ...
    ![啟動串流分析作業](media/iot-suite-integrate-data-lake/stream-analytics-start.png)
...
1. 按一下 [啟動] 。
    ...
    ![挑選自訂日期](media/iot-suite-integrate-data-lake/stream-analytics-start-custom.png)

    等候作業進入執行狀態；如果您看到錯誤，則可能是來自您的查詢，請務必確認語法正確。

    ![工作正在執行](media/iot-suite-integrate-data-lake/stream-analytics-running.png)
...
1. 在 [資料總管] 中，向下切入至 **/streaming** 資料夾。 您將看到使用 YYYY/MM/DD/HH 格式建立的資料夾。
    ...
    ![探索串流資料](media/iot-suite-integrate-data-lake/data-lake-store-data-explorer.png)

    您將會看到以每小時一個檔案的頻率產生的 json 檔案。

    ![探索串流資料](media/iot-suite-integrate-data-lake/data-lake-store-file-preview.png)
