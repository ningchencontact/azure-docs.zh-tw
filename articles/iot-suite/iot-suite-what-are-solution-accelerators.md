...
ms.locfileid: "33779917"
---
# <a name="what-are-the-iot-solution-accelerators"></a>IoT 解決方案加速器是什麼？

Azure IoT 解決方案加速器是一組解決方案，其優點是：

* 快速部署
* 協助您快速入門
* 可以自訂以符合特定需求

解決方案加速器皆根據相同原則和目標而設計。

下列影片會提供遠端監視解決方案加速器的概觀：
...
## <a name="solution-accelerators-overview"></a>解決方案加速器概觀

解決方案加速器是常見 IoT 解決方案模式的開放原始碼實作，您可以使用您的訂用帳戶將這些模式部署到 Azure。 每個解決方案加速器都會結合自訂程式碼和 Azure 服務，來實作特定的 IoT 案例。 您可以自訂任何案例以符合特定需求。 這些案例包括：
...
每個解決方案加速器都是可以使用模擬或實體裝置建置的完整端對端實作，藉以產生遙測資料。 您可以使用解決方案加速器作為解決方案的加速器，來完成下列作業：
...
當您部署解決方案加速器時，佈建程序會設定許多 Azure 服務。 下表顯示解決方案加速器中使用的服務：
...
## <a name="whats-new-in-solution-accelerators"></a>解決方案加速器的新功能是什麼？

Microsoft 正在將解決方案加速器更新為最新的微服務型架構。 下表顯示解決方案加速器的目前狀態：
...
下一節會說明微服務型解決方案加速器的新功能：

### <a name="microservices"></a>微服務

新版的遠端監視解決方案加速器會使用微服務架構。 此解決方案加速器結合多種微服務，例如 *IoT 中樞管理員*和*存放管理員*。 每個微服務的 Java 和 .NET 版本皆可下載，並隨附於相關的開發人員文件中。 如需有關微服務的詳細資訊，請參閱[遠端監視架構](iot-suite-remote-monitoring-sample-walkthrough.md)。
...
* [探索遠端監視解決方案](iot-suite-remote-monitoring-explore.md)。
* [預測性維護解決方案加速器概觀](iot-suite-predictive-overview.md)。
* [開始使用連線處理站解決方案加速器](iot-suite-connected-factory-overview.md)。
