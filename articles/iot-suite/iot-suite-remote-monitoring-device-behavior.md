...
[了解裝置模型結構描述](iot-suite-remote-monitoring-device-schema.md)一文說明了定義模擬裝置模型的結構描述。 該文參考兩種類型的 JavaScript 檔案，這兩種類型都可以實作模擬裝置的行為：
...
裝置模型結構描述的 [模擬](iot-suite-remote-monitoring-device-schema.md#simulation) 區段會定義模擬裝置的內部狀態：
...
裝置模型結構描述的 [CloudToDeviceMethods](iot-suite-remote-monitoring-device-schema.md#cloudtodevicemethods) 區段會定義模擬裝置回應的方法。
...
[建立模擬的裝置](iot-suite-remote-monitoring-test.md)一文會向您示範如何在本機執行裝置模擬服務。 在本機執行服務可以更輕鬆地為您模擬的裝置偵錯，再將其部署至雲端。
...
現在您已經學會如何指定模擬裝置的行為，建議的下一個步驟是了解如何[建立模擬的裝置](iot-suite-remote-monitoring-test.md)。
