...
* [部署遠端監視預先設定解決方案](../iot-accelerators/iot-accelerators-remote-monitoring-deploy.md)
...
1. 選取 [建立資源] > [物聯網] > [時間序列深入解析]。
    ...
    ![新增時間序列深入解析](media/iot-suite-time-series-insights/new-time-series-insights.png)
...
1. 若要建立時間序列深入解析環境，請使用下表中的值：
    ...
    ![建立時間序列深入解析](media/iot-suite-time-series-insights/new-time-series-insights-create.png)
...
1. 選取左側的 [事件來源]。
    ...
    ![檢視事件來源](media/iot-suite-time-series-insights/time-series-insights-event-sources.png)

1. 按一下 [新增] 。

    ![新增事件來源](media/iot-suite-time-series-insights/time-series-insights-event-sources-add.png)
...
1. 若要設定您的 IoT 中樞作為新的事件來源，請使用下表中的值：
    ...
    ![建立事件來源](media/iot-suite-time-series-insights/time-series-insights-event-source-create.png)
...
1. 按一下 [移至 環境]，即會開啟時間序列深入解析總管 Web 應用程式。
    ...
    ![時間序列深入解析總管](media/iot-suite-time-series-insights/time-series-insights-environment.png)

1. 在時間選取範圍面板中，從快速時間功能表中選取 [過去 12 小時]，然後按一下 [搜尋]。

    ![時間序列深入解析總管的搜尋](media/iot-suite-time-series-insights/time-series-insights-search-time.png)

1. 在左側的字詞面板中，選取 [量值] 值 **temperature** 和 [分割依據] 值 **iothub-connection-device-id**。

    ![時間序列深入解析總管的查詢](media/iot-suite-time-series-insights/time-series-insights-query1.png)

1. 以滑鼠右鍵按一下圖表，然後選取 [探索事件]。

    ![時間序列深入解析總管的事件](media/iot-suite-time-series-insights/time-series-insights-explore-events.png)

1. 事件會以表格式格式呈現於方格中。

    ![時間序列深入解析總管的表格](media/iot-suite-time-series-insights/time-series-insights-table.png)

1. 按一下檢視方塊檢視按鈕。

    ![時間序列深入解析總管的檢視方塊](media/iot-suite-time-series-insights/time-series-insights-explorer-perspective.png)

1. 按一下 [新增] 以在檢視方塊中建立新的查詢。

    ![時間序列深入解析總管的新增查詢](media/iot-suite-time-series-insights/time-series-insights-new-query.png)

1. 選取 [過去 12 小時] 的快速時間、[量值] 的 **Humidity**，以及 [分割依據] 的 **iothub-connection-device-id**。

    ![時間序列深入解析總管的查詢](media/iot-suite-time-series-insights/time-series-insights-query2.png)

1. 按一下檢視方塊檢視按鈕，以檢視裝置計量的儀表板。

    ![時間序列深入解析總管的儀表板](media/iot-suite-time-series-insights/time-series-insights-dashboard.png)

## <a name="next-steps"></a>後續步驟

若要深入了解如何在時間序列深入解析中探索和查詢資料，請參閱 [Azure 時間序列深入解析總管](https://docs.microsoft.com/azure/time-series-insights/time-series-insights-dashboard.png)。
