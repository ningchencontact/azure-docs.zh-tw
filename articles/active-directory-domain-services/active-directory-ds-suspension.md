...
ms.locfileid: "39126088"
---
# <a name="suspended-domains"></a>暫時停權的網域
當 Azure AD Domain Services 有很長的一段時間無法服務受控網域時，該受控網域就會進入暫時停權狀態。 本文會說明受控網域被暫時停權的原因，以及如何修復被暫時停權的網域。
...
上圖概述 Azure AD Domain Services 受控網域可能會處於的狀態。
...
如果有一或多個問題需要系統管理員採取動作，受控網域即會處於**需要注意**狀態。 您受控網域的健康情況頁面在此狀態下將會列出一或多個警示。 例如，如果您已針對虛擬網路設定受限的 NSG，Microsoft 可能會無法更新和監視您的受控網域。 這個無效設定會導致警示產生，並使您的受控網域進入「需要注意」狀態。
...
在某些情況下 (例如，如果您的網路設定無效)，可能會無法連線到您受控網域的網域控制站。 當您的受控網域處於此狀態時，Microsoft 無法保證它能定期被監視、修補、更新或備份。

* 您的受控網域會處於健康情況不良的狀態，而持續進行的健康情況監視可能會停止，直到警示解決為止。
* 您受控網域的網域控制站可能不會被修補或更新。
* 來自 Azure Active Directory 的變更可能無法同步處理到您的受控網域。
* 如果可行，可能會對您的受控網域進行備份。
* 如果您解決了影響受控網域的警示，可能就可以將您的受控網域還原為「執行中」狀態。
* 針對 Microsoft 無法連線到您網域控制站的情況，系統會針對設定問題觸發重大警示。 如果這類問題未在 15 天內解決，您的受控網域將會進入「暫時停權」狀態。


### <a name="suspended-state"></a>「暫時停權」狀態
受控網域會基於下列因素而進入**暫時停權**狀態：
* 有一或多個重大警示未在 15 天內解決。 重大警示可能是因為設定錯誤而使 Azure AD Domain Services 所需資源的存取被封鎖而導致。
    * 例如，如果受控網域有未解決且已超過 15 天的警示 [AADDS104: 網路錯誤](active-directory-ds-troubleshoot-nsg.md)。
...
* 在對受控網域進行驗證、登入已加入網域的虛擬機器，以及透過 LDAP/LDAPS 連線時會發生失敗。
* 不會再為您的受控網域進行備份。
* 與 Azure AD 的同步處理會停止。
* 解決導致您受控網域處於「暫時停權」狀態的警示，然後連絡支援服務。
* 只有在存在少於 30 天的備份時，支援服務才有可能還原您的受控網域。

受控網域將只會保持暫時停權狀態 15 天。 若要復原您的受控網域，Microsoft 建議您立即解決重大警示。


### <a name="deleted-state"></a>「已刪除」狀態
處於「暫時停權」狀態 15 天的受控網域會被**刪除**。

**您可以預期的情況：**
* 該受控網域的所有資源和備份都會被刪除。
* 您無法還原該受控網域，並需要建立新的受控網域以使用 Azure AD Domain Services。
* 一旦刪除之後，您就不需繼續支付該受控網域的費用。


## <a name="how-do-you-know-if-your-managed-domain-is-suspended"></a>如何得知您的受控網域已被暫時停權？
您會在 Azure 入口網站中的 Azure AD Domain Services [健康情況] 頁面上看到[警示](active-directory-ds-troubleshoot-alerts.md)，宣告該網域已被暫時停權。 網域的狀態也會顯示「暫時停權」。


## <a name="restore-a-suspended-domain"></a>還原暫時停權的網域
若要還原處於「暫時停權」狀態的網域，請完成下列步驟：
...
2. 按一下受控網域。
3. 在左側導覽中，按一下 [健康情況]。
4. 按一下警示。 依據暫時停權的原因而定，該警示識別碼將會是 AADDS503 或 AADDS504。
5. 按一下警示中提供的解決連結，然後遵循步驟來解決警示。
