...
**補救：**您需要 Azure AD PowerShell 才能完成這些步驟。 如需安裝 Azure AD PowerShell 的相關資訊，請參閱[這篇文章](https://docs.microsoft.com/powershell/azure/active-directory/install-adv2?view=azureadps-2.0.)。
...
**補救：**使用下列步驟，在目錄上還原 Domain Services：
...
**補救：**Azure AD Domain Services 可在此服務主體遺失時、設定不正確或遭到刪除時偵測到。 服務會自動重新建立此服務主體。 請在兩個小時之後檢查受控網域的健康情況，以確認服務主體已重新建立。
