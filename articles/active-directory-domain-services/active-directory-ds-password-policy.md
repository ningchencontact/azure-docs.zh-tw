...
在受控網域上建立自訂的 FGPP：
1. 登入您用來管理受控網域的 Windows VM。 如果您沒有 VM，請依照指示來[管理受控網域](active-directory-ds-admin-guide-administer-domain.md)
...
> [!TIP]
> **為自訂 OU 中的使用者帳戶套用自訂密碼原則：** 更細緻的密碼原則只能套用至群組。 若要只針對來自自訂 OU 的使用者設定自訂密碼原則，請建立一個包含該 OU 中使用者的群組。
>
>
