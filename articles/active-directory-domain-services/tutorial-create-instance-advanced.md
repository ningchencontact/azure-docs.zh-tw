...
description: 在本教學課程中，您會了解如何使用 Azure 入口網站建立和設定 Azure Active Directory Domain Services 執行個體並指定進階設定選項。
author: iainfoulds
manager: daveba
...
您可以[使用預設的設定選項建立受控網域][tutorial-create-instance]，以進行網路連線和同步，或手動定義這些設定。 本教學課程將示範如何使用 Azure 入口網站來定義這些進階設定選項，以及建立和設定 Azure AD DS 執行個體。
...
1. 如有需要，請變更或新增收件者，以在 Azure AD DS 受控網域中有警示且需要注意時收到通知。
1. 一切就緒後，選擇 [下一步 - 同步]  。
