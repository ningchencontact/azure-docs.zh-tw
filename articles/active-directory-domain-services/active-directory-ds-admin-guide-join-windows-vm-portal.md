...
建立 Windows 虛擬機器，並將其加入已在其中啟用 Azure AD DS 的虛擬網路，請執行下列動作：
...
2. 若要連線至您的 VM，請開啟下載的 RDP 檔案。 如果您看到系統提示，請選取 [連接]。
3. 在登入提示中，輸入您在建立虛擬機器 (例如 *localhost\mahesh*) 時所指定的**本機系統管理員認證**。
4. 如果您在登入過程中收到憑證警告，請選取 [是] 或 [繼續] 以繼續進行連線。
...
將 Windows Server 虛擬機器加入 Azure AD DS 受控網域，請執行下列動作：
...
6. 系統會提示您輸入認證以便加入網域。 請確定您指定的認證可供「屬於 Azure AD DC 系統管理員群組的使用者」使用。 只有此群組的成員才有權限可以將機器加入受控網域。
...
如果虛擬機器找不到網域，請嘗試下列一或多種方法：

* 確定虛擬機器已連線到您已在其中啟用 Azure AD DS 的相同虛擬網路。 若並未連線，虛擬機器便無法連線到網域，因此無法加入網域。

* 確定虛擬機器所在的虛擬網路會輪流連接到您已啟用 Azure AD DS 的虛擬網路。

* 嘗試使用受控網域的網域名稱來 ping 網域 (例如 ping contoso100.com)。 如果您無法這麼做，請嘗試 ping 頁面上所顯示且您已在其中啟用 Azure AD DS 之網域的 IP 位址 (例如 ping 10.0.0.4)。 如果您能夠 ping 該 IP 位址，但無法 ping 網域，則表示 DNS 的設定可能不正確。 檢查網域的 IP 位址以查看是否設定為虛擬網路的 DNS 伺服器。
...
* 確定您已根據《快速入門指南》中所述的步驟來[啟用密碼同步處理](active-directory-ds-getting-started-password-sync.md)。

* 確定您使用 Azure AD 中所設定的使用者 UPN (例如 bob@domainservicespreview.onmicrosoft.com) 來登入。

* 確定您已如《快速入門指南》中所指定，等候夠久的時間以讓密碼同步處理完成。
