---
title: 使用 Azure 網路監看員管理封包擷取 - REST API | Microsoft Docs
description: 此頁面說明如何使用 Azure REST API 管理網路監看員的封包擷取功能
...
- [**查詢封包擷取的狀態**](#query-packet-capture-status)
...
執行下列指令碼，以傳回虛擬機器。 需要有這項資訊才能啟動封包擷取。
...
下列範例會在虛擬機器上建立封包擷取。  此範例中的參數會設定為允許彈性建立範例。
...
下列範例會停止虛擬機器上的封包擷取。  此範例中的參數會設定為允許彈性建立範例。
...
下列範例會刪除虛擬機器上的封包擷取。  此範例中的參數會設定為允許彈性建立範例。
...
如需從 Azure 儲存體帳戶下載檔案的指示，請參閱[以 .NET 開始使用 Azure Blob 儲存體](../storage/blobs/storage-dotnet-how-to-use-blobs.md)。 另一項可用工具為儲存體總管。 如需有關「儲存體總管」的詳細資訊，請參閱下列連結：[儲存體總管](http://storageexplorer.com/)
