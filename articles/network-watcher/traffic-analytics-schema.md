...
下列查詢可協助您尋找完全流量從內部部署在過去 30 天的記錄。
```
AzureNetworkAnalytics_CL
| where SubType_s == "FlowLog" and FlowStartTime_t >= ago(30d) and FlowType_s == "ExternalPublic"
| project Subnet_s  
```
若要檢視流程的 blob 路徑，在上述查詢中，使用下列查詢：
...
上述查詢會建構要直接存取 blob 的 URL。 具有預留位置的 URL 如下：
...
| SubType_s | FlowLog | 流量記錄的子類型。 使用僅 」 到 FlowLog 」，是產品的內部運作的其他值的 SubType_s |
