...
需要虛擬機器上的 NIC IP 位址。 使用後續命令將連結至虛擬機器的 NIC 取出。 如果您已經知道您想要在虛擬機器上測試的 IP 位址，您可以略過此步驟。
...
執行 `Test-AzureRmNetworkWatcherIPFlow` Cmdlet 來測試流量。 在此範例中，會使用第一個 NIC 上的第一個 IP 位址。
