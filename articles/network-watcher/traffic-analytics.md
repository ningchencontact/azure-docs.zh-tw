...
要保有穩固的安全性、合規性和效能，監視、管理及了解您本身的網路，是很重要的。 了解您本身的環境，是保護和最佳化環境的第一要務。 您通常必須了解網路目前的狀態、哪些人正在連線哪些標的、對網際網路開啟了哪些連接埠、預期的網路行為、異常的網路行為，以及流量的陡增等。
...
流量分析現在以預覽形式提供。 預覽版的功能並沒有與一般版本中的功能相同層級的可用性和可靠性。  在預覽版本中，您可以在下列任何一個區域中使用 NSG 的流量分析：美國中西部、美國東部、美國東部 2、美國中北部、美國中南部、美國中部、美國西部、美國西部 2、西歐、北歐、英國西部、英國南部、澳大利亞東部和澳大利亞東南部。 Log Analytics 工作區必須存在於美國中西部、美國東部、西歐、澳大利亞東南部或英國南部區域中。
...
您必須先重新註冊網路資源提供者，才能在預覽期間使用流量分析。 請按一下以下程式碼方塊中的**試試看**，以開啟 Azure Cloud Shell。 Cloud Shell 會自動將您登入 Azure 訂用帳戶。 Cloud Shell 開啟後，請輸入下列命令以重新註冊網路資源提供者：
...
在入口網站左側選取 [所有服務]，然後在 [篩選] 方塊中輸入 [監視器]。 當 [監視器] 出現在搜尋結果中時，請加以選取。 若要開始瀏覽流量分析及其功能，請選取 [網路監看員]，然後選取 [流量分析 (預覽)]。
...
- 哪些主機傳送或接收了最多流量？
    - 了解哪些主機傳送或接收了最多流量，有助於您識別處理最多流量的主機。
...
- 最常交談的主機配對為何？
    - 預期行為 (如前端-後端通訊) 或異常行為 (如後端-網際網路流量)。
- 允許/封鎖流量的統計資料
    - 主機允許或封鎖大量流量的原因為何？
- 最常交談的主機配對之間最常用的應用程式通訊協定：
    - 此網路上是否允許這些應用程式？
    - 應用程式已正確設定？ 它們是否使用適當的通訊協定進行通訊？ 選取 [最常交談] 下的 [更多詳細資料]，如下圖所示：
...
- 您的環境中最常使用哪個應用程式通訊協定？哪些交談主機配對最常使用該應用程式通訊協定？
    - 此網路上是否允許這些應用程式？
    - 應用程式已正確設定？ 它們是否使用適當的通訊協定進行通訊？ 預期的行為是通用連接埠，例如 80 和 443。 就標準通訊而言，如果顯示了任何不尋常的連接埠，則可能需要為其變更設定。 選取 [最高排名應用程式通訊協定] 下的 [更多詳細資料]，如下圖所示：
...
- 哪些是最常交談的主機？經由哪個 VPN 閘道？透過哪個連接埠？
    - 此模式是否正常？ 選取 [最高排名作用中 VPN 連線] 下的 [更多詳細資料]，如下圖所示：
...
- 每個資料中心的流量分布，例如，資料中心流量最常見的來源、最常與資料中心交談的 Rogue 網路，以及最常用的交談應用程式通訊協定。
    ...
    選取 [Azure 資料中心間的流量分布] 下的 [按一下這裡以查看即時 Geomap]，如下圖所示：
...
- 每個虛擬網路的流量分布、拓撲、虛擬網路流量最常見的來源、最常與虛擬網路交談的 Rogue 網路，以及最常用的交談應用程式通訊協定。
    ...
    選取 [虛擬網路分布] 下的 [按一下這裡以查看 VNet 流量拓撲]，如下圖所示： 
...
- 每個子網路的流量分布、拓撲、子網路流量最常見的來源、最常與子網路交談的 Rogue 網路，以及最常用的交談應用程式通訊協定。
    ...
    - 如果有 Rouge 網路與子網路交談，您可以將其更正，例如，藉由設定 NSG 規則來封鎖 Rogue 網路。
...
### <a name="visualize-the-trends-in-nsg-rule-hits"></a>呈現 NSG 規則命中的趨勢

**尋找**

- 命中率最高的 NSG/規則為何？
- 每個 NSG 最常見的來源與目的地交談配對為何？
