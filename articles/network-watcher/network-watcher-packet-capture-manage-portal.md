...
3. 選取 [新增]  以建立封包擷取。 您可以選取下列屬性的值：
   - 訂用帳戶  ：針對虛擬機器，您想要建立封包擷取的訂用帳戶內。
   - **资源组**：虛擬機器的資源群組。
   - **目標虛擬機器**：您想要建立的封包擷取虛擬機器。
   - **封包擷取名稱**:封包擷取名稱。
   - **儲存體帳戶或檔案**:選取 **儲存體帳戶**，**檔案**，或兩者。 如果您選取 [檔案]  ，擷取將會寫入至虛擬機器內的路徑。
   - **本機檔案路徑**:要在其中儲存封包擷取的虛擬機器上的本機路徑 (時才有效*檔案*已選取)。 路徑必須是有效路徑。 如果您使用 Linux 虛擬機器，路徑必須以 */var/captures* 開頭。
   - **儲存體帳戶**：如果您選取，請選取現有的儲存體帳戶*儲存體帳戶*。 只有已選取 [儲存體]  時才可使用此選項。
   
     > [!NOTE]
     > 儲存封包擷取目前不支援進階儲存體帳戶。

   - **每個封包的最大位元組**:從每個封包所擷取的位元組數目。 如果保留為空白，則會擷取所有位元組。
   - **每個工作階段的最大位元組**:所擷取的位元組總數。 一旦達到此值，封包擷取就會停止。
   - **時間限制 （秒）** :停止封包擷取之前的時間限制。 預設值為 18,000 秒。
   - 篩選 (選擇性)。 選取 [+ 新增篩選] 
     - **通訊協定**：用來篩選封包擷取的通訊協定。 可用的值為 TCP、UDP 和 Any。
     - **本機 IP 位址**:其中本機 IP 位址符合此值來篩選封包擷取的封包。
     - **本機連接埠**:其中的本機連接埠符合此值來篩選封包擷取的封包。
     - **遠端 IP 位址**:其中遠端 IP 位址符合此值來篩選封包擷取的封包。
     - **遠端連接埠**:其中遠端連接埠符合此值來篩選封包擷取的封包。
