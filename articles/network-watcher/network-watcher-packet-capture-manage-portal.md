...
3. 選取 [新增] 以建立封包擷取。 您可以選取下列屬性的值：
   - 訂用帳戶：您想要為其建立封包捕捉的虛擬機器所在的訂用帳戶。
   - **资源组**：虛擬機器的資源群組。
   - **目標虛擬機器**：您想要為其建立封包捕獲的虛擬機器。
   - 封**包捕獲名稱**:封包捕獲的名稱。
   - **儲存體帳戶或**檔案:選取 [**儲存體帳戶**]、[檔案] 或兩者。 如果您選取 [檔案]，擷取將會寫入至虛擬機器內的路徑。
   - **本機檔案路徑**:將儲存封包捕獲之虛擬機器上的本機路徑 (只有在選取 [檔案 ] 時才有效)。 路徑必須是有效路徑。 如果您使用 Linux 虛擬機器，路徑必須以 */var/captures* 開頭。
   - **儲存體帳戶**：如果您選取了 [*儲存體帳戶*], 請選取現有的儲存體帳戶。 只有已選取 [儲存體] 時才可使用此選項。
   
     > [!NOTE]
     > 儲存封包擷取目前不支援進階儲存體帳戶。

   - **每個封包的最大位元組數**:每個封包中所捕獲的位元組數目。 如果保留為空白，則會擷取所有位元組。
   - **每個會話的最大位元組數**:已捕獲的位元組總數。 一旦達到此值，封包擷取就會停止。
   - **時間限制 (秒)** :封包捕獲停止之前的時間限制。 預設值為 18,000 秒。
   - 篩選 (選擇性)。 選取 [+ 新增篩選]
     - **通訊協定**：用來篩選封包擷取的通訊協定。 可用的值為 TCP、UDP 和 Any。
     - **本機 IP 位址**:針對本機 IP 位址符合此值的封包, 篩選封包捕獲。
     - **本機埠**:針對本機埠符合此值的封包, 篩選封包捕獲。
     - **遠端 IP 位址**:針對遠端 IP 位址符合此值的封包, 篩選封包捕獲。
     - **遠端埠**:針對遠端埠符合此值的封包, 篩選封包捕獲。
