...
![Visual Studio 中的新 Edge 專案](./media/stream-analytics-tools-for-visual-studio-edge-jobs/new-stream-analytics-edge-project.png)
...
您可以在 [伺服器總管] 中檢視作業的狀態和作業圖表。 從 [伺服器總管] 的 [串流分析]，展開訂用帳戶以及您部署 Edge 作業的資源群組。 您可以檢視 [已建立] 狀態的 MyASAEdgejob。 展開作業節點並按兩下來開啟作業檢視。
