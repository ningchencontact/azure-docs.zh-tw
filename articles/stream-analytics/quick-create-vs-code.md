---
title: 快速入門 - 在 Visual Studio Code 中建立 Azure 串流分析雲端作業
...
2. 新增下列查詢：
   ...
   ```
## <a name="test-with-sample-data"></a>使用範例資料來測試
在雲端中執行查詢之前，您可以使用本機範例資料來測試您的查詢，以驗證查詢邏輯。

如需詳細資訊，請遵循[使用範例資料來測試](vscode-local-run.md)中的指示。 

 ![在 VS Code 中使用範例資料來測試](./media/quick-create-vs-code/vscode-localrun.gif)
...
5. 以下列值編輯 **BlobStorage**。 將以下未提及的欄位保留為預設值。 使用 CodeLens 協助您從下拉式清單中選取或輸入字串。 
...
![在串流分析總管中列出作業](./media/quick-create-vs-code/list-job.png)
