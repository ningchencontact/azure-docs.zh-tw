...
description: 瞭解時間處理的運作方式, 以及如何解決 Azure 串流分析中的時間處理問題。
author: jasonwhowell
ms.author: zhongc
...
- **浮水印**：事件時間標記, 表示已輸入至串流處理器的點事件。 浮水印讓系統明確指出擷取事件的進度。 依據資料流的特質，內送事件資料永遠不會停止，因此浮水印可指出資料流中某個時間點的進度。
