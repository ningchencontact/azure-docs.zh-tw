...
1. **Ctrl** 可拖曳檢視中的元件到 ViewController.h，並新增這些新的輸出。
    ...
    ```
2. 在 ViewController.h 中，在匯入陳述式後面新增以下的 `#define`。 將 *<輸入您的後端端點\>* 預留位置替換成上一節中用來部署應用程式後端的目的地 URL。 例如：*http://you_backend.azurewebsites.net*。
...
1. 在 **ViewDidLoad**函數中，新增下列內容以具現化 RegisterClient 執行個體，並設定文字欄位的委派。
    ...
    ```
2. 現在於 **AppDelegate.m** 中，移除 `application:didRegisterForPushNotificationWithDeviceToken:` 方法中的所有內容，並使用下列內容取代它，以確定檢視控制器包含從 APN 擷取的最新裝置權杖：
