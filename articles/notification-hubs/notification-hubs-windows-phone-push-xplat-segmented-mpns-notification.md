...
1. 開啟 `MainPage.xaml` 檔案，然後使用下列程式碼來取代名為 `TitlePanel` 和 `ContentPanel` 的 `Grid` 元素：
    ...
    ```
2. 將名為 `Notifications` 的類別新增至專案。 將 `public` 修飾詞新增至類別定義。 然後，將下列 `using` 陳述式新增到新檔案：
    ...
    ```
3. 將下列程式碼複製到新的 `Notifications` 類別中：
...
4. 在 `App.xaml.cs` 專案檔案中，新增下列屬性至 `App` 類別。 以您的通知中樞名稱與先前取得的 *DefaultListenSharedAccessSignature* 連接字串，取代 `<hub name>` 和 `<connection string with listen access>` 預留位置。
...
1. 開啟 App.xaml.cs 檔案，將 `async` 修飾詞新增至 `Application_Launching` 方法，並使用以下程式碼取代您在[開始使用通知中樞]中新增的通知中樞註冊碼：
