...
5. 在 `MainPage.xaml.cs` 檔案中新增以下這一行：

    ```csharp
    using Windows.UI.Popups;
    ```

6. 在 `MainPage.xaml.cs` 檔案中新增以下方法：
...
1. 若要使用 `notifications` 類別根據類別來訂閱，開啟 App.xaml.cs 檔案並更新 `InitNotificationsAsync` 方法。
    ...
    此流程會確保應用程式啟動時會從本機儲存體擷取類別，並要求這些類別的註冊。 您在[開始使用通知中樞][get-started]教學課程中建立了 `InitNotificationsAsync` 方法。
2. 在 `MainPage.xaml.cs` 專案檔案中，將下列程式碼新增至 `OnNavigatedTo` 方法：
