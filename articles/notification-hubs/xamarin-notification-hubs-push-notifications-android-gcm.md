...
5. 將下列 using 陳述式新增至 `MainActivity.cs`：
    ...
    using Android.Util;
    ```
6. 將執行個體變數新增至 `MainActivity.cs*`，以用來在應用程式執行時顯示警示對話方塊：

    ```csharp
    public const string TAG = "MainActivity";
    ```
7. 在 `MainActivity.cs` 中，將下列程式碼新增到 `base.OnCreate(savedInstanceState)` 之後的 `OnCreate`：
