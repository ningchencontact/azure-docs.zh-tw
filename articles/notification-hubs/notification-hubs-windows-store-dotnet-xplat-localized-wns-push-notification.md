...
本教學課程說明如何將當地語系化的通知推送至已向通知中樞服務註冊的行動裝置。 在本教學課程中，您會更新在[教學課程：將通知傳送至特定裝置 (通用 Windows 平台)](notification-hubs-windows-phone-push-xplat-segmented-mpns-notification.md) 中建立的應用程式，以支援下列案例：
...
若要深入了解範本，請參閱[推送範本](notification-hubs-templates-cross-platform-push-messages.md)。

## <a name="update-windows-app-to-support-locale-information"></a>更新 Windows 應用程式以支援地區設定資訊

1. 開啟您為以下教學課程所建立的 Visual Studio 方案：[教學課程：將通知傳送至特定裝置 (通用 Windows 平台)](notification-hubs-windows-phone-push-xplat-segmented-mpns-notification.md)。
2. 更新方案的 `MainPage.xaml` 檔案，以納入地區設定下拉式方塊：
    ...
    ```
3. 在 `Notifications` 類別中，將地區設定參數新增至 `StoreCategoriesAndSubscribe` 和 `SubscribeToCategories` 方法。
    ...
    請呼叫 `RegisterTemplateAsync` 方法，而非呼叫 `RegisterNativeAsync`。 您會註冊範本取決於地區設定的特定通知格式。 您也會提供範本名稱 (例如 "localizedWNSTemplateExample")，因為您可能想要註冊多個範本 (例如，一個用於快顯通知，另一個用於圖格)。 您也需加以命名，以便將其更新或刪除。

    如果有裝置註冊了多個使用相同標記的範本，一個以該標記為目標的傳入訊息，將會使多個通知傳遞至裝置 (每個範本各一個)。 此行為在相同的邏輯訊息必須產生多個視覺化通知時將有所幫助，例如，在一個 Windows 市集應用程式中同時顯示徽章和快顯通知。
4. 新增下列方法，以擷取已儲存的地區設定：
...
5. 在 `MainPage.xaml.cs` 檔案中，更新按鈕點擊處理常式來擷取 [地區設定] 下拉式方塊的現行值，並將其提供給對 `Notifications` 類別的呼叫：
    ...
    ```
6. 最後，在 `App.xaml.cs` 檔案中，更新 `InitNotificationsAsync` 方法來擷取地區設定，並且在訂閱時使用：
