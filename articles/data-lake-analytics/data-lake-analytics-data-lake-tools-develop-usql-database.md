---
title: 使用 U-SQL 資料庫專案開發 Azure Data Lake 的 U-SQL 資料庫 | Microsoft Docs
...
ms.locfileid: "37890712"
---
# <a name="use-u-sql-database-project-to-develop-u-sql-database-for-azure-data-lake"></a>使用 U-SQL 資料庫專案開發 Azure Data Lake 的 U-SQL 資料庫

U-SQL 資料庫提供了結構化的檢視畫面，能用以檢視非結構化的資料，也可在資料表中管理結構化資料，並且提供了通用的中繼資料目錄系統，可供整理結構化資料和自訂程式碼。 資料庫是將這些相關物件編排在一起的概念。
...
Azure Data Lake Tools for Visual Studio 在 2.3.3000.0 之後的版本中，新增了稱為 U-SQL 資料庫專案的專案範本。 若要建立 U-SQL 專案，請依序前往 [檔案] > [新增] > [專案]，您可以在 [Azure Data Lake] > [U-SQL] 節點下方找到 U-SQL 資料庫專案。
...
## <a name="develop-u-sql-database-objects-using-database-project"></a>使用資料庫專案開發 U-SQL 資料庫物件

在 U-SQL 資料庫專案上按一下滑鼠右鍵，再按一下 [新增] > [新增項目]，您可以在 [新增項目精靈] 中找到所有支援的物件類型。 

1.  至於資料表值函式之類的非組件物件，新的 U-SQL 指令碼會在新項目新增後建立。 您可以開始在編輯器中開發該物件的 DDL 陳述式。
2.  至於組件物件，此工具提供了方便使用的 UI 編輯器，可協助您註冊組件及部署 .dll 和其他檔案。 請遵循以下步驟，將組件物件定義新增到 U-SQL 資料庫專案：

1.  將含有 UDO/UDAG/UDF 的 C# 專案的參考，新增到 U-SQL 資料庫專案。
...
2.  在組件設計檢視中，從 [從參考建立組件] 下拉式清單選擇參考組件。
...
3.  新增 [受控相依性] 和 [其他檔案] (若有的話)。 在新增其他檔案時，工具會使用相對路徑來確保能在本機電腦和稍後的組建電腦上找到組件。 @_DeployTempDirectory 是預先定義的變數，能將工具指向建置輸出資料夾。 在建置輸出下方，每個組件下方都有一個以組件名稱命名的子資料夾，您可以在這個子資料夾內找到所有 dll 和其他檔案。 
...
深入了解[如何使用 MSBuild 命令列和 Visual Studio Team Service 建置工作來建置 U-SQL 資料庫專案](data-lake-analytics-cicd-overview.md#build-u-sql-database-project) (英文)。

## <a name="deploy-u-sql-database"></a>部署 U-SQL 資料庫

您可以使用 Visual Studio 或部署 SDK，將 .usqldbpack 套件部署到本機帳戶或 Azure Data Lake Analytics 帳戶。 

### <a name="deploy-u-sql-database-in-visual-studio"></a>在 Visual Studio 中部署 U-SQL 資料庫

您可以在 Visual Studio 中透過 U-SQL 資料庫專案或 .usqldbpack 套件部署 U-SQL 資料庫。

#### <a name="deploy-through-u-sql-database-project"></a>透過 U-SQL 資料庫專案部署

1.  以滑鼠右鍵按一下 U-SQL 資料庫專案，然後選擇 [部署]。
2.  在 [部署 U-SQL 資料庫精靈] 中，選擇要部署資料庫的目的地 [ADLA 帳戶]。 (Local) 帳戶和 ADLA 帳戶兩者均可支援。
3.  [資料庫來源] 會自動填入，並指向專案建置輸出資料夾中的 .usqldbpack 套件
4.  輸入 [資料庫名稱] 以供建立資料庫。 如果目標的 Azure Data Lake Analytics 帳戶已有相同的資料庫，在資料庫專案中定義的所有物件均會建立，但不會重新建立資料庫。
5.  按一下 [提交] 部署 U-SQL 資料庫。 所有資源 (組件和其他檔案) 隨即會上傳，而且包含所有 DDL 陳述式的 U-SQL 作業也會提交。
...
#### <a name="deploy-through-u-sql-database-deployment-package"></a>透過 U-SQL 資料庫部署套件部署

1.  開啟 [伺服器總管]，展開要部署資料庫的目的地 **Azure Data Lake Analytics 帳戶**。
...
### <a name="deploy-u-sql-database-using-sdk"></a>使用 SDK 部署 U-SQL 資料庫
...
- 專案建置：會在編譯 U-SQL 指令碼之前設定參考資料庫環境。 
...
### <a name="how-to-add-u-sql-database-reference"></a>如何新增 U-SQL 資料庫參考
...
2. 從目前的解決方案中的 U-SQL 資料庫專案或 U-SQL 資料庫套件檔案設定資料庫參考。
3. 提供資料庫的名稱。
