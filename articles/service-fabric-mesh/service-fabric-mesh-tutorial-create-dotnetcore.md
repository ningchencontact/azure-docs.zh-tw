...
為了簡單起見，待辦事項項目會儲存在記憶體中的清單內。 請建立待辦事項項目的類別庫，以及用來存放這些項目的清單。 在目前已載入 **todolistapp** 解決方案的 Visual Studio 中，選取 [檔案] > [新增] > [新增專案]。
...
[新增 Service Fabric 服務] 對話方塊隨即出現。 選取 [ASP.NET Core] 專案類型，並確定 [容器 OS] 設定為 [Windows]。 將 [服務名稱] 設定為 **ToDoService**。 按一下 [確定] 以建立 ASP.NET Core 服務。

此時會出現 [新增 ASP.NET Core Web 應用程式] 對話方塊。 在該對話方塊中選取 [API]，然後選取 [確定]，服務的專案即會新增至解決方案。
...
在出現的 [新增項目] 對話方塊中，確定已選取 [類別]，並將 [名稱] 設定為 `DataContext.cs`，然後按一下 [新增]。
...
在**方案總管**中按一下 **Index.cshtml** 檔案的下拉式清單圖示，然後開啟 **Index.cshtml.cs**。
...
> [!IMPORTANT]
> 在下列步驟中，將會修改 YAML 檔案。
> 您必須使用空格 (而非定位字元) 來縮排 service.yaml 檔案中的變數，否則檔案將無法編譯。 Visual Studio 可能會在您建立環境變數時插入定位字元。 請將所有定位字元取代為空格。 雖然您會在**建置**偵錯輸出中看到錯誤，應用程式仍會啟動，但必須等到您將定位字元轉換為空格並重建後才會啟動。 若要確保 service.yaml 檔案中沒有定位字元，您可以透過 [編輯]  > [進階]  > [檢視空白字元]，在 Visual Studio 編輯器中顯示空白字元。
> 請注意，service.yaml 檔案會使用英文的地區設定來處理。 例如，如果您需要使用小數點，便要使用句號，而不是逗號。
...
* 服務名稱 `ToDoService` 位於 `services:` 下方，如上圖中的 (1) 所示。
* 連接埠 `80` 位於 `endpoints:` 下方，如上圖中的 (2) 所示。 專案的連接埠號碼可能會不同。

接著，我們必須在 WebFrontEnd 專案中定義表示服務名稱和連接埠號碼的環境變數，使該專案能夠呼叫後端服務。

在 [方案總管] 中，瀏覽至 [WebFrontEnd] > [服務資源] > [service.yaml]，以定義指定後端服務位址的變數。

在 service.yaml 檔案中的 `environmentVariables:` 下方新增下列變數 (您必須先移除 `#` 以取消註解 `environmentVariables:`)。間距有其重要性，因此請對齊您所新增的變數與 `environmentVariables:` 下方的其他變數。 ApiHostPort 的值務必要符合 ToDoService 的 service.yaml 檔案中先前針對 ToDoServiceListener 顯示的連接埠值。
...
> [!Tip]
> 有兩種方式可指定 `ToDoServiceName` 的值： 
> - 僅指定服務名稱；此方式在 Windows 10 上的偵錯案例中，以及將服務部署至 Azure Service Fabric Mesh 時，都會進行解析。
> - 指定完整名稱 servicename.appname。 此方式僅適用於在 Windows 10 上偵錯時。
> 僅使用服務名稱進行服務解析，是不錯的實務做法。
