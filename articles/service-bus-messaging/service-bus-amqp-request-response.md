...
|略過|int|是|要略過的規則數目。 定義規則清單中的起始索引 (+ 1)。 | 
...
|---------|----------------|--------------|--------------------|  
|statusCode|int|是|HTTP 回應碼 [RFC2616]<br /><br /> 200：確定 – 成功，否則失敗|  
|rules| 對應的陣列|是|規則的陣列。 每個規則都是以對應表示。|
...
|Key|值類型|必要項|值內容|  
|---------|----------------|--------------|--------------------|  
|disposition-status|string|是|已完成<br /><br /> 放棄<br /><br /> 暫止|  
