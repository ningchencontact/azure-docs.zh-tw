...
3. 在「主要」命名空間與「次要」命名空間之間建立配對以取得***別名***。
    ...
    >[!NOTE] 
    > 如果您有[您的 Azure 服務匯流排標準命名空間移轉至 Azure 服務匯流排進階](service-bus-migrate-standard-premium.md)，則您必須使用預先存在的別名 （也就是您服務匯流排標準命名空間連接字串） 來建立災害復原透過設定**PS/CLI**或是**REST API**。
    >
    >
    > 這是因為在移轉期間，Azure 服務匯流排標準命名空間連接字串/DNS 名稱本身會成為您的 Azure 服務匯流排進階命名空間別名。
    >
    > 用戶端應用程式必須利用連線到進階命名空間，災害復原配對中已設定此別名 （也就是 Azure 服務匯流排標準命名空間的連接字串）。
    >
    > 如果您使用入口網站來設定災害復原設定時，入口網站將抽象此警告您。
