...
* 可以避免在服务总线会话上下文中处理消息时出现的不一致，方法是：使用会话状态来存储应用程序的状态（与处理会话的消息序列的进程相关），以及使用与处理接收的消息和更新会话状态相关的事务。 這種一致性功能有時會標示*完全-一次處理*其他廠商的產品，但交易失敗很明顯地會重新傳遞訊息，因此一詞是不完全足夠。
...
* 使用服務匯流排佇列時，儲存在佇列中的每個訊息都包含兩個部分：標頭和主體。 消息的总大小不能超过服务层级支持的最大消息大小。
