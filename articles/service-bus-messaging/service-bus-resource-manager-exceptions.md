...
| 不正確的要求 | 無 | 命名空間 *' namespace name '* 不存在。 | 命名空間不存在於您的 Azure 訂用帳戶中。 | 若要解決此錯誤，請嘗試下列 <ul> <li> 請確定 Azure 訂用帳戶正確。 </li> <li> 確定命名空間存在。 </li> <li> 請確認命名空間名稱是否正確（沒有任何拼寫錯誤或 null 字串）。 </li> </ul> | 
