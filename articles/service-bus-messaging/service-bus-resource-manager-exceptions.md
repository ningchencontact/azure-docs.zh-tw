...
| 錯誤碼 | 錯誤子代碼 | 錯誤訊息 | 說明 | 建議 |
| ---------- | ------------- | ------------- | ----------- | -------------- |
| 未經授權 | None | 次要命名空間上的操作無效。 次要命名空間是唯讀的。 | 已針對次要命名空間執行此作業，這會設定為 readonly 命名空間。 | 請針對主要命名空間重試命令。 深入瞭解[次要命名空間](service-bus-geo-dr.md) |
| 未經授權 | None | MissingToken：找不到授權標頭。 | 當授權具有 null 或不正確的值時，就會發生此錯誤。 | 請確定授權標頭中提及的 token 值正確，且不是 null。 |