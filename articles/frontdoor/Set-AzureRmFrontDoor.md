...
## 描述
**Set-AzureRmFrontDoor** Cmdlet 會更新 Front Door 負載平衡器。 如果未提供輸入參數，將使用來自現有 Front Door 的舊參數。

## 範例

### 範例 1：使用 FrontDoorName 與 ResourceGroupName 更新現有的 Front Door。
```powershell
...
### 範例 2︰使用 PSFrontDoor 物件更新現有的 Front Door。
```powershell
...
### 範例 3︰使用 ResourceId 更新現有的 Front Door
```powershell
...
### -HealthProbeSetting
與此 Front Door 執行個體相關聯的健康情況探查設定。
