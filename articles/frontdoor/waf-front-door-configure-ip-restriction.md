...
用戶端 IP 位址可能與 IP 位址 WAF 觀察不同，例如，當用戶端透過 proxy 存取 WAF 時。 您可以根據 WAF （SocketAddr）所見的用戶端 IP 位址（RemoteAddr）或通訊端 IP 位址，建立 IP 限制規則。 
...
首先，為上一個步驟所建立的原則建立 IP 允許規則。 注意 **--延遲**是必要的，因為規則必須包含至少一個比對條件。 
