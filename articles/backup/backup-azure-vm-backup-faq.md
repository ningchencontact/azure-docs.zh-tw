---
title: 有關使用 Azure 備份備份 Azure Vm 的常見問題
...
資料分割 備份成本與 VM 的成本不同。 深入瞭解[Azure 備份定價](https://azure.microsoft.com/pricing/details/backup/)。
...
資料分割 指定隨選備份作業的保留範圍。 根據預設，若從入口網站觸發，則會保留 30 天。
...
是的。 備份會在機器關機時執行。 復原點會標示為「當機時保持一致」。
...
我們正在為 SAP Hana 備份執行私人預覽, 其 RPO 為15分鐘。 它會以類似 SQL DB 備份的方式建置，並針對 SAP HANA 所認證的協力廠商解決方案使用 backInt 介面。 如果您`AskAzureBackupTeam@microsoft.com`有興趣, 請傳送電子郵件給我們, 並在**Azure vm 中使用 [註冊私人預覽**] 以進行 SAP Hana 備份。
...
排定的備份將會在排程備份時間的2小時內觸發。 針對 ex 如果 100 Vm 的備份開始時間是在上午 2:00, 則每個100VMs 的最大 4:00 am 都會有備份作業正在進行中。 如果排定的備份因中斷而暫停並繼續/重試, 則備份可以在此排程的與2小時時段以外的時間啟動。

### <a name="what-is-the-minimum-allowed-retention-range-for-daily-backup-point"></a>每日備份點的最小允許保留範圍為何？
Azure 虛擬機器備份原則最多可支援7天的最小保留範圍9999天。 任何修改現有 VM 備份原則的時間若少於7天, 將需要更新以符合7天的最低保留範圍。
...
是的。 即使您刪除 VM，還是可以移至保存庫中對應的備份項目，然後從復原點還原。
