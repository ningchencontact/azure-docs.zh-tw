...
|Hyper-v VM 必要條件|-在虛擬機器上執行的整合元件版本應該與 Hyper-v 主機的版本相同。 <br />-針對每個虛擬機器備份，裝載虛擬硬碟檔案的磁片區上需要有可用空間，以允許 Hyper-v 在備份期間有足夠的空間可供差異磁片（.AVHD）。 空間必須至少等於計算**初始磁片大小，\*變換率\*備份** 視窗時間。 如果您在叢集上執行多個備份，您將需要足夠的儲存容量，以配合使用此計算的每個虛擬機器的 Avhd。<br />-若要備份位於執行 Windows Server 2012 R2 的 Hyper-v 主機伺服器上的虛擬機器，虛擬機器應該已指定 SCSI 控制器，即使它並未連接到任何專案。 （在 Windows Server 2012 R2 線上備份中，Hyper-v 主機會在 VM 中裝載新的 VHD，然後在稍後將它卸載。 只有 SCSI 控制器可以支援這種情況，因此，虛擬機器的線上備份需要此服務。  如果沒有此設定，當您嘗試備份虛擬機器時，就會發出事件識別碼10103。）|
...
|使用 SMB 儲存體備份 Vm|-在執行 Hyper-v 的伺服器上開啟自動掛接，以啟用虛擬機器保護。<br />   -停用 TCP 煙囪卸載。<br />-確定所有 Hyper-v machine $ 帳戶都具有特定遠端 SMB 檔案共用的完整許可權。<br />-確定復原至替代位置期間，所有虛擬機器元件的檔案路徑少於260個字元。 如果沒有，則復原可能會成功，但 Hyper-v 將無法掛接虛擬機器。<br />-不支援下列案例：<br />     虛擬機器的某些元件部署在本機磁片區，而某些元件則位於遠端磁片區;儲存位置檔案伺服器的 IPv4 或 IPv6 位址，以及將虛擬機器復原到使用遠端 SMB 共用的電腦。<br />-您必須在每個 SMB 伺服器上啟用檔案伺服器 VSS 代理程式服務-將它新增到 **新增角色及功能** > **選取 伺服器角色** > 檔案**和存放服務** ** >  > 檔案服務** **檔案服務 > ** 檔**伺服器 VSS 代理程式服務**。|
...
6. 在 [**指定短期目標**] > [**保留範圍**] 中，指定要保留磁片資料的時間長度。 在 **[同步處理頻率**] 中，指定應執行增量備份資料的頻率。 或者，您可以啟用 [在**復原點之前**]，而不是選取增量備份的間隔。 啟用這項設定時，MABS 會在每個排程的復原點之前執行快速完整備份。
...
6. 設定完成後，當虛擬機器從某個叢集遷移至另一個叢集時，所有備份都會如預期般繼續進行。 您可以如預期般啟用即時移轉，如下所示：
   ...
   3. 執行下列查詢，以驗證特定虛擬機器的 `PhysicalPathXML` 中的*VMMIdentifier*參數。 以虛擬機器的名稱取代 `VMName`。
...
4. 在 [**選取復原類型**] 畫面上，選取您要還原資料的位置，然後按 **[下一步]** 。
    ...
    - **將虛擬機器復原到任何主機**： MABS 支援替代位置復原（ALR），可將受保護的 hyper-v 虛擬機器順暢地復原到另一部 hyper-v 主機，而與處理器架構無關。 復原到叢集節點的 hyper-v 虛擬機器將不會有高可用性。 如果您選擇此選項，則 [復原嚮導] 會向您顯示用來識別目的地和目的地路徑的其他畫面。
