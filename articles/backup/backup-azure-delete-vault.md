...
description: "本文說明如何刪除復原服務保存庫。 發行項包含疑難排解步驟，當您嘗試刪除保存庫，但不能。"
...
本文說明如何刪除復原服務保存庫在 Azure 入口網站。 如果您在備份保存庫時，它們已轉換成復原服務保存庫。   
...
## <a name="removing-items-from-a-vault-protecting-a-vm"></a>移除保護 VM 的保存庫中的項目
...
2. 在保存庫檢視中，找到 [基本]  窗格。 若要刪除保存庫，不能有任何受保護的項目。 如果**備份項目**或**備份管理伺服器**不會顯示零，您必須先移除這些項目。 如果它包含資料，您無法刪除保存庫。
...
3. 若要開始從保存庫移除受保護的項目，請在保存庫中找到這些項目。 保存庫儀表板中按一下**設定**，然後按一下 **備份項目**開啟該功能表。

    ![從清單中選擇保存庫](./media/backup-azure-delete-vault/open-settings-and-backup-items.png)

    **備份項目**功能表有個別的清單，根據項目類型： Azure 虛擬機器或檔案資料夾 （請參閱映像）。 顯示的預設項目清單類型是 Azure 虛擬機器。 若要檢視保存庫中的檔案資料夾項目清單，請從下拉式選單中選取 [檔案-資料夾]  。
4. 您可以從保護 VM 的保存庫中刪除項目之前，必須先停止該項目的備份作業，並刪除復原點資料。 針對保存庫中的每個項目，執行下列步驟︰

    a. 在**備份項目**功能表上，以滑鼠右鍵按一下項目，並從內容功能表中，選取**停止備份**。

    ![停止備份作業](./media/backup-azure-delete-vault/stop-the-backup-process.png)

    [停止備份] 功能表隨即開啟。

    b. 上**停止備份**功能表上，從**選擇選項**功能表上，選取**刪除備份資料**> 類型的項目名稱 > 按一下**停止備份**.
    ...
    作業完成之後，服務會傳送一則訊息： 備份程序已停止，且已刪除的備份資料。

    c. 刪除清單中的項目後，在 [備份項目] 功能表上按一下 [重新整理]，以查看保存庫中的其餘項目。

      ![刪除備份資料](./media/backup-azure-delete-vault/empty-items-list.png)

      當清單中有任何項目時，捲動到**Essentials** [復原服務保存庫] 功能表中的窗格。 清單中不應該有任何 [備份項目]、[備份管理伺服器] 或 [複寫的項目]。 如果保存庫中仍有項目，請返回步驟三，並選擇其他項目類型清單。  
...
1. 在**備份項目**功能表上，以滑鼠右鍵按一下項目，和內容功能表上按一下**刪除備份資料**。

    ![刪除備份資料](./media/backup-azure-delete-vault/delete-backup-data-menu.png)

    **刪除備份資料** 功能表隨即開啟。
2. 在**刪除備份資料**功能表上，輸入項目的名稱，然後按一下**刪除**。
...
2. 在 [停止保護] 對話方塊中，選取 [刪除受保護的資料]，然後按一下 [停止保護]。
    ...
    若要刪除保存庫，您必須清除 (或刪除) 受保護資料的保存庫。 如果有多個復原點與保護群組中的資料，可能需要幾分鐘才能刪除的資料。 **停止保護**對話方塊會顯示在工作完成時。
...
1. 在 Azure 入口網站中，開啟保存庫儀表板，然後按一下 [設定]  >  > 。
    ...
    ![開啟 [實際執行伺服器] 功能表](./media/backup-azure-delete-vault/delete-production-server.png)

    **實際執行伺服器**功能表隨即開啟，並列出在保存庫中的所有實際執行伺服器。

    ![生產伺服器的清單](./media/backup-azure-delete-vault/list-of-production-servers.png)
2. 在**實際執行伺服器**功能表上，以滑鼠右鍵按一下伺服器，然後按一下**刪除**。
...
1. 開啟保存庫儀表板。
2. 在**Essentials** ] 窗格中，按一下 [**設定**開啟該功能表。

    ![開啟 [設定] 功能表](./media/backup-azure-delete-vault/backup-vault-click-settings.png)
3. 在**設定**功能表上，按一下 **備份基礎結構**。
4. 在**備份基礎結構**功能表上，按一下 **備份管理伺服器**。 [到備份管理伺服器] 功能表隨即開啟。

    ![備份管理伺服器的清單](./media/backup-azure-delete-vault/list-of-backup-management-servers.png)
5. 若要刪除清單中的伺服器，以滑鼠右鍵按一下伺服器的名稱，然後按一下 [刪除] 。
    **刪除** 功能表隨即開啟。
6. 在**刪除**功能表上，提供伺服器的名稱。 如果名稱很長，您可以從的備份管理伺服器的清單中複製並貼上。 然後按一下 [刪除] 。  
