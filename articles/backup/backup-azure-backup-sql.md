...
3. DPM 會顯示開始畫面，其中包含建立 **保護群組**的指引。 单击“下一步”。
4. 選取 [伺服器] 。

    ![選取保護群組類型 - 伺服器](./media/backup-azure-backup-sql/pg-servers.png)
5. 展開要備份之資料庫所在的 SQL Server 電腦。 DPM 顯示可從該伺服器備份的各種資料來源。 展開 [所有 SQL 共用]  並選取要備份的資料庫 (在本例中我們選取 ReportServer$MSDPM2012 和 ReportServer$MSDPM2012TempDB)。 单击“下一步”。
