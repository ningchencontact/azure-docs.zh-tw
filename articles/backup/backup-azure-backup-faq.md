...
號 所有在備份作業取消前傳輸到保存庫的資料都會保留在保存庫中。
...
號 保留原則僅能套用在復原點上。 例如，此影像會顯示在上午12和下午6點執行之備份的保留原則。
...
號 復原最舊或最新時間點所需的時間都相同。 每個復原點的功能就像一個完整的復原點。
