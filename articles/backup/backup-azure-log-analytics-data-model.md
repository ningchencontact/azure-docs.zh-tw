...
| 類別 |文字 |類別的診斷資料推送至 Azure 監視器記錄檔。 一律為 Always AzureBackupReport |
...
| ProtectionGroupName_s |文字 | 保護群組 備份項目名稱中受到保護，SC DPM 和 MABS，如果適用的話|
| SecondaryBackupProtectionState_s |文字 |備份的項目是否啟用次要保護|
| SchemaVersion_s |文字 |結構描述版本，例如**V2** |
...
| SchemaVersion_s |文字 |此欄位代表目前版本的結構描述，它是**V2** |
| State_s |文字 |備份項目關聯物件的目前狀態 (例如，作用中、已刪除) |
| BackupManagementType_s |文字 |執行備份作業的伺服器所屬的提供者類型，例如 IaaSVM、FileFolder |
| BackupItemSourceSize_s |文字 | 前端的大小，備份的項目 |
| BackupManagementServerUniqueId_s |文字 | 如果適用的話，將受到保護，透過的欄位來唯一識別備份管理伺服器備份項目 |
...
|AzureBackupAgentVersion_s     |文字         |備份管理伺服器上的 Azure 備份代理程式版本          |
|BackupManagementServerVersion_s     |文字         |備份管理伺服器的版本|
|BackupManagementServerOSVersion_s    |文字            |備份管理伺服器的 OS 版本|
|BackupManagementServerType_s     |文字         |備份管理伺服器，與 MABS、 SC DPM 的類型|
|BackupManagementServerUniqueId_s     |文字         |可唯一識別備份管理伺服器的欄位       |
| SourceSystem |文字 |目前資料的來源系統 - Azure |
| ResourceId |文字 |正在收集資料的資源識別碼。 例如，復原服務保存庫資源識別碼 |
| SubscriptionId |文字 |正在收集資料的資源 (例如復原服務保存庫) 的訂用帳戶識別碼 。 |
| ResourceGroup |文字 |正在收集資料的資源 (例如復原服務保存庫) 的資源群組 。 |
...
| 類別 |文字 |此欄位代表推送至 Azure 監視器記錄檔的診斷資料的類別目錄，則是 AzureBackupReport |
| 資源 |文字 |這是所收集資料的資源，會顯示復原服務保存庫名稱 |
| ProtectedServerUniqueId_s |文字 |與作業相關聯的受保護伺服器所具備的唯一識別碼 |
| ProtectedContainerUniqueId_s |文字 | 用來識別受保護的容器執行作業的唯一識別碼 |
...
| DiffBackupFormat_s |文字 |v2|格式為差異備份的的 SQL Azure VM 備份的 |
...
| DiffBackupDaysofTheWeek_s |文字 |v2|Sql Azure VM 備份中的差異備份一週天數|
| SourceSystem |文字 ||目前資料的來源系統 - Azure |
| ResourceId |文字 ||正在收集資料的資源識別碼。 例如，復原服務保存庫資源識別碼 |
| SubscriptionId |文字 ||要收集其資料的資源 (例如復原服務保存庫) 的訂用帳戶識別碼 。 |
| ResourceGroup |文字 ||要收集其資料的資源 (例如復原服務保存庫) 的資源群組 。 |
...
| BackupManagementServerUniqueId_s |文字 |v2 |如果適用的話，將受到保護，透過的欄位來唯一識別備份管理伺服器備份項目        |
| SourceSystem |文字 ||目前資料的來源系統 - Azure |
| ResourceId |文字 ||正在收集資料的資源識別碼。 例如，復原服務保存庫資源識別碼 |
| SubscriptionId |文字 ||要收集其資料的資源 (例如復原服務保存庫) 的訂用帳戶識別碼 。 |
| ResourceGroup |文字 ||要收集其資料的資源 (例如復原服務保存庫) 的資源群組 。 |
...
| 欄位 | 資料類型 | 描述 |
| --- | --- | --- |
| ProtectedContainerUniqueId_s |文字 | 欄位來唯一識別受保護的容器 |
| ProtectedContainerOSType_s |文字 |受保護的容器的 OS 類型 |
| ProtectedContainerOSVersion_s |文字 |OS 版本的受保護的容器 |
| AgentVersion_s |文字 |版本號碼的代理程式備份或保護代理程式 （如果是 SC DPM 和 MABS） |
| BackupManagementType_s |文字 |用於執行備份的提供者類型 (例如，IaaSVM、FileFolder) |
| EntityState_s |文字 |受保護伺服器物件的目前狀態 (例如，作用中、已刪除) |
| ProtectedContainerFriendlyName_s |文字 |受保護伺服器的易記名稱 |
| ProtectedContainerName_s |文字 |受保護的容器名稱 |
| ProtectedContainerWorkloadType_s |文字 |例如，IaaSVMContainer 備份的受保護的容器型別 |
| ProtectedContainerLocation_s |文字 |受保護的容器是否位於的內部或在 Azure 中 |
...
| SchemaVersion_s |文字 |此欄位代表目前版本的結構描述，它是**V2** |
| BackupItemUniqueId_s |文字 |用來識別備份的項目與儲存體實體相關的唯一識別碼 |
| BackupManagementServerUniqueId_s |文字 |用來識別儲存體與實體相關的備份管理伺服器的唯一識別碼|
| VaultUniqueId_s |文字 |用來識別保存庫儲存體與實體相關的唯一識別碼|
...
|欄位  |資料類型  | 描述  |
|---------|---------|----------|
|BackupManagmentServerName_s     |文字         |備份管理伺服器的名稱        |
|AzureBackupAgentVersion_s     |文字         |備份管理伺服器上的 Azure 備份代理程式版本          |
|BackupManagmentServerVersion_s     |文字         |備份管理伺服器的版本|
|BackupManagmentServerOSVersion_s     |文字            |備份管理伺服器的 OS 版本|
|BackupManagementServerType_s     |文字         |備份管理伺服器，與 MABS、 SC DPM 的類型|
|BackupManagmentServerUniqueId_s     |文字         |可唯一識別備份管理伺服器的欄位       |
...
| --- | --- | --- |
| StorageUniqueId_s |文字 |用來識別儲存體實體的唯一識別碼 |
| BackupItemType_s |文字 |此磁碟區的慣用儲存體工作負載|
...
| 欄位 | 資料類型 |適用版本 | 描述 |
| --- | --- | --- | --- |
| BackupItemUniqueId_s |文字 |v2|用來識別備份的項目適用於 Vm 的唯一識別碼使用備份 DPM，MABS|
| ProtectedContainerUniqueId_s |文字 |v2|用來識別受保護的容器的 Vm 以外的所有內容的唯一識別碼使用備份 DPM，MABS|
| ProtectedInstanceCount_s |文字 |v2|計數的受保護執行個體相關聯的備份項目或受保護的容器，該日期時間|
...
| 欄位 | 資料類型 | 描述 |
| --- | --- | --- |
| BackupItemUniqueId_s |文字 |用來識別備份的項目適用於 Vm 的唯一識別碼使用備份 DPM，MABS|
| OldestRecoveryPointTime_s |文字 |備份的項目，最舊的復原點的日期時間|
| OldestRecoveryPointLocation_s |文字 |備份項目的最早的復原點的位置|
| LatestRecoveryPointTime_s |文字 |備份項目的最新的復原點的日期時間|
| LatestRecoveryPointLocation_s |文字 |備份項目的最新的復原點的位置|
