...
> [!IMPORTANT]
> Azure 備份所提供的角色僅限於可在 Azure 入口網站中或透過 REST API 執行的動作或復原服務保存庫 PowerShell 或 CLI cmdlet。 在 Azure 備份代理程式用戶端 UI、System Center Data Protection Manager UI 或 Azure 備份伺服器 UI 中執行的動作則非這些角色所能控制。
