...
請檢查檔案共用是否已在相同的復原服務保存庫中受到保護，或最近遭受刪除。
...
刪除檔案共用時，系統會顯示也將一併刪除的備份清單並要求您確認。 無法還原已刪除的檔案共用。
...
在入口網站、PowerShell 或 CLI 中檢視快照集，即可存取 Azure Backup 所產生的所有快照集。 您可以使用[這裡](../storage/files/storage-how-to-use-files-snapshots.md#mount-a-file-share)的程序進行裝載。
