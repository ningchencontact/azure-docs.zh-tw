...
每個 VM 的備份包含 2 個階段，備份作業詳細資料**快照集**階段後面**將資料傳輸至保存庫**階段。<br/>
快照集階段可確保儲存的磁碟以及復原點的可用性**立即還原**且可供根據使用者所設定的快照集保留 5 天的最大值。 將資料傳輸到保存庫以長期保留保存庫中建立的復原點。 將資料傳輸至保存庫只會啟動，快照集階段完成後。

  ![備份作業狀態](./media/backup-azure-arm-vms-prepare/backup-job-status.png)

有兩個**子工作**執行前端的備份作業可以檢查從後端**備份作業**詳細資料刀鋒視窗提供下列：

  ![備份作業狀態](./media/backup-azure-arm-vms-prepare/backup-job-phase.png)

**將資料傳輸至保存庫**階段可能需要好幾天才能完成視磁碟大小而定，每個磁碟和許多其他因素變換。
...
現在使用這項功能，針對相同的 VM 中，兩個備份可以平行執行，但只有一個子工作可以執行在任一階段 （快照集，將資料傳輸到保存庫）。 因此在案例所備份的作業進行中導致失敗的隔天的備份就能避免與此分離的功能。 後續一天的備份可以了快照集時，您已完成**將資料傳輸至保存庫**略過早一天的備份工作是否在進行中狀態。
建立保存庫中的增量復原點將會擷取從建立保存庫中的最後一個復原點的所有客戶流失。 使用者沒有任何成本的影響。
