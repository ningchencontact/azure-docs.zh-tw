...
| EntityState |Text |備份專案物件的目前狀態。 例如, 作用中、已刪除 |
| LastBackupDateTime |Date/Time |所選備份項目的最後一次備份時間 |
| LastBackupState |Text |所選備份專案的上次備份狀態。 例如, 成功、失敗 |
| LastSuccessfulBackupDateTime |Date/Time |所選備份項目最後一次成功備份的時間 |
| ProtectionState |Text |備份專案的目前保護狀態。 例如, Protected、Protectionstopped) |
...
| DayDiff |十進位數字 |篩選資料的日差。 例如, 0 表示當天的資料,-1 表示前一天的資料, 0 和-1 表示目前和前一天的資料  |
| 月 |Text |一年中所選用於篩選資料的月份，月份的計算會從第一天開始，到第 31 天結束 |
| MonthDate | Date |當月結束時，該月份所選用於篩選資料的日期 |
| MonthDiff |十進位數字 |篩選資料的月份差異。 例如, 0 表示當月的資料,-1 表示上個月的資料, 0 和-1 表示目前和上個月的資料 |
| 週 |Text |所選用於篩選資料的週別，一週的計算會從星期日開始，到星期六結束 |
| WeekDate |Date |當週結束時，該週所選用於篩選資料的日期 |
| WeekDiff |十進位數字 |篩選資料的周間差異。 例如, 0 表示目前的周資料,-1 表示上星期的資料, 0 和-1 表示目前和上周的資料 |
...
| BackupStorageDestination |Text |備份儲存體的目的地。 例如, 雲端、磁片  |
| EntityState |Text |工作物件的目前狀態。 例如, 作用中、已刪除 |
| JobFailureCode |Text |由於發生作業失敗而產生的失敗碼字串 |
| JobOperation |Text |執行工作的作業。 例如, 備份、還原、設定備份 |
| JobStartDate |Date |開始執行作業的日期 |
| JobStartTime |Time |開始執行作業的時間 |
| JobStatus |Text |已完成工作的狀態。 例如, Completed、Failed |
...
| RetentionType |Text |保留原則的類型。 例如, 每日、每週、每月、每年 |
...
| YearlyRetentionFormat |Text |年度保留期的設定類型。 例如, 每日以日為基礎, 每週以周為基礎 |
| YearlyRetentionMonthsOfTheYear |Text |一年中所選用於每年保留期的月數 |
| YearlyRetentionTimes |Text |每年保留期的設定日期和時間 |
| YearlyRetentionWeeksOfTheMonth |Text |設定每年保留期的月份周數。 例如, First、Last 等等。 |
...
| 欄位 | 資料類型 | 描述 |
| --- | --- | --- |
| 小時 |Time |一天中的小時。 例如, 1:00:00 PM |
| HourNumber |十進位數字 |一天中的小時數。 例如, 13.00 |
| Minute |十進位數字 |一小時的分鐘數 |
| PeriodOfTheDay |Text |一天中的時間週期位置。 例如, 12-3 AM |
| Time |Time |一天中的時間。 例如, 12:00:01 AM |
