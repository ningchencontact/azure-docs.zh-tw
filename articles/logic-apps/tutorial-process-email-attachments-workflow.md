...
* 下載並安裝<a href="http://storageexplorer.com/" target="_blank">免費 Microsoft Azure 儲存體總管</a>。 此工具可協助您確認儲存體容器已正確設定。
...
1. 您必須先透過下列設定[建立儲存體帳戶](../storage/common/storage-create-storage-account.md#create-a-storage-account)，才能建立儲存體容器：
   ...
   您也可以使用 [Azure PowerShell](../storage/common/storage-quickstart-create-storage-account-powershell.md) 或 [Azure CLI](../storage/common/storage-quickstart-create-storage-account-cli.md)。
  
2. 在 Azure 部署您的儲存體帳戶之後，請取得儲存體帳戶的存取金鑰：

   1. 在儲存體帳戶功能表的 [設定] 下，選擇 [存取金鑰]。 
   2. 在 [預設金鑰] 下找出 **key1**，並找出您的儲存體帳戶名稱。

      ![複製並儲存儲存體帳戶名稱和金鑰](./media/tutorial-process-email-attachments-workflow/copy-save-storage-name-key.png)

   您也可以使用 [Azure PowerShell](https://docs.microsoft.com/powershell/module/azurerm.storage/get-azurermstorageaccountkey) 或 [Azure CLI](https://docs.microsoft.com/cli/azure/storage/account/keys?view=azure-cli-latest.md#az_storage_account_keys_list)。 

3. 建立電子郵件附件的儲存體容器。
   
   1. 在儲存體帳戶功能表的 [概觀] 窗格上，選擇 [服務] 下的 [Blob]，然後選擇 [+ 容器]。

   2. 輸入「附件」作為容器名稱。 在 [公用存取層級] 下選取 [容器 (容器和 Blob 匿名讀取權限)]，然後選擇 [確定]。

   您也可以使用 [Azure PowerShell](https://docs.microsoft.com/powershell/module/azure.storage/new-azurestoragecontainer) 或 [Azure CLI](https://docs.microsoft.com/cli/azure/storage/container?view=azure-cli-latest#az_storage_container_create)。 
...
現在，將儲存體總管連線至儲存體帳戶，以確認邏輯應用程式已將附件正確地儲存為儲存體容器中的 Blob。

1. 開啟 [Microsoft Azure 儲存體總管]。 當儲存體總管提示您連線至 Azure 儲存體時，請選擇 [使用儲存體帳戶名稱和金鑰] > [下一步]。
若未出現提示，請選擇總管工具列上的 [新增帳戶]。

2. 在 [使用名稱和金鑰連結] 下，輸入您的儲存體帳戶名稱和您先前儲存的存取金鑰。 選擇 [下一步] > [連線]。
...
3. 確認您的儲存體帳戶和容器正確地顯示在儲存體總管中：
   ...
   例如︰
      ...
      ![儲存體總管 - 確認儲存體容器](./media/tutorial-process-email-attachments-workflow/storage-explorer-check-contianer.png)

接著，建立從內送電子郵件中移除 HTML 的 [Azure 函式](../azure-functions/functions-overview.md)。

## <a name="create-a-function-to-clean-html"></a>建立會清除 HTML 的函式
...
1. 您必須先透過下列設定[建立函式應用程式](../azure-functions/functions-create-function-app-portal.md)，才能建立函式：
   ...
   如果函式應用程式未在部署之後自動開啟，請在 <a href="https://portal.azure.com" target="_blank">Azure 入口網站</a>中尋找您的應用程式。 在 Azure 主功能表上選擇 [應用程式服務]，然後選取您的函式應用程式。

   ![已建立的函式應用程式](./media/tutorial-process-email-attachments-workflow/function-app-created.png)

   如果 [應用程式服務] 未出現在 Azure 功能表上，請移至 [更多服務]。 在搜尋方塊中尋找 [函式應用程式]，並加以選取。 如需詳細資訊，請參閱[建立函式](../azure-functions/functions-create-first-azure-function.md)。

   您也可以使用 [Azure CLI](../azure-functions/functions-create-first-azure-function-azure-cli.md)，或是 [PowerShell 和 Resource Manager 範本](../azure-resource-manager/resource-group-template-deploy.md)。

2. 在 [函式應用程式] 下展開 **CleanTextFunctionApp**，然後選取 [函式]。 在函式工具列上選擇 [+ 新增函式]。

   ![建立新的函式](./media/tutorial-process-email-attachments-workflow/function-app-new-function.png)

3. 在 [選擇下列範本或移至快速入門] 下，選取 **HttpTrigger - C#** 函式範本。

   ![選取函式範本](./media/tutorial-process-email-attachments-workflow/function-select-httptrigger-csharp-function-template.png)

4. 在 [函式命名] 下，輸入 ```RemoveHTMLFunction```。 在 [HTTP 觸發程序] > [授權層級] 下保留預設的 [函式] 值，然後選擇 [建立]。

   ![函式命名](./media/tutorial-process-email-attachments-workflow/function-provide-name.png)

5. 編輯器開啟之後，請將範本程式碼取代為此程式碼，以移除 HTML 並將結果傳回至呼叫端：
...
確認您的函式運作正常後，請建立邏輯應用程式。 雖然此教學課程說明的是如何建立從電子郵件中移除 HTML 的函式，但 Logic Apps 也會有 **HTML 轉換為文字**的連接器。

## <a name="create-your-logic-app"></a>建立邏輯應用程式

1. 在 Azure 主要功能表上，選擇 [新增] > [企業整合] > [邏輯應用程式]。

   ![建立邏輯應用程式](./media/tutorial-process-email-attachments-workflow/create-logic-app.png)

2. 在 [建立邏輯應用程式] 下，提供有關於邏輯應用程式的這項資訊，如下所示。 完成時，請選擇 [釘選到儀表板] > [建立]。
   ...
   | **資源群組** | LA-Tutorial-RG | 您先前使用的相同 Azure 資源群組 |
   | **位置** | 美國東部 2 | 您先前使用的相同區域 | 
   | **Log Analytics** | 關閉 | 在此教學課程中，請保留 [關閉] 設定。 | 
   |||| 
...
1. 在設計工具的搜尋方塊中，輸入 "when email arrives (電子郵件送達時)"。 請為您的電子郵件提供者選取此觸發程序：**<您的電子郵件提供者> - 新的電子郵件送達時**，例如：
...
1. 在觸發程序下，選擇 [+ 新增步驟] > [新增條件]。

   條件圖形出現時，依預設會顯示參數清單或動態內容清單，並顯示上一個步驟中可納入為工作流程輸入的任何參數。 
   您的瀏覽器寬度可決定哪一份清單會出現。
...
2. 以更適當的說明為條件重新命名。
   ...
   1. 在條件的標題列上，選擇**省略符號** (**...**) 按鈕 > [重新命名]。
      ...
      ![關閉動態內容清單](./media/tutorial-process-email-attachments-workflow/close-dynamic-content-list.png)

   2. 以下列說明為條件重新命名：```If email has attachments and key subject phrase```

3. 藉由提供運算式來說明條件。 

   1. 在條件圖形內，選擇 [在進階模式中編輯]。

      ![在進階模式中編輯條件](./media/tutorial-process-email-attachments-workflow/edit-advanced-mode.png)

   2. 在文字方塊中，輸入下列運算式：

      ```@equals(triggerBody()?['HasAttachment'], bool('true'))```

      此運算式會比較觸發程序主體 (在此教學課程中為電子郵件) 中的 **HasAttachment** 屬性值與布林值物件 ```True```。 
      如果這兩個值相等，表示電子郵件至少有一個附件，因此符合條件，工作流程會繼續執行。

      您的條件現在看起來就像下面這個範例︰

      ![條件運算式](./media/tutorial-process-email-attachments-workflow/condition-expression.png)

   3. 選擇 [在基本模式中編輯]。 現在，您的運算式會解析如下：

      ![解析的運算式](./media/tutorial-process-email-attachments-workflow/condition-expression-resolved.png)

      > [!NOTE]
      > 若要手動建置運算式，您必須以基本模式運作，並且開啟動態清單，以便使用運算式產生器。 在 [運算式] 下，您可以選取函式。 在 [動態內容] 下，您可以選取要在這些函式中使用的參數欄位。
      > 此教學課程稍後會說明如何手動建置運算式。
...
## <a name="call-the-removehtmlfunction"></a>呼叫 RemoveHTMLFunction

1. 在邏輯應用程式功能表上，選擇 [邏輯應用程式設計工具]。 在 [若為 true] 分支中，選擇 [新增動作]。

2. 搜尋「azure 函式」，並選取下列動作：**Azure Functions – 選擇 Azure 函式**

   ![選取「Azure Functions - 選擇 Azure 函式」的動作](./media/tutorial-process-email-attachments-workflow/add-action-azure-function.png)
...
6. 在函式圖形中，輸入函式的輸入以進行處理。 依照下列說明指定電子郵件內文：

   ![指定函式預期的要求本文](./media/tutorial-process-email-attachments-workflow/add-email-body-for-function-processing.png)

   1. 在 [要求本文] 下，輸入以下文字： 
   
      ```{ "emailBody": ``` 

      在您於後續步驟中完成此輸入之前，會出現 JSON 無效的相關錯誤。
      在您先前測試此函式時，為此函式指定的輸入使用的是 JavaScript 物件標記法 (JSON)。 
      因此，要求本文必須也使用相同的格式。 

   2. 從參數清單或動態內容清單中，選取 [新的電子郵件送達時] 下的 [內文] 欄位。
   在 [內文] 欄位後面，加上右大括號：```}```

      ![指定要傳至函式的要求本文](./media/tutorial-process-email-attachments-workflow/add-email-body-for-function-processing2.png)

      在邏輯應用程式定義中，此項目會以下列格式呈現：

      ```{ "emailBody": "@triggerBody()?['Body']" }```

7. 儲存您的邏輯應用程式。

接著，新增在您儲存體容器中建立 Blob 用以儲存電子郵件內文的動作。

## <a name="create-blob-for-email-body"></a>為電子郵件內文建立 Blob

1. 在 Azure 函式圖形下，選擇 [新增動作]。 

2. 在 [選擇動作] 下搜尋 "Blob"，並選取下列動作：**Azure Blob 儲存體 – 建立 Blob**

   ![新增為電子郵件內文建立 Blob 的動作](./media/tutorial-process-email-attachments-workflow/create-blob-action-for-email-body.png)

3. 如果您沒有 Azure 儲存體帳戶的連線，請透過以下說明的設定建立儲存體帳戶的連線。 完成之後，請選擇 [建立]。
...
5. 在 [建立 Blob] 動作中提供此資訊並選取這些參數，以建立 Blob，說明如下：

   ![提供電子郵件內文的 Blob 資訊](./media/tutorial-process-email-attachments-workflow/create-blob-for-email-body.png)

   | 設定 | 值 | 說明 | 
   | ------- | ----- | ----------- | 
   | **資料夾路徑** | /attachments | 您先前建立之容器的路徑和名稱。 您也可以瀏覽並選取一個容器。 | 
   | **Blob 名稱** | **寄件者**欄位 | 傳入電子郵件寄件者名稱作為 Blob 名稱。 從參數清單或動態內容清單中，選取 [新的電子郵件送達時] 下的 [寄件者]。 | 
   | **Blob 內容** | **內容**欄位 | 傳入無 HTML 電子郵件內文作為 Blob 內容。 從參數清單或動態內容清單中，選取 [呼叫 RemoveHTMLFunction 以清除電子郵件內文] 下的 [內文]。 |
   |||| 
...
此邏輯應用程式會使用 **for each** 迴圈來處理電子郵件中的每個附件。

1. 在 [為電子郵件內文建立 Blob] 圖形中選擇 [...更多]，然後選取下列命令：**新增 for each**
...
3. 現在，指定要由迴圈處理的資料。 按一下 [選取先前步驟中的輸出] 方塊內部。 從參數清單或動態內容清單中，選取 [附件]。 

   ![取得「附件」](./media/tutorial-process-email-attachments-workflow/select-attachments.png)

   [附件] 欄位會傳遞一個陣列，其中包含電子郵件所含的所有附件。 
...
## <a name="create-blobs-for-attachments"></a>為附件建立 Blob

1. 在 **For each** 迴圈中選擇 [新增動作]，以便您對每個找到的附件指定執行工作。

   ![將動作新增至迴圈](./media/tutorial-process-email-attachments-workflow/for-each-add-action.png)

2. 在 [選擇動作] 下搜尋 "Blob"，然後選取下列動作：**Azure Blob 儲存體 – 建立 Blob**
...
4. 在 [為每個電子郵件附件建立 Blob] 動作中提供此資訊並選取參數，以建立每個 Blob，說明如下：
   ...
   | ------- | ----- | ----------- | 
   | **資料夾路徑** | /attachments | 您先前建立之容器的路徑和名稱。 您也可以瀏覽並選取一個容器。 | 
   | **Blob 名稱** | **名稱**欄位 | 從參數清單或動態內容清單中選取 [名稱]，以傳入附件名稱作為 Blob 名稱。 | 
   | **Blob 內容** | **內容**欄位 | 從參數清單或動態內容清單中選取 [內容]，以傳入附件內容作為 Blob 內容。 |
   |||| 
...
5. 提供此動作的資訊，然後選取要包含在電子郵件中的欄位，說明如下。 若要在編輯方塊中新增空白的行，請按 Shift + Enter。  
   ...
   如果您在清單中找不到預期的欄位，請在動態內容清單中或參數清單結尾處選取 [新的電子郵件送達時] 旁的 [更多資訊]。

   | 設定 | 值 | 注意 | 
   | ------- | ----- | ----- | 
   | **To** | <*recipient-email-address*> | 為了測試用途，您可以使用自己的電子郵件地址。 | 
   | **主旨**  | ```ASAP - Review applicant for position: ``` **主旨** | 您要包含的電子郵件主旨。 從參數清單或動態內容清單中，選取 [新的電子郵件送達時] 下的 [主旨] 欄位。 | 
   | **內文** | ```Please review new applicant:``` <p>```Applicant name: ``` **寄件者** <p>```Application file location: ``` **路徑** <p>```Application email content: ``` **內文** | 電子郵件內文的內容。 從參數清單或動態內容清單中，選取下列欄位： <p>- [新的電子郵件送達時] 下的 [寄件者] 欄位 </br>- [為電子郵件內文建立 Blob] 下的 [路徑] 欄位 </br>- [呼叫 RemoveHTMLFunction 以清除電子郵件內文] 下的 [內文] 欄位 | 
   |||| 

   如果您選取其中包含陣列的欄位 (例如 [內容]，這是包含附件的陣列)，設計工具會自動為參考該欄位的動作新增 "For each" 迴圈。 
   如此一來，應用程式邏輯可以在每個陣列項目上執行該動作。 
   若要移除迴圈，請移除陣列的欄位、將參考動作移至迴圈外、選擇迴圈標題列上的省略符號 (**...**)，然後選擇 [刪除]。
     
6. 儲存您的邏輯應用程式。 

接著請測試您的邏輯應用程式，此時它看起來會如同下列範例：
