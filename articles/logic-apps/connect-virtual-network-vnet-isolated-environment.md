---
title: 使用 ISE Azure Logic Apps 連接到 Azure 虛擬網路
...
當您搭配 Azure 虛擬網路使用 ISE 時，常見的設定問題是有一或多個已封鎖的埠。 您用來在 ISE 與目的地系統之間建立連線的連接器，可能也會有自己的埠需求。 例如，如果您使用 FTP 連接器與 FTP 系統通訊，請確定您在 FTP 系統上使用的埠可用，例如，用於傳送命令的埠21。 若要確定您的 ISE 保持可供存取並可正常運作，請開啟下表所指定的埠。 否則，如果有任何必要的埠無法使用，ISE 就會停止運作。

> [!IMPORTANT]
> 來源埠是暫時的，因此請確定您已將其設定為所有規則的 `*`。
> 針對子網內的內部通訊，ISE 會要求您在這些子網內開啟所有埠。

* 如果您建立新的虛擬網路和子網，但沒有任何限制，您就不需要在虛擬網路中設定[網路安全性群組（nsg）](../virtual-network/security-overview.md#network-security-groups)來控制子網之間的流量。

* 在現有的虛擬網路上，您可以藉由[篩選跨子網的網路流量](../virtual-network/tutorial-filter-network-traffic.md)，*選擇性地*設定 nsg。 如果您選擇 [此路由]，請在您要設定 Nsg 的虛擬網路上，確定您已開啟下表所指定的埠。 如果您使用[NSG 安全性規則](../virtual-network/security-overview.md#security-rules)，則需要 TCP 和 UDP 通訊協定。

* 如果您先前已在虛擬網路中有現有的 Nsg 或防火牆，請務必開啟下表所指定的埠。 如果您使用[NSG 安全性規則](../virtual-network/security-overview.md#security-rules)，則需要 TCP 和 UDP 通訊協定。
