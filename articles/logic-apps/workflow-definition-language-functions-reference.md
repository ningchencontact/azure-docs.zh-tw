...
| [formatNumber](../logic-apps/workflow-definition-language-functions-reference.md#formatNumber) | 根據指定的格式傳回數位做為字串 |
...
| [產出](../logic-apps/workflow-definition-language-functions-reference.md#outputs) | 傳回動作在執行階段的輸出。 |
...
傳回動作在執行階段的輸出。  和是 `actions('<actionName>').outputs`的縮寫。 請參閱 [actions()](#actions)。 `actionOutputs()` 函式會解析為邏輯應用程式設計工具中的 `outputs()`，因此請考慮使用[輸出（）](#outputs)，而不是 `actionOutputs()`。 雖然這兩個函數的運作方式相同，但是較常使用 `outputs()`。
...
| --------- | -------- | ---- | ----------- |
| <*object*> | 是 | 物件 | 您要對其新增屬性的 JSON 物件 |
| <*父屬性*> | 是 | String | 您想要在其中加入子屬性的父屬性名稱 |
| <*子屬性*> | 是 | String | 要加入之子屬性的名稱 |
...
| 參數 | 必要項 | 類型 | 說明 |
| --------- | -------- | ---- | ----------- |
| <*數位*> | 是 | 整數或雙精度浮點數 | 您想要格式化的值。 |
| <*format*> | 是 | String | 複合格式字串，指定您要使用的格式。 如需支援的數值格式字串，請參閱 `number.ToString(<format>, <locale>)`支援的[標準數值格式字串](https://docs.microsoft.com/dotnet/standard/base-types/standard-numeric-format-strings)。 |
| <*locale*> | 否 | String | `number.ToString(<format>, <locale>)`所支援使用的地區設定。 若未指定，則預設值為 `en-us`。 |
|||||

| 傳回值 | 類型 | 說明 |
| ------------ | ---- | ----------- |
| <*格式化-數位*> | String | 指定的數位，表示為您所指定格式的字串。 您可以將此傳回值轉換成 `int` 或 `float`。 |
||||

*範例 1*

假設您想要格式化 `1234567890`的數位。 這個範例會將該數位格式化為字串 "1234567890.00"。
...
假設您想要格式化 `1234567890`的數位。 這個範例會將數位格式化為字串 "1.234.567.890，00"。
...
假設您想要格式化 `17.35`的數位。 這個範例會將數位格式化為字串 "$17.35"。
...
假設您想要格式化 `17.35`的數位。 這個範例會將數位格式化為字串 "17，35 kr"。
...
| 傳回值 | 類型 | 說明 | 
| ------------ | ---- | ----------- | 
| <*index*> | 整數 | 指定之 Until 迴圈內目前反覆運算的索引值 | 
|||| 
...
| 參數 | 必要項 | 類型 | 說明 |
| --------- | -------- | ---- | ----------- |
| <*startIndex*> | 是 | 整數 | 整數值，可將陣列當做第一個專案來啟動 |
| <*count*> | 是 | 整數 | 陣列中的整數數量 |
|||||
...
| 參數 | 必要項 | 類型 | 說明 |
| --------- | -------- | ---- | ----------- |
| <*object*> | 是 | 物件 | 您想要移除其屬性的 JSON 物件 |
| <*父屬性*> | 是 | String | 父屬性的名稱，其中包含您要移除的子屬性 |
| <*子屬性*> | 是 | String | 要移除之子屬性的名稱 |
|||||

| 傳回值 | 類型 | 說明 |
| ------------ | ---- | ----------- |
| <*updated-object*> | 物件 | 已更新的 JSON 物件，其子屬性已移除 |
||||
...
| 參數 | 必要項 | 類型 | 說明 |
| --------- | -------- | ---- | ----------- |
| <*scopedActionName*> | 是 | String | 要從其傳回所有內部動作之輸入和輸出的範圍動作名稱 |
||||
...
| --------- | -------- | ---- | ----------- |
| <*object*> | 是 | 物件 | 您想要設定其屬性的 JSON 物件 |
| <*父屬性*> | 是 | String | 父屬性的名稱，其中包含您想要設定的子屬性 |
| <*子屬性*> | 是 | String | 要設定之子屬性的名稱。 |
| <*value*> | 是 | 任意 | 要為指定屬性設定的值 |
|||||
...
| <*startIndex*> | 是 | 整數 | 要當做開始位置或索引值使用的正數，等於或大於0 |
...
在此範例中，有兩個運算式會在指定的引數中 (其中包含具有命名空間的 XML) 尋找符合 `<location></location>` 節點的節點。 

> [!NOTE]
>
> 如果您是在程式碼視圖中工作，請使用反斜線字元（\\）來將雙引號（"）換成引號。 
> 例如，當您將運算式序列化為 JSON 字串時，您必須使用 escape 字元。 
> 不過，如果您是在邏輯應用程式設計工具或運算式編輯器中工作，則不需要將雙引號轉義，因為會自動將反斜線字元新增至基礎定義，例如：
> 
> * 程式碼視圖： `xpath(xml(body('Http')), '/*[name()=\"file\"]/*[name()=\"location\"]')`
>
> * 運算式編輯器： `xpath(xml(body('Http')), '/*[name()="file"]/*[name()="location"]')`
> 
> 下列範例適用于您在 [運算式編輯器] 中輸入的運算式。
