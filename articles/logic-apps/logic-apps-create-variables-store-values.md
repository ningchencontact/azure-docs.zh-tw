---
title: 建立和管理用於儲存和傳遞值的變數
description: 瞭解如何在您使用建立的自動化工作和工作流程中，使用變數來儲存、管理、使用和傳遞值 Azure Logic Apps
services: logic-apps
...
1. 提供您變數的相關資訊，如下所述：
   ...
   | **ReplTest1** | 否 | <*start-value*> | 您的變數初始值 <p><p>**提示**：雖然此值為選擇性，但最佳做法是設定此值，如此您便一律知道變數的開始值。 |
   |||||
...
1. 提供下列資訊來遞增您的變數：
   ...
   |----------|----------|-------|--------------|
   | **名稱** | 是 | <*variable-name*> | 要遞增的變數名稱 |
   | **ReplTest1** | 否 | <*increment-value*> | 用來遞增變數的值。 預設值為一。 <p><p>**提示**：雖然此值為選擇性，但最佳做法是設定此值，如此您便一律知道用來遞增變數的特定值。 |
   ||||
...
| **ReplTest1** | 否 | <*increment-value*> | 用來遞減變數的值。 預設值為一。 <p><p>**提示**：雖然此值為選擇性，但最佳做法是設定此值，如此您便一律知道用來遞減變數的特定值。 |
||||| 
...
| **ReplTest1** | 是 | <*new-value*> | 您想要指派給變數的值。 兩者必須具有相同的資料類型。 |
||||| 
...
|----------|----------|-------|--------------|
| **名稱** | 是 | <*variable-name*> | 要變更的變數名稱 |
| **ReplTest1** | 是 | <*append-value*> | 您想要附加的值 (可以具有任何類型) |
|||||
