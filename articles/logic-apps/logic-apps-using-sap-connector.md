...
2. 在搜尋方塊中，輸入「sap 伺服器」做為篩選條件。 從 [動作] 清單中，為 SAP 伺服器選取動作。 
   ...
   * **SAP 訊息伺服器 - 傳送至 SAP**
   ...
   ![選取 [SAP 應用程式伺服器] 或 [SAP 訊息伺服器]](media/logic-apps-using-sap-connector/select-sap-action.png)
   ...
   **建立內部部署 SAP 連線**
   ...
   針對 [閘道] 屬性，請選取您在 Azure 入口網站中建立用於安裝閘道的資料閘道，例如：
      ...
      **SAP 應用程式伺服器**
      ...
      ![建立 SAP 應用程式伺服器連線](./media/logic-apps-using-sap-connector/create-SAP-app-server-connection.png)  
