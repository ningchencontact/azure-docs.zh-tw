...
當您想要自動建立並部署邏輯應用程式之類的 Azure 資源，您可以使用 [Azure Resource Manager 範本](../logic-apps/logic-apps-azure-resource-manager-templates-overview.md)。 若要透過範本為您的邏輯應用程式建立系統指派的受控識別，請在部署範本中將 `"identity"` 元素與 `"type"` 屬性新增到您的邏輯應用程式工作流程定義： 
