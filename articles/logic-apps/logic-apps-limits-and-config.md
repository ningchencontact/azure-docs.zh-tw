...
相同區域中的所有邏輯應用程式都會使用相同的 IP 位址範圍。 若要支援您的邏輯應用程式直接使用[HTTP](../connectors/connectors-native-http.md)、 [HTTP + SWAGGER](../connectors/connectors-native-http-swagger.md)及其他 HTTP 要求所進行的呼叫，請使用 Logic Apps 所使用的*所有*[輸入](#inbound)*和*[輸出](#outbound)IP 位址來設定您的防火牆。服務，根據您的邏輯應用程式所在的區域。 這些位址會出現在本節中的**輸入**和**輸出**標題底下，並且依照區域排序。
...
| 西歐 | 13.95.147.65、23.97.210.126、23.97.211.179、23.97.218.130、40.68.209.23、40.68.222.65、51.144.182.201、104.45.9.52 | 13.69.64.208 - 13.69.64.223, 52.174.88.118 |
