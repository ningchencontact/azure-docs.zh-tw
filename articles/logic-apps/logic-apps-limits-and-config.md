...
您可以將此限制變更為 7 天到 90 天之間的值。 但若要超過上限，請[連絡 Logic Apps 小組](mailto://logicappsemail@microsoft.com)，讓其協助處理您的需求。
...
| 使用區塊化時的訊息大小 | 1 GB | 此限制適用於原生支援區塊化的動作，或可在其執行階段設定中啟用區塊化支援的動作。 如需詳細資訊，請參閱[使用區塊化處理大型訊息](../logic-apps/logic-apps-handle-large-messages.md)。 | 
...
區域中的所有邏輯應用程式都會使用相同的 IP 位址範圍。
邏輯應用程式直接透過 [HTTP](../connectors/connectors-native-http.md)、[HTTP + Swagger](../connectors/connectors-native-http-swagger.md) 或其他 HTTP 要求所發出的呼叫，會來自以下清單中的 IP 位址。 
