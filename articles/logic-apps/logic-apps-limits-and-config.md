...
* 邏輯應用程式無法直接存取具有[防火牆規則](https://docs.microsoft.com/azure/storage/common/storage-network-security)且存在於相同區域中的 Azure 儲存體帳戶。 不過，邏輯應用程式可以存取存在於不同區域中的 Azure 儲存體帳戶，因為公用 IP 位址是用來跨區域進行通訊。 或者，您可以在這裡使用其中一個選項：
