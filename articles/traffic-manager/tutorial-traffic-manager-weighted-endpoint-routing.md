---
title: 教學課程 - 使用 Azure 流量管理員將流量路由傳送至加權端點 | Microsoft Docs
description: 本教學課程文章說明如何使用流量管理員將流量路由傳送至加權端點。
...
ms.locfileid: "49649429"
---
# <a name="tutorial-control-traffic-routing-with-weighted-endpoints-using-traffic-manager"></a>教學課程：使用流量管理員控制加權端點的流量路由 

本教學課程說明如何使用流量管理員，透過加權路由方法控制端點之間的使用者流量路由。 在這個路由方法中，您需要在流量管理員設定檔設定中對每個端點指派權數，然後使用者流量就會根據指派給每個端點的權數來加以路由傳送。 權數是從 1 到 1000 的整數。 指派給端點的權數值越高，其優先順序越高。
...
- 用於測試流量管理員的兩個測試 VM - 一個 VM 位於**美國東部**，另一個 VM 位於**西歐**。 測試 VM 可用來說明流量管理員如何將使用者流量路由傳送至端點獲派權數較高的網站。
...
6. 在 [摘要] 的 [建立] 底下，選取 [建立] 來開始部署虛擬機器。
8. 建立 VM 需要幾分鐘的時間。 請等到 VM 建立後，再繼續進行其餘步驟。
...
新增兩部執行 IIS 伺服器的 VM (myIISVMEastUS  & myIISVMWEurope)，以對其路由傳送使用者流量。
...
4. 重複步驟 2 和 3，以針對與 IIS 伺服器 VM myIISVMWEurope 相關聯的公用 IP 位址 myIISVMWEurope-ip 新增另一個名為 myWestEuropeEndpoint 的端點，並對 [權數] 輸入 **25**。 
5.  這兩個端點新增完畢後，它們會顯示在 [流量管理員設定檔] 中，而且監視狀態是 [線上]。

## <a name="test-traffic-manager-profile"></a>測試流量管理員設定檔
若要查看運作中的流量管理員，請完成下列步驟：
...
4. 選取 [確定] 。
5. 您可能會在登入過程中收到憑證警告。 如果您收到警告，請選取 [是] 或 [繼續] 以繼續進行連線。 
6. 在 VM *myVMEastUS* 的網頁瀏覽器中，輸入流量管理員設定檔的 DNS 名稱，以檢視您的網站。 系統會將您路由傳送至裝載於 IIS 伺服器 myIISVMEastUS 上的網站，因為其獲派的權數 (**100**) 比 IIS 伺服器 myIISVMWEurope (獲派較低的端點權數值 **25**) 還高。
...
如果您不再需要本教學課程所建立的資源群組，請加以刪除。 若要這麼做，請選取資源群組 (**ResourceGroupTM1** 或 **ResourceGroupTM2**)，然後選取 [刪除]。

## <a name="next-steps"></a>後續步驟

> [!div class="nextstepaction"]
> [根據使用者的地理位置將流量路由傳送至特定端點](traffic-manager-configure-geographic-routing-method.md)
