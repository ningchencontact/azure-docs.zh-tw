...
* **流量管理員**提供全域 DNS 負載平衡。 它會查看傳入 DNS 要求並根據客戶所選取的路由原則，以狀況良好的端點回應。 路由方法的選項如下︰
  ...
  * 多值路由，可讓您在單一 DNS 回應中傳送多個應用程式端點的 IP 位址。
  ...
  用戶端直接連接到流量管理員傳回的端點。 Azure 流量管理員會在偵測到端點狀況不良時，將用戶端重新導向至另一個狀況良好的執行個體。 若要详细了解此服务，请参阅 [Azure 流量管理器文档](traffic-manager-overview.md)。
...
* **負載平衡器**是 Azure SDN 堆疊不可或缺的一部分，針對所有 UDP 和 TCP 通訊協定提供高效能、低延遲的第 4 層負載平衡服務。 它管理入站和出站连接。 您可以設定公用和內部負載平衡端點，並使用 TCP 和 HTTP 健全狀況探查選項定義規則，將輸入連線對應至後端集區目的地，以管理服務可用性。
...
### <a name="step-1-create-a-traffic-manager-profile"></a>步骤 1：建立流量管理員設定檔
...
2. 輸入下列基本資訊：
   ...
   * 訂用帳戶：選取包含設定檔的訂用帳戶。
   * **资源组**：選取包含設定檔的資源群組。 可以是新的或現有的資源群組。
...
### <a name="step-2-create-the-application-gateways"></a>步骤 2：建立應用程式閘道
...
2. 輸入下列關於應用程式閘道的基本資訊︰
   ...
   * **资源组**：保存應用程式閘道的資源群組。 它可以是現有的或新的資源群組。
...
### <a name="step-3-add-application-gateways-to-the-traffic-manager-endpoints"></a>步骤 3：將應用程式閘道新增至流量管理員端點
...
### <a name="step-4-create-a-load-balancer"></a>步骤 4：建立負載平衡器
...
### <a name="step-5-connect-web-tier-vms-to-the-load-balancer"></a>步骤 5：將 Web 層 VM 連線到負載平衡器
