...
當您在開發人員空間中執行服務時, 該服務的 pod 會[插入其他容器以進行檢測](how-dev-spaces-works.md#prepare-your-aks-cluster)。 這些容器未定義資源要求或限制, 因此會停用 pod 的水準 pod 自動調整程式。

### <a name="try"></a>嘗試
在未啟用 Dev Spaces 的命名空間中執行水準 pod 自動調整程式。
