---
title: 使用 .NET Core 和 Visual Studio 在雲端建立 Kubernetes 開發環境 | Microsoft Docs
...
- 在 Azure 中建立以 Kubernetes 為基礎、最適合用於開發的環境。
...
使用 Azure 開發人員空間，您可以建立以 Kubernetes 為基礎的開發環境，該開發環境完全受 Azure 控管並已針對開發最佳化。 開啟您剛建立的專案，然後從啟動設定下拉式清單中選取 [Azure 開發人員空間]，如下所示。
...
Visual Studio 會與開發環境通訊，以建置和部署應用程式，然後以執行中的 Web 應用程式開啟瀏覽器。 容器可能看起來像在本機執行，但實際是在 Azure 的開發環境中執行。 localhost 位址的原因是因為 Azure 開發人員空間會對在 Azure 中執行的容器建立暫存 SSH 通道。
...
1. 從啟動設定下拉式清單中選取 [Azure 開發人員空間]，如同您先前對 `webfrontend` 專案所做的。 這是請不要建立新的開發環境，而應選取您已建立的同一個開發環境。 與之前相同，將 [空間] 保留為預設的 `default`，然後按一下 [確定]。 在 [輸出] 視窗中，您可能會發現 Visual Studio 開始在開發環境中「準備」這項新服務，以在您開始偵錯時加速作業。
1. 按 F5，並等候服務進行建置和部署。 當 Visual Studio 的狀態列變成橙色時，即表示已就緒
1. 記下 [輸出] 視窗的 [適用於 AKS 的 Azure 開發人員空間] 窗格中所顯示的 URL 端點。 輸出應該會類似於 http://localhost:\<portnumber\>。 容器可能看起來像在本機執行，但實際是在 Azure 的開發環境中執行。
...
上述程式碼範例也會使用 `HeaderPropagatingHttpClient` 類別。 此協助程式類別是您在將其設定為使用 Azure 開發人員空間時新增至專案的檔案 `HeaderPropagation.cs`。 `HeaderPropagatingHttpClient` 衍生自已知的 `HttpClient` 類別，且會新增功能，將特定標頭從現有的 ASP .NET HttpRequest 物件傳播到傳出的 HttpRequestMessage 物件中。 您稍後將會了解此類別如何促進小組案例中的生產力開發體驗。

## <a name="debug-across-multiple-services"></a>在多個服務間進行偵錯
1. 此時，`mywebapi` 仍應使用附加的偵錯工具執行中。 如果不是，在 `mywebapi` 專案中按 F5。
1. 在 `ValuesController.cs` 檔案中負責處理 `api/values/{id}` GET 要求的 `Get(int id)` 方法內設定中斷點。
...
目前為止，您已如同您是應用程式唯一的開發人員一般，執行您的應用程式程式碼。 在本節中，您將了解 Azure 開發人員空間如何簡化小組開發：
* 讓一組開發人員在相同的開發環境中工作。
* 支援每位開發人員獨立在其程式碼中反覆執行，而不需要擔心會中斷其他開發人員。
...
- 您可以嘗試只執行您依賴的服務，但是這表示您需要知道完整的相依性 (例如，相依性的相依性)。 否則，因為您不是在處理它們，無法輕易得知如何建置及執行您的相依性就會是問題。
...
### <a name="work-in-a-shared-development-environment"></a>在共用開發環境中工作
使用 Azure 開發人員空間，您可以在 Azure 中設定共用開發環境。 每位開發人員可以專注於他們自己的應用程式部分，並且可以在環境 (已經包含其他所有服務和他們的案例依賴的雲端資源) 中反覆地開發認可前程式碼。 相依性永遠保持最新狀態，而且開發人員會以鏡像生產環境的方式工作。

### <a name="work-in-your-own-space"></a>在您自己的空間工作
當您為服務開發程式碼時，且在您準備將其簽入之前，程式碼通常不會處於良好狀態。 您仍反覆地讓它成形、加以測試，並且實驗解決方案。 Azure 開發人員空間提供**空間**的概念，可讓您獨立作業，而不需要擔心中斷您的小組成員。

請執行下列工作，以確保您的 `webfrontend` 和 `mywebapi` 都在開發環境中執行，**並且在 `default` 空間中執行**。
...
開啟公用 URL 並瀏覽至 Web 應用程式的任何人，都會叫用您先前撰寫、會透過兩項服務使用預設 `default` 空間執行的程式碼路徑。 現在，假設您想要繼續開發 `mywebapi` - 當您這麼做的時候，要如何才能不影響到其他使用開發環境的開發人員？ 為此，您應設定自己的空間。
...
7. 您現在應該會看到專案屬性頁面上選取了您的開發環境和新的空間。

    ![](media/get-started-netcore-visualstudio/Settings2.png)

### <a name="update-code-for-mywebapi"></a>更新 *mywebapi* 的程式碼

1. 在 `mywebapi` 專案中，對檔案 `ValuesController.cs` 中的 `string Get(int id)` 方法進行程式碼變更，如下所示：
...
3. 按 F5 以啟動 `mywebapi` 服務。 此時將會在開發環境中使用選取的空間啟動服務，在此案例中為 `scott`。

下圖有助於您了解不同空間的運作方式。 藍色路徑顯示透過 `default` 空間的要求，這是未將空間附加至 URL 時所使用的預設路徑。 綠色路徑顯示透過 `scott` 空間的要求。
...
若要測試新版的 `webfrontend` 與 `mywebapi` 的搭配運作情形，請開啟瀏覽器並進入 `webfrontend` 的公用存取點 URL (例如 http://webfrontend-teamenv.123456abcdef.eastus.aksapp.io))，然後移至 [關於] 頁面。 您應該會看到原始訊息「來自 webfrontend 的 Hello 和來自 mywebapi 的 Hello」。

現在，請將 "scott.s."  部分新增至 URL，使其類似於 http://scott.s.webfrontend-teamenv.123456abcdef.eastus.aksapp.io，然後重新整理瀏覽器。 系統應會叫用您在 `mywebapi` 專案中設定的中斷點。 請按 f5 繼續作業，而您此時應會在瀏覽器中看到新的訊息「來自 webfrontend 和 mywebapi 的 Hello 現在顯示了新的內容。」 這是因為您已更新的程式碼在 `mywebapi` 中的路徑執行於 `scott` 空間中。
