...
ms.locfileid: "68362460"
---
1. 開啟 Visual Studio 2019。

1. 在 [開始] 視窗中，選取 [建立新專案]  。 

1. 選取 [主控台應用程式 (.NET Framework)]  ，然後選取 [下一步]  。

1. 在 [專案名稱]  中，輸入 `helloworld`，然後選取 [建立]  。

1. 從 Visual Studio 的功能表列，選取 [工具]   > [取得工具和功能]  ，並檢查 **.NET 桌面開發**工作負載是否可用。 如果尚未安裝工作負載，請勾選核取方塊，然後選取 [修改]  以開始安裝。 下載及安裝可能需要幾分鐘的時間。
...
1. 在 [方案總管] 中，以滑鼠右鍵按一下 `helloworld`，然後選取 [管理 NuGet 套件]  以顯示 NuGet 套件管理員。
...
1. 在搜尋方塊中，輸入 `Microsoft.CognitiveServices.Speech` 套件並按 Enter 鍵。

1. 選取 `Microsoft.CognitiveServices.Speech`，然後選取 [安裝]  以安裝最新的穩定版本。
