...
沒有。 VNet 對 VNet 流量會透過 Microsoft Azure 骨幹傳輸，而非透過網際網路。
...
沒有。 將多個 Azure 虛擬網路連接在一起並不需要 VPN 裝置，除非需要跨單位連線能力。

### <a name="do-my-vnets-need-to-be-in-the-same-region"></a>我的 VNet 需要位於相同區域嗎？

沒有。 虛擬網路可位於相同或不同的 Azure 區域 (位置)。
...
沒有。 VNet 對 VNet 支援連線相同 Azure 執行個體中的虛擬網路。 例如，您無法建立全域 Azure 與中文版/德文版/美國政府版 Azure 執行個體間的連線。 這些案例中，請考慮使用站對站 VPN 連線。
...
沒有。 VNet 對 VNet 和多站台連線需要 VPN 類型為 RouteBased (前稱為動態路由) 的 Azure VPN 閘道。
