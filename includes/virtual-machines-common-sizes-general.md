...
---
一般用途的虛擬機器大小可讓 CPU 與記憶體比例達到平均。 適用於測試和開發、小型至中型資料庫，以及低至中流量 Web 伺服器。 此文章提供 vCPU 數量、資料磁碟和 NIC 的相關資訊，以及此群組中各種大小之儲存體輸送量的相關資訊。 

- [DC 系列](#dc-series)是 Azure 中的新虛擬機器系列，可協助您在您的資料與程式碼在公用雲端處理時的機密性與完整性。 這些機器由最新一代的 3.7GHz Intel XEON E-2176G 處理器 (具 SGX 技術) 支援。 在 Intel Turbo Boost 技術加持下，這些機器最高可達到 4.7GHz 的時脈。 DC 系列執行個體可讓客戶建置安全的飛地型應用程式，以在其程式碼與資料使用期間保護其安全。
