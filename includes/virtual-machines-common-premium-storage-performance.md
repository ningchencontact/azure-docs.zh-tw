...
在托管磁盘上进行的下述控制平面操作可能涉及将磁盘从一个存储位置移动到另一个存储位置。 这是通过在后台复制数据来安排的，可能需要花费数小时才能完成，通常少于 24 小时，具体取决于磁盘中的数据量。 在這段期間，某些讀取可以重新導向至原始位置，並可能需要較長的時間完成，可以高於一般的讀取延遲體驗您的應用程式。 在這段期間，對於寫入延遲沒有任何影響。
...
如果要測量應用程式的效能需求，最佳方式就是使用伺服器的作業系統所提供的效能監視工具。 您可以使用 Windows 的 PerfMon 和 Linux 的 iostat。 這些工具可擷取與上一節所述的每個量值相對應的計數器。 必须在应用程序运行其正常工作负荷、高峰工作负荷和非工作时间工作负荷时捕获这些计数器的值。

伺服器的處理器、記憶體及每個邏輯磁碟和實體磁碟，都有可用的 PerfMon 計數器。 當您使用進階儲存體磁碟和 VM 時，實體磁碟計數器適用於每個進階儲存體磁碟，邏輯磁碟計數器適用於進階儲存體上建立的每個磁碟區。 您必須為裝載應用程式工作負載的磁碟擷取這些值。 如果邏輯與實體磁碟之間有一對一對應，您可以參考實體磁碟計數器，否則請參考邏輯磁碟計數器。 在 Linux 上，iostat 命令會產生 CPU 和磁碟使用率報告。 磁碟使用率報告提供每個實體裝置或分割區的統計資料。 如果数据库服务器的数据和日志位于不同的磁盘上，则请针对两种磁盘收集此类数据。 下表描述了磁盘、处理器和内存的计数器：
...
影响运行在高级存储上的应用程序的性能的主要因素包括：IO 请求的性质、VM 大小、磁盘大小、磁盘数目、磁盘缓存、多线程处理和队列深度。 您可以使用系統提供的參數來控制這些因素。 大部分應用程式可能沒有選項讓您直接改變 IO 大小和佇列深度。 例如，如果您使用 SQL Server，您無法選擇 IO 大小和佇列深度。 SQL Server 會選擇最佳 IO 大小和佇列深度值，以獲得最高效能。 務必了解這兩種因素對應用程式效能的影響，才能佈建適當的資源來滿足效能需求。

在本節中，請參閱您建立的應用程式需求檢查清單，以識別您需要將應用程式效能最佳化到何種程度。 據此，您將能夠判斷需要調整本節中的哪些因素。 若要證明每個因素對應用程式效能的影響，請在應用程式安裝上執行效能評定工具。 请参阅本文末尾的“基准测试”部分，了解需要执行哪些步骤才能在 Windows 和 Linux VM 上运行常见的基准测试工具。

### <a name="optimize-iops-throughput-and-latency-at-a-glance"></a>迅速优化 IOPS、吞吐量和延迟

下表汇总了性能因素以及进行 IOPS、吞吐量和延迟优化所需的步骤。 這份摘要之後的幾節更深入說明每一個因素。
...
| **磁碟串接** |使用多個磁碟並串接在一起，可結合產生較高的 IOPS 和輸送量限制。 单个 VM 的组合限制应高于所连接的高级磁盘的组合限制。 | &nbsp; | &nbsp; |
| **等量大小** |在 OLTP 應用程式中，隨機小型 IO 模式使用較小的等量大小。 例如，SQL Server OLTP 应用程序使用 64 KB 的条带大小。 |在資料倉儲應用程式中，循序大型 IO 模式使用較大的等量大小。 例如，SQL Server 数据仓库应用程序使用 256 KB 的条带大小。 | &nbsp; |
...
IO 要求是指應用程式執行的輸入/輸出作業單位。 識別 IO 要求的本質、隨機或循序、讀取或寫入、小型或大型，可協助您判斷應用程式的效能需求。 了解 IO 请求的性质很重要，这有助于在设计应用程序基础结构时作出正确决策。

IO 大小是其中一個很重要的因素。 IO 大小是指應用程式所產生的輸入/輸出作業要求的大小。 IO 大小對效能有很重大的影響，特別是應用程式可達到的 IOPS 和頻寬。 下面的公式说明了 IOPS、IO 大小和带宽/吞吐量之间的关系。  
...
* OLTP 應用程式會產生數百萬個小型和隨機的 IO 要求。 若要处理这些类型的 IO 请求，必须对应用程序基础结构进行设计，以便提高 IOPS。  
* 資料倉儲應用程式會產生大型和循序的 IO 要求。 若要处理这些类型的 IO 请求，必须对应用程序基础结构进行设计，以便提高带宽或吞吐量。
...
若要證明 IO 大小對應用程式效能的影響，您可以在 VM 和磁碟上執行效能評定工具。 請建立多個測試回合，而且每個回合使用不同的 IO 大小，以查看影響。 如需更多详细信息，请参阅本文末尾的“基准测试”部分。
...
Azure 進階儲存體提供八個 GA 磁碟大小和目前為預覽版的三種磁碟大小。 每种磁盘大小对 IOPS、带宽和存储空间设置了不同规格的限制。 請根據應用程式需求和高延展性 VM 大小，選擇正確的進階儲存體磁碟大小。 下表顯示在 11 磁碟大小及其功能。 目前只有針對「受控磁碟」才支援 P4、P6、P15、P60、P70 和 P80 磁碟大小。
...
> [!WARNING]
> 大於 4 TiB 的磁碟不支援磁碟快取。 如果多個磁碟已附加至您的 VM，每個磁碟是 4 TiB，或將較小支援快取。
>
...
重要：您可以使用伺服器管理員 UI，將一個等量磁碟區的總欄數最多設定為 8 個。 连接 8 个以上的磁盘时，可使用 PowerShell 来创建卷。 您可以使用 PowerShell 將欄數設定為等於磁碟數量。 例如，如果單一等量磁碟區中有 16 個磁碟，請在 *New-VirtualDisk* PowerShell Cmdlet 的 *NumberOfColumns* 參數中指定 16 欄。
...
通常，當每個連接的磁碟上有 8-16+ 個未完成的 IO 時，應用程式可以達到最大輸送量。 如果队列深度为 1，则应用程序不会将足够的 IO 推送到系统，在给定时间内所能处理的 IO 数目会较少。 換句話說，輸送量較少。

例如，在 SQL Server 中，將查詢的 MAXDOP 值設為 "4"，會向 SQL Server 表示它最多可以使用四個核心來執行查詢。 SQL Server 會決定查詢執行的最佳佇列深度值和核心數目。

*最佳佇列深度*  
 佇列深度值太高也有其缺點。 如果佇列深度值太高，應用程式會嘗試推動非常高的 IOPS。 除非應用程式的永續性磁碟已佈建足夠的 IOPS，否則這可能對應用程式延遲造成負面影響。 以下公式显示了 IOPS、延迟和队列深度之间的关系。  
