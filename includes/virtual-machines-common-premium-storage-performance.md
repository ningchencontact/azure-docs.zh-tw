...
受控磁碟上的下列控制平面作業可能會移動到另一個儲存體位置的磁碟。 這是進行協調，透過背景可能需要數小時才能完成，通常不會超過 24 小時內的磁碟中的資料量而定的資料複本。 在這段期間，某些讀取可以重新導向至原始位置，並可能需要較長的時間完成，可以高於一般的讀取延遲體驗您的應用程式。 在這段期間，對於寫入延遲沒有任何影響。
