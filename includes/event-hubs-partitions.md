...
雖然可以識別並直接傳送至資料分割，但是不建議直接傳送至資料分割。 相反地, 您可以使用在[事件發行者](../articles/event-hubs/event-hubs-features.md#event-publishers)一節中引進的較高層級結構。 

資料分割中會填入一連串事件資料，內含事件本文、使用者定義屬性包，以及中繼資料，例如在資料分割中的位移和串流序列中的編號。

我們建議您平衡1:1 輸送量單位和分割區, 以達到最佳規模。 單一資料分割具有保證的輸入和輸出, 最多可達一個輸送量單位。 雖然您可以在磁碟分割上達到較高的輸送量, 但無法保證效能。 這就是為什麼我們強烈建議事件中樞內的資料分割數目大於或等於輸送量單位數目。

根據您所規劃的總輸送量, 您會知道所需的輸送量單位數目和最少的分割區數目, 但您應該擁有多少個分割區？ 根據您想要達成的下游平行處理原則, 以及您的未來輸送量需求, 選擇資料分割數目。 事件中樞內的資料分割數目不會有任何費用。
