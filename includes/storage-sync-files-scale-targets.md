...
| 目錄中的檔案系統物件 (目錄和檔案) 數目上限 | 200,000 個物件 | yes |
| 物件 (目錄和檔案) 安全性描述元大小上限 | 4 KiB | yes |
| 檔案大小 | 100 GiB | 否 |
| 要分層之檔案的檔案大小下限 | 64 KiB | yes |
| 並行同步處理工作階段 | V4 代理程式：限制隨可用的系統資源而不同。 <BR> V3 代理程式：每個處理器 2 個作用中的同步處理工作階段，或每部伺服器最多 8 個作用中的同步處理工作階段 | yes
