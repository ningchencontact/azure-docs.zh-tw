...
ms.locfileid: "51283902"
---
## <a name="download-the-source-code"></a>下載原始程式碼

遠端監視原始程式碼存放庫包含您執行微服務 Docker 映像所需的設定檔。

若要複製及建立本機版本的存放庫，請使用您的命令列環境來瀏覽到您本機電腦上的適當資料夾。 然後執行下列其中一組命令，複製 .NET 或 Java 存放庫：
...
1. 執行 **start.cmd** 指令碼。 指令碼會提示您輸入下列資訊：
    ...
    * 要使用之 Azure 資料中心的位置。
    ...
    此指令碼也會將一組有前置詞 **PCS** 的環境變數新增到您的本機電腦。 當您在本機上啟動 Docker 容器時，它們會從這些環境變數讀取其設定值。

> [!TIP]
> 當指令碼完成時，它會顯示環境變數清單。 如果您將這些值儲存到 **services\\scripts\\local\\.env** 檔案，您就可將它們用於未來的解決方案加速器部署。 請注意，當您執行 **docker-compose** 時，您本機電腦上設定的任何環境變數會覆寫 **services\\scripts\\local\\.env** 檔案中的值。

### <a name="use-existing-azure-resources"></a>使用現有的 Azure 資源

如果您已經建立必要的 Azure 資源，請在本機電腦上建立對應的環境變數。 您也可以將這些值儲存在 **services\\scripts\\local\\.env** 檔案中，作為上次部署的一部分。 請注意，當您執行 **docker-compose** 時，您本機電腦上設定的環境變數會覆寫 **services\\scripts\\local\\.env** 檔案中的值。