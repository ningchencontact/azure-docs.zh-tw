...
**步驟 1：停用 VM 上的超執行緒**-VM 需要停用超執行緒，或移至非超執行緒 VM 超執行緒上執行不受信任的程式碼的客戶。  若要檢查執行超執行緒的 VM，請執行`lscpu`命令，在 Linux VM。 
...
如果您執行的超執行緒的 VM，請[連絡 Azure 支援](https://aka.ms/MicrocodeEnablementRequest-SupportTechnical)取得停用超執行緒。  停用超執行緒，則一旦**支援需要完整的 VM 重新開機**。
