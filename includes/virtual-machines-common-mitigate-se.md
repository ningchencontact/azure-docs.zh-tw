...
ms.locfileid: "65620255"
---
**上次文件更新**:14 2019 年太平洋標準時間上午 10:00。
...
> - CVE-2018年-11091-微資料取樣不可快取的記憶體 (MDSUM)
...
**步驟 1：停用 VM 上的超執行緒**-VM 需要停用超執行緒，或移至非超執行緒的 VM 大小的超執行緒上執行不受信任的程式碼的客戶。 若要檢查您的 VM 是否啟用超執行緒，請參閱下列使用 Windows 命令列從 VM 內的指令碼。
...
如果邏輯處理器的數目大於實體處理器 （核心），已啟用超執行緒。  如果您執行的超執行緒的 VM，請[連絡 Azure 支援](https://aka.ms/MicrocodeEnablementRequest-SupportTechnical)取得停用超執行緒。  停用超執行緒，則一旦**支援需要完整的 VM 重新開機**。 
...
**步驟 1：停用 VM 上的超執行緒**-VM 需要停用超執行緒，或移至非超執行緒 VM 超執行緒上執行不受信任的程式碼的客戶。  若要檢查執行超執行緒的 VM，請執行`lspcu`命令，在 Linux VM。 

如果`Thread(s) per core = 2`，則已啟用超執行緒。 

如果`Thread(s) per core = 1`，則已停用超執行緒。 
...
如果您執行的超執行緒的 VM，請[連絡 Azure 支援](https://aka.ms/MicrocodeEnablementRequest-SupportTechnical)取得停用超執行緒。  注意：停用超執行緒，則一旦**支援需要完整的 VM 重新開機**。
...
[L1 終端機的錯誤 (L1TF)](https://portal.msrc.microsoft.com/security-guidance/advisory/ADV180018):
- CVE-2018年-3615-Intel 軟體防護擴充功能 (Intel SGX)
- CVE-2018年-3620-作業系統 (OS) 和系統管理模式 (SMM)
- CVE-2018年-3646 – Virtual Machine Manager (VMM) 的影響

[微資料取樣](https://portal.msrc.microsoft.com/security-guidance/advisory/ADV190013): 
- CVE-2018年-11091-微資料取樣不可快取的記憶體 (MDSUM)
- CVE-2018年-12126-微儲存緩衝區資料取樣 (MSBDS)
- CVE-2018年-12127-微負載連接埠資料取樣 (MLPDS)
- CVE-2018年-12130-微填滿緩衝區資料取樣 (MFBDS)
