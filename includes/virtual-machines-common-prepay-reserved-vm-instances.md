...
使用量資料中的 [計量子類別] 和 [產品] 欄位無法區分使用進階儲存體的 VM 大小與未使用進階儲存體的 VM 大小，使用這些欄位來決定所購買保留項目的 VM 大小，會導致所購買的保留項目不正確，且無法為您提供保留項目折扣。 請使用下列其中一個方法來決定所要購買保留項目的正確 VM 大小。

- 參閱使用量檔案或使用量 API 中的 AdditionalInfo 欄位，判斷保留購買的正確 VM 大小。 請勿使用 [計量子類別目錄] 或 [產品] 欄位中的值，因為這些欄位不會區分 VM 的 S 和非 S 版本。
- 您也可以使用 PowerShell、Azure Resource Manager，或從 Azure 入口網站的 VM 詳細資料，取得正確的 VM 大小資訊。
...
4. 填寫必要欄位。 符合您選取之屬性的執行中 VM 執行個體符合取得保留項目折扣的資格。 取得折扣的 VM 執行個體實際數目取決於選取的範圍和數量。
    ...
    |VM 大小     |VM 執行個體的大小。|
    |已針對下列項目最佳化     |VM 執行個體大小彈性會將保留項目折扣套用至同一個 [VM 大小群組](https://aka.ms/RIVMGroups)中的其他 VM。 容量優先順序會保留資料中心容量供您的部署使用。 這可讓您更加確信您能夠在需要時啟動 VM 執行個體。 保留項目範圍是單一訂用帳戶時，才可使用容量優先順序。 |
    |詞彙        |一年或三年。|
