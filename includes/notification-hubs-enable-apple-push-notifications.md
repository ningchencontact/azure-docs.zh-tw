...
3. 選取您的 [使用者電子郵件地址]  和 [一般名稱]  ，確定已勾選 [儲存至磁碟]  ，然後按一下 [繼續]  。 請將 [CA 電子郵件地址]  欄位留空，因為它不是必要資訊。
...
2. 將新應用程式的下列三個欄位予以更新，然後按一下 [繼續]  ：
   ...
   * **套件組合識別碼**：在 [明確的應用程式識別碼]  區段之下，以[應用程式分發指南](https://help.apple.com/xcode/mac/current/#/dev91fe7130a) (英文) 中所提的 `<Organization Identifier>.<Product Name>` 形式輸入 [套件組合識別碼]  。 您使用的 [組織識別碼]  和 [產品名稱]  必須符合當您建立 XCode 專案時使用的組織識別碼和產品名稱。 在下列螢幕擷取畫面中，*NotificationHubs* 值會作為組織識別碼，*GetStarted* 會作為產品名稱。 確定此值符合您在 XCode 專案中使用的值，讓您使用正確的發行設定檔搭配 XCode。
...
4. 選取您先前匯出的 **.p12** 檔案。
5. 指定正確的**密碼**。
6. 選取 [沙箱]  模式。 只有在您想傳送推播通知給從市集購買應用程式的使用者時，才可使用 [生產]  模式。
