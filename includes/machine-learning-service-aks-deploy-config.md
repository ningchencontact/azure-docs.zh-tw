...
| JSON 實體 | 方法參數 | 說明 |
| ----- | ----- | ----- |
| `computeType` | NA | 計算目標。 若為 AKS，此值必須 `aks`。 |
...
| `authEnabled` | `auth_enabled` | 是否要啟用 web 服務的金鑰驗證。 `tokenAuthEnabled` 和 `authEnabled` 都無法 `True`。 預設值，`True`。 |
| `tokenAuthEnabled` | `token_auth_enabled` | 是否要啟用 web 服務的權杖驗證。 `tokenAuthEnabled` 和 `authEnabled` 都無法 `True`。 預設值，`False`。 |
| `containerResourceRequirements` | NA | CPU 和記憶體實體的容器。 |
| &emsp;&emsp;`cpu` | `cpu_cores` | 要為此 web 服務配置的 CPU 核心數目。 預設值，`0.1` |
