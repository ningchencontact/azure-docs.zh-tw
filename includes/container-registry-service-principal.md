...
如果您想要授`--role`與不同的許可權, 您可以選擇性地修改[az ad sp create-rbac][az-ad-sp-create-for-rbac]命令中的值。 如需角色的完整清單，請參閱 [ACR 角色和權限](https://github.com/Azure/acr/blob/master/docs/roles-and-permissions.md) \(英文\)。
...
下列腳本會使用[az role 指派 create][az-role-assignment-create]命令, 將*提取*許可權授與您在`SERVICE_PRINCIPAL_ID`變數中指定的服務主體。 如果您要授與不同層級的存取權，請調整 `--role` 值。
