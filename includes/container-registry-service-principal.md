...
(選擇性) 如果您要授與不同權限，則可修改 [az ad sp create-for-rbac][/cli/azure/ad/sp?view=azure-cli-latest#az-ad-sp-create-for-rbac] 命令中的 `--role` 值。 如需角色的完整清單，請參閱 [ACR 角色和權限](https://github.com/Azure/acr/blob/master/docs/roles-and-permissions.md) \(英文\)。
...
以下指令碼會使用 [az role assignment create][/cli/azure/role/assignment?view=azure-cli-latest#az-role-assignment-create] 命令，為您在 `SERVICE_PRINCIPAL_ID` 變數中指定的服務主體授與「提取」權限。 如果您要授與不同層級的存取權，請調整 `--role` 值。
