...
* M 系列提供最大 vCPU 計數 (最多 128 個 Vcpu) 和任何 VM 雲端中的最大記憶體 (最多 3.8 TiB)。  它很適合用於受益於高 vCPU 計數和大量記憶體的極大型資料庫或其他應用程式。
...
## <a name="esv3-series-sup1sup"></a>Esv3 數列<sup>1</sup>
...
| Standard_E32s_v3 <sup>2</sup> | 32     | 256         | 512            | 32             | 64,000 / 512 (800)                                                    | 51,200 / 768                              | 8 / 極高                             |
| Standard_E64s_v3 <sup>2</sup> | 64     | 432         | 864            | 32             | 128,000/1024 (1600)                                                   | 80,000 / 1200                             | 8 / 極高                             |

<sup>1</sup> Esv3 系列 VM 的功能 Intel® 超執行緒技術中<sup>2</sup>限制可用的核心大小 

## <a name="ev3-series-sup1sup"></a>Ev3 數列<sup>1</sup>
...
<sup>1</sup> Ev3 系列 VM 的功能 Intel® 超執行緒技術中
...
<sup>1</sup> M 系列 VM 的功能 Intel® 超執行緒技術中

<sup>2</sup> 64 個以上 vCPU 需要其中一個這些支援的客體作業系統： Windows Server 2016、 Ubuntu 16.04 LTS、 SLES 12 SP2 和 Red Hat Enterprise Linux 或具有 LIS 4.2.1 CentOS 7.3 
...
<sup>1</sup>數目，可能會受限於 （IOPS 或 MBps） 可能與 GS 系列 VM 的最大磁碟輸送量大小和的附加磁碟條狀配置。 如需詳細資訊，請參閱 [進階儲存體：Azure 虛擬機器工作負載適用的高效能儲存體](../articles/virtual-machines/windows/premium-storage.md)。 

<sup>2</sup>執行個體都專用於單一客戶的硬體隔離。
...
<sup>1</sup>數目，可能會受限於 （IOPS 或 MBps） 可能與 DSv2 系列 VM 的最大磁碟輸送量大小和的附加磁碟條狀配置。  如需詳細資訊，請參閱 [進階儲存體：Azure 虛擬機器工作負載適用的高效能儲存體](../articles/virtual-machines/windows/premium-storage.md)。

<sup>2</sup>執行個體是外掛式的節點，可確保您的 VM 是只有 VM 我們 Intel Haswell 節點上的。

<sup>3</sup> 25000 Mbps 含加速網路功能。
...
<sup>1</sup>執行個體是外掛式的節點，可確保您的 VM 是只有 VM 我們 Intel Haswell 節點上的。

<sup>2</sup> 25000 Mbps 含加速網路功能。
...
<sup>1</sup>數目，可能會受限於 （IOPS 或 MBps） 可能與 DS 系列 VM 的最大磁碟輸送量大小和的附加磁碟條狀配置。  如需詳細資訊，請參閱 [進階儲存體：Azure 虛擬機器工作負載適用的高效能儲存體](../articles/virtual-machines/windows/premium-storage.md)。
...
| 標準_D14 | 16        | 112         | 800            | 48000 / 750 / 375                                        | 64 / 64x500                       | 8 / 6000 - 8000 &#8224;                |
