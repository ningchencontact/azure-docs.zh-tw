...
- 位於 Azure VNet 中所部署虛擬機器設定內的集區，會自動配置額外的 Azure 網路資源。 在 VNet 中，每 50 個集區節點就需要下列資源：1 個網路安全性群組、1 個公用 IP 位址和 1 個負載平衡器。 在包含建立 Batch 集區時所提供虛擬網路的訂用帳戶中，其中的[配額](../articles/batch/batch-quota-limit.md)會限制這些資源。
