...
1. <a name="getnet"></a>取得叢集網路資源名稱。
    ...
    c. 选择“网络”节点，并记下群集网络名称。 請在 PowerShell 指令碼的 `$ClusterNetworkName` 變數中使用這個名稱。 下圖中的叢集網路名稱為**叢集網路 1**：
...
4. <a name="congroup"></a>為可用性群組設定 IP 資源。
...
6. <a name="listname"></a>讓用戶端存取點資源依存於 IP 位址。
...
7. <a name="setparam"></a>在 PowerShell 中設定叢集參數。
...
3. 複製 IP 位址的 [名稱]。 它可能是 `Cluster IP Address`。 

4. <a name="setwsfcparam"></a>在 PowerShell 中設定叢集參數。
