...
1. 為時間序列深入解析特定**容錯移轉執行個體**資料重新導向和流量。
1. 進行稽核和資料保留。

一般情況下，重複的 Time Series Insights 環境的最佳方式是建立備份的 Azure 區域中的第二個的 Time Series Insights 環境。 事件也會從您的主要事件來源傳送到這個次要的環境。 請確定您使用第二個且專用的取用者群組，並遵循該來源的商務災害復原指導方針 （在上面提供）。
...
* 使用您的第二個區域備份和復原所有的 Time Series Insights 的遙測和查詢資料。
