...
可以。 我們會根據每一筆交易向您收費。 如需詳細資訊，請參閱[價格頁面](https://azure.microsoft.com/pricing/details/storage)。
...
可以。 您可以使用 Azure 入口網站、PowerShell 或 Azure CLI 來匯出受控磁碟。
...
編號 目前的快照集功能會建立受控磁碟的完整複本。 不過，我們已規劃在未來支援增量快照集。
...
可以。 我們支援受控磁碟和非受控磁碟。 我們建議您使用受控磁碟來處理新的工作負載，並將您目前的工作負載移轉至受控磁碟。
...
**是否支援將現有 VM 擴展集 (VMSS) 從非受控磁碟自動移轉至受控磁碟？**

編號 您可以使用您舊 VMSS 中的映像搭配非受控磁碟，建立包含受控磁碟的新 VMSS。 
...
可以。 ASR Azure 對 Azure 的保護不支援具有受控磁碟的 VM。 它會受到 Q1 CY2018 結尾。 
...
編號 只有當 VM 未使用支援進階儲存體的大小系列時，您才能將標準資料磁碟連結至 VM。
